...
> "실시간"이라고 하면 곧 스트리밍이라고 여기기 쉽지만, 요구마다 뜻이 다릅니다. "실시간으로 해주세요"를 어떤 기준으로 읽을지, 이벤트 시간과 처리 시간이 왜 어긋나고 윈도우와 워터마크가 그 틈을 어떻게 다루는지, Lambda와 Kappa 중 무엇을 기본값으로 둘지를 가려냅니다.
...
- 이상 거래를 막아야 합니다 → **거래를 승인하기 전에** 판단이 끝나야 합니다 (수백 밀리초)
- 배달 기사 위치를 보여줍니다 → 몇 초 지연은 괜찮습니다
- 오늘 매출 현황판 → 5~10분이면 충분합니다
- 마케팅 팀 주간 리포트 → 어제까지 집계돼 있으면 됩니다

지연 요구가 밀리초인 경우와 하루인 경우에 같은 아키텍처를 쓸 이유는 없습니다. 그리고 실무에서 훨씬 흔한 실수는 **필요 없는 실시간성을 위해 감당 못 할 복잡도를 떠안는 것**입니다.

### 비유: 월말 정산과 편의점 POS

배치는 회계팀이 월말에 영수증을 모아 한꺼번에 정산하는 방식입니다. 시간은 걸리지만 그달 것이 다 모였으니 결과가 완전합니다. 스트리밍은 POS 단말이 결제될 때마다 즉시 매출을 올리는 방식입니다. 지금 이 순간의 숫자를 볼 수 있습니다. 다만 뒤늦게 들어온 취소 전표를 어떻게 반영할지는 정해야 합니다.

> **비유의 한계**: 회계와 달리 데이터 시스템은 배치와 스트리밍을 동시에 돌릴 수 있습니다. 같은 소스에서 둘을 함께 돌려 결과를 대조하기도 합니다. 그리고 배치라고 해서 "그달 것이 다 모였다"가 보장되지는 않습니다. 지연 도착 문제는 배치에도 있습니다.
...
스트리밍은 구간이 열려 있는 상태에서 계속 답을 갱신합니다. "지금까지의 3월 1일 매출"은 시간이 지나면서 계속 바뀝니다. 언제 "확정"으로 볼지는 **엔지니어가 정책으로 결정해야 합니다.** 4~5장의 윈도우와 워터마크가 존재하는 이유가 여기에 있습니다.

### 운영 복잡도 축의 실체

- 배치가 실패하면 → 고치고 그 구간을 다시 돌립니다. [02-airflow.md](./02-airflow.md)의 백필 그대로입니다.
- 스트리밍이 실패하면 → 어디까지 처리했는지(오프셋), 진행 중이던 집계 상태(윈도우별 카운트)를 복구해야 합니다. 체크포인트가 손상되면 복구가 까다롭고, 코드를 바꿔 배포할 때는 기존 상태와 호환되는지도 따져야 합니다.
...
2. **장애가 데이터를 왜곡한다** — 컨슈머가 30분 멈췄다가 재개하면, 그 30분치가 재개 시점의 버킷에 전부 몰려 가짜 스파이크가 생깁니다.
...
**집계 기준은 거의 항상 이벤트 시간이어야 합니다.** 처리 시간 기준이 맞는 경우는 "시스템 처리량 모니터링"처럼 우리 시스템 자체가 관심사일 때뿐입니다.

이벤트 시간을 쓰려면 전제가 하나 붙습니다. **이벤트를 만드는 쪽이 시각을 함께 실어 보내야 하고, 그 시계를 어느 정도 믿을 수 있어야 합니다.** 클라이언트 시계는 조작되거나 틀어질 수 있으므로, 서버 수신 시각을 함께 기록해 두고 비정상적으로 벗어난 값은 걸러 내는 방어가 필요합니다.
...
**워터마크(Watermark)는 "이벤트 시간 T 이전의 데이터는 이제 거의 다 도착했다고 보겠다"는 시스템의 추정치**입니다.
...
**"정확하면서 동시에 빠른" 설정은 없습니다.** 실제 지연 분포를 관측해서 정하는 것이 정석입니다. 이벤트 시간과 도착 시간의 차이를 히스토그램으로 그려 보고, 대다수 이벤트가 들어오는 지점을 기준으로 잡되 비용이 감당되는 선에서 여유를 줍니다.
...
> 실무에서 자주 쓰는 절충: **스트리밍으로 대략적인 실시간 값을 내보내되, 배치로 하루 뒤에 같은 구간을 재계산해 확정 값으로 덮어씁니다.** 실시간성은 스트리밍이, 정확성은 배치가 책임집니다. 이 발상이 다음 장의 Lambda 아키텍처입니다.
...
Lambda의 문제는 구조가 아닙니다. **같은 비즈니스 로직을 두 벌 유지해야 한다**는 것이 부담입니다.

- "활성 사용자"의 정의가 바뀌면 배치 코드와 스트리밍 코드를 둘 다 고쳐야 합니다
- 두 구현이 미묘하게 다르면 배치 결과와 스피드 결과가 어긋나고, 사용자는 대시보드 숫자가 어느 순간 갑자기 바뀌는 것을 봅니다
- 테스트도 두 벌, 배포도 두 벌, 장애 대응도 두 벌입니다
- 두 엔진이 서로 다른 프레임워크라면(예: 배치는 Spark, 스트리밍은 Flink) 인력의 학습 부담도 두 배가 됩니다
...
핵심 발상은 **"재처리는 별도 배치가 아니라 스트림을 처음부터 다시 재생하는 것"** 입니다. 로직을 바꿔야 하면 새 버전의 스트리밍 잡을 오프셋 처음부터 돌려 새 결과 테이블을 만듭니다. 다 따라잡으면 조회 대상을 바꿉니다. 로직이 한 벌이므로 Lambda의 불일치 문제가 사라집니다.
...
3. **스트리밍 엔진이 정확성을 보장해야 한다** — 이 조건은 최근 엔진들이 상당 부분 충족합니다. 상태 관리와 체크포인트, 이벤트 시간 처리가 성숙해진 것이 Kappa 논의가 가능해진 배경입니다.
...
> 선택 기준: **로직 변경이 잦고 두 코드베이스의 불일치가 실제로 문제가 되고 있으면 Kappa로 수렴할 가치가 있습니다.** 반대로 재처리 범위가 수년 단위이고 그 재계산이 드물다면, 배치로 대량 재계산하는 경로를 남겨 두는 편이 현실적입니다.
...
실제로 필요한 지연 요구가 "5분 이내"인 경우가 많습니다. 이 정도는 **스트리밍이 아니라 짧은 주기의 마이크로배치로 충분합니다.** 5분마다 도는 증분 배치는 스트리밍의 운영 복잡도 없이 5분 지연을 줍니다.

### 스트리밍으로 가는 조건

다음 중 하나에 해당하면 스트리밍이 정당화됩니다.

- 지연 자체가 비즈니스 가치입니다 — 이상 거래 차단, 실시간 재고 확인, 광고 입찰
- 사용자에게 즉각적인 반응을 보여야 합니다 — 알림, 실시간 위치
- 배치 주기를 줄일수록 오버헤드가 지배적이 됩니다 — 1분마다 전체 스캔을 반복하느니 증분 스트림이 쌉니다
...
**수집은 스트리밍으로 통일하고 변환은 요구 지연에 따라 나눕니다.** 대부분의 지표는 배치로 만들고, 정말 실시간이 필요한 소수의 지표만 스트리밍 잡으로 따로 뺍니다. 그러면 유지할 스트리밍 코드가 적어서 Lambda의 이중 구현 부담도 감당할 만한 수준으로 줄어듭니다.

또 하나, **스트리밍 결과와 배치 결과가 어긋날 때 어느 쪽이 정답인지 미리 정해 둬야 합니다.** 보통 배치를 정답으로 두고 스트리밍은 "잠정치"라고 명시합니다. 대시보드에 "실시간(잠정)" 표기를 넣는 사소한 조치가 수많은 문의를 줄입니다.
...
배치와 스트리밍의 진짜 차이는 속도가 아니라 "구간을 닫고 계산하느냐, 열어 둔 채 계속 갱신하느냐"입니다. 열어 두기로 한 순간 이벤트 시간·윈도우·워터마크로 "언제 마감할 것인가"를 직접 결정해야 하므로, 그 복잡도를 지불할 이유가 분명할 때만 스트리밍으로 갑니다.
