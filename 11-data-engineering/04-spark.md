...
> 한 대의 메모리를 넘는 데이터를 어떻게 나눠 처리하는지, 드라이버와 익스큐터가 각각 무슨 일을 하는지, 왜 RDD 대신 DataFrame을 쓰라고 하는지, 그리고 느린 Spark 잡의 원인이 대개 셔플과 스큐인 이유를 설명할 수 있게 된다.
...
로그 분석을 pandas로 시작한다. 하루치 500MB는 노트북에서 잘 돈다. 서비스가 커져 하루치가 3GB가 되고, 한 달치 90GB를 한 번에 읽으려는 순간 프로세스가 죽는다.

```python
df = pd.read_csv("s3://logs/2026-03/merged.csv")   # MemoryError
```

첫 번째 대응은 수직 확장이다. 메모리 512GB짜리 인스턴스를 띄운다. 이것도 방법이고 실제로 상당 부분을 커버한다. 하지만 한계가 명확하다. 살 수 있는 가장 큰 머신은 정해져 있는데 데이터는 계속 늘어나고, 대형 인스턴스는 같은 자원을 여러 대로 나눠 사는 것보다 단가가 비싸며, 메모리를 늘려도 코어 수가 그만큼 늘지는 않아 CPU가 병목으로 남는다. 무엇보다 3시간짜리 작업이 2시간 50분에 죽으면 전부 다시 해야 한다.

### 나누면 새 문제가 생긴다

"여러 대에 나눠서 하자"는 결론은 쉽다. 어려운 것은 그다음이다.
...
이걸 매번 손으로 짜는 것은 현실적이지 않다. **Spark는 이 네 가지를 프레임워크가 대신 처리하고, 개발자에게는 "한 대에서 돌리는 것처럼 보이는 API"를 주는 분산 처리 엔진이다.**
...
전국에서 온 우편물 1000만 통을 지역별로 세는 일을 생각해 보자. 혼자 하면 오래 걸리니 100명이 나눠 센다. 그런데 각자 손에 든 뭉치에는 모든 지역의 우편물이 섞여 있다. 지역별 합계를 내려면 **모두가 서로에게 우편물을 던져 지역별로 재배치하는 과정**이 필요하다. 이 재배치가 Spark의 셔플(shuffle)이고, 실제로 가장 오래 걸리는 단계다.

> **비유의 한계**: 실제 분류 센터에서 우편물을 잃어버리면 끝이지만, Spark는 "이 데이터가 어떤 연산으로 만들어졌는지"를 계보(lineage)로 기억하기 때문에 노드가 죽어도 해당 조각만 다시 계산해 복구한다.
...
"파티션 하나 = 태스크 하나"라는 대응이 핵심이다. 파티션이 200개면 태스크 200개가 만들어지고, 익스큐터의 코어 수만큼 동시에 실행된다.
...
**왜 문제인가**: `collect()`는 분산된 결과를 전부 드라이버 한 대의 메모리에 모은다. 데이터가 크면 드라이버가 OOM으로 죽고, 죽는 것은 애플리케이션 전체다. 게다가 이후 `for` 루프는 단일 스레드라 분산 처리의 이점이 완전히 사라진다.

**드라이버에 데이터를 모으는 것은 최종 결과가 확실히 작을 때만** 한다. 그렇지 않으면 결과도 분산 상태로 스토리지에 쓴다.
...
RDD(Resilient Distributed Dataset)는 Spark의 최초 추상화로, 분산된 불변 객체 컬렉션이다.
...
여기서 Spark가 아는 것은 "사용자가 준 함수를 각 요소에 적용하라"뿐이다. **람다 안에서 무슨 일이 벌어지는지 엔진은 모른다.** 그래서 최적화할 여지가 없다. 필터를 먼저 하면 훨씬 빠르다는 것을 엔진이 알아채도 순서를 바꿔 줄 수 없다. 필요한 컬럼이 두 개뿐이라는 것도 모르니 파일에서 전부 읽는다.
...
여기에 Tungsten 실행 엔진이 붙어 JVM 객체 오버헤드를 피하는 바이너리 포맷과 코드 생성으로 CPU 효율을 높인다. **DataFrame이 빠른 이유는 "메모리를 써서"가 아니라 "엔진이 무엇을 하려는지 알기 때문"이다.**
...
1. **직렬화 비용** — PySpark의 파이썬 UDF는 JVM에 있는 데이터를 별도의 파이썬 워커 프로세스로 보내고 결과를 다시 받아 온다. 이때 값이 **행 단위로 직렬화·역직렬화되고 함수도 행마다 호출**되므로, 행 수에 비례해 오버헤드가 쌓인다.
2. **옵티마이저의 블랙박스** — Catalyst는 UDF 안을 볼 수 없어 그 지점 이후의 최적화를 포기한다. 조건 푸시다운 같은 것이 막힌다.
...
Pandas UDF는 Apache Arrow를 통해 데이터를 배치로 주고받으므로 행마다 왕복하는 비용이 크게 줄어든다. 그래도 **내장 함수로 표현할 수 있으면 내장 함수가 언제나 최선**이다.
...
지연 실행의 이점은 **전체 계획을 다 본 뒤에 최적화할 수 있다**는 것이다. 각 단계를 즉시 실행하면 "이 필터를 먼저 했으면 뒤 조인이 10분의 1이었다"는 사실을 알아도 이미 늦다.
...
**왜 문제인가**: 지연 실행이라는 것은 뒤집어 말하면 **Action마다 계보를 따라 처음부터 다시 계산한다**는 뜻이다. `filtered`는 결과를 들고 있는 변수가 아니라 계산 방법의 설명서다.

단, `cache()`는 익스큐터 메모리를 점유하므로 **두 번 이상 쓰는 중간 결과에만** 건다. 한 번만 쓰는 데이터에 캐시를 걸면 메모리만 잡아먹고 이득이 없다.
...
메모리 연산이 나노초 단위인 데 비해 디스크와 네트워크는 그보다 몇 자릿수 느리다. 게다가 셔플은 **모든 익스큐터가 모든 익스큐터와 통신**하는 형태라 노드가 늘수록 연결 수가 급증한다. Spark 잡이 느리다면 원인은 십중팔구 셔플이거나 다음 절의 스큐다.
...
Catalyst가 상당 부분 알아서 밀어 주지만, 명시적으로 쓰는 편이 확실하고 읽기도 좋다.

**2. 작은 테이블은 브로드캐스트 조인으로 바꾼다**

한쪽 테이블이 충분히 작으면 셔플 대신 **그 테이블을 모든 익스큐터에 복사**해 로컬 조인을 한다. 큰 쪽은 움직이지 않으므로 셔플이 아예 사라진다.
...
Spark는 통계상 작다고 판단되면 자동으로 브로드캐스트하며, 그 기준은 `spark.sql.autoBroadcastJoinThreshold`로 조정한다. 통계가 없어 자동 판단이 빗나갈 때 위처럼 명시하면 된다. 반대로 **크지 않다고 착각한 테이블을 브로드캐스트하면 드라이버와 익스큐터가 함께 OOM**으로 죽으므로 크기 확인이 전제다.

**3. 집계를 먼저, 조인을 나중에** — 행 수를 줄이는 연산을 앞으로 당기면 셔플로 오가는 데이터량이 줄어든다.

**4. 반복 조인할 키가 정해져 있으면 미리 나눠 저장한다** — 같은 키로 매일 조인한다면, 저장 시점에 그 키로 파티셔닝하거나 버킷팅해 두면 조인 때 셔플을 건너뛸 수 있다.

**5. 셔플 파티션 수를 데이터 크기에 맞춘다**

셔플 후 파티션 수는 `spark.sql.shuffle.partitions`로 정해지고 기본값은 200이다. 데이터가 작은데 200개면 태스크마다 처리량이 미미해 스케줄링 오버헤드만 커지고, 데이터가 아주 큰데 200개면 태스크 하나가 감당할 크기를 넘겨 디스크로 흘러넘친다. Spark 3.x의 AQE(Adaptive Query Execution)는 실행 중 실제 통계를 보고 파티션 수를 조정해 이 문제를 상당 부분 자동으로 완화한다.
...
Spark UI에서 Stage를 보면 이런 모습이 나온다.
...
전체 Stage는 가장 느린 태스크가 끝나야 완료된다. 익스큐터 99%가 놀면서 하나를 기다린다. 원인은 키 분포가 균등하지 않기 때문이다. `groupBy("user_id")`를 하는데 특정 봇 계정이 전체 로그의 30%를 차지한다거나, `user_id`가 NULL인 행이 수천만 건이라면 그 키를 맡은 파티션만 거대해진다.

### 대응

**1. 원인 키를 먼저 찾는다** — 상위 몇 개 키가 압도적이면 스큐가 맞다.

```python
df.groupBy("user_id").count().orderBy("count", ascending=False).show(20)
```

**2. NULL이나 의미 없는 키는 분리한다** — NULL 키끼리는 조인해도 의미가 없는 경우가 많다. 미리 걸러 내거나 따로 처리한다.
...
핫 키 하나가 10개 파티션으로 흩어지므로 부하가 나뉜다. 작은 쪽을 복제하는 비용이 있어 만능은 아니고, 스큐가 확인된 뒤에 쓰는 처방이다.

**4. AQE의 스큐 조인 처리에 맡긴다**

Spark 3.x의 AQE는 실행 중 비정상적으로 큰 파티션을 감지해 자동으로 분할한다. 먼저 AQE가 켜져 있는지 확인하고, 그래도 남는 경우에 솔팅을 고려하는 순서가 합리적이다.
...
Spark가 빠른 이유를 "메모리를 쓰니까"로만 설명하면 절반이다. **중간 결과를 매번 디스크에 쓰지 않는다는 점, 그리고 전체 계획을 옵티마이저가 다시 쓴다는 점**이 함께 작용한다. 셔플할 때는 Spark도 디스크에 쓴다.

> 정리: 오늘 새로 배치 처리를 만든다면 MapReduce를 직접 쓸 일은 거의 없다. 다만 "왜 중간 결과 저장이 비싼가"라는 감각은 5장의 셔플 이해로 이어진다.

---

## 8. 배치를 넘어: Structured Streaming

Spark는 스트리밍도 같은 DataFrame API로 다룬다.
...
핵심 발상은 **끝없이 행이 추가되는 무한 테이블**로 스트림을 바라보는 것이다. 배치 쿼리와 같은 문법으로 쓰고, 엔진이 이를 작은 배치들의 연속으로 실행한다.

`checkpointLocation`은 선택이 아니다. 진행 위치와 상태를 저장해 재시작 시 이어서 처리하게 하고, 싱크와 함께 동작해 결과의 정확성을 유지한다. **체크포인트 없이 프로덕션 스트리밍을 돌리면 재시작이 곧 데이터 사고다.** 이벤트 시간, 워터마크, 윈도우처럼 스트리밍 고유의 개념은 [05-batch-vs-streaming.md](./05-batch-vs-streaming.md)에서 다룬다.

---

## 9. 실무에서는

- **Spark를 직접 클러스터로 운영하는 조직은 줄고 있다.** Databricks, AWS EMR, Google Dataproc 같은 관리형 환경에서 필요할 때 클러스터를 띄우고 잡이 끝나면 내리는 형태가 흔하다. Kubernetes 위에서 익스큐터를 파드로 띄우는 방식도 자리를 잡았다.
- **파일 포맷 선택이 성능의 절반이다.** CSV/JSON 대신 Parquet 같은 컬럼 지향 포맷을 쓰면 컬럼 프루닝과 압축이 동작해 읽는 양 자체가 줄어든다. 여기에 자주 필터링하는 컬럼(보통 날짜)으로 파티션 디렉터리를 나누면 아예 파일을 건너뛴다. 반대로 스트리밍이나 잦은 배치가 수만 개의 작은 파일을 만들면 파일 목록 조회와 태스크 생성 오버헤드가 실제 처리보다 커지므로, 주기적으로 병합(compaction)하는 잡을 함께 운영한다.
- **성능 튜닝의 출발점은 항상 Spark UI다.** 어느 Stage가 오래 걸리는지, 그 Stage의 태스크 시간 분포가 균등한지(스큐), 셔플 읽기/쓰기 양이 얼마인지를 먼저 본다. 설정값을 감으로 바꾸기 전에 병목을 확인하는 순서가 중요하다.
- **Airflow가 Spark 잡을 제출하고 기다리는 조합이 표준이다.** [02-airflow.md](./02-airflow.md)에서 말한 대로 Airflow 워커에서 직접 처리하지 않고, Spark에 제출한 뒤 완료를 기다리는 태스크로 구성한다.
...
- 꼬리 질문: "그럼 메모리가 부족하면요?" → 디스크로 흘러넘치며(spill) 동작은 계속되지만 속도가 크게 떨어진다고 답한다.
...
- 꼬리 질문: "무조건 cache를 걸면 되나요?" → 익스큐터 메모리를 점유하므로 두 번 이상 쓰는 결과에만 걸고, 다 쓴 뒤 `unpersist()`한다고 답한다.
...
- 꼬리 질문: "브로드캐스트 조인의 위험은요?" → 작다고 판단한 테이블이 실제로 크면 모든 익스큐터로 복사되면서 OOM이 나므로 크기 확인이 전제라고 답한다.
...
- 꼬리 질문: "셔플을 아예 없앨 수 있나요?" → 집계와 조인이 필요한 이상 완전히 없앨 수는 없고, 브로드캐스트 조인이나 사전 파티셔닝으로 특정 셔플을 건너뛰거나 셔플 대상 데이터량을 줄이는 것이 목표라고 답한다.
...
Spark는 데이터를 파티션으로 나눠 익스큐터에 분산 처리시키고 실패하면 계보로 되살리는 엔진이며, DataFrame이 빠른 이유는 메모리가 아니라 옵티마이저가 의도를 알기 때문이고, 느린 잡의 원인은 거의 항상 셔플 아니면 스큐다.
