...
> 데이터가 한 대의 메모리보다 크면 어떻게 나눠 처리할까요? 드라이버와 익스큐터는 각각 무슨 일을 하고, 왜 RDD 대신 DataFrame을 쓰라고 할까요? 느린 Spark 잡의 원인이 대개 셔플과 스큐인 이유까지 이 문서에서 다룹니다.
...
첫 번째 대응은 수직 확장입니다. 메모리 512GB짜리 인스턴스를 띄웁니다. 이것도 방법이고 실제로 상당 부분을 커버합니다. 하지만 한계가 명확합니다. 살 수 있는 가장 큰 머신은 정해져 있는데 데이터는 계속 늘어납니다. 대형 인스턴스는 같은 자원을 여러 대로 나눠 사는 것보다 단가가 비싸고, 메모리를 늘려도 코어 수가 그만큼 늘지는 않아 CPU가 병목으로 남습니다. 무엇보다 3시간짜리 작업이 2시간 50분에 죽으면 전부 다시 해야 합니다.
...
전국에서 온 우편물 1000만 통을 지역별로 세는 일을 생각해 보겠습니다. 혼자 하면 오래 걸리니 100명이 나눠 셉니다. 그런데 각자 손에 든 뭉치에는 모든 지역의 우편물이 섞여 있습니다. 지역별 합계를 내려면 **모두가 서로에게 우편물을 던져 지역별로 재배치하는 과정**이 필요합니다. 이 재배치가 Spark의 셔플(shuffle)이고, 실제로 가장 오래 걸리는 단계입니다.

> **비유의 한계**: 실제 분류 센터에서는 우편물을 잃어버리면 끝입니다. Spark는 다릅니다. "이 데이터가 어떤 연산으로 만들어졌는지"를 계보(lineage)로 기억하기 때문에, 노드가 죽어도 해당 조각만 다시 계산해 복구합니다.
...
RDD(Resilient Distributed Dataset)는 Spark의 최초 추상화입니다. 분산된 불변 객체 컬렉션입니다.
...
- **컬럼 프루닝(column pruning)** — 필요한 컬럼만 읽습니다. Parquet 같은 컬럼 지향 포맷에서 효과가 극적입니다.
- **조건 푸시다운(predicate pushdown)** — 필터를 데이터 소스 가까이 내려보내 애초에 적게 읽습니다.
- **연산 순서 재배치** — 조인 전에 필터를 적용해 조인 대상을 줄입니다.
...
2. **옵티마이저의 블랙박스** — Catalyst는 UDF 안을 볼 수 없어 UDF가 관여하는 부분은 최적화하지 못합니다. 조건 푸시다운 같은 것이 막힙니다.
...
Pandas UDF는 Apache Arrow를 써서 데이터를 배치로 주고받으므로 행마다 왕복하는 비용이 크게 줄어듭니다. 그래도 최선은 내장 함수입니다. 내장 함수로 표현할 수 있다면 언제나 그렇습니다.
...
Spark는 데이터를 파티션으로 나눠 익스큐터에 분산 처리시키는 엔진이고, 실패한 조각은 계보로 되살립니다. DataFrame이 빠른 것은 옵티마이저가 의도를 알기 때문입니다. 느린 잡의 원인은 거의 항상 셔플 아니면 스큐입니다.
