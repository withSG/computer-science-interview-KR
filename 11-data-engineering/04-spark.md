...
> 한 대의 메모리를 넘는 데이터를 어떻게 나눠 처리하는지, 드라이버와 익스큐터가 각각 무슨 일을 하는지, 왜 RDD 대신 DataFrame을 쓰라고 하는지, 그리고 느린 Spark 잡의 원인이 대개 셔플과 스큐인 이유를 설명할 수 있게 됩니다.
...
"여러 대에 나눠서 하자"는 결론은 쉽습니다. 어려운 것은 그다음입니다.
...
이걸 매번 손으로 짜는 것은 현실적이지 않습니다. **Spark는 이 네 가지를 프레임워크가 대신 처리하고, 개발자에게는 "한 대에서 돌리는 것처럼 보이는 API"를 주는 분산 처리 엔진이다.**
...
**왜 문제인가**: `collect()`는 분산된 결과를 전부 드라이버 한 대의 메모리에 모읍니다. 데이터가 크면 드라이버가 OOM으로 죽고, 죽는 것은 애플리케이션 전체입니다. 게다가 이후 `for` 루프는 단일 스레드라 분산 처리의 이점이 완전히 사라집니다.
...
여기서 Spark가 아는 것은 "사용자가 준 함수를 각 요소에 적용하라"뿐입니다. **람다 안에서 무슨 일이 벌어지는지 엔진은 모른다.** 그래서 최적화할 여지가 없습니다. 필터를 먼저 하면 훨씬 빠르다는 것을 엔진이 알아채도 순서를 바꿔 줄 수 없습니다. 필요한 컬럼이 두 개뿐이라는 것도 모르니 파일에서 전부 읽습니다.
...
여기에 Tungsten 실행 엔진이 붙어 JVM 객체 오버헤드를 피하는 바이너리 포맷과 코드 생성으로 CPU 효율을 높입니다. **DataFrame이 빠른 이유는 "메모리를 써서"가 아니라 "엔진이 무엇을 하려는지 알기 때문"이다.**
...
Pandas UDF는 Apache Arrow를 통해 데이터를 배치로 주고받으므로 행마다 왕복하는 비용이 크게 줄어듭니다. 그래도 **내장 함수로 표현할 수 있으면 내장 함수가 언제나 최선**입니다.
...
지연 실행의 이점은 **전체 계획을 다 본 뒤에 최적화할 수 있다**는 것입니다. 각 단계를 즉시 실행하면 "이 필터를 먼저 했으면 뒤 조인이 10분의 1이었다"는 사실을 알아도 이미 늦습니다.
...
**Stage 경계는 정확히 이 wide 변환에서 생긴다.** Spark UI에서 Stage가 여러 개면 그만큼 셔플이 있었다는 뜻입니다.
...
**왜 문제인가**: 지연 실행이라는 것은 뒤집어 말하면 **Action마다 계보를 따라 처음부터 다시 계산한다**는 뜻입니다. `filtered`는 결과를 들고 있는 변수가 아니라 계산 방법의 설명서입니다.
...
**3. 집계를 먼저, 조인을 나중에** — 행 수를 줄이는 연산을 앞으로 당기면 셔플로 오가는 데이터량이 줄어듭니다.

**4. 반복 조인할 키가 정해져 있으면 미리 나눠 저장한다** — 같은 키로 매일 조인한다면, 저장 시점에 그 키로 파티셔닝하거나 버킷팅해 두면 조인 때 셔플을 건너뛸 수 있습니다.
...
전체 Stage는 가장 느린 태스크가 끝나야 완료됩니다. 익스큐터 99%가 놀면서 하나를 기다립니다. 원인은 키 분포가 균등하지 않기 때문입니다. `groupBy("user_id")`를 하는데 특정 봇 계정이 전체 로그의 30%를 차지한다거나, `user_id`가 NULL인 행이 수천만 건이라면 그 키를 맡은 파티션만 거대해집니다.

### 대응

**1. 원인 키를 먼저 찾는다** — 상위 몇 개 키가 압도적이면 스큐가 맞습니다.

```python
df.groupBy("user_id").count().orderBy("count", ascending=False).show(20)
```

**2. NULL이나 의미 없는 키는 분리한다** — NULL 키끼리는 조인해도 의미가 없는 경우가 많습니다. 미리 걸러 내거나 따로 처리합니다.
...
핵심 발상은 **끝없이 행이 추가되는 무한 테이블**로 스트림을 바라보는 것입니다. 배치 쿼리와 같은 문법으로 쓰고, 엔진이 이를 작은 배치들의 연속으로 실행합니다.

`checkpointLocation`은 선택이 아닙니다. 진행 위치와 상태를 저장해 재시작 시 이어서 처리하게 하고, 싱크와 함께 동작해 결과의 정확성을 유지합니다. **체크포인트 없이 프로덕션 스트리밍을 돌리면 재시작이 곧 데이터 사고다.** 이벤트 시간, 워터마크, 윈도우처럼 스트리밍 고유의 개념은 [05-batch-vs-streaming.md](./05-batch-vs-streaming.md)에서 다룹니다.

---

## 9. 실무에서는

- **Spark를 직접 클러스터로 운영하는 조직은 줄고 있다.** Databricks, AWS EMR, Google Dataproc 같은 관리형 환경에서 필요할 때 클러스터를 띄우고 잡이 끝나면 내리는 형태가 흔합니다. Kubernetes 위에서 익스큐터를 파드로 띄우는 방식도 자리를 잡았습니다.
- **파일 포맷 선택이 성능의 절반이다.** CSV/JSON 대신 Parquet 같은 컬럼 지향 포맷을 쓰면 컬럼 프루닝과 압축이 동작해 읽는 양 자체가 줄어듭니다. 여기에 자주 필터링하는 컬럼(보통 날짜)으로 파티션 디렉터리를 나누면 아예 파일을 건너뜁니다. 반대로 스트리밍이나 잦은 배치가 수만 개의 작은 파일을 만들면 파일 목록 조회와 태스크 생성 오버헤드가 실제 처리보다 커지므로, 주기적으로 병합(compaction)하는 잡을 함께 운영합니다.
- **성능 튜닝의 출발점은 항상 Spark UI다.** 어느 Stage가 오래 걸리는지, 그 Stage의 태스크 시간 분포가 균등한지(스큐), 셔플 읽기/쓰기 양이 얼마인지를 먼저 봅니다. 설정값을 감으로 바꾸기 전에 병목을 확인하는 순서가 중요합니다.
- **Airflow가 Spark 잡을 제출하고 기다리는 조합이 표준이다.** [02-airflow.md](./02-airflow.md)에서 말한 대로 Airflow 워커에서 직접 처리하지 않고, Spark에 제출한 뒤 완료를 기다리는 태스크로 구성합니다.
...
Spark는 데이터를 파티션으로 나눠 익스큐터에 분산 처리시키고 실패하면 계보로 되살리는 엔진이며, DataFrame이 빠른 이유는 메모리가 아니라 옵티마이저가 의도를 알기 때문이고, 느린 잡의 원인은 거의 항상 셔플 아니면 스큐입니다.
