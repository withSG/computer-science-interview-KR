...
정수장은 강, 지하수, 저수지에서 물을 끌어와 침전·여과·소독을 거쳐 배수지에 담아 둡니다. 각 가정은 수도꼭지만 틀면 그 물을 마십니다. 데이터 파이프라인이 하는 일이 똑같습니다. 원수를 여러 곳에서 취수(수집)하고 정제(변환)해서 배수지(웨어하우스)에 담고 분석가는 SQL이라는 수도꼭지만 틀면 됩니다.
...
**처리 대상 구간은 오케스트레이터가 정해서 넘겨주는 값이어야 합니다.** 태스크는 "언제 실행되는지"를 몰라야 하고, "어느 구간을 처리하는지"만 알아야 합니다. 이 원칙 하나가 백필과 재시도를 모두 가능하게 만듭니다.
...
데이터 레이크는 "일단 다 넣어라"는 철학이라 진입 장벽이 낮습니다. 대가는 분명합니다. 스키마를 강제하지 않으니 누가 어떤 포맷으로 넣었는지 알 수 없고 트랜잭션이 없으니 쓰는 도중에 읽으면 반쯤 쓰인 파일이 딸려 오고 파일을 지우고 다시 쓰는 동안에는 쿼리가 그대로 실패합니다. 1년쯤 지나면 아무도 어떤 경로의 데이터가 신뢰할 만한지 모릅니다. 이 상태를 **데이터 늪(data swamp)** 이라고 부릅니다.
...
- [02-airflow.md](./02-airflow.md) - 오케스트레이터. 파이프라인의 실행 순서·재시도·백필을 여기서 관리한다
- [03-kafka.md](./03-kafka.md) - 이벤트 로그. 수집 단계의 실시간 이벤트와 CDC가 이 위로 흘러간다
- [04-spark.md](./04-spark.md) - 분산 처리 엔진 — 단일 머신을 넘는 규모의 변환 단계를 담당한다
