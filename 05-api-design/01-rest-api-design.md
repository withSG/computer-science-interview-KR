...
> URI에 동사를 쓰면 왜 안 되는지, 400과 422를 무엇으로 가르는지, 그리고 "우리 API는 RESTful합니다"라는 말이 실제로 어디까지 참인지 설명할 수 있게 된다.
...
- [HTTP와 HTTPS](../01-computer-science-fundamentals/network/02-http-https.md) — 캐시 헤더와 메서드 의미를 다룬다
...
REST 이전, 그리고 지금도 규칙이 없는 팀의 API는 이런 모습이다.
...
전부 동작한다. 문제는 **새 기능을 붙일 때마다 "이번엔 어떻게 이름 짓지?"를 매번 고민해야 한다**는 것이다. 그 결과:

1. **예측 불가** — 클라이언트 개발자가 "게시글 삭제는 어떻게 부르죠?"를 매번 물어봐야 한다. 문서를 안 보면 아무것도 못 한다.
2. **중간 계층이 무력해진다** — `GET /deleteUserById/1`은 GET이라서 프록시나 브라우저가 마음대로 캐싱하거나 미리 요청(prefetch)해도 된다고 판단한다. 그러다 데이터가 지워진다.
3. **재시도 판단 불가** — 네트워크가 끊겼을 때 `POST /user_update`를 다시 보내도 되는지 요청 이름만 봐서는 알 수 없다.

### REST가 제안한 것

REST(REpresentational State Transfer)는 Roy Fielding이 2000년 박사 논문에서 정리한 **아키텍처 스타일**이다. 프로토콜도 아니고 표준 스펙도 아니다. "웹이 이렇게 커질 수 있었던 이유가 뭘까"를 역으로 분석해 얻은 제약조건의 모음에 가깝다.

핵심 아이디어는 하나다. **이름은 자원(명사)으로 짓고, 행위는 이미 HTTP에 정의된 메서드로 표현하라.** 그러면 위 세 문제가 한 번에 풀린다. `DELETE /users/1`은 이름을 몰라도 뜻이 통하고, 프록시가 GET만 캐싱하면 되고, DELETE는 여러 번 보내도 안전하다.

> 비유: 도서관의 청구기호. 책마다 담당 사서가 자기 마음대로 이름을 붙이면 아무도 책을 못 찾는다. 청구기호 체계를 정해두면 처음 온 사람도 규칙만 알면 어느 책이든 찾아간다.
> **비유의 한계**: 청구기호는 위치를 알려주지만 URI는 위치가 아니라 **식별자**다. `/users/1`이 어느 서버 어느 DB에 있는지는 클라이언트가 알 필요도 없고 알 수도 없다.
...
> 표 요약: 면접에서 "RESTful하게 설계했나요?"라는 질문은 사실상 **Uniform Interface(URI·메서드·상태 코드)와 Stateless를 얼마나 지켰나**를 묻는 것이다. 나머지 넷은 개념만 알면 충분하다.
...
4. **소문자 + 하이픈**. 언더스코어는 링크에 걸리는 밑줄과 겹쳐 잘 안 보인다. 대문자를 피하는 이유는 URI 경로가 명세상(RFC 3986) 대소문자를 구분하는 자리인데도 서버·프레임워크에 따라 같은 것으로 취급하기도 해서, 어느 쪽으로 동작할지 예측이 안 되기 때문이다
5. **확장자를 쓰지 않는다.** 포맷은 `Accept` 헤더로 협상한다
...
| 나쁜 예 | 무엇이 문제인가 | 좋은 예 |
|--------|---------------|--------|
| `GET /getUsers` | 행위가 URI에 있다. GET이 이미 "조회"를 뜻하는데 중복 | `GET /users` |
| `POST /createUser` | 위와 동일. POST가 이미 생성을 뜻한다 | `POST /users` |
| `GET /user/delete/1` | GET으로 상태를 바꾼다. 프록시·브라우저가 멋대로 호출할 수 있다 | `DELETE /users/1` |
...
| `GET /users/1/orders/2/items/3/options` | 계층이 너무 깊다. 주문 항목 3의 옵션을 보려고 상위 경로를 전부 알아야 한다 | `GET /order-items/3/options` |
| `GET /users?action=delete&id=1` | 쿼리 파라미터에 행위를 담았다 | `DELETE /users/1` |
| `GET /postList` | 복수형이 아니라 컬렉션임이 드러나지 않는다 | `GET /posts` |

> 표 요약: 잘못된 URI의 90%는 **동사가 URI에 들어간 경우**와 **메서드 의미를 무시한 경우** 둘 중 하나다. 이 둘만 걸러도 대부분 해결된다.

### 명사로 안 떨어지는 행위는 어떻게 하나

"비밀번호 재설정", "결제 취소", "장바구니 비우기" 같은 것들이 있다. 두 가지 접근이 있다.
...
②는 REST 원칙에서 벗어나지만, 마땅한 명사가 없는 행위에까지 억지로 이름을 지어내 팀원 누구도 뜻을 짐작하지 못하는 URI를 만드는 것보다는 낫다. **일관성 있게 소수의 예외만** 두는 것이 실무 타협점이다.

---

## 4. HTTP 메서드: 안전성과 멱등성

두 성질을 헷갈리는 경우가 많은데 묻는 것이 다르다.

- **안전성(Safe)**: 이 요청이 서버의 상태를 바꾸는가? 안 바꾸면 Safe.
- **멱등성(Idempotent)**: 같은 요청을 여러 번 보냈을 때 **최종 상태**가 한 번 보낸 것과 같은가?

Safe한 메서드는 전부 멱등이다(상태를 안 바꾸니 당연하다). 반대는 성립하지 않는다.
...
> 표 요약: **멱등이면 네트워크 실패 시 그냥 재시도해도 된다.** POST와 PATCH만 재시도 전략을 따로 고민하면 된다는 뜻이라, 이 표는 클라이언트 재시도 설계의 근거가 된다.

### DELETE가 멱등인 이유 — 응답 코드가 달라도 멱등이다

첫 `DELETE /users/1`은 204, 두 번째는 404가 나온다. "결과가 다른데 멱등인가?" 하는 의문이 여기서 생긴다. 멱등성이 보장하는 것은 **응답이 같다**가 아니라 **서버의 상태가 같다**는 것이다. 두 요청 모두 끝난 뒤 "1번 유저가 없다"는 상태는 동일하므로 멱등이다.
...
즉 PATCH의 멱등성은 **메서드가 아니라 본문 설계에 달려 있다**. 스펙이 "PATCH는 멱등이 아니다"라고 규정하는 이유는 멱등하지 않은 본문을 허용하기 때문이지, 멱등하게 쓸 수 없다는 뜻이 아니다.
...
**왜 문제인가**: GET은 안전하다는 계약을 깬다. 그 계약을 믿는 주체가 한둘이 아니다. 브라우저는 링크를 미리 가져올(prefetch) 수 있고, CDN은 응답을 캐싱해 다음 사람에게 그대로 내줄 수 있고, 크롤러는 페이지의 모든 링크를 밟아본다. 사내 위키의 링크 미리보기 봇이 게시글 전체에 좋아요를 누르는 사고가 실제로 이 패턴에서 나온다.
...
좋아요를 "자원"으로 보고 생성/삭제로 표현하면 의미도 맞고 안전성 계약도 지켜진다.
...
서버는 이 키를 저장소(주로 Redis)에 남긴다. 같은 키가 다시 오면 **처리하지 않고 처음 저장해둔 응답을 그대로 돌려준다.** 클라이언트가 지켜야 할 핵심 규칙은 하나다. **재시도할 때 키를 새로 만들지 않는다.** 새 UUID를 만들면 서버 입장에서는 완전히 다른 요청이라 그대로 두 번 결제된다.

Stripe, 토스페이먼츠 등 결제 API가 이 방식을 쓴다. HTTP 표준에 들어간 헤더는 아니고 업계 관례에 가깝지만, 이름은 대체로 `Idempotency-Key`로 통일되어 있다.
...
> 표 요약: 4xx는 "원인이 요청 쪽에 있다", 5xx는 "원인이 서버 쪽에 있다"로 갈린다. 헷갈리면 **요청을 고쳐야만 통과하는가**를 따져보면 된다. 고쳐야 통과하면 4xx, 서버 사정만 풀리면 같은 요청이 그대로 통과하면 5xx다. 429는 원인이 요청 쪽(너무 많이 보냄)이라 4xx이면서도 시간이 지나면 그대로 통과한다는 점에서 예외적이라, `Retry-After`로 언제 다시 오면 되는지를 함께 알려준다.
...
보안 관점에서 403 대신 404를 내는 선택도 있다. 남의 비공개 게시글에 403을 주면 "그 ID의 글이 존재한다"는 정보가 새기 때문이다. 존재 자체를 숨겨야 하는 자원이면 404가 낫다.
...
**왜 문제인가**: HTTP 계층은 이 응답을 성공으로 본다. 그래서 (1) 모니터링 대시보드의 에러율이 0%로 나와 장애를 놓치고, (2) CDN이 에러 응답을 캐싱해 다른 사용자에게도 뿌리고, (3) 클라이언트의 HTTP 라이브러리가 자동으로 예외를 던져주지 못해 모든 호출부에서 `if (!res.success)`를 손으로 검사해야 한다. 한 군데만 빼먹으면 에러 응답을 정상 데이터로 취급한다.
...
에러 포맷이 엔드포인트마다 다르면 클라이언트는 엔드포인트 수만큼 에러 처리 코드를 짜야 한다. 그래서 **하나의 포맷을 정하고 전역 예외 핸들러에서만 만들도록** 강제하는 것이 정석이다.

표준 포맷으로 `application/problem+json`(Problem Details for HTTP APIs, RFC 7807 → RFC 9457)이 있다. 필드는 `type`, `title`, `status`, `detail`, `instance` 다섯 개가 기본이고 확장 필드를 자유롭게 추가할 수 있다.

Spring에서는 전역 핸들러 하나로 모으면 된다.
...
에러 응답에 담아야 할 것과 담으면 안 되는 것도 갈린다.

- 담는다: 기계가 분기할 수 있는 **에러 코드 문자열**, 사람이 읽을 메시지, 어느 필드가 틀렸는지, 추적용 요청 ID
- 담지 않는다: 스택 트레이스, SQL 문, 내부 서버 호스트명, 예외 클래스 전체 경로 — 공격자에게 내부 구조를 알려주는 정보다

에러 코드를 문자열로 두는 이유는 상태 코드만으로는 구분이 안 되기 때문이다. 400 하나로 "필수값 누락"과 "쿠폰 만료"를 구분할 수 없으니, 클라이언트가 분기할 수 있는 코드를 별도로 준다.

---

## 7. HATEOAS와 리처드슨 성숙도 모델

Uniform Interface의 네 번째 세부 규칙이 **HATEOAS**(Hypermedia As The Engine Of Application State)다. 응답에 "여기서 다음에 할 수 있는 행동"의 링크를 함께 담으라는 것이다.
...
링크를 담는 형식 자체는 REST가 정하지 않는다. 위는 HAL(`_links`) 스타일을 흉내 낸 것이고, 실제로는 HAL·JSON:API·Siren 등 미디어 타입마다 표기가 다르다.

의도는 클라이언트가 URI를 하드코딩하지 않고 서버가 준 링크만 따라가게 만드는 것이다. 그러면 서버가 URI를 바꿔도 클라이언트를 안 고쳐도 된다. 주문이 이미 배송 중이면 `cancel` 링크를 빼는 것만으로 "취소 불가"를 표현할 수도 있다.
...
면접에서 이 주제가 나오면 **"엄밀히는 Level 3까지 가야 REST지만, 실무에서 REST API라 부르는 것은 대부분 Level 2입니다"**라고 답하면 정확하다.

---

## 8. 실무에서는

- **버전 관리는 대부분 URI 경로 방식**을 쓴다. `/v1/users` 형태다. REST 철학상으로는 미디어 타입 협상이 더 맞지만, 브라우저 주소창에 쳐볼 수 있다는 실용성이 이긴다. 자세한 비교는 형제 문서에서 다룬다.
- **컬렉션 응답은 배열이 아니라 객체로 감싼다.** `[{...}, {...}]`로 내리면 나중에 페이지네이션 메타를 넣을 자리가 없어 응답 구조를 통째로 바꿔야 한다(하위 호환을 깨는 변경이다). 처음부터 `{ "data": [...], "page": {...} }`로 시작한다.
- **DELETE는 실제로는 소프트 삭제**인 경우가 많다. `deleted_at`을 채우고 목록 조회에서 제외하는 방식이다. 이때도 API 계약은 `DELETE /users/1` → 204로 유지한다. 내부 구현은 클라이언트가 알 바가 아니다.
- **PUT보다 PATCH를 훨씬 많이 쓴다.** PUT은 전체 교체라 클라이언트가 모든 필드를 다 보내야 하는데, 필드가 20개인 자원에서 닉네임 하나 바꾸려고 20개를 다 보내는 것은 비현실적이고, 그 사이 다른 사람이 바꾼 값을 덮어쓰는 문제도 생긴다.
- **동시 수정 충돌은 `ETag` + `If-Match`로 막는다.** 조회 응답의 `ETag`를 수정 요청의 `If-Match`에 실어 보내면, 그 사이 값이 바뀌었을 때 서버가 412 Precondition Failed로 거절한다. 낙관적 락을 HTTP 계층에서 하는 셈이다.
...
- 꼬리 질문: "그럼 지금 만드신 API는 RESTful한가요?" → Level 2까지는 지켰고 HATEOAS는 비용 대비 효용이 낮아 적용하지 않았다고 근거를 대며 답한다. 무조건 "네"보다 훨씬 좋은 답이다.
...
- 꼬리 질문: "DELETE는 두 번째에 404가 나는데 멱등인가요?" → 멱등성은 응답이 같다는 뜻이 아니라 최종 서버 상태가 같다는 뜻이므로 멱등이 맞다고 답한다.
...
- 꼬리 질문: "만료된 토큰은 401인가요 403인가요?" → 갱신하면 통과하므로 401이다.
...
- 꼬리 질문: "그럼 GraphQL은 왜 200으로 내리나요?" → GraphQL은 단일 엔드포인트에 부분 성공이 가능한 구조라 HTTP 상태 코드로 표현할 수 없기 때문이라고 답한다.
...
| "PUT은 수정, POST는 생성" | PUT도 클라이언트가 URI를 아는 경우 생성에 쓸 수 있다. 차이는 생성/수정이 아니라 **멱등성과 전체 교체 여부** | PUT은 "이 URI를 이 표현으로 통째로 만들어라(있으면 교체)", POST는 "이 컬렉션에 알아서 처리해라" |
| "`Cache-Control: no-cache`는 캐시하지 말라는 뜻" | `no-cache`는 저장은 하되 쓰기 전에 서버에 유효성을 물어보라는 뜻 | 저장 자체를 막는 것은 `no-store` |
| "204를 쓰면 응답 본문에 메시지를 넣을 수 있다" | 204는 정의상 본문이 없다. 넣어도 클라이언트가 무시하거나 파싱 에러를 낸다 | 본문을 줄 거면 200을 쓴다 |
...
REST는 "이름은 자원으로, 행위는 HTTP 메서드로, 결과는 상태 코드로"라는 합의를 통해 클라이언트·프록시·캐시가 API 문서를 읽지 않고도 요청의 의미를 알 수 있게 만드는 설계 스타일이다.
