...
> **비유의 한계**: 청구기호는 위치를 알려주지만 URI는 위치가 아니라 **식별자**입니다. `/users/1`이 어느 서버 어느 DB에 있는지는 클라이언트가 알 필요도 없고 알 수도 없습니다.
...
5. **확장자를 쓰지 않습니다.** 포맷은 `Accept` 헤더로 협상한다
...
> 표 요약: **멱등이면 네트워크 실패 시 그냥 재시도해도 됩니다.** POST와 PATCH만 재시도 전략을 따로 고민하면 된다는 뜻이라, 이 표는 클라이언트 재시도 설계의 근거가 됩니다.
...
- **컬렉션 응답은 배열이 아니라 객체로 감쌉니다.** `[{...}, {...}]`로 내리면 나중에 페이지네이션 메타를 넣을 자리가 없어 응답 구조를 통째로 바꿔야 한다(하위 호환을 깨는 변경이다). 처음부터 `{ "data": [...], "page": {...} }`로 시작합니다.
...
- **PUT보다 PATCH를 훨씬 많이 씁니다.** PUT은 전체 교체라 클라이언트가 모든 필드를 다 보내야 하는데, 필드가 20개인 자원에서 닉네임 하나 바꾸려고 20개를 다 보내는 것은 비현실적이고, 그 사이 다른 사람이 바꾼 값을 덮어쓰는 문제도 생깁니다.
