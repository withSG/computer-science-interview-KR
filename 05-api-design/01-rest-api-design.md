...
> URI에 동사를 쓰면 왜 안 되는지, 400과 422를 무엇으로 가르는지, 그리고 "우리 API는 RESTful합니다"라는 말이 실제로 어디까지 참인지 설명할 수 있게 됩니다.
...
전부 동작합니다. 문제는 **새 기능을 붙일 때마다 "이번엔 어떻게 이름 짓지?"를 매번 고민해야 한다**는 것입니다. 그 결과:
...
> **비유의 한계**: 청구기호는 위치를 알려주지만 URI는 위치가 아니라 **식별자입니다. `/users/1`이 어느 서버 어느 DB에 있는지는 클라이언트가 알 필요도 없고 알 수도 없습니다.
...
주의할 점은 **"상태를 저장하지 마라"가 아니라 "세션 상태를 서버 메모리에 두지 마라"**라는 것입니다. 사용자 데이터는 당연히 DB에 있습니다. Stateless가 금지하는 것은 *이 클라이언트가 지금 어느 단계까지 왔는지*를 서버가 기억하는 일입니다.
...
4. **소문자 + 하이픈**. 언더스코어는 링크에 걸리는 밑줄과 겹쳐 잘 안 보입니다. 대문자를 피하는 이유는 URI 경로가 명세상(RFC 3986) 대소문자를 구분하는 자리인데도 서버·프레임워크에 따라 같은 것으로 취급하기도 해서, 어느 쪽으로 동작할지 예측이 안 되기 때문이다
...
| `GET /user/delete/1` | GET으로 상태를 바꿉니다. 프록시·브라우저가 멋대로 호출할 수 있다 | `DELETE /users/1` |
...
| `GET /users/1/orders/2/items/3/options` | 계층이 너무 깊습니다. 주문 항목 3의 옵션을 보려고 상위 경로를 전부 알아야 한다 | `GET /order-items/3/options` |
...
"비밀번호 재설정", "결제 취소", "장바구니 비우기" 같은 것들이 있습니다. 두 가지 접근이 있습니다.
...
②는 REST 원칙에서 벗어나지만, 마땅한 명사가 없는 행위에까지 억지로 이름을 지어내 팀원 누구도 뜻을 짐작하지 못하는 URI를 만드는 것보다는 낫습니다. **일관성 있게 소수의 예외만** 두는 것이 실무 타협점입니다.
...
Safe한 메서드는 전부 멱등이다(상태를 안 바꾸니 당연하다). 반대는 성립하지 않습니다.
...
> 표 요약: **멱등이면 네트워크 실패 시 그냥 재시도해도 된다.** POST와 PATCH만 재시도 전략을 따로 고민하면 된다는 뜻이라, 이 표는 클라이언트 재시도 설계의 근거가 됩니다.
...
서버는 이 키를 저장소(주로 Redis)에 남깁니다. 같은 키가 다시 오면 **처리하지 않고 처음 저장해둔 응답을 그대로 돌려준다.** 클라이언트가 지켜야 할 핵심 규칙은 하나입니다. **재시도할 때 키를 새로 만들지 않는다.** 새 UUID를 만들면 서버 입장에서는 완전히 다른 요청이라 그대로 두 번 결제됩니다.
...
> 표 요약: 4xx는 "원인이 요청 쪽에 있다", 5xx는 "원인이 서버 쪽에 있다"로 갈립니다. 헷갈리면 **요청을 고쳐야만 통과하는가**를 따져보면 됩니다. 고쳐야 통과하면 4xx, 서버 사정만 풀리면 같은 요청이 그대로 통과하면 5xx다. 429는 원인이 요청 쪽(너무 많이 보냄)이라 4xx이면서도 시간이 지나면 그대로 통과한다는 점에서 예외적이라, `Retry-After`로 언제 다시 오면 되는지를 함께 알려줍니다.
...
경계는 **"요청을 이해했는가"**다. 이해조차 못 했으면 400, 이해는 했는데 받아들일 수 없으면 422다. 다만 422를 아예 안 쓰고 400으로 통일하는 팀도 많습니다. 어느 쪽이든 **팀 안에서 일관되면 됩니다. 면접에서는 둘의 차이를 알고 있다는 것만 보이면 충분합니다.
...
에러 응답에 담아야 할 것과 담으면 안 되는 것도 갈립니다.
...
Uniform Interface의 네 번째 세부 규칙이 **HATEOAS**(Hypermedia As The Engine Of Application State)다. 응답에 "여기서 다음에 할 수 있는 행동"의 링크를 함께 담으라는 것입니다.
...
**Level 3까지 가는 프로젝트는 거의 없다.** 이유는 분명합니다. 링크를 따라가는 클라이언트를 만드는 비용이 URI를 하드코딩하는 비용보다 훨씬 크고, 응답 크기도 커지고, 결정적으로 클라이언트 팀이 어차피 API 문서를 보고 개발하기 때문입니다. Fielding 본인은 하이퍼미디어가 빠진 것을 REST라 부르면 안 된다는 입장을 여러 차례 밝혔지만, 업계 용어로서의 "REST API"는 사실상 Level 2를 가리킵니다.
...
- **컬렉션 응답은 배열이 아니라 객체로 감싼다.** `[{...}, {...}]`로 내리면 나중에 페이지네이션 메타를 넣을 자리가 없어 응답 구조를 통째로 바꿔야 한다(하위 호환을 깨는 변경이다). 처음부터 `{ "data": [...], "page": {...} }`로 시작합니다.
...
- **PUT보다 PATCH를 훨씬 많이 쓴다.** PUT은 전체 교체라 클라이언트가 모든 필드를 다 보내야 하는데, 필드가 20개인 자원에서 닉네임 하나 바꾸려고 20개를 다 보내는 것은 비현실적이고, 그 사이 다른 사람이 바꾼 값을 덮어쓰는 문제도 생깁니다.
- **동시 수정 충돌은 `ETag` + `If-Match`로 막는다.** 조회 응답의 `ETag`를 수정 요청의 `If-Match`에 실어 보내면, 그 사이 값이 바뀌었을 때 서버가 412 Precondition Failed로 거절합니다. 낙관적 락을 HTTP 계층에서 하는 셈입니다.
...
| "204를 쓰면 응답 본문에 메시지를 넣을 수 있다" | 204는 정의상 본문이 없습니다. 넣어도 클라이언트가 무시하거나 파싱 에러를 낸다 | 본문을 줄 거면 200을 쓴다 |
...
REST는 "이름은 자원으로, 행위는 HTTP 메서드로, 결과는 상태 코드로"라는 합의를 통해 클라이언트·프록시·캐시가 API 문서를 읽지 않고도 요청의 의미를 알 수 있게 만드는 설계 스타일입니다.
