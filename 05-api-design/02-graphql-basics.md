...
> REST에서 화면 하나 그리는 데 요청을 세 번 해야 했던 문제가 왜 생기는지, GraphQL이 그것을 어떻게 없애는지, 그리고 그 대가로 무엇을 잃는지 설명할 수 있게 된다.
...
**Over-fetching(과다 조회)** — 필요 없는 필드까지 받는다. 서버 입장에서는 낭비된 직렬화 비용이고, 모바일 사용자 입장에서는 낭비된 데이터 요금이다.

**Under-fetching(과소 조회)** — 한 화면을 채우려고 여러 번 왕복한다. 왕복 한 번의 지연이 100ms라면 순차 호출 시 300ms가 그냥 사라진다. 게다가 첫 응답을 받아야 다음 URI를 알 수 있는 경우(게시글 목록 → 각 게시글의 작성자)에는 병렬 처리도 불가능하다.
...
동작한다. 문제는 **화면 종류만큼 엔드포인트가 늘어난다**는 것이다. iOS 프로필 화면, 안드로이드 프로필 화면, 웹 프로필 화면이 각각 다른 필드를 원하면 `?fields=name,posts.title` 같은 쿼리 파라미터를 직접 설계하기 시작하고, 그 파싱 규칙과 중첩 처리 규칙을 팀이 자체적으로 만들게 된다.

**GraphQL은 그 "필드 선택 규칙"을 타입 시스템까지 갖춰 표준화한 것**이라고 보면 이해가 빠르다. 페이스북이 2012년에 만들어 2015년에 공개했고, 만들어진 배경 자체가 모바일 앱의 데이터 왕복 최소화였다.

> 비유: 뷔페와 코스 요리. REST는 코스 요리다. 주방이 정해둔 구성이 그대로 나오므로 안 먹을 것도 접시에 담겨 온다. GraphQL은 뷔페다. 먹을 것만 접시에 담는다.
> **비유의 한계**: 뷔페에서는 손님이 아무리 많이 담아도 주방 부담이 같지만, GraphQL은 깊이 중첩된 쿼리 하나가 DB를 수백 번 때릴 수 있다. 그래서 서버가 "한 접시에 담을 수 있는 양"을 따로 제한해야 한다.

---

## 2. 스키마와 타입 시스템

GraphQL의 출발점은 URL이 아니라 **스키마**다. 서버가 제공할 수 있는 데이터의 모양을 SDL(Schema Definition Language)로 미리 선언한다.
...
> 표 요약: 조회는 순서가 상관없으니 병렬로 돌리고, 변경은 순서가 결과를 바꾸니 순차로 돌린다. **Mutation이 순차 실행이라는 점은 스펙에 명시된 보장**이라, 한 요청에 여러 변경을 순서대로 넣을 수 있다.
...
응답은 쿼리와 **모양이 같다**. 요청한 필드만, 요청한 중첩 구조 그대로 온다.
...
변수는 쿼리 문자열에 문자열 결합으로 끼워 넣지 않고 별도로 전달한다.
...
Mutation도 **결과로 무엇을 돌려받을지 클라이언트가 고른다.** 위에서는 `id`와 `title`만 받았다. 생성 직후 화면 갱신에 필요한 필드만 받을 수 있다는 뜻이다.
...
Query/Mutation과 달리 연결을 유지하고 이벤트가 생길 때마다 서버가 밀어준다. 전송 계층은 GraphQL 스펙이 정하지 않으며, 실무에서는 WebSocket 기반 프로토콜이나 SSE를 쓴다. 채팅, 알림, 실시간 대시보드가 주 용도다.
...
스키마는 "무엇을 줄 수 있는가"만 선언한다. **실제로 값을 어디서 가져올지 정하는 함수가 리졸버(resolver)다.** 필드 하나에 리졸버 하나가 대응한다.
...
리졸버는 **위에서 아래로 트리처럼 호출된다.** 여기가 N+1의 발원지다.
...
1. **배치 함수는 키 배열과 같은 순서·같은 길이의 배열을 반환해야 한다.** DB는 `IN (3,7,5)`의 결과를 어떤 순서로 줄지 보장하지 않고, 없는 id는 아예 빠져서 온다. 위처럼 Map으로 다시 정렬해주지 않으면 다른 사용자의 데이터가 엉뚱한 게시글에 붙는다.
2. **DataLoader 인스턴스는 요청마다 새로 만든다.** DataLoader는 배치뿐 아니라 캐싱도 하는데, 이 캐시를 애플리케이션 전역으로 공유하면 A가 조회한 값을 B가 그대로 받아버린다. 권한이 다른 사용자 사이에서는 그대로 정보 유출이다.

자바 진영에서는 Spring for GraphQL의 `@BatchMapping`이 같은 일을 한다. 메서드가 부모 객체 리스트를 통째로 받아 한 번에 조회하고 매핑을 돌려주는 방식이다.
...
> 표 요약: **GraphQL은 "클라이언트 편의를 서버 복잡도로 사 오는 거래"**다. 클라이언트가 여럿이고 요구가 자주 바뀌면 남는 장사고, 단순 CRUD 하나에 웹 클라이언트 하나면 손해다.

### 캐싱이 왜 어려운가

REST에서 `GET /users/1`은 URL 자체가 캐시 키다. CDN도 브라우저도 프록시도 별도 설정 없이 캐싱한다. GraphQL은 대부분 `POST /graphql` 하나로 가고 본문이 매번 다르므로 **URL 기준 캐싱이 원천적으로 안 된다.**

우회 방법이 두 갈래다.

1. **클라이언트 측 정규화 캐시** — Apollo Client 같은 라이브러리가 응답을 `User:1`, `Post:5` 단위로 쪼개 저장한다. 다른 쿼리가 같은 객체를 참조하면 재사용한다. HTTP 캐시가 아니라 애플리케이션 캐시다.
2. **Persisted Query** — 쿼리 문자열을 미리 서버에 등록하고 클라이언트는 그 쿼리의 해시만 보낸다. 본문이 짧아지므로 `GET /graphql?<해시>&variables=...`처럼 GET 요청으로 만들 수 있고, 그 순간 URL이 다시 캐시 키가 되어 CDN 캐싱이 가능해진다. 해시를 어떤 파라미터 이름으로 싣는지는 스펙이 아니라 구현(Apollo 등)이 정한다. 덤으로 등록되지 않은 임의의 쿼리를 서버가 아예 안 받게 되어 보안에도 도움이 된다.
...
GraphQL은 **부분 성공**이 가능하다. 위 응답은 이름은 성공했고 게시글만 실패했다. 이걸 HTTP 상태 코드 하나로 표현할 방법이 없다. 200을 내리는 것은 "HTTP 계층에서는 요청을 정상 처리했고, 애플리케이션 결과는 본문을 봐라"는 뜻이다.

REST 절에서 "200에 에러 담지 마라"고 했던 것과 모순처럼 보이지만 다르다. REST는 요청 하나가 자원 하나에 대응하므로 전부 성공 아니면 전부 실패인데, GraphQL은 한 요청에 여러 필드가 섞여 있어 이분법이 성립하지 않는다.

---

## 6. 서버를 지키는 장치

REST에서는 엔드포인트가 곧 비용의 상한선이다. GraphQL에서는 클라이언트가 쿼리를 짜므로 상한선이 없다.
...
- **타임아웃** — 실행 시간 상한
- **introspection 비활성화** — 운영 환경에서 스키마 전체를 긁어가지 못하게 한다. 다만 프론트 개발 편의성과 상충하므로 팀에 따라 다르다
- **Persisted Query만 허용** — 미리 등록한 쿼리 외에는 아예 받지 않는다. 가장 강력하다

---

## 7. 스키마 진화 — 버전 대신 폐기 예고

REST는 깨지는 변경이 생기면 `/v2`를 만든다. GraphQL은 엔드포인트가 하나라 그 방법을 쓸 수 없다. 대신 **필드 단위로 폐기를 예고**한다.
...
`@deprecated`가 붙은 필드는 여전히 동작하지만, 개발 도구와 introspection 결과에 경고로 표시된다. 클라이언트가 마이그레이션할 시간을 벌어주는 장치다.

여기서 GraphQL의 구조적 장점이 하나 나온다. **어떤 클라이언트가 어떤 필드를 실제로 쓰는지 서버가 안다.** REST에서 `GET /users/1` 응답의 `fullName` 필드를 지워도 되는지 알려면 전체 클라이언트 코드를 뒤져야 하지만, GraphQL 서버는 요청받은 필드를 그대로 알고 있으므로 "지난 30일간 `fullName`을 요청한 클라이언트가 0건"을 통계로 확인하고 안전하게 지울 수 있다.

물론 다음은 여전히 깨는 변경이다.

- 필드나 타입을 **제거**하는 것
- 조회 결과로 나가는 필드를 널 불가에서 널 허용으로 완화하는 것(`String!` → `String`). 널이 올 리 없다고 믿고 짠 클라이언트가 깨진다. 반대 방향(`String` → `String!`)은 클라이언트가 이미 널을 처리하고 있으므로 안전하다
...
- **전면 교체보다 병행이 많다.** 외부 공개 API와 파일 업로드는 REST로 두고, 앱이 쓰는 조회 API만 GraphQL로 만드는 구성이 흔하다.
- **BFF(Backend For Frontend) 계층으로 쓰는 경우**가 많다. 내부 마이크로서비스들은 REST/gRPC로 통신하고, 그것들을 모아 클라이언트에 내려주는 앞단만 GraphQL로 둔다. GraphQL의 강점인 "여러 소스를 한 요청으로 합치기"가 그대로 살아난다.
- **DataLoader 없이 GraphQL을 운영하면 거의 확실히 성능 문제가 난다.** 도입 초기에 잘 돌던 API가 데이터가 쌓이면서 느려지는 전형적인 원인이다.
- **GitHub API v4가 GraphQL, v3가 REST**로 둘 다 공개되어 있다. 실제 대규모 GraphQL 스키마가 어떻게 생겼는지 보고 싶으면 좋은 참고 자료다.
...
- 꼬리 질문: "그럼 REST는 이제 안 쓰나요?" → HTTP 캐싱이 중요한 공개 API, 파일 업로드, 단순 CRUD는 여전히 REST가 낫다고 답한다. 트레이드오프를 아는지 보는 질문이다.
...
- 꼬리 질문: "REST에는 N+1이 없나요?" → 있지만 서버 코드 안에서 발생 지점이 고정돼 있어 미리 조인이나 fetch join으로 잡을 수 있고, GraphQL은 쿼리 모양에 따라 달라져 구조적으로 더 취약하다고 답한다.
...
- 꼬리 질문: "그럼 인증 실패도 200인가요?" → 쿼리 실행 전에 걸리는 문제라 401로 내리는 구현이 많다고 답한다. 스펙이 강제하지 않는 영역이다.
...
- 꼬리 질문: "Rate Limiting은 어떻게 거나요?" → 요청 수가 아니라 쿼리 복잡도 점수를 예산처럼 차감하는 방식을 쓴다고 답한다.
...
| "스키마에 필드를 추가하면 기존 클라이언트가 깨진다" | 클라이언트는 자기가 요청한 필드만 받는다 | 필드 추가는 안전한 변경이다. 제거와 널 허용 완화가 깨는 변경이다 |

---

## 한 줄 정리

GraphQL은 "무엇을 받을지"의 결정권을 서버에서 클라이언트로 옮겨 over/under-fetching을 없애는 대신, 캐싱·에러 표현·부하 통제라는 세 가지 숙제를 서버에 떠넘기는 기술이다.
