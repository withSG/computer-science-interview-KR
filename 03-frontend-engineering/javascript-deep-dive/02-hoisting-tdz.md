...
> "선언이 위로 끌어올려진다"는 설명은 결과만 맞고 원리는 틀렸다. 이 문서를 읽으면 호이스팅을 **등록과 초기화의 시점 차이**로 설명하고, TDZ가 왜 언어 설계상 필요했는지 근거를 댈 수 있다.
...
호이스팅은 누가 "만들자"고 설계한 기능이 아니다. **실행 컨텍스트가 두 단계로 동작한 결과 자연스럽게 드러난 현상**이다.

엔진은 코드를 실행하기 전에 스코프 전체를 한 번 훑는다. 왜냐하면 실행 도중에 처음 보는 이름이 튀어나올 때마다 "이게 지역 변수인가 전역 변수인가"를 다시 판정하는 건 비효율적이고, 서로를 호출하는 두 함수(상호 재귀)를 정의할 방법도 없어지기 때문이다.
...
즉 **선언을 먼저 수집하는 것 자체는 합리적인 설계**다. 문제는 초기 JavaScript가 여기서 한 발 더 나가 "값도 `undefined`로 미리 채워두자"고 결정한 것이었다. 그 결정이 만든 부작용을 20년 뒤에 바로잡은 것이 TDZ다.

> **비유**: 회의 시작 전에 참석자 명패를 미리 책상에 깔아두는 것. `var`는 명패에 "미정"이라고 적어두고, `let`/`const`는 명패는 놓되 **비닐을 씌워** 본인이 도착하기 전에는 못 읽게 한다.
>
> **비유의 한계**: 명패는 그냥 종이지만 환경 레코드의 바인딩은 값의 저장 위치 그 자체다. 그리고 "비닐"은 물리적 가림막이 아니라, 초기화 여부를 나타내는 내부 상태다.

---

## 2. 호이스팅의 실체 — 등록과 초기화

생성 단계에서 엔진이 하는 일은 두 가지로 쪼갤 수 있다.
...
선언 종류별로 이 두 가지가 언제 일어나는지만 보면 모든 차이가 정리된다.
...
**표의 결론**: 호이스팅되지 않는 선언은 없다. 전부 생성 단계에 등록된다. 갈리는 것은 **초기화가 언제 되는가** 하나뿐이다.
...
에러가 나지 않는 것이 왜 문제인가? **버그가 에러가 아니라 잘못된 값으로 나타나기 때문**이다.
...
`rate`가 선언되지 않았다면 `ReferenceError`가 나서 즉시 알아챘을 것이다. 그런데 `var` 덕분에 `undefined`가 반환되고, `price * undefined`는 `NaN`이 되어 화면 어딘가에 "NaN원"으로 조용히 흘러간다. **에러 위치와 증상 위치가 멀어지는 것**이 디버깅을 가장 어렵게 만든다.

여기에 `var`의 나머지 특성이 겹친다.
...
마지막 항목은 특히 위험하다. 전역 `var`는 `globalThis`의 프로퍼티가 되므로 다른 스크립트·라이브러리와 이름이 충돌할 수 있고, 전역 객체가 살아 있는 한 GC되지 않는다. `let`/`const`는 전역 스코프에서 선언해도 별도의 선언적 환경 레코드에 들어가 전역 객체를 오염시키지 않는다.

---

## 4. TDZ — 왜 굳이 에러를 내는가

`let`/`const`는 등록만 되고 초기화되지 않은 상태로 남는다. 이 "스코프 시작 ~ 선언문 도달 직전" 구간이 **TDZ(Temporal Dead Zone, 일시적 사각지대)** 다.
...
`typeof`까지 막는 게 과해 보이지만 의도적이다. 선언되지 않은 변수는 `typeof`가 `'undefined'`를 반환하는데, TDZ 변수도 똑같이 `'undefined'`를 반환한다면 **"선언 안 함"과 "아직 초기화 안 됨"을 구분할 수 없다**. 두 상황은 원인도 해결책도 다르므로 구분해서 알려주는 편이 낫다.
...
`var`였다면 `undefined`가 찍히고 조용히 지나갔을 코드다. TDZ 덕분에 "이 스코프에 `step`을 새로 선언했으니 위쪽에서 바깥 `step`을 쓰는 건 실수 아니냐"고 즉시 알려준다. **TDZ의 목적은 불편함이 아니라, 실수를 값이 아니라 에러로 드러내는 것**이다.
...
같은 "함수를 만든다"인데 호이스팅 결과가 다르고, **에러 종류까지 다르다**. 이 차이는 면접 단골이다.
...
에러 메시지가 다른 이유를 정확히 말할 수 있어야 한다.

- `var expressed`는 이름을 읽는 데는 성공한다. 다만 그 값이 `undefined`라 **호출이 실패**한다 → `TypeError`
...
생성 단계에서는 함수 선언문의 초기화가 `var`의 `undefined`를 덮어쓴다. 그리고 실행 단계에서 대입문이 실행되면 다시 값이 바뀐다. 이런 코드는 읽는 사람을 혼란시키므로 **이름 충돌 자체를 만들지 않는 것**이 정답이다.
...
블록 레벨 함수 선언문은 과거 웹 호환성 때문에 규칙이 복잡하다. 조건부로 함수를 만들어야 한다면 **함수 표현식과 `const`** 를 쓰면 고민할 일이 없다.
...
**왜 문제인가**: 지금은 동작한다. 하지만 나중에 누군가 `function init()`을 `const init = () => {}`로 리팩터링하는 순간 전체가 `ReferenceError`로 죽는다. 문법 스타일 변경이 런타임 크래시를 만드는 코드는 깨지기 쉽다. 또한 읽는 사람은 "이 함수가 어디서 오는지" 아래로 스크롤해야 한다.
...
ESLint의 `no-use-before-define` 규칙으로 강제할 수 있다.
...
**왜 문제인가**: `var i`는 루프가 아니라 **함수 스코프에 단 하나만** 등록된다. 세 콜백이 전부 같은 바인딩을 참조하고, 콜백이 실행될 때쯤 `i`는 이미 종료값 `3`이다. "각 반복마다 i의 복사본이 생긴다"는 직관이 틀린 것이다.
...
`let`을 쓰면 **반복마다 새 바인딩**이 만들어지고 직전 값이 복사되므로, 각 콜백이 서로 다른 `i`를 붙잡는다. 이 동작의 원리와 다른 해법은 [03-closure.md](./03-closure.md)에서 자세히 다룬다.
...
**왜 문제인가**: `const`는 **바인딩의 재할당**을 막을 뿐, 그 바인딩이 가리키는 객체 내부는 건드릴 수 있다. `const`를 붙였으니 안전하다고 믿고 객체를 여기저기 넘기면, 어딘가에서 몰래 수정된 값 때문에 추적이 어려운 버그가 생긴다.
...
`Object.freeze`는 **1단계만** 얼린다는 점도 기억해야 한다. 중첩 객체까지 막으려면 재귀적으로 동결하거나, 애초에 값을 복사해 새로 만드는 방식을 택한다.

---

## 7. 실무에서는

**린트로 강제한다**: 대부분의 팀은 ESLint `no-var`, `prefer-const`, `no-use-before-define`을 켠다. 규칙을 외우게 하는 대신 도구가 잡아주게 만드는 편이 안전하다.

**빌드 도구와의 관계**: 트랜스파일러가 `let`을 `var`로 낮출 때(구형 브라우저 타깃) 이름이 겹치면 변수명을 바꾸고, 반복마다 새 바인딩이 필요한 자리는 함수로 감싸 블록 스코프 의미를 보존한다. 그래서 소스에서 `let`을 쓰는 것과 손으로 `var`를 쓰는 것은 결과물이 다르다. "어차피 var로 바뀌니까 상관없다"는 말은 틀렸다.

**모듈 환경**: ES Module은 자동으로 strict 모드이고, 최상위 `var`도 전역 객체의 프로퍼티가 되지 않는다. 모듈을 쓰는 것만으로 전역 오염 문제의 상당 부분이 사라진다.

**TypeScript**: 타입 시스템이 있어도 TDZ는 런타임 개념이라 그대로 남는다. 다만 대부분의 선언 전 사용은 컴파일 단계에서 `Block-scoped variable used before its declaration` 오류로 먼저 걸러진다.
...
- 꼬리 질문: "`let`은 호이스팅이 안 되는 거 아닌가요?" → 등록은 된다. 등록이 안 된다면 바깥 스코프의 같은 이름을 찾아 올라갔을 텐데, 실제로는 `ReferenceError`가 난다. 이것이 `let`도 호이스팅된다는 증거다.
...
- 꼬리 질문: "TDZ에서 `typeof`도 에러가 나는 이유는?" → 미선언 변수와 구분하기 위해서. 둘 다 `'undefined'`를 반환하면 원인 파악이 불가능해진다.
...
호이스팅은 코드 이동이 아니라 생성 단계의 선언 등록이며, `var`는 등록과 동시에 `undefined`로 초기화되지만 `let`/`const`/`class`는 선언문에 도달해야 초기화되기에 그 사이 구간(TDZ)에서 접근하면 `ReferenceError`로 실수를 즉시 알려준다.
