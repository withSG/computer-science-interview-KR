...
> 큐가 왜 하나가 아니라 둘인가. 이 문서를 읽으면 임의의 비동기 코드 조각을 보고 출력 순서를 손으로 추적할 수 있고, "Promise가 setTimeout보다 먼저"라는 암기를 원리로 대체할 수 있다.
...
Promise를 언어에 도입할 때 설계자들은 `.then()` 콜백을 언제 실행할지 정해야 했다. 후보는 둘이었다.
...
이러면 Promise가 이미 이행된 상태냐 아니냐에 따라 코드 실행 순서가 달라진다. 같은 함수가 캐시 히트일 때는 동기, 미스일 때는 비동기로 동작하면 호출하는 쪽에서 순서를 예측할 수 없다. **비동기 API는 언제나 비동기여야 한다**는 원칙에 어긋난다.
...
이러면 `.then` 콜백이 실행되기 전에 타이머 콜백이나 렌더링이 끼어들 수 있다. Promise 체인이 세 단계면 최악의 경우 세 프레임에 걸쳐 처리되고, 그 사이 중간 상태가 화면에 노출된다. 또 이미 큐에 쌓여 있던 다른 태스크 뒤로 밀리므로 응답이 느려진다.

그래서 제3의 답이 나왔다. **"현재 태스크가 끝난 직후, 다음 태스크가 시작되기 전, 렌더링보다도 먼저"** 처리되는 별도의 줄. 그것이 **마이크로태스크 큐**다.

> **비유**: 매크로태스크는 접수 순서대로 부르는 병원 대기표, 마이크로태스크는 진료를 마친 환자에게 "나가기 전에 처방전 받고 가세요"라고 붙이는 후속 절차다. 다음 환자를 부르기 전에 이 절차는 반드시 끝난다.
>
> **비유의 한계**: 실제 병원에서는 후속 절차가 무한히 늘어나지 않지만, 마이크로태스크는 **처리 도중 새로 추가된 것까지 이번에 다 처리**한다. 이 성질이 뒤에 나올 기아 문제를 만든다.
...
**표의 결론**: "Promise 계열은 마이크로, 타이머·이벤트·I/O는 매크로"가 기본 원칙이고, `rAF`와 `process.nextTick`만 별도 취급하면 된다.
...
여기서 배울 점은 `4`가 `5`보다 먼저 나온다는 것이다. 마이크로태스크 처리 중에 추가된 `4`가 **다음 사이클로 밀리지 않고 이번 소진 과정에 합류**한다. 반면 `2`와 `6`은 매크로태스크라 각각 다른 사이클에 실행된다.
...
1. **`await`를 만나기 전까지 async 함수는 동기적으로 실행된다.** `'A'`가 `'B'`보다 먼저 나오는 이유다.
2. **`await` 뒤의 코드는 마이크로태스크 한 틱 뒤에 재개된다.** `await`가 스레드를 재우는 것이 아니라, 함수의 나머지를 콜백으로 등록하고 콜 스택을 비우는 것이다.

(`await` 이후 재개까지 걸리는 마이크로태스크 틱 수는 과거 엔진에서 더 많았다. 현행 엔진은 네이티브 Promise에 대해 한 틱으로 최적화되어 있으므로 위 순서를 기준으로 삼으면 된다.)
...
- **화면 갱신 직전에 실행된다.** 여기서 DOM을 바꾸면 바로 다음 프레임에 반영되므로 중간 상태가 노출되지 않는다.
- **화면 주사율에 맞춰 호출된다.** 60Hz면 초당 약 60회. `setInterval(fn, 16)`처럼 주사율과 어긋나 프레임이 밀리는 문제가 없다.
- **탭이 백그라운드면 호출되지 않는다.** 보이지 않는 화면을 그리느라 배터리를 쓰지 않는다. 애니메이션 루프에 `setInterval` 대신 rAF를 쓰는 큰 이유다.
...
한편 **마이크로태스크로는 렌더링을 얻을 수 없다**. 마이크로태스크 큐를 비우는 동안에는 렌더링 단계로 넘어가지 않기 때문이다.
...
**왜 문제인가**: `await Promise.resolve()`는 마이크로태스크를 하나 만들 뿐이다. 마이크로태스크 큐가 비워질 때까지 렌더링 단계로 못 넘어가므로, 진행률 바는 루프가 전부 끝난 뒤에 한 번만 그려진다.
...
마이크로태스크 큐는 **완전히 빌 때까지** 처리된다. 처리 중에 계속 새 마이크로태스크를 넣으면 큐는 영원히 비지 않는다.
...
실무에서 무한 재귀를 일부러 쓰는 일은 없지만, **재귀적으로 Promise를 체이닝하는 폴링 로직**에서 비슷한 상황이 만들어질 수 있다.
...
**MutationObserver**: DOM 변경 알림을 마이크로태스크로 전달한다. 덕분에 한 태스크 안에서 노드를 100번 바꿔도 콜백은 변경 목록을 모아 한 번만 호출된다. "변경마다 즉시 알림"이었다면 성능이 무너졌을 것이다. 마이크로태스크 큐가 "현재 태스크의 마무리 작업" 자리라는 성질을 정확히 활용한 사례다.

**상태 라이브러리의 배칭**: 여러 상태 변경을 모아 한 번만 다시 그리는 배칭은 대부분 마이크로태스크 시점에 처리를 예약하는 방식으로 구현된다. 같은 태스크 안의 변경들을 모두 모은 뒤 렌더링 전에 한 번에 반영할 수 있기 때문이다.

**디버깅**: 순서가 헷갈릴 때는 코드를 눈으로 읽지 말고 `console.log`로 실제 순서를 찍어 확인하는 편이 빠르다. Chrome DevTools의 Performance 패널에서도 태스크 경계와 마이크로태스크 실행을 확인할 수 있다.

**타이머 클램핑 회피**: 4ms 최소 지연 없이 태스크를 끊고 싶을 때 `MessageChannel`을 쓰는 기법이 있다. 스케줄러 구현에서 볼 수 있는 패턴이지만, 애플리케이션 코드에서 직접 쓸 일은 드물다.
...
- 꼬리 질문: "왜 Promise는 별도 큐를 쓰나요?" → 태스크 큐에 넣으면 타이머나 렌더링이 중간에 끼어들어 Promise 체인이 여러 프레임에 걸치고 중간 상태가 노출된다. 현재 태스크의 마무리 작업으로 처리하려는 설계다.
...
- 꼬리 질문: "`.then` 안에서 `queueMicrotask`를 부르면 언제 실행되나요?" → 같은 소진 과정 안에서, 이미 큐에 있던 항목 뒤에 실행된다.
...
- 꼬리 질문: "애니메이션에 `setInterval` 대신 rAF를 쓰는 이유는?" → 주사율과 동기화되어 프레임 드롭과 티어링이 줄고, 보이지 않을 때 자동으로 멈춰 자원을 아낀다.
...
이벤트 루프는 매크로태스크를 하나 실행할 때마다 마이크로태스크 큐를 완전히 비우고 그다음에야 렌더링 기회를 주므로, Promise 계열은 항상 타이머보다 먼저 실행되고 렌더링을 양보하려면 반드시 매크로태스크로 사이클을 끊어야 한다.
