...
1. **`await`를 만나기 전까지 async 함수는 동기적으로 실행됩니다.** `'A'`가 `'B'`보다 먼저 나오는 이유입니다.
...
- **탭이 백그라운드면 호출되지 않습니다.** 보이지 않는 화면을 그리느라 배터리를 쓰지 않습니다. 애니메이션 루프에 `setInterval` 대신 rAF를 쓰는 큰 이유입니다.
