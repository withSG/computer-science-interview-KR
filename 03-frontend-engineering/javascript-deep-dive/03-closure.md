...
> 함수가 태어난 자리의 기억을 들고 다니는 현상. 이 문서를 읽으면 클로저를 "함수 + 렉시컬 환경"으로 정의하고, 은닉·팩토리·이벤트 핸들러 같은 실전 패턴이 왜 클로저 없이는 성립하지 않는지 설명할 수 있다.
...
"호출될 때마다 1씩 증가하는 카운터"를 만들어보자. 클로저를 모른다면 선택지는 둘뿐이다.
...
필요한 건 **"함수만 접근할 수 있고, 호출 사이에 살아남는 저장 공간"** 이다. 다른 언어라면 클래스의 `private` 필드나 정적 변수로 해결한다. JavaScript는 클래스 문법이 없던 시절부터 이 문제를 **함수의 스코프**로 풀었다. 그 결과가 클로저다.
...
> **비유**: 배낭을 멘 여행자. 함수가 태어난 방에서 필요한 물건(변수)을 배낭에 담아 나오면, 그 방이 헐린 뒤에도 물건을 꺼내 쓸 수 있다.
>
> **비유의 한계**: 배낭에는 물건의 **복사본**이 들어가지만 클로저가 담는 건 **원본 서랍 자체**다. 같은 방에서 나온 여행자 둘은 같은 서랍을 공유한다. 이 차이가 뒤에 나올 반복문 문제의 핵심이다.
...
거창해 보이지만 JavaScript에서는 모든 함수가 이 조건을 만족한다. 함수 객체가 만들어질 때 엔진은 **자신이 정의된 렉시컬 환경을 `[[Environment]]` 내부 슬롯에 저장**한다. 그래서 엄밀히 말하면 "모든 함수는 클로저"이며, 우리가 클로저라고 부를 때는 보통 **외부 함수가 종료된 뒤에도 그 변수를 계속 참조하는 경우**를 가리킨다.
...
핵심은 **실행 컨텍스트와 렉시컬 환경의 수명이 다르다**는 점이다.
...
컨텍스트는 스택에서 걷혔지만, **`fn`이 `inner`를 참조하고 `inner`가 `[[Environment]]`로 환경 레코드를 참조**하므로 가비지 컬렉터 입장에서 `secret`은 아직 "도달 가능한" 객체다. 그래서 수거되지 않는다. 클로저는 GC 규칙을 어기는 예외가 아니라, **도달 가능성 규칙을 그대로 따른 결과**다.

### 클로저는 값이 아니라 바인딩을 캡처한다

가장 많이 틀리는 지점이다. 클로저는 함수가 만들어진 순간의 **값을 복사해두는 것이 아니다**. 변수 자리(바인딩) 자체를 공유한다.
...
`read`와 `write`는 **같은 환경 레코드의 같은 `v`** 를 본다. 이 성질 덕분에 은닉된 상태를 여러 메서드가 조작할 수 있고, 동시에 이 성질 때문에 반복문 문제가 생긴다.
...
핵심은 은닉 자체가 아니라 **불변식(invariant)을 지킬 수 있다는 것**이다. `balance`를 직접 못 바꾸므로 "잔액은 음수가 될 수 없다" 같은 규칙을 `deposit` 안에서 강제할 수 있다. 요즘은 클래스의 `#private` 필드로도 같은 목적을 달성한다.

### 3-2. 모듈 패턴

ES Module이 없던 시절, 전역 오염 없이 코드를 묶는 유일한 방법이었다.
...
즉시 실행 함수(IIFE)로 스코프를 만들고, 밖에 보여줄 것만 객체로 반환한다. `levels`, `shouldLog`는 외부에서 볼 수 없다. 지금은 ESM의 `export`가 같은 역할을 하므로 신규 코드에서 이 패턴을 쓸 일은 거의 없지만, **레거시 코드를 읽을 때 반드시 만나는 형태**다.
...
`apiLog`가 `prefix`를 기억하는 이유가 클로저다. 설정값을 미리 고정한 "부분 적용된 함수"를 만들 수 있어, 반복되는 인자를 매번 넘기지 않아도 된다.
...
`called`와 `cache`를 전역에 두지 않고도 호출 사이에 유지할 수 있다는 점이 핵심이다.
...
디바운스·쓰로틀·재시도 카운터처럼 "이전 호출의 흔적을 기억해야 하는" 유틸리티는 전부 클로저 위에 세워진다.
...
`var i`는 함수 스코프에 **바인딩 하나**만 만든다. 세 콜백은 전부 그 하나를 참조하고, 콜백이 실행되는 시점(동기 코드가 전부 끝난 뒤)에 `i`는 이미 종료 조건을 만족한 `3`이다. 클로저가 값이 아니라 바인딩을 캡처한다는 성질이 그대로 드러난 사례다.
...
`let`으로 선언한 `for` 루프는 **반복마다 새 렉시컬 환경을 만들고 직전 반복의 값을 복사해 넣는다**. 각 콜백이 서로 다른 바인딩을 붙잡으므로 의도대로 동작한다. 코드 변경이 한 글자뿐이라 실무에서는 사실상 이 방법만 쓴다.
...
즉시 실행 함수를 호출하면 그때마다 새 실행 컨텍스트가 생기고, 매개변수 `captured`는 그 반복의 값을 받은 **독립된 바인딩**이 된다. `let`이 없던 ES5 시절의 표준 해법이다.
...
`setTimeout`의 세 번째 이후 인자는 콜백 호출 시 그대로 전달된다. 이 값은 `setTimeout`을 **호출하는 순간** 평가되므로 당시의 `i` 값이 넘어간다. 클로저를 아예 쓰지 않고 문제를 우회하는 방식이다.
...
**결론**: `let`을 쓰면 끝난다. 나머지 둘은 "왜 `let`이 통하는지"를 이해하기 위한 대조군으로 알아두면 된다.

---

## 5. 메모리 관점 — 언제 문제가 되나

클로저가 유지하는 환경은 **참조가 끊길 때까지 GC 대상이 아니다**. 대부분은 의도된 동작이지만, 두 경우에 누수가 된다.
...
**왜 문제인가**: 반환된 함수가 `hugeBuffer`를 참조하므로, 화면에서 사라진 컴포넌트의 핸들러 하나가 수 MB를 붙들 수 있다. 필요한 값은 숫자 하나인데 원본 배열 전체가 남는다.
...
주의할 점이 하나 더 있다. 같은 외부 함수 안에서 만들어진 여러 클로저는 **환경을 공유**한다. 그래서 형제 함수 중 하나라도 큰 변수를 참조하면, 그 변수는 엔진 구현에 따라 다른 클로저가 살아 있는 동안에도 유지될 수 있다. 큰 데이터는 아예 그 스코프에 두지 않는 편이 안전하다.
...
`setInterval`도 마찬가지다. `clearInterval`을 호출하지 않으면 콜백과 그 클로저가 영원히 살아남는다. **"등록했으면 해제한다"** 는 규칙 하나로 클로저 누수의 대부분을 막을 수 있다.

객체에 부가 데이터를 매달아야 한다면 `WeakMap`을 쓰는 것도 방법이다. 키 객체가 수거되면 값도 함께 정리된다.

---

## 6. 실무에서는

**React**: 함수 컴포넌트는 렌더될 때마다 새로 호출되고, 그 안에서 만들어진 핸들러는 **그 렌더 시점의 props/state를 캡처**한다. 그래서 `useEffect`의 의존성 배열을 빼먹으면, 이펙트 안 함수가 첫 렌더의 값을 계속 붙잡는 **stale closure(오래된 클로저)** 버그가 생긴다. "이상하게 값이 안 바뀐다"의 상당수가 이 문제이며, 해결책은 의존성을 정확히 적거나 최신 값을 참조하는 형태(함수형 업데이트, ref)로 바꾸는 것이다.

**번들 크기와 성능**: 클로저는 힙에 환경 객체를 만든다. 초당 수만 번 호출되는 경로에서 매번 새 클로저를 생성하면 GC 압력이 생긴다. 다만 이는 측정 후에 다룰 문제이며, 가독성을 먼저 희생할 이유는 없다.

**보안 오해**: 클로저 은닉은 "다른 코드가 실수로 건드리는 것"을 막는 캡슐화 장치다. 브라우저 개발자 도구에서 디버거로 멈추면 스코프 내용이 그대로 보인다. **비밀 키를 클로저에 넣는 것은 보안이 아니다.**
...
- 꼬리 질문: "그럼 모든 함수가 클로저인가요?" → 명세상으로는 그렇다. 다만 실무에서 클로저라고 부를 때는 외부 스코프 변수를 실제로 참조해 그 환경을 살려두는 경우를 가리킨다.
...
- 꼬리 질문: "`let`은 왜 다른가요?" → `for (let ...)`은 반복마다 새 환경을 만들고 직전 값을 복사해 넣기 때문에 각 콜백이 서로 다른 바인딩을 캡처한다.
...
- 꼬리 질문: "메모리 누수를 어떻게 확인하나요?" → DevTools Memory 패널에서 힙 스냅샷을 비교해 해제되지 않은 객체와 그 retainer 경로를 추적한다.
...
클로저는 함수가 `[[Environment]]`로 붙잡은 렉시컬 환경 덕분에 외부 함수 종료 후에도 그 **바인딩**에 접근하는 현상이며, 상태 은닉과 함수 팩토리를 가능하게 하는 대신 참조를 놓지 않으면 메모리를 계속 붙든다.
