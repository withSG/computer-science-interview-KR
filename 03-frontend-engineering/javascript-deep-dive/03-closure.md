...
A. 상태를 은닉한 카운터·캐시, 설정을 고정한 함수 팩토리와 커링, 디바운스·쓰로틀처럼 호출 사이에 타이머 ID를 유지해야 하는 유틸리티, 그리고 React 함수 컴포넌트의 이벤트 핸들러가 대표적입니다. 공통점은 "전역을 쓰지 않고 호출 사이에 상태를 유지"해야 한다는 데 있습니다.
