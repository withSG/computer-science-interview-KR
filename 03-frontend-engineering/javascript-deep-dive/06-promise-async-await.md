...
> 콜백으로 시작해 Promise를 거쳐 `async/await`에 도달한 비동기 코드의 진화. 이 문서를 읽으면 각 단계가 어떤 문제를 풀었는지 말할 수 있고, 병렬로 돌아야 할 요청을 순차로 만들어버리는 실수를 잡아낼 수 있다.
...
비동기 결과를 받는 가장 원시적인 방법은 콜백을 넘기는 것이다.
...
**한계 1 — 중첩이 깊어진다.** 단계가 늘 때마다 들여쓰기가 한 칸씩 깊어진다. 흔히 콜백 지옥이라 부르지만, 진짜 문제는 보기 흉한 게 아니라 **중간에 조건 분기나 반복을 넣기 어렵다**는 점이다.
...
콜백은 나중에 **다른 콜 스택에서** 실행되므로, 등록 시점의 `try/catch`와는 아무 관계가 없다. 그래서 콜백마다 `(err, data)` 형태로 에러를 직접 나르고 매번 `if (err)`를 반복해야 했다.

**한계 3 — 제어 흐름을 남에게 맡긴다.** 콜백을 넘기는 순간 "언제, 몇 번 호출할지"는 넘겨받은 쪽이 정한다. 라이브러리가 콜백을 두 번 호출하거나 아예 호출하지 않아도 막을 방법이 없다.

Promise는 이 셋을 한 번에 해결한다. **결과를 담을 객체를 즉시 돌려주고, 그 객체를 받은 쪽이 후속 처리를 결정한다.** 상태 전이는 단 한 번만 허용되므로 "두 번 호출" 문제도 구조적으로 막힌다.
...
`async/await`는 여기서 한 걸음 더 나가, 이 체인을 **위에서 아래로 읽히는 코드**로 바꾼다.
...
> **비유**: Promise는 음식점의 진동벨이다. 주문하면 음식 대신 벨을 받고(즉시 반환), 준비되면 벨이 울린다(상태 전이). 벨을 들고 있으면 자리로 돌아가 다른 일을 할 수 있다.
>
> **비유의 한계**: 진동벨은 받은 사람만 쓸 수 있지만, 하나의 Promise에는 `.then`을 여러 개 붙일 수 있고 이미 완료된 뒤에 붙여도 결과를 받는다.
...
체이닝이 가능한 이유는 `.then()`이 **항상 새로운 Promise를 반환**하기 때문이다. 콜백이 무엇을 반환하느냐에 따라 그 Promise의 결과가 정해진다.
...
**반환문을 빼먹는 것이 체이닝의 가장 흔한 버그**다. `then` 안에서 비동기 작업을 시작해놓고 반환하지 않으면, 다음 `then`은 그 작업을 기다리지 않고 즉시 실행된다.
...
`catch`가 값을 반환하면 체인은 **정상 흐름으로 복귀**한다. 이걸 모르면 "에러를 잡았는데 왜 뒤 코드가 계속 실행되지"에서 막힌다.
...
**특별한 이유가 없으면 `.catch()`를 쓴다.** 2인자 형태는 "이전 단계의 실패와 이번 단계의 실패를 구분해서 처리하고 싶을 때"만 의미가 있다.
...
`finally`의 콜백은 인자를 받지 않고, 값을 반환해도 체인의 값을 바꾸지 않는다. 성공/실패와 무관하게 실행되면서 값을 그대로 통과시키므로 정리 작업에 적합하다.
...
자주 놓치는 지점이다. `Promise.all`은 하나가 거부되는 즉시 거부되지만, **나머지 작업을 취소하지는 않는다.** 이미 시작된 네트워크 요청은 그대로 진행되고 응답도 도착한다.
...
그리고 `await`는 **스레드를 재우지 않는다.** 해당 함수의 실행을 중단하고 나머지를 마이크로태스크로 예약한 뒤 콜 스택을 비운다. 그 사이 다른 코드가 실행된다.
...
`fetch`는 **HTTP 에러 상태(404, 500)를 거부로 만들지 않는다.** 네트워크 자체가 실패했을 때만 거부된다. 그래서 `res.ok` 검사를 직접 넣어야 하며, 이걸 빼먹은 코드는 500 응답 본문을 JSON으로 파싱하려다 엉뚱한 곳에서 터진다.
...
`try`(그리고 `using` 같은 정리 구문) 안에서 Promise를 반환할 때 이 차이가 결정적이다. `try` 밖에서는 두 형태의 동작이 사실상 같고, 오히려 `return await` 쪽이 스택 트레이스에 해당 함수 프레임을 남겨 디버깅에 유리하다. 예전에는 `return await`를 불필요한 낭비로 봐서 ESLint `no-return-await` 규칙이 이를 금지했지만, 이 규칙은 현재 비권장으로 분류되어 있다.
...
**왜 문제인가**: `fetchOrders`는 `user`를 필요로 하지 않는다. 그런데 `await` 때문에 첫 요청이 끝나기 전에는 시작조차 하지 않는다. 서버는 놀고 있는데 시간만 흐른다.
...
**왜 문제인가**: `forEach`는 콜백의 반환값을 무시한다. `async` 콜백이 반환하는 Promise는 버려지므로 `forEach`는 즉시 끝나고, 저장이 끝나기 전에 다음 줄이 실행된다. 게다가 중간에 실패한 Promise는 아무도 잡지 않아 unhandled rejection이 된다.
...
`for...of` + `await`는 "느린 코드"가 아니라 **의도적으로 순차 실행이 필요할 때 쓰는 도구**다. 이전 결과에 의존하거나, 외부 API의 요청 제한을 지켜야 할 때가 그렇다.
...
**왜 문제인가**: 만 건의 요청이 동시에 출발한다. 브라우저 커넥션 한도에 걸려 대기열이 생기고, 서버는 요청 폭주로 429나 타임아웃을 반환한다. 병렬화가 항상 선인 것은 아니다.
...
**왜 문제인가**: 아무것도 추가하지 않으면서 코드만 늘린다. 더 나쁜 건 래퍼 안에서 던진 동기 에러나 누락된 `reject` 경로가 조용히 사라질 수 있다는 점이다. `new Promise`는 **콜백 기반의 옛 API를 Promise로 감쌀 때만** 쓴다. `const delay = (ms) => new Promise((r) => setTimeout(r, ms));`가 정당한 사용 예다.

---

## 7. 실무에서는

**처리되지 않은 거부**: `.catch`가 붙지 않은 거부는 브라우저에서 `unhandledrejection` 이벤트로 보고되고 콘솔에 경고가 남는다. Node.js는 15 버전부터 기본적으로 프로세스를 종료시킨다. 서버 코드에서 에러를 흘리면 서비스 전체가 죽을 수 있으므로 진입점마다 에러 경계를 둔다.

**요청 취소**: 사용자가 검색어를 빠르게 바꾸면 이전 요청의 응답이 나중에 도착해 화면을 덮어쓸 수 있다. `AbortController`로 이전 요청을 취소하고, `AbortError`는 정상 흐름으로 처리한다.

**최상위 await**: ES Module에서는 함수 밖에서도 `await`를 쓸 수 있다. 다만 그 모듈을 import하는 쪽의 평가가 그만큼 지연되므로, 초기화 경로에서 남용하면 첫 화면이 늦어진다. 에러를 다시 던질 때 `new Error(msg, { cause: 원본에러 })`로 원인을 붙여두면 로그에서 근본 원인을 추적하기 쉽다.
...
- 꼬리 질문: "async/await가 Promise를 대체하나요?" → 아니다. `await`는 Promise를 소비하는 문법이고, 여러 작업을 조합할 때는 여전히 `Promise.all` 같은 조합기가 필요하다.
...
- 꼬리 질문: "`all`이 실패하면 나머지 요청은 취소되나요?" → 취소되지 않는다. 이미 시작된 작업은 계속 진행되므로, 실제 중단이 필요하면 `AbortController` 같은 취소 수단을 따로 써야 한다.
...
- 꼬리 질문: "수천 개를 `Promise.all`로 묶으면요?" → 동시 요청 폭주로 커넥션 한도와 서버 제한에 걸린다. 동시 실행 개수를 제한하는 워커 패턴을 써야 한다.
...
Promise는 비가역적인 세 상태와 체이닝으로 콜백의 중첩·에러 분산·제어권 상실을 해결했고, `async/await`는 그 체인을 동기처럼 읽히게 만들었지만 `await`를 줄줄이 쓰면 독립 작업까지 순차 실행되므로 "결과를 쓰지 않는 작업은 `Promise.all`로 묶는다"는 기준을 항상 적용해야 한다.
