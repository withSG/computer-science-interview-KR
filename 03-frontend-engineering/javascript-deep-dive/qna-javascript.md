...
### 핵심 답변
실행 컨텍스트는 JavaScript 코드가 실행되는 환경입니다. **변수, 함수 선언, this 바인딩** 같은 정보를 이 안에 담고 있습니다.
...
### 핵심 답변
호이스팅은 변수와 함수 선언이 해당 스코프의 최상단으로 끌어올려진 것처럼 동작하는 현상입니다. 코드가 실행되기 전에 일어납니다.
...
### 핵심 답변
클로저는 **함수와 그 함수가 선언된 렉시컬 환경의 조합**입니다. 풀어 말하면 내부 함수가 외부 함수의 변수에 접근할 수 있다는 뜻입니다.
...
### 핵심 답변
JavaScript는 싱글 스레드인데도 비동기 작업을 처리합니다. 그 일을 하는 메커니즘이 **이벤트 루프**입니다. 콜 스택, 태스크 큐, 마이크로태스크 큐를 지켜보면서 작업의 순서를 조율합니다.
...
### 핵심 답변
async/await는 Promise를 더 **동기 코드처럼 읽기 쉽게** 쓸 수 있게 해 주는 문법적 설탕(Syntactic Sugar)입니다.
...
### 핵심 답변
JavaScript의 데이터 타입은 **7가지 원시 타입**(number, string, boolean, undefined, null, symbol, bigint)과 **객체 타입**으로 나뉩니다. 원시 타입은 변경할 수 없는 값이고, 객체 타입은 변경할 수 있는 값입니다. 그리고 JavaScript는 **동적 타이핑** 언어라서, 변수의 타입이 할당되는 시점에 결정됩니다.
...
### 핵심 답변
스코프는 **식별자(변수)가 유효한 범위**이고, JavaScript 엔진이 식별자를 찾을 때 따르는 규칙입니다. 스코프가 **계층적으로 연결된 구조**가 스코프 체인이며, 변수를 찾을 때는 현재 스코프에서 시작해 상위 스코프로 차례로 올라갑니다. JavaScript는 **렉시컬 스코프**를 따르므로, 상위 스코프는 함수가 **정의된 위치**에 따라 정해집니다.
...
### 핵심 답변
프로토타입은 JavaScript가 **상속을 구현하는 메커니즘**입니다. 모든 객체는 `__proto__`로 자신의 프로토타입 객체를 참조하고, 생성자 함수의 `prototype` 프로퍼티가 인스턴스의 프로토타입이 됩니다. 프로토타입 체인은 객체의 프로퍼티에 접근할 때 **현재 객체 → 프로토타입 → 상위 프로토타입** 순으로 탐색하는 구조입니다.
...
### 핵심 답변
ES6 클래스는 프로토타입 기반 객체지향을 **더 쉽게 작성하게 해 주는 문법적 설탕**입니다. 그래도 생성자 함수와는 차이가 있습니다. **new 없이 호출하면 에러**가 나고, **extends/super로 상속**을 지원하며, **암묵적 strict mode**가 적용되고, **호이스팅이 발생하지 않는 것처럼 동작**합니다.
...
### 핵심 답변
DOM(Document Object Model)은 **HTML 문서의 계층적 구조와 정보를 표현**하는 트리 자료구조이고, 이를 제어하는 **API(프로퍼티와 메서드)를 제공**합니다. 브라우저의 렌더링 엔진이 HTML을 파싱해서 DOM 트리를 만듭니다.
...
### 핵심 답변
이벤트 전파는 DOM 트리에서 이벤트가 **캡처링 단계**(상위→하위), **타깃 단계**(이벤트 타깃 도달), **버블링 단계**(하위→상위) 순으로 퍼지는 것입니다. 브라우저는 기본적으로 **버블링 단계**에서 이벤트를 캐치합니다. **이벤트 위임**은 이 버블링을 활용해 부모 요소에 이벤트 핸들러를 등록하는 패턴입니다.
...
### 핵심 답변
가비지 컬렉션은 더 이상 사용하지 않는 메모리를 해제해 주는 기능으로, **메모리 관리를 자동화**합니다. 현대 JavaScript 엔진은 주로 **mark-and-sweep 알고리즘**을 씁니다. GC 루트에서 도달할 수 있는 객체를 마크하고, 도달할 수 없는 객체를 수거합니다.
...
### 핵심 답변
일급 객체란 **변수에 할당할 수 있고, 함수의 인자로 전달할 수 있고, 함수의 반환값으로 쓸 수 있는** 객체입니다. JavaScript의 함수는 일급 객체라서 이 특성을 모두 갖습니다. **고차 함수**는 함수를 인수로 받거나 함수를 반환하는 함수이고, 함수형 프로그래밍의 핵심입니다.
...
### 핵심 답변
제너레이터는 ES6에서 도입된, **코드 실행을 일시 중지했다가 필요할 때 재개할 수 있는 특수한 함수**입니다. `function*`로 선언하고, `yield` 키워드로 실행을 양도하고, `next()` 메서드로 재개합니다. 호출하면 **이터러블이면서 이터레이터인 제너레이터 객체**를 반환합니다.
...
### 핵심 답변
**Map**은 키-값 쌍을 저장하는 컬렉션이고, 객체와 달리 **키에 모든 타입을 쓸 수** 있습니다. **Set**은 **중복을 허용하지 않는 값**의 컬렉션입니다. **WeakMap/WeakSet**은 약한 참조를 써서 가비지 컬렉션 대상이 될 수 있게 합니다.
...
### 핵심 답변
REST는 **HTTP를 기반으로 클라이언트가 서버의 리소스에 접근하는 방식을 규정한 아키텍처**이고, REST API는 이 REST에 맞춰 서비스 API를 구현한 것입니다. REST의 기본 원칙은 두 가지입니다. **URI는 리소스를 표현**하는 데 집중하고, **행위는 HTTP 요청 메서드**로 정의합니다.
