...
> 요소가 차지하는 크기가 왜 내가 적은 숫자와 다른지, `position: absolute`가 무엇을 기준으로 움직이는지, Flexbox와 Grid 중 무엇을 언제 꺼내야 하는지 설명할 수 있게 된다.
...
HTML은 논문을 공유하려고 만든 문서 포맷이다. 기본 배치 규칙인 **일반 흐름(normal flow)** 은 단순하다. 블록 요소는 위에서 아래로 한 줄씩 쌓이고, 인라인 요소는 왼쪽에서 오른쪽으로 이어지다 줄이 차면 넘어간다. 그게 전부였다.

그런데 사람들은 신문 같은 다단 레이아웃을 원했다. 없는 도구를 만들어내는 역사가 그때부터 시작된다.
...
여기서 얻을 교훈은 하나다. **float 기반 레이아웃은 이제 실무 기술이 아니라 역사 지식이다.** 다만 면접에서 "왜 float를 안 쓰게 됐나"를 묻기 때문에, 무엇이 불편해서 대체되었는지는 알아야 한다.
...
> **비유의 한계**: 액자는 사진 크기가 고정이지만 CSS 박스는 content가 늘면 같이 늘어난다. 또 CSS의 margin은 옆 요소의 margin과 **합쳐질 수 있다**(뒤에 나올 마진 병합). 실제 액자는 그런 일이 없다.
...
**왜 문제인가**: 기본값 `content-box`에서 `width`는 content 영역만 가리킨다. 부모 폭이 400px이면 이 input의 실제 폭은 `400 + 12×2 + 1×2 = 426px`가 되어 부모를 26px 삐져나온다. 가로 스크롤이 생기거나 옆 요소를 밀어낸다. `calc(100% - 26px)`로 때우기 시작하면 padding을 바꿀 때마다 계산식도 같이 고쳐야 한다.
...
`border-box`에서는 padding과 border가 `width` **안쪽으로** 들어간다. 위 예시라면 폭은 400px 그대로고 content 영역이 374px로 줄어든다. 내가 적은 숫자가 곧 화면 폭이 되므로 계산이 직관적이다. 가상 요소도 별도 박스를 만들기 때문에 `::before`, `::after`까지 포함시킨다.
...
**flex와 grid 컨테이너의 자식 사이에서는 마진 병합이 아예 일어나지 않는다.** 요즘 이 현상을 덜 만나게 된 이유가 이것이다.
...
> 표 요약: **`inline`에 `width`가 안 먹는다**는 걸 모르면 `<span>`에 크기를 주려다 30분을 날린다. 크기를 줘야 하면 `inline-block`으로 바꾸거나, 애초에 flex 아이템으로 만든다(flex 아이템은 자기 `display` 값과 무관하게 블록처럼 취급된다). 다만 `<img>`, `<video>`처럼 바깥에서 내용을 끌어오는 **대체 요소(replaced element)** 는 기본값이 `inline`인데도 `width`/`height`가 그대로 적용된다. 이건 예외로 외워두는 편이 빠르다.
...
두 chip 사이에 요청하지 않은 4~5px 간격이 생긴다. **왜 문제인가**: 태그 사이의 줄바꿈과 들여쓰기가 HTML에서 공백 문자 하나로 취급되고, 인라인 맥락에서 공백은 실제 띄어쓰기로 렌더링되기 때문이다. 태그를 붙여 쓰거나 부모에 `font-size: 0`을 주는 옛 편법은 둘 다 코드를 읽기 어렵게 만든다.
...
`position: sticky`가 안 붙는다면 세 가지를 확인한다. `top`/`bottom` 같은 임계점을 안 줬거나(하나는 반드시 필요하다), 조상 중에 `overflow: hidden`(또는 `auto`, `scroll`)이 있어 그쪽이 스크롤 컨테이너가 되어버렸거나, 직계 부모 높이가 sticky 요소와 같아 굳을 공간이 없는 경우다.
...
성능 최적화 의도로 넣은 `transform: translateZ(0)`이 레이어 순서를 뒤집는 일이 실제로 자주 일어난다.
...
React Portal 등으로 모달을 `body` 직속으로 렌더링하면 부모의 stacking context 영향에서 아예 벗어난다. 근본 해법은 숫자가 아니라 구조다.

---

## 5. float와 clearfix — 왜 역사인가

`float`의 원래 목적은 **텍스트가 이미지를 감싸게 하는 것**이다. float된 요소는 일반 흐름에서 **절반만** 빠진다. 박스로서는 빠지지만 주변 인라인 콘텐츠는 여전히 그 자리를 피해간다. `position: absolute`와 다른 지점이 여기다.
...
가상 요소를 float 아래로 밀어내면 부모가 그 높이까지 인식한다. `overflow: hidden`도 같은 효과를 내지만 **콘텐츠가 잘린다.** 드롭다운과 box-shadow가 잘려나가고 안쪽의 `position: sticky`가 깨진다. 지금은 부작용 없는 대안이 있다.
...
float가 지금도 최선인 자리는 원래 목적, 즉 텍스트가 이미지·인용구를 감싸는 배치뿐이다.
...
`flex-grow`는 남는 공간을 나눠 갖는 비율, `flex-shrink`는 모자랄 때 줄어드는 비율, `flex-basis`는 그 계산을 시작하는 출발 크기다.

`flex: 1`과 `flex: auto`의 차이가 실전에서 자주 문제가 된다. `flex: 1`은 basis가 `0`이라 **콘텐츠 길이를 무시하고 모든 아이템이 똑같은 폭**을 갖는다. `flex: auto`는 basis가 콘텐츠 크기라 **글자가 긴 아이템이 더 넓어진다.** 탭 메뉴를 균등 분할하려면 `flex: 1`, 콘텐츠 비율을 살리려면 `flex: auto`다. 단 `flex: 1`의 균등 분할에도 조건이 하나 붙는다. 어떤 아이템의 콘텐츠가 지나치게 길면 바로 다음 절에서 볼 자동 최소 크기에 걸려 그 칸만 넓어진다.
...
**왜 문제인가**: flex 아이템의 `min-width` 기본값은 `0`이 아니라 `auto`다. 즉 **자기 콘텐츠보다 작아지기를 거부한다.** `flex: 1`로 줄어들라고 지시해도 이 하한선에 막힌다.
...
`flex-direction: column`에서 같은 문제가 생기면 `min-height: 0`을 쓴다. 안쪽 스크롤 영역이 안 생기고 부모를 뚫는 현상의 원인도 대개 이것이다.

---

## 7. Grid: 행과 열을 동시에 잡기

Flexbox가 옷걸이 봉 하나에 옷을 거는 것이라면, Grid는 서랍장 칸을 먼저 그려두고 물건을 넣는 것이다.
...
`fr`은 grid 전용 단위로 **남은 공간의 비율**을 뜻한다. `%`와 달리 `gap`을 뺀 나머지를 기준으로 계산하므로 "3등분했더니 gap 때문에 넘친다"는 문제가 없다. 위 선언은 "칸 하나가 최소 220px은 되어야 하고, 들어갈 수 있는 만큼 넣어라"는 뜻이라 **미디어 쿼리 없이도** 화면 폭에 따라 열 개수가 알아서 바뀐다. `auto-fit`은 빈 칸을 접어 아이템을 늘리고, `auto-fill`은 빈 칸을 그대로 남긴다.
...
> 결론: **칸을 내가 정하고 싶으면 Grid, 콘텐츠가 정하게 두고 싶으면 Flexbox.** 실무에서는 Grid로 페이지 골격을 잡고 각 영역 안쪽 정렬을 Flexbox로 하는 조합이 표준이다. 둘은 경쟁 관계가 아니다.
...
세 번째가 `margin-top: -50px` 같은 하드코딩과 다른 점은, `transform`의 `%`가 **자기 자신의 크기 기준**이라 자식 크기를 몰라도 된다는 것이다.

### 홀리 그레일 레이아웃

헤더·푸터는 가로 전체, 가운데는 좌우 사이드바 + 본문, 화면 높이가 남으면 본문이 늘어나 푸터가 바닥에 붙는 배치. float 시절 악명 높던 난제인데 Grid로는 선언 한 번이면 끝난다.
...
`grid-template-areas`의 진짜 강점은 **CSS만 봐도 레이아웃이 눈에 그려진다**는 것이다. 모바일 대응도 HTML을 건드리지 않고 area 배치만 다시 적으면 된다.

> 단, 화면 순서와 DOM 순서가 어긋나면 키보드 Tab 순서는 DOM을 따라가므로 시각 순서와 달라진다. 순서를 크게 뒤집을 때는 마크업 순서 자체를 손보는 편이 안전하다.

---

## 9. 실무에서는

- **`gap`이 margin을 대체했다.** 예전에는 `.item + .item { margin-left: 8px }` 같은 인접 선택자로 마지막 요소의 여백을 뺐다. flex와 grid 모두 `gap`을 지원하는 지금은 컨테이너에 한 줄이면 끝난다.
- **레이아웃 디버깅은 개발자 도구로 한다.** Chrome/Firefox의 Elements 패널에서 `grid`, `flex` 배지를 누르면 트랙 번호와 축이 화면에 오버레이된다. 머리로 상상하는 것보다 훨씬 빠르다.
- **레이아웃을 바꾸는 속성은 비싸다.** `width`, `top`, `margin`을 애니메이션하면 매 프레임 레이아웃을 다시 계산한다. 이동·확대는 `transform`, 투명도는 `opacity`가 원칙이다. 자세한 내용은 [../browser-fundamentals/04-reflow-repaint.md](../browser-fundamentals/04-reflow-repaint.md)에 있다.
- **이미지에는 `width`/`height` 속성을 적는다.** 값이 없으면 이미지가 로드되는 순간 아래 콘텐츠가 밀려 레이아웃이 튄다(CLS). HTML 속성으로 크기를 적어두면 브라우저가 비율을 미리 계산해 자리를 잡아둔다.
...
A. 기본값 `content-box`에서는 `width`가 content 영역만 가리켜서, padding이나 border를 주면 실제 박스가 그만큼 커집니다. `width: 100%`에 padding을 얹으면 부모를 삐져나가 가로 스크롤이 생기죠. `border-box`는 padding과 border를 `width` 안쪽에 포함시켜 내가 적은 숫자가 곧 화면 폭이 되므로 계산이 직관적입니다. `::before`, `::after`까지 포함해 전역으로 깔아두는 게 관행입니다.
- 꼬리 질문: "쿼크 모드와 관련이 있나요?" → DOCTYPE이 없어 쿼크 모드가 되면 표준 모드와 달리 옛 IE 방식, 즉 border-box에 가까운 계산을 한다. DOCTYPE 누락이 레이아웃을 통째로 어긋나게 하는 이유다.
...
- 꼬리 질문: "`position: static`에 z-index를 주면요?" → 무시된다. z-index는 positioned 요소이거나 flex/grid 아이템일 때만 동작한다.
...
- 꼬리 질문: "미디어 쿼리 없이 반응형 갤러리를 만들려면?" → `grid-template-columns: repeat(auto-fit, minmax(220px, 1fr))`을 쓰면 화면 폭에 따라 열 개수가 자동으로 바뀐다.
...
- 꼬리 질문: "BFC가 뭔가요?" → 내부 박스들이 독립적으로 배치되는 영역이다. BFC 안에서는 float가 부모 밖으로 새지 않고 마진 병합도 경계를 넘지 못한다.
...
| "`justify-content`는 가로 정렬이다" | 주축 정렬이다. `column`이면 세로가 된다 | `justify-*`=주축, `align-*`=교차축으로 외운다 |
...
CSS 레이아웃은 "박스가 얼마나 큰가(box-sizing)", "흐름에 남아 있는가(position/float)", "어느 축으로 정렬하는가(flex/grid)" 세 질문의 조합이며, 오늘 새로 짜는 레이아웃의 답은 대부분 `border-box` + Grid 골격 + Flexbox 내부 정렬이다.
