...
> 요소가 차지하는 크기가 왜 내가 적은 숫자와 다른지, `position: absolute`가 무엇을 기준으로 움직이는지, Flexbox와 Grid 중 무엇을 언제 꺼내야 하는지 설명할 수 있게 됩니다.
...
여기서 얻을 교훈은 하나입니다. **float 기반 레이아웃은 이제 실무 기술이 아니라 역사 지식이다.** 다만 면접에서 "왜 float를 안 쓰게 됐나"를 묻기 때문에, 무엇이 불편해서 대체되었는지는 알아야 합니다.
...
> **비유의 한계**: 액자는 사진 크기가 고정이지만 CSS 박스는 content가 늘면 같이 늘어납니다. 또 CSS의 margin은 옆 요소의 margin과 **합쳐질 수 있다**(뒤에 나올 마진 병합). 실제 액자는 그런 일이 없습니다.
...
세로로 맞닿은 두 블록 요소의 margin은 각자 제 몫을 차지하지 않습니다. **둘 중 큰 값 하나만 남고 나머지는 그 안으로 삼켜진다.**
...
병합은 세 상황에서 일어납니다. 인접한 형제 사이, 부모와 첫/마지막 자식 사이(그 경계에 border·padding이 없고 부모가 BFC를 만들지 않을 때), 내용이 빈 블록의 위아래 margin 사입니다. 특히 두 번째가 사고를 많이 냅니다. 자식에게 준 `margin-top`이 부모 밖으로 튀어나가 **부모 전체가 아래로 밀리는** 현상입니다.
...
**flex와 grid 컨테이너의 자식 사이에서는 마진 병합이 아예 일어나지 않는다.** 요즘 이 현상을 덜 만나게 된 이유가 이것입니다.
...
> 표 요약: **`inline`에 `width`가 안 먹는다**는 걸 모르면 `<span>`에 크기를 주려다 30분을 날립니다. 크기를 줘야 하면 `inline-block`으로 바꾸거나, 애초에 flex 아이템으로 만든다(flex 아이템은 자기 `display` 값과 무관하게 블록처럼 취급된다). 다만 `<img>`, `<video>`처럼 바깥에서 내용을 끌어오는 **대체 요소(replaced element)** 는 기본값이 `inline`인데도 `width`/`height`가 그대로 적용됩니다. 이건 예외로 외워두는 편이 빠릅니다.
...
`position: sticky`가 안 붙는다면 세 가지를 확인합니다. `top`/`bottom` 같은 임계점을 안 줬거나(하나는 반드시 필요하다), 조상 중에 `overflow: hidden`(또는 `auto`, `scroll`)이 있어 그쪽이 스크롤 컨테이너가 되어버렸거나, 직계 부모 높이가 sticky 요소와 같아 굳을 공간이 없는 경우입니다.
...
배경색과 테두리가 안 보이고 아래 요소가 A, B 위로 겹쳐 올라옵니다. 이 "높이 붕괴"를 막으려고 나온 것이 clearfix다.

```css
.clearfix::after { content: ''; display: block; clear: both; }   /* 2010년대 관용구 */
```

가상 요소를 float 아래로 밀어내면 부모가 그 높이까지 인식합니다. `overflow: hidden`도 같은 효과를 내지만 **콘텐츠가 잘린다.** 드롭다운과 box-shadow가 잘려나가고 안쪽의 `position: sticky`가 깨집니다. 지금은 부작용 없는 대안이 있습니다.
...
Flexbox가 헷갈리는 이유는 대부분 하나입니다. **`justify-content`와 `align-items`가 가로/세로 중 무엇을 담당하는지 외우려 하기 때문이다.** 외울 필요가 없습니다. 축은 `flex-direction`이 정합니다.
...
`flex: 1`과 `flex: auto`의 차이가 실전에서 자주 문제가 됩니다. `flex: 1`은 basis가 `0`이라 **콘텐츠 길이를 무시하고 모든 아이템이 똑같은 폭**을 갖습니다. `flex: auto`는 basis가 콘텐츠 크기라 **글자가 긴 아이템이 더 넓어진다.** 탭 메뉴를 균등 분할하려면 `flex: 1`, 콘텐츠 비율을 살리려면 `flex: auto`다. 단 `flex: 1`의 균등 분할에도 조건이 하나 붙습니다. 어떤 아이템의 콘텐츠가 지나치게 길면 바로 다음 절에서 볼 자동 최소 크기에 걸려 그 칸만 넓어집니다.
...
**왜 문제인가**: flex 아이템의 `min-width` 기본값은 `0`이 아니라 `auto`다. 즉 **자기 콘텐츠보다 작아지기를 거부한다.** `flex: 1`로 줄어들라고 지시해도 이 하한선에 막힙니다.
...
`fr`은 grid 전용 단위로 **남은 공간의 비율**을 뜻합니다. `%`와 달리 `gap`을 뺀 나머지를 기준으로 계산하므로 "3등분했더니 gap 때문에 넘친다"는 문제가 없습니다. 위 선언은 "칸 하나가 최소 220px은 되어야 하고, 들어갈 수 있는 만큼 넣어라"는 뜻이라 **미디어 쿼리 없이도** 화면 폭에 따라 열 개수가 알아서 바뀝니다. `auto-fit`은 빈 칸을 접어 아이템을 늘리고, `auto-fill`은 빈 칸을 그대로 남깁니다.
...
세 번째가 `margin-top: -50px` 같은 하드코딩과 다른 점은, `transform`의 `%`가 **자기 자신의 크기 기준**이라 자식 크기를 몰라도 된다는 것입니다.
...
- **`gap`이 margin을 대체했다.** 예전에는 `.item + .item { margin-left: 8px }` 같은 인접 선택자로 마지막 요소의 여백을 뺐습니다. flex와 grid 모두 `gap`을 지원하는 지금은 컨테이너에 한 줄이면 끝납니다.
- **레이아웃 디버깅은 개발자 도구로 한다.** Chrome/Firefox의 Elements 패널에서 `grid`, `flex` 배지를 누르면 트랙 번호와 축이 화면에 오버레이됩니다. 머리로 상상하는 것보다 훨씬 빠릅니다.
- **레이아웃을 바꾸는 속성은 비싸다.** `width`, `top`, `margin`을 애니메이션하면 매 프레임 레이아웃을 다시 계산합니다. 이동·확대는 `transform`, 투명도는 `opacity`가 원칙입니다. 자세한 내용은 [../browser-fundamentals/04-reflow-repaint.md](../browser-fundamentals/04-reflow-repaint.md)에 있습니다.
- **이미지에는 `width`/`height` 속성을 적는다.** 값이 없으면 이미지가 로드되는 순간 아래 콘텐츠가 밀려 레이아웃이 튄다(CLS). HTML 속성으로 크기를 적어두면 브라우저가 비율을 미리 계산해 자리를 잡아둡니다.
...
CSS 레이아웃은 "박스가 얼마나 큰가(box-sizing)", "흐름에 남아 있는가(position/float)", "어느 축으로 정렬하는가(flex/grid)" 세 질문의 조합이며, 오늘 새로 짜는 레이아웃의 답은 대부분 `border-box` + Grid 골격 + Flexbox 내부 정렬입니다.
