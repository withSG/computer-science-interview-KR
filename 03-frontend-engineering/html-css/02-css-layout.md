...
`flex: 1`과 `flex: auto`의 차이가 실전에서 자주 문제가 됩니다. `flex: 1`은 basis가 `0`이라 **콘텐츠 길이를 무시하고 모든 아이템이 똑같은 폭**을 갖습니다. `flex: auto`는 basis가 콘텐츠 크기라 **글자가 긴 아이템이 더 넓어집니다.** 탭 메뉴를 균등 분할하려면 `flex: 1`, 콘텐츠 비율을 살리려면 `flex: auto`다. 단 `flex: 1`의 균등 분할에도 조건이 하나 붙습니다. 어떤 아이템의 콘텐츠가 지나치게 길면 바로 다음 절에서 볼 자동 최소 크기에 걸려 그 칸만 넓어집니다.
...
**왜 문제인가**: flex 아이템의 `min-width` 기본값은 `0`이 아니라 `auto`다. 즉 **자기 콘텐츠보다 작아지기를 거부합니다.** `flex: 1`로 줄어들라고 지시해도 이 하한선에 막힙니다.
...
- **이미지에는 `width`/`height` 속성을 적습니다.** 값이 없으면 이미지가 로드되는 순간 아래 콘텐츠가 밀려 레이아웃이 튄다(CLS). HTML 속성으로 크기를 적어두면 브라우저가 비율을 미리 계산해 자리를 잡아둡니다.
