...
### 핵심 답변
시멘틱 태그(Semantic Element)는 HTML 문서에 **추가적인 의미**를 부여하는 태그입니다. `<div>`, `<span>` 같은 무의미한 태그와 달리 콘텐츠의 역할과 구조를 명확히 나타냅니다. SEO와 접근성이 좋아지고 코드 가독성도 올라가며 검색 엔진과 스크린 리더가 문서 구조를 쉽게 파악할 수 있습니다.
...
### 핵심 답변
`px`는 절대 단위로 화면의 픽셀 수를 직접 지정합니다. `em`은 **부모 요소의 폰트 크기**를 기준으로 한 상대 단위로 중첩 시 복잡해질 수 있습니다. `rem`(root em)은 **루트(html) 요소의 폰트 크기**를 기준으로 하여 예측 가능하고 일관되게 크기를 지정할 수 있습니다. 접근성을 위해 폰트 크기에는 rem 사용을 권장합니다.
