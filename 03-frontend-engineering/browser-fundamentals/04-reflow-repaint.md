...
그래서 최적화의 1순위는 **"리플로우를 줄이거나 아예 건너뛰기"**입니다.
...
브라우저는 **또 레이아웃을 처음부터 돌려야 합니다.** 카드가 500개면 레이아웃이 500번 돕니다.
...
A. `rAF` 콜백은 브라우저가 다음 프레임을 그리기 직전에 실행됩니다. 여기서 한 DOM 변경이 바로 이어지는
...
리플로우는 **위치와 크기를 다시 재는 일**, 리페인트는 **다시 칠하는 일**입니다.
