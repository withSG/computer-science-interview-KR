...
> 화면이 갱신될 때 브라우저가 무엇을 다시 계산하는지 알고, 강제 동기 레이아웃을 코드에서 찾아내 고칠 수 있게 된다.
...
첫 화면이 그려졌다고 끝이 아니다. 스크롤하고, 버튼을 누르고, 데이터가 들어오면 화면은 계속 바뀐다.
문제는 **바뀔 때마다 파이프라인을 어디서부터 다시 도느냐**가 변경 종류에 따라 다르다는 것이다.

부드러운 화면은 갱신 주기를 놓치지 않는다는 뜻이다. 흔한 60Hz 화면이라면 프레임 하나에 쓸 수 있는 시간이 약 16.7ms다.
브라우저 자체 작업을 빼면 실제로 우리 코드에 허용되는 시간은 그보다 짧다.
버튼 하나 누를 때마다 전체 레이아웃을 다시 계산하면 이 예산은 순식간에 넘어가고, 화면은 끊긴다.

**비유**: 사무실 가구 배치를 바꾼다고 하자. 벽 색만 새로 칠하는 건 다른 가구에 영향이 없다(페인트).
그런데 책상 하나를 크게 바꾸면 옆 책상, 통로 폭, 의자 위치까지 다시 재야 한다(리플로우).
**비유의 한계**: 실제 브라우저는 "옆 책상만"으로 끝나지 않는 경우가 많다. 문서 흐름 특성상 한 요소의 크기 변화가
뒤따르는 형제와 조상까지 번지면, 사실상 문서 전체를 다시 재는 결과가 되기도 한다.
...
**요점**: 리플로우가 일어나면 그 결과를 다시 그려야 하니 페인트도 따라온다.
반대는 성립하지 않는다. 색만 바꾸면 레이아웃은 건드리지 않는다.
...
둘의 y좌표가 전부 밀린다. `.container`의 높이도 바뀌고, 그러면 컨테이너의 형제도 밀린다.
즉 **한 요소의 변경이 문서 위쪽으로도 아래쪽으로도 전파**된다.

반대로 `position: absolute`나 `fixed`인 요소는 일반 흐름에서 빠져 있어서, 그 요소를 바꿔도
형제들의 위치에 영향을 주지 않는다. 애니메이션 대상 요소를 흐름에서 빼내는 것이 성능에 유리한 이유다.
...
`font-family` 교체가 리플로우인 게 의외일 수 있는데, 글꼴이 바뀌면 글자 폭이 달라져 줄바꿈 위치가 전부 바뀐다.
웹폰트가 늦게 도착했을 때 화면이 출렁이는(레이아웃 시프트) 원인이 이것이다.

### JS API — "읽기"가 레이아웃을 강제한다

여기가 진짜 함정이다. 다음 값들을 읽으면 브라우저는 **최신 레이아웃 결과를 즉시 알아야** 하므로,
대기 중인 변경이 있으면 그 자리에서 레이아웃을 돌린다.
...
이걸 **강제 동기 레이아웃(forced synchronous layout)**이라 부른다.
"쓰기는 미뤄도 되지만, 읽기는 미룰 수 없다"가 핵심 원리다.
...
**왜 문제인가**: 첫 반복에서 `offsetWidth`를 읽으면 레이아웃이 계산된다. 그다음 `style.width`를 쓰면
...
이 코드에서는 읽는 값이 하나뿐이라 간단했다. 실제로는 요소마다 다른 값을 읽어야 하는 경우가 많다.
...
읽기 단계에서 레이아웃이 한 번 계산되면 그 결과가 유효한 동안 나머지 읽기는 캐시된 값을 쓴다.
쓰기는 어차피 프레임 끝에 몰아서 처리된다. **"읽기 → 쓰기" 순서를 지키는 것**만으로 N번이 1번이 된다.
...
쓰기와 쓰기 사이에 읽기가 없으면 브라우저가 알아서 모아 프레임 끝에 한 번만 처리한다.
그런데도 클래스 토글을 권하는 이유는 성능이 아니라 **스타일 값이 JS 코드에 흩어지지 않는다**는 데 있다.
값이 CSS 한곳에 모여 있으면 미디어 쿼리나 테마 변수로 확장하기도 쉽다.
"인라인 스타일을 한 줄로 합치면 빨라진다"는 식의 조언은 오늘날 근거가 약하다.

### 개선 4: 문서에서 떼어내고 작업하기

큰 리스트를 통째로 갈아엎을 때는 아예 렌더 트리에서 빼놓고 작업한 뒤 되돌리는 방법도 있다.
...
`display:none`으로 숨겼다가 되돌리는 방법도 같은 원리지만, 숨기고 되살릴 때 각각 리플로우가 한 번씩 든다.
변경량이 충분히 클 때만 이득이므로 항상 측정하고 쓴다.
...
**왜 문제인가**: 스크롤 이벤트는 한 프레임에 여러 번 발화할 수 있다.
그때마다 레이아웃을 강제하니 스크롤이 뚝뚝 끊긴다.
...
`{ passive: true }`도 중요하다. 이 옵션은 "이 핸들러는 `preventDefault()`를 호출하지 않는다"는 약속이라,
브라우저가 핸들러 실행을 기다리지 않고 스크롤을 먼저 진행할 수 있다.

### 더 나은 대안: 관찰자 API

"어떤 요소가 화면에 보이는가"를 알고 싶은 것뿐이라면 스크롤 위치를 직접 재지 않는 편이 낫다.
...
두 API 모두 브라우저가 자신의 렌더링 주기에 맞춰 결과를 전달한다.
개발자가 레이아웃 값을 직접 캐물을 필요가 없어지므로 강제 동기 레이아웃이 원천적으로 사라진다.

---

## 6. 실무에서는

- **DevTools Performance 패널**: 기록 후 타임라인에서 보라색 `Layout` 막대를 본다.
  같은 프레임 안에 `Layout`이 여러 번 반복되면 스래싱이다. 경고 삼각형이 붙은 항목은
  "Forced reflow"로 표시되고, 클릭하면 원인이 된 JS 줄로 이동한다.
- **Rendering 탭**: `Paint flashing`을 켜면 다시 그려지는 영역이 초록색으로 번쩍인다.
  스크롤만 했는데 화면 전체가 번쩍이면 페인트 범위가 과한 것이다.
- **레이아웃 시프트(CLS)**: 이미지에 `width`/`height`(또는 `aspect-ratio`)를 지정하지 않으면
  이미지가 도착하는 순간 뒤 콘텐츠가 밀린다. 사용자가 누르려던 버튼이 도망가는 그 현상이다.
  자리를 미리 잡아 두는 것이 유일한 해법이다.
- **`contain` 속성**: `contain: layout`을 주면 그 요소 내부의 레이아웃 변경이 바깥으로 전파되지 않는다고
  브라우저에 알려, 재계산 범위를 가둘 수 있다. 독립적인 카드·위젯 컨테이너에 어울린다.
- **가상 스크롤(virtual scroll)**: 수천 개 행을 전부 DOM에 넣으면 레이아웃 대상 자체가 커진다.
  화면에 보이는 만큼만 DOM에 유지하는 것이 근본 해결책이다.
...
- 꼬리 질문: "리플로우도 페인트도 없는 변경이 있나요?" → `transform`과 `opacity`는 합성 단계만으로 처리될 수 있다.
...
- 꼬리 질문: "왜 쓰기는 미룰 수 있는데 읽기는 못 미루나요?" → 쓰기는 최종 상태만 맞으면 되지만,
  읽기는 그 시점의 정확한 값을 반환해야 하므로 미룰 수 없다.
...
성능 문제의 대부분은 쓰기 사이에 끼어든 읽기가 만드는 강제 동기 레이아웃에서 나온다.
