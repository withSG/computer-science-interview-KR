...
> HTML과 CSS가 각각 어떤 트리로 바뀌는지, 그 둘을 합친 렌더 트리가 왜 DOM과 다른지, 그리고 DOM 조작이 왜 비싼지 설명할 수 있게 됩니다.
...
"실제로 화면에 그릴 것들"만 골라낸 렌더 트리를 만듭니다. 세 트리가 각각 무엇을 담는지 구분하는 것이 이 문서의 핵심입니다.
...
**비유의 한계**: 평면도의 방과 시공 목록의 항목이 1:1이 아닙니다. 렌더 트리에는 평면도에 없던 항목(`::before` 같은 가상 요소)이 생기기도 합니다.
...
**`<head>` 최상단**에 와야 합니다. 뒤에 두면 브라우저가 인코딩을 추측하다가 뒤늦게 정정하면서
이미 만든 파싱 결과를 버리고 다시 시작할 수 있습니다. 한글이 깨지는 고전적 원인입니다.

### 파싱은 스트리밍이다

브라우저는 HTML을 다 받고 시작하지 않습니다. 도착한 조각부터 토큰화하고 노드를 붙여 나갑니다.
...
DOM이 만들어지는 동안 CSS도 파싱되어 규칙 집합이 됩니다. 그다음 브라우저는 각 DOM 노드에 대해
"이 노드에 적용될 최종 값은 무엇인가"를 계산합니다. 이 계산에는 세 가지가 관여합니다.
...
**같은 점수면 나중에 선언된 규칙이 이긴다.** 그래서 CSS 파일을 불러오는 순서가 결과를 바꿉니다.
...
기준은 단순합니다. **텍스트 표현에 관한 것은 대체로 상속되고, 박스 자체의 기하나 배경은 상속되지 않는다.**
...
왼쪽부터 내려가면 매칭 실패할 후보를 잔뜩 탐색하게 되지만, 오른쪽에서 시작하면 대부분의 요소를 첫 검사에서 탈락시킬 수 있습니다.

여기서 나오는 실무 지침이 **"가장 오른쪽 선택자(key selector)를 구체적으로 쓰라"**는 것입니다.
...
"`display:none`인 요소는 DOM에도 없다"는 말은 틀립니다. **DOM에는 그대로 있다.**
...
그래서 데이터가 신뢰할 수 없을 땐 노드를 직접 만드는 쪽이 안전합니다.
...
렌더링 파이프라인이 돌지 않습니다. 그리고 fragment를 `appendChild`하면 **fragment 자체가 아니라 그 자식들이 옮겨진다.**
