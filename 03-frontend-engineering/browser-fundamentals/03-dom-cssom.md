...
> HTML과 CSS가 각각 어떤 트리로 바뀌는지, 그 둘을 합친 렌더 트리가 왜 DOM과 다른지, 그리고 DOM 조작이 왜 비싼지 설명할 수 있게 된다.
...
HTML은 그냥 텍스트다. `<div>안녕</div>`이라는 문자열을 화면에 그리려면
"div라는 요소가 있고, 그 안에 '안녕'이라는 텍스트가 있다"는 **구조화된 표현**이 필요하다.
게다가 JS가 나중에 `document.querySelector('div').textContent = '반가워'`처럼 문서를 바꿀 수 있어야 하니,
그 표현은 **읽고 쓸 수 있는 객체 모델**이어야 한다. 그게 DOM(Document Object Model)이다.

CSS도 같은 이유로 CSSOM(CSS Object Model)이 된다. 그리고 브라우저는 이 둘을 합쳐
"실제로 화면에 그릴 것들"만 골라낸 렌더 트리를 만든다. 세 트리가 각각 무엇을 담는지 구분하는 것이 이 문서의 핵심이다.

**비유**: DOM은 건물의 **평면도**(어떤 방이 어떤 위치에 있나), CSSOM은 **인테리어 사양서**(각 방을 어떤 색·크기로 마감하나),
렌더 트리는 **실제 시공 대상 목록**이다. 창고로 안 쓰기로 한 방은 사양서에 "시공 제외"로 표시되고 시공 목록에서 빠진다.
**비유의 한계**: 평면도의 방과 시공 목록의 항목이 1:1이 아니다. 렌더 트리에는 평면도에 없던 항목(`::before` 같은 가상 요소)이 생기기도 한다.
...
파서는 바이트를 문자로 바꿔야 태그를 알아본다. 그래서 `<meta charset="UTF-8">`은
**`<head>` 최상단**에 와야 한다. 뒤에 두면 브라우저가 인코딩을 추측하다가 뒤늦게 정정하면서
이미 만든 파싱 결과를 버리고 다시 시작할 수 있다. 한글이 깨지는 고전적 원인이다.

### 파싱은 스트리밍이다

브라우저는 HTML을 다 받고 시작하지 않는다. 도착한 조각부터 토큰화하고 노드를 붙여 나간다.
그래서 서버가 HTML을 조금씩 흘려보내면 사용자는 더 빨리 무언가를 볼 수 있다.

### 관대한 파서

HTML 파서는 잘못된 마크업을 만나도 멈추지 않고 **알아서 고친다.**
...
닫는 태그가 없어도 파서는 `<p>`가 다른 `<p>`를 포함할 수 없다는 규칙에 따라 앞 문단을 자동으로 닫는다.
편리하지만 함정도 있다. 셀(`<td>`) 안이 아니라 `<table>` 바로 아래에 `<div>`를 넣으면
파서가 그 `<div>`를 테이블 **바깥으로 옮겨** 버린다(foster parenting).
"분명 이렇게 썼는데 DevTools에서 보면 구조가 다르다"면 파서의 교정 규칙을 의심해야 한다.
**DevTools의 Elements 패널은 소스가 아니라 DOM을 보여준다**는 점을 기억하자.

---

## 3. CSSOM과 최종 스타일 결정

DOM이 만들어지는 동안 CSS도 파싱되어 규칙 집합이 된다. 그다음 브라우저는 각 DOM 노드에 대해
"이 노드에 적용될 최종 값은 무엇인가"를 계산한다. 이 계산에는 세 가지가 관여한다.

### 3-1. 캐스케이드와 명시도(Specificity)

한 요소에 여러 규칙이 붙으면 우선순위를 따진다.

```
!important  >  인라인 style  >  명시도 점수  >  나중에 선언된 것
```

명시도 점수는 (ID, 클래스·속성·의사클래스, 타입·의사요소) 세 자리로 센다.
...
앞자리가 크면 뒷자리는 아무리 커도 진다. `#a`(1,0,0)는 클래스를 10개 붙인 선택자(0,10,0)를 이긴다.
자릿수 올림이 없는, 사전식 비교다.

**같은 점수면 나중에 선언된 규칙이 이긴다.** 그래서 CSS 파일을 불러오는 순서가 결과를 바꾼다.

### 3-2. 상속(Inheritance)

모든 속성이 자식에게 내려가는 게 아니다.
...
기준은 단순하다. **텍스트 표현에 관한 것은 대체로 상속되고, 박스 자체의 기하나 배경은 상속되지 않는다.**
자식이 부모 margin을 물려받으면 레이아웃이 걷잡을 수 없어지기 때문이다.

값이 필요하면 명시적으로 물려받게 할 수 있다.
...
사람은 "사이드바 안의 ul 안의 li 안의 a"로 읽지만, 브라우저는 **`a`를 먼저 찾고** 조상을 거슬러 올라가며 확인한다.
왼쪽부터 내려가면 매칭 실패할 후보를 잔뜩 탐색하게 되지만, 오른쪽에서 시작하면 대부분의 요소를 첫 검사에서 탈락시킬 수 있다.

여기서 나오는 실무 지침이 **"가장 오른쪽 선택자(key selector)를 구체적으로 쓰라"**는 것이다.
`div * { }` 같은 규칙은 모든 요소를 후보로 만들어 최악이다.
다만 현대 브라우저의 스타일 매칭은 충분히 빨라서, 선택자 성능이 병목이 되는 경우는 드물다.
선택자를 단순하게 쓰는 진짜 이유는 성능보다 **명시도 관리와 가독성**에 가깝다.
...
- `<head>`, `<meta>`, `<script>`, `<title>` 같은 **비시각적 노드는 렌더 트리에 없다.**
- `display: none`인 노드와 **그 자손 전체**가 빠진다.
- `::before` / `::after` 같은 **가상 요소는 DOM에 없지만 렌더 트리에는 있다.** 그래서 `querySelector`로 잡히지 않고
  이벤트 리스너도 붙일 수 없다. 스타일 값만 `getComputedStyle(el, '::before')`로 읽을 수 있다.
- 인라인 요소와 블록 요소가 섞이면 브라우저가 **익명 박스(anonymous box)**를 끼워 넣기도 한다. DOM에는 대응하는 노드가 없다.
...
페이드 애니메이션에는 `opacity`를 쓰되 끝나고 `visibility:hidden`을 함께 걸어 클릭·스크린 리더 노출을 막는다.

`opacity: 0`만 쓴 요소가 여전히 클릭된다는 점은 실제 버그로 자주 나온다.
"안 보이는데 버튼이 눌린다"는 제보의 단골 원인이다.

### 흔한 오해

"`display:none`인 요소는 DOM에도 없다"는 말은 틀리다. **DOM에는 그대로 있다.**
그래서 `document.querySelector()`로 찾을 수 있고 `textContent`도 읽힌다.
다만 렌더 트리에 없으니 `offsetWidth`나 `getBoundingClientRect()`는 전부 0을 돌려준다.
숨겨진 요소의 크기를 재려다 0이 나오는 문제가 여기서 비롯된다.
...
`element.textContent = 'x'` 한 줄이 비싼 게 아니다. 비용은 그 변경이 **파이프라인의 어느 단계부터 다시 돌게 만드느냐**에서 온다.
...
브라우저는 똑똑해서 한 프레임 안의 변경을 모아 마지막에 한 번만 처리한다.
문제는 그 배칭을 개발자가 깨뜨릴 때 생긴다(자세한 내용은 [04-reflow-repaint.md](./04-reflow-repaint.md)).
...
**왜 문제인가**: `innerHTML +=`는 세 가지 일을 한다. ① 현재 자식 전체를 문자열로 직렬화하고,
② 문자열을 이어 붙인 뒤, ③ **기존 노드를 전부 버리고 처음부터 다시 파싱해 만든다.**
1000번 반복하면 이 파괴-재생성이 1000번 일어난다.
게다가 기존 노드에 걸어 둔 이벤트 리스너와 입력값 같은 상태가 **매번 날아간다.**
...
문자열 조립은 빠르지만, 값이 사용자 입력에서 온다면 HTML로 해석되어 XSS 위험이 있다.
그래서 데이터가 신뢰할 수 없을 땐 노드를 직접 만드는 쪽이 안전하다.
...
삽입 후 fragment는 빈 상태가 되고, 문서에는 `<li>`들만 들어간다. 껍데기가 남지 않는 것이 `<div>`로 감싸는 방식과 다른 점이다.
...
**live collection**이라 문서가 바뀌면 내용도 실시간으로 바뀐다. 위 코드는 요소를 제거하는 동시에
길이가 줄어들어 절반만 처리된다. 게다가 `length`를 읽을 때마다 문서를 다시 훑는 비용도 든다.
...
**언제 뭘 쓰나**: 특별한 이유가 없으면 `querySelectorAll`. live collection은 "지금 이 순간 조건을 만족하는 요소"를
계속 추적해야 하는 드문 경우에만 의미가 있다.
...
- **프레임워크의 역할**: React·Vue는 상태가 바뀔 때마다 DOM을 직접 만지지 않고, 메모리에서 이전/다음 구조를 비교해
  **바뀐 부분만 모아 한 번에** 적용한다. 이 문서에서 말한 "모았다가 한 번에"를 프레임워크가 대신 해 주는 셈이다.
  다만 diffing 자체에도 비용이 있어 항상 직접 조작보다 빠른 것은 아니다. 진짜 가치는 속도보다 선언적 모델에 있다.
- **`<template>` 요소**: 내용이 파싱은 되지만 렌더링되지 않는 표준 요소다. `content`가 `DocumentFragment`이므로
  반복 렌더링의 원본으로 쓰기 좋다.
- **`:where()`로 명시도 낮추기**: 라이브러리 기본 스타일처럼 "사용자가 쉽게 덮어쓰길 바라는" 규칙에 유용하다.
  `:where(...)`는 명시도가 0으로 계산되므로 사용자의 단순한 클래스 선택자로도 덮을 수 있다.
- **CSS Modules / CSS-in-JS**: 클래스명을 고유하게 만들어 명시도 경쟁 자체를 없앤다.
  `!important` 남발이 시작되는 지점을 구조적으로 차단하는 접근이다.
- **DevTools Computed 탭**: 어떤 규칙이 이겼고 무엇이 취소선으로 밀렸는지 그대로 보여준다. 명시도 문제는 추측하지 말고 여기서 확인한다.
...
- 꼬리 질문: "`visibility:hidden`은 어떻게 되나요?" → 렌더 트리에 남아 공간을 차지하되 그려지지만 않는다.
  그래서 숨기고 나타낼 때 레이아웃이 다시 계산되지 않는 이점이 있다.
...
앞자리가 크면 뒷자리와 무관하게 이기고, 같으면 나중에 선언된 규칙이 적용됩니다.
인라인 스타일은 그보다 위, `!important`는 그 위입니다. `!important`를 남용하면 이 체계가 무너져
덮어쓰려면 또 `!important`가 필요해지므로 유지보수가 급격히 나빠집니다.
...
- 꼬리 질문: "`innerHTML +=`가 왜 특히 나쁜가요?" → 기존 자식을 전부 직렬화하고 다시 파싱해 재생성하므로
  이벤트 리스너와 입력 상태가 매번 사라지고 작업량도 누적된다.
...
DOM 조작의 비용은 노드 생성이 아니라 그 변경이 되돌리는 파이프라인의 길이에서 나온다.
