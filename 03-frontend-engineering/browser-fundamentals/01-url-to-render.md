...
> 주소창에 주소를 치고 Enter를 누른 순간부터 첫 픽셀이 찍히기까지, 브라우저가 어떤 순서로 무엇을 하는지 단계별로 설명할 수 있게 된다.
...
- 네트워크 계층을 더 깊게 보고 싶다면 [DNS 동작 원리](../../01-computer-science-fundamentals/network/04-dns-resolution.md),
  ...
  [SSL/TLS 핸드셰이크](../../01-computer-science-fundamentals/network/05-ssl-tls-handshake.md)를 함께 본다.

---

## 1. 왜 이 질문이 단골인가

"주소창에 URL을 입력하면 무슨 일이 일어나나요?"는 프론트엔드·백엔드를 가리지 않고 나오는 질문이다.
답을 외웠는지 보려는 게 아니다. 이 질문 하나에 **DNS·TCP·TLS·HTTP·브라우저 렌더링**이 전부 걸려 있어서,
지원자가 어느 계층까지 이해하고 있는지 한 번에 드러나기 때문이다.

그래서 "DNS 조회하고 TCP 연결하고 HTML 받아서 그립니다"로 끝내면 감점이다.
각 단계가 **어떤 문제를 풀려고 존재하는지**까지 말할 수 있어야 한다. 이 문서는 그 흐름을 따라간다.
...
**비유**: 국제 택배와 비슷하다. 받는 사람 이름(도메인)만으로는 배송이 안 되니 주소록에서 실제 주소(IP)를 찾고,
배송 경로를 확보하고(TCP), 내용물이 새지 않게 봉인하고(TLS), 물건을 주고받은 뒤(HTTP), 받은 부품으로 조립한다(렌더링).
**비유의 한계**: 택배는 한 번에 하나를 배송하지만, 브라우저는 HTML을 받는 즉시 그 안의 CSS·JS·이미지를 **병렬로** 다시 요청한다.
1~5단계는 리소스 하나마다 반복되며, 6단계와도 겹쳐서 진행된다.
...
주소창에 들어온 문자열은 아직 아무 의미가 없는 텍스트다. 브라우저는 이걸 규격에 맞춰 조각낸다.
...
표의 마지막 줄이 실무에서 자주 걸리는 지점이다. `#reviews` 같은 프래그먼트는 **브라우저 안에서만** 쓰인다.
그래서 SPA에서 해시 라우팅을 쓰면 서버 로그에 어떤 화면을 봤는지 남지 않고, 액세스 토큰을 프래그먼트로 넘기는
OAuth 암묵적 흐름은 "서버 로그에 토큰이 남지 않는다"는 성질을 이용한 설계였다.

### 파싱 전에 일어나는 일들

- **검색어 판별**: `날씨`처럼 URL로 해석되지 않으면 기본 검색 엔진 질의로 넘긴다.
- **정규화**: 대소문자 정리, 퍼센트 인코딩, 한글 도메인의 퓨니코드(Punycode) 변환.
- **HSTS 확인**: 해당 도메인이 HSTS 목록에 있으면 `http://`로 입력해도 브라우저가 요청을 보내기 **전에**
  `https://`로 바꾼다. 평문 요청 자체가 나가지 않는 것이 핵심이다.
...
인터넷 통신의 실제 목적지는 `93.184.x.x` 같은 IP 주소다. 하지만 사람은 숫자를 못 외우고,
서버가 이사하면 IP가 바뀐다. 이름과 주소를 분리해 두면 **서버를 옮겨도 사용자는 같은 이름을 계속 쓸 수 있다.**
DNS는 이 이름-주소 대응을 관리하는 전 지구적 분산 데이터베이스다.

### 캐시 계층을 왜 이렇게 많이 두나

도메인 하나 볼 때마다 루트 서버까지 물어보면 왕복 시간이 쌓이고 루트 서버는 터진다.
그래서 **가까운 곳부터 순서대로 확인**하고, 어디서든 답이 나오면 즉시 반환한다.
...
각 응답에는 **TTL(Time To Live)**이 붙는다. TTL 동안은 캐시된 값을 그대로 쓴다.
그래서 서버 IP를 바꾸면 즉시 반영되지 않고, "TTL만큼 기다려야 한다"는 말이 나온다.
배포 직전에 TTL을 짧게 줄여 두는 것이 실무의 정석이다.

**비유**: 전화번호부. 자주 거는 번호는 휴대폰에 저장(브라우저 캐시)하고, 없으면 회사 내선표(라우터), 그래도 없으면 114(재귀 리졸버)에 묻는다.
**비유의 한계**: 저장된 번호는 지우기 전까지 유효하지만 DNS 캐시는 TTL이 지나면 스스로 만료된다. "한 번 찾으면 영원히"가 아니다.

---

## 4. TCP 연결 — 대화가 가능한 상태 만들기

IP는 패킷을 목적지로 보내주긴 하지만 **도착 보장도, 순서 보장도 없다.**
HTML 파일을 받는데 중간 조각이 빠지거나 순서가 뒤바뀌면 문서가 깨진다.
TCP는 그 위에서 손실 재전송·순서 재조립·흐름 제어를 담당해 **신뢰할 수 있는 바이트 스트림**을 만든다.
...
3번 왕복이 아니라 **1.5 왕복(약 1 RTT)** 이다. 세 번째 ACK는 클라이언트가 보내는 방향이라 데이터 전송과 붙어 나갈 수 있다.
서울↔미국 서버의 RTT가 150ms라면 이 악수 한 번에만 150ms가 든다. CDN이 "사용자와 가까운 곳"에 있어야 하는 첫 번째 이유다.

---

## 5. TLS 핸드셰이크 — HTTPS가 추가로 하는 일

HTTP는 평문이다. 같은 와이파이에 붙은 사람이 패킷을 들여다보면 로그인 정보가 그대로 보이고,
중간 장비가 응답에 광고 스크립트를 끼워 넣어도 브라우저는 알 수 없다. TLS는 이 셋을 막는다.
...
셋 중 **인증**이 가장 자주 빠지는 답이다. 암호화만 하고 상대가 누군지 확인하지 않으면
중간자와 안전하게 암호 통신하는 꼴이 된다.
...
세부 메시지 순서는 TLS 버전마다 다르다. 골자는 **"암호 스위트와 키 재료를 주고받고, 인증서로 상대를 확인하고,
양쪽이 같은 세션 키를 만든다"**는 것이다.

핸드셰이크만큼 왕복이 추가되므로 HTTPS는 첫 연결이 HTTP보다 느리다.
TLS 1.3은 이 왕복 수를 1 RTT로 줄였고, 이전에 접속한 적 있는 서버라면 세션 재개로 더 줄인다.
"HTTPS는 느리다"는 인식이 지금은 거의 통하지 않는 이유다. 게다가 HTTP/2·HTTP/3은 사실상 TLS 위에서만 쓰이므로,
HTTPS를 쓰지 않으면 최신 프로토콜의 성능 이득도 못 받는다.

---

## 6. HTTP 요청과 응답

연결이 뚫리면 그제서야 실제 요청이 나간다.
...
서버는 내용이 그대로면 본문 없이 `304 Not Modified`만 돌려준다. 이게 가장 저렴한 응답이다.

응답에서 브라우저가 가장 먼저 보는 건 **`Content-Type`**이다.
`text/html`이면 파싱해서 렌더링하고, `application/octet-stream`이면 다운로드 창을 띄운다.
확장자가 아니라 헤더가 결정한다는 점이 중요하다.
...
**언제 뭘 쓰나**: 선택의 문제가 아니라 서버/CDN이 지원하면 브라우저가 알아서 최신 버전을 협상한다.
개발자가 할 일은 HTTP/2 이상에서는 **파일을 잘게 쪼개 스프라이트로 합치던 옛 최적화가 오히려 손해**라는 걸 아는 것이다.
참고로 HTTP/2 서버 푸시는 기대만큼 효과가 없어 사실상 폐기됐고, 대신 `preload`를 쓴다.

---

## 7. 응답을 받은 다음 — 렌더링으로 넘어가는 지점

HTML 첫 바이트가 도착하면 브라우저는 **전체를 다 받을 때까지 기다리지 않고** 즉시 파싱을 시작한다.
...
파싱 도중 `<link rel="stylesheet">`, `<script src>`, `<img>`를 만나면 그때마다 새 리소스 요청이 나간다.
그 요청 각각이 다시 2~6단계를 거친다. 그래서 실제 타임라인은 다음처럼 겹쳐서 흐른다.
...
이 그림에서 얻을 교훈은 두 가지다.
첫째, **첫 화면이 늦는 원인은 대개 네트워크 왕복 횟수**다. 파일을 아무리 최적화해도 왕복이 많으면 느리다.
둘째, **HTML 안에서 리소스가 늦게 발견될수록 요청도 늦게 나간다.** 그래서 중요한 리소스는 `<head>` 위쪽에 두거나 `preload`로 미리 알린다.

여기서부터 뒤 이야기는 [Critical Rendering Path 문서](./02-critical-rendering-path.md)에서 이어진다.

---

## 8. 실무에서는

- **CDN**: 정적 파일을 사용자와 가까운 엣지 서버에 두면 DNS가 가리키는 IP 자체가 가까워진다. 2~5단계의 RTT가 통째로 줄어든다.
- **`preconnect` / `dns-prefetch`**: 곧 쓸 도메인의 DNS·TCP·TLS를 미리 끝내 둔다.
  ```html
  <link rel="preconnect" href="https://cdn.example.com" crossorigin>
  <link rel="dns-prefetch" href="https://analytics.example.com">
  ```
  `preconnect`는 연결까지, `dns-prefetch`는 이름 조회까지만 미리 한다. 연결은 자원을 쓰므로 정말 쓸 도메인에만 건다.
- **압축**: `Accept-Encoding: gzip, br`에 맞춰 서버가 압축해 보내면 전송 바이트가 크게 줄어든다. 텍스트 리소스(HTML/CSS/JS)에서 효과가 크다.
- **HSTS**: `Strict-Transport-Security` 헤더로 이후 접속을 강제 HTTPS로 만든다. http→https 리다이렉트 왕복 한 번도 아낀다.
- **DNS TTL 관리**: 도메인 이전이나 장애 조치(failover) 전에는 TTL을 미리 낮춰 둬야 전환이 빠르다.
...
- 꼬리 질문: "그 과정에서 가장 오래 걸리는 부분은?" → 대개 네트워크 왕복. 특히 첫 연결의 DNS+TCP+TLS 구간과,
  HTML 안에서 렌더링 차단 리소스를 발견해 추가로 왕복하는 구간이 병목이라고 답한다.
...
- 꼬리 질문: "인증서 검증은 정확히 뭘 하나요?" → 인증서 체인이 신뢰하는 루트 CA까지 이어지는지, 서명이 유효한지,
  만료되지 않았는지, 그리고 인증서의 도메인이 접속한 호스트와 일치하는지를 확인한다.
