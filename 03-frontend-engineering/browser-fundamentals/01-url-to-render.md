...
배송 경로를 확보하고(TCP), 내용물이 새지 않게 봉인하고(TLS), 물건을 주고받은 뒤(HTTP), 받은 부품으로 조립합니다(렌더링).
...
표의 마지막 줄에서 실무가 자주 걸립니다. `#reviews` 같은 프래그먼트는 **브라우저 안에서만** 쓰입니다.
SPA에서 해시 라우팅을 쓰면 서버 로그에 어떤 화면을 봤는지 남지 않습니다. OAuth 암묵적 흐름은 액세스 토큰을
프래그먼트로 넘기는데 "서버 로그에 토큰이 남지 않는다"는 성질을 이용한 설계였습니다.
...
도메인 하나 볼 때마다 루트 서버까지 물어보면 왕복 시간이 쌓이고 루트 서버는 터지므로 **가까운 곳부터 순서대로 확인**하고
...
각 응답에는 **TTL(Time To Live)**이 붙습니다. TTL 동안은 캐시된 값을 그대로 쓰므로 서버 IP를 바꿔도
즉시 반영되지 않고 "TTL만큼 기다려야 한다"는 말이 나옵니다.
실무에서는 배포 직전에 TTL을 짧게 줄여 둡니다.
...
오가는 패킷은 3개지만 왕복 횟수로는 **1.5 왕복(약 1 RTT)** 입니다. 세 번째 ACK는 클라이언트가 보내는 방향이라 데이터 전송과 붙어 나갈 수 있습니다.
...
그 요청 각각이 다시 2~6단계를 거치므로 실제 타임라인은 다음처럼 겹쳐서 흐릅니다.
