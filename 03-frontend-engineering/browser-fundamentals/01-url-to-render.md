...
지원자가 어느 계층까지 이해하고 있는지 한 번에 드러납니다.
...
**비유**: 국제 택배와 비슷합니다. 받는 사람 이름(도메인)만으로는 배송이 안 되니 주소록에서 실제 주소(IP)를 찾습니다.
배송 경로를 확보하고(TCP), 내용물이 새지 않게 봉인합니다(TLS). 물건을 주고받은 뒤(HTTP)에는 받은 부품으로 조립합니다(렌더링).
**비유의 한계**: 택배는 한 번에 하나만 배송합니다. 하지만 브라우저는 HTML을 받는 즉시 그 안의 CSS·JS·이미지를 **병렬로** 다시 요청합니다.
1~5단계는 리소스 하나마다 되풀이되고, 6단계와도 겹쳐서 진행됩니다.
...
표의 마지막 줄은 실무에서 자주 걸리는 부분입니다. `#reviews` 같은 프래그먼트는 **브라우저 안에서만** 쓰입니다.
SPA에서 해시 라우팅을 쓰면 서버 로그에 어떤 화면을 봤는지 남지 않습니다. OAuth 암묵적 흐름은 액세스 토큰을
프래그먼트로 넘겼는데, "서버 로그에 토큰이 남지 않는다"는 성질을 이용한 설계였습니다.
...
- **HSTS 확인**: 해당 도메인이 HSTS 목록에 있으면 `http://`로 입력해도 브라우저가 요청을 보내기 **전에**
  `https://`로 바꿉니다. 평문 요청 자체가 아예 나가지 않습니다.
...
도메인 하나 볼 때마다 루트 서버까지 물어보면 왕복 시간이 쌓이고 루트 서버는 터집니다. 그래서 **가까운 곳부터 순서대로 확인**합니다.
어디서든 답이 나오면 즉시 반환합니다.
...
각 응답에는 **TTL(Time To Live)**이 붙습니다. TTL 동안은 캐시된 값을 그대로 쓰기 때문에 서버 IP를 바꿔도 곧바로 반영되지 않습니다.
"TTL만큼 기다려야 한다"는 말이 여기서 나옵니다. 실무에서는 배포 직전에 TTL을 짧게 줄여 둡니다.
...
IP는 패킷을 목적지까지 보내 주지만 **도착 보장도, 순서 보장도 없습니다.**
HTML 파일을 받는데 중간 조각이 빠지거나 순서가 뒤바뀌면 문서가 깨집니다.
이 빈틈은 그 위의 TCP가 메웁니다. 손실 재전송·순서 재조립·흐름 제어를 맡아 **신뢰할 수 있는 바이트 스트림**을 만들어 줍니다.
...
오가는 패킷은 3개입니다. 그래도 왕복 횟수로 세면 **1.5 왕복(약 1 RTT)** 입니다. 세 번째 ACK는 클라이언트가 보내는 방향이라 데이터 전송과 붙어 나갈 수 있습니다.
...
셋 중 **인증**이 가장 자주 빠집니다. 암호화만 하고 상대가 누군지 확인하지 않으면
...
`Host` 헤더 덕분에 IP 하나에 여러 도메인을 올릴 수 있습니다(가상 호스팅).
`If-None-Match`가 붙었다면 브라우저에 이미 캐시본이 있다는 뜻입니다.
서버는 내용이 그대로면 본문 없이 `304 Not Modified`만 돌려줍니다. 이보다 저렴한 응답은 없습니다.

응답에서 브라우저가 가장 먼저 보는 건 **`Content-Type`**입니다.
`text/html`이면 파싱해서 렌더링하고, `application/octet-stream`이면 다운로드 창을 띄웁니다.
확장자가 무엇이든 처리 방식은 헤더가 결정합니다.
...
**언제 뭘 쓰나**: 서버/CDN이 지원하면 브라우저가 알아서 최신 버전을 협상하므로 개발자가 버전을 고를 일은 없습니다.
다만 HTTP/2 이상에서는 **파일을 잘게 쪼개 스프라이트로 합치던 옛 최적화가 오히려 손해**라는 건 알아 둬야 합니다.
참고로 HTTP/2 서버 푸시는 기대만큼 효과가 없어 사실상 폐기됐습니다. 대신 `preload`를 씁니다.
...
둘째, **HTML 안에서 리소스가 늦게 발견될수록 요청도 늦게 나갑니다.** 중요한 리소스는 `<head>` 위쪽에 두거나 `preload`로 미리 알립니다.
...
- 꼬리 질문: "그 과정에서 가장 오래 걸리는 부분은?" → 대개 네트워크 왕복. 특히 첫 연결의 DNS+TCP+TLS 구간이 병목입니다.
  HTML 안에서 렌더링 차단 리소스를 발견해 추가로 왕복하는 구간도 병목이라고 답합니다.
...
URL 입력부터 화면까지는 **이름을 주소로 바꾸고(DNS) → 연결을 만들고(TCP/TLS) → 문서를 받아(HTTP) → 픽셀로 바꾸는(렌더링)** 네 가지 일이 이어지는 과정입니다.
