...
> 렌더링 파이프라인의 마지막 단계인 합성이 무엇인지 알고, `transform`/`opacity` 애니메이션이 왜 싼지, `will-change`를 언제 쓰고 언제 쓰면 안 되는지 판단할 수 있게 된다.
...
[리플로우와 리페인트](./04-reflow-repaint.md)에서 "레이아웃을 줄여라"까지 왔다.
그런데 애니메이션은 화면 갱신 주기마다 그림을 바꿔야 한다. 흔한 60Hz 화면이라면 초당 60번,
프레임 하나에 허용된 시간은 약 16.7ms다. 아무리 레이아웃을 줄여도 매 프레임 페인트까지 다시 하면
이 예산을 지키기 어렵다.

그래서 브라우저는 다른 접근을 택했다. **화면을 여러 장의 그림으로 나눠 미리 그려 두고,
움직일 때는 그림 자체를 다시 그리는 대신 위치만 바꿔 겹친다.** 이게 합성이다.

**비유**: 옛날 셀 애니메이션이 딱 이 방식이다. 배경은 한 장 그려 놓고, 캐릭터만 투명 필름에 그려
프레임마다 필름을 조금씩 옮긴다. 배경을 매번 다시 그리지 않으니 작업량이 확 준다.
**비유의 한계**: 필름은 종이 몇 장 값이지만, 브라우저 레이어는 **GPU 메모리**를 먹는다.
필름을 무한정 늘리면 오히려 느려진다는 게 이 비유가 놓치는 부분이다.

---

## 2. 페인트와 래스터, 그리고 합성

"Paint 단계에서 픽셀이 만들어진다"는 설명은 조금 거칠다. 실제로는 두 단계로 나뉜다.
...
두 코드는 눈에 보이는 결과가 같지만, 매 프레임 하는 일이 다르다.
...
**언제 뭘 쓰나**: 움직임·크기 변화·투명도 변화를 애니메이션할 때는 `transform`과 `opacity`만 쓴다.
`left`/`top`/`width`/`height`/`margin`은 애니메이션 대상으로 삼지 않는다.

### 흔한 오해: "transform은 무조건 공짜"

아니다. 두 가지 조건이 붙는다.

첫째, 그 요소가 **독립 레이어로 승격되어 있어야** 페인트를 건너뛴다.
레이어가 아니면 `transform`을 바꿔도 자신이 속한 레이어를 다시 그려야 한다.
다만 브라우저는 `transform`/`opacity`를 애니메이션하는 요소를 대체로 알아서 승격시킨다.

둘째, `transform`의 **어떤 값**을 바꾸느냐가 아니라 **어떻게 바꾸느냐**가 스레드를 가른다.
...
JS 방식도 레이아웃과 페인트는 건너뛰지만, 메인 스레드가 다른 작업으로 막히면 프레임을 놓친다.
**"CSS로 표현할 수 있는 애니메이션은 CSS로"**가 권장되는 진짜 이유가 이것이다.

---

## 4. "GPU 가속"이 실제로 의미하는 것

이 표현은 오해를 많이 낳는다. GPU가 CSS를 해석하거나 레이아웃을 계산하는 게 아니다.
실제로 일어나는 일은 이렇다.

1. 레이어의 비트맵을 **GPU 텍스처로 업로드**한다.
2. 화면을 만들 때 GPU가 텍스처들을 **변환 행렬과 알파 값에 따라 합성**한다.
3. `transform`이 바뀌면 텍스처는 그대로 두고 **행렬만 바꿔** 다시 합성한다.

즉 GPU가 잘하는 건 **"이미 만들어진 이미지를 옮기고, 돌리고, 확대하고, 투명하게 겹치는 일"**이다.
이건 GPU 하드웨어가 원래 하던 작업(텍스처 매핑)이라 대량으로 병렬 처리된다.
반대로 "이 텍스트를 이 폰트로 그려라" 같은 작업은 여전히 래스터 단계의 몫이다.
...
`transform`과 `opacity`가 특별한 이유도 여기서 나온다. 이 둘은 **텍스처 내용을 바꾸지 않고**
합성 파라미터(행렬, 알파)만 바꾸는 속성이라, 3번 단계만으로 끝난다.
`background-color`는 텍스처 내용 자체가 달라지므로 래스터를 다시 해야 한다.
...
"페이지는 멈췄는데 스크롤은 되더라" 하는 경험이 여기서 나온다.
반대로 말하면, 애니메이션을 합성 스레드로 넘기는 것은 단순한 속도 향상이 아니라
**메인 스레드 지연에 대한 내성**을 얻는 일이다.
...
마지막 항목이 사고를 많이 낸다. 합성 레이어 위에 다른 요소가 겹치면,
겹침 순서를 지키기 위해 **그 요소도 레이어로 승격**될 수 있다. 레이어 하나를 만들었더니
연쇄적으로 수십 개가 생기는 현상을 **레이어 폭발(layer explosion)**이라 부른다.
...
레이어를 준비하게 하는 힌트다. 레이어 승격에는 텍스처 생성 비용이 드는데,
그 비용을 애니메이션 첫 프레임에 치르면 시작이 뚝 끊긴다. `will-change`는 그 비용을 앞당긴다.
...
**왜 문제인가**: 모든 요소가 레이어가 되면 GPU 메모리가 폭증한다.
레이어 하나의 비트맵은 대략 `가로 × 세로 × 픽셀당 4바이트`이고, 고해상도 화면에서는 배율만큼 더 커진다.
전체 화면 크기의 레이어 몇 장이면 수십 MB가 우습게 나가고, 메모리가 부족한 기기에서는
오히려 프레임이 떨어지거나 탭이 죽는다.
또 `will-change`에 여러 속성을 나열하면 브라우저가 최적화 대상을 좁힐 수 없어 힌트의 의미가 사라진다.
...
레이어를 계속 유지한다. 애니메이션이 끝났는데도 메모리를 붙잡고 있는 셈이다.
...
**왜 문제인가**: 이 값은 "3D 변환이니 레이어를 만들라"는 부수 효과를 노린 것이지, 의도를 표현하지 않는다.
코드를 읽는 사람은 왜 이게 있는지 알 수 없고, 브라우저도 "곧 바뀔 것"이라는 정보를 받지 못해
`will-change`만큼 똑똑하게 대응하지 못한다.
...
`will-change: transform`은 `transform`을 실제로 지정한 것과 같은 부수 효과를 일부 가진다.
그중 실무에서 걸리는 것이 **`position: fixed`인 자손의 기준이 뷰포트가 아니라 이 요소가 되는 것**이다.
...
"툴팁이 갑자기 엉뚱한 데 붙는다"는 버그의 흔한 원인이다. 새 쌓임 맥락(stacking context)도 만들어지므로
`z-index` 계산도 달라진다. 성능 힌트라고 생각하고 붙였다가 레이아웃이 깨지는 일이 실제로 생긴다.
...
`Ctrl/Cmd + Shift + P` → "Show Rendering"으로 연다.
...
합성만으로 처리되지 않고 있다는 뜻이므로, 원인(레이어 미승격, 다른 속성 동시 변경 등)을 찾아야 한다.

### Layers 패널

Chrome DevTools의 Layers 패널은 페이지의 레이어를 3D로 펼쳐 보여주고,
각 레이어를 클릭하면 **크기, 메모리 추정치, 그리고 이 레이어가 만들어진 이유(compositing reason)**를 알려준다.
"왜 이 요소가 레이어가 됐지?"에 대한 답을 추측하지 않고 확인할 수 있는 곳이다.
...
애니메이션 구간에서 Main 줄에 `Layout`이나 `Paint`가 반복해서 찍힌다면 합성 최적화가 실패한 것이다.

---

## 8. 실무에서는

- **모달·드로어·토스트**: 등장/퇴장 애니메이션은 `transform: translateY()` + `opacity`로만 구성한다.
  `height`를 0에서 늘리는 방식은 매 프레임 레이아웃이라 반드시 끊긴다.
- **스크롤 연동 효과**: 스크롤 이벤트에서 `top`을 바꾸는 대신, `transform`을 바꾸거나
  `position: sticky` 같은 선언적 수단을 먼저 검토한다. 브라우저가 합성 스레드에서 처리할 여지가 커진다.
- **긴 리스트**: 항목 수천 개를 DOM에 두면 합성 이전에 레이아웃·페인트 대상 자체가 커진다.
  화면에 보이는 만큼만 유지하는 가상 스크롤이 먼저다. 합성 최적화는 그다음이다.
- **텍스트 품질**: 합성 레이어로 승격된 요소는 배경이 투명할 경우 텍스트 안티에일리어싱 품질이 떨어져
  글자가 살짝 흐릿해 보일 수 있다. 애니메이션이 끝나면 `will-change`를 해제하는 편이 시각적으로도 낫다.
- **모바일**: GPU 메모리가 데스크톱보다 훨씬 적다. 데스크톱에서 멀쩡하던 레이어 수십 개가
  저사양 기기에서는 그대로 프레임 드랍이 된다. 성능 확인은 반드시 실제 기기나 CPU 스로틀링을 켜고 한다.
...
- 꼬리 질문: "`transform`이면 항상 페인트가 없나요?" → 아니다. 해당 요소가 독립 레이어로 승격돼 있어야 한다.
  승격되지 않았다면 속한 레이어를 다시 그려야 한다.
...
`transform`과 `opacity`만 이 단계만으로 처리되기 때문에 애니메이션의 기본 도구가 된다.
단, 레이어는 공짜가 아니므로 `will-change`는 필요한 곳에만 걸고 끝나면 해제한다.
