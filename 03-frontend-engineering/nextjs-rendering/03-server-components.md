...
> "왜 글자만 보여주는 컴포넌트의 코드까지 브라우저가 다운로드해야 하는가"라는 질문에서 출발해, 서버 전용 컴포넌트가 무엇을 바꾸는지와 `'use client'` 경계를 어떻게 설계하는지 설명할 수 있게 된다.
...
- [02-hydration.md](./02-hydration.md) - 하이드레이션 비용을 알아야 RSC의 동기가 이해된다
...
이 컴포넌트에는 클릭도 상태도 이벤트도 없다. 결과물은 그냥 HTML 덩어리다. 그런데 전통적인 SSR에서는 두 라이브러리가 **전부 클라이언트 번들에 포함된다.** 서버에서 이미 HTML을 다 만들었어도, 브라우저가 하이드레이션을 하려면 같은 컴포넌트를 한 번 더 실행해야 하고 그러려면 이 코드가 필요하기 때문이다.

즉 사용자는 **한 번 실행된 결과를 받고, 똑같은 결과를 다시 만들기 위한 코드를 또 받는다.**

### 문제 2 — 데이터를 가져오려고 만드는 계층

CSR 시대에는 브라우저가 DB에 직접 접근할 수 없으니 반드시 사이에 API가 있어야 했다.

```
컴포넌트 → fetch('/api/posts') → API 라우트 핸들러 → DB
           └─ 옮기기만 하는데도 타입 정의·에러 처리·스키마 검증이 각각 필요
```
게다가 컴포넌트가 렌더된 다음에야 데이터를 요청하므로, 중첩된 컴포넌트가 각자 데이터를 가져오면 요청 폭포(waterfall)가 생긴다. 부모가 로딩을 끝내야 자식이 요청을 시작하는 구조다.

### RSC의 답

**컴포넌트를 서버에서만 실행하고 그 코드는 클라이언트로 보내지 않는다.** 이 한 문장이 두 문제를 동시에 푼다. 코드가 안 가니 번들이 줄고, 서버에서 실행되니 DB에 직접 접근할 수 있다.

> **비유** — 이사 견적을 낼 때 업체가 직원을 우리 집에 보내는 대신, 사무실에서 계산을 끝내고 완성된 견적서만 보내는 것과 같다. 계산에 쓴 요금표와 계산기는 우리 집에 올 필요가 없다.
>
> **비유의 한계** — 견적서는 최종 결과물이지만 RSC의 결과물은 그렇지 않다. 서버 컴포넌트가 보내는 것은 "여기에 이 클라이언트 컴포넌트를 이런 props로 놓아라"라는 지시가 섞인 트리 구조이고, 클라이언트는 그걸 받아 자기 몫의 컴포넌트를 실행한다. 서버와 클라이언트가 하나의 트리를 나눠 그린다.

---

## 2. RSC는 무엇을 보내나

서버 컴포넌트의 렌더 결과는 HTML이 아니라 **RSC Payload**라 부르는 직렬화된 트리다.
...
첫 요청에서는 이 payload와 함께 SSR로 만든 HTML도 같이 내려간다. 그래서 첫 화면은 여전히 빠르고 SEO도 된다. 이후 라우트 이동에서는 HTML 없이 payload만 받아 기존 트리에 병합한다. 브라우저가 문서를 다시 로드하는 게 아니라 React가 바뀐 부분만 갈아 끼우는 것이라, 이동 전후로 유지되는 공통 레이아웃의 클라이언트 상태(열려 있는 사이드바, 재생 중인 플레이어 등)는 그대로 살아남는다. 반대로 교체되는 페이지 구간의 상태는 남지 않는다.
...
**한 줄 결론** — SSR은 "언제 렌더하나", RSC는 "어디에서만 존재하나"를 정한다. 둘은 배타적이지 않고 App Router는 서버 컴포넌트를 SSR로 HTML화해 첫 응답에 함께 보내므로, "RSC가 SSR을 대체한다"는 말은 틀렸다.
...
**그래서 언제 뭘 쓰나** — 데이터 조회와 정적 마크업은 Server Component, 상태·이벤트·브라우저 API가 필요한 지점만 Client Component. 판단 기준은 "사용자 입력에 반응해야 하는가" 하나면 충분하다.

### 3.2 `'use client'`는 컴포넌트 표시가 아니라 **모듈 경계**다

RSC에서 가장 자주 틀리는 부분이다. `'use client'`는 파일 맨 위에 쓰는 지시어이고, **그 파일부터 시작해 import로 이어지는 모든 모듈이 클라이언트 번들에 들어간다.**
...
**규칙**: `'use client'`는 트리의 가능한 한 **말단(leaf)** 에 둔다. 상호작용이 필요한 버튼 하나 때문에 페이지 전체를 클라이언트로 만들지 않는다.

### 3.3 Client Component 안에 Server Component를 넣는 법

"클라이언트 컴포넌트 안에는 서버 컴포넌트를 못 넣는다"고 알려져 있는데, 정확히는 **import는 안 되지만 props로 받는 것은 된다.**
...
**왜 문제인가** — `PostBody`가 마크다운 파서를 쓰고 있었다면 그 라이브러리까지 통째로 클라이언트 번들에 끌려 들어간다. 서버 컴포넌트로 만든 이유가 사라진다.

**개선 — `children` 슬롯으로 받는다.**
...
`PostBody`는 서버에서 렌더되고 `Collapsible`은 그 **결과**를 `children`으로 받아 위치만 잡아 준다. 마크다운 파서는 클라이언트로 가지 않는다. 이 패턴은 컨텍스트 프로바이더에도 그대로 쓰인다. 프로바이더 자체는 클라이언트여야 하지만 그 안의 트리는 서버로 남길 수 있다.

---

## 4. 직렬화 제약

서버에서 클라이언트 컴포넌트로 넘기는 props는 네트워크를 건너가므로 **직렬화 가능해야 한다.**
...
**흔한 오해 — "JSON으로 직렬화 가능한 것만 된다"**는 틀렸다. RSC는 `JSON.stringify`보다 넓은 포맷을 쓴다. `Date`를 넘기면 클라이언트에서도 `Date` 객체로 받고, `Map`과 `Set`도 그대로 살아 있으며, `Promise`를 넘겨 클라이언트에서 읽는 것도 된다. "JSON만 된다"고 외우면 `Date`를 굳이 문자열로 바꿔 넘기는 불필요한 코드를 쓰게 된다.
...
**왜 문제인가** — 함수는 코드와 클로저 환경을 함께 가지는데, 그 환경은 서버 메모리에만 있다. 네트워크로 보낼 방법이 없다. 서버 컴포넌트를 처음 쓸 때 가장 먼저 부딪히는 벽이다.

**개선 1 — 클라이언트 로직이라면 클라이언트 쪽에 정의한다.**
...
**개선 2 — 서버에서 실행돼야 하는 일이라면 Server Function으로 만든다.**
...
`'use server'`가 붙은 함수는 실제 코드가 넘어가는 게 아니라 **참조(엔드포인트 주소)** 가 넘어간다. 클라이언트가 그걸 호출하면 네트워크 요청이 나가고 함수 본문은 서버에서 실행된다.

### `'use server'`는 Server Component 선언이 아니다

이름 때문에 `'use client'`의 반대라고 생각하기 쉬운데 아니다. `'use client'`는 **모듈 경계** 선언이라 그 파일부터 클라이언트 번들에 포함되고, `'use server'`는 **함수 선언**이라 그 함수가 서버에서 실행되면서 클라이언트에서 호출 가능해진다(Server Function / Server Action). 하나는 번들 경계를 긋고 다른 하나는 호출 규약을 만든다.

App Router에서 **서버 컴포넌트는 기본값이라 아무 지시어도 필요 없다.** 서버 컴포넌트 파일 위에 `'use server'`를 붙이면 그 파일의 export를 전부 Server Function으로 취급하려 해서 오히려 에러가 난다.
...
**왜 문제인가** — 두 가지다. 첫째, props는 RSC payload에 실려 브라우저로 전송되므로 **`passwordHash`가 페이지 소스에 그대로 남는다.** 화면에 안 그린다고 안 가는 게 아니다. 둘째, 필요 없는 필드까지 실려 payload가 커진다.

**개선 — 서버에서 필요한 필드만 뽑아 넘긴다.**
...
`lib/db.ts`가 커넥션 문자열을 들고 있는데 어쩌다 클라이언트 컴포넌트에서 import되면 그 값이 번들에 들어갈 수 있다. 사람이 주의하는 것으로는 막을 수 없다.

**개선 — `server-only` 패키지를 import해 두면 클라이언트 번들에 포함되는 순간 빌드가 실패한다.** 런타임 사고를 빌드 타임 에러로 앞당기는 장치다.
...
가장 큰 실질적 차이는 **데이터를 가져올 수 있는 위치**다.
...
"같은 데이터를 여러 컴포넌트가 각자 가져오면 중복 요청 아닌가?" — 이 걱정을 없애 주는 게 다음 절의 Request Memoization이다.

중첩 레이아웃도 실무 차이가 크다. `app/dashboard/layout.tsx`를 두면 대시보드 하위 라우트를 오갈 때 사이드바가 다시 렌더되지 않고 그대로 유지된다. Pages Router에서는 직접 구현해야 했다.

---

## 7. Next.js 캐싱 계층

> **버전 주의** — 캐싱 기본값은 메이저 버전 사이에 바뀌었다. Next.js 13/14에서는 `fetch`가 기본적으로 캐시됐지만 **15부터는 캐시하지 않는 것이 기본**이며, 캐시하려면 `cache: 'force-cache'`나 `next: { revalidate }`를 명시해야 한다. 클라이언트 Router Cache의 기본 보존 시간도 버전에 따라 다르다. 계층의 **구조는 그대로**이니 구조를 이해하고 기본값은 쓰는 버전 문서를 확인하는 게 맞다.
...
**한 줄 결론** — 서버 쪽 세 개는 "서버가 일을 덜 하게", 클라이언트 쪽 하나는 "전환이 즉각적으로 느껴지게" 하는 장치다. 이 중 6절의 걱정을 해소하는 건 Request Memoization이다.
...
세 번 호출했지만 **실제 네트워크 요청은 한 번**이다. 같은 URL과 옵션이면 첫 호출 결과를 나머지가 공유한다. 그래서 prop drilling 없이 각 컴포넌트가 필요한 데이터를 직접 가져오는 설계가 성립한다.

다만 이 중복 제거는 `fetch`에 걸린 것이다. ORM 쿼리나 파일 읽기처럼 `fetch`를 거치지 않는 조회는 세 번 호출하면 세 번 실행되므로, 같은 효과를 원하면 React의 `cache()`로 감싼다.
...
호출 단위는 `fetch` 옵션으로, 라우트 단위는 `page.tsx` / `layout.tsx` 상단의 export로 제어한다.
...
온디맨드 무효화는 Server Function이나 Route Handler 안에서 호출한다. 아래는 별도 파일이다.
...
**시간 기반과 태그 기반의 선택 기준** — 갱신 시점을 우리가 알 수 없으면(외부 API 데이터 등) 시간 기반, 우리가 데이터를 바꾸는 주체라면(관리자 저장, CMS 발행) 태그 기반이 정확하고 낭비가 없다. 실무에서는 안전망으로 넉넉한 `revalidate`를 걸어 두고 실제 갱신은 태그로 처리하는 조합을 자주 쓴다.

---

## 8. 실무에서는

- **마이그레이션은 라우트 단위로 한다.** App Router와 Pages Router는 한 프로젝트에 공존할 수 있어서, 트래픽이 적은 라우트부터 옮기며 검증하는 방식이 일반적이다.
- **경계 설계의 실전 규칙**: 페이지와 레이아웃은 서버 컴포넌트로 두고 데이터 조회를 맡긴다. 버튼·폼·모달·캐러셀처럼 상태를 가진 것만 별도 파일로 빼서 `'use client'`를 붙인다. 상태 관리 라이브러리의 프로바이더는 클라이언트일 수밖에 없지만, `children`으로 서버 트리를 감싸면 하위는 서버로 유지된다. `'use client'` 하나를 잘못된 위치에 붙이면 수십 개 모듈이 딸려 들어가는데 코드만 봐서는 안 보이므로, 번들 분석 도구로 주기적으로 확인하는 습관이 필요하다.
- **RSC에서 DB에 직접 접근할 수 있다는 게 곧 그렇게 하라는 뜻은 아니다.** 컴포넌트가 ORM 쿼리에 직접 결합되면 인가 검사와 트랜잭션 경계가 화면 코드에 흩어지고 재사용도 어려워진다. 데이터 접근은 서비스 계층 함수로 감싸고 컴포넌트는 그것만 호출하는 편이 낫다. Spring에서 컨트롤러가 Repository를 직접 호출하지 않고 Service를 두는 것과 같은 이유다.
...
- 꼬리 질문: "RSC의 장점은?" → 무거운 라이브러리를 서버에만 두어 번들이 줄고, 컴포넌트에서 백엔드 리소스에 직접 접근할 수 있어 데이터 전용 API 계층이 줄고, `'use client'` 경계에서 코드 분할이 자동으로 일어난다.
...
- 꼬리 질문: "클라이언트 컴포넌트 안에서 서버 컴포넌트를 import하면 어떻게 되나요?" → 그 서버 컴포넌트가 클라이언트 컴포넌트로 바뀌어 딸린 라이브러리까지 번들에 들어간다. `children`으로 넘기면 서버에서 렌더된 결과만 자리에 들어가므로 서버로 유지된다.
...
- 꼬리 질문: "JSON 직렬화 가능한 값만 되나요?" → 아니다. RSC 포맷은 `JSON.stringify`보다 넓어 `Date`, `Map`, `Set`, `Promise`도 그대로 전달된다. 다만 props는 payload에 실려 브라우저로 가므로, 화면에 그리지 않는 필드라도 민감 정보는 서버에서 걸러야 한다.
...
- 꼬리 질문: "시간 기반과 태그 기반을 어떻게 나눠 쓰나요?" → 갱신 시점을 우리가 모르면 시간 기반, 우리가 데이터를 바꾸는 주체면 태그 기반. 태그 쪽이 정확하고 불필요한 재생성이 없다.
- 꼬리 질문: "버전 차이가 있나요?" → 구조는 같지만 기본값이 달라졌다. Next.js 13/14는 `fetch`가 기본 캐시였고 15부터는 캐시하지 않는 것이 기본이다.
...
RSC는 "이 컴포넌트의 코드가 브라우저에 있을 이유가 있는가"를 되묻는 기술이고, 그 답이 아니오인 부분을 서버에 남겨 번들과 데이터 계층을 동시에 줄인다. 설계의 전부는 `'use client'` 경계를 얼마나 아래에 두느냐에 달려 있다.
