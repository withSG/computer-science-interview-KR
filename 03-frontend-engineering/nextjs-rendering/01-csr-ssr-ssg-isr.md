...
App Router에서는 "이 페이지는 SSR입니다"라고 선언하지 않습니다. **Next.js가 코드를 보고 정적/동적을 자동으로 판단합니다.** 판정의 축은 요청에 의존하는 API를 쓰느냐입니다. `cookies()`, `headers()`, `searchParams` 접근처럼 요청이 있어야만 값이 정해지는 것을 쓰면 동적, 아니면 정적입니다. (캐시하지 않는 데이터 조회가 라우트를 동적으로 만드는지는 버전에 따라 다르게 다뤄져 왔으니, 판정 결과는 `next build` 출력으로 확인하는 게 확실하다.)
