...
600줄짜리 주문 페이지에서 배송지 표시 형식을 바꿔야 한다고 해 봅시다. 600줄을 읽으며 관련 코드를 찾아야 하고 고친 뒤에는 상품 목록이나 결제 요약에 영향이 갔는지 확인해야 합니다. `AddressSection`이 따로 있었다면 40줄만 읽고 40줄만 확인하면 됩니다. **컴포넌트를 나눈다는 것은 "여기까지가 한 덩어리다"라고 선을 긋는 일**이고, 그 선이 잘 그어져 있으면 변경할 때 읽어야 할 코드가 줄어듭니다.
...
3. 이 컴포넌트가 실제로 어떻게 보이는지 알려면 `isAdmin`과 `compact` 값을 머릿속에서 대입해가며 JSX를 따라가야 합니다. **읽는 사람이 조립해야 합니다.**
...
"컴포넌트는 100줄을 넘기지 말자" 같은 규칙은 다루기 쉬워 보이지만 잘못된 분리를 유도합니다. 300줄짜리 컴포넌트를 줄 수만 맞춰 세 개로 쪼개면 컴포넌트 셋이 생기는데 서로 상태를 주고받느라 props가 얽힙니다. 읽어야 할 코드는 오히려 늘어납니다.
...
컴포넌트 설계는 **변경할 때 읽어야 할 코드를 줄이는 일**입니다. 재사용을 늘리는 일이 아닙니다. 그래서 잘 나누는 것보다 성급하게 나누지 않는 것이 먼저이고 props가 늘어나기 시작했다면 조립 책임을 사용처로 되돌려야 합니다.
