...
> 컴포넌트를 언제 나누고 언제 나누지 말아야 하는지, props가 늘어나는 컴포넌트를 합성으로 어떻게 되돌리는지, Context를 어디까지 써야 하는지 설명할 수 있게 된다.
...
"컴포넌트로 나누면 재사용할 수 있다"는 설명은 절반만 맞다. 실제 프로젝트에서 재사용되는 컴포넌트는 버튼, 입력창, 모달 정도이고 나머지 대부분은 **한 곳에서만 쓰인다.** 그런데도 우리는 계속 컴포넌트를 나눈다. 이유는 다른 데 있다. **변경 비용을 낮추기 위해서**다.

600줄짜리 주문 페이지에서 배송지 표시 형식을 바꿔야 한다고 하자. 600줄을 읽으며 관련 코드를 찾아야 하고, 고친 뒤에는 상품 목록이나 결제 요약에 영향이 갔는지 확인해야 한다. `AddressSection`이 따로 있었다면 40줄만 읽고 40줄만 확인하면 된다. **컴포넌트를 나눈다는 것은 "여기까지가 한 덩어리다"라고 선을 긋는 일**이고, 그 선이 잘 그어져 있으면 변경할 때 읽어야 할 코드가 줄어든다.

그런데 나누는 것은 쉽지만, 잘못 나눈 것을 합치는 일은 훨씬 어렵다. 이미 여러 곳에서 쓰이고 있고 각 사용처가 조금씩 다른 기대를 하고 있기 때문이다. 그래서 컴포넌트 설계의 첫 번째 규칙은 "잘 나누는 법"이 아니라 **"성급하게 나누지 않는 법"**이다.

### 비유: 방을 가르는 벽

컴포넌트 경계는 집 안에 세우는 벽이다. 벽이 있으면 한 방을 새로 칠할 때 옆방에 페인트가 튀지 않는다. 대신 벽을 잘못 세우면, 이를테면 싱크대와 가스레인지 사이에 벽이 지나가면 요리할 때마다 문을 드나들어야 한다. 그리고 한번 세운 벽은 허무는 것보다 세우는 게 훨씬 싸다.

> **비유의 한계**: 벽은 물리적 공간을 실제로 막지만 컴포넌트 경계는 그렇지 않다. props와 Context, 전역 스토어를 통해 값이 경계를 자유롭게 넘나들 수 있어서, **파일을 나눴다고 관심사가 나뉘었다고 믿는 것**이 이 주제에서 가장 흔한 착각이다.
...
관리자 화면과 사용자 화면에 비슷하게 생긴 카드가 있다. "코드가 거의 같으니 합치자"는 생각이 자연스럽게 든다.
...
1. **우연한 중복이었다.** 두 카드가 비슷했던 이유는 지금 요구사항이 우연히 겹쳤기 때문이지 같은 개념이어서가 아니다. 관리자 화면과 사용자 화면은 서로 다른 이유로, 서로 다른 시점에 바뀐다. 하나로 묶으면 한쪽이 바뀔 때마다 다른 쪽을 검증해야 한다.
2. **플래그가 기하급수로 늘어난다.** 불리언 props가 4개면 가능한 조합이 16가지고 실제로 쓰이는 것은 2~3가지다. 나머지 13가지는 아무도 테스트하지 않은 채 코드에 남는다.
3. **읽는 사람이 조립해야 한다.** 이 컴포넌트가 실제로 어떻게 보이는지 알려면 `isAdmin`과 `compact` 값을 머릿속에서 대입해가며 JSX를 따라가야 한다.
4. **되돌리기 어렵다.** 관리자 카드에만 필요한 기능이 생겨도 이미 여러 곳에서 쓰이므로 분리가 큰 작업이 된다.
...
`Card`와 `UserAvatar`는 **어떤 화면에서 쓰이든 똑같이 동작하는 것**만 담았다. 화면마다 다른 부분은 각 화면이 직접 조립한다. 코드 줄 수는 늘었지만, 두 화면이 서로에게 영향을 주지 않는다.

경험적 규칙이 두 가지 있다. 같은 코드가 **세 번째** 나타났을 때 추출하는 Rule of Three, 그리고 실제로 두 번째 호출부가 생겼을 때 추출하는 "두 번째 사용처" 원칙이다. 무엇을 따르든 핵심은 같다. **미래를 예측해서 추상화하지 않는다.** 예측이 빗나가면 잘못된 추상화가 남고, 그 비용은 중복보다 크다.
...
"컴포넌트는 100줄을 넘기지 말자" 같은 규칙은 다루기 쉬워 보이지만 잘못된 분리를 유도한다. 300줄짜리 컴포넌트를 줄 수만 맞춰 세 개로 쪼개면, 서로 상태를 주고받느라 props가 얽힌 세 개의 컴포넌트가 생긴다. 읽어야 할 코드는 오히려 늘어난다.
...
**JSX에 구역 주석을 달고 있다면** — `{/* 배송지 영역 */}` 같은 주석을 달았다면 이미 그 자리가 경계다. 주석 대신 컴포넌트 이름을 붙이면 된다. **조건부 렌더링 블록이 통째로 다르다면** — `if (isEditing)` 위아래가 완전히 다른 화면이라면 두 컴포넌트다. **리렌더 단위가 다르다면** — 매초 갱신되는 타이머와 거의 안 바뀌는 상품 정보가 한 컴포넌트에 있으면 상품 정보도 매초 다시 그려진다. 성능 문제로 드러나기 전에 분리 신호로 읽는 편이 낫다.
...
클래스 컴포넌트 시절에는 **상태 로직을 재사용할 방법이 마땅치 않았다.** 데이터 페칭 로직을 여러 컴포넌트에서 쓰려면 고차 컴포넌트(HOC)나 render props를 써야 했고, 둘 다 중첩이 깊어지면 읽기 어려웠다. 그래서 나온 타협이 **데이터를 다루는 컨테이너와 렌더링만 하는 프레젠테이션을 분리**하는 것이었다. 로직 재사용은 어렵더라도 최소한 렌더링 부분은 재사용하고 테스트할 수 있게 하자는 발상이다.

### 훅이 바꾼 것

`useState`와 `useEffect`가 나오면서 **상태 로직 자체를 함수로 뽑아 재사용**할 수 있게 됐다. 커스텀 훅이 컨테이너의 역할을 대신하게 된 것이다.
...
"파일을 `containers/`와 `components/`로 나눈다"는 형태의 강제는 사실상 사라졌다. 이 패턴을 널리 알린 Dan Abramov 본인도 원 글에 훅 이후로는 더 이상 권하지 않는다는 주석을 달았다.

다만 **한 가지 가치는 남았다. props만 받아 렌더링하는 컴포넌트는 테스트와 스토리북이 압도적으로 쉽다.** 데이터를 가져오는 컴포넌트를 테스트하려면 네트워크를 흉내 내야 하지만, props만 받는 컴포넌트는 값을 넘기면 끝이다. 그래서 실무의 균형점은 **폴더로 강제하지는 않되, 재사용하거나 여러 상태(로딩·빈 목록·에러)를 눈으로 확인해야 하는 UI는 props만 받는 순수한 형태로 만들어두는 것**이다.

---

## 5. 합성 vs props 폭발

모달을 하나 만들고 요구사항이 올 때마다 props를 하나씩 늘리면 이렇게 된다.
...
**의존 방향이 뒤집혔다.** 원래는 사용처가 `Modal`을 알면 되는데, 이제 `Modal`이 모든 사용처의 요구를 알아야 한다. 새로운 화면에서 푸터에 체크박스를 넣고 싶으면 `Modal`을 고쳐야 하고, 그 수정이 기존 열 곳에 영향을 준다. 부수적으로 `Modal` 내부는 `{icon && ...}` 같은 조건문으로 가득 차고, `footerAlign`과 `hideFooterDivider`가 동시에 켜졌을 때의 모양은 아무도 확인한 적 없는 상태로 남는다.
...
조각들이 부모의 상태를 알아야 할 때가 있다. 탭이 대표적이다. 각 탭 버튼은 "지금 어느 탭이 선택됐는지"를 알아야 한다. 이때 Context를 **그 컴포넌트 내부 통신 용도로만** 쓴다.
...
Context의 범위가 `Tabs` 안쪽으로 제한된다는 점이 중요하다. 탭을 바꾸면 Provider의 value가 새 객체가 되어 `Tabs.Tab`과 `Tabs.Panel`이 함께 리렌더되지만, **경계가 컴포넌트 트리로 그어져 있어 그 여파가 `Tabs` 하위를 벗어나지 않는다.** 다음에 볼 전역 Context의 리렌더 문제와 갈리는 지점이 여기다.
...
> 결론: **입력하는 동안 무언가를 해야 하면 제어, 제출할 때만 값이 필요하면 비제어.** 필드가 수십 개인 폼에서 비제어 방식을 쓰는 이유는 타이핑마다 폼 전체가 리렌더되는 것을 피하기 위해서다. React Hook Form이 기본적으로 비제어 방식을 택한 것도 같은 이유다.

### 재사용 컴포넌트는 둘 다 지원한다

라이브러리 성격의 컴포넌트를 만들 때는 사용처가 어느 쪽을 원할지 모른다. 표준 `<input>`이 쓰는 방식 — `value`가 오면 제어, `defaultValue`만 오면 비제어 — 을 그대로 따르면 된다.
...
props를 두세 단계 내려보내는 것은 **의존 관계가 시그니처에 그대로 드러난다는 장점**이 있다. 어떤 컴포넌트가 무엇을 필요로 하는지 함수 선언만 봐도 안다. Context나 전역 스토어로 옮기면 이 정보가 사라진다. 문제가 되는 것은 **중간 컴포넌트들이 쓰지도 않는 값을 그저 아래로 전달만 하는 경우**, 그것도 다섯 단계쯤 이어질 때다.
...
`Layout`과 `Sidebar`는 이제 `user`라는 개념 자체를 모른다. **JSX 엘리먼트도 값이라서 props로 넘길 수 있다**는 점을 이용한 것으로, drilling의 상당수는 이 방법으로 사라진다.
...
| 상황 | Context 적합도 | 이유 |
|------|--------------|------|
| 테마, 로케일, 인증된 사용자 정보 | 적합 | 거의 안 바뀐다. 바뀌면 어차피 화면 전체가 바뀌어야 한다 |
...
> 결론: **Context는 "값 전달 수단"이지 "상태 관리 도구"가 아니다.** 자주 바뀌는 값을 공유해야 한다면 셀렉터로 구독을 좁힐 수 있는 상태 라이브러리를 쓰는 편이 낫다. 자세한 내용은 [01-state-management.md](./01-state-management.md)를 참고할 만하다.

---

## 8. 디자인 시스템 관점의 컴포넌트 API

디자인 시스템의 컴포넌트는 수십 곳에서 쓰인다. **API를 한 번 잘못 열면 되돌리는 비용이 일반 컴포넌트와 비교가 안 된다.**
...
**왜 문제인가**: 래핑한 의미가 사라진다. 호출부가 `color="secondary"` 같은 MUI 고유 props를 직접 쓰기 시작하면, 나중에 다른 라이브러리로 바꿀 때 호출부 수백 곳을 전부 고쳐야 한다. 결합을 한 곳에 가두려고 래핑했는데 결합이 그대로 새어 나간 것이다.
...
`primary`, `danger`는 **디자인 시스템의 용어**지 MUI의 용어가 아니다. 라이브러리를 교체해도 호출부의 이 이름은 그대로 남고, 고칠 곳은 `TONE_MAP` 하나다. 반대로 앞의 안티패턴처럼 `MuiButtonProps`를 그대로 노출하면 호출부가 `contained`나 `error` 같은 MUI 어휘를 직접 쓰기 시작하고, 그 순간 대응표를 둘 자리가 사라진다.

### 둘째, 탈출구를 어디까지 열 것인가

`className`이나 `style`을 열어두면 급할 때 편하지만 디자인 시스템을 우회하는 코드가 쌓인다. 실무의 절충안은 여백처럼 **화면마다 다를 수밖에 없는 것**은 열어두고, 색·타이포그래피처럼 **일관성이 핵심인 것**은 닫아두는 것이다. 열어둔 탈출구로 같은 우회가 세 번 나오면 정식 옵션으로 승격시킨다. 위 예제라면 `Tone`에 값을 하나 추가하는 식이다.

**props 개수 자체보다 props의 성격이 중요하다.** `tone`, `size`처럼 의미를 담은 props는 값이 늘어도 대응표 한 곳에서 관리되지만, `hideBorder`, `footerAlign` 같은 표현 기반 불리언은 서너 개만 넘어도 조합을 감당할 수 없게 된다.

---

## 9. 실무에서는

- **스토리북(Storybook)이 설계 압력으로 작동한다.** 컴포넌트를 스토리로 만들려면 props만으로 모든 상태를 재현할 수 있어야 하는데, 내부에서 데이터를 가져오는 컴포넌트는 이게 안 된다. 스토리를 쓰기 힘든 컴포넌트는 대개 책임이 섞여 있다는 신호다.
- **shadcn/ui 계열의 접근이 늘고 있다.** 라이브러리를 의존성으로 설치하는 대신 컴포넌트 소스를 프로젝트에 복사해 두고 직접 고치는 방식으로, props를 무한히 열어 모든 요구를 수용하려는 시도를 포기하고 "필요하면 소스를 고쳐라"로 방향을 튼 것으로 볼 수 있다.
- **접근성이 API 설계를 좌우한다.** 위 `Toggle` 예제의 `role="switch"`, `aria-checked`처럼 재사용 컴포넌트는 ARIA 속성을 내부에서 처리해야 한다. 호출부마다 이걸 기억하게 만들면 반드시 누락된다. Radix UI나 React Aria 같은 헤드리스 라이브러리는 동작과 접근성만 제공하고 스타일을 비워두는 방식으로 이 문제를 다룬다.
...
- 꼬리 질문: "한 컴포넌트가 500줄이면 무조건 나눠야 하나요?" → 하나의 이유로만 바뀌는 폼이라면 나누는 게 오히려 손해일 수 있다고 답한다. 다만 대개는 그 안에 여러 관심사가 섞여 있다.
...
- 꼬리 질문: "이미 props가 많아진 컴포넌트는 어떻게 하나요?" → children과 슬롯을 이용한 합성으로 바꾼다고 답한다. 무엇을 넣을지 결정하는 책임을 사용처로 되돌리는 방향이다.
...
- 꼬리 질문: "재사용 컴포넌트는 어느 쪽으로 만들어야 하나요?" → 표준 `<input>`처럼 `value`가 오면 제어, `defaultValue`만 오면 비제어로 동작하도록 둘 다 지원한다고 답한다.
...
- 꼬리 질문: "Context를 여러 개로 나누면 해결되나요?" → 리렌더 범위 문제는 상당히 해결되지만 Provider 중첩이 깊어진다고 답한다. Tabs 같은 합성 컴포넌트 내부 통신은 범위가 좁아 Context가 잘 맞는 사례다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|------|----------|-----------|
| "코드가 중복이면 무조건 합친다" | 겉모습이 같아도 변경 이유가 다르면 서로 다른 개념이다 | 세 번째 등장까지 기다린다. 중복은 잘못된 추상화보다 싸다 |
...
컴포넌트 설계는 재사용을 늘리는 일이 아니라 **변경할 때 읽어야 할 코드를 줄이는 일**이고, 그래서 잘 나누는 것보다 성급하게 나누지 않는 것이 먼저이며, props가 늘어나기 시작했다면 조립 책임을 사용처로 되돌릴 때다.
