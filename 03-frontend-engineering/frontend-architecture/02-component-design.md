...
> 먼저 정할 것은 나눌지 말지입니다. 컴포넌트를 언제 나누고 언제 나누지 말아야 하는지, props가 늘어나는 컴포넌트를 합성으로 어떻게 되돌리는지, Context를 어디까지 써야 하는지를 설명합니다.
...
- React 컴포넌트와 props, `useState` 사용 경험. [01-state-management.md](./01-state-management.md)를 먼저 읽으면 좋지만 필수는 아닙니다
...
600줄짜리 주문 페이지에서 배송지 표시 형식을 바꿔야 한다고 해 봅시다. 600줄을 읽으며 관련 코드를 찾아야 하고, 고친 뒤에는 상품 목록이나 결제 요약에 영향이 갔는지도 확인해야 합니다. `AddressSection`이 따로 있었다면 40줄만 읽고 40줄만 확인하면 됩니다. **컴포넌트를 나눈다는 것은 "여기까지가 한 덩어리다"라고 선을 긋는 일**이고, 그 선이 잘 그어져 있으면 변경할 때 읽어야 할 코드가 줄어듭니다.

그런데 나누는 것은 쉽지만, 잘못 나눈 것을 합치는 일은 훨씬 어렵습니다. 이미 여러 곳에서 쓰이고 있고 각 사용처가 조금씩 다른 기대를 하고 있기 때문입니다. 그래서 컴포넌트 설계의 첫 번째 규칙은 **성급하게 나누지 않는 법**입니다. 잘 나누는 법은 그다음입니다.
...
> **비유의 한계**: 벽은 물리적 공간을 실제로 막습니다. 컴포넌트 경계는 그렇지 않습니다. props와 Context, 전역 스토어로 값이 경계를 자유롭게 넘나들 수 있어서, **파일을 나눴다고 관심사가 나뉘었다고 믿는 것**이 이 주제에서 가장 흔한 착각입니다.
...
1. **우연한 중복이었습니다.** 두 카드가 비슷했던 것은 지금 요구사항이 우연히 겹친 결과입니다. 같은 개념이어서가 아닙니다. 관리자 화면과 사용자 화면은 서로 다른 이유로, 서로 다른 시점에 바뀝니다. 하나로 묶으면 한쪽이 바뀔 때마다 다른 쪽을 검증해야 합니다.
2. **플래그가 기하급수로 늘어납니다.** 불리언 props가 4개면 가능한 조합이 16가지이고, 실제로 쓰이는 것은 2~3가지입니다. 나머지 13~14가지는 아무도 테스트하지 않은 채 코드에 남습니다.
3. 이 컴포넌트가 실제로 어떻게 보이는지 알려면 `isAdmin`과 `compact` 값을 머릿속에서 대입해 가며 JSX를 따라가야 합니다. **읽는 사람이 조립해야 합니다.**
...
경험적 규칙이 두 가지 있습니다. 같은 코드가 **세 번째** 나타났을 때 추출하는 Rule of Three, 그리고 실제로 두 번째 호출부가 생겼을 때 추출하는 두 번째 사용처 원칙입니다. 어느 쪽을 따르든 다르지 않습니다. **미래를 예측해서 추상화하지 않습니다.** 예측이 빗나가면 잘못된 추상화가 남고, 그 비용은 중복보다 큽니다.
...
"컴포넌트는 100줄을 넘기지 말자" 같은 규칙은 다루기 쉬워 보이지만 잘못된 분리를 유도합니다. 300줄짜리 컴포넌트를 줄 수만 맞춰 세 개로 쪼개면, 컴포넌트 셋이 서로 상태를 주고받느라 props가 얽힙니다. 읽어야 할 코드는 오히려 늘어납니다.

단일 책임 원칙(SRP, Single Responsibility Principle)은 흔히 "하나의 일만 한다"고 옮깁니다. 원문에 더 가까운 표현은 "**변경할 이유가 하나뿐이어야 한다**"입니다. 컴포넌트에 적용하면 이렇게 됩니다.
...
**JSX에 구역 주석을 달고 있다면** — `{/* 배송지 영역 */}` 같은 주석을 달았다면 이미 그 자리가 경계입니다. 주석 대신 컴포넌트 이름을 붙이면 됩니다. **조건부 렌더링 블록이 통째로 다르다면** — `if (isEditing)` 위아래가 완전히 다른 화면이라면 두 컴포넌트입니다. **리렌더 단위가 다르다면** — 타이머는 매초 갱신되는데 상품 정보는 거의 안 바뀝니다. 둘이 한 컴포넌트에 있으면 상품 정보까지 매초 다시 그려집니다. 성능 문제로 드러나기 전에 분리 신호로 읽는 편이 낫습니다.
...
"파일을 `containers/`와 `components/`로 나눈다"는 형태의 강제는 사실상 사라졌습니다. 이 패턴을 널리 알린 Dan Abramov 본인도 원 글에 주석을 달았습니다. 훅 이후로는 더 이상 권하지 않는다는 내용입니다.

다만 **한 가지 가치는 남았습니다. props만 받아 렌더링하는 컴포넌트는 테스트와 스토리북이 압도적으로 쉽습니다.** 데이터를 가져오는 컴포넌트를 테스트하려면 네트워크를 흉내 내야 하지만, props만 받는 컴포넌트는 값을 넘기면 끝입니다. 그래서 실무에서는 폴더로 강제하지는 않되, 재사용하거나 여러 상태(로딩·빈 목록·에러)를 눈으로 확인해야 하는 UI는 **props만 받는 순수한 형태로 만들어두는 쪽으로 균형을 잡습니다.**
...
**의존 방향이 뒤집혔습니다.** 원래는 사용처가 `Modal`을 알면 되는데, 이제 `Modal`이 모든 사용처의 요구를 알아야 합니다. 새로운 화면에서 푸터에 체크박스를 넣고 싶으면 `Modal`을 고쳐야 하고, 그 수정이 기존 열 곳에 영향을 줍니다. 부수적으로 `Modal` 내부는 `{icon && ...}` 같은 조건문으로 가득 차고, `footerAlign`과 `hideFooterDivider`를 동시에 켜면 어떤 모양이 되는지는 아무도 확인해 본 적이 없습니다.
...
조각들이 부모의 상태를 알아야 할 때가 있습니다. 탭이 대표적입니다. 각 탭 버튼은 지금 어느 탭이 선택됐는지 알아야 합니다. 이때 Context를 **그 컴포넌트 내부 통신 용도로만** 씁니다.
...
Context의 범위가 `Tabs` 안쪽으로 제한된다는 게 핵심입니다. 탭을 바꾸면 Provider의 value가 새 객체가 되고, `Tabs.Tab`과 `Tabs.Panel`이 함께 리렌더됩니다. 하지만 **경계가 컴포넌트 트리로 그어져 있어 그 여파가 `Tabs` 하위를 벗어나지 않습니다.** 다음에 볼 전역 Context의 리렌더 문제와 갈리는 지점이 여기입니다.
...
> 결론: **입력하는 동안 무언가를 해야 하면 제어, 제출할 때만 값이 필요하면 비제어.** 필드가 수십 개인 폼에서 비제어 방식을 쓰는 이유는 타이핑마다 폼 전체가 리렌더되는 것을 피하기 위해서입니다. React Hook Form도 같은 이유로 비제어 방식을 기본으로 택했습니다.

### 재사용 컴포넌트는 둘 다 지원한다

라이브러리 성격의 컴포넌트를 만들 때는 사용처가 어느 쪽을 원할지 모릅니다. 표준 `<input>`이 쓰는 방식을 그대로 따르면 됩니다. `value`가 오면 제어, `defaultValue`만 오면 비제어입니다.
...
props를 두세 단계 내려보내는 것은 **의존 관계가 시그니처에 그대로 드러난다는 장점**이 있습니다. 어떤 컴포넌트가 무엇을 필요로 하는지 함수 선언만 봐도 압니다. Context나 전역 스토어로 옮기면 이 정보가 사라집니다. **중간 컴포넌트들이 쓰지도 않는 값을 그저 아래로 전달만 할 때**, 그것도 다섯 단계쯤 이어질 때 문제가 됩니다.
...
`Layout`과 `Sidebar`는 이제 `user`라는 개념 자체를 모릅니다. **JSX 엘리먼트도 값이라서 props로 넘길 수 있습니다.** 이 성질을 이용하면 drilling의 상당수가 사라집니다.

### Context가 맞는 경우와 안 맞는 경우

Context에는 결정적인 제약이 하나 있습니다. **값이 바뀌면 구독하는 모든 컴포넌트가 리렌더됩니다.** 값의 일부만 구독하는 방법이 없습니다.
...
[하나의 큰 Context]                    [분리된 Context]
 AppContext = { user, theme, cart }     UserContext  ──> Header
   ...
   cart가 바뀌면 구독한 셋 다 리렌더
```
-->

**얼마나 자주 바뀌느냐**로 판단하면 됩니다.
...
> 결론: **Context는 "값 전달 수단"이지 "상태 관리 도구"가 아닙니다.** 자주 바뀌는 값을 공유해야 한다면 상태 라이브러리를 쓰는 편이 낫습니다. 셀렉터로 구독 범위를 좁힐 수 있기 때문입니다. 자세한 내용은 [01-state-management.md](./01-state-management.md)를 참고할 만합니다.
...
**왜 문제인가**: 래핑한 의미가 사라집니다. 호출부가 `color="secondary"` 같은 MUI 고유 props를 직접 쓰기 시작하면, 나중에 다른 라이브러리로 바꿀 때 호출부 수백 곳을 전부 고쳐야 합니다. 결합을 한 곳에 가두려고 래핑했는데, 결합이 그대로 새어 나갔습니다.
...
`primary`, `danger`는 **디자인 시스템의 용어**입니다. MUI의 용어가 아닙니다. 라이브러리를 교체해도 호출부의 이 이름은 그대로 남고, 고칠 곳은 `TONE_MAP` 하나입니다. 반대로 앞의 안티패턴처럼 `MuiButtonProps`를 그대로 노출하면 호출부가 `contained`나 `error` 같은 MUI 어휘를 직접 쓰기 시작하고, 그 순간 대응표를 둘 자리가 사라집니다.

### 둘째, 탈출구를 어디까지 열 것인가

`className`이나 `style`을 열어두면 급할 때 편하지만 디자인 시스템을 우회하는 코드가 쌓입니다. 실무에서는 여백처럼 **화면마다 다를 수밖에 없는 것**은 열어두고 색·타이포그래피처럼 **일관성이 핵심인 것**은 닫아두는 쪽으로 절충합니다. 열어둔 탈출구로 같은 우회가 세 번 나오면 정식 옵션으로 승격시킵니다. 위 예제라면 `Tone`에 값을 하나 추가하는 식입니다.
...
- **shadcn/ui 계열**의 접근이 늘고 있습니다. 라이브러리를 의존성으로 설치하는 대신 컴포넌트 소스를 프로젝트에 복사해 두고 직접 고치는 방식입니다. props를 무한히 열어 모든 요구를 수용하려는 시도를 포기하고 "필요하면 소스를 고쳐라"로 방향을 튼 것으로 볼 수 있습니다.
...
A. 성급하게 추상화하지 않는 것입니다. 겉모습이 비슷하다는 이유로 두 컴포넌트를 합치면 우연한 중복을 진짜 공통점으로 착각하게 되고, 이후 한쪽 요구사항이 바뀔 때마다 불리언 props가 하나씩 늘어납니다. props 네 개면 조합이 열여섯 가지인데 실제로 쓰는 건 두세 가지입니다. 잘못된 추상화를 되돌리는 비용이 중복을 유지하는 비용보다 크기 때문에, 세 번째 사용처가 나올 때까지 기다리는 편이 안전합니다.
...
컴포넌트 설계는 **변경할 때 읽어야 할 코드를 줄이는 일**입니다. 재사용을 늘리는 일이 아닙니다. 그래서 잘 나누는 것보다 성급하게 나누지 않는 것이 먼저입니다. props가 늘어나기 시작했다면 조립 책임을 사용처로 되돌려야 합니다.
