...
### 핵심 답변
**모놀리식**은 모든 기능이 하나의 서비스에 통합된 단순한 구조입니다. 초기 개발은 빠르지만 규모가 커질수록 확장이 어렵습니다. **마이크로서비스(MSA)**는 도메인별로 서비스를 분리해 독립 배포가 가능한 대신 복잡도가 높습니다. **BFF(Backend For Frontend)**는 프론트엔드를 위한 전용 백엔드 레이어로, 여러 마이크로서비스의 인증과 데이터를 통합합니다.
...
### 핵심 답변
BFF는 **프론트엔드를 위한 전용 백엔드 레이어**입니다. 여러 마이크로서비스와 프론트엔드가 직접 통신하면 인증/권한 처리가 흩어지고 API 호출이 복잡해집니다. BFF는 이 문제를 풀려고 인증/권한 처리를 한 곳으로 모으고, 여러 서비스의 데이터를 집계(aggregation)해서 프론트엔드에 맞는 형태로 내려 줍니다.
...
### 핵심 답변
프론트엔드 상태는 **서버 상태, 전역 클라이언트 상태, 로컬 상태, URL 상태** 4가지로 나누고, 각각 다른 도구로 관리하면 효과적입니다. 서버 상태는 React Query로 캐싱과 동기화를 선언적으로 처리하고, 전역 클라이언트 상태(인증, 테마 등)는 Zustand/Redux로 관리합니다. UI 관련 로컬 상태는 useState로 충분합니다. 검색어·필터·페이지 번호처럼 새로고침이나 링크 공유 후에도 같은 화면이어야 하는 값은 URL 상태로 둡니다.
...
### 핵심 답변
폴더 구조는 최상위를 **feature 기반 구조**로 잡고, 각 feature 내부는 **layer 기반**으로 나누는 혼합형을 권장합니다. 최상위는 `/features/auth`, `/features/dashboard` 등 도메인별로 나누고, 내부는 `components/`, `hooks/`, `api/`, `types/`로 구성합니다. 여러 feature에서 공통으로 쓰는 것은 `/shared`에 둡니다.
...
### 핵심 답변
MUI, Ant Design 같은 외부 UI 라이브러리 컴포넌트를 그대로 쓰지 않고 **프로젝트의 디자인 시스템에 맞게 래핑(wrapping)해서 사용하는 전략**입니다. 디자인이 바뀌거나 라이브러리를 교체할 때 래핑 레이어만 고치면 되고, 사용하는 쪽은 영향을 받지 않습니다. 디자인 시스템의 일관성을 지키려면 필요한 작업입니다.
...
### 핵심 답변
비즈니스 로직을 어디에 둘지는 **재사용 여부**로 정합니다. 한 컴포넌트에서만 쓰는 로직은 컴포넌트 내부에 두고, 여러 컴포넌트가 공유하는 로직은 커스텀 훅으로 뺍니다. UI와 무관한 순수 비즈니스 로직은 유틸 함수나 서비스 레이어로 분리합니다. 기준이 되는 원칙은 **"두 번째 사용처가 생길 때 추상화한다"**입니다.
...
### 핵심 답변
프론트엔드에서 의존성 방향은 **UI Layer → Business Logic Layer → Data Access Layer** 단방향으로 유지해야 합니다. 상위 레이어는 하위 레이어에만 의존하고, 하위 레이어가 상위 레이어를 알면 안 됩니다. 이 원칙을 지키면 API 응답 구조가 바뀌어도 Data Access Layer만 고치면 되고, UI를 리디자인해도 비즈니스 로직은 그대로입니다.
...
### 핵심 답변
관심사 분리(Separation of Concerns)는 **UI 렌더링, 비즈니스 로직, 데이터 접근을 각각 독립적인 단위로 분리**하는 설계 원칙입니다. React에서는 Presentational/Container 패턴이나 커스텀 훅으로 구현합니다. 나누고 나면 각 부분을 따로 테스트할 수 있고, 바뀌었을 때 영향 범위도 줄일 수 있습니다.
