...
> 폴더 구조는 규모가 커지면 어디서 깨지는지, 기능별 구조가 기술별 구조를 왜 이기는지, 의존성 방향을 어떤 도구로 강제하는지 설명할 수 있게 됩니다. 마이크로 프론트엔드를 도입해도 되는 조건까지 여기서 다룹니다.
...
폴더 구조를 "찾기 쉬움"의 문제로 좁히면 답이 안 나옵니다. 요즘 에디터는 파일명 일부만 입력해도 찾아줍니다. 폴더는 변경 범위를 예측하고 기능을 통째로 삭제하려고 나눕니다. 이 기능을 고치면 어디까지 영향이 가는지 폴더만 봐도 알 수 있어야 하고 기능을 한 덩어리로 지울 수 있어야 코드베이스가 늙지 않습니다. 남는 하나가 **의존 통제**인데, 아무 곳에서 아무 곳이나 import할 수 있으면 구조는 문서상으로만 존재합니다.
...
기술별 구조도 작을 때는 오히려 명확합니다. 다만 이 구조는 "파일의 종류"로 나뉘어 있고 우리가 실제로 하는 작업은 "기능"단위입니다. 여기서 어긋납니다.
...
> 결론: **화면이 서너 개를 넘고 사람이 둘 이상이면 기능별로 갑니다.** 다만 현실에서는 혼합형으로 갑니다. 최상위를 기능으로 나누고 기능 폴더 안쪽을 기술별(`components/`, `hooks/`, `api/`)로 나누면 두 구조의 장점을 함께 가져갑니다. 이때 안쪽 기술별 폴더는 파일 수가 적어서 기술별 구조의 단점이 드러나지 않습니다.
...
디자이너와 개발자가 같은 어휘로 이야기하게 되고 재사용 단위가 명시적으로 드러납니다. 디자인 시스템을 만들 때 특히 유용합니다.
...
`features/order`가 `features/user`를 import하고 `features/user`도 `features/order`를 import하면 순환이 생깁니다. 폴더는 나뉘어 있지만 실제로는 하나의 덩어리입니다. 구체적으로는 어느 쪽도 혼자 삭제할 수 없습니다. 코드 스플리팅으로 나누려 해도 서로를 끌어당겨 한 청크가 됩니다. 모듈 평가 시점에 따라 한쪽이 `undefined`로 보이는 간헐적 버그도 생기고 한쪽만 테스트하려 해도 다른 쪽까지 전부 로드됩니다.
...
형제 기능끼리 import를 금지하면 처음엔 답답합니다. 주문 화면에서 사용자 정보가 필요한 상황은 흔하니까요. 이럴 때는 공통으로 필요한 개념을 **아래 레이어(entities)로 내립니다.** `User` 타입과 조회 로직은 양쪽이 다 쓰므로 entities에 있는 게 맞습니다. 조합은 위 레이어에서 합니다. 주문 페이지가 `features/order`와 `features/user`를 각각 가져와 붙이는 식입니다.
...
응답이 `data.data`에서 `data.result`로 바뀌거나 `user_name`이 `name`으로 바뀌면 이 필드를 쓰는 컴포넌트를 전부 찾아 고쳐야 합니다. 백엔드 변경이 UI까지 번지는 겁니다. `snake_case`, 유닉스 타임스탬프, 응답 봉투(`data.data`)는 전부 백엔드 규약입니다. 프론트의 도메인 모델이 아닌데 그대로 프론트 코드에 새어 들어옵니다. 테스트가 무거워지고 재사용도 막힙니다. 이 컴포넌트를 테스트하려면 HTTP를 흉내 내야 하고 같은 사용자 정보가 다른 화면에서도 필요하면 이 `useEffect`를 복사하게 됩니다.
...
- 중복 번들을 피하려면 각 조각이 React를 따로 들고 오지 않게 공유 설정을 맞춰야 한다
- 버전 스큐(skew) — A팀이 React를 올리고 B팀이 안 올리면 한 페이지에 두 버전이 공존한다
- **디자인 일관성** — 공통 디자인 시스템을 별도 패키지로 배포하고 버전을 관리해야 한다
- 조각 각각은 통과하는데 합치면 깨지는 상황을 잡을 통합 테스트 방법이 필요하다
- 라우팅·인증을 공유하려면 어느 조각이 라우터를 소유하고, 토큰을 어떻게 나눌지 정해야 한다
- 에러 스택이 여러 배포 산출물에 걸쳐 디버깅 난이도가 올라간다

### 필요 조건

아래를 **전부** 만족할 때만 이득이 비용을 넘습니다.

1. **조직 경계가 이미 나뉘어 있다** — 코드 크기와는 무관하다. 팀이 여럿이고 각자 오너십이 있어야 한다
