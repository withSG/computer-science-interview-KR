...
> 폴더 구조가 규모에 따라 깨지는 지점, 기능별 구조가 기술별 구조를 이기는 이유, 의존성 방향을 도구로 강제하는 방법, 그리고 마이크로 프론트엔드를 도입해도 되는 조건을 설명할 수 있게 된다.
...
- 없어도 읽을 수 있다. 폴더를 만들어본 경험이면 충분하다
...
프로젝트를 시작할 때는 어떤 구조를 써도 잘 굴러간다. 파일이 서른 개면 전부 기억할 수 있고, 어디에 무엇이 있는지 찾는 데 몇 초면 된다. 그래서 구조 논의는 대개 **이미 늦은 시점에** 시작된다.

깨지기 시작하는 증상은 매번 비슷하다.
...
이 다섯 개는 전부 같은 원인에서 나온다. **폴더가 나뉘어 있는 단위와 실제로 코드를 변경하는 단위가 어긋나 있다.**

### 구조의 목적은 "찾기 쉬움"이 아니다

폴더 구조를 파일 검색 문제로 생각하면 답이 안 나온다. 요즘 에디터는 파일명 일부만 입력해도 찾아준다. 진짜 목적은 **변경 범위를 예측 가능하게** 하고(이 기능을 고치면 어디까지 영향이 가는지 폴더만 봐도 알 수 있게), **삭제를 가능하게** 하고(기능을 통째로 지울 수 있어야 코드베이스가 늙지 않는다), **의존을 통제 가능하게** 하는 것이다. 아무 곳에서 아무 곳이나 import할 수 있으면 구조는 문서상으로만 존재한다.

### 비유: 부엌 정리

조리 도구를 "재질별"로 정리한 부엌을 상상해보자. 금속 서랍, 플라스틱 서랍, 나무 서랍. 찾기는 명확하다. 그런데 커피를 내리려면 세 서랍을 다 열어야 한다. 반면 "커피 코너, 베이킹 코너"로 나누면 한 코너만 열면 된다. 대신 계량컵처럼 여러 코너에서 쓰는 물건을 어디에 둘지 고민이 생긴다.

> **비유의 한계**: 부엌 도구는 하나뿐이라 한 곳에만 둘 수 있지만, 코드는 복사할 수 있다. 공유할지 복제할지가 선택 가능하다는 점이 코드 구조 논의를 더 어렵게 만든다.
...
기술별 구조가 잘못된 것은 아니다. **작을 때는 오히려 명확하다.** 문제는 이 구조가 "파일의 종류"로 나뉘어 있는데, 우리가 실제로 하는 작업은 "기능"단위라는 데 있다.

기획자가 "로그인에 소셜 로그인을 추가해달라"고 하면 손대야 할 곳은 컴포넌트, 훅, API 함수, 타입이다. 기술별 구조에서는 이 네 개가 서로 다른 폴더에 흩어져 있고, 각 폴더에는 관계없는 파일이 수십 개 함께 있다. **변경의 단위와 폴더의 단위가 다르다.**

더 나쁜 것은 삭제다. `components/` 안의 `ProductFilter.tsx`를 지워도 되는지 알려면 프로젝트 전체를 검색해야 한다. 확신이 안 서니 안 지운다. 이렇게 죽은 코드가 쌓인다.
...
> 결론: **화면이 서너 개를 넘고 사람이 둘 이상이면 기능별로 간다.** 다만 현실적인 답은 혼합형이다. 최상위를 기능으로 나누고, 기능 폴더 **안쪽**을 기술별(`components/`, `hooks/`, `api/`)로 나누면 두 구조의 장점을 함께 가져간다. 이때 안쪽 기술별 폴더는 파일 수가 적어서 기술별 구조의 단점이 드러나지 않는다.

### `/shared`로 올리는 기준

기능별 구조에서 유일하게 어려운 판단이다. 두 가지 질문으로 대부분 정리된다.

1. **둘 이상의 기능에서 실제로 쓰이고 있는가** — "쓰일 것 같다"는 안 된다. 실제 두 번째 사용처가 나타난 뒤 올린다.
2. **도메인 지식이 들어 있는가** — `formatDate`는 도메인을 모르니 `/shared`가 맞다. `formatOrderStatus`는 주문 도메인을 알므로 `features/order`에 남는다.

두 번째 조건이 더 중요하다. **도메인 지식이 담긴 코드를 `/shared`에 올리면 `/shared`가 모든 도메인을 아는 거대한 덩어리가 된다.** 그러면 어떤 기능을 고쳐도 `/shared`를 건드리게 되고, `/shared`를 건드리면 모든 기능을 확인해야 한다. 처음 피하려던 문제가 그대로 돌아온다.
...
Brad Frost가 제안한 UI 구성 방법론으로, 화학의 원자·분자 비유를 빌려 컴포넌트를 다섯 단계로 나눈다. `atoms`(Button, Input처럼 더 쪼갤 수 없는 최소 단위) → `molecules`(SearchBar처럼 atom 몇 개의 조합) → `organisms`(Header처럼 독립적으로 의미 있는 영역) → `templates`(데이터 없는 레이아웃) → `pages`(실제 데이터가 들어간 화면) 순이다.

**얻는 것**: 디자이너와 개발자가 같은 어휘로 이야기할 수 있고 재사용 단위가 명시적이다. 디자인 시스템을 만들 때 특히 유용하다.

**실무에서 자주 깨지는 지점**: molecule과 organism의 경계가 주관적이다. `SearchBar`에 자동완성 드롭다운이 붙으면 molecule인가 organism인가. 이 논쟁에 쓰는 시간이 구조에서 얻는 이득보다 클 때가 많다. 그래서 요즘은 다섯 단계를 다 쓰기보다 **`ui`(원자 수준의 공통 컴포넌트)와 기능별 컴포넌트 두 층으로 단순화**하는 경우가 흔하다.
...
**과설계 위험**: 화면이 다섯 개인 프로젝트에 FSD를 그대로 적용하면 `features/auth/ui/LoginForm/index.ts`처럼 경로가 깊어지고, 파일 하나 만들 때마다 "이건 feature인가 entity인가"를 판단해야 한다. 폴더 수가 실제 코드보다 빨리 늘어난다.
...
> 결론: **방법론은 규칙 모음이지 목표가 아니다.** FSD를 도입한다면 폴더 이름을 전부 따라 하기보다 "레이어 단방향 의존, 형제 슬라이스 간 import 금지" 두 규칙부터 lint로 강제하는 편이 실질적인 이득을 준다.
...
`features/order`가 `features/user`를 import하고 `features/user`도 `features/order`를 import하면 순환이 생긴다. 폴더는 나뉘어 있지만 실제로는 하나의 덩어리다. 구체적으로는 어느 쪽도 혼자 **삭제할 수 없고**, 코드 스플리팅으로 나누려 해도 서로를 끌어당겨 **한 청크가 되며**, 모듈 평가 시점에 따라 한쪽이 `undefined`로 보이는 **간헐적 버그**가 생긴다. 한쪽만 테스트하려 해도 다른 쪽까지 전부 로드된다.
...
README에 "features끼리 import하지 마세요"라고 적어두면 3개월 안에 반드시 깨진다. 리뷰에서 매번 잡기도 어렵다. lint로 막아야 한다.
...
`import/no-restricted-paths`는 `eslint-plugin-import`가 제공하는 규칙으로, `target` 폴더 안에서 `from` 폴더를 import하면 에러를 낸다. 형제 슬라이스 간 금지처럼 규칙이 많아지면 `eslint-plugin-boundaries`나 `dependency-cruiser` 같은 전용 도구가 설정을 더 간결하게 표현해준다.

상대 경로가 길어지는 문제는 별개로 alias를 잡아 해결한다. `tsconfig.json`에 `"baseUrl": "."`과 `"paths": { "@/*": ["src/*"] }`를 두면 `../../../shared/lib/formatDate`가 `@/shared/lib/formatDate`가 된다. 다만 `tsconfig`의 `paths`는 타입 검사기가 모듈을 찾는 방식만 정할 뿐이라, 번들러가 같은 매핑을 모르면 타입 검사는 통과하는데 실행이 깨진다. Vite와 webpack 모두 각자의 `resolve.alias`에 같은 매핑을 적어주거나, `vite-tsconfig-paths`처럼 `tsconfig`를 대신 읽어주는 플러그인을 붙여야 한다. Next.js처럼 프레임워크가 `tsconfig`의 `paths`를 직접 읽어주는 환경도 있으니, 쓰는 도구가 어느 쪽인지 먼저 확인하는 편이 빠르다.

### 각 기능의 공개 API를 정한다

폴더 경계를 진짜 경계로 만들려면, **밖에서 접근 가능한 것을 명시**해야 한다. 각 기능 폴더 루트에 `index.ts`를 두고 그것만 import하도록 규칙을 정하는 방식이 널리 쓰인다.
...
이렇게 하면 `features/auth` 내부 구조를 마음대로 리팩터링해도 밖이 안 깨진다. 자바에서 `public`과 `private`을 구분하는 것과 같은 발상이다.
...
1. **백엔드 변경이 UI까지 번진다** — 응답이 `data.data`에서 `data.result`로 바뀌거나 `user_name`이 `name`으로 바뀌면 이 필드를 쓰는 컴포넌트를 전부 찾아 고쳐야 한다.
2. **백엔드 사정이 프론트 코드에 새어 들어온다** — `snake_case`, 유닉스 타임스탬프, 응답 봉투(`data.data`)는 전부 백엔드 규약이지 프론트의 도메인 모델이 아니다.
3. **테스트가 무거워지고 재사용이 막힌다** — 이 컴포넌트를 테스트하려면 HTTP를 흉내 내야 하고, 같은 사용자 정보가 다른 화면에서도 필요하면 이 `useEffect`를 복사하게 된다.
...
**백엔드가 필드명을 바꿔도 고칠 곳은 `toUser` 한 함수**다. 백엔드의 Entity를 그대로 노출하지 않고 DTO로 변환해 내보내는 것과 정확히 같은 사고이며, 방향만 반대다.

---

## 6. Monolithic, MSA, BFF가 프론트에 미치는 영향

백엔드 아키텍처는 프론트엔드의 데이터 접근 계층 모양을 직접 결정한다.

**모놀리식 백엔드**에서는 서버 하나가 화면에 필요한 데이터를 대체로 한 번에 준다. 프론트의 API 계층은 단순하다.

**MSA**로 나뉘면 이야기가 달라진다. 주문 상세 화면 하나를 그리려고 주문·사용자·상품·배송 서비스를 각각 호출하고 프론트에서 합쳐야 한다. 그러면 세 가지 문제가 생긴다. 요청 왕복이 늘어 느려지고, 인증 처리가 서비스마다 반복되고, **프론트가 백엔드 내부 분할 구조를 알게 된다.** 서비스를 하나 더 쪼개면 프론트도 따라 고쳐야 한다.
...
Next.js의 Route Handler나 서버 컴포넌트가 사실상 경량 BFF 역할을 하는 경우도 많다. 프론트 팀이 별도 서버 없이 집계 계층을 가질 수 있다는 점이 이 방식의 매력이다.
...
한 화면을 **여러 팀이 각자 빌드·배포한 조각으로 조립**하는 방식이다. Webpack Module Federation처럼 런타임에 다른 번들의 모듈을 불러오는 기술이 대표적이고, iframe이나 라우트 단위 분리처럼 더 단순한 방법도 있다.

### 비용부터 본다

도입하면 다음 항목이 전부 팀의 일이 된다.
...
단일 팀이 "코드베이스가 커서" 도입하는 것은 거의 항상 손해다. 그 문제는 **모노레포와 패키지 경계, lint로 강제한 의존성 규칙**으로 대부분 해결된다. 빌드가 하나로 유지되므로 버전 스큐도 중복 번들도 없다.

바꿔 말하면 마이크로 프론트엔드는 **기술 문제의 해법이 아니라 조직 문제의 해법**이다. 조직이 나뉘지 않았는데 도입하면 조직 문제 없이 분산 시스템 비용만 치르게 된다. MSA에 대해 백엔드에서 하는 이야기와 정확히 같다.

---

## 8. 실무에서는

- **구조는 처음부터 완성하지 않는다.** 화면 두세 개일 때 FSD 폴더를 전부 만들어두면 빈 폴더만 늘어난다. `features/`와 `shared/` 둘로 시작해서, 실제로 아픈 지점이 생겼을 때 규칙을 추가하는 순서가 현실적이다.
- **경계를 새로 정하는 것보다 지키게 만드는 것이 어렵다.** 새 규칙을 도입할 때는 lint를 `error`가 아니라 `warn`으로 켜고 기존 위반을 목록화한 뒤, 새로 추가되는 코드부터 막는 방식이 마찰이 적다.
- **모노레포는 구조 문제의 흔한 다음 단계다.** pnpm workspace나 Turborepo로 디자인 시스템·유틸을 별도 패키지로 떼어내면, 폴더 규칙이 아니라 `package.json`의 의존성으로 경계가 강제된다. 어기면 lint가 아니라 빌드가 실패한다.
- **Next.js App Router는 라우팅을 파일 구조에 묶는다.** `app/` 폴더가 라우트를 결정하므로 기능별 구조와 충돌하는 것처럼 보이는데, 실무에서는 `app/`을 얇은 진입점으로 두고 실제 구현을 `features/`나 `src/`의 기능 폴더에 두는 방식이 많이 쓰인다. 라우팅 방식은 [../nextjs-rendering/01-csr-ssr-ssg-isr.md](../nextjs-rendering/01-csr-ssr-ssg-isr.md)를 함께 보면 이해가 빠르다.
...
- 꼬리 질문: "도메인 지식이 있는 공통 코드는요?" → 두 기능이 같은 도메인 개체를 다룬다면 그건 공통 유틸이 아니라 별도 도메인 계층(entities)으로 내려야 할 신호라고 답한다.
...
- 꼬리 질문: "순환 의존이 왜 위험한가요?" → 코드 스플리팅이 안 되고, 모듈 평가 순서에 따라 한쪽이 `undefined`로 보이는 간헐적 버그가 생기며, 어느 쪽도 혼자 삭제할 수 없게 된다고 답한다.
...
- 꼬리 질문: "아토믹 디자인의 단점은?" → molecule과 organism의 경계가 주관적이라 논쟁 비용이 크다고 답한다. 디자인 시스템의 공통 어휘로는 유용하지만 애플리케이션 폴더 구조로 그대로 쓰기에는 애매하다.
...
- 꼬리 질문: "MSA와 같은 논리인가요?" → 그렇다고 답한다. 둘 다 조직 문제의 해법이지 기술 문제의 해법이 아니고, 조직이 안 나뉘었는데 도입하면 분산 시스템 비용만 남는다.
...
프로젝트 구조의 목적은 **변경과 삭제의 단위를 폴더 단위와 일치시키는 것**이고, 기능별로 나누는 것이 그 출발점이며, 정해둔 의존성 방향을 lint로 강제하지 않으면 어떤 구조도 몇 달 안에 문서상으로만 남는다.
