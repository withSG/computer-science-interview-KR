...
> 폴더 이름 짓기는 다루지 않습니다. 규모가 커지면 구조가 어디서 깨지는지, 기능별 구조가 기술별 구조를 왜 이기는지, 의존성 방향을 어떤 도구로 강제하는지를 짚습니다. 마이크로 프론트엔드는 도입해도 되는 조건까지만 봅니다.
...
- 없어도 읽을 수 있습니다. 폴더를 만들어본 경험이면 충분합니다
...
폴더 구조를 "찾기 쉬움"의 문제로 좁히면 답이 안 나옵니다. 요즘 에디터는 파일명 일부만 입력해도 찾아 줍니다. 폴더는 변경 범위를 예측하고 기능을 통째로 삭제하려고 나눕니다. 이 기능을 고치면 어디까지 영향이 가는지 폴더만 봐도 알 수 있어야 하고, 기능을 한 덩어리로 지울 수 있어야 코드베이스가 늙지 않습니다. 남는 하나가 **의존 통제**인데, 아무 곳에서 아무 곳이나 import할 수 있으면 구조는 문서상으로만 존재합니다.

### 비유: 부엌 정리

조리 도구를 "재질별"로 정리한 부엌을 상상해 봅시다. 금속 서랍, 플라스틱 서랍, 나무 서랍. 찾기는 명확합니다. 그런데 커피를 내리려면 세 서랍을 다 열어야 합니다. 반면 "커피 코너, 베이킹 코너"로 나누면 한 코너만 열면 됩니다. 대신 계량컵처럼 여러 코너에서 쓰는 물건을 어디에 둘지 고민이 생깁니다.

> **비유의 한계**: 부엌에서는 도구 하나를 한 곳에만 둘 수 있습니다. 코드는 복사할 수 있습니다. 공유할지 복제할지 고를 수 있다는 점이 코드 구조 논의를 더 어렵게 만듭니다.
...
기술별 구조도 작을 때는 오히려 명확합니다. 다만 이 구조는 "파일의 종류"로 나뉘어 있는데, 우리가 실제로 하는 작업은 "기능" 단위입니다. 여기서 어긋납니다.
...
삭제는 더 곤란합니다. `components/` 안의 `ProductFilter.tsx`를 지워도 되는지 알려면 프로젝트 전체를 검색해야 합니다. 확신이 안 서니 안 지웁니다. 이렇게 죽은 코드가 쌓입니다.
...
> 결론: **화면이 서너 개를 넘고 사람이 둘 이상이면 기능별로 갑니다.** 다만 현실에서는 혼합형으로 갑니다. 최상위는 기능으로 나누고, 기능 폴더 안쪽은 기술별(`components/`, `hooks/`, `api/`)로 나눕니다. 이렇게 하면 두 구조의 장점을 함께 가져갑니다. 안쪽 기술별 폴더는 파일 수가 적어서 기술별 구조의 단점이 드러나지 않습니다.

### `/shared`로 올리는 기준

기능별 구조에서 판단이 어려운 자리는 여기 하나입니다. 다음을 따져 보면 대부분 정리됩니다.
...
둘 중에서는 두 번째를 더 봐야 합니다. **도메인 지식이 담긴 코드를 `/shared`에 올리면 `/shared`가 모든 도메인을 아는 거대한 덩어리가 됩니다.** 그러면 어떤 기능을 고쳐도 `/shared`를 건드리게 되고, `/shared`를 건드리면 모든 기능을 확인해야 합니다. 처음 피하려던 문제가 그대로 돌아옵니다.
...
Brad Frost가 제안한 UI 구성 방법론입니다. 화학의 원자·분자 비유를 빌려 컴포넌트를 다섯 단계로 나눕니다. `atoms`(Button, Input처럼 더 쪼갤 수 없는 최소 단위) → `molecules`(SearchBar처럼 atom 몇 개의 조합) → `organisms`(Header처럼 독립적으로 의미 있는 영역) → `templates`(데이터 없는 레이아웃) → `pages`(실제 데이터가 들어간 화면) 순입니다.

디자이너와 개발자가 같은 어휘로 이야기하게 되고, 재사용 단위가 눈에 보입니다. 디자인 시스템을 만들 때 특히 유용합니다.

실무에서는 molecule과 organism의 경계에서 자주 깨집니다. 경계가 주관적이기 때문입니다. `SearchBar`에 자동완성 드롭다운이 붙으면 molecule인가 organism인가. 이 논쟁에 쓰는 시간이 구조에서 얻는 이득보다 클 때가 많습니다. 그래서 요즘은 다섯 단계를 다 쓰기보다 **`ui`(원자 수준의 공통 컴포넌트)와 기능별 컴포넌트 두 층으로 단순화**하는 경우가 흔합니다.

### FSD (Feature-Sliced Design)

기능별 구조를 규칙으로 정형화한 방법론입니다. 레이어·슬라이스·세그먼트로 코드를 가릅니다.
...
기억할 규칙은 짧습니다. **위 레이어는 아래 레이어만 import합니다. 같은 레이어의 형제 슬라이스끼리는 import하지 않습니다.** 이 둘만 지켜도 레이어와 슬라이스 사이에는 순환이 생길 수 없습니다. 다만 슬라이스 하나 안에서 파일끼리 도는 순환까지 막아주지는 않아서 뒤에서 볼 `import/no-cycle`이 여전히 필요합니다.

과설계 위험도 분명합니다. 화면이 다섯 개인 프로젝트에 FSD를 그대로 적용하면 `features/auth/ui/LoginForm/index.ts`처럼 경로가 깊어지고, 파일 하나 만들 때마다 "이건 feature인가 entity인가"를 붙들고 있어야 합니다. 폴더 수가 실제 코드보다 빨리 늘어납니다.
...
> 결론: **방법론은 규칙 모음입니다.** 도입 자체를 목표로 삼을 이유는 없습니다. FSD를 쓴다면 폴더 이름을 전부 따라 하기보다 "레이어 단방향 의존, 형제 슬라이스 간 import 금지"부터 lint로 강제하는 편이 실질적인 이득을 줍니다.
...
`features/order`가 `features/user`를 import하고 `features/user`도 `features/order`를 import하면 순환이 생깁니다. 폴더는 나뉘어 있지만 실제로는 하나의 덩어리입니다. 어느 쪽도 혼자 삭제할 수 없고, 코드 스플리팅으로 나누려 해도 서로를 끌어당겨 한 청크가 됩니다. 모듈 평가 시점에 따라 한쪽이 `undefined`로 보이는 간헐적 버그도 생깁니다. 한쪽만 테스트하려 해도 다른 쪽까지 전부 로드됩니다.
...
형제 기능끼리 import를 금지하면 처음에는 답답합니다. 주문 화면에서 사용자 정보가 필요한 일은 흔합니다. 이럴 때는 공통으로 필요한 개념을 **아래 레이어(entities)로 내립니다.** `User` 타입과 조회 로직은 양쪽이 다 쓰므로 entities에 있는 게 맞습니다. 조합은 위 레이어에서 합니다. 주문 페이지가 `features/order`와 `features/user`를 각각 가져와 붙이는 식입니다.
...
응답이 `data.data`에서 `data.result`로 바뀌거나 `user_name`이 `name`으로 바뀌면 이 필드를 쓰는 컴포넌트를 전부 찾아 고쳐야 합니다. 백엔드 변경이 UI까지 번지는 겁니다. `snake_case`, 유닉스 타임스탬프, 응답 봉투(`data.data`)는 전부 백엔드 규약입니다. 프론트의 도메인 모델이 아닌데 그대로 프론트 코드에 새어 들어옵니다.

테스트가 무거워지고 재사용도 막힙니다. 이 컴포넌트를 테스트하려면 HTTP를 흉내 내야 합니다. 같은 사용자 정보가 다른 화면에서도 필요하면 이 `useEffect`를 복사하게 됩니다.
...
**MSA**로 나뉘면 이야기가 달라집니다. 주문 상세 화면 하나를 그리려고 주문·사용자·상품·배송 서비스를 각각 호출하고 프론트에서 합쳐야 합니다. 요청 왕복이 늘어 느려지고 인증 처리가 서비스마다 반복됩니다. 무엇보다 **프론트가 백엔드 내부 분할 구조를 알게 됩니다.** 서비스를 하나 더 쪼개면 프론트도 따라 고쳐야 합니다.
...
BFF는 공짜가 아닙니다. **BFF가 죽으면 프론트 전체가 멈춥니다(SPOF, 단일 장애점).** 완화 수단은 백엔드 쪽 관례를 그대로 따릅니다. 인스턴스를 여러 개 띄우고 로드 밸런서를 앞에 둡니다. health check로 비정상 인스턴스를 빼고 배포 중 요청이 유실되지 않게 graceful shutdown을 겁니다. 하위 서비스 장애가 번지지 않도록 타임아웃·서킷 브레이커·폴백을 둡니다. 웹용과 모바일용 BFF를 나눠 장애를 격리하기도 합니다.
...
도입하면 아래가 전부 팀의 일이 됩니다.

- 중복 번들을 피하려면 각 조각이 React를 따로 들고 오지 않게 공유 설정을 맞춰야 합니다
- 버전 스큐(skew) — A팀이 React를 올리고 B팀이 안 올리면 한 페이지에 두 버전이 공존합니다
- **디자인 일관성** — 공통 디자인 시스템을 별도 패키지로 배포하고 버전을 관리해야 합니다
- 조각 각각은 통과하는데 합치면 깨지는 상황을 잡을 통합 테스트 방법이 필요합니다
- 라우팅·인증을 공유하려면 어느 조각이 라우터를 소유하고, 토큰을 어떻게 나눌지 정해야 합니다
- 에러 스택이 여러 배포 산출물에 걸쳐 디버깅 난이도가 올라갑니다
...
3. **화면 경계가 팀 경계와 일치한다** — 한 화면 안에서 두 팀이 같은 컴포넌트를 고쳐야 한다면 조각을 나눠도 조율 비용은 그대로입니다
4. **점진적 마이그레이션이 필요하다** — 레거시를 한 번에 못 걷어내고 화면 단위로 새 스택으로 옮겨야 하는 상황입니다. 마이크로 프론트엔드를 도입하는 가장 설득력 있는 이유입니다

### 그렇지 않다면

단일 팀이 "코드베이스가 커서" 도입하는 것은 거의 항상 손해입니다. 그 문제는 모노레포와 패키지 경계, lint로 강제한 의존성 규칙으로 대부분 해결됩니다. 빌드가 하나로 유지되므로 버전 스큐도 중복 번들도 없습니다.

바꿔 말하면 마이크로 프론트엔드는 **기술 문제의 해법이 아니라 조직 문제의 해법**입니다. 조직이 나뉘지 않았는데 도입하면 조직 문제 없이 분산 시스템 비용만 치르게 됩니다. 백엔드에서 MSA를 두고 하는 이야기와 정확히 같습니다.
...
- 경계를 새로 정하기보다 지키게 만드는 쪽이 어렵습니다. 새 규칙을 도입할 때는 lint를 곧장 `error`로 켜지 말고 `warn`으로 두면서 기존 위반을 목록화한 뒤, 새로 추가되는 코드부터 막으면 마찰이 적습니다.
- 모노레포는 구조 문제의 흔한 다음 단계입니다. pnpm workspace나 Turborepo로 디자인 시스템·유틸을 별도 패키지로 떼어내면 경계를 `package.json`의 의존성이 강제합니다. 폴더 규칙일 때는 lint가 경고하고 말지만 여기서는 어기는 순간 빌드가 실패합니다.
...
A. 최상위를 기능(도메인)으로 나누고 기능 폴더 안쪽만 기술별로 나눕니다. 기술별로만 나누면 `components/`에 파일이 수십 개 쌓이고, 기능 하나를 고치려고 네 폴더를 오가야 하고, 무엇보다 기능을 통째로 삭제할 수 없게 됩니다. 기능별로 나누면 변경과 삭제가 폴더 단위로 떨어집니다. 공통 코드는 `/shared`로 올리되, 실제 두 번째 사용처가 생겼고 도메인 지식이 없는 것만 올립니다.
- 꼬리 질문: "도메인 지식이 있는 공통 코드는요?" → 두 기능이 같은 도메인 개체를 다룬다면 그건 공통 유틸이 아니라 별도 도메인 계층(entities)으로 내려야 할 신호라고 답합니다.

**Q. 의존성 방향을 어떻게 관리하나요?**

A. 상위 레이어가 하위 레이어만 import하고 하위는 상위를 모르게 하는 단방향 규칙을 두고, 같은 레이어의 형제 기능끼리도 직접 import를 막습니다. 이 규칙은 문서로 적어두는 대신 도구로 강제합니다. `eslint-plugin-import`의 `no-restricted-paths`로 방향을 지정하고 `no-cycle`로 순환을 막습니다. 각 기능 폴더에 `index.ts`를 두고 내부 경로 직접 접근을 금지하면 폴더가 공개 API를 가진 모듈처럼 동작합니다.
...
A. 규모에 따라 다릅니다. 방법론에서 가져갈 가치는 폴더 이름보다 규칙에 있습니다. FSD라면 "레이어 단방향, 형제 슬라이스 간 import 금지" 두 가지가 핵심입니다. 화면이 다섯 개인 프로젝트에 전체 레이어를 다 만들면 폴더가 코드보다 빨리 늘고, 파일 하나 만들 때마다 feature인지 entity인지 판단하는 비용이 생깁니다. 저는 단순한 기능별 구조로 시작해서 경계 위반이 실제로 문제를 일으킬 때 규칙을 lint로 추가하는 쪽을 택합니다.
...
A. 코드베이스 크기로 정하지 않고 조직 구조를 봅니다. 팀이 여럿이고 각자 오너십이 있고 배포 주기가 실제로 다르며, 서로를 기다리는 것이 병목일 때 의미가 있습니다. 레거시를 화면 단위로 점진 마이그레이션해야 하는 경우도 설득력 있는 이유입니다. 반대로 단일 팀에서 코드가 크다는 이유로 도입하면 버전 스큐, 중복 번들, 통합 테스트 부담만 떠안습니다. 그 문제는 모노레포와 lint로 강제한 의존성 경계로 해결하는 편이 낫습니다.
...
프로젝트 구조는 **변경과 삭제의 단위를 폴더 단위와 맞추려고** 나눕니다. 그 출발점이 기능별 구조입니다. 정해둔 의존성 방향을 lint로 강제하지 않으면 어떤 구조도 몇 달 안에 문서상으로만 남습니다.
