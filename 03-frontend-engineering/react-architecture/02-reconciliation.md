...
React는 **최소 변경을 보장하지 않습니다.** 대신 "UI에서는 대개 이렇더라"는 두 가지 경험적 가정(휴리스틱)을 세우고, 그 가정 하에 O(n)으로 비교를 끝냅니다.
...
뒤집어 말하면, **트리에서 같은 자리에 같은 타입의 컴포넌트가 있으면 React는 그것을 "같은 컴포넌트"로 보고 상태를 유지합니다.** 파일이 다른 컴포넌트든, props가 전부 바뀌었든 상관없습니다.
...
**왜 문제인가**: 인덱스는 "몇 번째 자리인가"이지 "무엇인가"가 아닙니다. key로 인덱스를 주는 것은 key를 안 준 것과 사실상 같은 결과를 만듭니다. 목록의 순서가 바뀌거나 중간에 삽입·삭제가 일어나면 같은 인덱스가 다른 데이터를 가리키게 되고, React는 **엉뚱한 컴포넌트를 재사용합니다.**
