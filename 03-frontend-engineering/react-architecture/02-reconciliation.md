...
> 두 개의 Virtual DOM 트리에서 "무엇이 바뀌었는지"를 React가 어떤 규칙으로 판단하는지, 그 규칙을 어길 때 어떤 버그가 나는지 설명할 수 있게 된다.
...
- [01-virtual-dom.md](./01-virtual-dom.md) - 비교 대상인 트리가 무엇인지 먼저 알아야 한다
...
이전 트리와 새 트리가 있을 때, "최소 몇 번의 삽입·삭제·이동으로 이전 트리를 새 트리로 바꿀 수 있는가"를 정확히 푸는 것은 트리 편집 거리(tree edit distance) 문제다. 노드가 n개일 때 알려진 최적 알고리즘의 복잡도는 O(n³) 수준이다.

노드가 1,000개인 평범한 화면이면 10억 번의 연산이다. 클릭 한 번에 이걸 하고 있을 수는 없다.

### React의 선택: 정확함을 버리고 실용성을 취한다

React는 **최소 변경을 보장하지 않는다.** 대신 "UI에서는 대개 이렇더라"는 두 가지 경험적 가정(휴리스틱)을 세우고, 그 가정 하에 O(n)으로 비교를 끝낸다.
...
이 트레이드오프를 이해하는 것이 중요하다. **React가 가끔 "멍청하게" 동작하는 것처럼 보이는 순간들은 대부분 이 가정이 깨졌기 때문**이다. 아래 두 규칙과 그 부작용이 이 문서의 본론이다.
...
같은 위치의 노드를 비교할 때, **타입(`type`)이 다르면 비교를 중단하고 이전 서브트리 전체를 파괴한 뒤 새로 만든다.**
...
"내용이 같은데 왜 재사용하지 않냐"고 물을 수 있다. 재사용 가능성을 확인하려면 서브트리 전체를 다시 뒤져야 하고, 그게 바로 O(n³)로 가는 길이다. React는 "타입이 바뀌었다면 개발자가 완전히 다른 걸 그리려는 것"이라고 가정하고 확인을 포기한다.

### 위치가 같고 타입이 같으면 상태가 유지된다

뒤집어 말하면, **트리에서 같은 자리에 같은 타입의 컴포넌트가 있으면 React는 그것을 "같은 컴포넌트"로 보고 상태를 유지한다.** 파일이 다른 컴포넌트든, props가 전부 바뀌었든 상관없다.
...
**왜 문제인가**: Virtual DOM 노드의 `type`은 컴포넌트 함수 그 자체다. `Parent`가 렌더링될 때마다 `Child`는 새로 만들어진 함수이므로, 이전 렌더의 `Child`와 참조가 다르다. React 입장에서는 타입이 바뀐 것이니 휴리스틱 1이 발동해 **매 렌더마다 `<input>`을 파괴하고 새로 만든다.** 사용자는 버튼을 누를 때마다 입력하던 글자와 포커스를 잃는다.
...
이제 `Child`는 모듈 로드 시 한 번만 만들어지고 참조가 고정되므로, `Parent`가 몇 번 렌더링되든 `<input>`은 재사용된다.
...
같은 부모 아래의 자식 목록을 비교할 때, React는 기본적으로 **순서(인덱스)**로 짝을 맞춘다. `key`가 있으면 순서 대신 **key 값**으로 짝을 맞춘다.
...
`key`는 "이 자리에 있는 것"이 아니라 "이것이 무엇인지"를 알려 주는 신원 증명서다. **React가 key를 보는 범위는 같은 부모 아래의 형제들 사이뿐**이며, 전역에서 유일할 필요는 없다.
...
**왜 문제인가**: 인덱스는 "몇 번째 자리인가"이지 "무엇인가"가 아니다. key로 인덱스를 주는 것은 key를 안 준 것과 사실상 같은 결과를 만든다. 목록의 순서가 바뀌거나 중간에 삽입·삭제가 일어나면 같은 인덱스가 다른 데이터를 가리키게 되고, React는 **엉뚱한 컴포넌트를 재사용한다.**
...
사용자가 "설거지"에 체크한 뒤 "맨 앞에 추가" 버튼을 누른다.
...
**체크 표시가 다른 항목으로 옮겨 갔다.** 데이터(`todos`)는 완벽히 정확한데 화면만 틀렸다. 이런 버그는 콘솔에 에러도 안 나고, 데이터를 아무리 들여다봐도 원인이 안 보인다.
...
세 가지를 **모두** 만족할 때만 안전하다.

1. 목록이 절대 재정렬되지 않는다 (정렬·필터 기능이 없다)
2. 항목이 중간에 삽입·삭제되지 않는다 (끝에만 덧붙이는 경우는 인덱스가 밀리지 않아 상대적으로 안전하다)
3. 각 항목이 자체 상태나 비제어 입력(`<input>`의 값, 포커스 등)을 갖지 않는다

정적인 안내 문구 목록 정도가 여기 해당한다. 조건을 매번 따지느니 그냥 안정적인 ID를 쓰는 게 낫다.
...
**왜 문제인가**: 렌더링할 때마다 key가 전부 새 값이 된다. React는 모든 항목이 사라지고 전혀 다른 항목이 새로 생겼다고 판단해 **매 렌더마다 전체 목록을 언마운트하고 재마운트한다.** 상태가 전부 초기화되고, 애니메이션이 끊기고, 성능도 최악이 된다. ID가 없다면 데이터를 만들 때(서버에서 받을 때, 배열에 추가할 때) 한 번 부여해서 고정시켜야 한다.

### key의 반대 활용 — 일부러 상태를 초기화하기

key는 "재사용시키는" 도구인 동시에 "재사용을 끊는" 도구다. 다른 사용자를 선택했을 때 폼 내용을 초기화하고 싶다면, effect로 상태를 리셋하는 대신 key를 바꾸면 된다.
...
`userId`가 바뀌면 key가 바뀌고, React는 이전 `ProfileForm`을 버리고 새로 마운트한다. 초기화 코드를 직접 쓸 필요가 없고 초기화 대상을 빠뜨릴 위험도 없다.

---

## 4. 리렌더링은 언제 일어나는가

재조정은 "리렌더링이 일어난 뒤" 시작된다. 그렇다면 리렌더링 자체는 무엇이 유발하는가.
...
**결론**: "props가 안 바뀌면 자식은 안 그려진다"는 흔한 오해다. 기본 동작은 정반대이며, 막고 싶으면 `React.memo`로 감싸거나 컴포넌트 구조를 바꿔야 한다.

### 불변성이 필요한 이유가 여기 있다

state 비교는 `Object.is` 기반의 얕은 비교다. 객체 내부를 뒤져 보지 않는다.
...
리렌더링이 일어나도 재조정 결과 차이가 없으면 실제 DOM은 건드리지 않는다. 그래서 **리렌더링 횟수 자체가 목표 지표가 되어서는 안 된다.** 최적화의 대상은 "느린 렌더", 즉 렌더 함수 안에서 무거운 계산을 하거나 노드 수가 수천 개인 경우다. 무엇이 느린지는 React DevTools Profiler로 먼저 확인한다.

---

## 5. 실무에서는

- 목록 데이터에 ID가 없을 때는 **서버 응답을 받는 지점이나 상태에 넣는 지점에서** ID를 부여한다. 렌더링 함수 안에서 만들면 매번 달라진다.
- 정렬·필터 기능이 있는 테이블은 인덱스 key 버그가 가장 잘 드러나는 곳이다. 정렬 버튼을 누른 뒤 행의 체크박스나 펼침 상태가 엉뚱하게 따라다닌다면 거의 항상 key 문제다.
- `key` 경고(`Each child in a list should have a unique "key" prop`)를 인덱스로 막는 것은 경고만 끄고 버그는 남겨 두는 조치다. 경고를 없애는 게 목적이 아니라 신원을 부여하는 게 목적이다.
- 조건부 렌더링에서 두 분기가 같은 컴포넌트를 쓰는데 상태가 유지되기를 원치 않는다면, 분기마다 다른 `key`를 주면 된다.
...
재조정은 **"타입이 다르면 통째로 버리고, 형제는 key로 신원을 확인한다"**는 두 가정으로 트리 비교를 O(n)까지 낮춘 알고리즘이며, 이 가정을 깨는 코드(인덱스 key, 렌더 중 컴포넌트 정의)가 곧 버그가 된다.
