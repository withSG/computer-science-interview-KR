...
이런 부수효과를 "렌더링이 끝나고 화면이 확정된 뒤"로 미루는 장치가 effect입니다. 그런데 "화면이 확정된 뒤"에는 미묘하게 다른 두 시점이 있습니다. **브라우저가 실제로 픽셀을 그리기 직전**과 **그린 직후**입니다. 이 차이가 `useLayoutEffect`와 `useEffect`를 가릅니다.
...
핵심은 **`useLayoutEffect`는 페인트를 막고, `useEffect`는 막지 않는다**는 것입니다.

`useLayoutEffect` 안에서 `setState`를 하면 React는 그 업데이트까지 처리해 DOM에 반영한 뒤에야 브라우저에 제어권을 넘깁니다. 사용자는 중간 상태를 보지 못합니다. 반대로 `useEffect` 안에서 `setState`를 하면 사용자는 **변경 전 화면을 한 번 보고 나서** 바뀐 화면을 보게 됩니다.

정확히 말하면 `useEffect`는 "커밋 후 비동기로 예약"될 뿐, **정확한 실행 시점이 명세된 API가 아닙니다.** 클릭 같은 사용자 조작이 원인인 업데이트에서는 React가 페인트 전에 effect를 몰아서 실행하는 경우도 있습니다. 그래서 "`useEffect`면 무조건 페인트 뒤"라고 외우기보다, **페인트를 확실히 막고 싶으면 `useLayoutEffect`를 써야 한다**로 기억하는 편이 안전합니다. `useEffect`가 보장하는 것은 "페인트를 막지 않으려고 노력한다"이지 "절대 막지 않는다"가 아닙니다.
...
한 프레임이지만 사람 눈에 확실히 보입니다. 툴팁이 순간적으로 튀는 것처럼 느껴집니다.
...
공통점은 **"DOM을 읽어서 그 결과로 DOM을 다시 바꿔야 하는" 경우**입니다.
...
SSR 관련해서 한 가지 더. `useLayoutEffect`는 서버에서 실행될 수 없으므로 React가 경고를 띄웁니다. 서버에서 렌더된 HTML과 클라이언트 첫 렌더 결과가 달라질 여지가 생기기 때문입니다. 이 경우 로직을 `useEffect`로 옮기거나, 측정 없이도 서버에서 그릴 수 있는 초기 마크업을 만드는 쪽으로 설계를 바꾸는 것이 정석입니다.
...
effect가 반환한 함수가 cleanup입니다. 컴포넌트가 언마운트될 때, 그리고 **의존성이 바뀌어 다음 effect가 실행되기 직전**에 실행됩니다. 두 번째가 자주 간과됩니다.
...
StrictMode에서 실행하면 연결이 두 개 생기고 하나가 남습니다. 콘솔에 "연결됨"이 두 번 찍히는 걸 보고 개발자는 즉시 이상을 알아챕니다. cleanup을 제대로 붙이면 연결 → 해제 → 연결이 되어 최종적으로 연결 하나만 남습니다. 결과가 같아집니다.
...
**왜 문제인가**: 증상만 가리고 원인은 그대로 둡니다. 실제로 컴포넌트가 언마운트됐다가 다시 마운트되는 상황(라우팅으로 페이지를 오가는 경우)에서는 `connect()`가 아예 실행되지 않아 진짜 버그가 됩니다. **필요한 것은 실행을 막는 게 아니라 cleanup을 제대로 쓰는 것입니다.**

같은 이유로 "StrictMode를 끄면 해결된다"는 것도 해결이 아닙니다. 경고등을 뗀 것입니다.
...
계산이 정말 비쌀 때만 `useMemo`로 감쌉니다. **`useMemo`도 상태를 만들지는 않는다는 점이 핵심입니다.**
...
**왜 문제인가**: "전송 버튼을 눌렀다"는 사실을 상태로 저장했다가 effect가 그걸 눈치채게 만드는 우회로입니다. 상태 하나가 불필요하게 늘고, 다른 이유로 컴포넌트가 다시 마운트되면 토스트가 또 뜹니다. 무엇이 무엇을 유발하는지 코드에서 읽히지 않습니다.
...
**왜 문제인가**: `query`가 "ab" → "abc"로 빠르게 바뀌면 요청이 두 개 날아갑니다. 네트워크 사정에 따라 "ab"의 응답이 나중에 도착할 수 있고, 그러면 최종 화면에 **"abc"를 검색했는데 "ab"의 결과**가 남습니다. 재현이 어렵고 사용자만 이상하다고 느끼는 종류의 버그입니다.
...
`AbortController`로 요청 자체를 취소해도 됩니다. 다만 실무에서는 캐싱·재시도·중복 제거까지 필요해지므로, 데이터 페칭은 전용 라이브러리(TanStack Query, SWR 등)나 프레임워크의 데이터 로딩 기능에 맡기는 것이 일반적인 선택입니다.
...
**결론**: effect는 "React 바깥 세계와의 동기화"에만 씁니다. React 안에서 값이 값으로 이어지는 관계는 계산으로 표현합니다.
...
- effect 안에서 로직이 길어지면 대개 여러 개의 관심사가 섞인 것입니다. **하나의 effect는 하나의 동기화만** 담당하도록 나눕니다. 의존성 배열이 짧아지고 cleanup이 명확해집니다.
