...
React 컴포넌트 함수는 **순수해야 한다.** 같은 props와 state에 대해 항상 같은 결과를 반환해야 하고, 외부에 영향을 주면 안 됩니다. render 단계는 중단되거나 폐기되거나 여러 번 실행될 수 있기 때문입니다.
...
이런 부수효과를 "렌더링이 끝나고 화면이 확정된 뒤"로 미루는 장치가 effect다. 그런데 "화면이 확정된 뒤"에는 미묘하게 다른 두 시점이 있습니다. **브라우저가 실제로 픽셀을 그리기 직전**과 **그린 직후입니다. 이 차이가 `useLayoutEffect`와 `useEffect`를 가릅니다.
...
정확히 말하면 `useEffect`는 "커밋 후 비동기로 예약"될 뿐, **정확한 실행 시점이 명세된 API가 아니다.** 클릭 같은 사용자 조작이 원인인 업데이트에서는 React가 페인트 전에 effect를 몰아서 실행하는 경우도 있습니다. 그래서 "`useEffect`면 무조건 페인트 뒤"라고 외우기보다, **페인트를 확실히 막고 싶으면 `useLayoutEffect`를 써야 한다**로 기억하는 편이 안전합니다. `useEffect`가 보장하는 것은 "페인트를 막지 않으려고 노력한다"이지 "절대 막지 않는다"가 아닙니다.
...
개선은 간단합니다. **위 코드에서 `useEffect`를 `useLayoutEffect`로 바꾸기만 하면 된다.** 그러면 순서가 이렇게 달라집니다.
...
공통점은 **"DOM을 읽어서 그 결과로 DOM을 다시 바꿔야 하는" 경우입니다.
...
**결론**: 기본은 `useEffect`다. 화면에 잘못된 중간 상태가 눈에 보일 때만 `useLayoutEffect`로 바꿉니다. 순서를 반대로 하면(일단 `useLayoutEffect`부터) 불필요하게 페인트를 막아 반응성이 나빠집니다.
...
cleanup 함수 역시 자신이 만들어진 렌더의 클로저를 붙잡고 있으므로, 위 예에서 첫 cleanup이 보는 `roomId`는 'A'다. 이 덕분에 "자기가 연 것을 자기가 닫는" 대칭이 성립합니다.
...
React 18부터, 개발 모드에서 `<StrictMode>`로 감싼 트리는 마운트 시 effect를 `setup → cleanup → setup` 순으로 실행합니다. (그 이전 버전의 StrictMode는 렌더 함수만 두 번 호출했고 effect는 건드리지 않았다.) 즉 "마운트 → 언마운트 → 다시 마운트"를 인위적으로 시뮬레이션합니다. **개발 모드에서만 일어나고 프로덕션 빌드에서는 일어나지 않는다.**

### 왜 이런 걸 하나

React는 앞으로 컴포넌트 상태를 보존한 채 화면에서 떼었다 다시 붙이는 동작(탭 전환, 뒤로 가기 복원 등)을 하려 합니다. 그러려면 **effect가 여러 번 setup/cleanup 되어도 결과가 같아야 한다.** StrictMode는 그 조건을 만족하지 않는 코드를 개발 단계에서 즉시 드러냅니다.
...
**왜 문제인가**: 증상만 가리고 원인은 그대로 둡니다. 실제로 컴포넌트가 언마운트됐다가 다시 마운트되는 상황(라우팅으로 페이지를 오가는 경우)에서는 `connect()`가 아예 실행되지 않아 진짜 버그가 됩니다. **필요한 것은 실행을 막는 게 아니라 cleanup을 제대로 쓰는 것이다.**
...
계산이 정말 비쌀 때만 `useMemo`로 감쌉니다. **`useMemo`도 상태를 만들지는 않는다는 점이 핵심이다.**
...
**왜 문제인가**: 사용자가 바뀐 직후 한 프레임 동안 **이전 사용자의 입력 내용이 화면에 남는다.** 초기화해야 할 상태가 여러 개면 하나씩 빠뜨리기도 쉽습니다.
...
`useLayoutEffect`는 **페인트를 막고** 실행되어 깜빡임을 없애는 대신 화면 표시를 늦추고, `useEffect`는 페인트 후 실행되어 반응성을 지킵니다. 그리고 그보다 먼저 물어야 할 것은 **"이 일에 effect가 정말 필요한가"**다.
