...
> 함수 컴포넌트는 매번 처음부터 다시 실행되는데 어떻게 상태를 기억하는지, 그 구현 방식 때문에 왜 "훅은 최상위에서만 호출"이라는 규칙이 생겼는지 설명할 수 있게 된다.
...
- [03-fiber-architecture.md](./03-fiber-architecture.md) - 훅 데이터가 저장되는 곳이 fiber다
- JavaScript 클로저 개념
...
"창 크기를 구독해서 상태로 유지한다"는 로직을 여러 컴포넌트에서 쓰고 싶다고 하자. 클래스 컴포넌트에서는 이 로직이 **세 개의 생명주기 메서드에 흩어진다.**
...
**관련 있는 코드(A끼리, B끼리)는 떨어져 있고, 관련 없는 코드(A와 B)는 같은 메서드에 섞여 있다.** 이 상태에서 A만 빼내어 다른 컴포넌트와 공유하려면 고차 컴포넌트(HOC)나 render props를 써야 했고, 그 결과 컴포넌트 트리가 `<withRouter><withTheme><withWindowSize><connect>` 식으로 래퍼에 겹겹이 둘러싸이는 문제가 생겼다.

훅은 이 문제를 **상태 로직을 컴포넌트 계층 구조와 분리해서 함수로 뽑아낼 수 있게** 만들어 해결했다.
...
버튼을 누르면 `Counter` 함수가 **처음부터 다시 호출된다.** `useState(0)`도 다시 실행되는데 화면에는 0이 아니라 1이 표시된다. 함수 안의 지역 변수는 호출이 끝나면 사라지는데, 값은 어디에 남아 있는 것인가.
...
렌더링이 시작되면 React는 이 리스트의 커서를 맨 앞으로 돌려놓는다. 컴포넌트 함수가 실행되면서 훅을 호출할 때마다 커서가 한 칸씩 전진하며 해당 위치의 훅 객체를 읽는다.
...
`initialValue`가 첫 렌더링에서만 쓰인다는 점이 드러난다. `useState(0)`을 매번 호출해도 두 번째부터는 저장된 값을 그대로 돌려준다.

### 비유: 이름표 없는 옷장

훅 저장소는 **번호만 붙어 있고 이름표는 없는 사물함**이다. React는 "당신이 오늘 세 번째로 연 사물함"이라는 것만 기억한다. 어제도 오늘도 같은 순서로 열면 문제가 없지만, 오늘 두 번째 사물함을 건너뛰면 그 뒤로 전부 한 칸씩 밀린다.

**비유의 한계**: 실제 구현은 배열이 아니라 링크드 리스트이고, 컴포넌트 인스턴스마다 별도의 저장소를 갖는다. 같은 컴포넌트를 두 번 그려도 저장소는 서로 다르다.

---

## 3. 그래서 훅 호출 순서 규칙이 필요하다

규칙은 두 줄이다. 훅은 컴포넌트(또는 커스텀 훅) 함수의 **최상위에서만** 호출하고, 조건문·반복문·중첩 함수·`return` 이후에서는 호출하지 않는다.
...
위 그림의 짝 어긋남은 컴포넌트 함수가 실행되는 동안 실제로 일어나고, 함수가 끝난 뒤에야 React가 훅 개수 차이를 알아채고 `Rendered fewer hooks than expected` 같은 에러를 던진다. 이 검사는 개발 모드 전용이 아니라 프로덕션에서도 동작한다. 다만 **훅을 하나 건너뛴 대신 다른 훅이 하나 더 호출되어 개수가 우연히 같아지는 경우**에는 개수 검사에 걸리지 않고 값만 조용히 뒤엉키므로, 결국 규칙 자체를 지키는 것 말고는 안전장치가 없다.
...
이른 `return`도 같은 문제를 만든다. `if (!data) return null;`을 훅보다 위에 두면 그 아래 훅들이 호출되지 않아 개수가 달라진다. **조기 반환은 훅을 모두 호출한 뒤에 배치한다.**
...
`setState`는 값을 즉시 바꾸지 않는다. 업데이트를 큐에 넣고 리렌더링을 예약한다. 그래서 같은 핸들러 안에서 현재 값을 읽으면 여전히 이번 렌더링의 값이다.
...
**직전 상태를 기반으로 계산하는 업데이트는 항상 함수형으로 쓰는 것이 안전하다.**

초기값 계산이 비싸다면 값 대신 함수를 넘긴다.
...
또한 `Object.is`로 비교했을 때 같은 값을 설정하면 React는 리렌더링을 건너뛴다. 다만 그 판단을 위해 해당 컴포넌트를 한 번 더 렌더링할 수는 있으므로, "같은 값이면 절대 렌더 함수가 실행되지 않는다"고 단정하면 안 된다.

### useRef — 리렌더링을 유발하지 않는 상자

`useRef`가 반환하는 객체는 렌더링 사이에 동일한 참조로 유지되고, `.current`를 바꿔도 렌더링이 일어나지 않는다.
...
**결론**: "이 값이 바뀌면 화면이 달라져야 하는가"로 판단한다. 그렇다면 state, 아니라면 ref다. 타이머 ID, DOM 노드, 이전 렌더의 값, 최신 콜백 보관 같은 용도가 ref에 해당한다.

주의할 점은 **렌더링 중에 `ref.current`를 읽거나 쓰면 안 된다**는 것이다. render 단계는 순수해야 하는데, ref 변경은 부수효과이며 값을 읽어도 언제 갱신됐는지 보장되지 않는다. ref 접근은 이벤트 핸들러나 effect 안에서 한다.
...
`useMemo`는 계산 **결과 값**을, `useCallback`은 **함수 그 자체**를 의존성이 그대로면 재사용한다.

실제로 도움이 되는 경우는 세 가지뿐이다.

1. **`React.memo`로 감싼 자식에게 객체·배열·함수를 props로 넘길 때** — 참조가 매번 바뀌면 얕은 비교가 실패해서 `React.memo`가 무력화된다
2. **`useEffect` 등의 의존성 배열에 들어가는 값일 때** — 참조가 매번 바뀌면 effect가 매 렌더마다 재실행된다
...
**왜 문제인가**: 문자열 연결은 메모이제이션 관리 비용보다 싸다. `onClick`이 붙는 대상은 `React.memo` 컴포넌트가 아니라 DOM 엘리먼트라서 참조 안정화의 이득이 전혀 없다. 결과적으로 **훅 저장소만 늘고, 의존성 배열이라는 유지보수 부담만 생긴다.**
...
메모이제이션은 **성능 최적화이지 동작 보장이 아니다.** 캐시가 유지되지 않아도 코드가 올바르게 동작해야 한다. `useMemo` 안에서 상태를 바꾸거나 구독을 등록하는 식으로 "한 번만 실행됨"에 기대면 안 된다.
...
`count`를 의존성에 넣어도 동작은 하지만, 그러면 매초 interval을 해제하고 다시 등록하게 된다. 값을 **읽어야** 하면 의존성에 넣고, **갱신만** 하면 되면 함수형 업데이트로 없애는 것이 판단 기준이다.
...
의존성은 `Object.is`로 하나씩 비교된다. 객체·배열·함수 리터럴을 그대로 넣으면 내용이 같아도 참조가 달라 매번 "바뀌었다"고 판정되고, effect 안에서 상태를 갱신하고 있었다면 무한 루프로 이어진다.

---

## 6. 커스텀 훅 — 로직을 공유하지 상태를 공유하지 않는다

`use`로 시작하는 함수 안에서 다른 훅을 호출하면 커스텀 훅이다. 특별한 문법이 아니라 그냥 함수이며, 훅 규칙이 그대로 적용된다.
...
**두 컴포넌트가 같은 커스텀 훅을 써도 상태는 공유되지 않는다.** `A`와 `B`가 모두 `useCounter()`를 호출하면 각자의 fiber에 별개의 저장소가 생긴다.

훅 데이터는 fiber에 저장되고, fiber는 컴포넌트 인스턴스마다 하나씩이다. 커스텀 훅은 **호출한 쪽의 저장소를 쓰는 코드 조각**일 뿐이다. 상태를 진짜로 공유하려면 상태를 공통 부모로 올리거나(lifting state up), Context나 외부 스토어를 써야 한다.

같은 이유로 커스텀 훅 안에서 훅을 조건부로 호출해도 안 된다. 호출한 컴포넌트의 훅 순서가 어긋나기 때문이다.

---

## 7. 실무에서는

- `eslint-plugin-react-hooks`의 두 규칙(`rules-of-hooks`, `exhaustive-deps`)은 켜 두는 것이 사실상 표준이다. 순서 위반은 `rules-of-hooks`가 대부분 잡아 준다.
- React DevTools의 Components 탭에서 컴포넌트를 선택하면 hooks가 순서대로 나열된다. `useState` 값이 예상과 다르게 표시되면 훅 순서나 stale closure를 의심할 지점이다.
- 메모이제이션은 Profiler로 병목을 확인한 뒤에 적용한다. React 팀도 수동 메모이제이션의 부담을 줄이기 위해 컴파일러가 이를 대신하는 방향을 추진하고 있는데, 이는 곧 **손으로 감싸는 코드가 많다는 것 자체가 문제로 인식되고 있다**는 뜻이다.
- 반복되는 상태 로직(폼 입력, 모달 열림, 목록 페이징, 미디어 쿼리 구독 등)은 커스텀 훅으로 뽑는 것이 팀 단위에서 가장 체감이 큰 정리 작업이다.
...
- 꼬리 질문: "`useEffect` 의존성 배열을 빈 배열로 뒀는데 값이 안 바뀝니다." → 오래된 클로저 문제입니다. 첫 렌더의 스코프를 캡처해서 그 시점의 값이 고정됩니다. 직전 상태 기준으로 갱신만 하면 되는 경우에는 `setState(prev => ...)` 형태로 바꿔 의존성을 없애고, 정말 최신 값이 필요하면 의존성에 넣어 effect를 다시 실행시켜야 합니다.
...
훅은 **fiber에 매달린 순서 기반 연결 리스트에 저장되기 때문에**, 매번 같은 순서로 호출해야 하며 이 제약이 훅 규칙과 의존성 배열의 모든 함정의 뿌리다.
