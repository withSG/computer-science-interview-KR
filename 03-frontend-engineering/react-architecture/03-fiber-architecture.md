...
재귀는 호출 스택(call stack)에 진행 상황을 쌓습니다. 이 스택은 **JS 엔진이 관리하는 영역이라 개발자가 손댈 수 없다.** 중간에 "여기까지만 하고 나중에 여기서부터 이어서 하자"고 저장할 방법이 없습니다. 함수를 빠져나오면 스택은 사라집니다.
...
Fiber는 **컴포넌트 하나에 대응하는 작업 단위(unit of work)를 표현한 객체입니다. React 엘리먼트가 "무엇을 그릴지"를 담는다면, Fiber는 "그것을 그리기 위해 무슨 일이 남았는지"까지 담습니다.
...
흔한 오해 하나를 먼저 정리하자. **Fiber를 쓴다고 모든 렌더링이 잘게 쪼개지는 게 아니다.** 클릭, 입력 같은 사용자 조작에서 발생한 업데이트는 즉각 반응해야 하므로 동기적으로 끝까지 렌더링됩니다. 중단·재개는 "급하지 않다"고 표시된 업데이트에만 적용됩니다.
