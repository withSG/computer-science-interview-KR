...
> React가 렌더링 작업을 왜 잘게 쪼갰는지, 그 결과로 무엇이 가능해졌고 무엇은 여전히 불가능한지 설명할 수 있게 된다.
...
브라우저의 메인 스레드는 JavaScript 실행과 화면 그리기를 **번갈아** 처리한다. 동시에 하는 게 아니다. 60fps로 부드럽게 보이려면 프레임 하나당 약 16.7ms 안에 "JS 실행 + 스타일 계산 + 레이아웃 + 페인트"를 모두 끝내야 한다.

JavaScript가 이 예산을 다 써 버리면 그 프레임은 그려지지 않는다. 사용자에게는 화면이 멈추고, 클릭이 씹히고, 입력한 글자가 뒤늦게 나타나는 것으로 보인다.

### Stack Reconciler는 왜 멈출 수 없었나

React 15까지의 재조정 엔진은 트리를 **재귀 함수**로 순회했다.
...
재귀는 호출 스택(call stack)에 진행 상황을 쌓는다. 이 스택은 **JS 엔진이 관리하는 영역이라 개발자가 손댈 수 없다.** 중간에 "여기까지만 하고 나중에 여기서부터 이어서 하자"고 저장할 방법이 없다. 함수를 빠져나오면 스택은 사라진다.
...
React 16에서 도입된 Fiber는 재귀를 버리고, **진행 상황을 자체 자료구조에 저장하는 반복문**으로 순회 방식을 바꿨다. 스택을 JS 엔진이 아니라 React가 직접 들고 있으면, 언제든 멈추고 나중에 이어서 할 수 있다.
...
Fiber는 **컴포넌트 하나에 대응하는 작업 단위(unit of work)를 표현한 객체**다. React 엘리먼트가 "무엇을 그릴지"를 담는다면, Fiber는 "그것을 그리기 위해 무슨 일이 남았는지"까지 담는다.
...
Stack Reconciler는 두루마리에 이어서 쓰는 방식이었다. 중간에 멈추면 어디까지 썼는지 표시할 방법이 마땅치 않다. Fiber는 작업을 카드 한 장씩으로 나누고 "다음 카드"를 가리키는 화살표를 붙여 둔 것이다. 하던 카드에 클립을 꽂아 두고 자리를 떠도 된다.

**비유의 한계**: 카드는 물리적으로 한 장씩 처리되지만, React는 시간 예산이 남아 있는 동안 여러 카드를 연속으로 처리하고 예산이 떨어질 때만 양보한다. 매번 멈추는 게 아니다.

---

## 3. 작업 루프 (Work Loop)

React는 이런 형태의 반복문으로 트리를 처리한다.
...
`shouldYield()`는 "이번 시간 조각을 다 썼는가, 더 급한 일이 대기 중인가"를 확인한다. 답이 참이면 루프를 빠져나오고 브라우저가 페인트나 이벤트 처리를 할 수 있게 비켜 준다.
...
작업을 중단 가능하게 만들려면, **중단해도 안전한 구간**과 **절대 중단하면 안 되는 구간**을 분리해야 한다.

계산하다 만 상태로 멈추는 건 괜찮다. 아무도 못 본다. 하지만 실제 DOM을 절반만 바꾸다 멈추면 사용자에게 **깨진 화면**이 보인다. 헤더는 새 데이터인데 본문은 옛 데이터인 상태 같은 것이다. 그래서 React는 "모든 계산을 끝낸 뒤, 그 결과를 한 번에 적용"하는 2단계 구조를 택했다.
...
**결론**: 컴포넌트 함수 본문(= render 단계)에 부수효과를 쓰면 안 되는 이유가 바로 이것이다. 그 코드는 중단·폐기·재실행될 수 있다.
...
**왜 문제인가**: render 단계는 순수해야 한다. 동시성 렌더링에서 이 함수는 중단되었다가 다시 처음부터 실행될 수 있고, 그 결과가 버려질 수도 있다. 그러면 화면에 보이지도 않은 렌더에 대해 서버 로그가 남고, 카운터가 두 배로 오르고, 타이틀이 엉뚱한 값으로 바뀐다. 개발 모드의 `StrictMode`가 컴포넌트를 두 번 호출하는 이유도 이런 코드를 조기에 드러내기 위해서다.
...
흔한 오해 하나를 먼저 정리하자. **Fiber를 쓴다고 모든 렌더링이 잘게 쪼개지는 게 아니다.** 클릭, 입력 같은 사용자 조작에서 발생한 업데이트는 즉각 반응해야 하므로 동기적으로 끝까지 렌더링된다. 중단·재개는 "급하지 않다"고 표시된 업데이트에만 적용된다.

그 표시를 개발자가 하는 API가 `startTransition`과 `useDeferredValue`다. (동시성 기능은 React 18에서 `createRoot`로 마운트한 앱에서 동작한다.)
...
사용자가 빠르게 타이핑하면, 목록 렌더링이 진행되던 중에 새 키 입력이 들어온다. React는 진행 중이던 목록 렌더링을 **중단하고 버린 뒤**, 급한 입력 업데이트를 먼저 처리하고, 그다음 최신 값으로 목록 렌더링을 다시 시작한다. 그래서 입력이 끊기지 않는다.

`isPending`은 "뒤에서 아직 작업 중"임을 알려 주므로 흐릿하게 표시하는 등의 피드백에 쓴다.

### useDeferredValue — 값 자체를 늦춘다

`startTransition`으로 감쌀 setter에 접근할 수 없을 때(예: props로 값이 내려올 때) 쓴다.
...
**결론**: API 호출 횟수를 줄이는 게 목적이면 디바운스, 이미 가진 데이터를 렌더링하는 게 느려서 입력이 밀리는 거면 동시성 기능이다. 둘은 대체재가 아니라 서로 다른 문제를 푼다.

### Suspense도 같은 기반 위에 있다

`Suspense`는 "자식이 아직 준비되지 않았으면 fallback을 보여 준다"는 선언적 로딩 처리다.
...
준비되지 않은 컴포넌트를 만났을 때 **렌더 작업을 중단하고 트리의 다른 부분으로 넘어갔다가 나중에 재개**할 수 있어야 이 동작이 성립한다. 그래서 Suspense는 Fiber 구조 위에서만 가능하다. 또한 `startTransition` 안에서 발생한 업데이트가 무언가를 기다리게 되면, React는 fallback으로 화면을 비우는 대신 이전 화면을 유지한다. 이미 보고 있던 콘텐츠가 로딩 스켈레톤으로 되돌아가는 퇴행을 막기 위해서다.

---

## 6. 오해 정정

**"Fiber는 멀티스레딩이다"** — 아니다. JavaScript는 여전히 단일 스레드이고, React가 워커를 쓰는 것도 아니다. Fiber가 하는 일은 **자발적으로 실행권을 놓아 주는 협력적 스케줄링**이다. 긴 작업을 짧은 조각으로 나눠 사이사이에 브라우저가 일할 틈을 주는 것뿐이다.

**"Fiber를 쓰면 렌더링이 빨라진다"** — 아니다. 총 작업량은 같고, 스케줄링 관리 비용이 붙어 오히려 조금 늘어난다. 좋아지는 것은 **응답성**이다. 전체 소요 시간은 비슷하거나 약간 길어도, 그 사이에 사용자 입력이 처리되므로 체감이 완전히 달라진다.

**"내 앱은 자동으로 동시성 렌더링을 한다"** — 아니다. 아무 표시도 하지 않은 업데이트는 여전히 동기적으로 처리된다. `startTransition`이나 `useDeferredValue`로 명시적으로 낮은 우선순위를 지정해야 중단 가능한 렌더링이 된다.

---

## 7. 실무에서는

- 입력창과 무거운 목록이 한 화면에 있고 타이핑이 끊긴다면, 먼저 React DevTools Profiler로 어떤 컴포넌트가 얼마나 걸리는지 측정한다. 그다음 목록 갱신을 `startTransition`으로 감싸는 것이 표준 처방이다.
- 그래도 느리다면 우선순위 문제가 아니라 **작업량 자체**가 문제다. 목록 가상화로 렌더링할 노드 수를 줄이거나, 필터링 로직을 개선해야 한다. Fiber는 일을 없애 주지 않는다.
- `React.StrictMode`는 개발 모드에서 컴포넌트를 두 번 호출한다. 이는 render 단계가 순수하지 않은 코드를 미리 찾아내기 위한 장치이고, 프로덕션 빌드에서는 일어나지 않는다. "두 번 실행돼서 이상하다"고 StrictMode를 끄는 것은 경고등을 떼는 것과 같다.
- Fiber 내부 구현(`flags`, `lanes` 등)은 공개 API가 아니라 버전에 따라 바뀐다. 면접에서도 내부 필드명을 외우는 것보다 **"왜 이런 구조가 필요했는가"**를 설명하는 쪽이 훨씬 좋은 평가를 받는다.
...
Fiber는 **재귀 호출 스택에 갇혀 있던 렌더링 진행 상황을 React가 직접 관리하는 링크드 리스트 구조로 옮겨, 중단·재개·폐기와 우선순위 부여를 가능하게 만든 재조정 엔진**이다.
