...
### 핵심 답변
트리 쉐이킹은 번들링 시 **사용하지 않는 코드(Dead Code)를 제거**하는 최적화 기법입니다. 나무를 흔들면 죽은 잎이 떨어지는 것에 비유한 이름입니다. ESM(ES Modules)은 **정적 구조**라서 빌드 타임에 어떤 코드가 실제로 쓰이는지 분석할 수 있고, 트리 쉐이킹은 그 덕분에 가능합니다. CommonJS는 동적 require를 쓰기 때문에 정적 분석이 어려워 트리 쉐이킹이 제한적입니다.
