...
두 가지를 확인할 수 있습니다. 첫째, 각 모듈이 함수 안에 들어 있으므로 **모듈 안의 `const config`는 다른 모듈에서 보이지 않는다** — 전역 오염 문제 해결. 둘째, `cache` 덕분에 같은 모듈을 열 곳에서 import해도 **실행은 딱 한 번**이고 이후로는 같은 객체를 돌려줍니다. 그래서 모듈은 싱글톤처럼 동작합니다.
