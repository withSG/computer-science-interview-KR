...
> Webpack 설정 파일의 다섯 덩어리가 빌드 파이프라인의 어느 시점에 관여하는지, loader와 plugin이 왜 별개의 개념인지, 그리고 Babel이 문법은 바꿔주지만 폴리필은 왜 따로 챙겨야 하는지 설명할 수 있게 됩니다.
...
Webpack이 기본적으로 이해하는 것은 자바스크립트와 JSON뿐입니다. 나머지는 **"이 확장자를 만나면 이 도구로 자바스크립트가 이해할 수 있는 형태로 바꿔라"**를 사람이 알려줘야 합니다. 그것이 loader다.
...
다중 진입점은 서로 완전히 독립적인 페이지가 있을 때 쓰고, 요즘 SPA에서는 진입점 하나에 동적 `import()`로 나누는 쪽이 일반적입니다. `[contenthash]`는 파일 내용이 바뀔 때만 값이 바뀌는 해시로 캐싱 전략의 핵심이라 [03번 문서](./03-tree-shaking-optimization.md)에서 다룹니다. `publicPath`는 동적 청크를 어느 URL에서 가져올지 결정하므로, CDN을 쓰거나 앱이 서브 경로에 배포될 때 반드시 맞춰줘야 합니다.
...
`mode`는 단순한 라벨이 아니라 **여러 기본값을 한꺼번에 갈아끼우는 스위치입니다.
...
loader는 컨베이어 벨트 위의 **가공 기계입니다. 부품이 하나 지나갈 때마다 깎고, 도색하고, 다음 기계로 넘깁니다. 기계는 자기 앞에 놓인 부품 하나만 봅니다. plugin은 **공장 관리자입니다. "오늘 생산분 전체를 검수해라", "완성품을 포장 상자에 넣어라", "라인을 돌리기 전에 어제 잔여물을 치워라" 같은, 개별 부품이 아니라 공정 전체에 대한 지시를 내립니다.
...
loader는 자기가 맡은 파일 하나만 알지만, plugin은 `compilation`을 통해 **이번 빌드의 모든 모듈과 산출물**에 접근합니다. 이것이 둘을 나눈 진짜 이유입니다.
...
**왜 문제인가**: `style-loader`가 먼저 실행되어 아직 CSS 문자열인 것을 "DOM에 주입하는 JS 코드"로 감쌉니다. 그다음 `css-loader`가 그 자바스크립트를 CSS인 줄 알고 파싱하려다 문법 오류를 냅니다. 흔히 `Unexpected token` 계열의 에러 메시지가 원본 CSS와 아무 상관 없는 위치를 가리켜 원인 파악이 헷갈립니다.
...
옵셔널 체이닝과 널 병합 연산자입니다. 이 문법을 모르는 구형 엔진에 그대로 보내면 **파싱 단계에서 통째로 실패한다.** 한 줄 때문에 페이지 전체가 하얗게 뜹니다. Babel은 이 코드를 오래된 문법만 써서 같은 동작을 하는 코드로 바꿔줍니다.
...
(B)는 **엔진이 미리 만들어둔 객체와 메서드입니다. 코드 형태를 아무리 바꿔도 없는 `Promise` 생성자가 생겨나지는 않습니다. 이건 **직접 구현한 코드를 실행 시점에 전역에 심어주는 수밖에 없고**, 그 코드를 폴리필(polyfill)이라 부릅니다. 자바스크립트로 작성된 폴리필 모음이 `core-js`다.
...
`hidden-source-map`이 왜 있는지가 포인트입니다. 프로덕션에 `source-map`을 그대로 올리면 누구나 개발자 도구를 열어 **원본 소스 전체를 읽을 수 있다.** 내부 API 구조나 주석까지 노출됩니다. 그래서 소스맵 파일 자체는 만들되 공개 서버에는 올리지 않고 Sentry 같은 에러 추적 서비스에만 업로드하는 방식을 씁니다. 그러면 배포된 번들에는 참조 주석이 없어 사용자는 볼 수 없고, 에러 리포트에서는 원본 위치로 복원됩니다.

---

## 7. 실무에서는

- **설정 파일을 직접 쓰는 일이 많이 줄었다.** Next.js는 Webpack 설정을 감싸두고 필요한 부분만 열어주며, Vite는 Rollup 설정을 추상화합니다. 그래도 개념을 알아야 하는 이유는, 빌드가 깨졌을 때 에러 메시지가 여전히 loader와 plugin의 언어로 나오기 때문입니다.
- **`babel-loader` 대신 `swc-loader`나 `esbuild-loader`로 갈아타는 사례가 흔하다.** 문법 변환만 필요하다면 체감할 만큼 빨라집니다. 다만 `preset-env`의 `useBuiltIns` 같은 폴리필 자동 주입은 그대로 대체되지 않으므로 구형 브라우저 지원 범위를 먼저 확인해야 합니다.
- **`@babel/preset-typescript`가 타입 검사를 하지 않는 탓에 사고가 난다.** 빌드는 멀쩡히 통과하는데 타입 에러가 런타임 버그로 드러납니다. CI에 `tsc --noEmit`을 별도 단계로 반드시 넣습니다. 배포 번들이 예상보다 크다면 `mode: 'production'`이 제대로 적용됐는지부터 확인하는 것이 순서입니다.
