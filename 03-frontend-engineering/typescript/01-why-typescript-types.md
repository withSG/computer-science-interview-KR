...
두 번째 버전은 `sortBy`에 `"name"`을 넣는 실수를 컴파일러가 막아줍니다. 주석은 코드와 어긋나도 아무도 모르지만, 타입은 어긋나는 순간 빌드가 깨집니다. **검증되는 문서**라는 점이 주석과의 결정적 차이입니다.

### 반대로, TypeScript가 해주지 않는 것

여기서 초심자가 가장 많이 오해하는 지점을 먼저 못 박고 가자. **TypeScript의 타입은 컴파일이 끝나면 사라집니다(type erasure).** 빌드 결과물은 타입 표기가 전부 지워진 순수 JavaScript입니다.
