...
**객체 리터럴을 타입이 정해진 자리에 직접 대입할 때만** TypeScript는 추가로 **초과 속성 검사(excess property check)**를 돌립니다. 오타 난 선택적 속성을 잡기 위한 특수 규칙입니다. 리터럴이 아니라 변수를 거치면(`const raw = {...}; const opts: RequestOptions = raw;`) 이 검사는 작동하지 않습니다. 규칙이 아니라 **휴리스틱**이라는 점을 기억해두어야 합니다.
