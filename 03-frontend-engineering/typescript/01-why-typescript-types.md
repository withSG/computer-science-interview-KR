...
> 런타임까지 살아남던 오류가 어떻게 편집기 안에서 잡히는지, TypeScript가 "이름"이 아니라 "모양"으로 타입을 판단하는 이유가 무엇인지, `type`과 `interface` 중 무엇을 언제 쓰는지 설명할 수 있게 된다.
...
- JavaScript 문법(변수, 함수, 객체 리터럴)을 읽을 수 있으면 충분하다
- [../javascript-deep-dive/01-execution-context.md](../javascript-deep-dive/01-execution-context.md) — JS가 값을 어떻게 다루는지 감이 있으면 이해가 빠르다
...
이 함수에는 세 가지 폭탄이 들어 있다. `user`가 없을 수도, `profile`이 `null`일 수도, 서버 필드 이름이 사실 `nickname`(소문자 n)일 수도 있다. 그런데 JavaScript는 **이 함수가 실제로 호출되기 전까지 아무 말도 하지 않는다.** 조건이 맞아떨어져야 실행되는 경로라면 QA도 통과하고 배포까지 간다.
...
TypeScript가 파는 것은 "타입"이라는 문법이 아니라 **이 화살표를 왼쪽 끝으로 옮기는 것**이다. `user.profile.nickName`의 오타는 타입만 있으면 글자를 다 치기도 전에 잡힌다.

### 타입은 두 번째 문서다

타입이 주는 두 번째 가치는 사람에게 있다. 아래 두 시그니처를 비교해보자.
...
두 번째 버전은 `sortBy`에 `"name"`을 넣는 실수를 컴파일러가 막아준다. 주석은 코드와 어긋나도 아무도 모르지만, 타입은 어긋나는 순간 빌드가 깨진다. **검증되는 문서**라는 점이 주석과의 결정적 차이다.

### 반대로, TypeScript가 해주지 않는 것

여기서 초심자가 가장 많이 오해하는 지점을 먼저 못 박고 가자. **TypeScript의 타입은 컴파일이 끝나면 사라진다(type erasure).** 빌드 결과물은 타입 표기가 전부 지워진 순수 JavaScript다.
...
그래서 서버가 `name` 자리에 숫자를 보내면 런타임에서는 아무도 막지 않는다. **외부에서 들어오는 값(API 응답, `localStorage`, URL 쿼리, 사용자 입력)은 타입 선언만으로 안전해지지 않는다.** 경계에서는 실제 값을 검사하는 별도의 런타임 검증이 필요하다. 이 문제는 [03-type-guards-narrowing.md](./03-type-guards-narrowing.md)에서 본격적으로 다룬다.
...
TypeScript는 **구조적 타이핑(structural typing)**을 쓴다. 두 타입이 호환되는지를 **이름이 같은지가 아니라 구조(속성 구성)가 맞는지**로 판단한다. "오리처럼 걷고 오리처럼 운다면 그것은 오리다"라는 덕 타이핑(duck typing)과 발상이 같아 종종 같은 말처럼 쓰이는데, 엄밀히는 덕 타이핑이 런타임에 속성 유무를 보는 반면 구조적 타이핑은 컴파일 타임에 타입 구조를 비교한다.
...
왜 이렇게 설계했을까. TypeScript는 **이미 존재하는 방대한 JavaScript 코드 위에 타입을 얹는 언어**다. JavaScript는 객체 리터럴을 자유롭게 만들어 던지는 문화이고, 그런 코드에 "이 리터럴은 어느 인터페이스를 구현했다고 선언하시오"를 요구하면 아무도 쓰지 않았을 것이다. 구조적 타이핑은 **기존 JS 코드에 타입을 점진적으로 입히기 위한 필연적 선택**이었다.

### 비유: 콘센트 규격

명목적 타이핑은 "이 제품은 우리 브랜드 정품 인증 스티커가 붙어 있는가"를 따지고, 구조적 타이핑은 "220V 2핀에 꽂히는가"만 본다. 규격만 맞으면 어느 회사 제품이든 꽂힌다.

> **비유의 한계**: 콘센트는 물리적 형태가 완전히 일치해야 하지만, TypeScript는 **필요한 것보다 더 많이 가진 값**도 받아준다. 위 예시의 `point3d`처럼 `z`가 더 있어도 `Point` 자리에 들어간다. 이걸 부분 타입 관계(subtyping)라고 한다.

### 흔한 오해 — "그럼 오타도 안 잡히나?"

방금 `point3d`가 통과한 걸 보고 걱정될 수 있다. 하지만 이런 코드는 잡힌다.
...
**객체 리터럴을 타입이 정해진 자리에 직접 대입할 때만** TypeScript는 추가로 **초과 속성 검사(excess property check)**를 돌린다. 오타 난 선택적 속성을 잡기 위한 특수 규칙이다. 리터럴이 아니라 변수를 거치면(`const raw = {...}; const opts: RequestOptions = raw;`) 이 검사는 작동하지 않는다. 규칙이 아니라 **휴리스틱**이라는 점을 기억해두자.

### 구조가 같은데 구분하고 싶을 때 — 브랜딩

구조적 타이핑의 대가는 이런 상황이다.
...
`type`은 별칭일 뿐 새 타입을 만들지 않는다. 이걸 막으려면 **브랜디드 타입(branded type)**으로 실재하지 않는 표식을 붙여 구조를 다르게 만든다.
...
`__brand` 속성은 런타임에 실제로 존재하지 않는다. 오직 컴파일러를 속이지 못하게 하려고 존재하는 표식이다. 금액, 좌표계, 각종 ID처럼 **같은 원시 타입이지만 절대 섞이면 안 되는 값**에 쓴다.
...
`String`, `Number`처럼 대문자로 쓰는 것은 래퍼 객체 타입이라 거의 쓸 일이 없다. 소문자를 쓴다.

### 리터럴 타입 — 값 하나가 곧 타입

TypeScript에서는 **특정 값 하나만 허용하는 타입**을 만들 수 있다.
...
혼자서는 쓸모없어 보이지만, 유니온과 합쳐지는 순간 강력해진다.
...
문자열 상수를 `enum` 없이 표현하는 가장 흔한 방식이고, 뒤에서 볼 **판별 유니온**의 재료이기도 하다.

### 타입 넓히기(widening)와 `as const`

리터럴 타입에서 초심자가 반드시 한 번은 걸리는 지점이다.
...
**왜 문제인가**: `config.mode`가 `string`으로 넓혀졌기 때문에, 컴파일러 입장에서는 "이 객체의 mode에 나중에 `"purple"`이 들어갈 수도 있다"가 된다.
...
`as const`는 배열에도 유용하다. `["a", "b"]`는 기본적으로 `string[]`이지만 `as const`를 붙이면 `readonly ["a", "b"]` 튜플이 된다. 상수 목록에서 타입을 뽑아내는 관용구가 여기서 나온다.
...
값 하나만 관리하면 타입이 따라온다. 상수 배열과 타입이 어긋날 일이 없어진다.
...
> 표 요약: **표현력은 `type`이 넓고, 확장 개방성은 `interface`가 낫다.** 실무 기준선은 "객체 모양과 클래스 계약은 `interface`, 그 외 모든 타입 계산은 `type`" 정도면 충분하고, 팀 컨벤션이 이미 있다면 그걸 따르는 것이 더 중요하다.

### 선언 병합 — `interface`만의 능력

같은 이름의 `interface`를 여러 번 선언하면 TypeScript는 오류를 내지 않고 **하나로 합친다.**
...
한 파일 안에서 실수로 두 번 선언한 경우라면 오류를 놓치는 셈이라 반갑지 않다. 하지만 이 기능이 결정적인 순간이 있다. **내가 고칠 수 없는 남의 타입을 확장할 때**다.
...
`type Window = ...`로는 이게 불가능하다. 라이브러리 타입 확장, 테스트 러너의 `expect` 확장 등이 전부 이 메커니즘 위에 있다. **공개 라이브러리의 공개 타입을 `interface`로 내보내라**는 권고가 여기서 나온다.

### `extends`와 `&`의 충돌 처리 차이

같은 속성 이름이 서로 다른 타입으로 겹칠 때 동작이 다르다. 이건 실제 버그로 이어질 수 있어 알아둘 값어치가 있다.
...
`extends`는 **선언 시점에 소리를 지르고**, `&`는 **조용히 `never`를 만들어 나중에 이해할 수 없는 오류로 되돌아온다.** 확장 관계가 명확한 상속 구조라면 `extends` 쪽이 실수를 더 일찍 잡는다.
...
`any`는 "아무 타입"이 아니라 **"이 값에 대해서는 검사하지 마라"**는 지시다. 그래서 위험은 그 값 하나에 머물지 않는다.
...
**왜 문제인가**: `any`에서 파생된 값은 전부 `any`가 된다. 한 줄의 `any`가 호출 그래프를 타고 흘러 들어가 검사되지 않는 영역을 넓힌다. "타입 오류가 나서 일단 `any` 박았다"가 가장 위험한 이유다.
...
`unknown`은 **어떤 값이든 담을 수 있지만, 좁히기 전에는 아무것도 할 수 없다.**
...
`JSON.parse`, `catch`로 잡은 예외, 서드파티 응답처럼 **내가 통제할 수 없는 값**의 기본 타입은 `unknown`이어야 한다. `any` 대신 `unknown`을 쓰면 "검증하지 않고 쓰는 코드"가 전부 컴파일 오류로 드러난다.

### `never` — 존재할 수 없는 값

`never`는 값이 하나도 없는 타입이다. 실무에서 만나는 얼굴은 세 가지다.
...
3번이 핵심이다. 나중에 `{ kind: "triangle" }`을 `Shape`에 추가하는 순간, `s`가 더 이상 `never`가 아니게 되어 **이 `switch`가 컴파일 오류로 손을 든다.** 처리를 빠뜨린 곳을 컴파일러가 찾아주는 것이다. 자세한 활용은 [03-type-guards-narrowing.md](./03-type-guards-narrowing.md)에서 이어진다.

---

## 6. 추론과 명시의 균형

타입을 많이 쓸수록 좋은 것이 아니다. TypeScript의 추론은 강력하고, 불필요한 명시는 오히려 코드를 상하게 한다.
...
**왜 문제인가**: 정보가 늘지 않는데 코드만 길어진다. 더 나쁜 건 유지보수다. 나중에 `names`를 `number[]`로 바꾸면 콜백의 `(n: string)`이 오류를 내면서, 진짜 원인과 상관없는 지점에서 고쳐야 할 곳이 늘어난다.
...
콜백 매개변수에 타입을 적을 필요가 거의 없는 이유가 **문맥적 타이핑(contextual typing)** 때문이다. `map`의 시그니처가 이미 `n`이 무엇인지 알고 있다.

### 명시해야 할 곳

| 위치 | 명시하는 이유 |
|------|--------------|
| 문맥이 없는 함수 선언의 **매개변수** | 추론할 근거가 없다. `noImplicitAny`가 켜져 있으면 강제된다(문맥 추론이 되는 콜백 매개변수는 예외) |
| 모듈 밖으로 나가는 함수의 **반환 타입** | 구현이 실수로 바뀌어도 정의 지점에서 즉시 잡힌다. 안 적으면 오류가 호출부까지 밀린다 |
| 빈 배열, 빈 객체 초기값 | `const items = []`는 요소 타입의 근거가 없다. 이후 `push`를 보고 타입을 넓혀가는 동작이 있지만 흐름이 복잡해지면 `any[]`로 남는다. `const items: Todo[] = []`로 못 박는다 |
| 리터럴을 좁게 유지해야 할 때 | 3절에서 본 `as const` / 대입 지점 타입 명시 |
| 공개 API, 라이브러리 경계 | 타입이 곧 계약이므로 추론에 맡기면 의도치 않게 바뀐다 |

### `satisfies` — 검사는 받되 추론은 잃지 않기

TypeScript 4.9에서 들어온 연산자다. "타입 명시"와 "리터럴 추론 유지" 사이의 오랜 딜레마를 푼다.
...
`satisfies`는 "이 값이 저 타입에 맞는지 검사만 해라, 대신 값의 타입은 원래 추론된 것을 유지해라"는 뜻이다. 설정 객체, 라우트 테이블, 아이콘 맵처럼 **키 목록 자체가 정보인 상수**에 잘 맞는다.

---

## 7. 실무에서는

- **점진적 도입이 가능하다.** `tsconfig.json`의 `allowJs`를 켜면 `.js`와 `.ts`를 한 프로젝트에 섞어둘 수 있다. 기존 JS 프로젝트는 보통 파일 확장자를 하나씩 바꿔가며 옮긴다.
- **처음부터 `strict: true`로 시작하라.** 신규 프로젝트에서 `strict`를 끄고 시작하면 나중에 켜는 순간 오류가 수천 개 뜬다. 반대로 기존 프로젝트를 옮기는 중이라면 옵션을 하나씩 켜는 편이 현실적이다(자세한 옵션 구성은 [03-type-guards-narrowing.md](./03-type-guards-narrowing.md) 참고).
- **타입 검사와 트랜스파일은 분리되는 추세다.** esbuild, SWC 같은 도구는 속도를 위해 타입 검사를 하지 않고 타입 표기만 지운다. 그래서 빌드가 성공해도 타입 오류가 남아 있을 수 있고, CI에 `tsc --noEmit`을 별도 단계로 두는 구성이 표준처럼 쓰인다.
- **`enum` 대신 유니온 + `as const`를 쓰는 팀이 많다.** 일반 `enum`은 타입 소거 원칙의 예외라 컴파일 후에도 런타임 객체를 남긴다. `const enum`은 값을 호출부에 인라인해 이 문제를 피하지만, 파일 하나만 보고 변환하는 도구를 전제하는 `isolatedModules`에서는 사용이 제한된다. 번들에 코드가 남는 점과 이런 제약 때문에 문자열 리터럴 유니온이 선호된다.
- **DefinitelyTyped(`@types/*`)를 기억하자.** 타입 선언이 내장되지 않은 라이브러리는 `npm i -D @types/라이브러리명`으로 커뮤니티 타입을 받아 쓴다.
...
- 꼬리 질문: "그럼 TypeScript를 쓰면 런타임 오류가 없어지나요?" → 아니다. 타입은 컴파일 후 지워지므로 API 응답 같은 외부 입력은 여전히 런타임 검증이 필요하다고 답한다.
...
- 꼬리 질문: "그러면 객체 리터럴의 오타도 못 잡나요?" → 리터럴을 타입이 정해진 자리에 직접 대입할 때는 초과 속성 검사가 추가로 돌아 잡힌다. 다만 변수를 한 번 거치면 그 검사는 작동하지 않는다.
...
- 꼬리 질문: "`extends`와 `&`는 결과가 같나요?" → 속성이 충돌할 때 다르다. `extends`는 선언 시점에 오류를 내고, `&`는 조용히 `never`를 만들어 나중에 이상한 오류로 되돌아온다.
...
- 꼬리 질문: "`never`는 어디에 쓰나요?" → 값이 존재할 수 없는 타입이라, 유니온의 모든 케이스를 처리했는지 컴파일러에게 확인시키는 완전성 검사에 쓴다고 답한다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|------|----------|-----------|
| "TypeScript를 쓰면 런타임 타입 오류가 사라진다" | 타입은 컴파일 후 지워진다. 서버가 이상한 값을 보내면 그대로 통과한다 | 타입은 개발 시점 보증이다. 외부 입력은 경계에서 런타임 검증을 따로 한다 |
...
TypeScript의 타입은 런타임 오류를 편집기 안으로 당겨오는 개발 시점 계약이며, 이름이 아니라 구조로 판단하고 컴파일 후에는 사라지므로 — **경계에서의 런타임 검증과 짝을 이룰 때만 진짜 안전해진다.**
