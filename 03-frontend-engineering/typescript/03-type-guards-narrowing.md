...
> 유니온 타입이 왜 그대로는 쓸모가 없는지, 판별 유니온이 왜 실무에서 가장 자주 쓰이는 무기인지, `as`가 왜 "해결"이 아니라 "회피"인지 설명할 수 있게 된다.
...
값이 두 타입 중 하나일 수 있다고 선언하는 순간, TypeScript는 **양쪽 모두에 있는 것만** 쓰게 해준다.
...
당연한 얘기다. 지금 이 순간 `value`가 문자열일 수도 있으니, `toFixed`를 부르게 놔두면 런타임에 터진다. 컴파일러는 **가능한 모든 경우에 안전한 연산만** 허용한다.

그래서 유니온 타입은 선언만으로는 반쪽짜리다. **"지금 이 값은 실제로 무엇인가"를 런타임에 확인해서 컴파일러에게 알려주는 절차**가 반드시 붙어야 쓸 수 있다. 그 절차가 좁히기(narrowing)이고, 좁히기를 유발하는 검사가 타입 가드(type guard)다.
...
`strictNullChecks`가 켜져 있으면 `null`과 `undefined`가 다른 타입에 자동으로 섞이지 않는다. 그 결과 우리가 매일 만나는 코드가 전부 유니온 좁히기 문제로 바뀐다.
...
JavaScript 프로젝트를 TypeScript로 옮길 때 가장 먼저 쏟아지는 오류도 정교한 타입 불일치가 아니라 대체로 이런 `null` 누락이다. 좁히기는 문법 하나가 아니라 TypeScript를 쓰는 방식 그 자체다.

### 비유: 신분증 확인

유니온 타입은 "이 사람은 직원이거나 방문객이다"라는 명찰이다. 명찰만 보고는 서버실 문을 열어줄 수 없다. 신분증을 확인해서 직원임을 밝힌 뒤에야 그 권한을 준다. 확인이 끝난 구역 안에서만 "이 사람은 직원"이라는 사실이 유효하다.

> **비유의 한계**: 사람은 확인 후에도 계속 그 사람이지만, TypeScript의 좁히기는 **코드 블록을 벗어나거나 값이 재할당되면 즉시 무효**가 된다. 확인 결과가 붙는 대상은 사람이 아니라 "코드의 특정 지점"이다.
...
`return`, `throw`, `continue`로 흐름을 일찍 끊는 **얼리 리턴(early return)** 패턴이 TypeScript에서 특히 잘 맞는 이유다. 중첩된 `if`를 만들지 않으면서 타입도 자연스럽게 좁혀진다.
...
`typeof`가 반환할 수 있는 문자열은 `"string" | "number" | "bigint" | "boolean" | "symbol" | "undefined" | "object" | "function"`이고, TypeScript는 이 값들만 비교 대상으로 허용한다. `typeof v === "strnig"` 같은 오타는 컴파일러가 잡는다.

**함정 하나**: JavaScript에서 `typeof null`은 `"object"`다. 언어 초창기부터 있던 버그인데 호환성 때문에 고치지 못했다.
...
**왜 문제인가**: `typeof null === "object"`이므로 첫 번째 분기에는 `null`이 그대로 들어온다. TypeScript는 이 사실을 알기 때문에 좁히기를 완료해주지 않는다.
...
프로토타입 체인을 보는 검사라 **클래스나 생성자 함수로 만들어진 값**에만 쓸 수 있다. 인터페이스는 컴파일 후 사라지는 타입일 뿐 값이 아니므로, `x instanceof MyInterface`는 `'MyInterface' only refers to a type, but is being used as a value here.` 오류가 난다. 배열은 `Array.isArray`를 쓴다(표준 라이브러리에 `arg is any[]` 타입 가드로 선언되어 있어 좁히기가 작동한다).
...
`in`은 "이 속성이 있을 수 있는 타입"을 전부 남긴다. 선택적 속성으로 구분하려는 설계 자체가 위태롭다는 신호이고, 다음 절의 판별 유니온으로 가야 한다.

### 진릿값과 동등 비교

`if (value)`는 가장 짧은 좁히기지만, 여기 자주 나오는 버그가 있다. `undefined`뿐 아니라 **빈 문자열 `""`, 숫자 `0`, `NaN`도 함께 걸러낸다.**
...
`== null` 비교는 `null`과 `undefined`를 한 번에 배제하는 관용구로 TypeScript도 이를 이해한다. 나머지 값은 그대로 통과시키므로 `0`이나 `""`을 살려야 할 때 안전하다.
...
데이터를 불러오는 화면의 상태를 표현한다고 하자. 처음 떠오르는 방식은 이렇다.
...
각 멤버가 `status`라는 **같은 이름의 속성을, 서로 다른 리터럴 타입으로** 가진다. 이 속성을 판별자(discriminant)라고 부르고, 이런 유니온을 **판별 유니온(discriminated union)**이라 한다. TypeScript는 판별자를 검사하는 것만으로 나머지 속성까지 통째로 좁혀준다.
...
`state.data?.name`이 아니라 `state.data.name`이다. **성공 분기 안에서는 `data`가 반드시 있다는 것이 타입으로 보장**되기 때문이다. 방어 코드가 사라지고, 동시에 "로딩 중인데 에러도 있는" 상태는 애초에 만들 수 없다. 불린 플래그가 8가지 조합 중 4가지를 낭비했다면, 판별 유니온은 **표현 가능한 네 가지 상태가 전부 유효**하다.

### `never`로 완전성 보장하기

판별 유니온의 진짜 배당금은 **나중에 상태를 추가할 때** 나온다.
...
`State`에 `{ status: "cancelled" }`를 추가하는 순간, `default` 블록의 `state`는 더 이상 `never`가 아니게 되어 **이 줄이 컴파일 오류를 낸다.** 상태를 추가한 사람이 처리를 빠뜨린 모든 `switch`를 컴파일러가 찾아준다. 상태 머신, 액션 리듀서, 이벤트 핸들러처럼 케이스가 늘어나는 코드에서 이 안전장치가 결정적이다.

판별자로는 **리터럴 타입**을 써야 한다는 점이 중요하다. `status: string`이면 좁혀지지 않는다. 그래서 판별 유니온은 [01-why-typescript-types.md](./01-why-typescript-types.md)에서 본 리터럴 타입 위에 세워진 구조물이다.
...
컴파일러 입장에서는 `isValidUser`가 `true`를 반환했다는 사실과 `v`의 타입 사이에 아무 연결 고리가 없다. **"이 함수가 true면 인자는 이 타입이다"를 명시적으로 알려주는 문법**이 타입 서술어(type predicate)다.
...
반환 타입 자리에 `boolean` 대신 `매개변수명 is 타입`을 쓴다. 실제로는 여전히 불리언을 반환하지만, 컴파일러는 이 신호를 받아 호출부에서 좁히기를 수행한다.
...
**왜 문제인가**: TypeScript는 함수 본문이 서술어를 실제로 보장하는지 **검사하지 않는다.** `v is User`라고 선언한 순간 그 말을 그대로 믿는다. 잘못 구현한 타입 가드는 `as`와 똑같이 위험하고, 오히려 안전해 보이는 껍데기를 쓰고 있어서 더 나쁘다.
...
필드가 많아지면 이 코드는 금방 감당하기 어려워진다. 그래서 실무에서는 스키마 검증 라이브러리에 맡긴다(7절 참고).

### 단언 함수 — 좁히거나, 던지거나

TypeScript 3.7부터 `asserts` 문법이 생겼다. 조건을 만족하지 않으면 예외를 던지고, 통과하면 **그 지점 이후 전체**에서 타입이 좁혀진다.
...
`if` 블록으로 감싸지 않아도 되므로 들여쓰기가 줄어든다. `null` 배제용으로 `function assertDefined<T>(v: T): asserts v is NonNullable<T>`를 하나 만들어두면 프로젝트 전반에서 쓰인다.

한 가지 제약이 있다. **단언 함수는 호출 대상이 명시적 타입 표기를 가진 이름이어야 한다.** 화살표 함수를 `const`에 담으면 `Assertions require every name in the call target to be declared with an explicit type annotation.` 오류가 나므로, 변수 쪽에 타입을 직접 적거나 위 예시처럼 `function` 선언문을 쓰는 편이 편하다.

---

## 6. 좁히기가 풀리는 상황

좁힌 줄 알았는데 오류가 나는 순간들이 있다. 원리는 하나다. **컴파일러가 "그 사이에 값이 바뀌지 않았다"고 확신할 수 없으면 좁히기를 유지하지 않는다.**
...
**왜 문제인가**: `value`는 `let`이라 언제든 재할당될 수 있다. 콜백은 **나중에** 실행되므로 그 시점의 값이 여전히 문자열이라는 보장이 없다. 컴파일러의 판단이 옳다.
...
**가능하면 `const`를 쓰라**는 일반적인 조언이 TypeScript에서는 타입 안전성 문제로도 이어진다. 재할당도 마찬가지다. 좁힌 뒤에 `value = getValue()`로 다시 값을 넣으면 좁히기는 그 지점에서 초기화된다.

반대로 조건 결과를 변수에 담아두고 나중에 쓰는 패턴은 TypeScript 4.4부터 동작한다.
...
여기에도 조건이 붙는다. **조건을 담은 변수와 검사 대상 변수가 둘 다 `const`**(또는 `readonly` 속성, 재할당되지 않는 매개변수)여야 한다. 어느 한쪽이 `let`이면 컴파일러는 검사 시점과 사용 시점 사이에 값이 바뀌지 않았다고 확신할 수 없어 좁히기를 포기한다.
...
`as unknown as T`는 TypeScript가 직접 단언을 거부한 경우를 억지로 우회하는 문법이다. **컴파일러가 "이건 말이 안 된다"고 말한 것을 무시하는 것**이라 거의 항상 설계 문제의 징후다.
...
세 번째 대안은 zod 같은 스키마 검증 라이브러리다. 스키마 하나로 런타임 검증과 정적 타입을 동시에 얻으므로 둘이 어긋날 수 없다.

### `as`가 정당한 경우

전부 금지는 아니다. **컴파일러가 알 수 없지만 내가 확실히 아는 정보**가 있을 때는 쓴다.
...
기준은 이렇다. **`as`를 쓴 줄 옆에 "왜 안전한가"를 한 줄로 적을 수 있으면 써도 된다.** 적을 수 없으면 그것은 타입 오류를 이해하지 못한 채 덮은 것이다.

---

## 8. `strict` 옵션이 켜는 것들

`tsconfig.json`의 `"strict": true`는 단일 옵션이 아니라 여러 검사를 한 번에 켜는 스위치다. 각각이 무엇을 막는지 알아야 왜 그 오류가 나는지 이해된다.
...
> 표 요약: 실무 영향이 압도적으로 큰 것은 **`strictNullChecks`와 `noImplicitAny`** 둘이다. 레거시 프로젝트를 옮기는 중이라면 이 둘을 먼저 켜고 나머지를 순차적으로 붙이면 된다.

`strict`에 묶인 옵션 목록은 버전에 따라 늘어난다. 위 여덟 개가 오래 유지된 구성이고, TypeScript 5.6에서 `strictBuiltinIteratorReturn`이 추가됐다. 정확한 목록이 필요하면 쓰고 있는 버전의 컴파일러 옵션 문서를 확인하는 편이 안전하다.

`useUnknownInCatchVariables` 덕분에 예외 처리 코드가 이렇게 바뀐다.
...
`noUncheckedIndexedAccess`는 `arr[0]`의 타입을 `T`가 아니라 `T | undefined`로 만든다. `const first = arr[0]; first.name;`이 오류가 되면서 "빈 배열이면?"이라는 질문을 강제로 마주하게 하는 강력한 옵션이지만, 켜는 순간 기존 코드에서 오류가 크게 늘어난다. 그 밖에 `exactOptionalPropertyTypes`("속성 없음"과 "속성이 `undefined`"를 구분), `noImplicitReturns`(일부 경로에서만 반환하는 함수 금지), `noImplicitOverride`(재정의 시 `override` 키워드 강제)도 별도 옵션이다.

---

## 9. 실무에서는

- **API 경계에는 스키마 검증기를 둔다.** 손으로 쓴 `is` 가드는 필드가 늘어나면 유지가 안 되고, 무엇보다 서술어와 구현이 어긋나도 컴파일러가 잡지 않는다. zod, valibot 같은 라이브러리는 스키마 하나로 런타임 검증과 정적 타입을 동시에 만들어내므로 이 둘이 어긋날 수 없다.
- **비동기 상태와 액션 타입은 판별 유니온으로 모델링한다.** TanStack Query가 `status`와 `data`/`error`를 판별 유니온으로 노출하는 것, Redux 계열이 `type` 필드를 판별자로 삼아 리듀서 `switch`에서 `payload` 타입을 좁히는 것이 같은 패턴이다. 여기에 `never` 완전성 검사를 붙여 처리를 빠뜨린 케이스를 잡는다.
- **`!`(non-null 단언) 사용을 린트 규칙으로 막는 팀이 많다.** ESLint의 `@typescript-eslint/no-non-null-assertion`이 그것이다. 대신 실패 시 원인을 알려주는 명시적 예외를 던지도록 한다.
- **`strict`는 신규 프로젝트라면 무조건 처음부터 켠다.** 나중에 켜면 오류가 수천 개 뜨고, 그 압박 때문에 결국 `any`로 덮게 된다.
...
- 꼬리 질문: "`typeof`로 `null`을 걸러낼 수 있나요?" → 없다. `typeof null`이 `"object"`라서 `v !== null`을 따로 검사해야 한다고 답한다.
...
- 꼬리 질문: "판별자로 `string` 타입을 쓰면 되나요?" → 안 된다. 리터럴 타입이어야 컴파일러가 각 멤버를 구별할 수 있다.
...
- 꼬리 질문: "사용자 정의 타입 가드는 안전한가요?" → `is` 서술어와 함수 본문이 일치하는지 컴파일러가 검증하지 않으므로, 잘못 구현하면 `as`와 똑같이 위험하다. 그래서 실무에서는 스키마 검증기에 맡긴다.
...
- 꼬리 질문: "`strict`에 포함되지 않는 유용한 옵션이 있나요?" → `noUncheckedIndexedAccess`가 대표적이다. 배열 인덱스 접근 결과에 `undefined`를 붙여줘서 범위 초과 접근을 잡는다.
...
| "`is` 가드를 쓰면 안전하다" | 서술어와 본문이 일치하는지 컴파일러는 검사하지 않는다 | 잘못 구현한 가드는 `as`와 같다. 서술어가 주장하는 것을 전부 실제로 확인해야 한다 |
| "`as`로 고치면 타입 오류가 해결된다" | 오류를 없앤 게 아니라 검사를 끈 것이다 | 오류는 대개 진짜 문제를 가리킨다. 왜 안전한지 설명할 수 없으면 쓰지 않는다 |
| "한 번 좁히면 계속 유지된다" | 재할당이나 `let` 변수의 콜백 참조에서 풀린다 | 좁히기는 지점에 붙는 사실이다. `const`와 얼리 리턴을 기본으로 삼는다 |
...
유니온 타입은 좁히기와 짝을 이룰 때만 쓸모가 있고, 그 좁히기를 가장 안정적으로 얻는 방법은 리터럴 판별자를 둔 판별 유니온이며, `as`는 좁히기가 아니라 **좁히기를 포기하고 책임을 개발자가 떠안는 선언**이다.
