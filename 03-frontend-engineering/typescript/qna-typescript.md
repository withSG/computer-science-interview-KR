...
### 핵심 답변
TypeScript를 쓰는 가장 큰 이유는 **정적 타입 검사**입니다. 런타임 에러를 프로그래밍 단계에서 미리 막을 수 있기 때문입니다. JavaScript의 동적 타이핑에서는 의도치 않은 타입 오류를 런타임에야 발견하지만, TypeScript는 컴파일 타임에 오류를 잡아 줍니다. IDE 자동완성, 안전한 리팩터링, 협업할 때의 코드 가독성 향상도 이점입니다.
...
- "TypeScript의 단점은 무엇인가요?"
  → 컴파일 시간 증가, 초기 설정 복잡도, 타입 작성 비용이 단점입니다. 하지만 장기적으로 유지보수 비용이 줄어듭니다.
...
### 핵심 답변
`type`과 `interface`는 둘 다 객체의 타입을 정의하는 방법이지만 차이가 있습니다. **Interface**는 `extends`로 확장하고, 같은 이름으로 다시 선언하면 자동으로 병합(선언 병합)됩니다. **Type**은 `&`(intersection)으로 확장하며 같은 이름으로는 중복 선언할 수 없습니다. 권장하는 쓰임도 갈립니다. 객체 타입에는 Interface, 유니온·튜플·원시 타입 별칭에는 Type을 권장합니다.
...
- "라이브러리 개발 시 Interface가 더 유리한 이유는?"
  → 선언 병합 덕분에 사용자가 타입을 확장할 수 있어서, 유연한 타입 확장성을 제공할 수 있습니다.
...
### 핵심 답변
기존 타입을 변환해 새 타입을 만들어 주는 내장 제네릭 타입이 TypeScript Utility Types입니다. 반복되는 타입 정의가 줄어듭니다. 타입을 재사용하기도 쉬워집니다. 자주 쓰는 것은 `Partial`, `Required`, `Pick`, `Omit`, `Record`, `Readonly` 등입니다.
...
- "Pick과 Omit 중 어떤 것을 언제 사용하나요?"
  → 선택할 프로퍼티가 적을 때는 Pick, 제외할 프로퍼티가 적을 때는 Omit을 쓰는 편이 더 간결합니다.
...
### 핵심 답변
제네릭은 **타입을 파라미터로 받는 기능**입니다. 덕분에 다양한 타입에서 재사용할 수 있는 타입-안전(type-safe) 코드를 작성할 수 있습니다. `any`를 쓰면 타입 안전성을 잃지만, 제네릭은 유연성과 타입 안전성을 동시에 챙깁니다. 함수, 클래스, 인터페이스 어디에나 적용됩니다.
...
### 꼬리 질문 대비
- "제네릭과 오버로딩의 차이는?"
  → 오버로딩은 여러 시그니처를 정의하고, 제네릭은 하나의 시그니처로 다양한 타입을 처리합니다. 제네릭이 더 간결합니다.
...
### 핵심 답변
런타임에 특정 타입을 확인해서, TypeScript가 그 블록 안에서 타입을 좁히도록(narrowing) 만드는 기능이 타입 가드입니다. 쓰는 방법은 `typeof`, `instanceof`, `in` 연산자, 그리고 사용자 정의 타입 가드 함수입니다. 유니온 타입을 다룰 때 특히 유용합니다. 특정 타입에 맞는 로직을 안전하게 실행하게 해 주기 때문입니다.
...
- "타입 단언(as)과 타입 가드의 차이는?"
  → 타입 단언은 개발자가 강제로 타입을 지정하는 것이라 런타임 오류 위험이 있습니다. 타입 가드는 실제 런타임 값을 확인해서 안전하게 타입을 좁힙니다.
...
### 핵심 답변
`any`는 모든 타입을 허용하고 타입 검사를 완전히 우회해서 TypeScript의 안전성을 포기합니다. `unknown`도 모든 타입을 허용하지만 사용하기 전에 타입 확인이 필요해서 `any`보다 안전합니다. `never`는 절대 발생할 수 없는 타입입니다. 무한 루프 함수나 항상 예외를 던지는 함수의 반환 타입이 여기에 해당합니다.
...
- "외부 API 응답 처리 시 어떤 타입을 사용하는 것이 좋나요?"
  → unknown으로 받아서 런타임 타입 검증(타입 가드 또는 zod 등 라이브러리)을 거친 뒤에 사용합니다.
