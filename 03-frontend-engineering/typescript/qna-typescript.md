...
### 핵심 답변
TypeScript를 쓰는 가장 큰 이유는 **정적 타입 검사**입니다. 런타임 에러를 프로그래밍 단계에서 미리 막을 수 있기 때문입니다. JavaScript의 동적 타이핑에서는 의도치 않은 타입 오류를 런타임에야 발견하지만, TypeScript는 컴파일 타임에 오류를 잡아 줍니다. IDE 자동완성, 안전한 리팩토링, 협업할 때의 코드 가독성 향상도 이점입니다.
