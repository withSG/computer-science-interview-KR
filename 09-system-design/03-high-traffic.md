...
> **비유의 한계**: 사람은 밀리면 알아서 멈추지만 클라이언트는 실패하면 **더 세게 재시도합니다.** 느려질수록 부하가 커지는 양의 되먹임이 있다는 점이 결정적으로 다릅니다.
...
> 한 줄 결론: **위로 갈수록 늘리기 쉽고 아래로 갈수록 어렵습니다.** 모든 최적화의 방향은 아래 계층에 도달하는 요청 수를 줄이는 것입니다.
...
**애플리케이션 서버를 늘리는 것은 DB 입장에서 부하를 늘리는 행위**입니다. 무상태 계층은 늘리기 쉽지만 그 뒤의 상태를 가진 계층은 같이 늘어나지 않습니다.
...
구조적으로는 읽기를 Redis 캐시로 흡수하고(적중률 90%면 DB 부하가 1/10), 조회는 읽기 복제본으로 분리하며, 즉시성 없는 쓰기는 큐로 뺍니다. 그리고 `connection-timeout`을 짧게 두는 것이 중요합니다. 풀 고갈 시 30초씩 기다리면 그 스레드가 30초 동안 묶이고 요청이 쌓여 애플리케이션까지 함께 죽습니다. **빨리 실패하는 것이 오래 기다리는 것보다 낫습니다.**
...
응답이 빨라지는 것만 이득이 아닙니다. 더 중요한 것은 **버퍼 효과**입니다.
...
큐는 **처리 능력을 늘리지 않습니다.** 넘치는 부하를 시간축으로 펴서 처리 능력 안에 들어오게 할 뿐입니다. 이것이 부하 평탄화(load leveling)다.
...
부가 기능 하나가 느려졌을 뿐인데 서비스 전체가 죽었습니다. 이것이 연쇄 장애(cascading failure)이고, 이걸 막는 장치가 넷인데 **순서가 있습니다.**
...
핵심은 OPEN의 의미입니다. **실패할 것이 뻔한 호출을 하지 않고 즉시 대체 응답을 줍니다.** 호출자는 타임아웃만큼 기다리지 않아도 되고, 피호출자는 부하가 빠져 회복할 시간을 얻습니다.
...
`minimum-number-of-calls`는 표본이 적을 때의 오판을 막습니다. 2건 중 1건 실패했다고 회로를 열면 안 됩니다. `slow-call` 설정은 특히 중요한데, **응답이 느린 것은 실패로 잡히지 않지만 실질적으로 더 나쁩니다.** 실패는 스레드를 즉시 돌려주지만 느린 응답은 계속 묶어두기 때문입니다.
...
더 나쁜 함정도 있습니다. **인스턴스가 무너지기 시작하면 메트릭이 오히려 정상으로 보입니다.** 스레드 풀이 고갈되어 요청을 아예 못 받으면 CPU 사용률이 떨어지고, CPU 기반 정책은 이때 "부하가 줄었네"라며 확장을 멈춥니다.
...
- **부하 테스트 없이 만든 대응책은 추측입니다.** k6, JMeter, Gatling으로 예상 트래픽의 1.5~2배를 실제로 때려보면 예상하지 못한 곳이 먼저 무너지는 것을 거의 항상 발견합니다. 테스트 환경은 운영과 같은 구성이어야 하고, 특히 DB 데이터 양이 비슷해야 합니다. 데이터가 적으면 인덱스 없이도 잘 돕니다.
- **지표는 평균이 아니라 백분위로 봅니다.** 평균 200ms인데 P99가 8초면 100명 중 1명은 8초를 기다립니다. 트래픽이 100만이면 1만 명입니다. 평균은 이 사실을 감춥니다.
- **캐시 만료가 몰리지 않게 합니다.** 인기 상품의 캐시가 동시에 만료되면 그 순간 모든 요청이 DB로 간다(cache stampede). TTL에 무작위 값을 더해 만료 시각을 흩고, 만료 시 하나의 요청만 DB에 가도록 잠금을 두거나, 만료된 값을 잠시 그대로 반환하면서 뒤에서 갱신하는 방식을 씁니다.
- **장애 대응은 사람이 아니라 시스템이 해야 합니다.** 새벽 3시에 누군가 깨어 있어야만 살아남는 구조는 설계가 덜 된 것입니다.
