...
> 캐시가 어떤 문제를 풀어주는지, 그 대가로 어떤 문제를 새로 만드는지, 그리고 읽기·쓰기·무효화 전략을 상황에 맞게 고르는 기준을 설명할 수 있게 된다.
...
시스템의 각 계층은 데이터를 꺼내오는 비용이 극단적으로 다르다. 아래는 정확한 값이 아니라 **자릿수 감각**을 잡기 위한 표다. 하드웨어와 환경에 따라 달라지지만, 계층 간 차이가 10배·100배 단위라는 사실은 변하지 않는다.
...
여기서 캐시의 아이디어가 나온다. **"비싼 계산이나 조회의 결과를, 다음에 같은 요청이 왔을 때 다시 하지 않도록 싼 곳에 적어두자."**

### 캐시가 이득을 보려면 두 가지 전제가 필요하다

캐시는 공짜가 아니다. 아래 두 조건이 깨지면 캐시는 메모리만 먹고 아무것도 개선하지 못한다.

1. **지역성(locality)** — 한 번 조회된 데이터가 다시 조회될 확률이 높아야 한다. 모든 사용자가 완전히 다른 데이터를 한 번씩만 본다면 히트율은 0에 수렴한다.
2. **읽기 편중** — 읽기가 쓰기보다 훨씬 많아야 한다. 쓸 때마다 캐시를 지워야 하므로, 쓰기가 잦으면 캐시는 채워지자마자 버려진다.

### 캐시가 만드는 새 문제: 정합성

같은 데이터의 사본이 두 곳(DB와 캐시)에 생기는 순간, "둘이 다를 수 있다"는 문제가 필연적으로 따라온다.
...
캐시 설계의 거의 모든 논쟁은 **"이 불일치 구간을 얼마나 짧게 만들 것인가, 그 대가로 성능을 얼마나 포기할 것인가"** 한 줄로 요약된다. 아래 나오는 전략들은 전부 이 질문에 대한 서로 다른 답이다.

### 비유

자주 보는 책 몇 권을 도서관에서 빌려 책상 위에 올려두는 것과 같다. 도서관까지 걸어가는 시간(DB 조회)을 아끼는 대신, 책상 공간(메모리)을 쓴다.

> **비유의 한계**: 실제 책은 도서관 원본이 개정되면 내 책상 사본이 낡았다는 사실을 알려주지 않는다. 캐시도 마찬가지여서, "언제 사본을 버릴 것인가"를 시스템이 직접 정해야 한다. 이 문제가 캐싱에서 가장 어려운 부분이다.
...
- **필요한 데이터만 캐시에 올라간다.** 조회되지 않은 데이터는 메모리를 차지하지 않는다.
- **캐시가 죽어도 서비스는 산다.** `get`이 실패하면 DB로 우회하면 된다(단, DB가 그 부하를 견딜 수 있어야 한다).
- **첫 요청은 항상 느리다.** 캐시가 비어 있는 이 상태를 콜드 캐시(cold cache)라고 부른다. 배포 직후나 캐시 재시작 직후에 응답 시간이 튀는 원인이다.
- **불일치를 애플리케이션이 직접 관리해야 한다.** 데이터를 바꾸는 모든 코드 경로에서 캐시를 지워야 한다. 하나라도 빠뜨리면 그 데이터는 TTL이 끝날 때까지 낡은 값을 반환한다.

### Read-Through

Cache-Aside와 결과는 같지만 **DB를 읽는 책임이 애플리케이션이 아니라 캐시 계층에 있다.** 캐시에 "미스가 나면 이렇게 채워라"라는 로더를 등록해두는 방식이다.
...
Spring의 `@Cacheable`도 같은 형태다. 캐시 조회·미스 판정·저장이 프록시 안으로 숨는다.
...
Cache-Aside 대비 장점은 **캐시 조회 코드가 비즈니스 로직에서 사라진다**는 것이고, 단점은 **동작이 프레임워크에 숨어 통제하기 어렵다**는 것이다. 캐시 실패 시 어떻게 할지, 키를 어떻게 조합할지 같은 세밀한 제어가 필요하면 Cache-Aside가 낫다.
...
**왜 문제인가**: 존재하지 않는 ID(`user:99999999`)로 요청이 오면 캐시에는 영원히 아무것도 저장되지 않으므로, **그 요청은 매번 DB까지 내려간다.** 공격자가 없는 ID를 무작위로 대량 호출하면 캐시를 완전히 우회해 DB만 때리게 된다. 이것을 **캐시 관통(cache penetration)**이라고 부른다.
...
"없음"의 TTL을 짧게 주는 이유는, 나중에 그 ID로 데이터가 실제로 생성됐을 때 오래 안 보이면 곤란하기 때문이다. 키 공간이 넓고 공격 가능성이 크다면 블룸 필터(Bloom Filter)로 "확실히 없는 키"를 캐시 앞단에서 걸러내는 방법도 함께 쓴다.
...
> 결론: **기본값은 "Cache-Aside 읽기 + Write-Around 쓰기" 조합**이다. Write-Around의 원래 정의는 "쓰기가 캐시를 우회한다"까지지만 그대로 두면 기존 키가 낡은 채 남으므로, 실무에서는 여기에 **기존 키 삭제(무효화)를 덧붙인 형태**로 쓴다. 실무에서 캐싱이라고 하면 대개 이 조합을 가리킨다. Write-Through는 정합성이 돈과 직결되는 소수의 키에만, Write-Back은 유실을 감수할 수 있는 카운터류에만 제한적으로 쓴다.
...
**왜 문제인가**: 1번과 3번 사이에 다른 요청이 이 상품을 읽으면, 아직 커밋되지 않은 **옛날 값**을 DB에서 읽어 캐시에 다시 채워버린다. 트랜잭션이 커밋된 뒤에도 캐시에는 옛날 값이 남고, TTL이 끝날 때까지 아무도 그것을 고칠 수 없다. 캐시를 지웠는데 오히려 낡은 값이 고정되는, 재현이 매우 어려운 버그다.
...
`@TransactionalEventListener(AFTER_COMMIT)`은 트랜잭션이 실제로 커밋된 뒤에만 리스너를 실행한다. 롤백되면 캐시도 건드리지 않으므로, "DB는 안 바뀌었는데 캐시만 날아가는" 반대 방향 사고도 함께 막힌다.
...
TTL(Time To Live)은 저장할 때 만료 시각을 함께 지정해 그 시간이 지나면 자동으로 사라지게 하는 것이다. 구현이 단순하고 코드를 오염시키지 않는 대신, **TTL 동안은 낡은 값이 보인다.**

TTL의 진짜 가치는 성능이 아니라 **안전망**에 있다. 아래 이벤트 기반 무효화는 반드시 어딘가에서 누락된다(새로 추가된 API, 배치 잡, DBA의 직접 수정 등). TTL이 걸려 있으면 그 누락의 영향이 "영구히"에서 "최대 N분"으로 줄어든다. **TTL 없는 캐시 키는 만들지 않는 것을 규칙으로 삼는 편이 좋다.**

### 이벤트 기반 무효화 — 정확성

데이터가 바뀌는 순간 캐시를 명시적으로 삭제한다. 앞의 `@TransactionalEventListener` 예제가 여기에 해당한다. 즉시 일관성에 가까워지지만 두 가지 약점이 있다.

- **누락**: 캐시를 지우는 코드를 안 쓴 경로가 하나라도 있으면 그곳이 구멍이 된다.
- **전파**: 서버가 여러 대이고 각자 로컬 캐시를 들고 있다면, 한 서버에서 지운 것을 나머지 서버가 알아야 한다(6절 참고).

애플리케이션 코드를 거치지 않는 변경(다른 팀의 배치, 수동 SQL)까지 잡아야 한다면, DB의 변경 로그를 읽어 이벤트로 바꾸는 CDC(Change Data Capture, 예: Debezium) 방식을 쓴다. 애플리케이션이 아니라 **DB가 진실의 원천**이 되므로 누락이 구조적으로 줄어든다.

### 태그 기반 무효화

한 데이터가 여러 캐시에 영향을 줄 때 쓴다. 사용자 프로필이 바뀌면 그 사용자가 쓴 게시글 목록 캐시, 댓글 캐시가 전부 낡는다. 이때 각 캐시에 `user:42` 태그를 달아두고 태그 단위로 일괄 삭제한다. Redis에서는 태그를 Set으로 관리해(`SADD tag:user:42 "post:100" "comments:user:42"` 후 무효화 시 `SMEMBERS`로 꺼내 일괄 `DEL`) 흉내 낼 수 있다. 편리하지만 태그 인덱스 자체를 관리해야 하므로 메모리와 복잡도가 늘어난다. 연관 캐시가 정말 많을 때만 쓴다.
...
1. **분산 락** — 미스가 난 요청 중 하나만 DB를 조회하게 만든다. 나머지는 짧게 기다렸다가 캐시를 다시 읽는다. 락에는 반드시 TTL을 걸어, 락을 쥔 프로세스가 죽어도 영구 교착이 생기지 않게 한다.
...
재시도를 무한 루프나 무한 재귀로 짜면, 락을 쥔 요청이 실패했을 때 대기하던 요청들이 영원히 돌게 된다. **횟수 상한과 최종 폴백 경로를 반드시 둔다.**

2. **Stale-While-Revalidate** — 만료돼도 일정 시간은 낡은 값을 그대로 응답하고, 갱신은 백그라운드로 돌린다. 사용자는 대기하지 않고 DB도 한 번만 맞는다. HTTP 캐시 표준과 CDN에도 같은 이름의 지시어가 있다.
3. **확률적 사전 갱신** — 만료가 가까워질수록 확률적으로 미리 한 요청이 갱신을 시도하게 한다(XFetch 계열). 만료 시각이 자연스럽게 흩어진다.

보조 기법으로 **TTL 지터(jitter)**가 있다. 배포 시점에 한꺼번에 채워진 캐시들이 정확히 같은 시각에 만료되면 스탬피드가 여러 키에서 동시에 터진다. `TTL = 600초 ± 랜덤 60초`처럼 흔들어 두면 만료가 분산된다. 한 줄이면 되는데 효과가 크다.

### 핫키 (Hot Key)

특정 키 하나에 트래픽이 몰려 그 키를 담당하는 캐시 **노드 한 대**가 포화되는 현상이다. 아이돌 콘서트 티켓 상품, 메인 화면 배너처럼 전체 트래픽의 상당 부분이 키 하나로 향할 때 생긴다. 캐시가 여러 노드로 샤딩되어 있어도 소용없다. 키가 하나면 노드도 하나다.

대응:

- **로컬 캐시로 앞단을 덮는다.** 애플리케이션 서버마다 이 키를 몇 초간 메모리에 들고 있으면, 노드로 가는 요청 수가 서버 대수만큼으로 줄어든다. 가장 효과가 크다.
- **키를 쪼갠다.** `banner:main`을 `banner:main:0` ~ `banner:main:9`로 복제해두고 요청마다 무작위로 하나를 고른다. 키가 달라지면 해시 결과도 달라져 여러 노드로 흩어질 가능성이 높으므로 부하가 분산된다(어느 키가 어느 노드로 갈지는 해시가 정하므로 완전히 균등하지는 않다). 대신 갱신할 때 열 개를 다 지워야 한다.

### 캐시 관통 (Cache Penetration)

2절에서 다룬, 존재하지 않는 키가 캐시를 그냥 통과해 DB로 가는 문제다. "없음"을 짧은 TTL로 캐시하거나 블룸 필터로 앞에서 거른다. 세 가지를 한 문장으로 구분하면, **스탬피드는 "있던 게 사라진 순간"의 문제, 핫키는 "한 키가 너무 인기 있는" 문제, 관통은 "애초에 없는 키"의 문제다.**
...
| 무효화 | 어렵다. 전 서버에 전파해야 한다 | 쉽다. 한 번 지우면 끝 |
...
> 결론: **바뀌지 않는 소량의 데이터는 로컬, 그 외에는 분산.** 로컬 캐시의 치명적 약점은 무효화 전파이므로, "TTL이 만료될 때까지 서버마다 값이 달라도 괜찮은가?"에 그렇다고 답할 수 있는 데이터만 로컬에 둔다.
...
- **캐시 히트율을 항상 관측한다.** 히트율이 낮으면(예: 50% 미만) 캐시가 일을 안 하면서 메모리와 복잡도만 소모하고 있다는 뜻이다. 캐시 키 설계가 너무 잘게 쪼개져 있거나, 애초에 캐시할 만한 워크로드가 아니거나 둘 중 하나다.
- **캐시 미스 경로의 p99를 따로 본다.** 히트율 99%면 평균 응답 시간은 훌륭해 보이지만, p99는 정확히 그 미스 1%가 결정한다. "캐시 달았으니 끝"이 아니라 미스 경로가 감당 가능한지 확인해야 한다.
- **Spring 진영에서는** `spring-boot-starter-cache` + `@Cacheable` / `@CacheEvict` 조합이 표준이고, 캐시 구현체로 Caffeine(로컬)이나 Redis(분산)를 꽂는다. 다만 `@Cacheable`은 프록시 기반이라 같은 클래스 내부 호출(self-invocation)에서는 동작하지 않는다는 함정이 있다.
- **캐시를 지우는 코드는 도메인 이벤트로 몰아둔다.** 서비스 메서드마다 `redisTemplate.delete(...)`가 흩어져 있으면 누락은 시간 문제다. "가격이 바뀌었다"는 이벤트를 발행하고 리스너 한 곳에서 관련 캐시를 정리하는 구조가 훨씬 오래 버틴다.
...
- 꼬리 질문: "캐시를 먼저 지우고 DB를 수정하면 안 되나요?" → 안 된다. 삭제와 커밋 사이에 다른 요청이 커밋 전의 옛날 값을 읽어 캐시를 다시 채우면, TTL이 끝날 때까지 낡은 값이 고정된다.
...
- 꼬리 질문: "그럼 왜 Cache-Aside가 더 많이 쓰이나요?" → 캐시가 죽었을 때의 동작을 애플리케이션이 직접 정할 수 있고, 캐시 구현체에 덜 묶이기 때문이다.
...
- 꼬리 질문: "분산 락을 걸었는데 락을 쥔 서버가 죽으면요?" → 락 키에 TTL을 걸어두면 자동 해제된다. TTL 없는 분산 락은 영구 교착의 원인이다.
...
- 꼬리 질문: "핫키 문제는 어떻게 푸나요?" → 그 키만 로컬 캐시로 덮어 Redis 노드로 가는 요청을 서버 대수만큼으로 줄이거나, 키를 여러 개로 복제해 노드에 분산시킨다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|------|----------|-----------|
| 데이터 변경 시 캐시를 새 값으로 갱신한다 | 동시 쓰기에서 DB 쓰기 순서와 캐시 쓰기 순서가 뒤집히면 불일치가 영구히 남는다 | 갱신하지 말고 **삭제**한다. 삭제는 멱등해서 순서가 뒤집혀도 결과가 같다 |
| 캐시를 지운 뒤 DB를 수정한다 | 그 사이에 다른 요청이 커밋 전 값을 읽어 캐시를 다시 채운다 | DB 커밋 완료 후에 삭제한다 (`@TransactionalEventListener(AFTER_COMMIT)`) |
| 이벤트 기반 무효화를 하니 TTL은 필요 없다 | 무효화 호출은 반드시 어딘가에서 누락된다. 누락되면 그 키는 영원히 낡는다 | TTL은 성능이 아니라 **무효화 실패의 영향 시간을 제한하는 안전망**이다 |
...
캐시는 "같은 데이터의 사본을 싼 곳에 두는 것"이고, 그 순간부터 모든 설계는 **원본과 사본이 어긋나는 시간을 얼마나 짧게 만들지, 그 대가로 얼마나 느려질지**를 고르는 일이 된다.
