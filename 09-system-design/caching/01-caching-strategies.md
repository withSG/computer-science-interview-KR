...
> 캐시가 어떤 문제를 풀어주는지, 그 대가로 어떤 문제를 새로 만드는지, 그리고 읽기·쓰기·무효화 전략을 상황에 맞게 고르는 기준을 설명할 수 있게 됩니다.
...
같은 데이터의 사본이 두 곳(DB와 캐시)에 생기는 순간, "둘이 다를 수 있다"는 문제가 필연적으로 따라옵니다.
...
캐시 설계의 거의 모든 논쟁은 **"이 불일치 구간을 얼마나 짧게 만들 것인가, 그 대가로 성능을 얼마나 포기할 것인가"** 한 줄로 요약됩니다. 아래 나오는 전략들은 전부 이 질문에 대한 서로 다른 답입니다.

### 비유

자주 보는 책 몇 권을 도서관에서 빌려 책상 위에 올려두는 것과 같습니다. 도서관까지 걸어가는 시간(DB 조회)을 아끼는 대신, 책상 공간(메모리)을 씁니다.

> **비유의 한계**: 실제 책은 도서관 원본이 개정되면 내 책상 사본이 낡았다는 사실을 알려주지 않습니다. 캐시도 마찬가지여서, "언제 사본을 버릴 것인가"를 시스템이 직접 정해야 합니다. 이 문제가 캐싱에서 가장 어려운 부분입니다.
...
가장 널리 쓰이는 방식입니다. **애플리케이션이 캐시를 직접 조회하고, 없으면 DB에서 읽어 캐시에 채운다.** 캐시는 그냥 옆에 놓인(aside) 저장소일 뿐이고, 모든 판단은 애플리케이션 코드가 합니다.
...
- **필요한 데이터만 캐시에 올라간다.** 조회되지 않은 데이터는 메모리를 차지하지 않습니다.
- **캐시가 죽어도 서비스는 산다.** `get`이 실패하면 DB로 우회하면 된다(단, DB가 그 부하를 견딜 수 있어야 한다).
- **첫 요청은 항상 느리다.** 캐시가 비어 있는 이 상태를 콜드 캐시(cold cache)라고 부릅니다. 배포 직후나 캐시 재시작 직후에 응답 시간이 튀는 원인입니다.
- **불일치를 애플리케이션이 직접 관리해야 한다.** 데이터를 바꾸는 모든 코드 경로에서 캐시를 지워야 합니다. 하나라도 빠뜨리면 그 데이터는 TTL이 끝날 때까지 낡은 값을 반환합니다.

### Read-Through

Cache-Aside와 결과는 같지만 **DB를 읽는 책임이 애플리케이션이 아니라 캐시 계층에 있다.** 캐시에 "미스가 나면 이렇게 채워라"라는 로더를 등록해두는 방식입니다.
...
Cache-Aside 대비 장점은 **캐시 조회 코드가 비즈니스 로직에서 사라진다**는 것이고, 단점은 **동작이 프레임워크에 숨어 통제하기 어렵다**는 것입니다. 캐시 실패 시 어떻게 할지, 키를 어떻게 조합할지 같은 세밀한 제어가 필요하면 Cache-Aside가 낫습니다.
...
**왜 문제인가**: 존재하지 않는 ID(`user:99999999`)로 요청이 오면 캐시에는 영원히 아무것도 저장되지 않으므로, **그 요청은 매번 DB까지 내려간다.** 공격자가 없는 ID를 무작위로 대량 호출하면 캐시를 완전히 우회해 DB만 때리게 됩니다. 이것을 **캐시 관통(cache penetration)**이라고 부릅니다.
...
캐시를 지우는 대신 새 값으로 덮어쓰고 싶은 유혹이 있습니다. 다음 읽기가 빨라지니까. 하지만 두 요청이 동시에 같은 데이터를 수정하면 이런 일이 생깁니다.
...
DB 쓰기 순서와 캐시 쓰기 순서가 뒤집히면 불일치가 **영구히** 남습니다. **캐시는 갱신하지 말고 삭제하라(delete, don't update).** 삭제는 순서가 뒤집혀도 결과가 같다(멱등하다). 다음 읽기가 알아서 최신 값을 채웁니다.
...
TTL(Time To Live)은 저장할 때 만료 시각을 함께 지정해 그 시간이 지나면 자동으로 사라지게 하는 것입니다. 구현이 단순하고 코드를 오염시키지 않는 대신, **TTL 동안은 낡은 값이 보인다.**

TTL의 진짜 가치는 성능이 아니라 **안전망**에 있습니다. 아래 이벤트 기반 무효화는 반드시 어딘가에서 누락된다(새로 추가된 API, 배치 잡, DBA의 직접 수정 등). TTL이 걸려 있으면 그 누락의 영향이 "영구히"에서 "최대 N분"으로 줄어듭니다. **TTL 없는 캐시 키는 만들지 않는 것을 규칙으로 삼는 편이 좋다.**
...
재시도를 무한 루프나 무한 재귀로 짜면, 락을 쥔 요청이 실패했을 때 대기하던 요청들이 영원히 돌게 됩니다. **횟수 상한과 최종 폴백 경로를 반드시 둔다.**
...
3. **확률적 사전 갱신** — 만료가 가까워질수록 확률적으로 미리 한 요청이 갱신을 시도하게 한다(XFetch 계열). 만료 시각이 자연스럽게 흩어집니다.
...
- **로컬 캐시로 앞단을 덮는다.** 애플리케이션 서버마다 이 키를 몇 초간 메모리에 들고 있으면, 노드로 가는 요청 수가 서버 대수만큼으로 줄어듭니다. 가장 효과가 큽니다.
- **키를 쪼갠다.** `banner:main`을 `banner:main:0` ~ `banner:main:9`로 복제해두고 요청마다 무작위로 하나를 고릅니다. 키가 달라지면 해시 결과도 달라져 여러 노드로 흩어질 가능성이 높으므로 부하가 분산된다(어느 키가 어느 노드로 갈지는 해시가 정하므로 완전히 균등하지는 않다). 대신 갱신할 때 열 개를 다 지워야 합니다.

### 캐시 관통 (Cache Penetration)

2절에서 다룬, 존재하지 않는 키가 캐시를 그냥 통과해 DB로 가는 문제입니다. "없음"을 짧은 TTL로 캐시하거나 블룸 필터로 앞에서 거릅니다. 세 가지를 한 문장으로 구분하면, **스탬피드는 "있던 게 사라진 순간"의 문제, 핫키는 "한 키가 너무 인기 있는" 문제, 관통은 "애초에 없는 키"의 문제다.**
...
| 무효화 | 어렵습니다. 전 서버에 전파해야 한다 | 쉽습니다. 한 번 지우면 끝 |
...
L1에 짧은 TTL을 걸어 대부분의 요청을 프로세스 안에서 끝내고, 데이터가 바뀌면 Redis Pub/Sub으로 전 서버에 "이 키 버려라" 메시지를 뿌립니다. 다만 Pub/Sub 메시지는 유실될 수 있으므로 **L1의 TTL을 짧게(수 초~수십 초) 잡아 최종 안전망을 남겨두는 것이 필수입니다. 무효화 전파를 100% 신뢰하는 구조는 만들지 않습니다.

---

## 7. 실무에서는

- **캐시 히트율을 항상 관측한다.** 히트율이 낮으면(예: 50% 미만) 캐시가 일을 안 하면서 메모리와 복잡도만 소모하고 있다는 뜻입니다. 캐시 키 설계가 너무 잘게 쪼개져 있거나, 애초에 캐시할 만한 워크로드가 아니거나 둘 중 하나입니다.
- **캐시 미스 경로의 p99를 따로 본다.** 히트율 99%면 평균 응답 시간은 훌륭해 보이지만, p99는 정확히 그 미스 1%가 결정합니다. "캐시 달았으니 끝"이 아니라 미스 경로가 감당 가능한지 확인해야 합니다.
...
- **캐시를 지우는 코드는 도메인 이벤트로 몰아둔다.** 서비스 메서드마다 `redisTemplate.delete(...)`가 흩어져 있으면 누락은 시간 문제입니다. "가격이 바뀌었다"는 이벤트를 발행하고 리스너 한 곳에서 관련 캐시를 정리하는 구조가 훨씬 오래 버팁니다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|------|----------|-----------|
| 데이터 변경 시 캐시를 새 값으로 갱신한다 | 동시 쓰기에서 DB 쓰기 순서와 캐시 쓰기 순서가 뒤집히면 불일치가 영구히 남는다 | 갱신하지 말고 **삭제**합니다. 삭제는 멱등해서 순서가 뒤집혀도 결과가 같다 |
| 캐시를 지운 뒤 DB를 수정한다 | 그 사이에 다른 요청이 커밋 전 값을 읽어 캐시를 다시 채운다 | DB 커밋 완료 후에 삭제한다 (`@TransactionalEventListener(AFTER_COMMIT)`) |
| 이벤트 기반 무효화를 하니 TTL은 필요 없다 | 무효화 호출은 반드시 어딘가에서 누락됩니다. 누락되면 그 키는 영원히 낡는다 | TTL은 성능이 아니라 **무효화 실패의 영향 시간을 제한하는 안전망**이다 |
