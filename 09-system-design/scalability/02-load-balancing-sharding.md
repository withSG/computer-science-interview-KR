...
> 트래픽을 여러 서버로 나누는 문제와 데이터를 여러 DB로 나누는 문제가 왜 난이도가 다른지, 그리고 샤딩 키 하나가 왜 시스템의 수명을 결정하는지 설명할 수 있게 됩니다.
...
서버 한 대의 성능을 올리는 수직 확장(scale-up)에는 두 가지 벽이 있습니다. **가격이 성능보다 훨씬 가파르게 오르고**, 어느 지점 이상은 그런 하드웨어가 아예 존재하지 않습니다. 그리고 결정적으로 **서버가 한 대라는 것은 그 한 대가 죽으면 서비스가 죽는다**는 뜻입니다.
...
**두 문제의 난이도 차이가 이 문서의 주제다.** 애플리케이션 서버는 오늘 늘렸다 내일 줄여도 아무 일이 없지만, 샤딩 키를 잘못 고르면 몇 년 뒤 서비스 전체를 멈추고 데이터를 옮겨야 합니다.
...
> **비유의 한계**: 캐비닛은 문을 닫고 며칠 정리하면 되지만, 운영 중인 서비스는 그 며칠을 멈출 수 없습니다. 그래서 실제 리샤딩은 "이중 쓰기 → 과거 데이터 복사 → 검증 → 읽기 전환"처럼 여러 단계를 겹쳐 진행해야 하고, 이 복잡함이 샤딩 키를 사실상 못 바꾸게 만드는 진짜 이유입니다.
...
> 선택 기준 한 줄: **요청 처리 시간이 균일하면 라운드 로빈, 들쭉날쭉하면 최소 연결.** IP 해시는 "세션을 외부로 뺄 수 없는 사정이 있을 때"의 차선책이지 기본값이 아닙니다.

### 라운드 로빈의 함정

라운드 로빈은 **요청 개수**를 균등하게 나눌 뿐 **부하**를 균등하게 나누지 않습니다.
...
같은 클라이언트를 항상 같은 서버로 보내는 것을 세션 고정이라고 합니다. 세션을 서버 메모리에 들고 있을 때는 이렇게 하지 않으면 로그인이 유지되지 않습니다. 하지만 세 가지 문제가 따라옵니다.

1. **그 서버가 죽으면 세션도 죽는다.** 사용자는 로그아웃됩니다.
2. **부하가 기운다.** 오래 접속한 사용자가 특정 서버에 몰리면 그 서버만 무거워집니다.
3. **배포와 오토스케일링이 어렵다.** 서버를 내리려면 그 서버에 붙은 세션이 다 끝나기를 기다려야 합니다.

근본 해법은 **세션을 서버 밖으로 빼는 것**입니다. Redis 같은 외부 저장소에 세션을 두면 모든 서버가 같은 세션을 볼 수 있고, 애플리케이션 서버는 무상태가 되어 자유롭게 늘리고 줄일 수 있습니다. 세션 고정은 그 작업을 못 한 시스템의 임시방편으로 이해하는 것이 맞습니다.
...
TLS 종료를 짚어둘 만합니다. L7이 암호화를 대신 풀어주면 **뒤쪽 서버들은 복호화 부담에서 해방되고, 인증서 관리도 한 곳으로 모인다.** 서버가 50대일 때 인증서를 50번 갱신할 것인가 한 번 갱신할 것인가의 차입니다.

---

## 4. 헬스체크

로드 밸런서가 죽은 서버로 트래픽을 보내지 않으려면 상태를 주기적으로 확인해야 합니다. 그런데 헬스체크는 잘못 설계하면 **없느니만 못하다.**
...
**왜 문제인가**: 추천 서비스 하나가 느려지면 **모든 인스턴스의 헬스체크가 동시에 실패한다.** 로드 밸런서는 전 서버를 비정상으로 판단해 트래픽 분배 대상에서 전부 빼버리고, 서비스는 완전히 죽습니다. 실제로는 추천 없이도 주문은 처리할 수 있었는데 말입니다. 이것이 연쇄 장애(cascading failure)의 전형적인 형태입니다.
...
헬스체크의 질문은 **"이 서버가 완벽한가"가 아니라 "이 서버로 트래픽을 보내는 것이 안 보내는 것보다 나은가"**다. 필수 의존성(자기 DB)까지는 확인해도 되지만, 없어도 서비스가 되는 부가 의존성은 넣지 않습니다.
...
로드 밸런서 자체가 단일 장애점이 되면 아무 의미가 없습니다. 해법은 세 가지입니다.
...
둘 다 "큰 데이터를 나눈다"는 점은 같지만 **나눈 조각이 어디에 있느냐**가 다릅니다. 파티셔닝은 한 DB 서버 안에서 테이블을 조각내는 것이고, 샤딩은 조각을 물리적으로 다른 서버에 두는 것입니다. 그래서 파티셔닝은 조인과 트랜잭션이 그대로 되지만, 샤딩은 둘 다 어려워집니다.

샤딩으로 얻는 것은 **저장 용량, 쓰기 처리량, 인덱스 크기**의 한계를 넘는 것입니다. 읽기만 부족하면 레플리카로 충분합니다. **샤딩은 쓰기가 한 대로 감당이 안 될 때 마지막에 꺼내는 카드입니다.

### 샤딩 키가 왜 가장 중요한가

샤딩 키는 "이 데이터가 어느 샤드로 갈지"를 결정하는 컬럼입니다. 그리고 이것은 **한번 정하면 사실상 바꿀 수 없다.** 바꾸려면 전체 데이터를 다시 배치해야 하는데, 그 규모가 되면 무중단으로 하기가 대단히 어렵습니다.
...
**왜 문제인가**: 새로 들어오는 데이터는 언제나 "지금"입니다. 즉 **모든 쓰기가 마지막 샤드 하나로 몰린다.** 샤드를 열 개로 나눴는데 쓰기 처리량은 한 대짜리와 같습니다. 샤딩을 한 의미가 사라집니다. 자동 증가 기본 키를 범위로 나눠도 정확히 같은 문제가 생깁니다. 이 현상을 **핫 샤드(hot shard)**라고 부릅니다.

개선 방향은 둘입니다.

- **접근 주체를 키로 삼는다.** 주문을 `user_id`로 나누면 사용자는 전 시간대에 걸쳐 고르게 분포하므로 쓰기도 고르게 퍼집니다. 대부분의 조회가 "내 주문"이므로 쿼리 패턴과도 맞습니다.
- **시간 기반이 꼭 필요하면 접두어를 섞는다.** 로그처럼 본질적으로 시계열인 데이터는 `(hash(source_id) % 16, 날짜)` 같은 복합 키로 같은 시간대의 쓰기를 16갈래로 흩습니다.

### 세 가지 샤딩 전략

| 전략 | 매핑 방식 | 장점 | 단점 |
|------|----------|------|------|
| 범위(Range) | `id 1~100만 → 샤드1` | 범위 쿼리가 한 샤드에서 끝납니다. 샤드 추가가 쉽다 | 분포가 기울면 핫 샤드가 생긴다 |
| 해시(Hash) | `hash(key) % N → 샤드` | 분포가 균등하다 | 범위 쿼리가 전 샤드로 흩어집니다. **N이 바뀌면 거의 전부 이동한다** |
...
핵심 아이디어는 **키와 노드를 같은 해시 공간(원형 링) 위에 올려놓고, 키는 자기보다 큰 쪽으로 가다 처음 만나는 노드에 배정하는 것**입니다.
...
노드 B를 제거하면 어떻게 될까? **B가 담당하던 키만 다음 노드로 옮겨가고, 나머지 키는 전혀 움직이지 않는다.** 노드를 추가할 때도 마찬가지로, 새 노드가 끼어드는 구간의 키만 이동합니다.
...
노드가 링 위에 무작위로 배치되면 구간 크기가 고르지 않습니다. 노드 3개면 어떤 노드가 링의 절반을 담당하게 될 수도 있습니다. 또 노드 하나가 빠질 때 그 부하가 **바로 다음 노드 한 대에만 몰린다**는 문제도 있습니다.

해결책은 **노드 하나를 링 위 여러 지점에 올려두는 것**입니다. 노드 A를 `A#1`, `A#2`, ... `A#150`처럼 150개의 가상 노드로 링에 뿌리면,

- 구간이 잘게 쪼개져 통계적으로 균등해지고,
- 노드 A가 빠질 때 그 부하가 **나머지 모든 노드에 골고루 분산**됩니다.

Cassandra는 가상 노드(vnode)로 이 방식을 그대로 쓰고, Redis Cluster는 해시 슬롯 16384개를 노드에 나눠주는 방식으로 같은 목적(노드가 바뀔 때 옮겨야 할 데이터를 최소화)을 달성합니다. **캐시 클러스터에서 노드 한 대가 죽었을 때 캐시 전체가 날아가지 않는 이유가 바로 이것이다.**

---

## 7. 레플리카와 읽기 분산

샤딩이 쓰기를 나누는 것이라면, 복제는 읽기를 나누는 것입니다. 대부분의 서비스는 읽기가 쓰기보다 압도적으로 많으므로 **샤딩보다 먼저 검토해야 할 카드입니다.
...
Spring에서는 `@Transactional(readOnly = true)`가 붙은 트랜잭션을 레플리카로 라우팅하도록 데이터소스를 구성하는 방식이 일반적입니다. 코드 변경 없이 읽기 부하를 분산할 수 있습니다.

대가는 **복제 지연(replication lag)**입니다. 복제가 비동기이므로 프라이머리에 쓴 내용이 레플리카에 도착하기까지 시간이 걸립니다. 평소에는 밀리초 단위지만, 프라이머리에 쓰기가 몰리거나 레플리카 사양이 부족하면 수 초에서 수십 초까지 벌어집니다. 이때 "방금 쓴 글이 목록에 없다"는 문제가 나타납니다. 이 현상과 대응(읽은 내 쓰기 보장)은 [01-cap-consistency.md](./01-cap-consistency.md)의 4절에서 코드와 함께 다룹니다.

운영 측면에서 반드시 갖춰야 할 것은 **복제 지연 모니터링과 자동 제외입니다. 지연이 임계치를 넘은 레플리카는 읽기 대상에서 자동으로 빼야 합니다. 그러지 않으면 뒤처진 레플리카 한 대가 계속 낡은 데이터를 응답하면서, 사용자 일부에게만 재현되는 버그를 만듭니다.

또 하나 알아둘 것은 **페일오버 시 데이터 유실**입니다. 비동기 복제에서 프라이머리가 갑자기 죽으면, 아직 어느 레플리카에도 도달하지 못한 마지막 트랜잭션들은 영구히 사라집니다. 이것이 곤란한 도메인이라면 최소 한 대의 확인을 기다리는 반동기 복제를 검토합니다. 완전 동기보다 성능 영향이 작으면서 유실 위험은 크게 줄어듭니다.

### 다중 마스터는 왜 기본값이 아닌가

쓰기를 여러 노드가 동시에 받는 다중 마스터(multi-master, 또는 multi-leader) 구성도 있습니다. 쓰기까지 분산되고 한쪽이 죽어도 반대편이 쓰기를 계속 받는다는 점이 매력적입니다.
...
대가가 **쓰기 충돌**입니다. 같은 행을 양쪽에서 동시에 고쳤을 때 무엇이 맞는지 DB가 대신 정해줄 수 없습니다. "타임스탬프가 늦은 쪽이 이긴다" 같은 규칙을 택하면 다른 한쪽의 쓰기가 조용히 사라지고, 사라지면 곤란한 도메인이라면 병합 로직을 직접 써야 합니다. 게다가 두 노드의 시계가 어긋나 있으면 그 규칙 자체가 어긋납니다.

그래서 다중 마스터는 **지리적으로 떨어진 리전에서 각각 쓰기를 받아야 하거나, 오프라인 편집을 나중에 합쳐야 하는 경우**처럼 충돌을 감수할 이유가 분명할 때만 고릅니다. 단순히 쓰기 처리량이 부족한 것이라면 답은 다중 마스터가 아니라 샤딩입니다. **샤딩은 애초에 같은 데이터를 두 곳에서 쓰지 못하게 나누는 방식이라 충돌 자체가 발생하지 않는다.**

---

## 8. 실무에서는

- **순서가 있다.** 캐시 → 읽기 레플리카 → 샤딩 순으로 검토합니다. 샤딩은 조인, 트랜잭션, 유일성 제약, 통계 쿼리를 전부 다시 설계해야 하는 큰 공사입니다. 앞의 두 카드로 해결되면 그걸로 끝내는 것이 옳습니다.
- **샤딩을 도입하기로 했다면 키 설계에 가장 많은 시간을 쓴다.** 나머지는 나중에 고칠 수 있지만 키는 사실상 못 고칩니다. "이 서비스의 조회 요청 중 몇 퍼센트가 이 키를 조건에 포함하는가"를 실제 쿼리 로그로 세어보는 것이 가장 확실합니다.
- **샤드 간 유일성은 DB가 보장해주지 않는다.** 이메일 중복 방지 같은 제약은 샤드마다 따로 걸리므로 전역 유일성이 깨집니다. 별도의 유일성 관리 테이블을 두거나, 그 컬럼을 샤딩 키로 삼는 별도 인덱스 샤드를 운영합니다.
- **분산 ID 생성이 필요해진다.** 자동 증가 정수는 샤드마다 중복되므로 못 씁니다. 무작위 UUID(v4)는 값이 뒤섞여 들어가 인덱스 지역성이 나쁘므로, 시간 순서를 유지하면서 노드 정보를 담는 스노플레이크(Snowflake) 방식의 ID 생성기를 쓰는 경우가 많습니다. 앞부분에 타임스탬프를 두는 UUIDv7처럼 정렬 가능한 변형을 쓰는 방법도 있습니다.
