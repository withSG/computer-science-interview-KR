...
> 왜 측정 없는 최적화가 거의 항상 실패하는지, 병목을 계층별로 좁혀 들어가는 절차가 무엇인지, 그리고 평균이 아니라 p99를 봐야 하는 이유를 설명할 수 있게 됩니다.
...
**첫째, 병목이 아닌 곳을 고친다.** 전체 응답 시간이 1000ms인데 그중 900ms가 DB 쿼리라면, 애플리케이션 로직을 아무리 다듬어도 개선 폭은 최대 100ms 안쪽입니다. 반나절 걸려 자바 코드를 최적화해 50ms를 줄였다면 전체 응답은 5% 빨라집니다. 같은 시간에 인덱스 하나를 추가했다면 80%가 빨라졌을 수 있습니다.

**둘째, 개선됐는지 알 수 없다.** 기준선(baseline)이 없으면 "빨라진 것 같다"는 인상만 남습니다. 사실은 그날 트래픽이 적었을 뿐인데 최적화 덕분이라고 믿게 되고, 그 믿음이 다음 판단을 계속 오염시킵니다.

그래서 최적화의 첫 단계는 코드를 여는 것이 아니라 **숫자를 만드는 것**입니다. "주문 API p99가 800ms인데 200ms로 줄인다" 같은 문장이 없으면 시작한 것이 아닙니다.

### 비유

집이 추워서 난방비가 많이 나온다고 하자. 감으로 창문을 이중창으로 바꾸고, 커튼을 두꺼운 걸로 바꾸고, 문풍지를 붙입니다. 그런데 실제로는 지붕 단열재가 빠져 있어서 열의 70%가 위로 나가고 있었다면, 앞의 세 가지 공사를 다 해도 난방비는 크게 안 줄어듭니다. 열화상 카메라로 한 번 찍었다면 5분 만에 알 수 있었습니다.

> **비유의 한계**: 열화상 카메라는 원인을 즉시 보여주지만, 성능 프로파일링은 "여기가 오래 걸린다"까지만 알려주고 왜 오래 걸리는지는 여전히 사람이 판단해야 합니다. 도구는 범위를 좁혀줄 뿐 답을 주지 않습니다.

---

## 2. 지연 시간 숫자 감각

최적화 판단의 상당수는 "이 작업이 저 작업보다 몇 배 비싼가"만 알아도 내려집니다. 정확한 값을 외울 필요는 없고 **자릿수**만 알면 됩니다.
...
여기서 실무 규칙 몇 개가 바로 도출됩니다.

- **네트워크 왕복 횟수를 줄이는 것이 왕복 시간을 줄이는 것보다 효과적이다.** N+1 문제가 치명적인 이유가 이것입니다. 쿼리 하나하나는 빨라도 100번 왕복하면 이야기가 달라집니다.
- **디스크에 안 가는 것이 가장 빠르다.** 캐시가 존재하는 이유이자, 인덱스가 풀스캔보다 극적으로 빠른 이유입니다.
- **사용자와 서버의 물리적 거리는 코드로 못 줄인다.** CDN을 쓰는 이유가 이것입니다. 대륙 간 왕복 150ms는 어떤 최적화로도 사라지지 않습니다.
...
같이 쓰이지만 재는 대상이 다르고, 심지어 서로 반대로 움직이기도 합니다.
...
그래서 "성능을 개선했다"는 말은 어느 쪽을 개선했다는 뜻인지 밝히지 않으면 의미가 없습니다. 배치나 병렬 처리로 처리량을 올리면 개별 요청은 대기가 늘어 지연이 나빠지는 일이 흔합니다. **목표를 정할 때 둘 중 무엇이 우선인지 먼저 정한다.**
...
SLI 없이 SLO를 말할 수 없고, SLO 없이 SLA를 말할 수 없습니다. 그리고 **SLO는 100%로 잡지 않는다.** 100%를 목표로 두면 모든 배포가 위험 행위가 되어 아무것도 바꿀 수 없게 됩니다. 99.9%처럼 여유를 남겨두면 그 여유분(에러 예산)만큼은 새 기능을 내보내는 데 쓸 수 있다는 판단이 가능해집니다.
...
평균 1080ms는 **어떤 사용자의 경험도 대표하지 않는다.** 실제로 1080ms 근처에 있던 요청은 하나도 없습니다. 평균은 이상치를 희석해서 "아무도 겪지 않은 중간값"을 만들어냅니다.

백분위는 그 반대입니다. **p99 = 500ms는 "100명 중 1명은 500ms 이상 기다린다"는, 실제 사람이 겪는 경험을 그대로 말한다.**
...
마이크로서비스로 쪼갤수록 이 효과가 커집니다. **"p99를 SLO로 잡는다"는 말이 나오는 이유는 통계적 취향이 아니라 이 계산 때문이다.**

같은 이유로 **캐시 히트율만 보면 안 된다.** 히트율 99%면 평균은 아름답지만, p99는 정확히 그 미스 1%가 결정합니다. 캐시를 붙인 뒤에도 미스 경로가 감당 가능한지 따로 확인해야 합니다.
...
4단계에서 **한 번에 하나만 고치는 것**이 중요합니다. 세 곳을 동시에 고치면 무엇이 효과가 있었는지 알 수 없고, 그중 하나가 오히려 악화 요인이었어도 다른 개선에 가려 드러나지 않습니다.
...
**순서는 위에서 아래로 내려가는 것이 원칙**입니다. 사용자가 느낀 시간에서 출발해 "이 중 얼마가 어디서 소모됐나"를 나눠가야 합니다. 반대로 DB부터 뒤지기 시작하면, 사실은 이미지 5MB를 그대로 내려보내는 것이 원인이었는데 며칠을 씁니다.

실제 분포에서는 **DB에서 대부분이 풀린다.** 웹 애플리케이션 성능 문제의 압도적 다수는 쿼리 문제이므로, 범위를 좁히기 전이라도 슬로 쿼리 로그를 먼저 한 번 열어보는 것은 비용 대비 효과가 좋습니다.
...
**왜 문제인가**: 주문이 100건이면 쿼리가 101번 나갑니다. 쿼리 하나가 2ms로 빨라도 왕복 100번이면 200ms다. 2절에서 본 대로 **네트워크 왕복 횟수 자체가 비용**이기 때문입니다. 게다가 데이터가 늘어날수록 선형으로 악화되므로, 개발 환경(주문 10건)에서는 멀쩡하다가 운영(주문 5000건)에서 터집니다.
...
다만 인덱스는 공짜가 아닙니다. **쓰기마다 인덱스도 갱신되므로 삽입·수정이 느려지고 저장 공간도 늘어난다.** "일단 다 걸어두자"가 아니라 실제 쿼리 패턴에 맞춰 필요한 것만 겁니다.
...
**왜 문제인가**: DB 커넥션을 쥔 채 200ms 동안 외부 응답을 기다립니다. 풀 크기가 10이라면 **동시 요청 10건만으로 풀이 빈다.** 11번째 요청부터는 커넥션을 기다리다 타임아웃 나고, 사용자에게는 500 오류가 나갑니다. 증상이 "서버는 살아 있는데 DB 관련 기능만 안 되는" 형태로 나타나 원인을 찾기 어렵습니다. 외부 API가 평소보다 조금만 느려져도 즉시 장애가 되는 구조입니다.
...
두 메서드를 **같은 클래스에 두면 안 된다.** `@Transactional`은 프록시 기반이라 같은 클래스 안에서의 내부 호출(self-invocation)에는 적용되지 않습니다. 한 클래스에 그대로 두면 트랜잭션이 아예 시작되지 않아, 고치려던 문제와 다른 문제가 생깁니다.

더 나은 방향은 알림 발송 자체를 메시지 큐로 빼서 응답 경로에서 아예 제거하는 것입니다.

풀 크기에 대해서도 오해가 많습니다. **크게 잡는다고 좋아지지 않는다.** DB가 동시에 의미 있게 처리할 수 있는 쿼리 수는 결국 코어 수에 가깝고, 연결이 그보다 많아지면 컨텍스트 스위칭과 락 경합으로 오히려 처리량이 떨어집니다. 게다가 애플리케이션 인스턴스가 N대면 DB가 받는 총 연결은 `풀 크기 × N`이므로, 인스턴스를 늘릴 때 DB 쪽 한계를 반드시 함께 계산해야 합니다.

### GC 일시 정지

애플리케이션 코드는 빠른데 p99만 튀는 전형적인 원인입니다. GC 로그에서 Stop-the-World 구간의 길이와 빈도를 확인합니다. 힙이 부족해 GC가 잦은 것인지, 메모리 누수로 회수가 안 되는 것인지, 힙이 지나치게 커서 한 번의 정지가 긴 것인지에 따라 대응이 다릅니다. 힙 덤프로 어떤 객체가 쌓여 있는지 보는 것이 출발점입니다.
...
- **로컬에서 측정하지 않는다.** 데이터 100건짜리 로컬 DB와 1000만 건짜리 운영 DB는 실행 계획 자체가 다릅니다. 로컬에서 빠른 쿼리가 운영에서 풀스캔을 타는 일은 흔합니다.
- **워밍업 구간을 제외한다.** JVM은 처음엔 인터프리터로 실행하다가 JIT 컴파일 후에 빨라지고, 커넥션 풀과 캐시도 채워지는 시간이 필요합니다. 시작 직후 수치를 포함하면 결과가 왜곡됩니다.
- **캐시가 다 채워진 상태만 재지 않는다.** 같은 요청을 반복하면 캐시 히트율 100%가 되어 실제와 다른 결과가 나옵니다. 실제 트래픽과 비슷한 키 분포로 요청을 만들어야 합니다.
- **평균만 기록하지 않는다.** 3절의 이유 그대로입니다. 리포트에는 p50/p95/p99와 에러율이 함께 있어야 합니다.

---

## 7. 실무에서는

- **관측 가능성이 먼저다.** APM이나 분산 트레이싱이 없는 상태에서 병목을 찾는 것은 눈을 감고 하는 일입니다. 도구를 붙이는 데 드는 시간이 병목을 감으로 찾는 시간보다 항상 짧습니다.
- **개선은 코드 밖에서 오는 경우가 많다.** 인덱스 추가, 커넥션 풀 조정, 캐시 도입, 비동기화 같은 것들이 코드 리팩터링보다 훨씬 큰 폭을 만듭니다. 알고리즘 개선이 필요한 상황은 생각보다 드뭅니다.
- **개선한 지표는 알람에 건다.** 한 번 고친 성능은 새 기능이 들어오면서 조용히 되돌아갑니다. 회귀를 사람이 발견하게 두면 결국 장애로 발견됩니다.
- **성능 목표는 비즈니스 언어로 정의한다.** "쿼리를 100ms 이하로"가 아니라 "결제 완료 화면이 1초 안에 뜬다"가 되어야 어디까지 최적화할지 판단할 수 있습니다. 목표가 없으면 최적화는 끝나지 않습니다.
...
- 꼬리 질문: "경험상 어디가 가장 많이 걸리나요?" → 웹 애플리케이션은 대부분 DB다. 인덱스 누락과 N+1이 압도적으로 많고, 그래서 범위를 좁히기 전이라도 슬로 쿼리 로그를 먼저 열어보는 것이 효율적입니다.
...
| 평균 응답 시간으로 성능을 평가한다 | 평균은 이상치를 희석해 아무도 겪지 않은 값을 만든다 | p50/p95/p99를 함께 봅니다. SLO는 백분위로 잡는다 |
...
최적화는 코드 실력이 아니라 **절차**의 문제여서, 정량 목표를 세우고 계층을 좁혀 가장 큰 병목 하나만 고친 뒤 같은 조건으로 다시 재는 순환을 지키는 것이 어떤 기교보다 효과가 큽니다.
