...
**왜 문제인가**: DB 커넥션을 쥔 채 200ms 동안 외부 응답을 기다립니다. 풀 크기가 10이라면 **동시 요청 10건만으로 풀이 빕니다.** 11번째 요청부터는 커넥션을 기다려야 하고, 대기가 길어지면 타임아웃이 나서 사용자에게는 500 오류가 나갑니다. 증상이 "서버는 살아 있는데 DB 관련 기능만 안 되는" 형태로 나타나 원인을 찾기 어렵습니다. 외부 API가 평소보다 조금만 느려져도 즉시 장애가 되는 구조입니다.
