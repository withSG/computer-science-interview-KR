...
### 핵심 답변
병목은 **측정 → 분석 → 최적화 → 검증** 사이클을 돌면서 찾습니다.
...
### 핵심 답변
커넥션 풀은 **미리 생성해둔 DB 연결을 재사용**하는 기법입니다. 그만큼 연결 생성 오버헤드가 줄어듭니다.
...
### 핵심 답변
비동기 처리는 **요청 완료를 기다리지 않고 다음 작업을 진행**하는 방식입니다. 그래서 I/O 바운드 작업에 효과적입니다.
...
### 핵심 답변
배치 처리는 **여러 작업을 모아서 한 번에 처리**하는 기법입니다. 그만큼 오버헤드가 줄어듭니다.
...
### 핵심 답변
Core Web Vitals는 Google이 정의한 사용자 경험 핵심 지표입니다. LCP(로딩), CLS(안정성), INP(반응성)을 중심으로 측정합니다.
...
### 핵심 답변
CDN은 전 세계 엣지 서버에 콘텐츠를 분산 저장합니다. 사용자와 가장 가까운 서버에서 응답하므로 지연 시간이 줄어듭니다.
...
### 핵심 답변
HTTP 캐싱은 서버 요청을 줄여 줍니다. Cache-Control 헤더로 캐시 정책을, ETag로 조건부 요청을 구현합니다.
...
### 핵심 답변
폭포수 방법론은 순차적·문서 중심으로 진행됩니다. 애자일은 반복적 스프린트와 빠른 피드백이 중심입니다.
