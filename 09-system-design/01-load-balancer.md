...
서비스를 처음 만들면 서버는 한 대입니다. 트래픽이 늘면 "더 좋은 서버로 바꾸자"(수직 확장, Scale-Up)를 가장 먼저 떠올리는데 이 길은 여러 군데에서 막힙니다.
...
로드 밸런서가 이 질문에 답하는 장치입니다. 사용자에게는 주소 하나만 노출하고, 그 뒤에서 살아 있는 서버들에게 요청을 나눠줍니다.
...
> **비유의 한계**: 안내 데스크는 손님을 보내고 나면 관여하지 않지만, 로드 밸런서는 대개 요청과 응답이 **모두 자기를 통과**합니다. 그래서 로드 밸런서 자신도 대역폭과 커넥션 수에 한계가 있고 여기가 새로운 병목이 될 수 있습니다.

---

## 2. 어느 계층을 보느냐가 모든 차이를 만든다

L4와 L7의 차이를 "빠르다 vs 유연하다"로 외우면 실무에서 아무 판단도 못 합니다. 둘은 **패킷의 어디까지 읽느냐**에서 갈리고 기능 차이는 전부 여기서 따라 나옵니다.
...
들어온 순서대로 1번, 2번, 3번, 다시 1번. 가장 단순하고 기본값인 경우가 많습니다. **서버 사양이 같고 요청당 처리 비용이 비슷하다는 전제**가 깔려 있고 이 전제가 깨지면 바로 무너집니다.
...
활성 연결 수와 최근 평균 응답 시간을 함께 봅니다. 같은 사양이어도 어떤 서버만 느려지는 상황(디스크 성능 저하, GC 압박)에 반응합니다. 대신 측정 오버헤드가 있고, "가장 빠른 서버"로 요청이 순간적으로 쏠렸다가 그 서버가 느려지는 진동이 생길 수 있습니다. 그래서 실제 구현은 후보 두 개를 무작위로 뽑아 그중 나은 쪽을 고르기도 합니다(power of two choices).
...
**없으면 이 인스턴스가 아무 요청도 처리 못 하는 의존성**(자기 DB)만 헬스체크에 넣고 없어도 일부 기능만 막히는 의존성(추천, 알림, 외부 결제)은 넣지 않습니다.
...
AWS ALB에서는 **등록 취소 지연(deregistration delay)** 설정이 이 대기 시간을 다루고 기본값은 300초입니다. 대상 그룹에서 인스턴스를 빼도 진행 중인 요청이 끝날 때까지 최대 이 시간만큼 기다립니다. Kubernetes에서는 `preStop` 훅과 `terminationGracePeriodSeconds`가 같은 역할을 합니다. 구체적인 설정은 [02-zero-downtime-deployment.md](./02-zero-downtime-deployment.md)에서 다룹니다.
...
VIP를 누가 가질지는 VRRP(Virtual Router Redundancy Protocol)가 합의하고 리눅스에서 이를 구현한 소프트웨어가 Keepalived입니다.
...
**특별한 이유가 없으면 매니지드 로드 밸런서를 씁니다.** ALB/NLB는 여러 가용 영역에 노드를 자동 배치하므로 위 작업들을 설정 한 번으로 대체합니다. VRRP/Keepalived 구성은 온프레미스거나 클라우드를 쓸 수 없는 사정이 있을 때의 선택지입니다. 특히 단순 DNS 라운드 로빈에는 헬스체크가 없어 죽은 IP도 TTL이 만료될 때까지 계속 반환하므로, 분산에는 쓸 수 있어도 장애 대응 수단은 되지 못한다는 점을 기억해야 합니다. 장애 대응까지 하려면 헬스체크와 연동되는 GSLB나 매니지드 DNS의 상태 확인 기능이 필요합니다.
...
A. 해당 서버가 죽으면 그 서버에 붙은 사용자의 세션이 함께 사라지고, 헤비 유저가 특정 서버에 고정되어 부하가 기울고, 서버를 내리려면 세션 만료까지 기다려야 해서 배포와 오토스케일링이 어려워집니다. 근본 해법은 세션을 Redis 같은 외부 저장소로 빼거나 토큰 기반 인증으로 전환해 애플리케이션 서버를 무상태로 만드는 것입니다. 세션 고정은 그 작업을 아직 못 한 시스템의 임시방편으로 봐야 합니다.
...
로드 밸런서는 "여러 서버를 하나의 주소로 보이게 하는 장치"이고 L4와 L7의 차이는 패킷을 어디까지 읽느냐에서 전부 파생됩니다. 실무에서 사고가 나는 지점은 알고리즘 선택보다 **헬스체크 범위와 서버를 뺄 때의 순서** 쪽입니다.
