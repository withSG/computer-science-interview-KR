...
**교환 횟수가 정확히 n-1번**이라는 점이 특징입니다. 비교는 항상 n(n-1)/2번 하지만 실제 데이터 이동이 극히 적어서, 원소 하나가 매우 크고(구조체 통째 복사) 비교는 싼 상황에서 이론적 이점이 있습니다. 입력이 이미 정렬돼 있어도 O(n²)이라 적응성이 없고, 멀리 있는 원소와 교환하므로 **불안정 정렬**입니다.
