...
> **비유의 한계** — 자동차는 가속이 좋으면 대체로 최고 속도도 높지만, 알고리즘은 증가율이 나빠도 **작은 입력에서는 더 빠를 수 있습니다.** n이 10인데 O(n²) 삽입 정렬이 O(n log n) 병합 정렬보다 빠른 일은 흔합니다.

여기서 두 가지 관례가 나옵니다.

**상수 계수를 버립니다.** 3n이든 100n이든, n이 충분히 커지면 n² 앞에서는 둘 다 똑같이 무의미합니다. 상수는 하드웨어와 구현 디테일에 따라 바뀌는 값이라 알고리즘의 성질이 아닙니다.

**낮은 차수 항을 버립니다.** n² + 5n + 300에서 n = 100만이면 n²이 10¹², 5n이 500만입니다. 5n은 n²의 0.0005%도 안 됩니다. 지배항 하나만 남깁니다.
...
`n ≥ n₀에서만` 이라는 단서가 핵심입니다. 점근 표기법은 **작은 입력에 대해서는 아무 말도 하지 않습니다.** "O(n log n)이 O(n²)보다 항상 빠르다"는 말이 틀린 이유가 여기 있습니다.
...
수학적으로 가장 정확한 서술은 Θ입니다. 그런데 현업과 면접에서는 압도적으로 O를 씁니다. 이유는 두 가지입니다. 첫째, 우리가 궁금한 건 대개 "최악에 얼마나 느려질 수 있나"입니다. 둘째, Θ를 증명하려면 상한과 하한을 모두 보여야 해서 번거롭습니다. 그래서 실무에서 "이 알고리즘은 O(n log n)이다"라고 말할 때는 대부분 Θ(n log n)을 의미합니다.
...
표의 결론: **문제 크기를 나누면(n/2) 로그가 붙고, 하나씩 줄이면(n-1) 선형이 되며, 호출이 2갈래로 늘면서 크기가 안 줄면 지수가 됩니다.**
...
표의 결론: **n이 10⁵를 넘으면 이중 반복문은 포기하고 정렬·해시·이진 탐색으로 O(n log n) 이하를 노려야 합니다.**
