...
### 핵심 답변
이진 탐색은 **정렬된 배열**에서 중간값과 비교하여 탐색 범위를 절반씩 줄여나갑니다. 시간 복잡도는 O(log n)입니다.
...
### 핵심 답변
동적 프로그래밍은 큰 문제를 작은 부분 문제로 나누는 기법입니다. 결과를 저장(메모이제이션)해 두고 다시 꺼내 쓰기 때문에 같은 계산을 되풀이하지 않습니다.
...
### 핵심 답변
**커스텀 비교 함수**로 정렬합니다. 두 숫자 a, b는 "ab" vs "ba"로 맞대어 보고, 더 큰 조합이 앞에 오도록 합니다.
...
### 핵심 답변
**Floyd의 순환 탐지 알고리즘** (토끼와 거북이)을 쓰면 O(n) 시간, O(1) 공간으로 찾아냅니다.
...
### 핵심 답변
유전 알고리즘은 **생물의 진화 과정**을 모방한 최적화 알고리즘입니다. 자연 선택, 교차, 돌연변이를 거듭하며 해를 조금씩 개선합니다.
...
### 핵심 답변
피보나치 수열은 **재귀, 메모이제이션, 타뷸레이션, 행렬 거듭제곱** 등의 방법으로 구현합니다. 각각 시간 복잡도가 다릅니다.
...
### 핵심 답변
객체나 Map을 사용해 값의 빈도를 셉니다. 그러면 O(n²) 중첩 반복이 O(n)으로 줄어듭니다.
...
### 핵심 답변
정렬된 배열을 전제로 합니다. 두 포인터를 양쪽 끝(또는 같은 위치)에서 이동시켜 O(n²)을 O(n)으로 줄이는 패턴입니다.
...
### 핵심 답변
연속된 부분 배열이나 문자열 문제에서 윈도우를 이동시켜 중복 계산을 제거합니다.
...
### 핵심 답변
그리디 알고리즘입니다. 가중치가 있는 그래프에서 단일 출발점으로부터 모든 정점까지 최단 경로를 구합니다.
