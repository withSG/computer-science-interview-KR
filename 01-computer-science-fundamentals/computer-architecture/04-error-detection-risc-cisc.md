...
> 하드웨어가 조용히 틀렸을 때 그걸 어떻게 알아채고 고치는지, 그리고 "명령어를 몇 개 만들 것인가"라는 설계 선택이 어떻게 오늘날의 ARM/x86 구도를 만들었는지 설명할 수 있게 됩니다.
...
이 문서는 성격이 다른 두 주제를 묶고 있는데, 공통점이 하나 있습니다. 둘 다 **"하드웨어는 이상적이지 않다"**는 사실에서 출발합니다.

앞의 문서들은 CPU와 메모리가 시킨 대로 정확히 동작한다고 가정했습니다. 현실은 다릅니다. DRAM 셀에 저장된 전하는 우주에서 날아온 고에너지 입자나 전기적 잡음 때문에 아주 드물게 뒤집힌다(**soft error**, bit flip). 확률은 낮지만, 수천 대 서버가 수 TB 메모리를 24시간 돌리는 데이터센터에서는 "드물게"가 "매일"이 됩니다. 그래서 오류를 **검출**하고 나아가 **정정**하는 기술이 필요합니다.
...
데이터에 비트 1개를 덧붙여 **전체 1의 개수를 짝수(또는 홀수)로 맞춘다.** 그것뿐입니다.
...
**비유**: 짐을 부칠 때 상자 개수를 짝수로 맞춰 보내고, 받는 쪽에서 홀수면 하나 잃어버린 걸 아는 것과 같습니다.

**비유의 한계**: 상자는 두 개를 잃어버려도 개수를 세면 알지만, 패리티는 **두 비트가 동시에 뒤집히면 알아채지 못합니다. 1의 개수가 다시 짝수로 돌아오기 때문입니다.
...
핵심은 마지막 줄입니다. 위치를 모르면 **고칠 수 없다.** 검출만 되는 시스템은 결국 "다시 보내 주세요"라고 요청하는 수밖에 없는데, 메모리에서는 재전송이라는 개념이 없습니다. 이미 값이 오염됐고 원본은 사라졌습니다.
...
이 감시 범위가 겹치는 방식이 절묘합니다. 각 데이터 위치는 자기 위치 번호를 이진수로 쪼갠 조합의 패리티들에게만 감시받습니다. 위치 5(=101)는 P4와 P1이 감시하고 P2는 감시하지 않습니다. 그래서 P4와 P1만 깨지면 답은 5 하나로 확정됩니다. 위치마다 감시자 조합이 전부 다르기 때문에 조합만 보고 위치를 역산할 수 있습니다.
...
이제 전송 중 **5번 비트가 뒤집혔다**고 하자. 수신 측은 어느 비트가 틀렸는지 모르는 상태에서 시작합니다.
...
이게 해밍 코드의 전부입니다. **깨진 패리티들의 위치 번호를 더한 값이 곧 오류 위치입니다. 마법처럼 보이지만 실제로는 위치 번호를 이진수로 인코딩해 둔 것을 되읽는 것뿐입니다.

데이터 `m`비트에 패리티가 몇 개 필요한지는 `2^r ≥ m + r + 1`을 만족하는 최소 `r`입니다. 데이터 4비트면 r=3(2³=8 ≥ 8), 데이터 64비트면 r=7입니다.

### SECDED — 실제 ECC 메모리

기본 해밍 코드는 **1비트 정정**은 되지만, 2비트가 뒤집히면 신드롬이 엉뚱한 위치를 가리켜 **멀쩡한 비트를 망가뜨린다.** 그래서 실제 ECC RAM은 전체를 감싸는 패리티 비트 하나를 더 붙입니다.
...
**언제 뭘 쓰나**: 다시 보낼 수 있으면 검출만 하고 재전송하는 게 싸다(CRC + TCP). **다시 보낼 수 없으면 정정 능력을 사서 넣어야 한다**(메모리, 저장 장치, 우주 탐사선). ECC 메모리가 서버에서 표준이고 소비자용 PC에서 흔치 않은 이유도 이 기준입니다. 게임 중 픽셀 하나가 잘못 그려지는 것과, 은행 잔액 비트가 뒤집히는 것은 대가가 다릅니다.
...
1970년대의 제약을 알아야 CISC가 이해됩니다. 당시 메모리는 지금 감각으로 상상하기 어려울 만큼 비싸고 작았으며, 컴파일러도 원시적이었습니다. 사람이 어셈블리를 직접 쓰는 일이 흔했습니다.
...
이것이 **CISC(Complex Instruction Set Computer)**다. x86이 대표주자입니다.

### 그런데 문제가 드러났다

1980년대에 들어 상황이 바뀌었습니다. 메모리는 싸졌고 컴파일러는 똑똑해졌습니다. 그리고 실측 결과 충격적인 사실이 밝혀졌습니다. **컴파일러가 만드는 코드는 복잡한 명령어를 거의 쓰지 않았다.** 대부분의 실행 시간을 소수의 단순한 명령어가 차지했습니다.

더 심각한 건 파이프라인이었습니다. 명령어 길이가 제각각이고(x86-64는 1~15바이트) 실행 시간도 제각각이면, 명령어 경계를 찾는 것부터 어렵고 파이프라인 단계를 균일하게 나눌 수도 없습니다.
...
- **Load/Store 아키텍처** → 메모리 접근은 `LOAD`/`STORE`만 하고, 연산은 레지스터끼리만 합니다. 덕분에 명령어당 메모리 접근 횟수가 예측 가능해진다
...
**한 줄 결론**: 설계 철학의 차이는 "복잡함을 하드웨어에 둘 것인가, 컴파일러에 둘 것인가"다. 하드웨어에 두면 코드가 짧아지고, 컴파일러에 두면 하드웨어가 빨라집니다.

### 경계는 이미 흐려졌다 — 반드시 알아둘 것

면접에서 위 표만 외워 답하면 낡은 지식이라는 인상을 줍니다. 현대 CPU에서 RISC와 CISC의 구분은 **명령어 세트(ISA)라는 겉면에만 남아 있고 내부는 수렴했다.**
...
전환의 진짜 난관은 성능이 아니라 **기존 x86 바이너리 호환성**이었습니다. 해법은 세 가지를 함께 썼습니다.
...
여기서 한 가지 흥미로운 지점이 있습니다. x86은 **강한 메모리 순서(strong memory ordering)**를 보장하지만 ARM은 **약한 메모리 순서**를 택합니다. 멀티스레드 코드를 그대로 번역하면 x86에서 잘 돌던 코드가 ARM에서 미묘하게 깨질 수 있습니다. 애플은 이를 위해 칩에 x86 스타일 메모리 순서를 흉내 내는 모드를 넣고 Rosetta 프로세스에만 켜는 방식으로 해결했습니다. **아키텍처 차이는 명령어만의 문제가 아니라 동시성 의미론의 문제이기도 하다.**
...
**JVM 언어는 상대적으로 안전하다.** 바이트코드가 아키텍처를 추상화하므로 순수 Java/Kotlin 코드는 재빌드가 필요 없습니다. 문제는 **네이티브 코드에 의존하는 라이브러리입니다. JNI를 쓰는 압축·암호화 라이브러리, 네이티브 이미지 처리 도구 등은 아키텍처별 바이너리가 아티팩트에 포함되어 있어야 합니다. 없으면 런타임에 `UnsatisfiedLinkError`가 납니다.

**성능 특성이 다르다.** 벤치마크는 배포할 아키텍처에서 돌려야 의미가 있습니다. arm64 개발 머신에서 측정한 수치를 amd64 서버 용량 산정에 그대로 쓰면 틀립니다.
...
오류 검출은 "틀렸다"까지만 알려주고 정정은 "어디가 틀렸는지"까지 알려주는 차이이며, RISC와 CISC는 복잡함을 하드웨어에 둘지 컴파일러에 둘지에 대한 서로 다른 답이었지만 현대 CPU에서는 겉면만 남고 내부는 수렴했습니다.
