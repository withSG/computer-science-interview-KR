...
핵심은 마지막 줄입니다. 위치를 모르면 **고칠 수 없습니다.** 검출만 되는 시스템은 결국 "다시 보내 주세요"라고 요청하는 수밖에 없는데, 메모리에서는 재전송이라는 개념이 없습니다. 이미 값이 오염됐고 원본은 사라졌습니다.
...
기본 해밍 코드는 **1비트 정정**은 되지만, 2비트가 뒤집히면 신드롬이 엉뚱한 위치를 가리켜 **멀쩡한 비트를 망가뜨립니다.** 그래서 실제 ECC RAM은 전체를 감싸는 패리티 비트 하나를 더 붙입니다.
...
1980년대에 들어 상황이 바뀌었습니다. 메모리는 싸졌고 컴파일러는 똑똑해졌습니다. 그리고 실측 결과 충격적인 사실이 밝혀졌습니다. **컴파일러가 만드는 코드는 복잡한 명령어를 거의 쓰지 않았습니다.** 대부분의 실행 시간을 소수의 단순한 명령어가 차지했습니다.

더 심각한 건 파이프라인이었습니다. 명령어 길이가 제각각이고(x86-64는 1~15바이트) 실행 시간도 제각각이면, 명령어 경계를 찾는 것부터 어렵고 파이프라인 단계를 균일하게 나눌 수도 없습니다.

여기서 나온 반격이 **RISC(Reduced Instruction Set Computer)**입니다.
...
여기서 한 가지 흥미로운 지점이 있습니다. x86은 **강한 메모리 순서(strong memory ordering)**를 보장하지만 ARM은 **약한 메모리 순서**를 택합니다. 멀티스레드 코드를 그대로 번역하면 x86에서 잘 돌던 코드가 ARM에서 미묘하게 깨질 수 있습니다. 애플은 이를 위해 칩에 x86 스타일 메모리 순서를 흉내 내는 모드를 넣고 Rosetta 프로세스에만 켜는 방식으로 해결했습니다. **아키텍처 차이는 명령어만의 문제가 아니라 동시성 의미론의 문제이기도 합니다.**
