...
이 구성을 **SECDED**(Single Error Correction, Double Error Detection)라 부릅니다. 1비트 오류는 조용히 고치고, 2비트 오류는 "고칠 수 없지만 오류가 있다"고 보고합니다. DDR4까지의 서버용 DIMM이 64비트 데이터에 8비트를 더해 72비트 폭으로 만들어지는 이유가 이것입니다.
