...
컴퓨터의 저장 공간에는 0과 1밖에 없습니다. 마이너스 기호도, 소수점도 물리적으로 저장할 방법이 없습니다. 그런데 우리는 `-5`도 쓰고 `3.14`도 씁니다. 이건 전부 **약속(인코딩)**입니다. "이 비트 패턴은 이런 뜻으로 읽자"고 정해 놓은 규칙일 뿐입니다.

문제는 약속이 완벽할 수 없다는 데 있습니다. 비트 수가 유한하므로 표현할 수 있는 값의 개수도 유한합니다. 32비트로는 아무리 잘 짜도 42억 개 남짓의 서로 다른 값만 구분할 수 있습니다. 그런데 실수는 무한히 많습니다. **어떤 약속을 택하든 반드시 표현하지 못하는 값이 생긴다.**

이 문서는 두 가지 약속을 다룹니다. 정수를 다루는 **2의 보수**와, 실수를 다루는 **IEEE 754 부동 소수점**입니다. 그리고 두 약속이 각각 어디서 새는지를 봅니다. 새는 지점을 모르면 언젠가 정산 금액이 1원 틀린 버그를 만나게 됩니다.
...
가장 먼저 떠오르는 건 **부호-절댓값(sign-magnitude)** 방식입니다. 맨 앞 비트를 부호로 쓰고 나머지로 크기를 나타냅니다.
...
**문제 1 — 0이 둘이다.** `0000`(+0)과 `1000`(-0). 같은 값에 두 표현이 있으면 비교 연산이 지저분해집니다.
...
부호를 보고 "이건 사실 뺄셈이야"라고 판단하는 별도 회로가 필요해집니다. ALU가 복잡해지고 느려집니다.
...
**뺄셈 회로가 필요 없다.** `A - B`는 그냥 `A + (-B)`로 처리하면 되고, 덧셈기 하나로 전부 해결됩니다. 이게 2의 보수가 사실상 모든 CPU의 표준이 된 이유입니다.
...
n비트 2의 보수의 범위는 `-2^(n-1) ~ 2^(n-1) - 1`입니다. **음수 쪽이 하나 더 많다.** 8비트는 -128 ~ 127, 32비트는 -2,147,483,648 ~ 2,147,483,647입니다.
...
`Integer.MIN_VALUE`의 절댓값 `2147483648`은 int로 표현 자체가 불가능합니다. `Math.abs`가 할 수 있는 게 없습니다.

---

## 3. 정수 오버플로우

표현 범위를 넘으면 조용히 반대편으로 넘어갑니다. **예외도 없고 경고도 없다.**
...
참고로 언어마다 정책이 다릅니다. Python의 정수는 자동으로 무한 정밀도로 커지므로 오버플로우가 없고(대신 느리다), Java와 C는 조용히 순환합니다. **자기가 쓰는 언어가 어느 쪽인지 아는 게 먼저다.**
...
- **M (Mantissa/Fraction)**: 가수. 정규화하면 맨 앞이 항상 `1.`이므로 그 1은 저장하지 않는다(**hidden bit**). 덕분에 23비트로 24비트만큼의 정밀도를 얻습니다.
...
여기가 핵심입니다. **0.1은 이진법에서 순환 무한 소수다.** 10진법에서 1/3이 0.333...으로 끝나지 않는 것과 같은 이유로, 분모가 2의 거듭제곱이 아닌 분수(0.1 = 1/10)는 이진 소수로 유한하게 적을 수 없습니다.
...
`System.out.println(0.1)`이 `0.1`로 보이는 건 출력 루틴이 "이 double을 다시 읽었을 때 같은 값이 되는 가장 짧은 10진 표기"를 골라 보여주기 때문이지, 저장된 값이 정확히 0.1이라서가 아닙니다.
...
버그가 아니라 **유한 비트로 무한 소수를 담을 때 필연적으로 생기는 결과입니다. 언어를 바꿔도 IEEE 754를 쓰는 한 똑같습니다.
...
- **단점**: 통화마다 소수 자릿수가 다르다(원은 0자리, 달러는 2자리, 일부 통화는 3자리). 다국가 서비스에서는 단위 관리가 금방 복잡해집니다. 나눗셈(할인율, 세금)에서 반올림 정책을 직접 정해야 합니다.

**언제 뭘 쓰나**: 단일 통화 + 단순 가감산 위주면 정수 환산이 실용적입니다. 통화가 여럿이거나 이율·환율 같은 나눗셈이 끼면 `BigDecimal`이 안전합니다.

### 왜 이렇게까지 하나

오차 하나는 10^-17 수준입니다. 문제는 **누적**입니다. 거래 100만 건에서 건당 0.0001원씩 어긋나면 100원이 사라집니다. 회계 시스템에서 대차가 1원이라도 안 맞으면 마감이 안 됩니다. 정산 배치가 실패하고, 원인 추적에 며칠이 듭니다. 애초에 부동 소수점을 안 쓰는 게 가장 쌉니다.

---

## 7. 실무에서는

**JSON으로 큰 정수를 보낼 때.** JavaScript의 `Number`는 double이라 안전하게 표현되는 정수 상한이 `Number.MAX_SAFE_INTEGER = 9007199254740991`(2⁵³ - 1)입니다. 그런데 서버의 `Long` ID나 Snowflake 방식 ID는 이 범위를 쉽게 넘습니다. 그대로 내려보내면 브라우저에서 **끝자리가 조용히 바뀐다.**

```javascript
console.log(9007199254740993);  // 9007199254740992  ← 마지막 자리가 달라진다
```

해결책은 API 응답에서 큰 ID를 **문자열로 직렬화**하는 것입니다. Spring이라면 `@JsonSerialize(using = ToStringSerializer.class)`를 붙이거나 필드 타입을 String으로 노출합니다. "가끔 특정 게시글만 조회가 안 된다"는 제보의 흔한 원인입니다.

**float를 언제 쓰나.** 메모리와 대역폭이 중요한 경우입니다. 대량의 좌표 데이터, 머신러닝 가중치, 3D 그래픽 버텍스는 float(또는 더 작은 타입)로 충분한 경우가 많습니다. 반대로 일반 비즈니스 로직에서 float를 쓸 이유는 거의 없습니다. 정밀도가 double보다 훨씬 낮은데 얻는 게 없습니다.

**DB 컬럼 타입.** MySQL의 `FLOAT`/`DOUBLE`은 IEEE 754 그대로라 금액에 쓰면 안 됩니다. `DECIMAL(p, s)`을 씁니다. 이 타입은 내부적으로 10진 값을 정확히 보관하는 대신 연산이 느립니다. 그래서 통계 집계처럼 오차가 허용되는 곳에는 굳이 DECIMAL을 강요할 필요가 없습니다.
...
정수는 2의 보수로 뺄셈 회로를 없앤 대신 범위를 넘으면 조용히 순환하고, 실수는 IEEE 754로 넓은 범위를 얻은 대신 10진 소수를 정확히 담지 못하므로, 돈처럼 정확성이 필요한 값에는 부동 소수점을 쓰지 않습니다.
