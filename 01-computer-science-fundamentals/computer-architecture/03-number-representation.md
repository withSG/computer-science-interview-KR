...
> `0.1 + 0.2 != 0.3`이 나오거나 정수 오버플로우로 값이 뒤집히는 사고를 한 번쯤 만나게 됩니다. 컴퓨터가 0과 1만으로 음수와 소수를 어떻게 흉내 내는지, 그 흉내의 어디에 구멍이 있는지를 설명합니다.
...
컴퓨터의 저장 공간에는 0과 1밖에 없습니다. 마이너스 기호도 소수점도 물리적으로 저장할 방법이 없는데, 우리는 `-5`도 쓰고 `3.14`도 씁니다. 전부 **약속**(인코딩)입니다. "이 비트 패턴은 이런 뜻으로 읽자"고 정해 놓은 규칙일 뿐입니다.

그 약속은 완벽할 수 없습니다. 비트 수가 유한하니 표현할 수 있는 값의 개수도 유한합니다. 32비트로는 아무리 잘 짜도 42억 개 남짓의 서로 다른 값만 구분할 수 있는데, 실수는 무한히 많습니다. **어떤 약속을 택하든 반드시 표현하지 못하는 값이 생깁니다.**

정수는 **2의 보수**로 다루고, 실수는 **IEEE 754 부동 소수점**으로 다룹니다. 이 문서는 이 두 약속과 각각이 어디서 새는지를 봅니다. 새는 지점을 모르면 언젠가 정산 금액이 1원 틀린 버그를 만나게 됩니다.
...
**문제 1 — 0이 둘입니다.** `0000`(+0)과 `1000`(-0)이 따로 있습니다. 같은 값에 표현이 둘이면 비교 연산이 지저분해집니다.
...
시계 방향으로 한 칸 = +1입니다. `0111`(+7)에서 한 칸 더 가면 `1000`이 되는데, 이 자리는 +8이 아니라 **-8**입니다. 반대로 `1111`(-1)에서 한 칸 더 가면 `0000`(0)으로 자연스럽게 이어집니다. 순환 자체는 매끄럽습니다. 부호가 뒤집히는 지점은 딱 한 군데, +7과 -8 사이뿐입니다.

> **비유**: 12시간제 시계에서 시침을 뒤로 3칸 돌리는 것과 앞으로 9칸 돌리는 것은 결과가 같습니다. 2의 보수도 마찬가지로 "빼기"를 "돌아서 더하기"로 바꾼 것입니다.
>
> **비유의 한계**: 시계는 어디로 넘어가도 유효한 시각입니다. 하지만 정수는 `0111`(최댓값)에서 한 칸 더 가면 `1000`(최솟값)이 되고, **의미가 뒤집힙니다**. 이게 오버플로우입니다.

### 알아둘 비대칭

n비트 2의 보수의 범위는 `-2^(n-1) ~ 2^(n-1) - 1`, 8비트는 -128 ~ 127, 32비트는 -2,147,483,648 ~ 2,147,483,647입니다. 음수 쪽이 하나 더 많고, 이 비대칭이 실제 버그를 만듭니다.
...
`Integer.MIN_VALUE`의 절댓값 `2147483648`은 int로 표현 자체가 불가능합니다. `Math.abs`로서는 할 수 있는 게 없습니다.
...
참고로 언어마다 정책이 다릅니다. Python의 정수는 자동으로 무한 정밀도까지 커지므로 오버플로우가 없습니다(대신 느립니다). Java는 조용히 순환하고, C의 부호 있는 정수 오버플로우는 정의되지 않은 동작(undefined behavior)입니다. 자기가 쓰는 언어가 어느 쪽인지 아는 게 먼저입니다.
...
가장 단순한 방법은 이렇습니다. 비트 자리마다 소수점 위치를 **미리 못 박아 둡니다.**
...
지수 필드 E = -4 + 1023 = 1019
가수 필드 M = 1001100110011001100110011001100110011001100110011001|1001...
                                                                  ↑
                                            52비트에서 잘리고 반올림
```
-->
...
`NaN != NaN`은 정렬 함수나 `Set` 안에 들어가면 예측 불가능한 동작을 일으킵니다. 실수를 키로 쓸 때는 항상 조심해야 합니다.
...
금액을 "달러"가 아니라 "센트" 같은 최소 화폐 단위의 정수로 다룹니다. 1,234.56달러를 `123456`(단위: 센트)으로 저장하는 식입니다.
...
**float를 언제 쓰나.** 메모리와 대역폭이 중요한 경우입니다. 대량의 좌표 데이터, 머신러닝 가중치, 3D 그래픽 버텍스는 float(또는 더 작은 타입)로 충분한 경우가 많습니다. 반대로 일반 비즈니스 로직에서 float를 쓸 이유는 거의 없습니다. 정밀도가 double보다 훨씬 낮으면서 얻는 게 없기 때문입니다.
...
| 정수 오버플로우가 나면 에러가 날 것이다 | Java는 조용히 순환한다(C의 부호 있는 정수 오버플로우는 정의되지 않은 동작) | 필요하면 `Math.addExact` 등으로 명시적 검사 |

---

## 한 줄 정리

정수는 2의 보수 덕에 뺄셈 회로가 없어도 되지만 범위를 넘는 순간 조용히 순환합니다. 실수는 IEEE 754로 넓은 범위를 얻은 대신 10진 소수를 정확히 담지 못합니다. 그래서 돈에는 부동 소수점을 쓰지 않습니다.
