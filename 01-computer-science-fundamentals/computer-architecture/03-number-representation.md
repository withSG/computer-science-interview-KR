...
문제는 약속이 완벽할 수 없다는 데 있습니다. 비트 수가 유한하므로 표현할 수 있는 값의 개수도 유한합니다. 32비트로는 아무리 잘 짜도 42억 개 남짓의 서로 다른 값만 구분할 수 있습니다. 그런데 실수는 무한히 많습니다. **어떤 약속을 택하든 반드시 표현하지 못하는 값이 생깁니다.**
...
**문제 2 — 덧셈 회로가 하나로 안 됩니다.** `+5`와 `-5`를 그냥 더해 보자.
...
n비트 2의 보수의 범위는 `-2^(n-1) ~ 2^(n-1) - 1`입니다. **음수 쪽이 하나 더 많습니다.** 8비트는 -128 ~ 127, 32비트는 -2,147,483,648 ~ 2,147,483,647입니다.
...
참고로 언어마다 정책이 다릅니다. Python의 정수는 자동으로 무한 정밀도로 커지므로 오버플로우가 없고(대신 느리다), Java와 C는 조용히 순환합니다. **자기가 쓰는 언어가 어느 쪽인지 아는 게 먼저입니다.**
...
여기가 핵심입니다. **0.1은 이진법에서 순환 무한 소수입니다.** 10진법에서 1/3이 0.333...으로 끝나지 않는 것과 같은 이유로, 분모가 2의 거듭제곱이 아닌 분수(0.1 = 1/10)는 이진 소수로 유한하게 적을 수 없습니다.
...
버그가 아니라 **유한 비트로 무한 소수를 담을 때 필연적으로 생기는 결과**입니다. 언어를 바꿔도 IEEE 754를 쓰는 한 똑같습니다.
