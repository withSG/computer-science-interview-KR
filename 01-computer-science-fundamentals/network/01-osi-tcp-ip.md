...
계층 모델이 없다고 상상해 보겠습니다. "브라우저가 서버에서 HTML을 받아온다"는 기능 하나를 만들려면 **하나의 프로그램이 전부** 처리해야 하는 일이 있습니다.
전기 신호를 케이블에 어떻게 실을지, 여러 대가 붙은 랜선에서 누구 신호인지 어떻게 구분할지,
서울에서 버지니아까지 어느 경로로 보낼지, 중간에 유실되면 어떻게 다시 보낼지,
그리고 HTML을 어떤 형식으로 주고받을지까지 모두 그 프로그램 몫입니다. 여기서 생기는 문제가 세 가지입니다.
...
그래서 이더넷을 Wi-Fi로 바꿔도 HTTP는 코드 한 줄 안 바뀝니다.
문제가 생기면 "어느 계층까지는 정상인가"를 아래에서 위로 확인하며 범위를 좁힐 수 있습니다.

> **비유** — 국제 택배. 내가 쓴 편지(응용)는 봉투에 담겨 "몇 층 몇 호 김OO 담당자" 표시가 붙습니다(전송).
> 그 봉투는 "서울시 강남구 ..." 주소가 적힌 상자에 들어가고(네트워크), 상자는 구간마다 다른 배송 기사가
> 트럭에 싣고 옮깁니다(데이터링크). 편지 쓴 사람은 트럭 기종을 몰라도 되고, 기사는 편지 내용을 몰라도 됩니다.
>
> **비유의 한계** — 택배 상자는 한 번 포장하면 도착할 때까지 그대로입니다.
> 하지만 네트워크에서 데이터링크 헤더(L2)는 **라우터를 지날 때마다 뜯기고 새로 붙습니다.** 뒤(5절)에서 이 부분을 따로 짚습니다.
...
실제 인터넷이 이 모델대로 구현된 건 아닙니다. "네트워크 기능을 이렇게 나눠서 이야기하자"는 공용 언어에 가깝습니다.
...
프레임 끝에는 FCS(Frame Check Sequence) 라는 검사값이 붙습니다. 전송 중 비트가 뒤집힌 프레임은 버립니다.

이 계층의 통신 범위는 **인접한 한 구간(hop)** 이 전부입니다. 서울에서 도쿄까지 가는 것이 아니라
"내 노트북 → 공유기"까지만 책임집니다. 스위치(Switch)는 MAC 주소 테이블을 학습해서
프레임을 해당 포트로만 보내기 때문에, 허브와 달리 불필요한 트래픽을 만들지 않습니다.

### 3계층 — 네트워크 (Network)

MAC 주소만으로는 전 세계를 찾아갈 수 없습니다. MAC은 제조사가 박아 넣은 값이라 위치 정보가 없습니다.
...
신뢰성을 보장할지도 이 계층에서 고릅니다. TCP는 순서 보장 · 재전송 · 흐름 제어 · 혼잡 제어를 얹고,
UDP는 포트 구분과 체크섬만 붙이고 끝냅니다. 자세한 내용은 [03-tcp-udp.md](./03-tcp-udp.md)에서 다룹니다.
...
사용자가 실제로 쓰는 프로토콜, 그러니까 HTTP, DNS, SMTP, FTP, SSH, WebSocket이 여기 있습니다.
주의할 점이 하나 있습니다. 7계층은 "브라우저 자체"가 아니라 **브라우저가 사용하는 통신 규약**입니다.
...
이것이 **캡슐화(Encapsulation)** 입니다. 각 계층에서 다루는 데이터 덩어리는 **PDU(Protocol Data Unit)** 라 부릅니다.
...
수신 측은 정확히 반대로 올라갑니다. 프레임의 FCS를 검사하고 MAC 헤더를 뗍니다.
IP 헤더를 보고 자기 것이 맞으면 그것도 떼고, TCP 헤더의 포트를 보고 해당 소켓에 데이터를 넣습니다.
이 과정이 **비캡슐화(Decapsulation)** 입니다.
...
내가 아는 건 IP뿐입니다. 이 간극을 메우는 것이 **ARP(Address Resolution Protocol)** 입니다.
...
목적지 IP는 여전히 최종 서버지만, 목적지 MAC은 라우터입니다.
이 장면이 3계층과 2계층이 서로 다른 주소를 쓰는 이유를 가장 잘 보여줍니다.
...
`ping`은 되는데 `curl`이 안 되면 L1~L3는 정상이고 L4 이상의 문제입니다. 그러면 원인은 셋 중 하나입니다.
방화벽이 443을 막았거나, 서버 프로세스가 죽었거나, TLS 인증서가 만료된 경우입니다.
...
계층 모델은 "각 계층이 아래 계층을 신뢰하고 위 계층을 신경 쓰지 않는다"는 약속입니다.
데이터는 내려가며 헤더를 입고(캡슐화), 올라가며 헤더를 벗습니다(비캡슐화).
