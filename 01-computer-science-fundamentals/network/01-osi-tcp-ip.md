...
**첫째, 교체가 불가능하다.** Wi-Fi가 새로 나왔다고 브라우저를 다시 짜야 한다면 아무도 새 기술을 못 만듭니다.
**둘째, 조합 폭발이 일어난다.** 프로토콜 5개 × 전송 매체 5개 = 25가지 구현을 따로 만들어야 합니다.
**셋째, 고장 지점을 못 찾는다.** 페이지가 안 열릴 때 랜선 문제인지 DNS 문제인지 서버 코드 문제인지 구분할 방법이 없습니다.
...
> **라우터를 지날 때마다 뜯기고 새로 붙는다.** 뒤(5절)에서 이 부분을 따로 짚습니다.
...
이 계층은 데이터의 **의미를 전혀 모른다.** 허브(Hub)가 받은 신호를 모든 포트로 그대로 뿌리는 이유가 그래서입니다.
