...
> 난이도 ⭐⭐ · 해시 테이블은 언제나 빠르다고 외우기 쉽지만, 그 속도에는 조건이 붙습니다.
> 키를 배열 인덱스로 바꾸는 발상 하나로 평균 O(1) 검색이 나오는 원리, 그 O(1)이 무너지는 조건, Java HashMap이 그 무너짐을 막는 방법을 이 문서에서 가려냅니다.
...
배열에서 "값 42가 어디 있지?"를 찾으려면 처음부터 훑어야 합니다 — O(n). 정렬해 두고 이진 탐색을
하면 O(log n)이지만 정렬 상태를 계속 유지해야 합니다. 여기서 한 가지 관찰을 해 보겠습니다.
배열의 **인덱스 접근은 이미 O(1)** 입니다. `arr[7]`은 탐색이 아니라 주소 계산일 뿐입니다.
...
비슷한 키가 비슷한 버킷에 가면 오히려 몰림이 생겨서 나쁩니다.)
...
합니다. 그래야 `"user1"`과 `"user2"`가 인접 버킷에 몰리지 않고 흩어집니다.
...
31을 곱하는 데는 이유가 있습니다. 홀수 소수라 정보 손실이 적고, `31 * h`가 `(h << 5) - h`로 최적화되기 때문입니다.
...
`% n`을 `& (n-1)`로 바꿉니다. 훨씬 빠릅니다. 대신 하위 비트만 인덱스에 반영되는 약점이 생기고,
이걸 다음 방법으로 보완합니다.
...
양인데도 그렇습니다. 그래서 해시 테이블 설계의 절반은 "충돌을 어떻게 처리할 것인가"입니다.
...
버킷을 찾는 건 O(1)이고, 체인 안에서 키를 찾는 데는 체인 길이만큼 걸립니다. 평균 체인 길이는
`저장된 원소 수 / 버킷 수`이고, 이 값을 **로드 팩터(Load Factor) α**라 부릅니다. 그래서 조회 비용은
대략 `1 + α`입니다. α를 상수로 유지하면 O(1)이 됩니다.
...
**1차 군집(Primary Clustering)** 은 채워진 칸이 한 덩어리로 뭉치는 현상입니다. 그 근처에 해시된 키는
덩어리 전체를 지나야 합니다. 덩어리가 커질수록 새 키가 거기 붙을 확률도 올라갑니다. 악순환입니다.
...
즉 **묘비(Tombstone)** 를 남겨야 합니다. 묘비가 쌓이면 실제 데이터가 적어도 탐사가 길어집니다.
주기적으로 테이블을 재구축해야 하는 이유입니다.
...
**언제 뭘 쓰나**: 삭제가 잦고 로드 팩터 관리가 부담스러우면 체이닝을 씁니다. 메모리가 빠듯하고 캐시 성능이 중요하며
로드 팩터를 낮게 유지할 수 있다면 개방 주소법입니다. 쓰는 라이브러리가 어느 쪽인지 알면 성능 특성을 예측할 수 있습니다.
...
α가 작으면 충돌이 적어 빠르지만 빈 버킷이 많아 메모리를 낭비합니다. α가 크면 그 반대입니다.
...
버킷 개수가 바뀌면 `hash % n` 결과도 달라집니다. 그래서 기존 원소를 그냥 옮길 수 없고, 인덱스를 다시
계산해야 합니다. 이 과정을 rehashing이라 부릅니다.
...
유지됩니다. 다만 리사이징이 일어나는 순간에는 응답이 튑니다. 지연에 민감하면 예상 크기를 미리 지정해 아예 없애는 편이 낫습니다.
...
초기 용량은 저장할 원소 수가 아니라 **버킷 수**라는 점에 주의합니다. 1000을 그대로 넘기면 2의 거듭제곱으로
올림되어 1024 버킷이 잡힙니다. 임계값은 `1024 × 0.75 = 768`이라, 768개를 넘는 순간 리사이징이 일어납니다.
...
그 정도로 쌓였다는 건 해시가 나쁘거나 공격을 받고 있다는 신호입니다. 트리 노드는 메모리를 더 쓰므로 평소에는 리스트로 두고 이상 신호가 있을 때만 전환하는 게 이득입니다.
...
보내면 해시 테이블이 연결 리스트로 퇴화합니다. 조회마다 O(n)이 걸려 CPU가 마비됩니다. 트리화는 최악을
O(n)에서 O(log n)으로 묶어 이 공격의 위력을 크게 줄입니다. (Rust의 `HashMap`이 기본 해셔로 SipHash를
쓰는 것도 같은 문제의 다른 해법입니다. 무작위 시드를 섞어 공격자가 충돌 키를 예측하지 못하게 만듭니다.)

### 스레드 안전성

`HashMap`은 스레드 안전하지 않습니다. 여러 스레드가 동시에 put하면 갱신이 덮어써져 데이터가 유실되고, 리사이징이 겹치면
자료구조 자체가 깨집니다. Java 7까지는 리사이징 중 체인이 뒤집히며 순환이 생겨 조회가
...
hashCode가 같다고 같은 키는 아니므로(충돌이 있으니까) 2단계가 필요하고, 버킷이 다르면 equals를
부를 기회조차 없으므로 1단계가 먼저입니다.
...
1번이 깨지면 조회가 실패합니다. 2번은 충돌이 정상이라는 뜻이고, 3번은 다음 절의 함정과 이어집니다.
...
셋의 원소는 불변 객체를 쓰는 것이 원칙**입니다. `String`, `Integer`, `record`가 좋은 키인 이유가 여기 있습니다.
...
`hashCode ^ (hashCode >>> 16)`으로 상위 비트를 섞습니다. 조회할 때도 같은 방식으로 버킷을 찾고,
그 안에서 `equals()`로 실제 키를 비교합니다. 버킷 찾기가 O(1)이라 평균 O(1)입니다.
한 버킷에 몰리면 최악 O(n)이 됩니다.
...
붙이는 방식으로, 로드 팩터가 1을 넘어도 동작하고 삭제가 간단합니다. 개방 주소법은 충돌이 나면 다른 빈
버킷을 탐사하는 방식이고, 선형 탐사, 제곱 탐사, 이중 해싱이 있습니다. 캐시 효율이 좋지만 로드 팩터가 1 미만이어야 하고,
삭제할 때 묘비를 남기지 않으면 탐사가 끊겨 데이터를 못 찾습니다.
...
호출될 기회조차 없습니다. hashCode만 재정의하면 같은 버킷에 도달해도 equals가 참조 비교라 다른 키로
판정됩니다. 그래서 "equals가 true면 hashCode도 같아야 한다"는 계약이 있습니다.
두 메서드는 같은 필드 집합을 써야 합니다.
...
해시 테이블은 키를 배열 인덱스로 변환해 평균 O(1) 접근을 얻는 자료구조입니다. 그 O(1)은
