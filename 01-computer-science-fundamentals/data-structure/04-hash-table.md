...
> 그 O(1)이 무너지는 조건은 무엇인지, 그리고 Java HashMap이 그 무너짐을 어떻게 막는지 설명할 수 있게 된다.
...
하면 O(log n)이지만 정렬 상태를 계속 유지해야 한다. 여기서 한 가지 관찰을 해 보자.
배열의 **인덱스 접근은 이미 O(1)** 이다. `arr[7]`은 탐색이 아니라 주소 계산이니까.

> **찾으려는 값 자체로부터 인덱스를 계산해 낼 수 있다면?**

학번이 1~1000번인 학생 정보라면 크기 1001짜리 배열을 만들고 학번을 그대로 인덱스로 쓰면 조회가
O(1)이다. 문제는 키가 학번처럼 작은 정수가 아닐 때다. 이메일 주소, URL, 임의의 객체 —
이런 걸 인덱스로 쓸 수는 없다.

**해시 함수(Hash Function)** 가 이 간극을 메운다. 임의의 키를 정해진 범위의 정수로 뭉개서
배열 인덱스로 만들어 준다. 이 배열을 **해시 테이블**, 각 칸을 **버킷(Bucket)** 이라 부른다.
...
키가 아무리 길어도 인덱스 계산은 한 번, 그다음은 배열 접근 한 번. 이것이 평균 O(1)의 정체다.
비유하자면 도서관 청구기호다. 책 제목을 보고 규칙에 따라 번호를 만들면, 그 번호가 가리키는 서가로
곧장 가면 된다. 서가를 하나씩 뒤질 필요가 없다.
(비유의 한계: 청구기호는 사람이 의미를 담아 설계하지만, 해시 함수는 의미를 일부러 파괴한다.
...
**1. 결정적(Deterministic)** — 같은 키는 언제 넣어도 같은 값이 나와야 한다. 실행 도중 값이
바뀌면 방금 넣은 데이터도 못 찾는다.

**2. 빨라야 한다** — 해시 계산이 느리면 O(1)이라는 말이 무의미하다. 그래서 자료구조용 해시는
SHA-256 같은 암호학적 해시와 목표가 다르다. 암호학적 해시는 역산 불가능성이, 해시 테이블은
속도와 분산이 중요하다.

**3. 고르게 흩어져야 한다(Uniform Distribution)** — 제일 중요하다. 아무리 빨라도 모든 키가 버킷
3번에 몰리면 그냥 연결 리스트다. 나아가 **입력이 조금만 달라도 결과가 크게 달라야**(Avalanche)
`"user1"`과 `"user2"`가 인접 버킷에 몰리지 않고 흩어진다.
...
31을 곱하는 이유는 홀수 소수라 정보 손실이 적고, `31 * h`가 `(h << 5) - h`로 최적화되기 때문이다.
자릿수마다 다른 가중치를 주므로 `"ab"`와 `"ba"`가 다른 값을 갖는다.

### 인덱스로 바꾸기

해시값은 int 전체 범위(음수 포함)라 그대로 인덱스로 못 쓴다. `index = hash % bucketCount`처럼
버킷 개수로 나눈 나머지를 취한다. 버킷 개수를 **소수(prime)** 로 잡는 구현이 많은데, 키가 특정
배수 패턴을 가질 때 몰림을 줄이기 위해서다. 반면 Java `HashMap`은 **2의 거듭제곱**을 써서
`% n`을 `& (n-1)`로 바꾼다. 훨씬 빠르지만 하위 비트만 인덱스에 반영되는 약점이 생기는데,
이걸 다음 방법으로 보완한다.
...
상위 16비트를 하위로 내려 XOR한다. 이렇게 하면 원래 상위 비트에만 차이가 있던 두 키도
하위 비트에 그 차이가 반영되어 서로 다른 버킷으로 간다. 나눗셈을 비트 연산으로 바꿔 얻은 속도를,
비트 섞기 한 줄로 지켜 내는 설계다.

---

## 3. 충돌 (Collision)

키의 가짓수는 사실상 무한한데 버킷은 유한하다. 서로 다른 키가 같은 버킷으로 가는 일은
**드문 사고가 아니라 필연**이다. 생일 문제와 같은 계산을 해 보면, 버킷이 100만 개여도 키를 1000개쯤 넣는
순간 어딘가에서 충돌이 날 확률이 40%에 가깝고 1200개면 절반을 넘는다. 버킷 수에 비하면 턱없이 적은
양인데도 그렇다. 그래서 해시 테이블 설계의 절반은 "충돌을 어떻게 처리할 것인가"다.

### 방법 1: 분리 연결법 (Separate Chaining)

같은 버킷에 여러 개가 오면 **그 자리에 목록을 만들어 이어 붙인다.**
...
버킷을 찾는 건 O(1)이고, 체인 안에서 키를 찾는 데 체인 길이만큼 걸린다. 평균 체인 길이는
`저장된 원소 수 / 버킷 수`이며 이를 **로드 팩터(Load Factor) α**라 부른다. 따라서 조회 비용은
대략 `1 + α`이고, α를 상수로 유지하면 O(1)이 된다.

### 방법 2: 개방 주소법 (Open Addressing)

체인을 만들지 않고 **비어 있는 다른 버킷을 찾아간다.** 모든 데이터가 배열 안에 산다.
...
덩어리 전체를 지나야 하는 현상이다. 덩어리가 커질수록 새 키가 거기 붙을 확률도 올라가 악순환이 된다.

### 개방 주소법의 삭제 문제

여기가 진짜 함정이다. 위 예에서 K2를 그냥 지우면 이렇게 된다.
...
탐사가 빈 칸에서 멈추기 때문이다. 그래서 삭제한 자리에 "여기는 비었지만 지나가라"는 표식,
즉 **묘비(Tombstone)** 를 남겨야 한다. 묘비가 쌓이면 실제 데이터가 적어도 탐사가 길어지므로
주기적으로 테이블을 재구축해야 한다.
...
로드 팩터를 낮게 유지할 수 있으면 개방 주소법. 쓰는 라이브러리가 어느 쪽인지 알면 성능 특성이 예측된다.

---

## 4. 로드 팩터와 리사이징

`로드 팩터 α = 저장된 원소 수 / 버킷 개수`. α는 **시간과 공간의 조절 손잡이**다.
α가 작으면 충돌이 적어 빠르지만 빈 버킷이 많아 메모리를 낭비하고, α가 크면 그 반대다.

개방 주소법에서는 이 관계가 특히 가파르다. 선형 탐사의 실패 탐색 근사식 `(1 + 1/(1-α)²) / 2`에 대입하면
α가 0.5일 때 약 2.5회, 0.75일 때 약 8.5회, 0.9일 때 약 50회다. 0.75에서 0.9로 올렸을 뿐인데 여섯 배다.
"메모리 좀 아끼자"는 판단이 성능 절벽으로 이어지는 지점이 있다.

### 리사이징(Rehashing)

α가 기준을 넘으면 **버킷 배열을 키우고 전부 다시 배치한다.**
버킷 개수가 바뀌면 `hash % n` 결과가 달라지므로, 기존 원소를 그냥 옮길 수 없고 인덱스를 다시
계산해야 한다. 그래서 rehashing이라 부른다.
...
유지된다. 다만 리사이징 순간에는 응답이 튀므로, 지연에 민감하면 예상 크기를 미리 지정해 아예 없애는 게 낫다.
...
초기 용량은 "저장할 원소 수"가 아니라 **버킷 수**라는 점에 주의한다. 1000을 그대로 넘기면 2의 거듭제곱으로
올림되어 1024 버킷이 잡히고, 임계값이 `1024 × 0.75 = 768`이라 768개를 넘는 순간 리사이징이 일어난다.
...
**8**: 해시가 고르게 분포한다는 가정 아래 한 버킷에 노드가 8개 쌓일 확률은 통계적으로 극히 낮다.
그 정도로 쌓였다는 건 해시가 나쁘거나 공격을 받고 있다는 신호다. 트리 노드는 메모리를 더 쓰므로
평소에는 리스트로 두고 이상 신호가 있을 때만 전환하는 게 이득이다.

**64**: 테이블이 작을 때 버킷이 붐비는 건 해시가 나빠서가 아니라 **버킷이 부족해서**일 가능성이
높다. 트리로 바꿔 봐야 근본 해결이 안 되므로 테이블을 키우는 쪽을 택한다.
...
이게 실전에서 왜 중요하냐면, **HashDoS 공격** 때문이다. 공격자가 같은 버킷으로 가는 키를 대량으로 만들어
보내면 해시 테이블이 연결 리스트로 퇴화하고, 조회마다 O(n)이 걸려 CPU가 마비된다. 트리화는 최악을
O(n)에서 O(log n)으로 묶어 이 공격의 위력을 크게 줄인다. (Rust의 `HashMap`이 기본 해셔로 SipHash를
쓰는 것도 같은 문제의 다른 해법이다. 무작위 시드를 섞어 공격자가 충돌 키를 예측하지 못하게 만든다.)

### 스레드 안전성

`HashMap`은 스레드 안전하지 않다. 여러 스레드가 동시에 put하면 갱신이 덮어써져 데이터가 유실되고,
리사이징이 겹치면 자료구조 자체가 깨진다. Java 7까지는 리사이징 중 체인이 뒤집히며 순환이 생겨 조회가
무한 루프에 빠지는 사례가 유명했다. 멀티스레드에서는 버킷 단위로 잠금 범위를 좁힌 `ConcurrentHashMap`을
쓴다. `Collections.synchronizedMap`처럼 맵 전체를 잠그는 방식보다 처리량이 훨씬 좋다.

---

## 6. equals와 hashCode 계약

HashMap의 검색은 **두 단계**다. 이 하나만 기억하면 나머지는 저절로 따라온다.
...
버킷이 다르면 equals를 부를 기회조차 없으므로 1단계가 먼저다.
...
`new User("u1")` 두 개는 equals로는 같지만 hashCode가 다르다. 서로 다른 버킷으로 가므로
**equals가 호출조차 되지 않는다.** 논리적으로 같은 키인데 못 찾는다. 반대로 hashCode만 재정의하면
같은 버킷까지는 도달하지만, equals가 참조 비교라서 "같은 버킷의 다른 키"로 판정되어 역시 못 찾는다.
...
1번이 깨지면 조회가 실패한다. 2번은 충돌이 정상이라는 뜻이다. 3번은 다음 절의 함정과 연결된다.
구현할 때 지켜야 할 원칙은 하나다. **두 메서드가 같은 필드 집합을 써야 한다.**
...
객체는 여전히 버킷 3에 있는데 해시가 바뀌어 버킷 7을 뒤진다. 데이터가 맵 안에 갇힌다. 그래서 **맵의 키와
셋의 원소는 불변 객체를 쓰는 것이 원칙**이고, `String`, `Integer`, `record`가 좋은 키인 이유가 여기 있다.
`HashSet`도 내부가 `HashMap`이라 넣은 뒤 원소의 필드를 바꾸면 중복 제거가 깨진다.
...
- 꼬리 질문: "왜 나머지 연산 대신 AND를 쓰나요?" → 버킷 수가 2의 거듭제곱이면
  `hash % n == hash & (n-1)`이 성립하고, 나눗셈보다 비트 연산이 훨씬 빠르다.
...
- 꼬리 질문: "선형 탐사의 단점은?" → 1차 군집. 채워진 칸이 덩어리로 뭉치면 그 근처 키가
  덩어리 전체를 지나야 하고, 덩어리가 커질수록 더 커지는 악순환이 생긴다.
...
- 꼬리 질문: "그럼 항상 트리로 두면 안 되나요?" → 트리 노드가 메모리를 더 쓰고, 정상적인 해시라면
  버킷당 노드가 1~2개라 트리의 이점이 없다. 이상 상황에서만 전환하는 게 합리적이다.
...
- 꼬리 질문: "키로 가변 객체를 쓰면?" → 넣은 뒤 필드를 바꾸면 해시가 달라져 다른 버킷을 뒤지게
  되고, 객체가 맵 안에 갇혀 조회도 삭제도 안 된다. 키는 불변 객체를 쓴다.
