...
체인을 만들지 않고 **비어 있는 다른 버킷을 찾아간다.** 모든 데이터가 배열 안에 삽니다.
...
`로드 팩터 α = 저장된 원소 수 / 버킷 개수`. α는 **시간과 공간의 조절 손잡입니다.
...
HashMap의 검색은 **두 단계입니다. 이 하나만 기억하면 나머지는 저절로 따라옵니다.
...
**equals가 호출조차 되지 않는다.** 논리적으로 같은 키인데 못 찾습니다. 반대로 hashCode만 재정의하면
...
구현할 때 지켜야 할 원칙은 하나입니다. **두 메서드가 같은 필드 집합을 써야 한다.**
