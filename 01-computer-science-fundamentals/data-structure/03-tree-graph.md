...
완전 이진 트리가 중요한 건 **빈틈이 없어서 배열로 표현할 수 있어서입니다**(힙이 이 성질을 씁니다).
편향 트리는 트리의 이점을 전부 잃은 상태입니다. 이걸 막는 것이 3장의 핵심 주제입니다.
...
수는 어느 경로나 같은데(5번) 빨강은 연속될 수 없어(4번) 최대 절반까지만 끼어들 수 있습니다.
...
간선에 붙은 값이 **가중치(Weight)**, 한 정점에 붙은 간선 수가 **차수(Degree)** 입니다.
...
하나뿐이라 계층 구조를 표현합니다. 정점이 N개면 간선이 정확히 N-1개이고 두 노드 사이 경로가
...
O(log n)번 회전할 수 있는 반면 Red-Black은 3회 이내입니다. 갱신이 잦은 범용 라이브러리는 그래서
