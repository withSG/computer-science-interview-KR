...
배열과 리스트는 **일렬로 늘어선 관계**만 표현합니다. 그런데 현실의 데이터는 대부분 일렬이 아닙니다.

파일 시스템에서 폴더는 여러 하위 폴더를 갖고, 하위 폴더는 다시 하위를 갖습니다. HTML의 `<div>`도
마찬가지입니다. 조직도, 카테고리, 댓글의 대댓글도 그렇습니다. 이건 **한 방향으로 갈라지는 계층 구조입니다.
루트가 하나 있고, 각 노드는 부모가 정확히 하나이며, 돌아오는 길은 없습니다. 이걸 트리라고 부릅니다.

그런데 지하철 노선도는 다릅니다. 역 A에서 B로 갈 수 있고 B에서 A로도 갈 수 있으며, 빙 돌아 제자리로
오는 경로도 존재합니다. 루트라 부를 만한 것도 없습니다. SNS의 친구 관계, 웹 페이지의 링크, 도로망도
같은 성질인데, 이런 **자유로운 연결 관계**가 그래프입니다. 트리는 그래프의 특수한 경우 —
...
각 노드가 **자식을 최대 두 개까지만** 갖는 트리. 왼쪽 자식(left), 오른쪽 자식(right)으로 구분합니다.
자식 수를 2로 제한하는 이유는 단순합니다. 매 단계에서 선택지를 둘로 나눌 수 있으면
n개를 log₂n번 만에 좁힐 수 있고, 구현도 포인터 두 개로 끝납니다.
...
편향 트리는 트리의 이점을 전부 잃은 상태이며, 3장의 핵심 주제가 이걸 막는 것입니다.
...
"왼쪽 자식 하나만" 작은 게 아니라 **서브트리 전체**가 작아야 한다는 점이 중요합니다.
...
문제는 BST의 O(log n)이 **트리 높이가 log n일 때**의 이야기라는 것입니다. 그런데 높이는 삽입 순서에 좌우됩니다.
위 오른쪽처럼 정렬된 값을 순서대로 넣으면 한쪽으로만 자식이 붙어 높이가 n이 되고, 모든 연산이 O(n)으로
퇴화합니다. **정렬된 데이터를 넣는 건 아주 흔한 일**이라 더 심각합니다. DB에서 ID 순으로 읽어와 넣거나
타임스탬프 순으로 로그를 넣으면 그대로 최악의 경우입니다. "운이 나쁠 때"가 아니라 "평범한 입력"에서 터집니다.

그래서 삽입/삭제 때 **스스로 모양을 고치는** 트리가 필요합니다. 자가 균형 이진 탐색 트리
(Self-Balancing BST)라고 부르며, AVL과 Red-Black Tree가 대표입니다.

### 회전 (Rotation)

균형을 고치는 유일한 도구가 회전입니다. **BST 조건을 깨지 않으면서 부모-자식 관계를 바꾸는** 연산으로,
...
범위를 벗어나면 회전으로 복구하는데, 어느 쪽이 어떻게 기울었느냐에 따라 네 가지 경우가 있습니다.
...
LR과 RL이 두 번 회전하는 이유는, 지그재그 모양을 한 번에 펼 수 없기 때문입니다.
...
AVL처럼 높이 차 1을 고집하면 갱신마다 균형을 되돌리는 작업이 잦고, 특히 삭제에서는 회전이 루트까지 연쇄될 수 있습니다.
...
수는 어느 경로나 같고(5번), 빨강은 연속될 수 없어(4번) 최대 절반까지만 끼어들 수 있기 때문입니다.
결과적으로 높이가 O(log n)으로 묶입니다.
...
라이브러리가 대부분 Red-Black을 고른 이유는 사용 패턴을 미리 알 수 없기 때문입니다.
디스크 기반이라면 얘기가 또 달라집니다. 노드 하나가 곧 디스크 I/O 한 번이라 높이를 더 낮춰야 하고,
...
**희소 그래프(sparse)** 면 인접 리스트입니다. 사용자 1억 명에 평균 친구 수가 수백 명인 SNS를
...
꺼내니 가까운 곳부터 훑는다(BFS). 나머지 코드는 사실상 같습니다.
...
다익스트라나 A*를 돌리는 것인데, 가중치가 있으므로 단순 BFS로는 안 됩니다.

**데이터베이스 인덱스** — B+Tree. 균형 트리라는 점은 같지만 디스크 블록 단위 I/O에 맞춰
노드 하나에 키를 수백 개 담습니다. 트리 높이가 3~4로 유지되어 디스크 접근 횟수가 최소화됩니다.
...
- 꼬리 질문: "그 중간은 없나요?" → 인접 집합(정점별 `HashSet`)을 쓰면 연결 확인도 평균 O(1)이고
  공간도 O(V+E)다. 대신 상수 계수와 메모리 오버헤드가 늘어납니다.
