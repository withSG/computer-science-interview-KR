...
> 설명되지 않는 성능 차이를 만드는지, 그리고 동적 배열이 "무제한 크기"를 어떻게 흉내내는지 설명할 수 있게 됩니다.
...
만들 수는 없으니까. 그런데 그릇을 만드는 방법은 근본적으로 두 갈래뿐입니다.

**한 덩어리로 붙여 놓기** — 메모리를 한 번에 크게 잡고 값들을 나란히 채웁니다. 위치 계산이 쉬운 대신,
중간에 뭔가 끼워 넣으려면 뒤를 전부 밀어야 하고, 처음에 잡은 공간이 모자라면 곤란해집니다.

**흩어 놓고 화살표로 잇기** — 값 하나씩 따로 할당하고 "다음은 저기"라는 주소를 함께 저장합니다.
중간에 끼워 넣기는 화살표만 고치면 되는 대신, 3번째 값을 보려면 첫 번째부터 화살표를 따라가야 합니다.

전자가 배열(Array), 후자가 연결 리스트(Linked List)입니다. 둘은 서로의 약점을 정확히 맞바꾼 관계이고,
이후 나오는 거의 모든 자료구조는 이 둘 중 하나를 바닥에 깔고 만들어집니다. 스택도, 큐도, 해시 테이블의
버킷도 마찬가지입니다. 그래서 이 둘의 차이를 정확히 아는 것이 자료구조 공부의 출발점이 됩니다.
...
배열은 **같은 타입의 원소를 메모리상 연속된 위치에 저장하는** 자료구조입니다. 여기서 중요한 단어는
"같은 타입"과 "연속"입니다. 둘 다 O(1) 접근을 위한 전제 조건입니다.

### 동작 원리 — 주소 계산

`int arr[5]`가 메모리 주소 `0x1000`부터 시작한다고 하자. `int`가 4바이트라면 이렇게 놓입니다.
...
자료구조입니다. 노드들이 메모리 어디에 있든 상관없습니다. 주소만 알고 있으면 되니까.
...
10과 20 사이에 15를 끼워 넣는다고 하자. 옮길 데이터가 없습니다. 화살표만 고칩니다.
...
- **이중 연결 리스트(Doubly Linked List)**: `prev`도 있음. 양방향 순회 가능하고, 노드 하나만 알면
  그 노드를 O(1)에 삭제할 수 있습니다. 대신 포인터가 하나 더라 메모리를 더 씁니다.
...
나오는데, 실제로 벤치마크를 돌리면 배열이 이기는 경우가 대단히 많습니다. 이유는 CPU와 메모리의
속도 차이에 있습니다.
...
L1과 DRAM 사이에 두 자릿수 배율의 차이가 있습니다. 그래서 "O(n) 데이터 이동"이 "O(1) 포인터 변경"보다
빠른 역전이 흔하게 일어납니다. `memmove`로 배열 원소를 미는 작업은 연속 메모리를 통째로 복사하는,
CPU가 가장 잘하는 종류의 일입니다. 반면 연결 리스트의 O(1) 삽입은 그 노드까지 가는 O(n) 포인터 추적을
동반하고, 그 O(n)의 상수 계수가 아주 큽니다.

**정리하면**: 시간 복잡도는 n이 충분히 클 때의 증가율을 말할 뿐, 상수 계수를 말하지 않습니다.
...
한 번의 확장은 분명 O(n)입니다. 그런데 **확장은 자주 일어나지 않는다**는 게 핵심입니다.
...
약점이 사라집니다. Java의 `LinkedHashMap`이 이 조합(해시 테이블 + 이중 연결 리스트)으로 만들어져 있어서,
...
택하는데, 현대 CPU의 캐시 구조 때문에 실제 승부는 복잡도 표가 아니라 접근 패턴이 결정합니다.
