...
**비유의 한계**: 실제 Heap은 단순한 선반이 아닙니다. 크기별 프리 리스트와 아레나로 나뉜 정교한 구조이고,
...
있는 프레임을 먼저 치울 수 없습니다**. 함수가 리턴한 뒤에도 살아있어야 하는 값을 Stack에 둘 수 없는
근본적인 이유가 이것입니다.
...
들어가고 객체 본체는 Heap에 있습니다. C/C++에서는 `Person p;`로 스택에 객체를 직접 놓을 수 있으니 언어마다
다르긴 합니다. 그래도 Java·C#·Python 같은 언어에서 객체 본체는 항상 Heap입니다.
...
컴파일 타임에 정해지는 것은 **프레임 하나의 크기**입니다. 크기를 미리 알기 때문에 실행 시점에는
스택 포인터를 그만큼 밀기만 하면 됩니다. Heap은 얼마를 달라고 할지가 실행해 봐야 정해집니다.
...
여기서 말하는 주소는 모두 **가상 주소**입니다. 실제 물리 메모리 배치와는 다르고, 그 변환 과정은
...
세 경우 모두 **"누군가 아직 참조를 들고 있다"** 는 한 문장으로 설명됩니다. static 필드,
...
A. Stack은 함수 호출 구조에 맞춰 LIFO로 관리되는 영역입니다. 지역 변수와 매개변수, 리턴 주소가
저장되고 스레드마다 독립적으로 할당됩니다. 스택 포인터만 움직이면 되므로 할당과 해제가 매우 빠르고
동기화도 필요 없습니다. Heap은 런타임에 동적으로 할당되는 영역으로 객체 인스턴스가 저장됩니다. 모든
...
객체를 할당할 수 없을 때 발생합니다. 참조가 남아 GC가 회수하지 못하는 메모리 누수나 애초에 데이터양
