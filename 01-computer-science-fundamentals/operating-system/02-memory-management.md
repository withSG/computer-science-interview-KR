...
> 변수 하나를 선언했을 때 그 값이 어디에 놓이고 언제 사라지는지, 그리고 왜 어떤 에러는 StackOverflowError이고 어떤 에러는 OutOfMemoryError인지 설명할 수 있게 된다.
...
메모리를 한 덩어리로 쓰면 안 되는 걸까? 문제는 데이터마다 **살아있어야 하는 기간이 다르다**는 것이다.

함수 안의 지역 변수는 함수가 끝나면 필요 없다. 수명이 함수 호출과 정확히 일치하니, 함수가 끝날 때
자동으로 치워주면 관리 비용이 0에 가깝다. 반면 "사용자 목록"처럼 함수가 끝나도 계속 남아야 하는
데이터는 자동으로 치우면 안 된다. 언제 지울지 프로그램이 결정해야 한다.

수명이 예측 가능한 것과 그렇지 않은 것을 같은 곳에 두면, 예측 가능한 쪽까지 비싼 관리 비용을 물게
된다. 그래서 메모리를 성격별로 갈랐다.

- **Stack**: 수명이 호출 구조에 종속된 데이터. 포인터 하나 밀고 당기면 끝나므로 극단적으로 빠르다.
- **Heap**: 수명이 프로그램 논리에 달린 데이터. 임의 시점에 할당·해제되므로 관리 비용이 든다.
- **Data/BSS**: 프로그램 실행 내내 사는 데이터. 시작할 때 확보하고 끝날 때 반납한다.
- **Code**: 절대 안 바뀌는 명령어. 읽기 전용으로 막아두면 버그와 공격을 동시에 예방한다.

**비유**: Stack은 카페테리아 쟁반 더미다. 위에서만 놓고 위에서만 집으므로 어느 쟁반이 어디 있는지
찾을 필요가 없다. Heap은 창고 선반이다. 아무 자리에나 놓을 수 있어 유연하지만, 빈 자리를 찾고
나중에 치우는 관리자가 필요하다.

**비유의 한계**: 실제 Heap은 단순한 선반이 아니라 크기별 프리 리스트와 아레나로 나뉜 정교한 구조이며,
JVM은 여기에 세대(Young/Old) 구분까지 얹는다.
...
함수를 호출하면 **스택 프레임(Stack Frame)** 한 칸이 쌓인다. 함수가 리턴하면 그 칸이 통째로 사라진다.
"해제"라는 별도 작업이 없고, 스택 포인터 레지스터를 원위치로 되돌리는 것이 곧 해제다.
...
이 구조 때문에 Stack 할당은 명령어 한두 개 수준으로 끝나고, 단편화도 생기지 않는다. 대신 **아래에
있는 프레임을 먼저 치울 수 없다**. 함수가 리턴한 뒤에도 살아있어야 하는 값은 Stack에 둘 수 없는
근본적인 이유다.
...
들어가고 객체 본체는 Heap에 있다. C/C++에서는 `Person p;`로 스택에 객체를 직접 놓을 수 있어 언어마다
다르지만, Java·C#·Python 같은 언어에서 객체 본체는 항상 Heap이다.

**"재귀는 항상 StackOverflowError를 낸다"** — 깊이가 스택 한계 안이면 아무 문제 없다. 문제는 종료
조건이 잘못됐거나 입력 크기에 비례해 깊이가 무한정 늘어나는 경우다. 예를 들어 리스트 100만 개를
재귀로 순회하면 논리는 맞아도 스택이 터진다. 이럴 땐 반복문이나 명시적 스택 자료구조로 바꾼다.
...
Heap 할당은 "빈 자리 찾기"부터 시작한다. 할당과 해제가 임의 순서로 일어나므로 중간중간 구멍이 생기고,
요청 크기에 맞는 구멍을 골라야 한다. 여기에 여러 스레드가 동시에 요청하면 락 경합까지 붙는다.

현대 런타임은 이 비용을 줄이려고 여러 기법을 쓴다. JVM은 각 스레드에게 Eden 영역의 작은 조각을
전용으로 떼어주는 TLAB(Thread Local Allocation Buffer)를 써서, 대부분의 할당을 포인터 증가 한 번으로
처리한다. 그래서 "Heap 할당은 느리다"는 말은 원리상 맞지만, 실제 체감 차이는 생각보다 작다.
진짜 비용은 할당이 아니라 **회수(GC)** 쪽에 있다.
...
**"GC가 있으니 메모리 관리는 신경 안 써도 된다"** — GC는 *도달 불가능한* 객체만 회수한다. 참조가
살아있으면 다시는 안 쓸 객체라도 절대 회수되지 않는다. GC 언어의 메모리 누수는 "해제를 깜빡한 것"이
아니라 "참조를 끊는 걸 깜빡한 것"이다.
...
표의 "할당 시점"은 짚고 넘어갈 필요가 있다. Stack도 실제로 공간을 잡는 것은 실행 시점이다.
컴파일 타임에 정해지는 것은 **프레임 하나의 크기**이고, 크기를 미리 알기 때문에 실행 시점에는
스택 포인터를 그만큼 밀기만 하면 된다는 뜻이다. Heap은 얼마를 달라고 할지가 실행해 봐야 정해진다.
...
BSS와 Data를 나눈 이유가 궁금할 수 있다. 초기값이 있는 전역 변수는 그 값을 실행 파일에 실어야 하지만,
`int arr[1000000];`처럼 0으로 시작하는 변수는 "이만큼 필요하다"는 크기 정보만 있으면 된다. 후자를
BSS로 빼면 실행 파일 크기가 4MB 줄어든다. 로더가 실행 시점에 0으로 채워준다.

여기서 말하는 주소는 모두 **가상 주소**다. 실제 물리 메모리 배치와는 다르며, 그 변환 과정은
[05-virtual-memory.md](./05-virtual-memory.md)에서 다룬다.
...
**결론**: StackOverflowError는 거의 항상 코드 로직 버그다. OutOfMemoryError는 로직이 맞아도 발생할 수
있어 원인 규명에 도구가 필요하다. 전자는 스택 트레이스만 봐도 답이 나오고, 후자는 힙 덤프를 떠야 한다.
...
핵심은 세 경우 모두 **"누군가 아직 참조를 들고 있다"** 는 한 문장으로 설명된다는 점이다. static 필드,
이벤트 버스, 미반납 리소스는 모두 GC Root에서 도달 가능한 경로를 만든다.
...
`-XX:+HeapDumpOnOutOfMemoryError` 옵션을 미리 켜 두면 OOM이 터지는 순간의 덤프가 자동으로 남는다.
운영 환경에서 재현이 어려운 누수를 잡을 때 이 옵션 하나가 며칠을 아껴준다.
...
캐시 라이브러리를 쓰는 편이 안전하다.

---

## 8. 실무에서는

**스레드 수와 Stack 크기의 곱이 곧 메모리다.** JVM에서 스레드 하나당 스택은 `-Xss`로 정하며 64비트
HotSpot 기본값은 1MB 수준이다. 스레드를 2000개 만들면 스택만으로 2GB 가까이 잡아먹는다. 요청당 스레드
모델에서 동시 접속이 늘 때 힙이 아니라 스택 때문에 죽는 사례가 여기서 나온다.

**컨테이너에서는 힙 크기 계산이 달라진다.** 예전 JVM은 컨테이너 메모리 제한을 못 보고 호스트 전체
메모리를 기준으로 힙을 잡아 OOM Killer에 죽는 일이 잦았다. 요즘 JVM은 cgroup 제한을 인식하지만,
`-XX:MaxRAMPercentage`로 명시하는 편이 예측 가능하다. 컨테이너 메모리는 힙 + 메타스페이스 + 스레드
스택 + 네이티브 버퍼의 합이라는 점을 잊지 말아야 한다.

**힙을 키우는 게 항상 답은 아니다.** 누수가 원인이면 힙을 키워도 터지는 시점만 뒤로 밀린다. 오히려
힙이 커지면 한 번의 GC가 훑어야 할 범위가 넓어져 정지 시간이 길어질 수 있다. 원인을 밝히기 전에
`-Xmx`부터 올리는 것은 진단이 아니라 유예다.
...
- 꼬리 질문: "Java에서 `new`로 만든 객체의 참조 변수는 어디 있나요?" → 지역 변수라면 참조는 Stack,
  객체 본체는 Heap. 필드라면 참조도 객체와 함께 Heap에 있다.
...
- 꼬리 질문: "OOM이 났는데 힙 덤프가 없다면?" → GC 로그의 회수량 추이와 Old 영역 점유율로 누수
  여부를 판단하고, `-XX:+HeapDumpOnOutOfMemoryError`를 켜서 재발 시 확보한다.
...
- 꼬리 질문: "WeakReference를 쓰면 다 해결되나요?" → 참조 강도를 낮추는 건 수단일 뿐, 어떤 데이터를
  언제 버릴지 정책이 먼저다. 만료·크기 정책이 없으면 약한 참조도 근본 해결이 아니다.
...
데이터를 유연하게 담는 대신 회수 비용을 치르는 영역이라, 두 영역에서 나는 에러는 원인도 진단법도 다르다.
