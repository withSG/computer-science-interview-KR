...
> 프로그램이 실행되면 OS가 무엇을 만들어 주는지, 그 안에서 실행 흐름이 어떻게 여러 갈래로 나뉘는지를 메모리 구조 수준에서 설명할 수 있게 됩니다.
...
초창기 컴퓨터는 프로그램 하나를 처음부터 끝까지 돌리고 끝나면 다음 프로그램을 올렸습니다. 문제는 프로그램이
디스크에서 데이터를 읽는 동안 CPU가 아무 일도 못 하고 놀았다는 것입니다. 디스크 접근은 CPU 연산보다
수십만 배 느리기 때문에, 이 방식은 비싼 CPU를 대부분 놀리는 셈이었습니다.

그래서 "여러 프로그램을 메모리에 동시에 올려두고, 하나가 I/O를 기다리면 다른 것에 CPU를 주자"는 발상이
나왔습니다. 이걸 하려면 OS가 **각 실행 중인 프로그램의 상태를 따로 관리**해야 합니다. 어디까지 실행했는지,
레지스터 값이 뭐였는지, 어떤 메모리를 쓰고 있는지. 이 관리 단위가 바로 **프로세스(Process)**입니다.

그런데 프로세스만으로는 부족했습니다. 웹 서버가 요청 1000개를 동시에 처리하려고 프로세스 1000개를 띄우면
메모리가 감당하지 못합니다. 요청 처리 코드는 어차피 같은데 코드와 데이터를 1000벌 복사할 이유가 없습니다.
"메모리는 공유하되 실행 흐름만 여러 개"라는 요구가 생겼고 그 답이 **스레드(Thread)**입니다.
...
이 한 문장의 차이가 이 문서 전체의 모든 결론을 만들어냅니다.
...
프로세스는 **실행 중인 프로그램**으로, 운영체제로부터 자원을 할당받는 작업의 단위입니다.

프로그램은 디스크에 놓인 정적인 파일이고 프로세스는 그 파일이 메모리에 올라와 CPU를 배정받는 능동적인
실체입니다. `notepad.exe`를 세 번 실행하면 프로그램은 하나지만 프로세스는 셋입니다.
...
Stack은 높은 주소에서 아래로, Heap은 낮은 주소에서 위로 자랍니다. 가운데 빈 공간을 공유하는 이 구조는
"둘 중 어느 쪽이 많이 필요할지 미리 모르니 양끝에서 자라게 하자"는 실용적 설계입니다.
...
CPU를 뺏겼다가 다시 받았을 때 "아까 하던 그 지점"으로 정확히 복귀하려면 이 정보가 필요합니다.
...
PCB의 존재가 [03-context-switching.md](./03-context-switching.md)에서 말하는 "상태 저장/복원"의 실체입니다.
...
것이고 **Running → Waiting** 은 스스로 "I/O 끝날 때까지 나 빼주세요"라고 양보한 것입니다. 앞의 것이
선점(preemption), 뒤의 것이 블로킹입니다. 이 구분이 [06-cpu-scheduling.md](./06-cpu-scheduling.md)의
선점형/비선점형 이야기로 이어집니다.
...
메모리를 거의 쓰지 않지만 PID를 점유하므로 대량 발생 시 새 프로세스를 못 만드는 사태가 생깁니다.
...
스레드는 프로세스 내에서 **실행의 단위**입니다. 하나의 프로세스는 여러 스레드를 가질 수 있습니다.

프로세스를 만들 때 OS는 기본 스레드(main thread) 하나를 함께 만듭니다. 즉 "스레드 없는 프로세스"는
없고 우리가 흔히 말하는 싱글 스레드 프로그램도 실은 스레드가 하나인 프로세스입니다.
...
함수 호출 정보와 지역 변수는 "이 실행 흐름이 지금 어디까지 왔는지"를 담습니다. 흐름이 여러 개면 이
기록도 여러 벌이어야 합니다. T1이 `calculate()` 안에 있는데 T2가 같은 Stack을 쓰면 T1의 리턴 주소가
덮여서 프로그램이 엉뚱한 곳으로 점프합니다. 그래서 Stack만은 반드시 분리합니다.

반대로 Heap과 Data는 "프로그램 전체가 공유하는 상태"라서 나눌 이유가 없습니다. 그런데 나누지 않았기
때문에 두 스레드가 같은 객체를 동시에 건드릴 수 있고 여기서 경쟁 상태가 터집니다.
...
`local`은 스레드마다 Stack에 따로 있으니 언제나 정확히 100000입니다. `shared`는 Data 영역에 하나뿐이라
두 스레드가 동시에 읽고 쓰면서 갱신이 유실됩니다. 같은 반복문 안의 두 줄인데 결과가 갈리는 이 지점이
"Stack만 독립"이라는 문장의 실제 의미입니다.

### 흔한 오해

**"스레드를 늘리면 늘린 만큼 빨라진다"** — CPU 코어 수를 넘어서면 그때부터는 컨텍스트 스위칭
오버헤드만 늘어납니다. CPU 집약 작업은 코어 수 근처가 최적이고 I/O 대기가 많은 작업은 그보다 많이
잡아도 이득이 있습니다. 무조건 많이 잡는 설정은 오히려 처리량을 떨어뜨립니다.
...
두 명이 각자 다른 냄비를 맡는 것입니다. 전자는 손이 하나라 실제로는 한 번에 한 가지만 하지만 전체
소요 시간은 줄어듭니다.

**비유의 한계**: 요리사는 "물이 끓는지" 스스로 판단해 돌아가지만 CPU는 스스로 판단하지 않습니다.
타이머 인터럽트가 강제로 끊어주거나 프로세스가 I/O로 자진 양보해야 전환이 일어납니다.

동시성은 **구조**의 문제이고 병렬성은 **자원**의 문제입니다. 동시성 있게 설계한 프로그램을 멀티 코어에
올리면 병렬성이 따라오지만 반대는 성립하지 않습니다.
...
리눅스의 pthread와 Java의 플랫폼 스레드는 1:1입니다. Go의 고루틴과 Java 21의 가상 스레드(Virtual
Thread)는 M:N에 가깝습니다. 가상 스레드는 블로킹 I/O를 만나면 캐리어 스레드에서 자신을 떼어내고(unmount)
다른 가상 스레드에 자리를 내주기 때문에, 스레드 수만 늘려서는 얻을 수 없던 동시성 규모를 만듭니다.

여기서 얻을 교훈은 "스레드가 비싸다"는 말이 절대적이지 않다는 것입니다. 비싼 건 커널 스레드이고
...
크롬이 메모리를 많이 먹는다는 악명은 이 설계의 청구서입니다. 탭 하나가 렌더링 버그로 죽어도 브라우저
전체가 내려가지 않고 악성 사이트가 렌더러 프로세스를 장악해도 샌드박스 밖으로 나가기 어렵습니다.
사용자 경험에서 "탭 하나 때문에 작업 전부 날아감"이 주는 손해가 메모리 비용보다 크다고 본 것입니다.

PHP-FPM은 다른 이유입니다. PHP 생태계의 확장 라이브러리 중 스레드 안전하지 않은 것이 많았고 요청마다
프로세스를 새로 쓰면 이전 요청이 남긴 전역 상태나 메모리 누수가 다음 요청에 넘어가지 않습니다.
"매번 깨끗한 상태에서 시작한다"는 보장을 격리로 산 셈입니다.
...
Tomcat은 요청마다 스레드를 새로 만들지 않고 미리 만들어 둔 풀에서 꺼내 씁니다. 스레드 생성/소멸 비용을
없애고 동시 실행 스레드 수에 상한을 두어 과부하 시 시스템이 무너지지 않게 합니다. 풀 크기를 무한정
키우지 않는 이유가 바로 컨텍스트 스위칭 비용입니다.

Node.js는 반대 방향의 사례입니다. JS 실행은 단일 스레드로 두되 I/O는 논블로킹으로 처리해서 스레드를
늘리지 않고도 대량의 동시 연결을 감당합니다. 다만 이미지 리사이징 같은 CPU 집약 작업이 들어오면 이벤트
루프가 막히므로, 이때만 Worker Threads로 빼냅니다.
...
- 꼬리 질문: "그럼 스레드는 왜 Stack만 따로 갖나요?" → 함수 호출 정보와 리턴 주소가 실행 흐름별로
  달라야 하기 때문. 공유하면 서로의 리턴 주소를 덮어써 제어 흐름이 깨진다는 점을 듭니다.
...
- 꼬리 질문: "가상 스레드가 나온 뒤에도 이 기준이 유효한가요?" → 스레드 쪽 비용이 크게 낮아져
  "스레드가 비싸니 아껴 쓴다"는 전제는 약해졌지만 격리 관련 판단 기준은 그대로입니다.
