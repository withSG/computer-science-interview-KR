...
> 프로그램이 쓰는 주소가 실제 RAM 주소가 아니라는 사실에서 출발해, 페이지 테이블·TLB·페이지 폴트·스래싱이 왜 필요하고 어떻게 맞물리는지 설명할 수 있게 된다.
...
가상 메모리가 없던 시절을 상상해 보자. 프로그램은 물리 주소를 직접 썼다. 여기서 세 가지 문제가
동시에 터진다.

**첫째, 프로그램이 물리 메모리보다 크면 못 돌린다.** 개발자가 직접 프로그램을 조각내 필요한 부분만
올리고 내리는 오버레이(overlay) 기법을 손으로 짜야 했다.

**둘째, 프로세스끼리 서로의 메모리를 침범한다.** 프로그램 A가 실수로 `0x1000`에 쓰면 그게 마침 B의
데이터일 수 있다. 버그 하나가 시스템 전체를 무너뜨린다.

**셋째, 어디에 적재될지 미리 알 수 없다.** 같은 프로그램을 두 번 실행하면 두 번째는 다른 주소에
올라가므로, 코드 안의 모든 주소를 실행 시점에 고쳐야 한다.

가상 메모리는 이 셋을 **주소 한 겹을 더 두는 것**으로 한꺼번에 해결했다. 프로세스는 자기만의 가상
주소를 쓰고 하드웨어(MMU)가 실행 순간마다 물리 주소로 번역한다. 번역표를 프로세스마다 따로 두면 격리가
되고, 표에 "지금은 디스크에 있음"을 표시할 수 있으니 물리 메모리보다 큰 공간도 흉내 낼 수 있으며,
어디에 적재되든 표만 고치면 되니 주소도 고정된다.

**비유**: 회사 대표번호와 내선번호의 관계다. 명함에는 "대표번호 + 내선 301"만 적으면 되고, 실제
자리가 3층에서 5층으로 옮겨져도 교환기 설정만 바꾸면 명함을 다시 찍을 필요가 없다.

**비유의 한계**: 교환기는 사람이 가끔 설정을 바꾸지만, MMU는 **메모리 접근 명령어마다** 번역한다.
그래서 번역 속도가 시스템 전체 성능을 좌우하고, 그 때문에 TLB라는 캐시가 필요해진다.
...
프로그램이 보는 주소와 RAM의 실제 주소를 분리하고 그 사이를 하드웨어가 실시간으로 이어 붙이는 기법이다.
프로세스는 넓은 메모리를 혼자 통째로 쓴다고 믿지만 실제로는 필요한 조각만 물리 메모리에 올라와 있다.
...
여기에 두 가지를 덧붙일 수 있다. **페이지 단위 권한 설정**은 코드 영역을 읽기+실행, 데이터 영역을
읽기+쓰기로 나눠 데이터에 심어진 코드가 실행되는 공격을 하드웨어가 막게 한다. **공유와 지연 복사**는
여러 프로세스가 물리 프레임 하나를 공유하고 페이지 테이블만 각자 가리키게 해 메모리를 절약한다.

후자의 대표 사례가 **Copy-on-Write(CoW)** 다. `fork()`로 프로세스를 복제할 때 메모리를 실제로
복사하지 않고 페이지 테이블만 복제한 뒤 모든 페이지를 읽기 전용으로 표시하고, 둘 중 하나가 쓰기를
시도하는 순간에만 그 페이지를 복사한다. 대부분의 `fork()`는 곧바로 `exec()`으로 이어지므로 이
최적화 덕분에 낭비되는 복사가 사라진다.
...
가상 주소는 통째로 번역되지 않는다. **페이지 번호**와 **오프셋**으로 나뉘고, 번역되는 것은 페이지
번호뿐이다.
...
안에서도 같은 위치이기 때문이다. 덕분에 번역표는 페이지 개수만큼만 있으면 된다.

페이지 테이블 항목(PTE)에는 프레임 번호 말고도 여러 비트가 함께 들어간다.
...
Dirty 비트가 실질적인 성능 차이를 만든다. 읽기만 한 페이지는 디스크 사본과 동일하므로 그냥 버리면 되고,
수정된 페이지만 디스크에 다시 써야 한다.

### 다단계 페이지 테이블

64비트 주소 공간에서 페이지 테이블을 한 겹으로 두면 표 자체가 감당 못 할 크기가 된다. 그래서 실제
시스템은 테이블을 여러 단계로 나누고 실제로 쓰이는 구간의 하위 테이블만 만든다. 대가는 명확하다.
...
접근이고, 다단계 테이블이면 조회만 여러 번이라 접근 횟수가 몇 배로 늘어난다.

TLB는 최근 변환 결과를 CPU 안에 담아두고, 적중하면 페이지 테이블을 아예 건드리지 않는다. 프로그램은
보통 좁은 범위의 메모리를 반복해서 쓰기 때문에(지역성) 적중률이 매우 높다. 거꾸로 **메모리를 넓게
흩어서 접근하는 코드는 TLB 적중률이 떨어져 느려진다**. 대용량 메모리를 다루는 데이터베이스나 JVM에서
**huge page**(4KB 대신 2MB 페이지)를 쓰는 이유도 같은 맥락으로, 페이지 하나가 커지면 같은 메모리를
훨씬 적은 TLB 항목으로 덮을 수 있다.
...
현대 CPU는 TLB 항목에 주소 공간 식별자(x86의 PCID, ARM의 ASID)를 붙여 전체 플러시를 피한다. 식별자가
다르면 서로의 항목을 잘못 쓸 일이 없기 때문이다. 다만 TLB 용량은 한정적이라 여러 프로세스가 항목을
두고 경쟁하는 압박은 남는다. 자세한 비용 분석은
[03-context-switching.md](./03-context-switching.md)를 참고한다.
...
마지막 줄이 중요하다. 명령어를 "이어서" 하는 게 아니라 **처음부터 다시** 실행한다. 그래야 중간에
끊긴 명령어가 반쪽만 적용되는 사태를 막을 수 있다.
...
프로그램 시작 시 대량의 minor fault는 정상이다. 문제는 **major fault가 지속적으로 많은 경우**이고 이건
스왑이 돌고 있다는 신호다. `vmstat`의 `si`/`so`나 `ps -o min_flt,maj_flt`로 확인한다.
...
**그래서 언제 뭘 쓰나**: 이론적 최적은 미래를 아는 OPT지만 구현 불가능하다. 실제 OS는 접근마다
리스트를 재정렬하는 비용을 감당할 수 없어 참조 비트로 근사하는 Clock 계열을 쓴다. 반면 애플리케이션
레벨 캐시는 접근 빈도가 훨씬 낮아 LRU를 정확히 구현해도 괜찮다.

### Belady's Anomaly

보통은 메모리를 늘리면 페이지 폴트가 준다. 그런데 FIFO에서는 **프레임 수를 늘렸는데 폴트가 늘어나는**
역설이 생길 수 있다. "언제 들어왔는가"만 볼 뿐 "얼마나 쓸모 있는가"를 반영하지 않기 때문이며, LRU는
스택 알고리즘이라는 성질 덕분에 이 현상이 없다.

### LRU를 O(1)로 구현하기

`HashMap + 이중 연결 리스트` 조합이 정석이다.
...
- 조회: HashMap으로 노드를 O(1)에 찾고, 그 노드를 리스트에서 떼어 head로 옮긴다. 이중 연결 리스트라
  앞뒤 노드를 알고 있어 분리·삽입이 O(1)이다.
- 삽입: head에 새 노드를 넣는다. 용량을 넘으면 tail 노드를 제거하고 HashMap에서도 지운다.

단일 연결 리스트로는 안 된다. 노드를 떼어내려면 앞 노드를 알아야 하는데 찾는 데 O(n)이 걸리기
때문이다. Java의 `LinkedHashMap`은 접근 순서 모드와 `removeEldestEntry` 오버라이드로 간단한 LRU
캐시를 만들 수 있고, 만료·통계·동시성까지 필요하면 Caffeine 같은 전용 캐시 라이브러리를 쓴다.
...
메모리를 채우고 비우는 뒷일이 정작 해야 할 계산을 밀어내 버린 상태다. 페이지를 올리면 다른 페이지가
밀려나고 밀려난 페이지가 곧바로 다시 필요해지는 순환에 갇혀, CPU 사용률은 낮은데 시스템은 응답하지 않는다.
...
스래싱이 무서운 이유는 OS의 정상적인 판단이 상황을 악화시키기 때문이다.
...
CPU 이용률만 보고 다중 프로그래밍 정도를 올리는 정책이 문제를 증폭시킨다. 그래서 해법은 "CPU가 아니라
**메모리 요구량**을 보고 판단하자"는 방향으로 간다.

### Working Set 모델

Working Set은 **최근 시간 Δ 동안 프로세스가 참조한 페이지들의 집합**이다. 지금 이 프로세스가 활발히
쓰는 메모리의 크기라고 보면 된다.

각 프로세스에 자기 Working Set 크기만큼의 프레임을 보장하면 폴트가 거의 나지 않는다. 따라서 OS는
**모든 활성 프로세스의 Working Set 합이 물리 메모리를 넘지 않도록** 다중 프로그래밍 정도를 조절하고,
넘치면 일부 프로세스를 통째로 스왑 아웃해 나머지를 살린다.

`PFF(Page Fault Frequency)` 기법은 같은 목표에 더 직접적으로 접근한다. 폴트율이 상한을 넘으면 프레임을
더 주고 하한 아래로 내려가면 회수한다. 결과 지표만 보고 조절하므로 구현이 간단하다.
...
**스왑 설정**: 스래싱은 대개 "서버 메모리를 넘기는 설정"으로 나타난다. JVM 힙을 물리 메모리에 가깝게
잡으면 GC가 힙 전체를 훑는 순간 스왑이 돌면서 정지 시간이 초 단위로 튄다. 그래서 DB나 JVM 서버에서는
스왑을 끄거나 `vm.swappiness`를 낮추는 경우가 많다. 느리게 사느니 빠르게 실패하고 고치자는 판단이다.

**메모리 지표 읽기**: 프로세스의 RSS(실제 물리 메모리 점유)와 VSZ(가상 주소 공간 크기)는 크게 다를
수 있다. 매핑만 해두고 아직 건드리지 않은 영역은 물리 프레임을 차지하지 않으므로, VSZ가 크다고 메모리를
그만큼 쓰는 것이 아니다. 용량 산정은 RSS 기준으로 한다.
...
- 꼬리 질문: "Demand Paging의 단점은?" → 첫 접근 때마다 페이지 폴트가 나므로 프로그램 시작 직후
  지연이 있고, 메모리가 부족하면 major fault가 폭증해 스래싱으로 이어진다.
...
- 꼬리 질문: "페이지 폴트가 많으면 무조건 문제인가요?" → 아니다. 매핑만 없는 minor fault는 디스크
  접근이 없어 저렴하다. 문제는 디스크를 실제로 읽는 major fault가 지속적으로 많은 경우다.
...
- 꼬리 질문: "리눅스에서 스래싱을 어떻게 확인하나요?" → `vmstat`의 `si`/`so` 컬럼으로 스왑 in/out을
  보고, major fault 수치와 CPU의 `wa`(I/O 대기) 비중을 함께 확인한다.
...
빠르게 유지하는 TLB와 번역 실패를 처리하는 페이지 폴트 경로가 실제 성능을 결정한다.
