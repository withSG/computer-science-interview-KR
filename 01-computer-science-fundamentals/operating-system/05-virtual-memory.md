...
가상 메모리가 없던 시절을 상상해 봅시다. 프로그램은 물리 주소를 직접 썼습니다. 여기서 세 가지 문제가
동시에 터집니다.

**첫째, 프로그램이 물리 메모리보다 크면 못 돌립니다.** 개발자가 직접 프로그램을 조각내 필요한 부분만
올리고 내리는 오버레이(overlay) 기법을 손으로 짜야 했습니다.

**둘째, 프로세스끼리 서로의 메모리를 침범합니다.** 프로그램 A가 실수로 `0x1000`에 쓰면 그게 마침 B의
데이터일 수 있습니다. 버그 하나가 시스템 전체를 무너뜨립니다.

**셋째, 어디에 적재될지 미리 알 수 없습니다.** 같은 프로그램을 두 번 실행하면 두 번째는 다른 주소에
...
후자의 대표 사례가 **Copy-on-Write(CoW)**입니다. `fork()`로 프로세스를 복제할 때 메모리를 실제로
...
Dirty 비트가 실질적인 성능 차이를 냅니다. 읽기만 한 페이지는 디스크 사본과 동일하므로 그냥 버리면 되고
