...
TLB 미스가 나면 페이지 테이블을 한 번이 아니라 단계 수만큼 걸어야 합니다. 이 비용 때문에 TLB가
결정적으로 중요해집니다.
...
접근입니다. 다단계 테이블이면 조회만 여러 번이라 접근 횟수가 몇 배로 늘어납니다.
...
**huge page**(4KB 대신 2MB 페이지)를 쓰는 이유도 같은 맥락입니다. 페이지 하나가 커지면 같은 메모리를
...
프로그램 시작 시 대량의 minor fault는 정상입니다. 문제는 **major fault가 지속적으로 많은 경우**입니다.
이건 스왑이 돌고 있다는 신호입니다. `vmstat`의 `si`/`so`나 `ps -o min_flt,maj_flt`로 확인합니다.
...
역설이 생길 수 있습니다. "언제 들어왔는가"만 볼 뿐 "얼마나 쓸모 있는가"를 반영하지 않기 때문입니다.
LRU는 스택 알고리즘이라는 성질 덕분에 이 현상이 없습니다.
...
**스왑 설정**: 스래싱은 대개 서버 메모리를 넘기는 설정으로 나타납니다. JVM 힙을 물리 메모리에 가깝게
