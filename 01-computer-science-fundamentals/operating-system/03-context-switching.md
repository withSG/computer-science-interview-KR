...
**비유의 한계**: 책상 정리는 눈에 보이지만 컨텍스트 스위칭의 진짜 비용은 저장/복원 자체가 아닙니다.
...
이 둘을 셉니다. 자발적 스위칭이 많으면 I/O나 락 대기가 잦다는 뜻입니다. 비자발적 스위칭이 많으면 CPU를
두고 경쟁이 심하다는 뜻이고, 그래서 튜닝 방향이 완전히 달라집니다.
...
이 매핑 정보를 담은 것이 페이지 테이블이고, CPU는 페이지 테이블 베이스 레지스터로 지금 유효한 매핑을
...
다시 걸어야 합니다. 이 조회는 메모리 접근을 여러 번 유발합니다.

L1/L2 캐시도 마찬가지입니다. 캐시에 남아있던 A의 데이터는 B에게 아무 쓸모가 없습니다. B는 자기 데이터를
메모리에서 다시 끌어와야 하고, 그동안 CPU는 대기합니다.
...
따라 크게 달라지므로 "정확히 몇 사이클"을 외우는 건 의미가 없습니다. **스레드 쪽이 한 자릿수
가량 싸다**는 관계만 기억하면 됩니다. 대신 비용을 두 종류로 나눠 이해하는 편이 훨씬 쓸모 있습니다.
...
마지막 항목을 조금 더 보겠습니다. Java 21의 가상 스레드는 블로킹 I/O를 만나면 캐리어 스레드에서 자신을
...
Java 진영은 요청당 스레드를 쓰고 PHP-FPM은 요청당 프로세스를 씁니다. 이 차이는 취향이 아니라 지금까지 본
...
스위칭을 만들어내는 것이므로 동기화 범위를 줄이거나 lock-free 자료구조로 바꾸는 쪽이 답입니다.
...
프로세스가 메모리에 접근할 때마다 TLB 미스가 발생해 페이지 테이블을 다시 조회해야 합니다. CPU 캐시에
...
컨텍스트 스위칭은 코어 하나로 여러 흐름을 돌리기 위해 내는 세금입니다. 그 세금의 큰 몫은 저장·복원
