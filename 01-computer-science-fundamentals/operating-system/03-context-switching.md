...
> CPU 하나로 여러 작업을 돌리기 위해 OS가 치르는 대가가 무엇이고, 그 대가가 프로세스와 스레드에서 왜 다른지를 TLB와 캐시 수준까지 설명할 수 있게 된다.
...
CPU 코어 하나는 한순간에 명령어 하나만 실행한다. 그런데 지금 이 순간 우리 컴퓨터에서는 수백 개의
프로세스가 "동시에" 돌고 있다. 이 착시를 만드는 유일한 방법은 **아주 빠르게 번갈아 실행하는 것**이다.

문제는 CPU가 상태를 기억하지 못한다는 점이다. 레지스터는 한 벌뿐이고, Program Counter도 하나다.
A를 잠시 멈추고 B를 돌리면 A의 계산 중간값이 B의 값으로 덮인다. 그래서 전환 직전에 A의 상태를
어딘가 안전한 곳으로 옮겨 적고, B의 상태를 되돌려 놓아야 한다. 이 저장·복원 절차가 컨텍스트
스위칭이다.

즉 컨텍스트 스위칭은 우리가 원해서 하는 일이 아니라, **하드웨어 자원이 하나뿐인데 논리적 실행 흐름은
여럿이라서 어쩔 수 없이 치르는 세금**이다. 세금이니 줄이는 게 목표지만, 완전히 안 낼 수는 없다.

**비유**: 책상 하나에서 세 과목을 공부하는 상황이다. 수학을 접고 영어를 펴려면 수학 책과 필기를
치우고 영어 책을 꺼내야 한다. 이 정리 시간 자체는 공부가 아니다. 5분마다 과목을 바꾸면 정리만 하다
하루가 간다.

**비유의 한계**: 책상 정리는 눈에 보이지만, 컨텍스트 스위칭의 진짜 비용은 저장/복원 자체가 아니라
눈에 안 보이는 캐시와 TLB가 식는 데서 나온다. 이 점은 4장에서 다룬다.
...
이 둘을 센다. 자발적 스위칭이 많으면 I/O나 락 대기가 잦다는 뜻이고, 비자발적 스위칭이 많으면 CPU를
두고 경쟁이 심하다는 뜻이라 튜닝 방향이 완전히 달라진다.

**주의**: 시스템 콜을 호출한다고 반드시 컨텍스트 스위칭이 일어나는 것은 아니다. 시스템 콜은
"모드 전환(user → kernel)"이고, 컨텍스트 스위칭은 "실행 주체 교체"다. 같은 프로세스가 커널 모드로
들어갔다 바로 돌아오면 스위칭은 없다. 자세한 구분은
[08-system-call-interrupt.md](./08-system-call-interrupt.md)에서 다룬다.
...
두 단계 사이에 실제로는 더 많은 일이 끼어든다. 프로세스 간 전환일 때의 전체 흐름은 이렇다.
...
마지막 줄이 핵심이다. 스위칭은 "끝났다"고 해서 끝나지 않는다. **B가 다시 원래 속도를 회복하기까지
걸리는 시간**까지가 진짜 비용이다.
...
프로세스마다 가상 주소 공간이 다르다는 사실 하나에서 모든 차이가 나온다.

프로세스 A의 가상 주소 `0x400000`과 프로세스 B의 `0x400000`은 완전히 다른 물리 메모리를 가리킨다.
이 매핑 정보를 담은 것이 페이지 테이블이고, CPU는 페이지 테이블 베이스 레지스터로 "지금 유효한 매핑"을
가리킨다. 프로세스를 바꾸면 이 레지스터를 갈아 끼워야 한다.

문제는 그다음이다. TLB는 "가상 주소 → 물리 주소" 변환 결과를 캐싱해 두는 하드웨어인데, 여기 들어있는
`0x400000 → 물리 0x8A2000` 같은 항목이 프로세스가 바뀐 순간 **거짓말이 된다**. 그대로 두면 B가 A의
메모리를 읽는 대참사가 나므로 무효화해야 한다. 무효화하면 B는 메모리를 건드릴 때마다 페이지 테이블을
다시 걸어야 하고, 이 조회는 메모리 접근을 여러 번 유발한다.

L1/L2 캐시도 마찬가지다. 캐시에 남아있던 A의 데이터는 B에게 아무 쓸모가 없다. B는 자기 데이터를
메모리에서 다시 끌어와야 하고, 이 동안 CPU는 대기한다.

반면 같은 프로세스 안의 스레드끼리는 주소 공간이 동일하다. 페이지 테이블도 그대로, TLB도 그대로,
캐시에 있는 힙 데이터도 여전히 유효하다. 바꿀 것은 레지스터 묶음과 스택 포인터 정도다.

현대 CPU는 여기에 완화책을 뒀다. TLB 항목마다 주소 공간 식별자(x86의 PCID, ARM의 ASID)를 함께
저장하면 프로세스가 바뀌어도 통째로 비울 필요 없이 식별자로 구분할 수 있다. 그래서 "프로세스 전환 =
무조건 TLB 전체 플러시"는 오래된 CPU 기준의 설명이다. 다만 TLB 용량은 한정적이라 여러 프로세스의
항목이 서로를 밀어내는 압박은 여전하다.
...
이 범위는 자릿수 감각으로만 받아들여야 한다. CPU 세대, 캐시 크기, 보안 완화책 적용 여부, 워크로드에
따라 크게 달라지므로 "정확히 몇 사이클"을 외우는 건 의미가 없다. 기억할 것은 **스레드 쪽이 한 자릿수
가량 싸다**는 관계뿐이다. 대신 비용을 두 종류로 나눠 이해하는 편이 훨씬 쓸모 있다.
...
간접 비용이 크다는 사실이 실무 판단을 바꾼다. "스위칭 자체는 마이크로초밖에 안 되는데 뭐가 문제냐"고
생각하기 쉽지만, 캐시 지역성이 깨진 뒤 수십 마이크로초 동안 느려진 채로 도는 것이 진짜 손해다.
CPU 친화도 설정이 효과를 내는 이유도 여기 있다. 같은 코어로 되돌아가면 그 코어의 L1/L2에 내 데이터가
아직 남아있을 확률이 높다.
...
떼어내고 다른 가상 스레드에 자리를 넘긴다. 이 전환은 커널이 아니라 JVM 안에서 일어나므로 페이지 테이블
교체도 TLB 무효화도 없다. "블로킹 코드를 그대로 쓰면서 스위칭 비용은 낮추는" 접근이다.

다만 만능은 아니다. 커널 수준에서 진짜로 막히는 호출(일부 파일 I/O, 네이티브 라이브러리 호출)은
캐리어 스레드를 붙잡아 두므로 이 경우 이득이 사라진다. 도구의 전제 조건을 아는 것이 도구를 아는 것보다
중요하다.
...
비용 구조에서 나온다. 같은 코드로 수만 요청을 처리하는 서버에서는 코드와 힙을 공유하는 이득이 크고,
요청 사이 전환에서 페이지 테이블 교체와 TLB 무효화를 매번 피할 수 있다. PHP-FPM은 반대로 요청마다
상태를 완전히 버리는 격리를 택하고 그 비용을 지불한 쪽이다.

크롬은 같은 저울을 정반대로 기울인 사례다. 탭마다 프로세스를 띄우면 스위칭 비용도 메모리도 늘지만,
탭 하나의 크래시나 취약점이 나머지로 번지지 않는다. 사용자 입장에서는 "탭 하나 때문에 작업 전부
날아감"의 손해가 더 크다고 본 것이다. **스위칭 비용은 언제나 격리 가치와 함께 저울에 오른다.**

### 스위칭이 과도한지 확인하는 법

이론만 알고 측정을 못 하면 소용이 없다. 리눅스 기준으로 순서는 이렇다.
...
쓰고 있다는 신호다. `pidstat -w`에서 `cswch/s`(자발)가 지배적이면 I/O나 락 대기가 원인이고,
`nvcswch/s`(비자발)가 지배적이면 실행 가능한 스레드가 코어 수보다 훨씬 많다는 뜻이다. 전자는 비동기화나
락 범위 축소로, 후자는 스레드 풀 크기 축소로 접근한다.

Java 애플리케이션이라면 스레드 덤프도 같이 본다. `BLOCKED` 상태 스레드가 다수라면 락 경합이 자발적
스위칭을 만들어내고 있는 것이므로, 동기화 범위를 줄이거나 lock-free 자료구조로 바꾸는 쪽이 답이다.
...
- 꼬리 질문: "시스템 콜을 호출하면 항상 컨텍스트 스위칭이 일어나나요?" → 아니다. 시스템 콜은 모드
  전환이고, 같은 프로세스가 커널 작업을 마치고 바로 돌아오면 실행 주체는 바뀌지 않는다.
...
- 꼬리 질문: "요즘 CPU도 프로세스 전환마다 TLB를 전부 비우나요?" → PCID/ASID를 지원하는 CPU는 TLB
  항목에 주소 공간 식별자를 붙여 전체 플러시를 피한다. 다만 TLB 용량 경쟁은 남는다.
...
- 꼬리 질문: "스레드 풀 크기는 어떻게 정하나요?" → CPU 집약 작업은 코어 수 근처, I/O 대기가 많으면
  그보다 크게 잡되 실측이 원칙. 대기 비율이 높을수록 코어 수 대비 배수를 키운다.
...
자체가 아니라 주소 공간이 바뀌면서 캐시와 TLB가 식는 간접 비용에서 나온다.
...
- [06-cpu-scheduling.md](./06-cpu-scheduling.md) - 타임 퀀텀 설정이 스위칭 횟수를 결정한다
- [08-system-call-interrupt.md](./08-system-call-interrupt.md) - 모드 전환과 스위칭의 차이
- [qna-os.md](./qna-os.md) - Q2에 이 주제의 면접 질문이 정리되어 있다
