...
### 핵심 답변
프로세스는 **자원 할당의 단위**이고 스레드는 **실행의 단위**입니다. 프로세스는 독립적인 메모리 공간을 가집니다. 스레드는 같은 프로세스 안에서 Code, Data, Heap 영역을 공유하고 Stack만 독립적으로 가집니다.
...
### 면접관이 주목하는 포인트
- 컨텍스트 스위칭 비용이 왜 차이 나는지 설명
- 메모리 공유에서 생기는 동기화 이슈 짚기
- 실무에서 어느 쪽을 고를지 판단하는 기준
...
### 핵심 답변
컨텍스트 스위칭은 CPU가 실행 중인 프로세스(스레드)를 다른 프로세스(스레드)로 바꾸는 과정입니다. 현재 상태를 저장한 뒤 다음 상태를 복원해 실행을 전환합니다.
...
### 핵심 답변
경쟁 상태는 두 개 이상의 스레드가 공유 자원에 동시에 접근할 때 데이터의 정합성이 깨지는 현상입니다.
...
### 핵심 답변
Stack은 함수 호출과 지역 변수를 저장하는 영역입니다. 컴파일 타임에 크기가 결정되고 자동으로 관리됩니다. Heap은 동적으로 할당되는 객체를 저장하는 영역으로 런타임에 할당되고 GC 또는 수동으로 관리됩니다.
...
### 핵심 답변
가상 메모리는 물리적 메모리 크기와 관계없이 프로세스에게 큰 주소 공간을 제공하는 기술입니다. 디스크를 메모리의 확장으로 사용하므로 물리 메모리보다 큰 프로그램도 실행할 수 있습니다.
...
### 핵심 답변
뮤텍스는 임계 영역 접근을 **1개의 스레드**로 제한하는 동기화 도구입니다. 세마포어는 **지정된 개수의 스레드**까지 동시 접근을 허용합니다.
...
### 핵심 답변
LRU(Least Recently Used)는 가장 오래 사용되지 않은 페이지를 교체합니다. 최근에 사용된 페이지는 가까운 미래에도 사용될 가능성이 높다는 지역성 원리에 기반합니다.
...
### 핵심 답변
DMA는 CPU 개입 없이 **I/O 장치가 메모리에 직접 접근**해 데이터를 전송하는 방식입니다.
...
### 핵심 답변
CPU 스케줄링은 준비 큐에 있는 프로세스 중 **어떤 프로세스에게 CPU를 할당할지** 결정합니다. 선점형(Preemptive)과 비선점형(Non-preemptive)으로 나뉩니다.
...
적정 Time Quantum: CPU 버스트의 80%가 그 안에 끝나는 크기
```
-->
...
→ Windows 스케줄러도 이 개념 기반 (Linux CFS는 다른 계열)
```
-->
...
### 꼬리 질문 대비
- "실제 OS에서는 어떤 알고리즘을 사용하나요?" → Linux는 커널 6.6부터 EEVDF(그 전에는 CFS, 완전 공정 스케줄러), Windows는 Priority Queue 기반 멀티레벨 피드백
...
참고: RabbitMQ, Kafka 같은 분산 MQ는 OS 메시지 큐와 층위가 다름
```
...
### 핵심 답변
파일 시스템은 **OS가 저장 장치에서 데이터를 저장, 구성, 검색하는 방법**을 정의합니다. 파일과 디렉터리를 관리하며 메타데이터와 실제 데이터 영역으로 나뉩니다.
...
| 구조 | 설명 | 장단점 |
|------|------|--------|
| 1단계 (Single Level) | 모든 파일이 하나의 디렉터리 | 단순하지만 이름 충돌 |
| 2단계 (Two Level) | 사용자별 디렉터리 | 사용자 간 격리 |
| 트리 구조 (Tree) | 계층적 디렉터리 (현재 표준) | 체계적, 경로 표현 |
| 그래프 구조 (Graph) | 공유 디렉터리 허용 (심볼릭 링크) | 유연하지만 순환 가능 |
...
### 면접관이 주목하는 포인트
- i-node와 디렉터리의 관계 (파일명은 디렉터리에, 속성은 i-node에)
...
- [ ] 프로세스 vs 스레드 차이를 말할 수 있나
- [ ] 컨텍스트 스위칭 비용 설명 가능
- [ ] 데드락 4가지 조건 암기
- [ ] 경쟁 상태 해결 방법 3가지 이상 알기
- [ ] Stack vs Heap 차이 구분
- [ ] 가상 메모리가 왜 필요한지 떠올릴 수 있나
...
- [ ] 파일 시스템 접근 방법 3가지와 디렉터리 구조 4가지 이해
- [ ] 페이징 vs 세그멘테이션 차이와 단편화 유형을 짚을 수 있나
