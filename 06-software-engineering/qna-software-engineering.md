...
### 핵심 답변
애자일은 개발 방법론의 하나로, **짧은 주기의 반복적 개발**을 통해 변화에 빠르게 대응합니다.
...
### 핵심 답변
**클린 코드**는 처음부터 읽기 쉽고 유지보수하기 쉽게 코드를 작성하는 일입니다. **리팩토링**은 이미 작성된 코드가 대상입니다. 동작은 그대로 두고 내부 구조를 개선합니다.
...
### 꼬리 질문 대비
- "함수형 프로그래밍이 모든 상황에 적합한가?" → 아닙니다. 복잡한 상태 관리, 엔터프라이즈 도메인 모델링에는 OOP가 적합. 실무는 OOP+FP 혼합 사용
