...
### 핵심 답변
TDD는 **테스트를 먼저 작성하고 코드를 구현**하는 개발 방법을 뜻합니다.
...
### 핵심 답변
코드 리뷰는 협업 프로세스이고 목적은 **코드 품질 향상과 지식 공유**입니다.
...
### 핵심 답변
테스트 코드는 **코드의 정확성을 보장**하고 **변경에 대한 자신감**을 줍니다. **문서 역할**도 합니다.
...
- "테스트 코드 작성이 개발 속도를 늦추지 않나요?"
  → 초기에는 시간이 더 걸리지만 버그 수정과 수동 테스트 시간을 줄여 전체적으로는 빨라짐
...
### 핵심 답변
테스트 커버리지는 **테스트가 코드의 얼마나 많은 부분을 실행하는지**를 측정하는 지표입니다. 100% 자체가 목표는 아닙니다. **핵심 로직에 대한 의미 있는 테스트**를 봐야 합니다.
...
## Q11. 클린 코드(Clean Code)의 원칙과 리팩터링의 차이는? ⭐⭐

<details>
<summary>답변 보기</summary>

### 핵심 답변
**클린 코드**는 처음부터 읽기 쉽고 유지보수하기 쉽게 코드를 작성하는 일입니다. **리팩터링**은 이미 작성된 코드가 대상입니다. 동작은 그대로 두고 내부 구조를 개선합니다.
...
### 리팩터링 대상 코드 냄새 (Code Smell)

<!-- diagram:se-qna-software-engineering-4 -->
![리팩터링 대상 코드 냄새](../assets/diagrams/se-qna-software-engineering-4.svg)
...
### 핵심 답변
함수형 프로그래밍은 **순수 함수와 불변성을 중심**으로 부수 효과(Side Effect)를 최소화하는 프로그래밍 패러다임입니다.
...
### 꼬리 질문 대비
- "함수형 프로그래밍이 모든 상황에 적합한가?" → 아닙니다. 복잡한 상태 관리나 엔터프라이즈 도메인 모델링에는 OOP가 적합합니다. 실무에서는 OOP+FP를 섞어 씁니다
...
### 핵심 답변
MSA는 아키텍처 패턴입니다. **하나의 애플리케이션을 독립적으로 배포 가능한 작은 서비스들의 집합**으로 구성합니다. 각 서비스는 특정 비즈니스 기능을 담당하고 API로 통신합니다.
