...
- [ ] 폭포수와 애자일의 차이를 "피드백 주기" 관점에서 설명할 수 있다
- [ ] 스크럼과 칸반을 비교하고 상황에 맞게 고를 수 있다
...
- [qna-software-engineering.md](./qna-software-engineering.md) - 소프트웨어 공학 면접 질문 모음

---

## 핵심 키워드

`Agile` `Scrum` `Kanban` `Sprint` `브룩스의 법칙` `Code Review`
`TDD` `Red-Green-Refactor` `Unit Test` `Integration Test` `E2E Test`
`테스트 피라미드` `FIRST` `테스트 더블` `커버리지`
`Clean Code` `Refactoring` `코드 스멜` `KISS` `YAGNI` `DRY`
`Monolith` `MSA` `Saga` `Strangler Fig` `Bounded Context`
`CI/CD` `DevOps`
