...
줄 수는 팀과 코드 성격에 따라 달라지므로 정확한 임계값이 있는 것은 아닙니다. 리뷰어의 집중력에 한계가 있어서 어느 지점을 넘으면 결함 발견율이 급격히 떨어지고 승인 도장만 남습니다. **큰 PR은 리뷰 시간을 늘리는 게 아니라 리뷰 자체를 없앱니다.** 기능을 쪼개 여러 PR로 나누고, 리팩터링과 기능 추가를 한 PR에 섞지 않는 것이 가장 효과가 큽니다.
