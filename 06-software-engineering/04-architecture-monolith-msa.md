...
> 모놀리식이 왜 여전히 유효한 선택인지, MSA가 무엇을 사고 무엇을 대가로 내놓는지, 서비스 경계를 어떤 기준으로 긋는지, 그리고 옮겨가야 한다면 어떤 순서로 옮기는지 설명할 수 있게 됩니다.
...
3번이 분산 시스템에서 가장 풀기 어려운 난제입니다. **타임아웃은 "실패"가 아니라 "모름"입니다.** 재고가 이미 차감됐을 수도 있고 아닐 수도 있습니다. 재시도하면 이중 차감이 날 수 있고 재시도하지 않으면 유실이 날 수 있습니다. 해결하려면 API마다 **멱등성(idempotency)**을 넣어야 합니다. 요청마다 고유 키를 붙여 서버가 중복을 걸러내고 같은 요청을 여러 번 보내도 결과가 같게 만드는 설계입니다. 모놀리식에서는 존재하지 않던 작업입니다.
...
**롤백과 보상은 다릅니다.** DB 롤백은 흔적을 남기지 않지만 보상 트랜잭션은 "결제했다가 환불했다"는 사실이 그대로 남습니다. 그리고 그 사이의 짧은 시간 동안 다른 사용자에게는 결제가 성공한 것으로 보입니다. **Saga는 여기서 격리성(Isolation)을 포기합니다.** 이 중간 상태가 업무적으로 허용되는지 반드시 검토해야 합니다.
...
전면 재작성(빅뱅)은 실패율이 높습니다. 몇 개월 동안 기존 시스템의 기능을 다시 만드는 동안 기존 시스템도 계속 변하기 때문에 목표점이 도망갑니다. 대신 쓰는 것이 **스트랭글러 피그(Strangler Fig)** 패턴입니다. 이름은 나무를 감고 자라 결국 원래 나무를 대체하는 무화과나무에서 왔습니다. **기존 시스템을 살려둔 채 기능을 하나씩 새 시스템으로 옮기고 마지막에 껍데기를 걷어내는** 방식입니다.
