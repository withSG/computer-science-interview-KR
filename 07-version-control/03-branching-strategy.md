...
- **브랜치가 몇 주씩 살아남습니다.** [02번 문서](./02-merge-rebase-conflict.md)에서 봤듯 충돌 비용은 브랜치 수명에 비례해 커지고, 결국 통합 지옥이 됩니다.
...
`release/*` 브랜치의 존재 이유가 핵심입니다. 출시 준비(QA, 버그 수정, 문서 정리)에 며칠이 걸리는 동안 **다른 개발자는 다음 릴리스 기능을 `develop`에 계속 넣을 수 있습니다.** 출시 작업과 신규 개발이 서로를 막지 않습니다.
...
**왜 문제인가**: 매일 배포하는 팀에게 `release/*`는 하는 일이 없는데, 브랜치를 유지하는 비용은 그대로 듭니다. 3명뿐이라 `develop`에 모이는 변경도 적어 통합 지점으로서의 의미가 약합니다. 실제로 벌어지는 일은 **`develop`과 `main`이 계속 어긋나고, 역방향 머지를 잊어 핫픽스가 되살아나는 회귀 버그**입니다. 관리에 쓰는 시간이 얻는 안정성보다 큽니다.
...
1. **`git log --oneline`만으로 히스토리가 읽힙니다.** 타입 접두어가 있어 훑기만 해도 성격이 구분된다
2. **자동화가 붙습니다.** 커밋 타입을 읽어 CHANGELOG를 만들거나, `feat`은 마이너 버전, `fix`는 패치 버전으로 올리는 시맨틱 버저닝 자동화가 가능해진다
3. **본문에 "왜"를 적는 자리가 생깁니다.** 무엇을 바꿨는지는 diff가 이미 말해줍니다. diff가 절대 말해주지 못하는 것이 **왜 그렇게 했는가**이고, 그것만이 커밋 메시지의 고유한 가치다
...
- **브랜치 보호 규칙이 전략보다 먼저입니다.** 어떤 전략을 쓰든 `main`에 대해 직접 push 금지, PR 필수, CI 통과 필수, 최소 1인 승인, force push 금지를 켜 두는 것이 사고를 가장 많이 막습니다. GitHub의 branch protection rule, GitLab의 protected branch가 그 기능입니다.
- **브랜치 이름에 이슈 번호를 넣는 팀이 많습니다.** `feat/142-refresh-token`처럼 두면 브랜치만 보고 맥락을 찾을 수 있고, 이슈 트래커와 자동 연동도 붙습니다.
...
- **전략은 팀이 바뀌면 같이 바뀝니다.** 초기 스타트업이 GitHub Flow로 시작했다가 고객사별 버전을 지원하게 되면서 릴리스 브랜치를 도입하는 식의 이동은 자연스럽습니다. 중요한 것은 지금 쓰는 전략이 **어떤 전제 위에 서 있는지 팀이 알고 있는 것**입니다.
