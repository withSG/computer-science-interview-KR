...
`git pull`도 같은 선택의 대상입니다. 기본 `git pull`은 fetch + merge입니다. 로컬에 커밋이 없다면 fast-forward로 조용히 끝납니다. 하지만 내 커밋을 하나라도 쌓아 둔 상태에서 pull하면 그때마다 `Merge branch 'main' of ...` 같은 병합 커밋이 생깁니다. 내용상 아무 결정도 담고 있지 않은 커밋이라 히스토리만 지저분해집니다.
