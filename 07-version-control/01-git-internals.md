...
# 저장된 위치: 앞 2자리가 디렉터리, 나머지 38자리가 파일명
...
> 이 표의 핵심은 **"객체를 만드는 명령"과 "포인터만 옮기는 명령"이 나뉜다**는 것입니다. `add`/`commit`/`merge`는 데이터를 새로 쓰고, `switch`/`reset`/`restore`는 이미 있는 데이터를 가리키는 방향만 바꿉니다. 후자는 커밋된 데이터를 지우는 게 아니므로 대부분 되돌릴 수 있습니다.
