...
모든 노드가 같은 포트를 엽니다. 어느 노드로 들어와도 Service로 전달됩니다. 단, 포트 범위가 제한적이고(기본 30000~32767), 노드 IP를 외부에 노출해야 하며 노드가 늘고 줄 때 클라이언트가 알 방법이 없습니다. 그래서 프로덕션 직접 노출용으로는 잘 쓰지 않습니다. 다만 LoadBalancer 타입 내부에서 NodePort가 실제로 쓰이므로 개념 자체는 알아야 합니다.
