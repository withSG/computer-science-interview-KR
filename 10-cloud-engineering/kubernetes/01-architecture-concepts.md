...
첫째, **루프는 끝나지 않습니다.** 배포가 "완료"되는 게 아니라 계속 유지됩니다. 누군가 `kubectl delete pod`으로 Pod를 지워도 몇 초 안에 새로 생깁니다. 이게 자가 치유(Self-healing)의 정체입니다. 특별한 장애 감지 모듈이 있는 게 아니라, 그냥 루프가 계속 돌 뿐입니다.

둘째, 쿠버네티스는 **이벤트가 아니라 상태를 봅니다.** "Pod가 죽었다"는 이벤트를 놓쳐도 상관없습니다. 다음 루프에서 "지금 2개뿐이네"라고 현재 상태를 다시 읽기 때문입니다. 이 설계(level-triggered) 덕분에 컨트롤러가 잠깐 죽었다 살아나도 복구됩니다. 이벤트 하나를 놓치면 영원히 어긋나는 시스템(edge-triggered)과의 결정적 차이입니다.
