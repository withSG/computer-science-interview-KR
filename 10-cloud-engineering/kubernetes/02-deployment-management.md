...
> Deployment가 Pod를 어떤 계층으로 관리하는지, 무중단 배포와 롤백이 실제로 어떤 순서로 일어나는지, 그리고 상태가 있는 앱에는 왜 StatefulSet이 필요한지 설명할 수 있게 됩니다.
...
Recreate는 구버전과 신버전이 동시에 떠 있으면 안 되는 경우(예: 호환되지 않는 DB 스키마를 쓰는 경우)에만 씁니다.

롤링 업데이트의 속도는 두 값이 결정합니다.
