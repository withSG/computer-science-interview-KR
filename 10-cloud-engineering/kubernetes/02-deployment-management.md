...
> Deployment가 Pod를 어떤 계층으로 관리하는지, 무중단 배포와 롤백이 실제로 어떤 순서로 일어나는지, 그리고 상태가 있는 앱에는 왜 StatefulSet이 필요한지 설명할 수 있게 된다.
...
Pod를 YAML로 직접 정의해 배포할 수는 있다. 하지만 배포한 뒤 상황을 상상해 보자.

- Pod가 들어 있던 노드가 죽으면 **그대로 사라진다.** 다시 만들어 주는 주체가 없다.
- 복제본 5개를 원하면 YAML을 5번 쓰거나 이름만 바꿔 5번 apply해야 한다.
- 새 버전을 올리려면 기존 Pod를 지우고 새 Pod를 만들어야 하는데, 그 사이는 다운타임이다.
- 새 버전에 문제가 있어 되돌리려면 이전 이미지 태그를 기억해 다시 써야 한다.

[01번 문서](./01-architecture-concepts.md)에서 본 조정 루프는 **감시하는 컨트롤러가 있어야** 작동한다. Pod를 직접 만들면 그 Pod의 spec을 감시하는 컨트롤러가 없다. 그래서 쿠버네티스는 Pod 위에 목적별 워크로드 리소스를 얹는다.
...
선택 기준은 두 질문으로 갈린다. **"이 워크로드는 끝나는가, 계속 도는가?"** 와 **"복제본끼리 구별되어야 하는가?"** 이다.
...
가장 흔한 질문이 "ReplicaSet은 왜 있나, Deployment가 바로 Pod를 만들면 안 되나"이다. 답은 **롤백** 때문이다.
...
보관할 이전 ReplicaSet 개수는 `spec.revisionHistoryLimit`으로 정한다(기본 10). 0으로 두면 롤백이 불가능해지므로 주의한다.

### 롤링 업데이트가 진행되는 순서

`spec.strategy.type`은 기본이 `RollingUpdate`이고, `Recreate`도 있다.
...
Recreate는 구버전과 신버전이 동시에 떠 있으면 안 되는 경우(예: 호환되지 않는 DB 스키마를 쓰는 경우)에만 쓴다.

롤링 업데이트의 속도는 두 값이 결정한다.
...
`progressDeadlineSeconds` 안에 진전이 없으면 Deployment는 `Progressing=False`로 표시된다. **자동으로 롤백되지는 않는다.** CI 파이프라인에서 `rollout status`의 종료 코드를 보고 실패 시 `rollout undo`를 실행하도록 엮어 두는 것이 실무 패턴이다.
...
Deployment의 Pod는 이름이 `web-9a2-k2p`처럼 무작위이고, 죽으면 완전히 다른 이름으로 다시 태어난다. 모두 동등하기 때문에 로드밸런서가 아무 Pod에나 요청을 보내도 상관없다. 이 성질이 **상태가 있는 앱에서는 정확히 반대로 문제가 된다.**

MySQL 복제 구성을 생각해 보자. `mysql-0`이 프라이머리이고 `mysql-1`, `mysql-2`가 레플리카다. 여기서 필요한 것은 세 가지다.

1. **각 Pod가 자기 데이터에 계속 붙어야 한다.** `mysql-1`이 재시작했는데 `mysql-2`의 디스크를 잡으면 데이터가 뒤섞인다.
2. **각 Pod를 이름으로 지목할 수 있어야 한다.** 레플리카는 "프라이머리 주소"를 알아야 복제를 시작한다. 그 주소가 재시작마다 바뀌면 안 된다.
3. **순서가 보장되어야 한다.** 프라이머리가 먼저 떠 있어야 레플리카가 붙을 수 있다.

StatefulSet은 이 세 가지를 보장한다.
...
**순서 보장.** 기본값(`podManagementPolicy: OrderedReady`)에서는 `mysql-0`이 Ready가 되어야 `mysql-1`을 만들고, 축소할 때는 큰 번호부터 지운다. 순서가 필요 없고 빠른 기동이 중요하면 `Parallel`로 바꿀 수 있다.

**PVC 템플릿.** `volumeClaimTemplates`에 적어 두면 Pod마다 PVC가 하나씩 자동 생성되고 이름이 `<템플릿명>-<Pod명>`으로 고정된다. Pod가 죽었다 살아나면 같은 PVC에 다시 붙는다.
...
**StatefulSet을 지워도 PVC는 남는다.** 이건 버그가 아니라 안전장치다. 실수로 지운 DB의 데이터까지 함께 날아가면 복구가 불가능하기 때문이다. 정말 정리하려면 PVC를 직접 삭제해야 한다. 최신 버전에는 이 동작을 제어하는 `persistentVolumeClaimRetentionPolicy` 필드가 있으니 클러스터 버전 문서를 확인하고 쓴다.

**StatefulSet이 있다고 클러스터링이 되는 건 아니다.** StatefulSet은 이름과 순서와 디스크만 보장한다. "누가 프라이머리인지" 정하고 페일오버하는 로직은 애플리케이션이나 오퍼레이터의 몫이다. 그래서 실무에서는 StatefulSet을 직접 쓰기보다 해당 DB의 전용 오퍼레이터를 도입하거나, 아예 관리형 DB(RDS 등)를 쓰는 판단을 자주 한다.
...
"모든 노드에서 돌아야 하는 에이전트"를 위한 리소스다. 복제본 수를 지정하지 않는다. **노드가 추가되면 자동으로 하나 늘고, 노드가 빠지면 함께 사라진다.**

전형적인 용도는 로그 수집기(Fluent Bit), 노드 메트릭 수집기(node-exporter), 스토리지·네트워크 플러그인 에이전트다. kube-proxy 자체도 대개 DaemonSet으로 돌아간다.

컨트롤 플레인 노드에도 배치하려면 해당 노드의 taint를 감당할 `tolerations`가 필요하다. 특정 노드에만 두고 싶으면 `nodeSelector`로 좁힌다.

### Job — 끝나는 작업

Deployment의 Pod는 끝나면 안 되지만, 배치 작업은 끝나야 정상이다. Job은 **정해진 횟수만큼 성공적으로 완료될 때까지** Pod를 실행한다.
...
`restartPolicy: Always`는 Job에서 쓸 수 없다. 끝나야 하는 작업을 계속 되살리는 것은 모순이기 때문이다. `ttlSecondsAfterFinished`를 빼먹으면 완료된 Job과 Pod가 계속 쌓여 `kubectl get pods` 출력이 지저분해진다.
...
`concurrencyPolicy`가 운영상 중요하다. 기본값 `Allow`는 이전 작업이 안 끝났어도 새 작업을 만든다. 3시간 걸리는 배치를 1시간마다 돌리면 작업이 겹쳐 쌓이고 DB에 락 경합이 생긴다. 중복 실행이 위험한 작업은 `Forbid`, 항상 최신 것만 필요하면 `Replace`를 쓴다.

시간대도 함정이다. 기본적으로 컨트롤 플레인의 시간대를 따르므로, UTC 기준 클러스터에서 `"0 3 * * *"`은 한국 시간 정오다. 최신 버전에는 `timeZone` 필드가 있으니 지원 여부를 확인하고, 없다면 크론 표현식을 UTC로 환산해 적어야 한다.
...
`requests`는 **스케줄링에만** 쓰인다. `requests.cpu: 500m`인 Pod는 남은 CPU가 500m 이상인 노드에만 배치된다. 실제로 그만큼 쓰는지는 무관하다.

`limits`는 컨테이너가 뜬 뒤 cgroup으로 강제된다. 그런데 CPU와 메모리의 처리 방식이 다르다.

| 자원 | limit 초과 시 | 결과 |
|---|---|---|
| CPU | 스로틀링(throttling) | 죽지 않고 느려진다. 응답 지연으로 나타남 |
| 메모리 | OOMKill | 컨테이너가 즉시 종료되고 재시작된다 (Exit Code 137) |

CPU는 나눠 쓸 수 있는 자원이라 잘라 쓰면 되지만, 메모리는 이미 할당한 것을 뺏을 수 없기 때문이다. 이 차이 때문에 **메모리 limit은 CPU limit보다 훨씬 신중하게 잡아야 한다.**
...
노드 메모리가 부족해지면 kubelet은 Pod를 골라 축출(evict)한다. 이때 기준이 QoS 클래스이고, 이 클래스는 사용자가 지정하는 게 아니라 **requests/limits 설정에서 자동으로 결정된다.**
...
그래서 **결제 API처럼 절대 죽으면 안 되는 워크로드는 Guaranteed로, 배치 작업처럼 밀려도 되는 것은 Burstable이나 BestEffort로** 두는 것이 기본 전략이다.
...
"limits를 안 걸면 자원을 마음껏 써서 성능이 좋다"고 생각하기 쉽다. 실제로는 정반대의 위험이 있다. limits 없는 Pod 하나가 노드 메모리를 다 먹으면 **같은 노드의 다른 Pod들이 함께 죽는다.** 반대로 지나치게 낮은 limits는 조용한 스로틀링을 만들어 원인 모를 지연으로 나타난다. 부하 테스트로 실제 사용량을 측정한 뒤 여유를 얹는 것이 정석이다.

JVM 앱이라면 하나 더 주의할 것이 있다. 컨테이너 메모리 limit과 힙 크기를 함께 봐야 한다. 힙 외에도 메타스페이스, 스레드 스택, 네이티브 버퍼가 메모리를 쓰기 때문에, 힙 최대치를 limit과 같게 잡으면 거의 확실히 OOMKilled를 만난다.
...
HorizontalPodAutoscaler는 메트릭을 주기적으로 읽어 목표치와 비교하고 복제본 수를 계산한다. 계산식의 골자는 단순하다.
...
**metrics-server가 설치되어 있어야 한다.** 없으면 `kubectl top`도 안 되고 HPA는 메트릭을 못 읽어 `<unknown>` 상태에 머문다.

**CPU 사용률은 requests 대비 비율이다.** `averageUtilization: 50`은 "노드 CPU의 50%"가 아니라 "requests.cpu의 50%"다. 그래서 **requests가 설정되어 있지 않으면 사용률 기반 HPA는 아예 동작하지 않는다.** 이것이 requests를 반드시 잡아야 하는 또 하나의 이유다.

**축소는 확대보다 훨씬 보수적으로 일어난다.** 트래픽이 잠깐 출렁일 때마다 Pod를 줄였다 늘렸다 하면(플래핑) 서비스가 불안정해지기 때문에, 축소 판단에는 안정화 대기 시간이 적용된다. `behavior` 필드로 확대·축소 속도와 대기 시간을 따로 조절할 수 있다.
...
**배포 전략은 Deployment 하나로 끝나지 않는다.** 롤링 업데이트만으로는 "새 버전에 10% 트래픽만 흘려 보고 판단"하는 카나리 배포가 어렵다. 실무에서는 Argo Rollouts나 Flagger 같은 도구를 얹거나, 서비스 메시의 트래픽 분할 기능을 쓴다. 관련 흐름은 [qna-cicd.md](../devops-cicd/qna-cicd.md)에 이어진다.

**PodDisruptionBudget을 함께 건다.** 노드 업그레이드나 `kubectl drain`처럼 운영자가 일으키는 중단에서 "최소 몇 개는 살아 있어야 한다"를 선언해 두는 리소스다. 이게 없으면 노드 교체 작업 한 번에 서비스가 전부 내려앉을 수 있다.

**리소스 설정은 관측 없이 정할 수 없다.** 처음에는 넉넉하게 잡고, Prometheus로 실제 사용량 분포를 본 뒤 조정하는 것이 순서다. 관련 내용은 [qna-monitoring.md](../monitoring-observability/qna-monitoring.md)를 참고한다.

**네임스페이스에 ResourceQuota와 LimitRange를 건다.** 팀별로 쓸 수 있는 총량(ResourceQuota)과 개별 컨테이너의 기본값·상한(LimitRange)을 정해 두면, requests를 안 적은 Pod가 클러스터를 잠식하는 사고를 막을 수 있다.
...
- 꼬리 질문: "그럼 이전 ReplicaSet은 계속 쌓이나요?" → `revisionHistoryLimit`(기본 10)만큼만 남는다. 0으로 두면 롤백이 불가능해진다.
...
- 꼬리 질문: "StatefulSet을 지우면 데이터는요?" → PVC는 기본적으로 남는다. 실수 삭제로부터 데이터를 보호하기 위한 의도된 동작이다.
...
- 꼬리 질문: "requests를 아예 안 적으면 어떻게 되나요?" → QoS가 BestEffort가 되어 노드 압박 시 가장 먼저 축출된다. 또 사용률 기반 HPA도 동작하지 않는다.
...
- 꼬리 질문: "노드는 있는데도 Pending이면?" → taint/toleration 불일치, nodeSelector·affinity 조건, 바인딩되지 않은 PVC, ResourceQuota 초과를 확인한다.
...
Deployment는 "동등한 복제본을 무중단으로 교체"하는 데 최적화되어 있고, 복제본이 서로 구별되어야 하거나(StatefulSet) 노드마다 하나씩 필요하거나(DaemonSet) 끝나야 하는(Job/CronJob) 워크로드는 각각 다른 리소스를 써야 하며, 이 모든 것의 안정성은 결국 requests/limits를 얼마나 성실히 잡았는지에 달려 있다.
