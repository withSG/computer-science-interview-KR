...
컨테이너는 아파트입니다. 건물 한 채의 설비를 공유하되 각 세대는 서로 독립해서 삽니다. 입주가 빠르고 밀도가 높습니다.

> **비유의 한계**: 아파트의 벽은 물리적으로 존재합니다. 컨테이너의 벽은 **커널이 만들어 주는 논리적 구획**일 뿐입니다.
...
- PID 1은 리눅스에서 특별합니다. 좀비 프로세스를 거둬 가는 책임이 있고, 기본 시그널 핸들러가 없습니다.
  앱을 PID 1로 그냥 띄우면 SIGTERM을 무시하므로 `docker stop`이 10초 기다렸다 SIGKILL로 죽입니다.
...
- **capabilities**: 리눅스 root 권한을 잘게 쪼갠 것. Docker는 기본값으로 위험한 권한을 대부분 떼고 시작합니다.
  더 조이려면 `--cap-drop=ALL` 후 필요한 것만 `--cap-add`로 되돌립니다
- **seccomp**: 프로세스가 호출할 수 있는 시스템 콜 목록을 화이트리스트로 제한합니다.
  커널 공격 표면을 직접 줄이는 가장 효과가 큰 수단입니다
...
현실에서 이 둘은 대립하지 않습니다. AWS의 관리형 쿠버네티스 워커 노드는 EC2 인스턴스, 즉 **VM**이고,
...
사내 서비스만 돌리는 보통의 회사라면 여기까지 갈 일은 드뭅니다. 표준 컨테이너에
...
이런 과금 모델을 기술적으로 가능하게 만들었습니다. 이 정도만 기억하면 됩니다.
...
커널·CPU에서 그대로 실행돼야 한다**는 뜻입니다. 그래서 커널 계열뿐 아니라 아키텍처도 일치해야 합니다.
...
**대응**: 당장은 해당 컨테이너를 재생성하며 `--cpus`, `--memory`를 지정합니다.
...
자기 커널을 부팅합니다. 그래서 격리는 강하지만, 부팅이 느리고 이미지가 큽니다. 컨테이너는 호스트 커널 하나를
공유하면서 namespace로 보이는 자원을, cgroup으로 쓸 수 있는 양을 나눈 프로세스일 뿐입니다. 그래서 시작이 즉시고
...
A. namespace는 프로세스가 보는 자원의 범위를 나눕니다. PID, 마운트, 네트워크, UTS, IPC, User 등이 있습니다.
...
특정 권한이 필요하다면 `--cap-add`로 그 하나만 주는 것이 원칙입니다.
기본은 `--cap-drop=ALL`에 non-root 실행을 더하는 방향입니다.
...
namespace로 시야를, cgroup으로 사용량을 나눠 준 프로세스입니다. 속도·크기·밀도·보안 특성과 "쓰면 안 되는 경우"는 전부 이 커널 공유 여부에서 갈라져 나옵니다.
