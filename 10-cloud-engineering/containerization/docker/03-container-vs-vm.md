...
틀린 말은 아니지만 **결과만 말한 것**입니다. 면접관이 듣고 싶은 것은 원인입니다.

가볍다는 결과는 하나의 설계 결정에서 파생됩니다. **컨테이너는 자기 커널을 갖지 않는다.**
...
컨테이너에는 부팅이라는 단계 자체가 없습니다. `docker run`은 그냥 **호스트에서 프로세스를 하나 실행하는 것**입니다.
...
VM은 단독주택입니다. 집마다 보일러와 상수도 배관을 따로 갖습니다. 짓는 비용이 크지만 옆집과 완전히 분리됩니다.
...
**같은 프로세스에 번호가 두 개 붙는 것**입니다. 이 사실이 실무에서 두 가지 결과를 낳습니다.
...
- 호스트에서 `ps`로 보면 컨테이너 프로세스가 다 보입니다. **격리는 안에서 밖을 못 보는 방향으로만 작동한다**
...
NAT 계층이 빠져 성능이 조금 좋아집니다. 격리는 전부 아니면 전무가 아니라 **항목별로 조절되는 것**입니다.
...
같은 호스트의 다른 컨테이너가 죽습니다. 이걸 막는 것이 cgroup(control group)입니다.
...
핵심은 **"cgroup으로 제한했다"와 "앱이 그 제한을 안다"는 별개**라는 것입니다.
...
namespace와 cgroup만으로는 "컨테이너 안의 root"를 안전하게 만들 수 없습니다.
...
반대로 `--privileged`는 **이 방어선을 전부 해제한다.** 컨테이너 안 root가 사실상 호스트 root가 됩니다.
"권한 문제 같으니 일단 privileged로 돌려 보자"는 디버깅 습관이 그대로 운영에 남는 사고가 정말 흔합니다.
필요한 권한 한 개만 `--cap-add`로 주는 것이 옳습니다.
...
호스트 커널 전체에 영향을 줍니다. 컨테이너별로 다른 값을 가질 수 없습니다.
`sysctl` 중 일부(`net.*` 등)는 namespace로 분리되지만 대부분은 호스트 공유입니다.
컨테이너마다 다른 커널 튜닝이 필요하다면 이미 컨테이너의 전제를 벗어난 것입니다.

### 신뢰할 수 없는 코드를 실행할 때

사용자가 업로드한 임의의 코드를 실행하는 플랫폼(온라인 코드 실행기, 채점 서버, 서버리스 멀티테넌시)을
평범한 컨테이너로만 격리하면 위험합니다. 커널 취약점 하나가 호스트 전체와 같은 호스트의 다른 테넌트로 번집니다.
...
기술적 우열의 문제가 아니라 요구사항의 문제이므로, 이럴 때는 전용 인스턴스나 VM 기반 설계로 갑니다.

### 상태를 통째로 들고 이동해야 할 때

실행 중 메모리 상태까지 스냅샷 떠서 다른 호스트로 옮기는 라이브 마이그레이션은 VM 쪽이 훨씬 성숙합니다.
컨테이너는 애초에 **"죽이고 다시 만든다"** 를 전제로 설계됐습니다. 그 전제가 성립하지 않는 레거시 애플리케이션은
...
- **Firecracker**: AWS가 만든 오픈소스 VMM(가상 머신 모니터)으로, 불필요한 장치 에뮬레이션을 걷어내
  microVM을 매우 빠르게 만듭니다. AWS Lambda와 Fargate의 격리 기반이다
...
소켓 하나가 곧 호스트 권한이 됩니다. 대안은 `--privileged`가 필요한 DinD를 쓰거나,
데몬 없이 이미지를 만드는 빌더(BuildKit rootless, Kaniko 등)를 쓰는 것입니다.
