...
코드는 `git push`로 똑같이 옮겼습니다. 그런데 서버에서만 날짜가 9시간 밀리고 한글 파일명이 깨집니다.
네이티브 모듈도 로드되지 않습니다. **옮긴 것은 코드뿐이고 코드가 기대하던 환경은 옮기지 않았기 때문입니다.**

Docker 이전에도 이 문제를 풀려는 시도는 있었습니다. 위키에 "서버 세팅 절차"를 적어 두는 방식은 문서와 실제가
금방 어긋났습니다. `setup.sh` 같은 셸 스크립트는 이미 뭔가 깔린 서버에서 돌리면 결과가 달라졌습니다(비멱등).
VM 이미지로 환경째 복제하는 방식은 확실했지만, 이미지가 수 GB에 부팅이 수십 초라서 코드 한 줄을 고칠 때마다 이미지를 다시 구워야 했습니다.

Docker는 마지막 아이디어(환경째로 옮긴다)를 가져옵니다. 다만 **OS 커널은 복제하지 않고 호스트 것을 빌려 씁니다.**
...
컨테이너가 파일을 수정하면 그 파일만 쓰기 레이어로 복사된 뒤 수정됩니다(Copy-on-Write).
...
이미지가 통짜 파일 하나였다고 해 봅시다. 코드 한 줄만 고쳐도 처음부터 다시 만들어야 하고, push할 때마다
전체 용량을 전송해야 합니다. 같은 베이스를 쓰는 이미지 10개라면 디스크에 베이스가 10벌 쌓입니다.
레이어로 쪼개면 **바뀐 레이어만 다시 만들고 바뀐 레이어만 전송하며 같은 레이어는 한 벌만 저장**합니다.
...
upper 것을 고칩니다(copy-up). 그래서 **1GB짜리 로그 파일 한 줄을 고치면 1GB가 복사됩니다.**
DB 데이터 파일처럼 계속 쓰는 대상은 컨테이너 레이어에 두면 안 됩니다. 이유가 바로 이겁니다.
...
Node, Java도 원리는 같습니다. `package.json`과 `package-lock.json`만 먼저 복사해 `npm ci`를 돌립니다.
...
바뀐 파일만 증분 전송하지만, `.git`이나 `node_modules`가 컨텍스트에 그대로 남아 있으면 전송도 느리고 `COPY . .`의 체크섬이
매번 달라져 캐시가 무의미해지는 것은 마찬가지입니다.
...
`FROM`을 여러 번 쓰고 마지막 스테이지로 **산출물만** 복사하면 됩니다.
...
`docker build --target build .`처럼 중간 스테이지만 빌드할 수도 있습니다.
그래서 테스트 전용 스테이지를 따로 두고 CI에서만 쓰는 패턴도 흔합니다.
...
**원인**: 종료 코드 137은 SIGKILL(128+9)이고, `OOMKilled=true`면 메모리 한도 초과로 커널이 죽인 경우입니다.
...
**대응**: 즉시 조치는 `docker image prune -a --filter "until=168h"`와 `docker builder prune`입니다.
근본 대응으로는 CI 파이프라인 끝에 정리 단계를 넣고, 레지스트리에 태그 수명 정책을 겁니다.
...
`COPY`의 캐시 키는 파일 내용 체크섬이라서, 목록 파일이 같으면 그대로 재사용됩니다.
...
slim/alpine으로 낮추고, 같은 레이어 안에서 캐시를 정리한 뒤 `.dockerignore`를 정비합니다.
...
Docker는 "코드가 기대하는 파일시스템 전체"를 불변 레이어로 굳혀 어디서든 같은 실행 결과를 재현하는 도구입니다.
