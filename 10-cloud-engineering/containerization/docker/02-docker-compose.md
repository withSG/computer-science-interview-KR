...
> **비유의 한계**: 큐시트는 공연 중에 터진 사고를 수습해 주지 않습니다. Compose도 마찬가지로 **한 대의 호스트 안에서
> 상태를 맞춰줄 뿐**입니다. 서버가 죽었을 때 다른 서버로 옮겨 주지는 못하고, 그게 필요해지는 지점이 쿠버네티스입니다.
...
Compose는 프로젝트마다 브리지 네트워크를 만듭니다(기본 이름은 `<프로젝트명>_default`, 프로젝트명은
디렉터리명에서 옵니다). 같은 네트워크에 붙은 컨테이너는 **서비스 이름으로 서로를 찾습니다.**
...
**`ports`를 열지 않아도 내부 통신은 됩니다.**
backend는 `mysql:3306`으로 접속할 수 있지만, 내 노트북 브라우저에서는 MySQL에 닿을 수 없습니다. 운영에서 DB 포트를 호스트로 노출하지 않는 이유가 이것입니다. 쿠버네티스에서 DB를 ClusterIP Service로만 두는 것과 정확히 같은 발상입니다.
...
**대응**: 호스트 포트만 바꿉니다(`"3307:3306"`). 컨테이너 포트는 그대로여야 다른 서비스의 접속 설정이 안 깨집니다.
...
`.env`만 고친 경우도 마찬가지로 컨테이너 재생성이 필요합니다(`up -d`면 Compose가 변경을 감지해 재생성합니다).
...
`ports`는 호스트에 노출하는 용도라 내부 통신에는 필요 없고 DB는 노출하지 않는 것이 원칙입니다.
...
MySQL은 컨테이너가 뜬 뒤에도 초기화에 시간이 걸립니다. 해결은 두 단계입니다. 우선 `healthcheck`를 정의하고
`condition: service_healthy`로 준비될 때까지 기다리게 합니다. 근본적으로는 애플리케이션에 커넥션 재시도와
...
Compose는 여러 컨테이너의 최종 상태를 YAML로 선언해 한 명령으로 재현하는 도구입니다.
