...
> 여러 컨테이너를 YAML 한 장으로 정의하고 한 명령으로 띄우는 방법, 서비스 이름이 곧 DNS가 되는 원리, `depends_on`이 왜 기동 순서를 보장해 주지 못하는지를 설명할 수 있게 된다.
...
백엔드 + MySQL + Redis로 구성된 평범한 서비스를 `docker run`만으로 올려 보자.
...
- **순서를 사람이 기억해야 한다.** 네트워크와 볼륨을 먼저 만들지 않으면 실패한다
- **재현 불가.** 이 명령들이 어디에도 저장돼 있지 않다. 셸 히스토리가 유일한 인프라 명세다
- **한 개만 고치기 어렵다.** 백엔드 이미지를 올리려면 `docker stop && docker rm && docker run ...`을 손으로 조합
- **팀에 전파가 안 된다.** 새 팀원에게 이 다섯 줄을 슬랙으로 보내는 순간 오탈자 사고가 시작된다

Docker Compose는 이 명령들을 **선언적인 YAML 한 장**으로 바꾼다. 무엇을 어떤 순서로 실행할지가 아니라,
**최종적으로 어떤 상태여야 하는지**를 적는다. 나머지는 Compose가 계산한다.

### 비유: 공연 큐시트

`docker run` 반복은 무대감독이 배우 한 명 한 명에게 무전으로 지시하는 것이다. 큐시트(Compose 파일)가 있으면
"누가, 어디에, 어떤 소품과 함께" 서 있어야 하는지가 문서로 남고, 그걸 보고 누구나 같은 공연을 올릴 수 있다.

> **비유의 한계**: 큐시트는 공연 중 사고에 대응하지 못한다. Compose도 마찬가지로 **한 대의 호스트 안에서
> 상태를 맞춰줄 뿐**, 서버가 죽었을 때 다른 서버로 옮겨주지 않는다. 그게 필요해지는 지점이 쿠버네티스다.
...
`version:` 키는 이제 쓰지 않는다. Compose V2는 이 키를 무시하고 경고만 낸다.
오래된 블로그에서 `version: '3.8'`을 봤다면 그대로 따라 쓰지 말고 지워도 된다.

명령어도 V1의 `docker-compose`(하이픈)가 아니라 **V2의 `docker compose`(공백)** 를 쓴다.
...
`docker compose up -d` 한 줄이면 네트워크 생성, 볼륨 생성, 이미지 빌드, 컨테이너 기동이 순서대로 끝난다.
`networks`를 따로 적지 않았는데도 세 서비스가 서로 통신되는 이유는 다음 절에서 다룬다.
...
| `build` | Dockerfile로 직접 빌드 | 코드를 고쳐도 `up`만 하면 재빌드 안 된다. `--build` 필요 |
| `ports` | **호스트에 노출** | 내부 통신에는 필요 없다. 열면 외부 공격면이 늘어난다 |
| `expose` | 같은 네트워크에만 알림 | 문서 목적에 가깝다. 실제 접근 제어는 네트워크가 한다 |
| `environment` | 컨테이너 환경변수 | 여기 비밀번호를 하드코딩하고 커밋하는 사고가 잦다 |
| `volumes` | 볼륨/바인드 마운트 | `./경로`로 시작하면 바인드, 이름이면 Named Volume |
| `command` | 기본 CMD 덮어쓰기 | ENTRYPOINT는 안 바뀐다. 바꾸려면 `entrypoint` |
...
디렉터리명에서 온다). 같은 네트워크에 붙은 컨테이너는 **서비스 이름으로 서로를 찾는다.**
...
ClusterIP Service로만 두는 것과 정확히 같은 발상이다.

### 두 가지 흔한 오해

**오해 1: `DB_HOST=localhost`로 두면 되겠지**
컨테이너 안의 `localhost`는 **그 컨테이너 자신**이다. backend 컨테이너에서 `localhost:3306`을 찌르면
backend 자신에게 물어보는 꼴이라 `Connection refused`가 난다. 값은 서비스 이름인 `mysql`이어야 한다.

**오해 2: 포트 매핑을 해야 컨테이너끼리 통신되겠지**
`ports: "3307:3306"`을 걸었더라도, backend는 여전히 `mysql:3306`으로 접속한다.
포트 매핑은 **호스트 ↔ 컨테이너** 통로일 뿐 컨테이너 사이의 통신과 무관하다.
`mysql:3307`로 적으면 오히려 실패한다.
...
- 셸 환경변수가 `.env`보다 우선한다. `DB_PASSWORD=x docker compose up`이 `.env` 값을 이긴다
- `env_file`과 `environment`에 같은 키가 있으면 `environment`가 이긴다

기본값을 주면 `.env`가 없어도 실행이 깨지지 않는다.
...
렌더링 결과가 헷갈리면 실행 전에 확인한다. 실무에서 가장 많이 쓰는 디버깅 명령이다.

```bash
docker compose config          # 변수 치환까지 끝난 최종 YAML 출력
```

`.env`는 **커밋하지 않는다.** 대신 키 목록만 담은 `.env.example`을 커밋하고 `.gitignore`에 `.env`를 넣는다.
이 규칙 하나가 자격증명 유출 사고의 상당수를 막는다.
...
컨테이너에 "살아 있음"이 아니라 **"받을 준비됨"** 판정 기준을 부여한다.
...
`docker compose ps`를 치면 `(healthy)` 표시가 뜬다. PostgreSQL이면 `pg_isready -U app`,
HTTP 서버면 `curl -f http://localhost:8080/actuator/health`가 흔한 프로브다.
단, 이미지에 `curl`이 없으면 헬스체크가 영원히 실패한다. 이미지에 있는 도구로 짜야 한다.

### 해결 2: 앱이 재시도하게 만든다 (더 근본적)

healthcheck는 **기동 시점** 문제만 푼다. 운영 중 DB가 잠깐 재시작하면 어차피 같은 문제가 반복된다.
그래서 실무에서는 둘을 함께 쓴다.
...
애플리케이션 쪽에서는 커넥션 풀의 재시도·백오프 설정을 켜 둔다.
**"의존 서비스는 언제든 잠깐 사라질 수 있다"** 를 전제로 짜는 것이 컨테이너 환경의 기본 태도다.
...
여기까지 5분. MySQL 버전을 맞추거나 Redis를 로컬에 설치할 필요가 없다.
프로젝트를 떠날 때는 `docker compose down -v`로 흔적 없이 지운다.

### 개발용 오버라이드 분리

같은 Compose 파일을 개발과 운영에 쓰면 결국 둘 중 하나가 이상해진다. 파일을 나누는 것이 정석이다.
...
운영 배포용으로는 오버라이드를 빼고 명시적으로 조합한다.
...
모든 개발자가 Kafka까지 띄울 필요는 없다. 서비스에 `profiles: ["messaging"]`을 붙이면 기본 `up`에서
제외되고, `docker compose --profile messaging up -d`로 켤 때만 뜬다.

소스 변경을 감지해 자동으로 동기화·재빌드하고 싶다면 `develop.watch`를 정의하고 `docker compose watch`를
쓸 수도 있다. 바인드 마운트가 잘 안 통하는 컴파일 언어에서 특히 유용하다.
...
**원인**: 로컬에 이미 MySQL이 설치돼 3306을 쓰고 있거나, 다른 프로젝트의 Compose가 같은 포트를 잡았다.
**대응**: 호스트 포트만 바꾼다(`"3307:3306"`). 컨테이너 포트는 그대로여야 다른 서비스의 접속 설정이 안 깨진다.
애초에 외부 접근이 필요 없다면 `ports` 항목을 지우는 것이 최선이다.
...
**데이터 디렉터리가 비어 있을 때만** 실행된다. 볼륨에 예전 데이터가 남아 있으면 새 환경변수는 그냥 무시된다.
**대응**: 개발 환경이라면 `docker compose down -v`로 볼륨까지 지우고 다시 올린다.
운영이라면 볼륨을 지우면 안 되므로 DB 안에서 `ALTER USER`로 비밀번호를 바꾼다.
`down -v`의 `-v`는 되돌릴 수 없다. 치기 전에 어느 환경인지 반드시 확인한다.

### 시나리오 3 — 코드를 고쳤는데 반영이 안 된다

**원인**: `docker compose up -d`는 **이미 존재하는 이미지**로 컨테이너만 다시 만든다. 소스를 다시 빌드하지 않는다.
...
한 줄 결론: **개발 환경과 단일 서버 배포까지는 Compose로 충분하고, 서버가 여러 대가 되는 순간 쿠버네티스가 필요하다.**
Compose에서 배운 "서비스 이름으로 통신", "설정을 환경변수로 주입", "볼륨으로 상태 분리" 세 가지는
쿠버네티스에서 Service, ConfigMap/Secret, PersistentVolume으로 이름만 바뀐 채 그대로 이어진다.
...
- 꼬리 질문: "IP가 바뀌면요?" → 이름 기반 해석이라 재생성돼도 설정을 고칠 필요가 없다.
  쿠버네티스 Service의 DNS 디스커버리와 같은 개념이라고 연결한다.
...
- 꼬리 질문: "healthcheck를 어떻게 짜나요?" → 이미지에 실제로 존재하는 도구로,
  포트 열림이 아니라 쿼리 응답 같은 실질적 준비 상태를 검사해야 한다고 답한다.
...
서비스 이름 기반 DNS와 healthcheck 기반 기동 제어를 이해하면 대부분의 "왜 안 붙지" 문제가 사라진다.
