...
>
> **비유의 한계** — 택배는 한 번에 한 곳에만 있습니다. 요청은 여러 하위 호출로 갈라져 동시에
...
새 트레이스가 시작되어, 앞뒤가 영원히 이어지지 않습니다.**
...
**첫째, 병목은 payment → 외부 PG 호출입니다.** inventory와 pricing은 합쳐도 43ms로 무의미합니다.
가장 긴 스팬 하나가 전체를 결정하는 게 흔한 패턴입니다.

**둘째, 부모와 자식의 시간 차이도 정보입니다.** order-service는 2760ms인데 자식들의 합은 2723ms입니다.
차이 37ms가 자신의 처리 시간이고, 이 차이가 크면 **스팬으로 잡히지 않은 구간**(직렬화, GC 정지,
스레드 풀 대기)이 있다는 신호입니다.

**셋째, 스팬이 겹치는지가 순차/병렬을 알려줍니다.** inventory와 pricing이 겹치지 않으면 순차
...
트레이스가 경계에서 끊겼습니다. 이를 표준화한 W3C 규격이 Trace Context입니다.
...
자기 `span_id`로 바꿔서 보냅니다.** `trace_id`만 끝까지 같고 `parent-id`는 홉마다 바뀝니다.

`trace-flags`의 샘플링 비트가 중요합니다. 진입점에서 "이 트레이스를 수집한다"고 정하면 그 결정이
헤더를 타고 전파되어, **모든 서비스가 같은 판단을 내립니다.** 이게 없으면 어떤 서비스는 기록하고
...
**비동기 구간을 부모-자식으로 이을지, 별도 트레이스로 두고 Link로 연결할지는 설계 판단입니다.**
...
원하는 데이터를 정확히 남긴다는 점에서 압도적으로 우수합니다. 대신 **인프라가 복잡해집니다.**
...
메모리도 많이 듭니다. 트레이스가 완성될 때까지 스팬을 메모리에 들고 있어야 하기 때문입니다.
...
경로가 현실적입니다.** 다만 Head를 쓰더라도 최소한의 가시성은 확보해두는 게 좋습니다. 예외가 터진
뒤에 샘플링 비트를 켜기엔 이미 늦습니다. 그 시점에는 앞선 스팬들이 기록되지 않았기 때문입니다.
그래서 결제처럼 중요한 엔드포인트나 특정 디버그 헤더가 붙은 요청은 **진입점에서 처음부터 100% 샘플링**하도록
규칙을 섞습니다.
...
했습니다. 백엔드를 바꾸려면 전 서비스의 계측 코드를 고쳐야 했습니다. OpenTelemetry(OTel)는 **계측 API를
...
변경으로 끝납니다.** 민감정보 제거, 속성 정규화, 샘플링도 여기서 중앙 집중으로 처리합니다.
...
**Tempo가 Loki와 같은 설계 철학을 공유한다는 점은 기억해둘 만합니다.** 색인을 최소화하고 오브젝트
