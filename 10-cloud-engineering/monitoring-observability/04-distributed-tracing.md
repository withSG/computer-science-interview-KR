...
> 요청 하나가 여러 서비스를 지날 때 어디서 시간이 사라지는지 찾는 방법과, 그 데이터를 감당 가능한 비용으로 모으는 샘플링 전략을 설명할 수 있게 된다.
...
모놀리스에서 "주문 API가 느리다"는 문제는 프로파일러 하나로 해결된다. 모든 코드가 한 프로세스
안에 있으니 어느 메서드가 시간을 먹는지 바로 보인다.
...
언제 끝났는지 기록**한다. 그 결과 "이 요청의 2.8초는 어디서 왔는가"에 정확히 답할 수 있다.

> **비유** — 국제 택배 추적과 같다. 송장 번호 하나로 "인천 물류센터 도착 → 통관 대기 12시간 →
> 항공 운송 → 현지 배송"처럼 각 단계의 시각이 기록되니, 늦어진 지점이 통관이었다는 걸 알 수 있다.
>
> **비유의 한계** — 택배는 한 번에 한 곳에만 있지만, 요청은 여러 하위 호출로 갈라져 동시에
> 진행된다. 그래서 트레이스는 선형 목록이 아니라 **트리**이고, "총 소요 시간 = 각 단계의 합"이
> 성립하지 않는다. 병렬 구간에서는 가장 느린 가지가 전체를 결정한다.
...
**Span(스팬)** 은 그 여정 안의 한 작업 구간이다. 각 스팬은 다음을 갖는다.
...
행위다. 이게 트레이싱의 전부라고 해도 과장이 아니다. **전파가 한 군데라도 끊기면 그 지점부터
...
**첫째, 병목은 payment → 외부 PG 호출이다.** inventory와 pricing은 합쳐도 43ms로 무의미하다.
가장 긴 스팬 하나가 전체를 결정하는 게 일반적인 패턴이다.
...
자기 `span_id`로 바꿔서 보낸다.** `trace_id`만 끝까지 같고 `parent-id`는 홉마다 바뀐다.

`trace-flags`의 샘플링 비트가 중요하다. 진입점에서 "이 트레이스를 수집한다"고 정하면 그 결정이
헤더를 타고 전파되어, **모든 서비스가 같은 판단을 내린다.** 이게 없으면 어떤 서비스는 기록하고
어떤 서비스는 안 해서 구멍 뚫린 트레이스가 나온다.
...
민감정보는 절대 넣으면 안 된다. 외부로 나가는 요청에서 baggage를 제거하는 처리도 필요하다.

### 비동기 경계

HTTP는 헤더가 있으니 쉽지만, Kafka·SQS·RabbitMQ 경계에서는 메시지 헤더/속성에 직접 실어야 한다.
...
요청"처럼 보인다. 배치 컨슈머처럼 여러 메시지를 한꺼번에 처리하는 경우에는 Span Link로 여러
원본 트레이스를 참조하는 게 더 정확하다.
...
초당 5,000 요청을 처리하는 서비스에서 요청당 스팬이 20개라면 초당 10만 스팬이다. 전량 저장하면
저장 비용, 네트워크 대역폭, 백엔드 처리 능력이 모두 무너진다. 그런데 대부분의 트레이스는
**정상적이고 서로 똑같아서 볼 가치가 거의 없다.** 그래서 샘플링은 "무엇을 버릴 것인가"가 아니라
**"무엇을 반드시 남길 것인가"** 의 문제다.
...
Collector를 여러 대로 확장하면 트레이스가 쪼개져 판단이 불가능해진다. 그래서 앞단에 `trace_id`
해시로 라우팅하는 로드밸런싱 계층을 두고 뒷단에서 tail sampling을 적용하는 2단 구성을 쓴다.
또 트레이스가 완성될 때까지 스팬을 메모리에 들고 있어야 해서 메모리 사용량이 크다.
...
경로가 현실적이다.** 다만 Head를 쓰더라도 최소한의 가시성은 확보해두는 게 좋다. 예외가 터진
...
Kafka 클라이언트 등을 후킹해 스팬을 자동으로 만들어준다. Java라면 JVM 인자 하나로 시작한다.
...
스토리지에 던져 저장 비용을 낮추는 대신 "임의 속성으로 트레이스 찾기"에 제약이 있다.

### 세 축을 잇기

- **메트릭 → 트레이스**: Prometheus의 exemplar로 히스토그램 버킷에 대표 `trace_id`를 붙이면,
  그래프의 튄 지점을 클릭해 바로 그 트레이스로 이동할 수 있다.
- **트레이스 → 로그**: 스팬의 `trace_id`로 로그를 필터링한다.
  [03-logging-stack.md](./03-logging-stack.md)에서 다룬 MDC 주입이 전제 조건이다.
- **로그 → 트레이스**: 에러 로그에 담긴 `trace_id`로 전체 요청 경로를 복원한다.

---

## 6. 트레이스로 병목을 찾는 절차

> **상황** — 주문 API p99가 400ms에서 2.8초로 올랐다. 에러율은 정상.

**1) 메트릭에서 트레이스로 진입**

`histogram_quantile`로 p99가 튄 시간대와 엔드포인트를 특정한다. exemplar가 설정되어 있으면
그래프의 해당 지점에서 대표 트레이스로 바로 넘어가고, 없으면 트레이싱 UI에서 시간 범위와
`duration > 1s` 조건으로 검색한다.

**2) 한 건이 아니라 여러 건을 보고, 정상 트레이스와 비교한다**

느린 트레이스 하나만 보면 우연을 원인으로 착각한다. 최소 10건 이상 열어 **공통으로 긴 스팬이
무엇인지** 확인한 뒤, 같은 엔드포인트의 빠른 트레이스를 나란히 열어본다. 구조가 다르면(없던
호출이 생겼으면) 코드 경로가 바뀐 것이고, 구조가 같은데 특정 스팬만 길면 그 의존성이 느려진 것이다.
...
병목 스팬의 `trace_id`로 그 서비스 로그를 필터링해 그 순간 무엇이 찍혔는지 본다. 재시도, 폴백,
예외 스택이 여기서 드러난다. 수정 후에는 같은 조건으로 트레이스를 다시 뽑아 워터폴 모양이
바뀌었는지 확인한다. p99 그래프만 보면 트래픽 변화 덕인지 수정 덕인지 구분되지 않는다.
...
패턴이다. 매출과 직결되는 핵심 트랜잭션 경로 하나를 골라 끝에서 끝까지 이어보면, 그 자체로
"여기가 병목이었네" 같은 성과가 나오고 다른 팀이 따라오게 된다.

**전파가 끊기는 지점을 먼저 찾는다.** 자동 계측이 지원하지 않는 커스텀 RPC, 오래된 HTTP
클라이언트, 스레드 풀을 직접 쓰는 코드에서 컨텍스트가 사라진다. 트레이스가 짧게 끊겨 있으면
그 경계를 의심한다.

**속성 카디널리티는 메트릭만큼 조심하지 않아도 되지만 무한하지도 않다.** 트레이스는 개별
이벤트라서 `user_id` 같은 고유값을 속성으로 넣어도 메트릭처럼 폭발하지는 않는다. 다만 그
속성으로 검색하려면 백엔드가 색인을 만들어야 하므로, **검색 조건으로 쓸 속성만 색인 대상으로
지정**하는 게 실무 기준이다.

**오버헤드는 대개 문제가 되지 않는다.** 자동 계측의 CPU 오버헤드는 보통 한 자릿수 퍼센트
수준으로 보고된다. 문제가 되는 건 계측 자체가 아니라 스팬을 동기 전송하도록 설정한 경우다.
배치 스팬 프로세서(BatchSpanProcessor)로 비동기 전송하는지 반드시 확인한다.

**보관 기간은 짧게 잡는다.** 트레이스는 장애 조사용이라 며칠이면 충분하고, 장기 추세는 메트릭이
담당한다. 이 역할 분담을 지키지 않으면 저장 비용이 감당되지 않는다.
...
- 꼬리 질문: "Head 샘플링에서 에러 트레이스를 남길 방법은 없나요?" → 결정이 요청 시작 시점에
  나므로 근본적으로 어렵다. 특정 엔드포인트나 조건을 100% 샘플링하도록 규칙을 섞는 정도가
  현실적인 절충이라고 답한다.
...
- 꼬리 질문: "메시지 큐에서는 어떻게 전파하나요?" → 메시지 헤더에 traceparent를 주입하고
  컨슈머가 추출해 부모로 연결한다. 처리 시점이 많이 떨어져 있으면 부모-자식 대신 Span Link로
  연결하는 게 더 정확하다고 답한다.
...
만드는 건 **"에러와 느린 요청은 반드시 남긴다"는 샘플링 정책**이다.
