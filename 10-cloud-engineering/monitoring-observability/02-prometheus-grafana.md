...
> Pull 방식이 왜 표준이 되었는지, 메트릭 타입을 잘못 고르면 어떤 숫자가 거짓말을 하는지, 그리고 PromQL 한 줄로 에러율과 p99를 뽑아내는 방법을 설명할 수 있게 됩니다.
...
전부 훑고 상태 코드를 파싱해서 세어야 합니다. 수억 줄이면 몇 분이 걸리고, 이 계산을 15초마다
...
저장 비용이 수백 분의 일로 줄고 초 단위 질의가 가능해집니다.
...
둘은 별개 제품이지만 실무에서는 거의 항상 함께 쓰입니다.
...
`/metrics` 경로에 현재 값을 텍스트로 노출해두기만 하면 됩니다.
...
- **타겟 헬스 체크가 공짜다.** 스크랩에 실패하면 Prometheus가 `up{job="api"} 0`을 자동 생성합니다.
...
현재 메모리 사용량, 큐 길이, 활성 커넥션 수, 대기 중인 스레드 수. **지금 이 순간의 값**입니다.
...
전체 p99를 서버에서 계산할 수 있다.** 대신 버킷 경계는 직접 정해야 합니다. SLO가 200ms인데 경계가
...
태그 값이 유한한지 코드 리뷰 체크리스트에 넣어두는 게 실무적인 방어책입니다.
...
Grafana를 처음 쓰면 패널을 40개쯤 만들게 되고, 그 대시보드는 장애 때 아무도 보지 않습니다.
...
**템플릿 변수로 대시보드를 재사용한다.** 서비스마다 복사하면 개선이 한쪽에만 반영됩니다.
쿼리에서 `{service="$service"}`처럼 변수를 참조하면 하나로 전 서비스를 커버할 수 있습니다.

**패널마다 "그래서 뭘 해야 하나"가 있어야 한다.** 해석 방법을 모르는 패널은 지웁니다. 설명란에
...
그 경로의 p99는 180ms → 3.0s. **에러와 지연이 함께 올랐다면 타임아웃 계열**입니다. 지연 변화 없이
...
멈췄겠지만 대기 수 40이 심각도를 알려줍니다. **DB는 멀쩡한데 풀만 고갈이면 커넥션을 잡은 채
