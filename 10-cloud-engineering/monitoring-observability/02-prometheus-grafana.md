...
> Pull 방식이 왜 표준이 되었는지, 메트릭 타입을 잘못 고르면 어떤 숫자가 거짓말을 하는지, 그리고 PromQL 한 줄로 에러율과 p99를 뽑아내는 방법을 설명할 수 있게 된다.
...
로그만 있는 상태에서 "지난 1시간 동안 주문 API의 에러율"을 구해보자. 검색 엔진이 1시간치 로그를
전부 훑고 상태 코드를 파싱해서 세어야 한다. 수억 줄이면 몇 분이 걸리고, 이 계산을 15초마다
반복해서 알림을 걸어야 한다면 검색 엔진이 먼저 죽는다.

메트릭은 이 계산을 **애플리케이션이 미리 해두는 것**이다. 요청이 실패할 때마다 카운터를 1 올려두면
나중에 "1시간 동안 몇 번 올랐나"만 보면 된다. 원본 이벤트를 버리는 대신 집계된 숫자만 남기므로
저장 비용이 수백 분의 일로 줄고 초 단위 질의가 가능해진다.

Prometheus는 이 아이디어를 구현한 시계열 데이터베이스이자 수집기이고, Kubernetes에 이어 두 번째로
CNCF를 졸업한 프로젝트다. Grafana는 여러 데이터 소스를 한 화면에서 질의하고 그리는 시각화 도구다.
둘은 별개 제품이지만 실무에서는 거의 항상 함께 쓰인다.
...
- **타겟 헬스 체크가 공짜다.** 스크랩에 실패하면 Prometheus가 `up{job="api"} 0`을 자동 생성한다.
  Push 방식에서는 "데이터가 안 오는 것"이 인스턴스가 죽은 건지, 네트워크 문제인지, 원래 트래픽이
  없는 건지 구분되지 않는다.
- **부하를 수집 측이 통제한다.** 앱이 폭주해서 메트릭을 밀어 넣어 모니터링을 무너뜨리는 일이
  구조적으로 불가능하다.
- **디버깅이 쉽다.** `curl http://localhost:8080/metrics` 한 줄로 지금 이 인스턴스가 무슨 값을
  내보내는지 확인된다.
...
**Pushgateway는 배치 잡 전용이라고 생각하는 게 안전하다.** 일반 서비스에 붙이면 앱이 죽어도
값이 남아 헬스 체크 의미가 사라지고, Pushgateway 자체가 단일 장애점이 된다.

---

## 3. 메트릭 타입 네 가지

타입 선택은 문법 문제가 아니라 **그 숫자가 나중에 의미 있게 집계될 수 있는가**의 문제다.

### Counter — 누적만 증가

요청 수, 에러 수, 처리한 메시지 수처럼 **단조 증가하는 값**. 프로세스가 재시작하면 0으로
리셋되는데, 이는 버그가 아니라 정상 동작이고 `rate()`와 `increase()`가 알아서 보정한다.
따라서 **절대값은 거의 의미가 없다.** 인스턴스마다 재시작 시점이 달라 비교할 수 없으므로
항상 `rate()`로 변화량을 본다.

### Gauge — 오르내리는 현재 상태

현재 메모리 사용량, 큐 길이, 활성 커넥션 수, 대기 중인 스레드 수. **지금 이 순간의 값**이다.
에러 수를 Gauge로 만들어 "최근 값"을 넣어두면 스크랩 사이에 발생한 에러가 통째로 사라진다.
**"놓치면 안 되는 사건은 Counter, 현재 상태는 Gauge"** 가 판단 기준이다.
...
전체 p99를 서버에서 계산할 수 있다.** 대신 버킷 경계는 직접 정해야 한다. SLO가 200ms인데 경계가
`0.1, 0.5`뿐이면 p95를 보간으로 추정할 뿐이라 정확도가 크게 떨어진다.
...
**치명적 한계는 합산이 불가능하다는 것이다.** 파드 A의 p99가 1.4초, 파드 B가 0.9초일 때 전체
p99는 알 수 없다. 백분위수는 평균 낼 수 없기 때문이다. 파드가 여러 개인 환경에서 Summary를
쓰면 "전체 서비스의 p99"를 영원히 구할 수 없게 된다.
...
**분산 환경이면 Summary 대신 Histogram이 기본값이다.** Summary는 인스턴스가 하나이거나
버킷 경계를 미리 정할 수 없는 특수한 경우에만 고려한다.
...
`tag("userId", userId)` 같은 코드가 리뷰에서 걸러지지 않으면 카디널리티가 그대로 폭발한다.
태그 값이 유한한지 코드 리뷰 체크리스트에 넣어두는 게 실무적인 방어책이다.
...
**`rate()` — Counter의 초당 증가율.** 대괄호 안은 되돌아볼 시간 범위다.

```promql
rate(http_requests_total[5m])
```

"최근 5분 구간에서 초당 평균 몇 건 증가했나"를 계산한다. 구간 안에 최소 두 개의 샘플이 있어야
하므로 **범위는 스크랩 주기의 4배 이상**으로 잡는 게 안전하다. 스크랩이 15초면 여유 있게 `[5m]`을
쓴다. 인스턴스 하나가 잠깐 스크랩에 실패해도 값이 비지 않는다.
...
파드별로 흩어진 시계열을 handler 기준으로 합친다. `by`는 남길 라벨을, `without`은 버릴 라벨을
지정한다. 파드 라벨을 지워야 오토스케일링으로 파드가 바뀌어도 그래프가 끊기지 않는다.
...
`by (le, ...)`에서 `le`를 반드시 남겨야 한다. `le`는 버킷 경계를 나타내는 라벨이라 지우면
분포 정보가 사라져 계산 자체가 불가능해진다. PromQL 입문자가 가장 자주 겪는 함정이다.
...
대시보드에서 매번 무거운 쿼리를 돌리면 Prometheus가 느려진다. 자주 쓰는 표현식은 미리 계산해
새 메트릭으로 저장한다.
...
이름 규칙은 `수준:메트릭:연산` 형태를 관례로 쓴다. 대시보드와 알림 규칙이 모두
`service:http_errors:ratio5m` 하나만 참조하면 되므로 쿼리 중복이 사라지고, 정의를 고칠 때
한 군데만 고치면 된다. `for` 절 없이 알림을 만들면 순간 스파이크 하나에 새벽 3시 호출이 울린다.
...
PostgreSQL, Redis, Nginx는 Prometheus 형식으로 메트릭을 내보내지 않는다. Exporter는 그 대상의
네이티브 인터페이스로 상태를 물어본 뒤 Prometheus 형식으로 번역하는 작은 프로세스다.
...
**cAdvisor와 kube-state-metrics는 역할이 다르다.** cAdvisor는 "이 컨테이너가 실제로 CPU를 얼마나
쓰나"(실측)를, kube-state-metrics는 "레플리카 3개를 원하는데 2개만 Ready다"(선언 상태와 실제
상태의 차이)를 알려준다. 둘 다 필요하다.

### 서비스 디스커버리 — 사라지는 타겟 따라가기

파드가 수시로 생성·삭제되는 환경에서 타겟 IP를 설정 파일에 적어둘 수는 없다. Prometheus는
Kubernetes API, EC2 API, Consul 등에 물어봐서 타겟 목록을 갱신한다.
...
거르고 `replace`로 라벨을 붙인다. 카디널리티가 터졌을 때 계측 코드 배포 없이 즉시 막을 수 있는
곳도 여기다(`metric_relabel_configs`의 `drop`으로 특정 메트릭을 저장 직전에 버린다).

---

## 6. Grafana 대시보드 설계 원칙

Grafana를 처음 쓰면 패널을 40개쯤 만들게 되고, 그 대시보드는 장애 때 아무도 보지 않는다.

**하나의 대시보드는 하나의 질문에 답한다.** "서비스가 건강한가?"와 "왜 느린가?"는 다른
대시보드다. 앞의 것은 RED 지표만으로 한 화면에 끝나야 하고, 뒤의 것은 GC·커넥션 풀·캐시
히트율 같은 원인 지표를 잔뜩 담아도 된다.

**위에서 아래로 증상 → 원인 순서.** 맨 위는 사용자가 느끼는 것(에러율, p99, 처리량), 그 아래는
의존성(DB·외부 API 지연), 맨 아래가 인프라(CPU, 메모리)다.

**템플릿 변수로 대시보드를 재사용한다.** 서비스마다 복사하면 개선이 한쪽에만 반영된다.
쿼리에서 `{service="$service"}`처럼 변수를 참조하면 하나로 전 서비스를 커버할 수 있다.

**패널마다 "그래서 뭘 해야 하나"가 있어야 한다.** 해석 방법을 모르는 패널은 지운다. 설명란에
"이 값이 X를 넘으면 Y를 확인"을 적어두면 새벽에 호출받은 사람이 살아난다. 단위 지정도 기본이다.
단위 없는 `0.234`는 234ms인지 234초인지 알 수 없다.

---

## 7. 장애 시나리오로 보는 실전 흐름

> **상황** — 새벽 2시, `ApiHighErrorRate` 알림. 주문 API의 5xx가 0.1%에서 3%로 올랐다.
...
결과: `/orders/checkout` 하나만 튀고 나머지 핸들러는 정상 → 전면 장애가 아니라 특정 경로 문제.
그 경로의 p99는 180ms → 3.0s. **에러와 지연이 함께 올랐다면 타임아웃 계열**이다. 지연 변화 없이
에러만 늘었다면 로직 오류나 검증 실패 쪽을 먼저 의심한다.
...
결과: 사용률 1.0 고정, 대기 스레드 40, DB 쿼리 p99는 정상. 사용률만 봤다면 "100%니까 바쁘네"에서
멈췄겠지만 대기 수 40이 심각도를 알려준다. **DB는 멀쩡한데 풀만 고갈이면 커넥션을 잡은 채
외부 호출을 하고 있을 가능성**이 높다.

**3) 대응 순서**

복구가 원인 규명보다 먼저다. 직전 배포가 있었다면 롤백, 없었다면 문제 경로 차단으로 우선
지혈한다. 원인 분석은 서비스가 정상으로 돌아온 뒤에 한다. 이때 쓴 PromQL을 그대로 Runbook에
적어두면 다음 사람이 같은 길을 다시 헤매지 않는다.

---

## 8. 실무에서는

**장기 보관** — 로컬 TSDB는 수 주 단위 보관을 전제로 설계되어 있다. 더 길게 보려면 `remote_write`
또는 완성된 블록을 업로드하는 방식으로 Thanos, Grafana Mimir, VictoriaMetrics 같은 장기 저장소에
넘긴다. 이 계층은 오브젝트 스토리지를 쓰기 때문에 저장 단가가 크게 낮아진다.

**고가용성** — Prometheus는 클러스터링을 지원하지 않는다. 동일 설정의 인스턴스를 두 대 띄워
같은 타겟을 각자 스크랩하게 하는 게 표준 패턴이고, 중복 알림은 Alertmanager가 묶어준다.

**Alertmanager** — 발화한 알림을 받아 그룹핑·억제·라우팅·무음 처리를 담당한다. **알림 규칙은
Prometheus에, 전달 정책은 Alertmanager에** 있다는 역할 분리를 기억해야 한다. 자세한 설계는
[05-alerting-oncall.md](./05-alerting-oncall.md)에서 다룬다.

**관리형 서비스** — AWS에서는 Amazon Managed Service for Prometheus와 Amazon Managed Grafana를
쓸 수 있다. 과금은 **수집한 샘플 수, 저장 중인 데이터 양, 질의가 처리한 샘플 수** 축으로
구성된다. 여기서도 카디널리티가 청구서를 결정한다. 라벨 하나 잘못 넣으면 세 항목이 동시에 오른다.

**Prometheus를 쓰면 안 되는 곳** — 청구나 정산처럼 **한 건도 놓치면 안 되는 데이터**에는 쓰지
않는다. 스크랩 기반이라 구조적으로 100% 정확성을 보장하지 않는다. 그런 데이터는 이벤트 로그나
트랜잭션 DB로 다뤄야 한다.
...
- 꼬리 질문: "Pushgateway를 모든 서비스에 쓰면 안 되나요?" → Pushgateway는 값을 계속 보관하므로
  앱이 죽어도 메트릭이 남아 헬스 체크 의미가 사라지고, 단일 장애점이 된다고 답한다.
...
- 꼬리 질문: "버킷을 촘촘히 많이 두면 되지 않나요?" → 버킷 하나가 시계열 하나이므로 버킷 수가
  곧 카디널리티다. SLO 임계값 주변만 촘촘히 두는 게 현실적인 절충이라고 답한다.
...
- 꼬리 질문: "`irate`는 언제 쓰나요?" → `irate`는 마지막 두 샘플만 보므로 변화에 민감하다.
  그래프에서 순간 변화를 볼 때는 유용하지만 노이즈가 커서 알림에는 부적합하다고 답한다.
...
능력은 **메트릭 타입을 옳게 고르고 `rate`·`sum by`·`histogram_quantile`을 조합하는 문법**에서 나온다.
