...
**On-call 부하에 상한을 둡니다.** 구글 SRE의 널리 인용되는 기준은 **한 교대에 처리하는 사건이 평균
두 건을 넘지 않아야 한다**는 것입니다. 넘으면 사람을 더 넣을 게 아니라 **알림과 시스템을 고쳐야 합니다.**
그리고 **보상과 회복 시간을 제도화합니다.** 새벽에 대응한 사람에게 다음 날 늦게 출근할 권리를 주는
...
포스트모템에서 사람을 지목하면 딱 한 가지 결과가 나옵니다. **다음부터 사람들이 사실을 숨깁니다.**
...
그리고 **5 Whys를 기계적으로 적용하지 않는 게 좋습니다.** "왜"를 다섯 번 반복하면 선형 원인 사슬
...
Scribe 부담이 크게 줄어듭니다. **외부 커뮤니케이션은 상태 페이지로 분리합니다.** CS 문의가 폭주할 때
