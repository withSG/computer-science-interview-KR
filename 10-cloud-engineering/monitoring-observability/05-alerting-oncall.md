...
> 사람을 깨울 자격이 있는 알림이 무엇인지, 알림 피로가 왜 조직을 무너뜨리는지, 그리고 장애가 났을 때 무엇을 먼저 해야 하는지 설명할 수 있게 됩니다.
...
대시보드는 아무도 24시간 보지 않습니다. 새벽 3시에 결제가 멈추면 누군가 그 사실을 알아야 하고,
그 사람을 깨우는 유일한 수단이 알림입니다. 문제는 이 수단이 **한 번에 소진되는 신뢰**로 작동한다는
점입니다. 알림이 울렸는데 확인해보니 아무 일도 없었던 경험이 몇 번 반복되면 사람은 다음 알림을
진지하게 받아들이지 않습니다. 개인의 게으름이 아니라 예측 가능한 반응이고, 그래서 **알림 설계는
기술 문제가 아니라 신뢰 관리 문제입니다.

> **비유** — 화재경보기와 같습니다. 요리할 때마다 울리는 경보기는 몇 주 안에 배터리가 뽑히고,
> 진짜 불이 났을 때 아무도 대피하지 않습니다.
>
> **비유의 한계** — 화재경보기는 켜거나 끄는 둘 중 하나지만, 알림은 **긴급도별로 다른 경로로
> 보낼 수 있다.** 알리되 깨우지는 않는 중간 단계(티켓, 대시보드)를 설계할 수 있다는 점이 핵심입니다.
...
**셋째, 지금 해야 한다.** 내일 아침에 해도 되는 일이면 호출이 아니라 티켓입니다.
...
무서운 건 **이 상태가 겉으로는 정상으로 보인다**는 점입니다. 알림은 계속 울리고 모니터링 시스템도
...
- **알림 무력화** — 시끄러운 알림은 조용히 삭제됩니다. 그중 하나가 실제로 중요했다는 사실은 다음 장애 때 안다

### 줄이는 방법

- **지속 시간을 준다.** `for: 10m`처럼 조건이 일정 시간 유지되어야 발화하게 합니다. 순간
  스파이크는 대부분 저절로 회복됩니다.
- **그룹으로 묶고 연쇄 알림을 억제한다.** 파드 50개가 동시에 죽으면 알림은 1건이어야 합니다.
...
**예측 가능한 고갈**뿐입니다. 디스크가 가득 차면 반드시 장애가 나고 차기 전에 막을 수 있습니다.
이때도 "사용률 80%"가 아니라 **"현재 증가 속도로 4시간 내 가득 참"** 처럼 임박한 결과로 표현합니다.
...
보는 이유**는 서로의 약점을 보완하기 위해서입니다. 짧은 창만 보면 순간 스파이크에 오탐이 나고,
긴 창만 보면 이미 회복된 문제에도 알림이 남습니다. 둘이 동시에 참일 때만 울리면 "지금도 진행
중이고 충분히 오래 지속됐다"는 두 조건이 모두 확인됩니다.
...
알림이 몇 초에 걸쳐 순차 발화합니다. 30초를 기다렸다 한 번에 보내면 알림 20건이 1건이 됩니다.

**억제(inhibition)** 는 인과 관계를 알림 시스템에 알려주는 장치입니다. "DB가 죽었다"는 알림이 있을 때
"주문 API 에러율 증가"는 파생 증상이므로 숨깁니다. 다만 과하게 걸면 **진짜 독립적인 문제까지
숨겨지므로** 원인이 확실한 관계에만 적용합니다. **무음(silence)** 은 배포나 마이그레이션처럼 계획된
작업 중에 쓰되 만료 시간을 짧게 겁니다. 무기한 무음은 알림 삭제와 같고 아무도 기억하지 못합니다.
...
"30분 내 미해결 시 DB 팀 호출")이 명시되어야 합니다. 낡은 Runbook은 없는 것보다 나쁘므로
대응할 때마다 갱신합니다.
...
"고치는 데 오래 걸려서"인지에 따라 개선 대상이 완전히 달라지기 때문입니다.
...
**3) 완화 우선.** 여기가 핵심입니다. **원인을 이해하는 것보다 사용자를 살리는 게 먼저다.** 직전
...
재처리가 필요한지 확인합니다. **에러가 멈춘 것과 데이터가 정상인 것은 다르다.**

---

## 8. 비난 없는 포스트모템

포스트모템에서 사람을 지목하면 딱 한 가지 결과가 나옵니다. **다음부터 사람들이 사실을 숨긴다.**
"제가 프로덕션에서 그 명령을 실행했습니다"라는 진술이 징계로 이어지는 조직에서는 아무도 그
말을 하지 않고, 그러면 진짜 원인도 영원히 드러나지 않습니다. 비난 없는 포스트모템은 면죄부를
...
됩니다. 장애 선언 시 전용 채널을 자동 생성하고 타임라인을 기록하는 인시던트 관리 도구를 붙이면
Scribe 부담이 크게 줄어듭니다. **외부 커뮤니케이션은 상태 페이지로 분리한다.** CS 문의가 폭주할 때
담당자가 개별 응대를 하면 대응 인력이 그만큼 빠지는데, "알려진 문제, 영향 범위, 다음 업데이트
시각"만 공지해도 문의량이 크게 줍니다.
