...
> 로그·메트릭·트레이스가 각각 어떤 질문에 답하는 데이터인지, 그리고 SLO와 에러 버짓이 "고칠까 말까"를 어떻게 데이터로 바꿔주는지 설명할 수 있게 된다.
...
- 없음 — 이 문서부터 시작해도 된다. HTTP 상태 코드와 "서버가 여러 대로 돌아간다"는 감각만 있으면 충분하다.

---

## 1. 왜 필요한가

서버가 한 대이던 시절의 디버깅은 단순했다. 사용자가 "느려요"라고 하면 그 서버에 SSH로 들어가서
`top`으로 CPU를 보고, `tail -f`로 로그를 흘려보고, 재현되면 그 자리에서 고쳤다. 시스템 전체가
한 사람 머릿속에 들어왔기 때문에 가능한 일이었다.

이 방식이 무너지는 지점은 대체로 세 가지다.

**첫째, 서버가 사라진다.** 컨테이너와 오토스케일링 환경에서는 문제를 일으킨 인스턴스가 이미
교체되어 없다. SSH로 들어갈 대상 자체가 사라진 뒤에 제보가 도착한다.

**둘째, 요청 하나가 여러 서비스를 지난다.** 주문 API 하나가 인증 → 재고 → 가격 → 결제 →
알림으로 흘러간다면, "주문이 느리다"는 말은 다섯 개 서비스 중 어디가 느리다는 뜻인지 알려주지
않는다. 각 서비스 담당자가 자기 대시보드를 보고 "우리는 정상인데요"라고 말하는 상황이 반복된다.

**셋째, 문제가 조건부로 발생한다.** "전체 에러율 0.3%"는 평온해 보이지만, 실제로는 특정 결제
수단을 쓰는 특정 앱 버전 사용자만 100% 실패하고 있을 수 있다. 평균은 이런 국소적 재앙을 지운다.

관측성(Observability)은 이 세 문제에 대한 답이다. **시스템이 이미 뱉어놓은 데이터만으로,
새 코드를 배포하지 않고도 처음 던져보는 질문에 답할 수 있는 능력**을 말한다.

### 알려진 문제와 모르는 문제

두 용어를 가르는 가장 정확한 기준은 도구가 아니라 **질문의 성격**이다.
...
정리하면 **모니터링은 관측성의 부분집합**이다. "디스크 사용률 알림"은 우리가 디스크가 찰 수
있다는 걸 이미 알기 때문에 걸 수 있다. 반면 "지난 화요일 14시에 안드로이드 12 사용자만
결제 3단계에서 타임아웃이 났다"는 조건은 미리 대시보드로 만들어둘 수 없다. 이런 질문에
답하려면 데이터가 충분히 세밀하게, 충분한 컨텍스트를 달고 남아 있어야 한다.

> **비유** — 모니터링은 자동차 계기판이고, 관측성은 정비소의 진단 장비다. 계기판은 속도·연료·엔진
> 경고등처럼 제조사가 미리 고른 몇 개만 보여준다. 진단 장비는 ECU에 남은 원시 기록을 뒤져서
> "어떤 조건에서 몇 번 이상이 있었나"를 임의로 질의한다.
>
> **비유의 한계** — 진단 장비는 차를 세워놓고 쓰지만, 관측성은 서비스가 돌아가는 중에 써야 한다.
> 그래서 "사후에 얼마나 파고들 수 있느냐"가 아니라 **"장애 시점에 무엇을 미리 남겨뒀느냐"**가
> 실제 승부처다. 남기지 않은 데이터는 사후에 복구할 수 없다.

---

## 2. 세 가지 축이 각각 답하는 질문

Logs, Metrics, Traces를 흔히 "3대 축(Three Pillars)"이라 부른다. 중요한 건 이름 외우기가 아니라
**각각이 다른 질문에 답한다**는 사실이다.
...
**Metrics(메트릭)** 는 이미 집계된 숫자다. `http_requests_total{status="500"} 1423`처럼 한 시점에
값 하나만 남긴다. 저장 효율이 압도적으로 좋아서 몇 달치를 보관하고 초 단위로 알림을 걸 수 있다.
대신 개별 요청의 사연이 없다. "5xx가 늘었다"는 알 수 있어도 "어떤 요청이 왜 실패했는지"는 모른다.

**Traces(트레이스)** 는 요청 하나가 시스템을 통과한 경로다. 각 구간을 Span이라 부르고, Span들이
부모-자식으로 묶여 하나의 Trace를 이룬다. 서비스 경계를 넘는 지연을 눈으로 보여주는 유일한
데이터다. 대신 모든 서비스가 같은 규약으로 ID를 전파해야 하고, 전량 수집하면 비용이 감당되지
않아 샘플링이 거의 필수다.

**Logs(로그)** 는 특정 시점의 사건 기록이다. 스택 트레이스, 요청 파라미터, 외부 API 응답 본문처럼
가장 풍부한 컨텍스트를 담는다. 대신 양이 가장 많고, 검색·저장 비용이 세 축 중 가장 비싸다.
...
사이에서 사람이 손으로 시간대를 맞춰야 하고, 그 순간 대응 시간이 몇 배로 늘어난다.
...
**카디널리티(cardinality)를 잘못 설계해서** 발생한다.
...
300억개는 어떤 하드웨어로도 감당되지 않는다. 실제 사고는 대개 이렇게 시작한다.
...
**라벨에는 값의 종류가 유한하고 예측 가능한 것만 넣는다.** 판단 기준은 간단하다.
"이 라벨이 가질 수 있는 값이 몇 개인지 지금 말할 수 있는가?" 말할 수 없다면 라벨이 아니다.
...
이유이기도 하다. 메트릭은 "몇 건인가"를 싸게 답하고, 로그와 트레이스가 "그중 이 건은 무엇인가"를
답한다. 각 축에 맞지 않는 데이터를 밀어 넣으면 비용과 성능이 동시에 무너진다.

---

## 4. USE와 RED — 무엇을 측정할지 정하는 방법론

메트릭을 수집할 수 있게 되면 곧바로 다음 문제가 온다. **무엇을 재야 하는가.** 수집 가능한 지표는
수천 개인데, 대시보드에 다 올리면 아무도 보지 않는다. 여기서 쓰는 게 두 가지 체크리스트다.

**USE 방법(Brendan Gregg)** — *리소스*를 볼 때 쓴다. 모든 리소스에 대해 세 가지를 본다.
...
**RED 방법(Tom Wilkie)** — *요청을 처리하는 서비스*를 볼 때 쓴다.

- **Rate(처리율)**: 초당 요청 수
- **Errors(에러율)**: 실패한 요청의 비율
- **Duration(지연 분포)**: 응답 시간 분포(p50 / p95 / p99)

구글 SRE 책의 **네 가지 황금 신호(Four Golden Signals)** 는 이 둘의 절충이다.
Latency, Traffic, Errors, Saturation — RED에 포화도를 더한 형태로 이해하면 된다.
...
USE는 그 증상의 원인이 되는 자원을 짚어준다.

이 중 가장 자주 빠뜨리는 게 **Saturation**이다. 사용률은 100%가 상한이라 "90%"면 아직 여유가
있어 보이지만, 포화도는 상한이 없다. 커넥션 풀 사용률이 100%로 고정된 상태에서 대기 스레드가
2개인 것과 200개인 것은 완전히 다른 상황인데, 사용률만 보면 둘이 똑같아 보인다. **사용률이
...
지연을 "p99 = 180ms"처럼 값으로 잡지 않고 "200ms 이내인 요청의 비율"로 잡는 게 중요하다.
비율로 정의해야 여러 시간 구간·여러 인스턴스에 걸쳐 **합산할 수 있고**, 에러 버짓 계산으로
바로 이어진다. 백분위수는 평균낼 수 없다는 통계적 제약이 여기서 실무 결정으로 이어진다.

세 층위는 **SLA < SLO < 실제 성능** 순으로 마진을 둔다. SLA가 99.5%라면 SLO는 99.9%로 잡는다.
...
피크 시간대의 10분 장애가 새벽의 30분 장애보다 버짓을 훨씬 많이 태운다. 잔여 버짓을 정책의
근거로 쓸 거라면 시간이 아니라 요청 수로 계산해야 판단이 어긋나지 않는다.

에러 버짓이 바꾸는 건 계산이 아니라 **대화의 성격**이다. 에러 버짓이 없으면 개발팀은
"기능을 빨리 내보내자", 운영팀은 "안정성이 우선이다"라고 주장하고, 결론은 목소리 큰 쪽이
가져간다. 에러 버짓이 있으면 논쟁이 이렇게 바뀐다.

| 잔여 버짓 | 정책 |
|----------|------|
| 충분히 남음 | 배포 속도를 올린다. 위험한 실험도 허용한다 |
| 절반 이하 | 위험한 변경은 카나리로만. 배포 전 검증 강화 |
| 소진 | 신규 기능 동결. 안정화 작업만 진행 |

핵심 통찰은 **"버짓이 남았는데 안 쓰는 것도 낭비"** 라는 점이다. 가용성 99.99%를 달성하고
있는데 SLO가 99.9%라면, 그건 잘하고 있는 게 아니라 **과도하게 보수적으로 운영해서 기능 출시
속도를 손해 보고 있다**는 신호일 수 있다. SLO는 상한이 아니라 목표선이다.

### 소진 속도 기반 알림

"에러 버짓이 다 떨어졌습니다"라는 알림은 이미 늦다. 그래서 실무에서는 **소진 속도(burn rate)**
로 알림을 건다.

소진 속도 1은 "지금 속도가 유지되면 SLO 기간이 끝나는 시점에 버짓을 정확히 다 쓴다"는 뜻이다.
소진 속도 14.4는 그 14.4배 속도로 태우고 있다는 뜻이고, 30일 SLO 기준으로 1시간만 지속되어도
전체 버짓의 2%가 사라진다.

구글 SRE Workbook이 제시하는 방식은 **짧은 창과 긴 창을 동시에 보는 것**이다. 짧은 창(예: 5분)
하나만 보면 순간적인 튐에 오탐이 나고, 긴 창(예: 6시간)만 보면 급성 장애를 늦게 잡는다. 그래서
"긴 창과 짧은 창이 **동시에** 임계치를 넘을 때만" 알린다. 이 구조가 빠른 장애는 빠르게,
느린 출혈은 느리게 잡아내면서 오탐을 줄인다.

---

## 6. 실무에서는

**오픈소스 조합**이 가장 흔하다. 메트릭은 Prometheus, 시각화와 알림은 Grafana, 로그는 Loki나
Elasticsearch, 트레이스는 Jaeger나 Grafana Tempo를 쓴다. 계측 라이브러리는 OpenTelemetry로
통일하는 흐름이 뚜렷하다. OpenTelemetry는 CNCF 프로젝트로, 코드에서 세 축을 모두 같은 SDK로
내보내고 백엔드는 나중에 갈아끼울 수 있게 해준다.

**SaaS**로는 Datadog, New Relic, Grafana Cloud, Splunk 등이 있다. 운영 부담이 없다는 게 최대
장점이고, 데이터 양이 늘수록 비용이 가파르게 오른다는 게 최대 단점이다.

**AWS 환경**에서는 CloudWatch(로그·메트릭), X-Ray(트레이스), Amazon Managed Service for
Prometheus, Amazon Managed Grafana를 조합한다. 여기서 반드시 이해해야 할 건 **과금 모델**이다.

- CloudWatch Logs: **수집한 데이터 양**, **보관 중인 데이터 양**, **쿼리가 스캔한 데이터 양**이
  각각 별도로 과금된다. "로그를 많이 남기면 비싸다"가 아니라 "많이 남기고, 오래 두고, 자주
  검색하면 세 번 비싸진다"고 이해해야 한다.
- CloudWatch 커스텀 메트릭: **고유 메트릭 개수** 기준으로 과금된다. 앞서 본 카디널리티 문제가
  그대로 청구서에 반영된다. 라벨 하나 잘못 추가하면 메트릭 개수가 수백 배가 된다.
- X-Ray: **기록된 트레이스 수**와 **조회·스캔한 트레이스 수** 기준이다. 샘플링 비율이 곧 비용이다.

구체적 단가는 리전과 시점에 따라 달라지므로 항상 공식 요금 페이지에서 확인해야 하지만,
**"양 × 보관 기간 × 조회 빈도"라는 세 축의 곱으로 비용이 결정된다**는 구조는 어느 벤더나 같다.
설계 단계에서 이 구조를 알고 있으면 "일단 다 남기고 나중에 줄이자"는 결정을 하지 않게 된다.
...
- 꼬리 질문: "그럼 Prometheus를 쓰면 관측성이 확보된 건가요?" → 아니라고 답한다. Prometheus는
  집계된 메트릭이라 카디널리티 제약상 임의 조건 탐색이 어렵다. 로그·트레이스와 공통 ID로
  연결되어야 비로소 사후 탐색이 가능해진다는 방향으로 답한다.
...
- 꼬리 질문: "MSA라면 순서가 달라지나요?" → 서비스가 여러 개면 트레이싱의 가치가 급격히
  올라간다. 메트릭만으로는 서비스 경계 간 지연을 분해할 수 없기 때문에 우선순위가 올라간다고 답한다.
...
- 꼬리 질문: "이미 넣어버렸다면 어떻게 수습하나요?" → 계측 코드에서 라벨을 제거하고 배포한 뒤,
  기존 시계열은 보관 기간이 지나면서 자연히 정리된다. 배포를 기다릴 수 없으면 수집 측에서
  `metric_relabel_configs`로 그 메트릭을 저장 직전에 버리도록 걸어 즉시 차단할 수 있다고 답한다.
...
- 꼬리 질문: "99.99%로 잡으면 안 되나요?" → 신뢰성 한 자릿수를 더 올리는 비용은 선형이 아니라
  지수적으로 증가하고, 대개 배포 속도와 기능 출시를 희생한다. 사용자가 그 차이를 체감하지
  못하는 수준이면 과잉 투자라고 답한다.
...
그 능력은 메트릭·트레이스·로그를 공통 ID로 엮고 카디널리티 예산 안에서 설계했을 때만 생긴다.
