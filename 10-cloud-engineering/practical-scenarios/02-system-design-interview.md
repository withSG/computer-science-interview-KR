...
> 45분이라는 시간을 어떤 순서로 쓰는지, 규모 추정이 왜 설계를 바꾸는지, 그리고 면접관이 실제로 채점하는 것이 무엇인지 설명할 수 있게 됩니다.
...
> **비유의 한계**: 건축은 몇 달이 걸리고 결과물의 완성도로 평가받지만, 설계 면접은 45분이라 완성도를 평가할 수 없습니다. 그래서 **"이 정도 규모면 이런 구조가 필요하다"는 판단 근거**를 말로 드러내는 것이 도면보다 중요합니다. 도면만 예쁘게 그리고 근거를 말하지 않으면 떨어집니다.
...
- 피크 수백 QPS의 쓰기는 **단일 RDB로 충분합니다.** 여기서 샤딩을 꺼내면 과잉 설계다
- 피크 수만 QPS의 읽기는 **DB 단독으로는 부담입니다.** 캐시가 이 설계의 중심이 된다
...
여기서 이미 논점이 하나 생깁니다. **301(영구)과 302(임시) 중 무엇을 쓰는가.** 301은 브라우저가 응답을 캐싱해도 되는 상태 코드라, 한 번 방문한 클라이언트의 이후 요청이 서버까지 오지 않습니다. 부하는 줄지만 그만큼 클릭 집계가 누락되고, 이미 캐싱한 클라이언트는 매핑을 바꿔도 한동안 예전 목적지로 갑니다. 302는 명시적인 캐시 헤더가 없으면 브라우저가 캐싱하지 않아 매번 서버를 거치므로, 통계는 남고 목적지 변경도 즉시 반영되지만 부하가 그대로입니다. "통계가 요구사항에 있으니 302를 쓰겠다"고 말하는 순간, 요구사항과 설계를 연결하는 사고를 보여준 것입니다.
...
여러 서비스가 알림을 요청하고, 채널(푸시·SMS·이메일)마다 실패 양상이 다르다는 것이 특징입니다. 중복 발송은 사용자에게 즉시 보이는 사고이므로 **멱등성 키를 요청에 포함시키고 일정 기간 중복을 차단**하는 설계가 필수입니다. 재시도는 지수 백오프로 하되, 무한 재시도가 컨슈머를 마비시키지 않도록 횟수를 제한하고 DLQ로 뺍니다. 유효하지 않은 토큰처럼 **재시도해도 절대 성공하지 않는 실패**는 즉시 제거해야 한다는 구분도 좋은 신호입니다.

### 파일 저장·공유와 검색

두 문제는 **"무거운 것을 API 서버로 통과시키지 않는다"**는 원칙을 공유합니다. 파일은 메타데이터만 DB에 두고 본체는 오브젝트 스토리지에 직접 올립니다. 서버는 업로드용 서명 URL을 발급할 뿐 바이트 스트림을 중계하지 않고, 큰 파일은 조각으로 나눠 병렬 전송해 실패한 조각만 다시 보냅니다. 검색은 원본 저장소와 검색 인덱스를 분리하고 변경을 비동기로 색인에 반영하는데, 여기서 **색인 지연**이라는 논점이 생깁니다. "방금 등록한 글이 몇 초 뒤에 검색돼도 되는가"는 요구사항 명확화 단계에서 물었어야 할 질문입니다. 자동완성은 목표 지연이 검색보다 훨씬 짧아 별도 경로로 빼는 것이 보통입니다.
...
- **혼자 말한다**: 5분 이상 면접관 반응을 안 보면 방향이 틀려도 알 수 없습니다. "여기까지 괜찮을까요?"를 중간에 넣는다
- **모르는 걸 아는 척한다**: 꼬리 질문 두 번이면 드러납니다. "그 부분은 써본 적이 없는데, 제가 이해하기로는 ~인 것 같습니다"가 훨씬 낫다
- **단일 장애점을 안 짚는다**: 다이어그램에 박스가 하나만 있는 컴포넌트가 있으면 면접관은 반드시 묻습니다. 먼저 말하는 게 좋다
- **데이터 흐름을 안 따라간다**: 박스와 화살표만 있고 "요청이 들어오면 무슨 일이 순서대로 일어나는지"를 말하지 않으면 설계가 검증되지 않는다
- **시간 관리 실패**: 15분에 데이터 모델을 붙들고 있으면 확장성 논의를 못 합니다. 이 단계는 완벽할 필요가 없다
