...
> 설계 문제 하나를 받고 45분이 시작됩니다. 그 시간을 어떤 순서로 쓰는지, 규모 추정이 왜 설계를 바꾸는지, 면접관이 실제로 채점하는 것이 무엇인지를 이 문서에서 설명합니다.
...
"트위터를 설계해보세요"에 정답이 있을 리 없습니다. 실제 트위터도 십수 년간 구조를 여러 번 바꿨습니다. 그런데도 이 문제를 내는 이유는, **답이 없는 문제를 다루는 방식이 실무의 대부분**이기 때문입니다. 실무에서 주어지는 요구사항은 항상 불완전하고, 선택지는 여러 개이고, 어느 것도 완전히 옳지 않습니다.

면접관이 채점하는 축은 대략 이렇습니다.
...
> 표 요약: **채점되는 건 최종 그림이 아니라 그 그림에 도달한 과정입니다.** 같은 아키텍처를 그려도 "왜 이걸 골랐는지"를 말하지 못하면 점수가 나오지 않습니다.

### 비유: 건축 설계 미팅

건축가는 첫 미팅에서 도면부터 그리지 않습니다. 몇 명이 사나, 예산은 얼마인가, 땅의 모양은 어떤가, 나중에 증축할 생각이 있나를 묻습니다. 그 답에 따라 설계가 완전히 달라지기 때문입니다.

> **비유의 한계**: 건축은 몇 달이 걸리고, 결과물의 완성도로 평가받습니다. 설계 면접은 45분뿐이라 완성도를 볼 수가 없습니다. 그래서 **"이 정도 규모면 이런 구조가 필요하다"는 판단 근거**를 말로 드러내는 것이 도면보다 중요합니다. 도면만 예쁘게 그리고 근거를 말하지 않으면 떨어집니다.
...
시간을 지키는 것 자체가 평가 대상입니다. 요구사항 확인에 15분을 쓰면 설계를 보여줄 시간이 사라지고, 반대로 30초 만에 넘어가면 잘못된 문제를 풉니다. **5분 지점과 10분 지점에서 한 번씩 시계를 보는 습관**이 있으면 대부분의 배분 사고는 막을 수 있습니다.
...
기준은 하나입니다. **그 질문의 답이 설계를 바꾸는가.** 바꾸지 않는 질문은 하지 않습니다.

### 범위 밖을 명시적으로 선언한다

"인증, 결제, 어뷰징 탐지, 관리자 도구는 이번 설계에서 제외하겠습니다. 괜찮을까요?"

이 한마디의 효과가 큽니다. 다룰 수 있는 크기로 문제를 줄이면서, 동시에 "**이런 것들이 실제로 필요하다는 걸 안다**"는 신호를 줍니다. 면접관이 "아니요, 어뷰징은 다뤄주세요"라고 하면 그게 그 회사가 보고 싶은 지점입니다.

### 비기능 요구사항을 숫자로 받아낸다

기능 요구사항은 대부분 스스로 나열할 수 있지만, 비기능은 물어봐야 합니다.

- 규모: DAU 또는 총 사용자 수, 1인당 하루 활동 횟수
- 지연: p99 목표. "빠르게"가 아니라 "200ms 이내"
- 가용성: 99.9%인가 99.99%인가. 이 차이가 멀티 AZ/리전을 가릅니다
- 일관성: 방금 쓴 걸 즉시 읽어야 하나, 몇 초 늦어도 되나
- 보관 기간: 영구인가 90일인가. 저장량 계산이 여기서 갈립니다

면접관이 "적당히 정하세요"라고 하면 직접 정하고 소리 내어 말하면 됩니다. **가정을 명시하는 것 자체가 점수입니다.**
...
추정의 목적은 정확한 숫자가 아니라 **"이 문제의 어려운 지점이 어디인가"를 판정하는 것**입니다. 저장량이 50GB로 나오면 샤딩 이야기를 꺼낼 필요가 없습니다. 읽기가 쓰기의 100배로 나오면 그날의 주제는 캐시와 리드 레플리카입니다. 추정 없이 설계하면 필요 없는 컴포넌트를 잔뜩 붙이게 됩니다.
...
왼쪽이 실제 값이고 오른쪽이 계산에 쓰는 반올림 값입니다. 면접에서는 오른쪽만 씁니다.

지연 시간 감각이 왜 필요한가. "캐시를 쓰겠습니다"와 "이 조회는 디스크를 타면 10ms인데 캐시로 0.5ms가 되므로, p99 200ms 목표에서 20배의 여유가 생깁니다"는 전혀 다른 답변입니다.
...
이 숫자에서 곧바로 나오는 판단들이 있습니다.

- 피크 수백 QPS의 쓰기는 **단일 RDB로 충분합니다.** 여기서 샤딩을 꺼내면 과잉 설계입니다
- 피크 수만 QPS의 읽기는 **DB 단독으로는 부담입니다.** 캐시가 이 설계의 중심이 됩니다
- 5년 11TB는 단일 인스턴스에 부담이므로 **보관 정책이나 파티셔닝**을 논의해야 합니다
- 즉 이 문제의 진짜 주제는 "짧은 코드 생성 방식"이 아니라 **읽기 확장**입니다
...
상위 설계 다이어그램보다 API를 먼저 적으면 좋은 점이 있습니다. **박스를 그리기 전에 "이 시스템이 무엇을 하는 물건인지"가 확정됩니다.** 박스부터 그리면 나중에 요구사항과 안 맞는 컴포넌트가 남습니다.
...
여기서 이미 논점이 하나 생깁니다. **301(영구)과 302(임시) 중 무엇을 쓰는가.** 301은 브라우저가 응답을 캐싱해도 되는 상태 코드라서, 한 번 방문한 클라이언트의 이후 요청은 서버까지 오지 않습니다. 부하는 줄지만 그만큼 클릭 집계가 누락되고, 이미 캐싱한 클라이언트는 매핑을 바꿔도 한동안 예전 목적지로 갑니다.

302는 다릅니다. 명시적인 캐시 헤더가 없으면 브라우저가 캐싱하지 않아 매번 서버를 거칩니다. 통계는 남고 목적지 변경도 즉시 반영되지만, 부하는 그대로입니다. "통계가 요구사항에 있으니 302를 쓰겠다"고 말하는 순간, 요구사항과 설계를 연결하는 사고를 보여준 것입니다.

데이터 모델도 같은 방식입니다. 테이블 전체를 그릴 필요 없이 **접근 패턴이 갈리는 지점**만 보이면 됩니다.
...
`short_code`에 유니크 인덱스가 붙어 있다는 것 자체가 설명입니다. 조회는 항상 `short_code`로 들어오므로 이 인덱스가 읽기 경로 전체를 결정하고, 샤딩을 한다면 샤드 키도 `short_code`가 됩니다.
...
이 뼈대를 외워두고, 문제마다 **불필요한 박스를 지우고 필요한 박스를 추가**하는 방식이 안정적입니다. 처음부터 백지에 그리면 빠뜨리는 것이 생깁니다.

그리고 다이어그램만 그리지 말고 **요청 흐름 두 개를 말로 따라갑니다.** "URL을 만들면 ① 게이트웨이가 인증하고 ② 쓰기 API가 분산 ID를 받아 Base62로 인코딩하고 ③ DB에 저장하고 ④ 캐시에 미리 넣습니다." 흐름을 따라가는 동안 빠진 컴포넌트가 자연스럽게 드러납니다.

### 심화 구간에서 파고들 곳

면접관이 "여기 더 자세히 설명해주세요"라고 지목하지 않으면 직접 고릅니다. 고를 때 기준은 **규모 추정에서 가장 어렵다고 판정된 지점**입니다.
...
각 문제에는 면접관이 확인하고 싶은 **결정적 논점이 하나씩** 있습니다. 그것을 못 짚으면 나머지를 잘해도 통과하기 어렵습니다.
...
코드 생성 방식이 첫 갈림길입니다. 해시(MD5/SHA 앞자리)는 같은 URL에 같은 코드를 주지만 **충돌 처리 로직이 반드시 필요**합니다. 카운터 기반(분산 ID를 Base62로 인코딩)은 충돌이 원천적으로 없지만 **코드가 순차적이라 예측 가능**합니다. 예측 가능성이 문제라면 ID를 셔플하거나 난수 구간을 섞습니다. 어느 쪽을 고르든 "이 단점은 이렇게 막는다"까지 말해야 합니다.
...
이 트레이드오프를 스스로 꺼내는지가 이 문제의 통과 기준이라고 봐도 됩니다.

### 채팅

세 가지를 순서대로 다루면 됩니다. ① **연결**: 클라이언트가 어느 게이트웨이 서버에 붙어 있는지를 어딘가(예: Redis)에 기록해야 다른 서버가 메시지를 라우팅할 수 있습니다. ② **순서**: 같은 대화방의 메시지는 같은 파티션으로 보내 부분 순서를 보장하고, 클라이언트는 서버가 부여한 시퀀스로 정렬합니다. ③ **오프라인**: 수신자가 접속 중이 아니면 저장해두고, 재접속 시 마지막 수신 지점 이후를 내려줍니다. 푸시 알림은 별도 경로입니다.

### 알림

여러 서비스가 알림을 요청하고, 채널(푸시·SMS·이메일)마다 실패 양상이 다르다는 것이 특징입니다. 중복 발송은 사용자에게 즉시 보이는 사고이므로 **멱등성 키를 요청에 포함시키고 일정 기간 중복을 차단**하는 설계가 필수입니다. 재시도는 지수 백오프로 합니다. 다만 무한 재시도가 컨슈머를 마비시키지 않도록 횟수를 제한하고 DLQ로 뺍니다. 유효하지 않은 토큰처럼 **재시도해도 절대 성공하지 않는 실패**는 즉시 제거해야 한다는 구분도 좋은 신호입니다.

### 파일 저장·공유와 검색

두 문제는 "**무거운 것을 API 서버로 통과시키지 않는다**"는 원칙을 공유합니다. 파일은 메타데이터만 DB에 두고 본체는 오브젝트 스토리지에 직접 올립니다. 서버는 업로드용 서명 URL을 발급할 뿐, 바이트 스트림을 중계하지 않습니다. 큰 파일은 조각으로 나눠 병렬로 전송하고, 실패한 조각만 다시 보냅니다. 검색은 원본 저장소와 검색 인덱스를 분리하고, 변경을 비동기로 색인에 반영합니다. 여기서 **색인 지연**이라는 논점이 생깁니다. "방금 등록한 글이 몇 초 뒤에 검색돼도 되는가"는 요구사항 명확화 단계에서 물었어야 할 질문입니다. 자동완성은 목표 지연이 검색보다 훨씬 짧아 별도 경로로 빼는 것이 보통입니다.

뉴스피드를 제외한 각 문제의 전체 설계와 스키마는 [qna-architecture.md](./qna-architecture.md)에 있습니다. 이 문서는 "45분을 어떻게 쓰는가"에 집중합니다.
...
왜 문제인가:
  ...
  설계가 완전히 달라집니다. 잘못된 문제를 40분간 잘 푸는 셈입니다.
  게다가 "요구사항 명확화" 축에서 0점을 받습니다.

개선:
  "몇 가지 확인하고 시작하겠습니다"로 시작해 3~5개만 묻습니다.
...
왜 문제인가:
  QPS 100짜리 시스템에 이 구성을 넣으면 운영 비용이 기능 가치를 넘습니다.
  면접관은 "왜 필요한가"를 묻고, 대답을 못 하면 오히려 감점됩니다.
  기술 이름을 아는 것과 필요를 판단하는 것은 다른 능력입니다.

개선:
  가장 단순한 구조에서 시작해 병목이 나올 때마다 하나씩 추가합니다.
  "지금 규모에선 단일 DB로 충분하고, 읽기가 10배가 되면 레플리카를,
   100배가 되면 캐시를 넣겠습니다"처럼 조건과 함께 말합니다.
```
...
왜 문제인가:
  장점만 말하면 그 기술을 깊이 안 써봤다는 신호로 읽힙니다.
  실무에서 기술 선택은 항상 무언가를 포기하는 일입니다.

개선:
  "게시물은 시간순 조회가 대부분이고 조인이 거의 없어서 NoSQL이 맞습니다.
   대신 여러 테이블을 조인하는 관리자 통계 화면은 어려워지므로,
   그쪽은 별도 분석 저장소로 뺄 생각입니다."
  포기한 것과 그 대응까지 말하면 한 단계 위로 읽힙니다.
```

나머지 자주 나오는 실수들.

- **혼자 말한다**: 5분 이상 면접관 반응을 안 보면 방향이 틀려도 알 수 없습니다. "여기까지 괜찮을까요?"를 중간에 넣습니다
- **모르는 걸 아는 척한다**: 꼬리 질문 두 번이면 드러납니다. "그 부분은 써본 적이 없는데, 제가 이해하기로는 ~인 것 같습니다"가 훨씬 낫습니다
- **단일 장애점을 안 짚는다**: 다이어그램에 박스가 하나만 있는 컴포넌트가 있으면 면접관은 반드시 묻습니다. 먼저 말하는 게 좋습니다
- **데이터 흐름을 안 따라간다**: 박스와 화살표만 있고 "요청이 들어오면 무슨 일이 순서대로 일어나는지"를 말하지 않으면 설계가 검증되지 않습니다
- **시간 관리 실패**: 15분에 데이터 모델을 붙들고 있으면 확장성 논의를 못 합니다. 이 단계는 완벽할 필요가 없습니다

---

## 9. 실무에서는

- **면접의 단계 구분은 실무의 설계 리뷰와 거의 같습니다.** 사내 설계 문서(RFC, Design Doc)도 배경과 목표 → 비목표(Non-goals) → 제안 → 대안과 트레이드오프 → 위험 요소 순으로 씁니다. "비목표"를 명시하는 관행이 면접의 "범위 밖 선언"과 정확히 대응합니다.
- **추정은 실제로 씁니다.** 신규 기능의 저장량과 QPS를 미리 계산해두면 "리드 레플리카를 지금 붙일 것인가"를 감이 아니라 숫자로 결정할 수 있습니다. 클라우드 비용 산정도 같은 계산에서 나옵니다.
- **면접에서 그린 구조를 실무에 그대로 쓰지는 않습니다.** 실무에는 기존 시스템, 팀 역량, 마이그레이션 비용이라는 제약이 추가로 붙습니다. 면접은 그 제약이 없는 상태에서 판단 근거만 보는 자리입니다.
...
- 꼬리 질문: "면접관이 요구사항을 안 알려주면요?" → 직접 가정하고 소리 내어 말합니다. 가정을 명시하는 것 자체가 평가 항목입니다.
...
- 꼬리 질문: "저장량은요?" → 레코드 하나의 크기를 잡고 하루 건수를 곱한 뒤 보관 기간을 곱합니다. 2^30이 약 10억이라는 것만 알면 GB/TB 환산은 암산으로 됩니다.
...
- 꼬리 질문: "임계치는 어떻게 정하나요?" → 정답은 없고 팔로워 분포를 보고 실측으로 정합니다. 상위 몇 %가 fan-out 비용의 대부분을 차지하는지를 계산해 자른다고 답합니다.
...
- 꼬리 질문: "그러면 감점 아닌가요?" → 모르는 것보다 모르는 걸 숨기는 것이 훨씬 큰 감점입니다. 실무에서 그런 사람과 일하면 위험하기 때문입니다.
...
시스템 설계 면접은 아키텍처를 맞히는 시험이 아니라 **불완전한 요구사항을 숫자로 좁히고, 그 숫자에 근거해 선택하고, 포기한 것을 스스로 말할 수 있는지**를 보는 자리입니다.
