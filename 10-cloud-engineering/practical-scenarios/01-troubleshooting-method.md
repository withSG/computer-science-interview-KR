...
> 장애 상황에서 무엇을 먼저 해야 하는지, 후보를 어떻게 절반씩 줄여나가는지, 그리고 그 경험을 면접에서 어떻게 이야기로 만드는지 설명할 수 있게 된다.
...
- [../monitoring-observability/01-observability-concepts.md](../monitoring-observability/01-observability-concepts.md) - 메트릭·로그·트레이스의 역할 구분 (위 두 문서를 안 읽었어도 절차 자체는 이해할 수 있다)
...
같은 팀에서 실력이 갈리는 지점은 아는 명령어 개수가 아니다. **후보를 줄이는 순서**다.

장애 알림이 울린 순간, 원인 후보는 대략 이렇다. 방금 나간 배포, 어제 바뀐 설정값, 갑자기 늘어난 트래픽, 만료된 인증서, 죽은 노드, 느려진 외부 API, 커지기만 하는 테이블, 어제부터 도는 배치, 캐시 만료 타이밍, 그리고 아직 아무도 생각 못 한 무언가. 수십 개다.

여기서 로그부터 여는 사람은 후보를 하나도 지우지 못한 채 몇만 줄을 읽기 시작한다. 반면 "전체 API가 다 느린가, 특정 엔드포인트만인가"를 먼저 물으면 그 질문 하나로 후보의 절반이 사라진다. 방법론이란 결국 **매 단계마다 후보를 절반씩 지우는 질문의 순서**다.

### 비유: 정전이 났을 때

집이 어두워지면 전구부터 갈아 끼우는 사람은 없다. 창밖을 본다. 동네 전체가 어두우면 한전 문제고, 우리 집만 어두우면 두꺼비집이고, 방 하나만 어두우면 그 방 회로다. 세 번의 확인으로 범위가 동네 → 집 → 회로 → 전구로 좁혀진다.

> **비유의 한계**: 전기 회로는 상태가 없어서 원인이 사라지면 증상도 즉시 사라진다. 소프트웨어는 캐시, 큐, 커넥션 풀, 스레드 풀처럼 상태를 들고 있어서 **원인이 이미 끝난 뒤에도 증상이 계속된다.** 외부 API가 잠깐 느렸다가 회복돼도, 그 사이 고갈된 커넥션 풀 때문에 장애가 한참 더 이어지는 일이 흔하다. 그래서 "지금 이상한 것"이 아니라 **"가장 먼저 이상해진 것"**을 시간축에서 찾아야 한다.
...
**왜 문제인가**: 롤백 실행 자체는 4분이면 끝난다. 14:35에 시작했다면 사용자가 아팠던 시간은 에러율이 오르기 시작한 14:33부터 6분이다. 실제로는 51분이었다. 그 45분 동안 얻은 것은 "원인을 미리 알았다"는 것뿐인데, **원인 분석은 복구 후에 해도 결과가 똑같다.** 오히려 압박이 없는 상태에서 하는 분석이 더 정확하다.
...
원인 규명이 완화보다 앞에 오면 그 시간이 전부 MTTR에 더해진다. 이 한 줄이 장애 대응 원칙의 전부라고 해도 과장이 아니다.

### 손에 쥐고 있어야 할 완화 카드

완화란 "원인을 몰라도 지금 당장 사용자 영향을 줄이는 조치"다. 장애가 나기 전에 미리 준비돼 있어야 쓸 수 있다.
...
**데이터가 손상되고 있는 중이라면 재시작·롤백이 오히려 피해를 키운다.** 잘못된 마이그레이션이 돌고 있거나, 중복 결제가 발생 중이거나, 잘못된 값이 DB에 쌓이는 중이라면 첫 조치는 복구가 아니라 **차단**이다. 해당 기능을 멈추고, 오염 범위를 확정하고, 그다음에 복구를 논한다. "일단 서버를 재시작해보자"가 가장 위험한 순간이 이때다. 침해 사고가 의심될 때도 같다. 재시작이나 재배포는 프로세스 메모리와 로컬 로그 같은 증거를 함께 지워버려서, 무슨 일이 있었는지 영원히 알 수 없게 만든다. 이때의 순서는 격리 → 증거 보전(스냅샷·로그 확보) → 자격 증명 교체 → 복구다.
...
"서비스가 느려요"는 진단의 출발점이 될 수 없다. 다섯 가지를 채워야 한다.

- **누가**: 전체 사용자인가, 특정 리전·디바이스·요금제인가
- **언제부터**: 정확한 시각. 이 시각이 2단계의 검색 키다
- **무엇이**: 어떤 엔드포인트, 어떤 화면
- **얼마나**: p50인가 p99인가, 평소 값 대비 몇 배인가
- **어디는 멀쩡한가**: 이게 가장 강력하다. 정상인 부분이 후보를 지운다

마지막 항목을 빠뜨리는 사람이 많다. "결제만 느리고 조회는 정상"이면 공용 인프라(LB, DNS, 노드)는 후보에서 즉시 빠진다. 반대로 "모든 API가 균일하게 느리다"면 개별 쿼리가 아니라 공용 자원을 의심해야 한다.

### 2단계. 변경점부터 확인한다

경험적으로 장애의 상당수는 자연 발생이 아니라 **누군가 뭔가를 바꿔서** 생긴다. 그래서 증상 시작 시각을 기준으로 이 목록을 훑는 것이 가장 효율이 높다.
...
마지막 두 행이 중요하다. "우리는 아무것도 안 바꿨다"고 확신하는 장애의 상당수가 **시간이 바꾼 것**(인증서 만료, 데이터 누적)이거나 **남이 바꾼 것**(외부 API)이다.
...
각 질문은 "구간을 절반으로 가르는 관측 지점이 있는가"에 달려 있다. 관측 지점이 없으면 이분 탐색이 불가능해서 추측으로 돌아간다. 분산 트레이싱을 붙이는 진짜 이유가 이것이다.

### 4단계. 반증 가능한 가설을 세운다

좋은 가설은 **틀렸을 때 틀렸다고 판정되는** 형태다.

| 나쁜 가설 | 왜 나쁜가 | 좋은 가설 |
|-----------|----------|----------|
| "DB가 문제인 것 같다" | 확인할 대상이 없다 | "인덱스 없는 풀스캔이 원인이라면, 슬로우 쿼리 로그에 해당 쿼리가 있고 `EXPLAIN`에 풀스캔이 나와야 한다" |
| "메모리가 부족한 듯" | 결과일 수도 원인일 수도 | "메모리 누수라면 재시작 후 시간에 비례해 힙 사용량이 단조 증가해야 한다" |
| "트래픽이 몰려서" | 반증할 수 없다 | "트래픽이 원인이라면 RPS 증가 시점이 지연 증가 시점보다 앞서야 한다" |

세 번째 예시의 **시간 순서 확인**은 인과와 상관을 가르는 가장 값싼 도구다. 지연이 먼저 늘고 RPS가 나중에 늘었다면 트래픽은 원인이 아니라 재시도가 만든 결과다.

그리고 **한 번에 하나만 바꾼다.** 풀 사이즈를 늘리면서 인스턴스도 늘리고 캐시 TTL도 바꾸면, 복구는 되지만 무엇이 효과가 있었는지 영원히 모른다. 다음 장애 때 같은 상황에서 같은 시간이 다시 걸린다.

---

## 4. 계층별 점검 체크리스트

각 계층에서 "이 계층이 범인일 때만 나타나는 신호"를 정리한 표다. 순서대로 훑는 용도가 아니라, 3단계에서 좁혀진 계층에 대해 확인할 항목을 고르는 용도다.
...
> 표 요약: **"자원 사용률이 낮은데 느리다"면 거의 항상 풀 대기·락·외부 의존성이고, "자원 사용률이 높아서 느리다"면 CPU·메모리·디스크 중 하나다.** 이 한 갈래만 먼저 판정해도 절반이 갈린다.
...
전형적 원인을 확률 순으로 두면 대개 이렇다. ① 느린 쿼리 또는 N+1, ② 커넥션·스레드 풀 대기, ③ 외부 API 지연 전파, ④ GC 일시정지, ⑤ 캐시 히트율 하락.

판별 순서는 **"전체가 균일하게 느린가, 꼬리만 느린가"**다. p50까지 같이 올라갔다면 공용 자원(풀, DB, 외부 의존성)이고, p50은 그대로인데 p99만 튀었다면 GC나 일부 요청의 특수 경로(캐시 미스, 대용량 응답)를 본다.
...
앱 로그에 스택 트레이스가 있는가부터 확인한다.

- **앱 로그에 예외가 있다** → 코드·데이터 문제. 직전 배포 확인 후 롤백
- **앱 로그가 조용한데 5xx가 난다** → 앱에 요청이 도달하지 못한 것이다. LB 헬스체크 실패, 파드 재시작, 타임아웃으로 인한 게이트웨이 502/504
- **502/504가 섞여 있다** → 백엔드가 죽었거나(502) 응답이 제한 시간을 넘겼다(504). 후자는 사실 "지연 장애"이므로 위 항목으로 넘어간다

### 메모리 사용량이 계속 증가

메모리 증가는 그 자체로 장애가 아니라 **증가 곡선의 모양**이 진단이다.
...
(b)의 전형적 원인은 무한히 커지는 캐시(`HashMap`에 넣기만 하고 지우지 않음), 해제하지 않은 리스너, `remove()`하지 않은 `ThreadLocal`, 닫히지 않은 커넥션·스트림이다. (c)는 페이지네이션 없는 전체 조회가 대부분이다.

### 디스크 풀

`df -h`가 100%인데 큰 파일이 안 보이면 세 가지를 의심한다.

1. **삭제됐지만 열려 있는 파일** — 로그를 `rm` 했는데 프로세스가 아직 핸들을 쥐고 있으면 용량이 반환되지 않는다. 프로세스를 재시작하거나 로그 로테이션을 제대로 설정해야 한다
2. **inode 고갈** — 용량은 남았는데 파일 개수 한도에 걸린 경우다. `df -i`로 확인한다. 세션 파일, 임시 파일이 수백만 개 쌓인 상황
3. **다른 마운트 포인트** — 루트는 여유 있는데 데이터 볼륨만 꽉 찬 경우

디스크가 차면 DB가 쓰기를 거부하고, 로그를 못 남겨 진단도 어려워진다. **사용률 80% 알림은 성능이 아니라 진단 가능성을 지키기 위한 것**이다.

### 커넥션 고갈

`Connection is not available` 류의 에러는 원인이 아니라 결과다. 세 가지 중 하나다.
...
세 번째만 풀 사이즈로 해결되는데, 실무에서 만나는 대부분은 첫 번째다. **인스턴스 수 × 풀 사이즈가 DB의 최대 접속 수를 넘지 않는지**도 함께 확인해야 한다. 오토스케일로 파드가 늘어나는 순간 DB가 먼저 무너지는 사고가 여기서 나온다.
...
포스트모템에서 사람을 지목하면 결과는 하나다. **다음부터 아무도 사실을 말하지 않는다.** "제가 그 명령을 실행했습니다"라는 진술이 징계로 이어지는 조직에서는 진짜 원인이 영원히 드러나지 않는다.

비난 없는 포스트모템은 면죄부가 아니라 방법론이다. **"그 시점에 그 사람이 가진 정보로는 그 판단이 합리적이었다"**를 전제로 두면, 질문이 "왜 실수했나"에서 **"왜 그 실수가 가능했고 시스템이 왜 못 막았나"**로 바뀐다. 후자에서만 실행 가능한 개선안이 나온다.
...
액션 아이템에 **담당자와 기한이 없으면 아무도 하지 않는다.** 그리고 유형을 예방/탐지/대응으로 나누는 이유는, 대부분의 조직이 예방에만 항목을 몰아넣기 때문이다. 예방은 완벽할 수 없으므로 탐지·대응 개선이 실제로는 더 큰 효과를 낸다. 포스트모템 작성의 더 자세한 기준과 알림 설계는 [../monitoring-observability/05-alerting-oncall.md](../monitoring-observability/05-alerting-oncall.md)에 있다.
...
무엇이 문제인가. 증상이 숫자가 아니고, 진단 과정에 판단이 없고("로그를 보고 찾았다"에는 어떻게 좁혔는지가 없다), 결과가 정량화되지 않았고, "모니터링 강화"가 무엇인지 알 수 없다. 면접관 입장에서 이 답변으로는 지원자가 실제로 무엇을 했는지 판정할 수 없다.
...
트러블슈팅 이야기에는 거의 정해진 꼬리 질문이 따라온다. 미리 답을 준비해두면 이야기의 신뢰도가 올라간다.

- "다른 가설은 없었나요?" → 세웠다가 기각한 가설과 **기각한 근거**를 말할 수 있어야 한다
- "왜 3개월이나 걸렸나요?" / "지금 다시 한다면?" → 관측 가능성의 한계를 비난 없이 짚고, "락 대기 지표를 처음부터 봤다면 하루면 끝났다"처럼 구체적으로 답한다
- "본인이 한 건 어디까지인가요?" → 팀이 한 일과 본인이 한 일을 정확히 구분

---

## 8. 실무에서는

- **런북(Runbook)이 개인 역량을 팀 역량으로 바꾼다.** 잘 쓴 런북은 "증상 → 확인할 대시보드 링크 → 판단 기준 → 조치 명령"이 한 페이지에 있어서, 그 시스템을 모르는 사람도 1차 대응을 할 수 있다. 포스트모템의 액션 아이템에 런북 갱신이 자주 들어가는 이유다.
- **인시던트 채널에 진행 상황을 계속 흘린다.** 혼자 조용히 고치는 것이 빨라 보이지만, 다른 팀이 같은 시각에 겪는 증상을 공유하면 범위 좁히기가 훨씬 빨라진다. 기록 자체가 나중에 타임라인이 된다.
- **재현 환경이 없으면 진단은 추측이 된다.** 스테이징의 외부 API가 목이고, 데이터가 100건이고, 인스턴스가 1대라면 운영에서만 나는 문제는 영원히 재현되지 않는다. 지연 주입, 데이터 볼륨 모사 같은 투자가 장애 대응 시간을 직접 줄인다. 반대로 모든 장애에 정식 포스트모템을 요구하면 아무도 제대로 쓰지 않으므로, 심각도(SEV) 등급을 정해 상위 등급만 정식으로 남긴다.
...
- 꼬리 질문: "롤백이 항상 정답인가요?" → 아니다. 데이터가 손상되는 중이거나 스키마 변경이 이미 적용된 경우 롤백이 더 위험하다. 이때는 복구가 아니라 해당 기능 차단이 먼저다.
...
- 꼬리 질문: "관측 지점이 없는 구간은요?" → 그 구간은 이분 탐색이 불가능하므로 추측으로 돌아간다. 그래서 사후 액션 아이템에 그 구간의 지표 추가가 들어간다.

**Q. 자원 사용률은 정상인데 응답이 느립니다. 무엇을 의심하나요?**
A. CPU·메모리·디스크가 여유로운데 느리면 대기 계열입니다. 커넥션 풀이나 스레드 풀에서의 획득 대기, DB 락 경합, 외부 API 응답 지연 전파가 대표적입니다. 그래서 저는 "쿼리 실행 시간"과 "커넥션 획득 대기 시간"을 항상 분리해서 봅니다. 둘을 합쳐서 보면 풀 사이즈만 늘리는 잘못된 조치로 이어지기 쉽습니다.
- 꼬리 질문: "풀 사이즈를 늘리면 왜 나빠질 수 있나요?" → 병목이 DB면 동시 접속만 늘어 DB가 포화되고, 인스턴스 수 × 풀 사이즈가 DB 최대 접속 수를 넘으면 다른 서비스까지 함께 죽는다.
...
- 꼬리 질문: "근본 원인은 하나로 정리해야 하지 않나요?" → 실제 장애는 여러 요인이 겹쳐서 난다. 5 Whys로 선형 사슬 하나를 뽑는 것보다 기여 요인을 여러 개 나열하는 편이 현실에 가깝다.
...
장애 대응은 원인을 맞히는 시합이 아니라 **사용자가 아픈 시간을 줄이는 일**이고, 그 방법은 완화를 먼저 실행한 뒤 증상 정의 → 변경점 → 이분 탐색 → 반증 가능한 가설의 순서로 후보를 절반씩 지워나가는 것이다.
...
- [03-behavioral-star.md](./03-behavioral-star.md) - STAR 포맷과 결과 정량화를 더 깊이 다룬다
