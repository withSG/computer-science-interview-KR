...
같은 팀에서 실력이 갈리는 지점은 아는 명령어 개수가 아닙니다. **후보를 줄이는 순서**다.
...
여기서 로그부터 여는 사람은 후보를 하나도 지우지 못한 채 몇만 줄을 읽기 시작합니다. 반면 "전체 API가 다 느린가, 특정 엔드포인트만인가"를 먼저 물으면 그 질문 하나로 후보의 절반이 사라집니다. 방법론이란 결국 **매 단계마다 후보를 절반씩 지우는 질문의 순서**다.
