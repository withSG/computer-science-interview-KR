...
지금까지의 전략은 전부 **인프라 층위**에서 트래픽을 조절했습니다. 그런데 훨씬 세밀한 조절이 필요할 때가 있습니다. 기능은 완성됐는데 마케팅 일정에 맞춰 특정 날짜에 켜야 합니다. 내부 직원이나 특정 고객사에만 먼저 열어야 할 때도 있습니다. 문제가 생겼을 때 배포 없이 그 기능만 꺼야 하기도 합니다. 이런 요구는 인스턴스 단위 트래픽 전환으로는 못 풉니다. 코드 안에서 조건 분기해야 합니다.
...
배포 전략은 도구가 이미 대부분 제공합니다. 전략 선택 자체에는 시간이 별로 안 듭니다. 실무에서는 **그 전략이 성립할 조건을 갖추느라** 시간을 씁니다. Kubernetes를 쓴다면 Deployment의 RollingUpdate가 기본으로 주어지고, Argo Rollouts를 얹으면 Blue-Green과 Canary를 CRD로 선언할 수 있습니다. AWS에서 ECS나 Lambda를 쓴다면 CodeDeploy가 배포 방식과 자동 롤백 조건을 관리해줍니다. 어떤 도구를 쓰든 다음은 반드시 정해야 합니다.
...
배포 전략은 **트래픽을 얼마나 잘게 나눠 옮기느냐**에서 갈립니다. 어떤 전략을 고르든 실제 무중단은 **우아한 종료·헬스체크·두 버전 호환성**이 결정하고 그중 롤백되지 않는 **DB 스키마**가 가장 자주 발목을 잡습니다.
