...
> Rolling, Blue-Green, Canary, Recreate는 저마다 다운타임과 비용, 롤백 속도 가운데 무엇을 얻고 무엇을 내주는지가 다릅니다. 이 문서는 그 계산으로 전략을 고르는 법과, 무중단 배포를 실제로 망가뜨리는 DB 스키마 문제까지 다룹니다.
...
**두 버전이 절대 공존하면 안 될 때** 씁니다. 배타적 락이 걸린 리소스를 쓰거나, 스키마가 호환되지 않는 변경을 한꺼번에 해야 하는 경우입니다. 내부 관리 도구나 배치 잡처럼 **잠깐의 중단이 문제가 안 되는 시스템**에도 맞습니다. 스테이징 환경이라면 무중단을 흉내 낼 이유부터가 없습니다.
...
추가 자원이 거의 안 듭니다. 별도 도구 없이 기본 기능으로 됩니다. 대신 롤백도 같은 속도로 되감아야 해서 즉시가 아닙니다. 그리고 배포 중 두 버전이 공존하므로 **하위 호환이 깨지면 그 구간의 요청이 실패합니다.**
...
**단점**: 전환 전후 잠깐이지만 인프라를 두 벌 유지해야 합니다. 그리고 **상태를 가진 것**이 걸립니다. 두 환경이 같은 DB를 보므로 스키마 호환 문제는 그대로 남고, 인메모리 세션을 쓰면 전환 순간 사용자가 로그아웃됩니다.
...
> **비유의 한계**: 탄광의 새는 소모품입니다. 카나리 배포의 5%는 **실제 사용자**입니다. "5%니까 괜찮다"가 아니라 "5%로 제한하되 최대한 빨리 판정한다"가 목표입니다. 판정이 느리면 그만큼 오래 5%를 희생시킵니다.
...
**준비 검사가 의존성까지 확인해야 하는가**도 함께 정해야 합니다. DB가 잠깐 느려졌다고 모든 인스턴스의 준비 검사가 동시에 실패하면 서비스 전체가 로드 밸런서에서 빠집니다. 준비 검사는 "내가 요청을 처리할 수 있는가"만 보고, 외부 의존성 장애는 별도 알림으로 다루는 편이 안전합니다.
...
**롤백 가능한 배포를 하려면 스키마 변경도 롤백 가능해야 하는데 파괴적 변경은 롤백이 안 됩니다.** 그래서 파괴적 변경 자체를 하지 않는 방법이 필요합니다.
...
번거로워 보여도 원리 자체는 단순합니다. **어느 시점에 멈춰도, 어느 버전으로 롤백해도 시스템이 동작합니다.**
...
대응은 세 가지입니다. 쓰는 DB와 버전이 그 변경을 온라인 DDL로 처리하는지 미리 확인합니다. MySQL 계열이라면 `pt-online-schema-change`나 `gh-ost` 같은 무중단 스키마 변경 도구를 쓰면 됩니다. 마이그레이션에는 락 타임아웃을 걸어 오래 잡으면 스스로 포기하게 만듭니다.
...
여기서 원칙 하나를 얻습니다. 스키마 변경이 포함된 배포는 "롤백"이 항상 답이 아닙니다. 배포 전에 "이 배포는 롤백 가능한가, 아니면 롤포워드만 가능한가"를 미리 정해두고 그에 맞는 대응 절차를 준비합니다.
...
지금까지의 전략은 전부 **인프라 층위**에서 트래픽을 조절했습니다. 그런데 훨씬 세밀한 조절이 필요할 때가 있습니다. 기능은 완성됐는데 마케팅 일정에 맞춰 특정 날짜에 켜야 합니다. 내부 직원이나 특정 고객사에만 먼저 열어야 할 때도 있고, 문제가 생겼을 때 배포 없이 그 기능만 꺼야 할 때도 있습니다.

이런 요구는 인스턴스 단위 트래픽 전환으로는 못 풉니다. 코드 안에서 조건 분기해야 합니다.
...
배포 자체의 안전성을 확인할 때는 Canary를 씁니다. 특정 기능의 노출을 통제하는 쪽은 Feature Flag가 맡습니다. 실무에서는 둘을 겹쳐 씁니다. Canary로 배포가 안전한지 보고, 그 안에서 개별 기능은 플래그로 엽니다.

### 플래그 부채

Feature Flag는 **코드에 남는 조건 분기**로 비용을 치릅니다. 플래그가 100개 쌓이면 실제 실행 경로 조합이 폭발하고, 아무도 어떤 조합이 검증됐는지 모르게 됩니다. 그래서 플래그를 만들 때 **제거 예정일과 담당자를 함께 기록**하고, 완전히 켜진 지 오래된 플래그는 조건문과 함께 코드에서 삭제합니다. 요금제별 기능 차단처럼 장기 운영이 필요한 플래그와 임시 릴리스 플래그는 구분해서 관리합니다.

---

## 7. 실무에서는

배포 전략은 도구가 이미 대부분 제공합니다. 전략 선택 자체에는 시간이 별로 안 듭니다. 실무에서는 **그 전략이 성립할 조건을 갖추느라** 시간을 씁니다.

Kubernetes를 쓴다면 Deployment의 RollingUpdate가 기본으로 주어지고, Argo Rollouts를 얹으면 Blue-Green과 Canary를 CRD로 선언할 수 있습니다. AWS에서 ECS나 Lambda를 쓴다면 CodeDeploy가 배포 방식과 자동 롤백 조건을 관리해 줍니다. 어떤 도구를 쓰든 다음은 반드시 정해야 합니다.
...
배포 전략은 **트래픽을 얼마나 잘게 나눠 옮기느냐**에서 갈립니다. 어떤 전략을 고르든 실제 무중단은 **우아한 종료·헬스체크·두 버전 호환성**이 결정합니다. 그중에서도 롤백되지 않는 **DB 스키마**가 가장 자주 발목을 잡습니다.
