...
> Rolling·Blue-Green·Canary·Recreate를 외워서 나열하는 수준을 넘어, 다운타임·비용·롤백 속도의 트레이드오프로 전략을 고르고, 무중단 배포를 실제로 망가뜨리는 DB 스키마 문제까지 대응할 수 있게 된다.
...
예전 배포 공지는 이렇게 생겼다. "3월 1일 02:00~03:00 서비스 점검이 있습니다."
...
무중단 배포의 아이디어 자체는 단순하다. **새 버전과 기존 버전을 동시에 띄워두고 트래픽을 옮긴다.** 어떻게 옮기느냐가 전략의 차이다.
...
가장 단순하고, 유일하게 다운타임이 있는 전략이다. 그런데도 쓰는 이유가 있다.

- **두 버전이 절대 공존하면 안 되는 경우.** 배타적 락이 걸린 리소스를 쓰거나, 스키마가 호환되지 않는 변경을 한꺼번에 해야 할 때
- 내부 관리 도구나 배치 잡처럼 **잠깐의 중단이 문제가 안 되는 시스템**
- 스테이징 환경. 무중단을 흉내 낼 이유가 없다

Kubernetes Deployment에서 `strategy.type: Recreate`로 지정한다.
...
- `maxSurge`: 목표 개수보다 몇 개까지 더 띄울 수 있는가. 크면 빠르지만 순간 자원 사용이 늘어난다
- `maxUnavailable`: 몇 개까지 없어도 되는가. 0으로 두면 용량이 절대 줄지 않는다
...
**장점**: 추가 자원이 거의 안 든다. 별도 도구 없이 기본 기능으로 된다.
**단점**: 롤백도 같은 속도로 되감아야 해서 즉시가 아니다. 그리고 배포 중 두 버전이 공존하므로 **하위 호환이 깨지면 그 구간의 요청이 실패한다.**
...
> 비유의 한계: 탄광의 새는 소모품이지만 카나리 배포의 5%는 **실제 사용자**다. "5%니까 괜찮다"가 아니라 "5%로 제한하되 최대한 빨리 판정한다"가 목표다. 판정이 느리면 그만큼 오래 5%를 희생시킨다.

**장점**: 실사용 트래픽으로 검증한다. 스테이징에서 재현 불가능한 문제(실제 데이터 분포, 실제 동시성)를 잡아낸다. 장애 반경이 비율만큼으로 제한된다.
**단점**: 관측 체계가 없으면 성립하지 않는다. "5%를 보냈는데 잘 되고 있나?"를 판단할 지표가 없으면 그냥 느린 Rolling Update일 뿐이다.

Kubernetes 기본 기능만으로는 부족해서 Argo Rollouts나 Flagger 같은 도구, 또는 서비스 메시의 가중치 라우팅을 함께 쓴다.
...
**언제 뭘 쓰나**: 기본값은 Rolling이다. 여기서 "롤백이 5분 걸리는 게 용납 안 되는가?"라면 Blue-Green으로, "이 변경이 실제 트래픽에서 어떻게 될지 모르겠는가?"라면 Canary로 올린다. 전략을 올릴 때마다 필요한 인프라와 관측 요구가 함께 올라간다는 점을 기억한다.

실무에서는 섞어 쓴다. Blue-Green으로 환경을 통째로 만들되 전환을 한 번에 하지 않고 가중치를 5%부터 올리는 방식이 흔하다.

---

## 4. 무중단 배포가 성립하기 위한 전제 조건

전략을 고르기 전에 이것부터 갖춰야 한다. 이게 없으면 어떤 전략을 써도 배포 중 요청이 실패한다.
...
또 하나 중요한 건 **준비 검사가 의존성까지 확인해야 하는가**다. DB가 잠깐 느려졌다고 모든 인스턴스의 준비 검사가 동시에 실패하면 서비스 전체가 로드 밸런서에서 빠진다. 준비 검사는 "내가 요청을 처리할 수 있는가"만 보고, 외부 의존성 장애는 별도 알림으로 다루는 편이 안전하다.
...
Rolling과 Canary는 물론이고 Blue-Green도 전환 구간에는 두 버전이 함께 산다. 그래서 다음이 지켜져야 한다.

- **API 하위 호환**: v2가 v1의 요청 형식을 계속 받아줘야 한다. 필드를 삭제하거나 이름을 바꾸는 변경은 한 번에 하면 안 된다
- **공유 캐시 호환**: v1이 쓴 캐시 값을 v2가 읽어도 깨지지 않아야 한다. 직렬화 포맷이 바뀌면 캐시 키에 버전을 넣는다
- **메시지 큐 호환**: v1이 넣은 메시지를 v2가, v2가 넣은 메시지를 v1이 처리할 수 있어야 한다
- **DB 스키마 호환**: 다음 장 전체가 이 이야기다
...
호환성과는 별개의 문제도 있다. 큰 테이블에 인덱스를 추가하거나 컬럼 타입을 바꾸면, DB 엔진과 버전에 따라 테이블 전체 락이 걸려 그동안 쿼리가 대기한다. 개발 DB에서는 데이터가 적어 1초 만에 끝나던 것이 운영에서는 몇 분간 서비스를 멈춘다.

대응은 세 가지다. 쓰는 DB와 버전이 그 변경을 온라인 DDL로 처리하는지 미리 확인하고, MySQL 계열이라면 `pt-online-schema-change`나 `gh-ost` 같은 무중단 스키마 변경 도구를 쓰고, 마이그레이션에 락 타임아웃을 걸어 오래 잡으면 스스로 포기하게 만든다.
...
지금까지의 전략은 전부 **인프라 층위**에서 트래픽을 조절했다. 그런데 훨씬 세밀한 조절이 필요할 때가 있다. 기능은 완성됐지만 마케팅 일정에 맞춰 특정 날짜에 켜야 하거나, 내부 직원이나 특정 고객사에만 먼저 열어야 하거나, 문제가 생겼을 때 배포 없이 그 기능만 꺼야 하는 경우다. 이런 요구는 인스턴스 단위 트래픽 전환으로는 못 푼다. 코드 안에서 조건 분기해야 한다.
...
**언제 뭘 쓰나**: 배포 자체의 안전성을 확인하려면 Canary, 특정 기능의 노출을 통제하려면 Feature Flag. 실무에서는 둘을 겹쳐 쓴다. Canary로 배포가 안전한지 보고, 그 안에서 개별 기능은 플래그로 연다.

### 플래그 부채

Feature Flag의 비용은 **코드에 남는 조건 분기**다. 플래그가 100개 쌓이면 실제 실행 경로 조합이 폭발하고, 아무도 어떤 조합이 검증됐는지 모르게 된다. 그래서 플래그를 만들 때 **제거 예정일과 담당자를 함께 기록**하고, 완전히 켜진 지 오래된 플래그는 조건문과 함께 코드에서 삭제한다. 장기 운영이 필요한 플래그(요금제별 기능 차단 등)와 임시 릴리스 플래그는 구분해서 관리한다.

---

## 7. 실무에서는

배포 전략은 도구가 이미 대부분 제공한다. 실무에서 실제로 시간을 쓰는 건 전략 선택이 아니라 **그 전략이 성립할 조건을 갖추는 일**이다. Kubernetes를 쓴다면 Deployment의 RollingUpdate가 기본으로 주어지고, Argo Rollouts를 얹으면 Blue-Green과 Canary를 CRD로 선언할 수 있다. AWS에서 ECS나 Lambda를 쓴다면 CodeDeploy가 배포 방식과 자동 롤백 조건을 관리해준다. 어떤 도구를 쓰든 반드시 정해야 하는 것은 다음이다.

- **판정 지표와 임계치**: 무엇을 보고 진행/중단을 결정하는가. 보통 에러율과 p95 지연이 기본이고, 결제 성공률 같은 비즈니스 지표를 함께 본다
- **관찰 시간**: 각 단계에서 얼마나 지켜볼 것인가. 너무 짧으면 문제가 드러나기 전에 다음 단계로 넘어간다
- **자동 롤백 조건**: 사람이 새벽에 대시보드를 보고 있을 수 없다
- **롤백 가능 여부**: 스키마 변경이나 데이터 마이그레이션이 포함됐다면 롤백이 아니라 롤포워드가 답일 수 있다
...
- 꼬리 질문: "Blue-Green에서 DB는 어떻게 하나요?" → 두 환경이 같은 DB를 본다. 그래서 스키마는 두 버전 모두와 호환돼야 하고, 확장-수축 패턴이 필요하다.
...
- 꼬리 질문: "그래도 남는 에러는요?" → 클라이언트 쪽 커넥션 재사용 문제일 수 있다. keep-alive 연결이 서버 종료와 겹치는 경우로, 클라이언트 재시도 정책이 필요하다.
...
- 꼬리 질문: "왜 이렇게까지 번거롭게 하나요?" → 롤백 가능성을 유지하기 위해서다. 배포 중에는 두 버전이 공존하고, DB는 한 벌이며, 데이터는 앱과 달리 되돌릴 수 없다.
...
- 꼬리 질문: "Feature Flag의 단점은요?" → 조건 분기가 코드에 쌓여 실행 경로 조합이 폭발한다. 만들 때부터 제거 시점을 정해두고 다 켜진 플래그는 코드에서 지워야 한다.
...
배포 전략의 차이는 **트래픽을 얼마나 잘게 나눠 옮기느냐**이고, 어떤 전략을 고르든 실제 무중단을 결정하는 것은 **우아한 종료·헬스체크·두 버전 호환성**이며, 그중 가장 자주 발목을 잡는 것이 롤백되지 않는 **DB 스키마**다.
