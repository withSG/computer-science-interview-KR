...
> "연결이 안 된다"는 모호한 신고를 받았을 때, 아래 계층부터 위로 한 단계씩 잘라 들어가 **어디까지 되고 어디부터 안 되는지**를 명령어로 증명할 수 있게 된다.
...
- TCP/IP 계층 개념 (없어도 읽을 수 있게 필요한 부분은 본문에서 설명한다)

---

## 1. 왜 필요한가

운영 중 가장 자주 받는 신고가 "서버에 연결이 안 돼요"다. 그런데 이 한 문장 안에는 서로 완전히 다른 원인이 최소 여섯 가지 섞여 있다.

- 서버 인스턴스 자체가 죽었다
- 인스턴스는 살아 있는데 애플리케이션이 포트를 안 잡고 있다
- 애플리케이션은 떠 있는데 `127.0.0.1`에만 바인딩돼서 외부에서 못 붙는다
- 방화벽/보안 그룹이 막고 있다
- DNS가 옛날 IP를 돌려주고 있다
- TCP는 붙는데 HTTP 응답이 502다

이걸 감으로 찍으면 매번 다른 순서로 헤매게 된다. **정해진 순서로 아래 계층부터 하나씩 배제**하면 대부분 3~4개 명령 안에 범위가 절반 이하로 줄어든다. 이 문서의 목적은 그 순서를 몸에 붙이는 것이다.

**비유**: 전화가 안 걸릴 때 확인 순서와 같다. 전화선이 꽂혀 있나(L1/L2) → 우리 집 번호가 있나(L3) → 전화번호부에서 상대 번호를 찾을 수 있나(DNS) → 신호가 가나(L4) → 받았는데 말이 통하나(L7).
**비유의 한계**: 실제 네트워크는 계층이 깔끔하게 독립적이지 않다. MTU 불일치처럼 L3 설정 때문에 L7 큰 응답만 깨지는 문제도 있어서, 계층 순서는 "탐색 전략"이지 "인과의 순서"는 아니다.
...
**그래서 언제 뭘 쓰나**: 위에서부터 순서대로 쓰되, "어디까지 정상인지"가 확인되면 그 아래는 다시 안 본다. 반대로 위 계층에서 이상하면 아래 계층으로 되돌아가지 말고 그 계층 안에서 원인을 좁힌다.

---

## 4. L2~L3 — 인터페이스와 경로

`ifconfig`, `route`, `netstat`은 오래된 net-tools 계열이고, 현재 표준은 iproute2의 `ip`와 `ss`다. 최소 설치 이미지나 컨테이너에는 아예 `ifconfig`가 없는 경우가 많으므로 `ip` 계열을 손에 익히는 편이 낫다.
...
`default` 라우트가 없으면 같은 서브넷 안은 통신되는데 인터넷만 안 된다. 프라이빗 서브넷에 NAT 게이트웨이를 안 붙였을 때 나타나는 전형적인 증상이다.

`ip -s link`의 `errors`, `dropped` 카운터가 계속 증가한다면 애플리케이션이 아니라 물리/가상 NIC 레벨 문제를 의심한다.
...
`ping`은 ICMP Echo를 쓴다. 여기서 가장 중요한 사실은 **ping 실패가 곧 연결 불가가 아니라는 것**이다. 많은 방화벽과 클라우드 보안 그룹이 ICMP를 기본 차단한다. 실제로 AWS 보안 그룹에서 ICMP를 열지 않으면 TCP 80은 멀쩡히 붙는데 ping만 안 된다. 그러니 ping은 "되면 좋고, 안 돼도 TCP로 다시 확인"하는 도구다.

반대로 MTU 검증에는 강력하다. `-M do`(조각화 금지)로 크기를 키워가며 보내다가 특정 크기부터 실패하면 경로상의 MTU가 그보다 작다는 뜻이다. VPN이나 오버레이 네트워크에서 "작은 요청은 되는데 큰 응답만 멈춘다"는 증상의 원인이 대개 이것이다.
...
traceroute는 TTL을 1부터 올려가며 패킷을 보내고, TTL이 0이 된 지점의 라우터가 돌려주는 ICMP Time Exceeded로 경로를 그린다. 중간에 `* * *`가 나오는 것은 그 라우터가 ICMP 응답을 안 하도록 설정된 경우가 많아서, **`*`가 곧 장애는 아니다.** 그 이후 홉이 정상적으로 나오면 그냥 조용한 라우터일 뿐이다. 반대로 특정 홉 이후 끝까지 `*`라면 그 지점부터 막힌 것이다.

경로 문제를 판단할 때는 `mtr`이 더 낫다. 한 번의 스냅샷이 아니라 반복 측정으로 홉별 손실률과 지연을 같이 보여주기 때문에, 순간적인 튐과 지속적인 손실을 구분할 수 있다.
...
- `NXDOMAIN`: 그런 도메인이 없다. 오타 또는 미등록
- `SERVFAIL`: 네임서버가 응답을 못 만들었다. DNS 쪽 장애
- `NOERROR`인데 ANSWER 비어 있음: 도메인은 있는데 그 레코드 타입이 없다

**여기서 초심자가 가장 많이 속는 지점이 있다. `dig`는 `/etc/hosts`를 보지 않는다.** dig는 DNS 프로토콜로 직접 질의하는 도구이고, 애플리케이션은 `/etc/nsswitch.conf` 순서에 따라 `/etc/hosts`를 먼저 본 다음 DNS로 간다. 그래서 "dig는 잘 되는데 애플리케이션만 엉뚱한 IP로 붙는" 상황이 생긴다. 애플리케이션과 같은 경로로 확인하려면 이걸 쓴다.
...
DNS 레코드를 바꿨는데 반영이 안 된다면 십중팔구 **TTL 동안 캐시가 살아 있는 것**이다. `dig`의 ANSWER에 찍힌 TTL 값이 남은 캐시 시간이다. 그래서 IP 변경 작업 전에는 TTL을 미리 낮춰두는 것이 정석이다.

---

## 7. ss — 소켓 상태와 포트 점유

`ss`는 커널의 소켓 정보를 직접 읽어서 `netstat`보다 훨씬 빠르다. 연결이 수만 개인 서버에서 차이가 크게 난다.
...
`Address already in use`로 애플리케이션이 안 뜰 때의 표준 절차다.
...
`[::]`로 떠 있는데 IPv4로 붙는 경우가 있다. 커널의 `net.ipv6.bindv6only`가 꺼져 있으면 IPv6 소켓 하나가 IPv4 매핑 주소까지 함께 받기 때문이다. 그래서 `0.0.0.0` 줄이 없다고 곧바로 "IPv4는 안 열렸다"고 단정하면 안 된다.

"로컬에서 curl은 되는데 밖에서 안 된다"의 1순위 원인이 `127.0.0.1` 바인딩이다. 스프링 부트라면 `server.address`, 도커라면 `-p 8080:8080` 대신 `-p 127.0.0.1:8080:8080`을 쓴 경우다.
...
**CLOSE-WAIT 누적은 거의 항상 코드 문제다.** 상대가 FIN을 보냈으면 우리 애플리케이션이 소켓을 닫아야 하는데 그러지 않았다는 뜻이라, 커넥션 풀에 반납이 안 되거나 예외 경로에서 `close()`를 빠뜨린 것이다. 서버를 재시작하면 잠깐 사라졌다가 반드시 다시 쌓인다.

**TIME-WAIT는 대부분 정상이다.** 먼저 연결을 끊는 쪽에 남는 정상적인 상태이고, 늦게 도착한 패킷이 다음 연결에 섞이지 않도록 보호하는 장치다. 수만 개라면 커넥션을 재사용하지 않고 매 요청마다 새로 맺고 있다는 뜻이므로, 커널 파라미터를 만지기 전에 **HTTP Keep-Alive와 커넥션 풀 도입**을 먼저 검토한다. 인터넷에 흔히 돌아다니는 `tcp_tw_recycle` 튜닝은 따라 하지 않는 편이 낫다. NAT 뒤의 클라이언트들이 무작위로 연결에 실패하는 부작용이 있어서 리눅스 4.12에서 아예 제거된 옵션이다.

### Recv-Q / Send-Q 읽는 법

이 두 컬럼은 소켓 상태에 따라 뜻이 다르다.

- **LISTEN 소켓**: Recv-Q = accept 대기 중인 연결 수, Send-Q = backlog 최대치. Recv-Q가 Send-Q에 근접하면 애플리케이션이 `accept()`를 못 따라가고 있다는 뜻이고, 그 이후 연결은 버려진다.
- **ESTABLISHED 소켓**: Recv-Q = 애플리케이션이 아직 안 읽은 바이트, Send-Q = 상대가 아직 ACK 안 한 바이트. Recv-Q가 계속 크면 애플리케이션 스레드가 블로킹돼 있다는 신호다.
...
마지막 줄은 `nc`가 없는 최소 이미지에서 유용하다. bash 내장 기능으로 TCP 연결을 시도한다.
...
**refused와 timeout의 구분이 진단의 분기점이다.** refused는 "집에 도착했는데 문이 없다", timeout은 "집까지 가지도 못했다"에 해당한다. 전자는 서버 안쪽, 후자는 네트워크 경로를 본다.
...
`*`는 curl의 진단 메시지, `>`는 보낸 것, `<`는 받은 것이다. 어느 줄까지 나왔는지만 봐도 어디서 끊겼는지 알 수 있다. `Trying`에서 멈추면 네트워크, `Connected` 후 멈추면 TLS나 서버 응답 문제, `< HTTP/1.1 5xx`가 나오면 이미 L7 영역이다.
...
마지막 `--resolve`가 특히 유용하다. **DNS는 그대로 두고 특정 서버로만 붙여서** 테스트할 수 있다. 로드밸런서 뒤의 특정 인스턴스만 이상할 때, Host 헤더와 TLS SNI는 원래 도메인 그대로 유지하면서 대상 IP만 바꿔 검증하는 방법이다.

### -w 로 시간 분해하기

"느리다"는 신고를 받았을 때 어느 구간이 느린지 숫자로 쪼갠다.
...
모든 도구가 애매한 답을 줄 때 패킷을 직접 본다. 패킷은 해석의 여지가 없다.
...
1. **반드시 필터를 걸고 `-c`로 개수를 제한한다.** 무필터 캡처는 그 자체로 부하이고 디스크를 채운다.
2. **`-A`는 신중히.** 평문 트래픽이면 토큰과 비밀번호가 화면과 로그에 그대로 남는다.
3. **`-w`로 저장하고 분석은 Wireshark에서.** 터미널에서 읽는 것보다 훨씬 빠르다.
4. root 또는 `CAP_NET_RAW` 권한이 필요하다.
5. 캡처 파일 자체가 민감 정보다. 분석 후 삭제한다.

---

## 11. 실무에서는

**컨테이너 안에는 도구가 없다.** 슬림 이미지에는 `ip`, `ss`, `curl`이 아예 없다. 그래서 진단 도구를 모아둔 이미지를 사이드카나 임시 디버그 컨테이너로 붙여서 같은 네트워크 네임스페이스에서 명령을 실행하는 방식을 쓴다. 쿠버네티스의 `kubectl debug`가 이 목적으로 나온 기능이다. 여기서 중요한 것은 **파드의 네트워크 네임스페이스를 공유해야 의미가 있다**는 점이다. 다른 네임스페이스에서 `ss`를 쳐봐야 그 파드의 소켓은 안 보인다.

**클라우드에서는 방화벽이 두 겹이다.** 보안 그룹(인스턴스 단위)과 네트워크 ACL(서브넷 단위), 그리고 OS 내부의 iptables/ufw까지 있다. 어느 한 곳만 막혀도 `nc`는 타임아웃이 난다. 그래서 서버 안에서 `curl localhost:8080`은 되는데 밖에서 안 되면, 바인드 주소 → OS 방화벽 → 보안 그룹 → 네트워크 ACL 순으로 확인한다. 자세한 방화벽 구조는 [보안 기초](./05-security-basics.md)에서 다룬다.

**로드밸런서 헬스체크 실패.** ALB가 타겟을 unhealthy로 표시할 때는 헬스체크 경로를 그대로 흉내 내본다. `curl -v -H "Host: api.example.com" http://<인스턴스IP>:8080/health`처럼 LB가 보내는 것과 동일한 요청을 인스턴스에 직접 쏴서, 애플리케이션이 200을 주는지 확인하는 것이 첫 단계다.

**DNS 캐시.** JVM은 과거에 DNS 결과를 무기한 캐시하는 설정이 기본인 시절이 있었다. 그래서 RDS 페일오버로 엔드포인트가 새 IP를 가리켜도 애플리케이션이 옛 IP로 계속 붙는 사고가 유명하다. 애플리케이션 레벨 DNS 캐시 정책은 클라우드에서 반드시 확인해야 할 항목이다.
...
- 꼬리 질문: "`ping`이 안 되는데 웹은 되는 경우가 있나요?" → 있다. ICMP는 대부분의 방화벽과 보안 그룹에서 기본 차단되므로 ping 실패만으로 도달 불가를 단정할 수 없다.
...
- 꼬리 질문: "TIME_WAIT가 많은 것도 문제인가요?" → 대부분 정상이다. 먼저 close한 쪽에 남는 보호 장치이며, 많다면 커넥션을 재사용하지 않는다는 뜻이므로 Keep-Alive와 커넥션 풀을 먼저 본다. `tcp_tw_recycle` 같은 커널 옵션은 NAT 환경에서 문제를 일으키므로 쓰지 않는다.
...
- 꼬리 질문: "로드밸런서 뒤에 인스턴스가 여러 대인데 일부만 느립니다" → `curl --resolve`로 도메인과 TLS SNI는 그대로 둔 채 대상 IP만 바꿔가며 인스턴스별로 같은 요청을 보내 비교한다. 특정 인스턴스에서만 TTFB가 튄다면 LB가 아니라 그 인스턴스를 본다.
...
- 꼬리 질문: "컨테이너 안에는 tcpdump가 없는데요?" → 파드나 컨테이너의 네트워크 네임스페이스를 공유하는 임시 디버그 컨테이너를 붙여서 캡처한다. 네임스페이스를 공유하지 않으면 다른 곳의 트래픽을 보는 셈이라 의미가 없다.
...
| 서버에서 `curl localhost`가 되니 외부도 될 것이다 | 루프백은 바인드 주소·방화벽을 우회한다 | 바인드 주소와 외부에서의 접속을 따로 확인 |
...
네트워크 진단은 도구 암기가 아니라 **아래 계층부터 하나씩 배제해 "어디까지 정상인지"를 증명하는 절차**이며, 각 도구는 그 절차의 특정 계층을 담당하는 검증기다.
