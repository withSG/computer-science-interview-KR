...
네 가지 모두 이 문서에서 다루는 **디렉터리 구조 / 권한 / 프로세스 / 스트림**의 문제다. 이 넷은 리눅스 트러블슈팅의 좌표계라서, 여기가 흔들리면 그 위의 모든 진단이 추측이 된다.
...
리눅스는 모든 것을 하나의 트리로 표현한다. 윈도우처럼 `C:`, `D:` 같은 드라이브 문자가 없고, 디스크를 추가하면 트리의 어느 지점(`/data` 같은)에 **마운트(mount)** 해서 붙인다. 이 트리의 어느 디렉터리에 무엇을 두는지를 정한 규약이 FHS다.
...
`777`은 "권한 문제 같으니 일단 다 열자"의 결과물인데, 그 순간 누구나 그 파일을 덮어쓸 수 있게 된다. 웹 서버가 쓰기 가능한 디렉터리에 777을 주면 업로드된 스크립트가 실행 파일이 되는 경로가 열린다. 권한 에러가 나면 **막힌 쪽을 정확히 찾아서 최소한만 여는 것**이 정답이다.

### 디렉터리에서는 의미가 달라진다

디렉터리는 "이름 → inode" 매핑 테이블이다. 그래서 권한도 그 테이블 기준으로 해석된다.
...
2. **파일 삭제 권한은 파일이 아니라 그 파일이 든 디렉터리의 `w`가 결정한다.** 읽기 전용(444) 파일도 디렉터리에 쓰기 권한이 있으면 지워진다. 이걸 막으려고 나온 것이 뒤에 나올 Sticky Bit다.
...
`chmod -R 755`가 위험한 이유가 여기 있다. 디렉터리에 `x`를 주려고 재귀로 돌리면 그 안의 `.jar`, `.conf`, `.jpg` 전부가 실행 파일이 된다. 이럴 때 대문자 `X`를 쓴다.
...
여러 사람이 `/shared/team`에 파일을 만들면, 기본적으로 각자의 기본 그룹이 붙는다. 그러면 alice가 만든 파일을 bob이 못 고친다. SGID를 디렉터리에 걸면 **그 안에서 생성되는 파일은 무조건 디렉터리의 그룹을 물려받는다**. 협업 디렉터리에서 사실상 필수다.
...
**컨테이너의 사용자.** 컨테이너를 root로 돌리면 호스트 볼륨에 root 소유 파일이 생겨 호스트 쪽에서 지우지도 못하는 상황이 생긴다. `USER` 지시어로 비루트 사용자를 쓰고, 볼륨 디렉터리의 소유권을 맞춰주는 것이 기본이다.
...
- 꼬리 질문: "읽기 전용 파일을 지울 수 있나요?" → 삭제는 파일이 아니라 그 파일이 든 디렉터리의 쓰기 권한으로 결정되므로 지울 수 있다. 이를 막으려고 `/tmp`처럼 공유 디렉터리에 Sticky Bit를 건다.
