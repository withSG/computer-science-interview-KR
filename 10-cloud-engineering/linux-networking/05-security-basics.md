...
**OpenSSH는 개인키가 다른 사용자에게 읽힐 수 있으면 접속을 거부합니다.** 남이 읽을 수 있는 개인키는 이미 유출된 것으로 간주하는 정책입니다. 서버 쪽도 마찬가지라, `~/.ssh`가 700이 아니거나 홈 디렉터리가 그룹 쓰기 가능하면 키 인증이 조용히 실패합니다. CI에서 키 파일을 내려받은 뒤 `chmod 600`을 빠뜨려 실패하는 일이 매우 흔합니다.
