...
> **어떻게 들어오는가(SSH), 무엇을 통과시키는가(방화벽), 들어온 뒤 무엇을 할 수 있는가(권한)**, 이 세 축으로 공개 인터넷의 리눅스 서버를 잠글 수 있게 됩니다.
...
퍼블릭 IP를 가진 서버를 띄우고 아무에게도 알리지 않아도, **몇 분 안에 SSH 로그인 시도가 들어옵니다.** 인터넷 전체를 훑는 자동 스캐너가 상시 돌고 있기 때문입니다. `journalctl -u sshd | grep 'Failed password'`를 쳐 보면 `root`, `admin`, `test`, `ubuntu` 같은 계정으로 수천 건이 쌓여 있는 것을 볼 수 있습니다.

서버 보안의 출발점은 거창하지 않습니다. **가장 많이 두드리는 문을 잠그는 일**이 먼저입니다. 실제로 침해 사고의 상당수는 정교한 공격이 아니라 아래 셋 중 하나입니다.
...
**비유의 한계**: 아파트는 물리적으로 순서대로 통과할 수밖에 없습니다. 서버에는 애플리케이션 취약점처럼 **바깥 겹을 건너뛰고 안쪽에 바로 도달하는 경로**가 있습니다. 그래서 계층 방어는 확률을 낮추는 장치일 뿐, 완전한 순차 관문이 아닙니다.
...
1. **무한히 시도할 수 있습니다.** 공격자는 초당 수십 번씩 사전 공격을 돌립니다. 사람이 외울 만한 비밀번호는 결국 뚫립니다.
...
**개인키가 한 번도 전송되지 않는다**는 데 핵심이 있습니다. 서버가 나중에 털려도 거기 남는 것은 공개키뿐이라 다른 서버에 재사용할 수 없습니다. 게다가 키의 엔트로피가 사람이 만드는 비밀번호와 비교가 안 되게 커서, 무차별 대입은 현실적으로 불가능합니다.
...
설정 파일은 `/etc/ssh/sshd_config`입니다. 최근 배포판은 `/etc/ssh/sshd_config.d/*.conf`로 조각 파일을 두는 방식을 권장하므로, 원본을 통째로 고치기보다 조각 파일을 추가하는 편이 업그레이드에 안전합니다.
...
sshd 설정을 잘못 고치고 재시작했다가 서버에 못 들어가는 일은 초심자가 가장 흔하게 겪는 사고입니다. 순서를 지키면 막을 수 있습니다.
...
`sshd -t`는 문법만 검사합니다. 문법은 맞는데 논리가 틀린 경우(예: 자기 계정을 `AllowUsers`에서 빠뜨림)는 잡아 주지 못하니, 5번의 실접속 테스트가 반드시 필요합니다.
...
위 규칙에서 두 번째 줄이 특히 중요합니다. **conntrack 규칙이 없으면 우리가 밖으로 보낸 요청의 응답도 막힙니다.** 패키지 설치도 DNS 조회도 안 되어 서버가 사실상 고립됩니다.

첫 줄과 마지막 줄은 순서가 관건입니다. 원격에서 작업하면서 `iptables -P INPUT DROP`을 먼저 실행하면 그 순간 자기 세션이 끊깁니다. **허용 규칙을 모두 넣은 다음에 기본 정책을 바꿉니다.**
...
**iptables 규칙은 메모리에만 있어서 재부팅하면 사라집니다.** 따로 저장해야 합니다.
...
최신 배포판은 내부를 nftables로 바꾸는 중이라 `iptables` 명령이 호환 계층으로 동작하는 경우가 많습니다. 실제 규칙은 `nft list ruleset`으로 확인할 수 있습니다.
...
둘 중 하나만 쓰면 안 되냐는 질문이 자주 나옵니다. 클라우드 콘솔에서 실수로 보안 그룹을 열어 버리는 사고가 실제로 흔하다는 점을 떠올리면 답이 나옵니다. 다만 규칙을 두 군데 관리하는 부담이 있으니, **경계는 보안 그룹으로 관리하고 OS 방화벽은 최소한의 안전망으로 단순하게 유지**하는 절충이 일반적입니다.

디버깅할 때는 **어느 겹에서 막혔는지 구분**하는 일이 관건입니다. `nc -zv`가 타임아웃이면 보안 그룹이나 OS 방화벽에서 DROP된 것이고, `Connection refused`면 패킷은 도달했으니 방화벽은 통과했고 LISTEN이 없는 것입니다. 이 구분법은 [네트워크 진단 명령어](./02-networking-commands.md)에서 자세히 다룹니다.
...
각 주체에게 업무에 필요한 최소한의 권한만 준다는 원칙이 **최소 권한(Principle of Least Privilege)** 입니다. 목적은 사고를 막는 것보다 **사고의 영향 범위를 좁히는 것**에 가깝습니다. 계정 하나가 털렸을 때 그 계정이 서버 전체를 만질 수 있는지 로그 디렉터리만 볼 수 있는지에 따라 사고의 크기가 갈립니다.
...
`NOPASSWD`를 `ALL`에 주면 사실상 root 계정을 하나 더 만드는 셈입니다. 배포 자동화 때문에 필요하다면 **그 자동화가 실제로 실행하는 명령만** 나열합니다. 명령 경로에 와일드카드를 쓰면 우회 여지가 생기므로 절대 경로로 정확히 적습니다.

sudo 사용 기록은 인증 로그에 남습니다(배포판에 따라 `/var/log/auth.log` 또는 `/var/log/secure`, 또는 `journalctl`). 사고를 조사할 때 가장 먼저 보는 로그이므로, 로그가 서버 밖으로 수집되고 있는지도 확인해 둡니다. 서버가 털리면 서버 안의 로그는 신뢰할 수 없기 때문입니다.
...
보안 업데이트만 자동 적용하는 장치도 있습니다. Debian/Ubuntu의 `unattended-upgrades`, RHEL 계열의 `dnf-automatic`이 그것입니다. 운영 서버에 자동 업데이트를 거는 것을 두고는 의견이 갈리는데, 판단 기준은 이렇습니다. **재기동 없이 적용되는 보안 패치는 자동으로, 커널처럼 재부팅이 필요한 것은 점검 창에서 계획적으로.** 커널 업데이트는 재부팅하기 전까지 적용되지 않는데, 이 점을 잊기 쉽습니다.
...
`ss -tlnp` 결과를 한 줄씩 보면서 "이게 왜 외부로 열려 있지?"를 자문하는 것이 좋은 점검입니다. 자주 걸리는 것은 개발용으로 켠 관리 콘솔, 바인드 주소를 `0.0.0.0`으로 둔 DB, 디버그 포트, 모니터링 익스포터입니다. 꼭 필요하다면 최소한 `127.0.0.1`로 바인딩하거나 방화벽에서 소스를 제한합니다.
...
A. 체인의 규칙은 위에서 아래로 평가되고 첫 번째로 일치하는 규칙에서 처리가 결정되기 때문입니다. 그래서 앞쪽에 광범위한 DROP을 두면 뒤에 아무리 ACCEPT를 추가해도 도달하지 못합니다. 원격 작업 중에 기본 정책을 DROP으로 먼저 바꾸면 그 순간 자기 세션이 끊기므로, 루프백 허용과 ESTABLISHED,RELATED 허용을 포함한 필요한 ACCEPT를 모두 넣은 다음에 기본 정책을 바꿉니다. conntrack 규칙이 없으면 우리가 보낸 요청의 응답까지 막혀서 서버가 고립됩니다. iptables 규칙은 메모리에만 있으므로 별도로 저장하지 않으면 재부팅 시 사라집니다.
