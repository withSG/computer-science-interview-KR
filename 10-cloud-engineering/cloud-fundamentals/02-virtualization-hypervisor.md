...
초기에는 이 가로채기를 순수 소프트웨어로 구현하느라 비용이 컸고 지금은 CPU가 게스트 전용 실행 모드를 따로 제공해 훨씬 싸게 처리합니다(4절). 여기에 CPU·메모리 시분할, 가상 NIC과 가상 디스크 제공, VM별 자원 상한 관리가 얹힙니다.
...
- **격리 경계의 이동**: 검사와 통제가 호스트 소프트웨어에서 별도 하드웨어로 내려가므로 공격 표면이 줄어든다
