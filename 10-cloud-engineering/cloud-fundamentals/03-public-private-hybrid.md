...
> 왜 모두가 퍼블릭 클라우드로 가지 않는지, 규제·데이터 주권·락인·비용이 각각 어떤 선택을 강제하는지, 그리고 기존 시스템을 옮길 때 쓰는 6R 전략을 설명할 수 있게 됩니다.
...
1편에서 클라우드가 온프레미스의 세 가지 벽을 넘었다고 했습니다. 그러면 왜 아직도 자체 데이터센터를 운영하는 회사가 있을까? 이유는 감성이나 관성이 아니라 네 가지 구체적 제약입니다.
...
**락인(Vendor Lock-in).** 한 벤더의 관리형 서비스에 깊이 들어갈수록 나가기 어려워집니다. 협상력이 사라지고, 그 벤더의 대형 장애가 곧 우리 서비스의 대형 장애가 됩니다.

**비용.** 부하가 평탄하고 규모가 크며 수년간 안정적이라면 자체 구축이 더 쌀 수 있습니다. 실제로 크게 성장한 뒤 일부 워크로드를 자체 인프라로 되돌린 사례가 알려져 있습니다.

이 네 가지가 서로 다른 강도로 걸리기 때문에, 정답이 하나가 아니라 **배포 모델(Deployment Model)** 이라는 선택지가 생깁니다.
...
AWS, Azure, GCP, NCP처럼 벤더가 소유·운영하는 인프라를 불특정 다수 고객이 **공유**해서 씁니다. 물리 서버 한 대 위에 여러 고객의 가상 머신이 함께 돌아가는 **멀티테넌시(Multi-tenancy)** 가 전제입니다.

여기서 단가가 낮아지는 원리를 이해해야 합니다. 고객마다 트래픽 피크 시각이 다르기 때문에, 자원을 풀로 모으면 **전체 풀의 피크는 개별 피크의 합보다 훨씬 작다.** 벤더는 이 차이만큼 적은 하드웨어로 같은 수요를 감당하고, 그 이득의 일부를 가격에 반영합니다. 개별 회사가 자기 피크에 맞춰 서버를 사는 방식으로는 절대 나올 수 없는 효율입니다.

> **비유** — 공유 오피스와 같습니다. 회의실이 필요할 때만 예약하고 시간당 요금을 냅니다. 회사마다 회의 시간대가 다르니 건물 전체로 보면 회의실 개수가 입주사 수보다 훨씬 적어도 됩니다.
>
> **비유의 한계** — 공유 오피스는 옆 회사가 나가도 내 자리가 안 바뀌지만, 클라우드에서는 같은 물리 호스트를 쓰는 다른 테넌트의 부하가 내 성능에 영향을 줄 수 있다(이른바 noisy neighbor). 이걸 막으려면 전용 호스트 옵션처럼 격리 수준을 올려야 하고, 그러면 단가 이점이 줄어듭니다.
...
가장 많이 헷갈리는 지점입니다. **서버를 우리 건물에 두는 것 자체는 프라이빗 클라우드가 아니다.** 1편에서 본 5대 특성 — 셀프서비스, 네트워크 접근, 자원 풀링, 탄력성, 계량 — 이 갖춰져야 클라우드입니다.
...
또 하나. 프라이빗 클라우드가 반드시 우리 건물에 있을 필요는 없습니다. **호스티드 프라이빗**은 벤더 데이터센터에 있되 하드웨어가 우리 조직 전용으로 격리된 형태입니다. 판단 기준은 위치가 아니라 **단독 테넌시 여부입니다.
...
**1) 연결 구간이 새로운 단일 장애점(SPOF)이 된다.** 전용선 한 가닥으로 연결해 두면 그 회선이 곧 서비스의 목숨줄입니다. 그래서 실무에서는 서로 다른 물리 경로의 회선 이중화, 또는 전용선 + VPN 백업 구성을 씁니다.
...
**3) 클라우드 버스팅의 현실.** "평소엔 온프레미스, 피크 때만 퍼블릭으로 넘친다"는 아이디어는 매력적이지만, 이미지·설정·시크릿·모니터링을 양쪽에 동기화해 두어야 하고 데이터 접근 지연이 걸립니다. 상태를 거의 안 가지는 워크로드(렌더링, 배치 계산)에서는 잘 동작하지만, 상태를 많이 가진 트랜잭션 서비스에서는 기대만큼 매끄럽지 않습니다.
...
그래서 실무에서 자주 쓰는 절충안은 **"핵심 워크로드는 한 클라우드에, 이식성은 추상화 계층으로 확보"** 다. 컨테이너와 쿠버네티스, Terraform 같은 IaC 도구가 그 계층 역할을 합니다. 다만 이게 만능은 아닙니다. 다음 절을 보자.

### 락인은 어느 계층에서 생기는가

"락인"을 뭉뚱그리면 대응할 수 없습니다. 계층별로 이동 난이도가 완전히 다릅니다.
...
핵심은 **"멀티 클라우드는 기본값이 아니다"** 는 점입니다. 명확한 사업적 이유 없이 "혹시 모르니까" 두 클라우드를 쓰는 결정은 대개 손해로 끝납니다.
...
실무 순서도 정해져 있습니다. 먼저 **인벤토리를 만들고**(무엇이 몇 개나 돌고 있는지 아무도 모르는 경우가 대부분이다), Retire와 Retain으로 대상을 줄인 뒤, 남은 것에 Rehost/Replatform/Repurchase/Refactor를 배분합니다. 처음부터 전부 Refactor하겠다는 계획은 거의 실패합니다.

> **비유** — 이사와 같습니다. 짐을 통째로 트럭에 싣는 게 Rehost, 낡은 가전만 새로 사는 게 Replatform, 살림살이를 정리하고 구조를 새로 짜는 게 Refactor, 안 쓰는 물건을 버리는 게 Retire다.
>
> **비유의 한계** — 이사는 끝나면 끝이지만 마이그레이션은 옮긴 뒤에 운영 모델·비용 구조·조직 역할까지 바꿔야 완결됩니다. 옮겨만 놓고 온프레미스 시절 습관대로 운영하면 비용만 오르고 이점은 안 생깁니다.

---

## 8. 실무에서는 — 하이브리드 연결이 무너질 때

**증상.** 매일 새벽 도는 정산 배치가 평소 20분에 끝나던 것이 세 시간 넘게 걸리기 시작했습니다. 온프레미스에서 도는 배치가 클라우드 VPC 안의 DB를 조회하는 구조입니다. 애플리케이션 로그에는 에러가 없고 그냥 느립니다.
...
**원인.** 전용선이 벤더 유지보수로 내려가면서 트래픽이 백업 IPsec VPN(공용 인터넷 경유)으로 넘어갔습니다. 경로가 살아 있으니 에러는 없지만 왕복 지연이 크게 늘었습니다. 문제는 배치 쿼리가 행 단위로 왕복하는 패턴이라, **지연 증가가 왕복 횟수만큼 곱해져** 전체 소요 시간이 폭증했습니다.

**대응.**

1. **즉시**: 배치를 일시 중단하고 전용선 복구 후 재실행하거나, 우선순위 낮은 단계를 건너뜁니다.
2. **단기**: 왕복 횟수를 줄입니다. 행 단위 조회를 배치 페치로 바꾸고, 집계는 DB 쪽에서 끝내서 결과만 가져옵니다.
3. **구조적**: 회선을 서로 다른 물리 경로로 이중화합니다. 그리고 **지연에 민감한 연산은 데이터가 있는 쪽에 배치한다** — 배치 잡 자체를 클라우드로 옮기는 것이 근본 해법입니다.
4. **관측**: 구간 지연과 회선 상태에 알람을 겁니다. "느려짐"은 에러 로그에 안 남으므로 지표로만 잡을 수 있습니다.

**교훈.** 하이브리드에서 네트워크 구간은 **새로 생긴 SPOF이자 성능 변수입니다. 온프레미스 안에서는 무시해도 됐던 왕복 지연이, 경계를 넘는 순간 아키텍처의 제약 조건이 됩니다.
...
배포 모델 선택은 기술 취향이 아니라 **규제·데이터 주권·부하 패턴·락인 리스크**라는 네 개의 제약을 어떤 순서로 푸느냐의 문제이며, 멀티 클라우드는 기본값이 아니라 명확한 사업적 이유가 있을 때만 지불할 가치가 있는 비용입니다.
