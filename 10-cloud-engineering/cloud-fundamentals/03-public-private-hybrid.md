...
**3) 클라우드 버스팅의 현실.** "평소엔 온프레미스, 피크 때만 퍼블릭으로 넘친다"는 아이디어는 매력적이지만 이미지·설정·시크릿·모니터링을 양쪽에 동기화해 두어야 하고 데이터 접근 지연이 걸립니다. 상태가 거의 없는 워크로드(렌더링, 배치 계산)에서는 잘 동작하지만 상태가 많은 트랜잭션 서비스에서는 기대만큼 매끄럽지 않습니다.
