...
> 서버를 키울 것인가 늘릴 것인가부터, 가용성 99.99%가 실제로 몇 분의 다운타임인지, SPOF를 어떻게 찾아내고 RTO/RPO에 맞춰 DR 전략을 고르는지까지 설명할 수 있게 됩니다.
...
서버 한 대로 서비스를 시작하면 두 가지 사건이 반드시 찾아옵니다.

**첫째, 트래픽이 늘어 감당이 안 되는 순간.** 응답이 느려지고, 큐가 쌓이고, 타임아웃이 번지고, 재시도가 부하를 더 키웁니다. 이때 필요한 것이 **확장성(Scalability)** — 자원을 늘려 처리량을 올릴 수 있는 성질입니다.
...
두 개념은 자주 함께 묶이지만 목표가 다릅니다. 확장성은 **처리량**의 문제이고, 가용성은 **연속성**의 문제입니다. 서버를 100대로 늘려도 전부 같은 랙에 있으면 확장성은 얻고 가용성은 못 얻습니다. 반대로 이중화만 해두고 용량이 부족하면 죽지는 않지만 느려서 못 씁니다.

> **비유** — 식당의 셰프와 같습니다. 주문이 밀리면 셰프를 더 뽑거나(수평 확장) 한 명을 더 숙련되게 만든다(수직 확장). 그런데 오븐이 하나뿐이면 셰프를 아무리 늘려도 처리량은 오븐에서 막힙니다. 이 오븐이 시스템의 병목이자 SPOF다.
>
> **비유의 한계** — 셰프는 옆 사람이 쓰러지면 알아서 일을 넘겨받지만, 서버는 아무도 그런 판단을 대신 해주지 않습니다. 헬스체크와 페일오버를 **명시적으로 만들어 두어야** 그 일이 일어납니다.
...
수직 확장은 코드를 안 고쳐도 되므로 가장 손쉬운 첫 수단입니다. 하지만 네 가지 벽이 있습니다.
...
4. **가용성이 전혀 개선되지 않는다**: 아무리 큰 서버도 한 대는 한 대입니다. SPOF는 그대로 남습니다.
...
그리고 무엇보다 **병목이 공유 자원에 있으면 늘려도 소용없다.** 웹 서버를 10대로 늘려도 모두 같은 DB를 때리고 있으면 처리량은 DB에서 막힙니다. 확장 작업을 하기 전에 **어디가 병목인지부터 측정**해야 하는 이유입니다. 직렬로 처리될 수밖에 없는 구간의 비중이 클수록 서버를 늘려도 얻는 이득이 빠르게 줄어든다(암달의 법칙).
...
여기서 중요한 통찰이 나옵니다. **99.99%부터는 사람이 알아채고 조치하는 방식으로는 도달할 수 없다.** 알림을 받고 노트북을 열고 원인을 파악하는 데만 몇 분이 걸리기 때문입니다. 그래서 나인이 늘어날수록 비용이 급격히 오르는 것은 하드웨어 값이 아니라 **자동화와 검증에 드는 비용** 때문입니다.

### 3.2 직렬로 붙이면 가용성은 떨어진다

구성 요소가 **모두 살아 있어야** 서비스가 되는 구조(직렬)에서는 가용성이 곱해집니다.
...
단, 병렬 계산에는 전제가 있습니다. **두 서버의 고장이 서로 독립이어야** 합니다. 같은 랙, 같은 전원, 같은 스위치를 쓰면 한 번의 사고로 둘 다 죽어 계산이 무너집니다. 클라우드에서 **다중 AZ 배치**를 강조하는 이유가 바로 독립성 확보입니다. 나아가 같은 버그가 든 같은 이미지를 두 대에 배포했다면 그것도 독립이 아닙니다.

### 3.3 SLA는 보증이 아니라 보상 약정이다

벤더가 제시하는 SLA(Service Level Agreement)는 "이 수치를 못 지키면 요금을 일부 크레딧으로 돌려준다"는 계약입니다. **크레딧을 받는다고 우리 서비스의 장애가 되돌려지지 않는다.** 그래서 실무에서는 세 가지를 구분해서 씁니다.
...
**단일 장애점(Single Point of Failure, SPOF)** 은 그것 하나가 죽으면 전체가 죽는 지점입니다. 찾는 방법은 단순합니다. **요청이 지나가는 경로를 처음부터 끝까지 따라가며 각 홉에 "이게 죽으면?"을 물어보면 된다.**
...
Active-Active를 쓴다면 반드시 **N+1 용량 계획**을 해야 합니다. 2대로 각각 60%씩 쓰고 있다가 1대가 죽으면 남은 1대가 120%를 받아야 하므로 연쇄 장애가 납니다. "이중화했으니 안전하다"가 아니라 "한 대가 빠져도 남은 대수가 전체 부하를 감당하는가"로 확인해야 합니다.
...
여기서 판단해야 할 트레이드오프가 있습니다. **간격과 임계치를 낮추면 빨리 감지하지만, 일시적 지연에도 멀쩡한 서버를 빼버린다.** 반대로 높이면 안정적이지만 장애를 오래 방치합니다. 트래픽 패턴과 애플리케이션의 워밍업 시간을 보고 정해야 합니다.

또 하나 중요한 것이 **헬스체크의 깊입니다.

- **얕은 체크(shallow)**: `/health`가 그냥 200을 반환합니다. 프로세스는 살아 있지만 DB 커넥션이 끊겨 실제 요청은 전부 실패하는 상황을 못 잡습니다.
- **깊은 체크(deep)**: DB, 캐시 등 필수 의존성까지 확인합니다. 정확하지만 위험도 있습니다. DB가 잠시 흔들리면 **모든 인스턴스가 동시에 unhealthy가 되어 서비스 전체가 내려간다.**

실무의 절충은 이렇습니다. **로드 밸런서용 체크는 이 인스턴스 자신의 상태만 확인하고(얕게), 공유 의존성 상태는 별도 모니터링과 알람으로 잡는다.** 공유 자원 장애를 인스턴스 제거로 대응하면 상황이 더 나빠집니다.

### 5.3 Multi-AZ와 Multi-Region

- **Multi-AZ**: 같은 리전 안의 물리적으로 분리된 데이터센터에 분산 배치. AZ 간 지연이 매우 낮아 동기 복제가 가능합니다. **일반적인 HA의 기본 단위입니다.
- **Multi-Region**: 리전 전체 장애나 광역 재해에 대비합니다. 리전 간 지연이 크므로 대개 비동기 복제가 되고, 그만큼 **데이터 손실 가능성(RPO)** 이 생깁니다. 비용과 복잡도도 크게 올라갑니다.

정리하면, HA는 Multi-AZ로 만들고 DR은 Multi-Region으로 만듭니다. 이 둘을 섞어 말하면 대화가 꼬입니다.
...
이 둘은 비즈니스가 정하는 값이지 엔지니어가 정하는 값이 아닙니다. 결제 원장은 RPO가 0에 가까워야 하지만, 추천 랭킹 캐시는 하루치를 잃어도 재계산하면 됩니다. **시스템마다 다르게 잡는 것이 정상**입니다.
...
**표 아래 한 줄**: DR 전략은 "얼마나 안전하고 싶은가"가 아니라 "중단 1시간의 손실이 평상시 대기 비용보다 큰가"로 고릅니다.

### 6.3 검증하지 않은 백업은 백업이 아니다

가장 흔하고 가장 비싼 실수입니다. 백업 잡이 매일 성공했다는 로그만 보고 안심하다가, 정작 복구하려니 스키마가 안 맞거나 암호화 키가 없거나 복구에 예상보다 몇 배의 시간이 걸린다는 것을 그때 알게 됩니다.
...
1. 헬스 엔드포인트를 **"이 인스턴스가 지금 요청을 처리할 준비가 되었는가"** 를 반영하도록 바꿉니다. 워밍업이 끝나야 200을 반환하게 합니다. (공유 의존성까지 확인하는 깊은 체크로 만들지는 않는다 — 5.2절의 이유)
...
**교훈.** 이중화를 해두어도 **"트래픽을 언제 붙이고 언제 떼는가"** 를 정확히 다루지 못하면 배포마다 가용성을 깎아 먹습니다. HA는 장비 대수가 아니라 전환 절차의 문제입니다.
...
확장성은 **병목을 찾아 처리량을 늘리는 문제**이고 고가용성은 **독립적인 이중화 + 자동 감지/전환으로 연속성을 지키는 문제**이며, 두 가지 모두 "몇 대를 두었는가"가 아니라 "한 대가 빠졌을 때 시스템이 스스로 무엇을 하는가"로 검증됩니다.
