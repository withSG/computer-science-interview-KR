...
그리고 무엇보다 **병목이 공유 자원에 있으면 늘려도 소용없습니다.** 웹 서버를 10대로 늘려도 모두 같은 DB를 때리고 있으면 처리량은 DB에서 막힙니다. 확장 작업을 하기 전에 **어디가 병목인지부터 측정**해야 하는 이유입니다. 직렬로 처리될 수밖에 없는 구간의 비중이 클수록 서버를 늘려도 얻는 이득이 빠르게 줄어든다(암달의 법칙).
...
여기서 중요한 통찰이 나옵니다. **99.99%부터는 사람이 알아채고 조치하는 방식으로는 도달할 수 없습니다.** 알림을 받고 노트북을 열고 원인을 파악하는 데만 몇 분이 걸리기 때문입니다. 그래서 나인이 늘어날수록 비용이 급격히 오르는 것은 하드웨어 값이 아니라 **자동화와 검증에 드는 비용** 때문입니다.
