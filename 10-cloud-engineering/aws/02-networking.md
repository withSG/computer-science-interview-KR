...
- **계층 분리가 안 됩니다.** 웹/앱/DB를 서로 다른 신뢰 구역에 두는 전통적 설계를 옮길 수 없었습니다.
...
> **비유의 한계**: 아파트에서는 동의 위치가 물리적으로 정해지지만, VPC에서 서브넷의 성격(Public/Private)은 물리적 위치가 아니라 **표지판(라우팅 테이블)을 어떻게 세웠느냐로만 결정됩니다.** 같은 자리에 있는 서브넷도 표지판을 바꾸면 성격이 바뀝니다.
...
**서브넷에는 "Public 서브넷"이라는 타입 속성이 없습니다.** 콘솔에도 그런 체크박스가 없습니다. 어떤 서브넷이 Public인지 Private인지는 오직 **그 서브넷에 연결된 라우팅 테이블에 `0.0.0.0/0 → Internet Gateway` 경로가 있는가**로 결정됩니다.
...
`local` 경로는 VPC 생성 시 자동으로 들어가며 삭제할 수 없습니다. **같은 VPC 안에서는 서브넷이 달라도 서로 통신이 된다**는 뜻입니다. 즉 Private 서브넷의 DB가 "격리"되는 건 VPC 내부로부터가 아니라 **인터넷으로부터**입니다. 이 구분이 흐려지면 보안 설계가 어긋납니다.
...
**비대칭의 이유**는 NAT의 동작 방식 자체에 있습니다. NAT는 안에서 나가는 연결마다 "누가 어디로 나갔다"는 매핑 테이블을 만듭니다. 응답 패킷이 돌아오면 그 테이블을 보고 원래 인스턴스에 돌려줍니다. **바깥에서 먼저 시작한 패킷은 매핑 테이블에 대응하는 항목이 없으므로 버릴 수밖에 없습니다.** 정책으로 막는 게 아니라 구조적으로 불가능한 것입니다.
...
- **인터넷 접근용 NAT Gateway는 반드시 Public 서브넷에 있어야 합니다.** 자기 자신이 IGW로 나갈 경로가 있어야 하기 때문입니다. Private 서브넷에 만들어놓고 왜 안 나가는지 찾는 것은 흔한 초보 실수입니다. (인터넷이 아니라 다른 VPC나 온프레미스와만 통신하는 **프라이빗 NAT Gateway**라는 별도 유형이 있고, 그건 Private 서브넷에 둡니다. 지금 다루는 것은 인터넷 아웃바운드용 퍼블릭 NAT Gateway다.)
- **NAT는 아웃바운드 트래픽량에 비례해 과금됩니다.** 컨테이너 이미지를 매 배포마다 인터넷에서 당겨오거나, S3에 대용량 로그를 NAT 경유로 올리면 여기서 비용이 샙니다. 이 문제의 해법이 다음 절의 VPC Endpoint다.
...
가장 흔한 사고는 이것입니다. 보안을 강화하겠다고 NACL 아웃바운드를 "80, 443만 허용"으로 잠급니다. 그러면 **밖에서 들어온 요청에 대한 응답이 나가지 못합니다.**
...
응답 패킷의 **목적지 포트는 443이 아니라 클라이언트가 임의로 고른 임시 포트(ephemeral port)**입니다. 그래서 NACL 아웃바운드에는 임시 포트 범위(`1024-65535`)를 열어줘야 응답이 나갑니다. Security Group은 연결 상태를 추적하므로 이 작업이 필요 없습니다. **이것이 Stateful/Stateless 차이의 실질적 의미입니다.**
...
IP 대역으로 쓰면 앱 서버가 오토스케일링으로 늘어날 때마다 규칙을 고쳐야 합니다. SG 참조로 쓰면 **새로 뜬 인스턴스가 `sg-app`을 달고 있기만 하면 자동으로 DB 접근 권한을 얻습니다.** 인스턴스가 가축처럼 교체되는 환경에서 유일하게 유지보수 가능한 방식입니다.
...
- **DB 서브넷에는 기본 경로를 아예 두지 않습니다.** 인터넷으로 나갈 일이 없으면 경로 자체를 없애는 것이 가장 확실한 격리입니다.
