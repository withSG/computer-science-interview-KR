...
> Public과 Private 서브넷을 가르는 건 라우팅 테이블 한 줄뿐입니다. 이 사실에서 출발해 IGW와 NAT Gateway의 비대칭, Security Group과 NACL의 상태성 차이, VPC Endpoint의 존재 이유까지 스스로 설명하고 설계할 수 있게 됩니다.
...
> **비유의 한계**: 아파트에서는 동의 위치가 물리적으로 정해지지만 VPC에서 서브넷의 성격(Public/Private)은 물리적 위치와 무관합니다. **표지판(라우팅 테이블)을 어떻게 세웠느냐로만 결정됩니다.** 같은 자리에 있는 서브넷도 표지판을 바꾸면 성격이 바뀝니다.
