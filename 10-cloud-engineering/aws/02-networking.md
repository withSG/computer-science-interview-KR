...
> Public과 Private 서브넷을 가르는 것은 라우팅 테이블 한 줄뿐이라는 사실에서 출발해, IGW와 NAT Gateway의 비대칭, Security Group과 NACL의 상태성 차이, VPC Endpoint의 존재 이유까지 스스로 설명하고 설계할 수 있게 됩니다.
...
- **내부망 개념이 없다.** "이 서버는 외부에서 절대 못 닿게" 같은 요구를 네트워크 계층에서 표현할 방법이 없었습니다.
- **IP 대역을 못 정한다.** 온프레미스와 VPN으로 연결하려면 대역이 겹치지 않아야 하는데, 대역을 고를 수가 없습니다.
- **계층 분리가 안 된다.** 웹/앱/DB를 서로 다른 신뢰 구역에 두는 전통적 설계를 옮길 수 없었습니다.
...
> **비유의 한계**: 아파트에서는 동의 위치가 물리적으로 정해지지만, VPC에서 서브넷의 성격(Public/Private)은 물리적 위치가 아니라 **표지판(라우팅 테이블)을 어떻게 세웠느냐로만 결정된다.** 같은 자리에 있는 서브넷도 표지판을 바꾸면 성격이 바뀝니다.
...
**VPC**를 만들 때 가장 먼저 정하는 것은 CIDR 블록입니다. `10.0.0.0/16`처럼 잡으면 그 VPC가 쓸 수 있는 사설 IP 주소의 범위가 정해집니다. 이 범위는 **나중에 축소할 수 없으므로** 처음에 넉넉히 잡습니다. 실무에서는 `/16`을 기본으로 쓰고, 온프레미스나 다른 VPC와 겹치지 않도록 조직 차원에서 대역을 관리합니다.

**서브넷**은 VPC의 CIDR을 더 잘게 나눈 것입니다. 여기서 반드시 기억할 제약이 하나 있습니다.

> **하나의 서브넷은 정확히 하나의 AZ에 속합니다. 서브넷은 AZ를 가로지를 수 없다.**
...
그래서 `/24`의 실제 가용 IP는 251개입니다. `/28`(16개)로 서브넷을 자르면 쓸 수 있는 건 11개뿐이라, EKS처럼 Pod마다 IP를 소모하는 워크로드에서는 금방 고갈됩니다. **서브넷을 너무 잘게 자르지 않는 것**이 실무 교훈입니다.

---

## 3. 핵심 중의 핵심: Public/Private은 라우팅 테이블이 결정한다

이 문서에서 하나만 가져간다면 이것입니다.

**서브넷에는 "Public 서브넷"이라는 타입 속성이 없다.** 콘솔에도 그런 체크박스가 없습니다. 어떤 서브넷이 Public인지 Private인지는 오직 **그 서브넷에 연결된 라우팅 테이블에 `0.0.0.0/0 → Internet Gateway` 경로가 있는가**로 결정됩니다.
...
`local` 경로는 VPC 생성 시 자동으로 들어가며 삭제할 수 없습니다. **같은 VPC 안에서는 서브넷이 달라도 서로 통신이 된다**는 뜻입니다. 즉 Private 서브넷의 DB가 "격리"되는 건 VPC 내부로부터가 아니라 **인터넷으로부터입니다. 이 구분이 흐려지면 보안 설계가 어긋납니다.
...
2. 퍼블릭 IP가 할당된 인스턴스에 대해 **사설 IP ↔ 퍼블릭 IP의 1:1 NAT**를 수행합니다.

여기서 초심자가 자주 놀라는 사실 하나. 퍼블릭 IP를 가진 EC2에 SSH로 접속한 뒤 `ip addr`을 쳐도 **퍼블릭 IP는 보이지 않는다.** OS는 사설 IP만 알고 있고, 변환은 IGW가 바깥에서 해줍니다.

### NAT Gateway

Private 서브넷의 인스턴스도 인터넷으로 나가야 할 때가 있습니다. `apt update`, 외부 결제 API 호출, 컨테이너 이미지 pull 같은 것들입니다. 하지만 퍼블릭 IP를 주면 외부에서도 들어올 수 있게 됩니다. 이 딜레마를 푸는 것이 NAT Gateway다.
...
**비대칭의 이유**는 NAT의 동작 방식 자체에 있습니다. NAT는 안에서 나가는 연결마다 "누가 어디로 나갔다"는 매핑 테이블을 만듭니다. 응답 패킷이 돌아오면 그 테이블을 보고 원래 인스턴스에 돌려줍니다. **바깥에서 먼저 시작한 패킷은 매핑 테이블에 대응하는 항목이 없으므로 버릴 수밖에 없다.** 정책으로 막는 게 아니라 구조적으로 불가능한 것입니다.

### NAT Gateway 운영에서 반드시 아는 것

- **NAT Gateway는 AZ 단위 리소스다.** AZ-a에 하나만 만들고 AZ-c의 Private 서브넷이 그걸 바라보게 하면, AZ-a가 죽는 순간 **AZ-c의 아웃바운드까지 같이 끊긴다.** 게다가 평소에도 AZ 간 데이터 전송 요금이 붙습니다. → **AZ마다 NAT Gateway를 하나씩** 두고, 각 AZ의 Private 서브넷은 같은 AZ의 NAT를 바라보게 라우팅합니다.
- **인터넷 접근용 NAT Gateway는 반드시 Public 서브넷에 있어야 한다.** 자기 자신이 IGW로 나갈 경로가 있어야 하기 때문입니다. Private 서브넷에 만들어놓고 왜 안 나가는지 찾는 것은 흔한 초보 실수입니다. (인터넷이 아니라 다른 VPC나 온프레미스와만 통신하는 **프라이빗 NAT Gateway**라는 별도 유형이 있고, 그건 Private 서브넷에 둡니다. 지금 다루는 것은 인터넷 아웃바운드용 퍼블릭 NAT Gateway다.)
- **NAT는 아웃바운드 트래픽량에 비례해 과금된다.** 컨테이너 이미지를 매 배포마다 인터넷에서 당겨오거나, S3에 대용량 로그를 NAT 경유로 올리면 여기서 비용이 샙니다. 이 문제의 해법이 다음 절의 VPC Endpoint다.

---

## 5. Security Group과 NACL

AWS의 방화벽은 두 겹입니다. 이름이 비슷해 헷갈리지만 **동작 방식이 근본적으로 다르다.**
...
**결론: 권한 설계는 Security Group으로 하고, NACL은 기본값 그대로 두거나 광역 차단용 보조 방어선으로만 쓴다.** NACL을 세밀하게 조작하기 시작하면 디버깅 난이도가 급격히 올라갑니다.
...
가장 흔한 사고는 이것입니다. 보안을 강화하겠다고 NACL 아웃바운드를 "80, 443만 허용"으로 잠급니다. 그러면 **밖에서 들어온 요청에 대한 응답이 나가지 못한다.**
...
응답 패킷의 **목적지 포트는 443이 아니라 클라이언트가 임의로 고른 임시 포트(ephemeral port)** 다. 그래서 NACL 아웃바운드에는 임시 포트 범위(`1024-65535`)를 열어줘야 응답이 나갑니다. Security Group은 연결 상태를 추적하므로 이 작업이 필요 없습니다. **이것이 Stateful/Stateless 차이의 실질적 의미다.**

### Security Group 참조 패턴

Security Group의 진짜 강점은 소스로 **다른 Security Group을 지정할 수 있다는 것**입니다.
...
IP 대역으로 쓰면 앱 서버가 오토스케일링으로 늘어날 때마다 규칙을 고쳐야 합니다. SG 참조로 쓰면 **새로 뜬 인스턴스가 `sg-app`을 달고 있기만 하면 자동으로 DB 접근 권한을 얻는다.** 인스턴스가 가축처럼 교체되는 환경에서 유일하게 유지보수 가능한 방식입니다.

---

## 6. VPC Endpoint: 인터넷을 거치지 않고 AWS 서비스에 닿기

Private 서브넷의 앱이 S3에 로그를 올린다고 하자. S3는 VPC 밖의 서비스이므로 기본 경로는 이렇습니다.

```
App (Private) → NAT Gateway → IGW → 인터넷 → S3 엔드포인트
```

문제가 두 가지입니다. 첫째, **NAT 데이터 처리 요금이 붙는다.** 둘째, 트래픽이 논리적으로 인터넷을 경유하므로 "우리 데이터는 인터넷에 나가지 않는다"고 말할 수 없습니다.
...
- **AZ마다 NAT Gateway를 둔다.** 가용성과 AZ 간 전송 비용 두 가지 이유입니다.
- **라우팅 테이블은 AZ별로 분리한다.** Public용 1개는 공유해도 되지만, Private용은 AZ별 NAT를 가리켜야 하므로 반드시 분리됩니다.
- **DB 서브넷에는 기본 경로를 아예 두지 않는다.** 인터넷으로 나갈 일이 없으면 경로 자체를 없애는 것이 가장 확실한 격리입니다.
- **ALB와 RDS 서브넷 그룹은 2개 이상 AZ의 서브넷을 요구한다.** AWS가 아예 이 설계를 강제하는 지점입니다.
...
**진단을 이 순서로 합니다. 요청이 지나가는 순서 그대로다.**
...
**VPC Flow Logs**를 켜두면 이 진단이 훨씬 빨라집니다. 각 레코드의 `action` 필드가 `ACCEPT`인지 `REJECT`인지, 그리고 어느 방향에서 끊겼는지가 그대로 보입니다. **인바운드는 ACCEPT인데 응답이 REJECT라면 거의 확실히 NACL 문제입니다. Security Group에 막히면 애초에 인바운드 자체가 REJECT로 찍힙니다.
...
- 꼬리 질문: "그럼 Private 서브넷의 서버는 완전히 고립된 겁니까?" → 아닙니다. VPC CIDR에 대한 `local` 경로는 항상 존재하므로 같은 VPC 안에서는 통신됩니다. 격리되는 것은 인터넷으로부터의 인바운드입니다.
...
- 꼬리 질문: "SG의 소스에 다른 SG를 지정하는 게 왜 유용합니까?" → 오토스케일링으로 IP가 계속 바뀌는 환경에서 규칙을 고치지 않아도 되기 때문. 신뢰 관계를 IP가 아니라 역할로 표현합니다.
...
VPC 네트워킹의 전부는 "라우팅 테이블이 목적지를 정하고, Stateful한 Security Group이 실질적인 접근 제어를 하며, NAT의 구조적 비대칭이 Private 계층의 안전을 만든다"로 요약됩니다.
