...
> AWS의 글로벌 인프라가 어떻게 생겼는지, EC2/S3/RDS/ELB/Route 53/CloudFront가 각각 어떤 문제를 맡는지, "관리형 서비스를 쓸 것인가 직접 운영할 것인가"를 무엇으로 가르는지를 다룹니다. 서비스별 세부 옵션까지는 들어가지 않습니다.
...
- [가상화와 Hypervisor](../cloud-fundamentals/02-virtualization-hypervisor.md) - EC2가 결국 무엇인지 이해하는 데 필요합니다
...
이 흐름에는 구조적인 문제가 있었습니다. 우선 **결정을 너무 일찍 해야 합니다**. 서비스가 얼마나 뜰지 모르는 상태에서 "서버 몇 대를 살지"를 정해야 합니다. 적게 사면 오픈하자마자 장애가 나고, 많이 사면 몇 달 동안 놀고 있는 하드웨어에 돈이 묶입니다.
...
>
> **비유의 한계**: 렌터카는 반납하면 끝입니다. 클라우드는 **끄는 것과 지우는 것이 다릅니다**. EC2를 중지(stop)해도 붙어 있는 EBS 볼륨과 Elastic IP는 계속 과금됩니다. "안 쓰면 공짜"가 아니라 "삭제해야 공짜"입니다.
...
**"`us-east-1a`는 모든 계정에서 같은 데이터센터다"** — 아닙니다. `us-east-1`·`ap-northeast-1` 같은 초기 리전에서는(2025년 11월 이전에 만든 계정 기준) AZ 이름이 계정마다 다른 물리 AZ에 매핑됩니다(부하 분산을 위한 AWS의 의도된 설계). 서울처럼 2012년 11월 이후 개설된 리전은 모든 계정에서 매핑이 같습니다. 계정 간에 물리적으로 같은 AZ를 확실히 지칭하려면 **AZ ID**(`use1-az1` 형태)를 봐야 합니다. 이름만으로는 보장되지 않습니다.
...
S3 객체는 기본값이 비공개입니다. 그런데 사용자에게 파일을 주거나 받아야 할 때가 있습니다. 이때 백엔드가 파일을 중계하면 서버 대역폭과 메모리를 그대로 먹습니다.

이럴 때 쓰는 것이 **Pre-signed URL**입니다. 백엔드가 자기 권한으로 "이 객체를, 이 동작으로, 이 시각까지" 서명한 URL을 만들어 주고, 클라이언트는 그 URL로 S3에 직접 요청합니다.
...
"EC2 만들어서 MySQL 직접 깔면 안 되나?"는 정당한 질문입니다. 됩니다. 그다음이 문제입니다.
...
Standby는 평소에 아무 요청도 받지 않습니다. 놀고 있는 서버에 돈을 내는 대신 장애 시간을 사는 것입니다. (예외가 하나 있습니다. **Multi-AZ DB 클러스터** 배포 방식은 대기 인스턴스를 2대 두는데, 이쪽에서는 대기 인스턴스가 읽기 요청을 받을 수 있습니다. 여기서 말하는 "접속 불가"는 전통적인 Multi-AZ DB 인스턴스 배포 기준입니다.)

읽기 부하를 나누고 싶으면 Read Replica를 따로 만들어야 합니다. 이 복제는 **비동기 복제라 복제 지연(replica lag)이 존재**합니다. "방금 쓴 데이터를 바로 읽었는데 없더라"는 버그가 여기서 나옵니다.
...
**증상**: ALB가 간헐적으로 502 Bad Gateway를 반환합니다. CloudWatch에서 `HTTPCode_ELB_5XX_Count`만 오르고 앱 로그에는 에러가 없습니다.
...
AWS 핵심 서비스는 Region/AZ라는 장애 격리 격자 위에 컴퓨팅(EC2), 스토리지(S3/EBS), 데이터(RDS)를 놓고, 앞단에서 Route 53·CloudFront·ELB로 트래픽을 흘리는 구조입니다. 설계의 본질은 자리마다 무엇을 관리형에 맡길지 고르는 일입니다.
