...
> AWS의 과금 모델이 왜 On-Demand / Reserved / Savings Plans / Spot으로 나뉘는지, Spot 중단을 어떻게 설계로 흡수하는지, 데이터 전송비가 왜 청구서에서 가장 늦게 발견되는 비용인지, 그리고 비용을 "누구의 것"으로 만드는 태깅 체계를 설명할 수 있게 된다.
...
> **주의**: 이 문서는 구체적인 금액이나 할인율을 적지 않는다. AWS 요금은 리전과 시점에 따라 계속 바뀌기 때문에, 숫자를 외우는 것은 의미가 없고 오히려 위험하다. **과금이 어떤 축으로 계산되는지**를 이해하면 요금표를 볼 때 스스로 계산할 수 있다.

---

## 1. 왜 필요한가

클라우드 비용 문제의 본질은 "비싸다"가 아니다. **비용이 만들어지는 시점과 그것을 인지하는 시점이 멀리 떨어져 있다**는 것이다.

온프레미스에서는 서버를 사려면 결재를 받는다. 누군가 승인 문서를 읽고 대수를 따지고 금액을 확인하므로, 비용이 **의사결정 시점에** 눈앞에 있다. 클라우드에서는 개발자가 콘솔에서 클릭 몇 번으로 인스턴스를 만든다. 승인도 없고 금액도 안 보인다. 그 결정의 결과는 **한 달 뒤 청구서에** 나타나고, 그때는 이미 누가 왜 만들었는지 아무도 기억하지 못한다.

그래서 비용 최적화는 "싼 걸 고르는 기술"이 아니라 **가시성(누가 얼마를 쓰는지 보이게 하기)과 책임(그 비용의 주인이 있게 하기)의 문제**다. 인스턴스 타입을 한 단계 낮추는 것보다, 아무도 안 쓰는 스테이징 환경 열 개를 찾아 끄는 것이 대개 훨씬 큰 절감을 만든다.

> **비유**: 클라우드 비용은 수도 요금이다. 수도꼭지가 100개인 집에서 몇 개가 새고 있는지 모르는 상태.
> **비유의 한계**: 수도는 잠그면 즉시 멈추지만, 클라우드는 **인스턴스를 꺼도 붙어 있는 디스크와 스냅샷과 미할당 IP는 계속 흐른다.** "껐다"와 "지웠다"는 다르다.
...
**On-Demand**는 기준 가격이다. 약정도 위험도 없고, 초 단위로 계산된다(리눅스 기준 최소 과금 단위가 존재). 할인이 없다는 게 단점이지만, **예측이 안 되는 부분에는 이게 정답**이다. 필요 없어지면 그 순간 과금이 멈춘다는 것 자체가 가치다.

**Reserved Instance(RI)** 는 특정 인스턴스 구성을 1년 또는 3년 쓰겠다고 약정한다. 선결제 비중(전액/부분/없음)이 높을수록 할인이 커진다. Standard RI는 변경이 거의 불가능하고, Convertible RI는 비슷한 가치의 다른 타입으로 교환할 수 있는 대신 할인이 덜하다. 또 특정 AZ에 지정하는 방식(zonal)을 쓰면 **용량 예약** 효과까지 얻는다. 즉 "그 AZ에 인스턴스가 없어서 못 띄우는" 상황을 막는다.

**Savings Plans(SP)** 는 인스턴스 구성이 아니라 **시간당 지출 금액**을 약정한다. "매시간 이만큼은 쓸 테니 그만큼 할인해달라"는 계약이다.

- **Compute Savings Plans**: EC2뿐 아니라 Fargate와 Lambda에도 적용된다. 리전, 인스턴스 패밀리, OS를 바꿔도 약정이 그대로 따라온다. 유연성이 가장 크다.
- **EC2 Instance Savings Plans**: 특정 리전의 특정 인스턴스 패밀리로 범위를 좁히는 대신 할인 폭이 더 크다.

**Spot Instance**는 AWS의 유휴 용량을 빌린다. 할인 폭이 가장 크지만 **AWS가 용량이 필요해지면 2분의 알림을 주고 회수한다.** 이 2분을 어떻게 쓰느냐가 Spot 설계의 전부다.
...
Spot을 "싸니까 그냥 쓰자"로 접근하면 반드시 사고가 난다. Spot은 **인프라 할인이 아니라 아키텍처 요구사항**이다. 중단을 전제로 설계해야 쓸 수 있다.
...
- **무상태(stateless)**: 인스턴스 로컬에만 있는 데이터가 없어야 한다. 세션은 ElastiCache, 파일은 S3.
- **작업의 재시작 가능성**: 중간에 끊긴 작업이 다른 인스턴스에서 이어지거나 처음부터 다시 돌 수 있어야 한다. SQS의 가시성 타임아웃이 이 역할을 자동으로 해준다.
- **멱등성(idempotency)**: 같은 작업이 두 번 실행돼도 결과가 같아야 한다. 중단 후 재시도는 중복 실행을 만든다.

### 인프라 쪽 안전장치

- **인스턴스 타입을 다양화한다.** 한 타입만 쓰면 그 타입 풀이 마르는 순간 전멸한다. Auto Scaling Group의 혼합 인스턴스 정책(Mixed Instances Policy)으로 여러 타입과 여러 AZ를 섞는다.
- **할당 전략을 용량 기준으로 잡는다.** 가장 싼 풀만 고르면 그 풀이 가장 먼저 회수된다. 여유 용량이 많은 풀을 우선하는 전략을 쓰면 중단 빈도가 눈에 띄게 줄어든다.
- **On-Demand 기반선을 섞는다.** ASG에서 "최소 N대는 On-Demand, 나머지는 Spot"으로 구성하면 Spot이 전부 회수돼도 서비스가 죽지 않는다.
- **ASG 수명 주기 훅(lifecycle hook)** 으로 종료 전 정리 시간을 확보하고, ELB의 등록 해제 지연(deregistration delay)으로 진행 중 요청을 흘려보낸다.

**Spot이 잘 맞는 곳**: 배치 처리, 미디어 인코딩, CI 빌드 러너, ML 학습(체크포인트가 있는 경우), 크롤러, 개발/테스트 환경.
**Spot을 쓰면 안 되는 곳**: 상태를 가진 단일 인스턴스(DB), 중단 시 복구가 불가능한 장시간 단일 작업, 엄격한 가용성 SLA가 걸린 동기 API의 유일한 백엔드.

---

## 4. 스토리지 비용: S3 클래스와 수명 주기

S3 요금은 세 축으로 계산된다. 이 구조를 알면 클래스 선택이 자동으로 결정된다.
...
클래스가 나뉘는 이유는 이 세 축의 가중치가 다르기 때문이다. **저장이 싼 클래스는 검색이 비싸다.** 그래서 "얼마나 자주 읽는가"가 유일한 판단 기준이 된다.
...
**결론: 읽기 빈도를 알면 클래스가 정해지고, 모르면 Intelligent-Tiering이 안전한 기본값이다.**

주의할 함정: **최소 보관 기간이 있는 클래스에 짧게 머무는 데이터를 넣으면 손해다.** IA로 옮긴 지 3일 만에 삭제해도 30일치가 청구된다. 그리고 클래스 전환 자체에도 객체당 요청 요금이 붙어서, **아주 작은 파일 수백만 개를 Glacier로 옮기면 전환 비용이 절감액을 넘어설 수 있다.**

### 수명 주기 정책

수동으로 옮길 수는 없다. 규칙을 걸어 자동화한다.
...
두 번째 규칙을 특히 눈여겨보자. **멀티파트 업로드가 실패하면 이미 올라간 조각들이 버킷에 남아 계속 과금된다.** 그런데 이 조각은 `aws s3 ls`로 보이지 않아서 "버킷에 아무것도 없는데 저장 요금이 나온다"는 미스터리가 된다. 모든 버킷에 이 규칙을 기본으로 넣어두는 것이 좋다.

버전 관리를 켠 버킷이라면 **이전 버전(noncurrent version)의 만료 규칙**도 반드시 넣는다. 안 넣으면 삭제한 파일이 영원히 남아 계속 과금된다.

---

## 5. 데이터 전송 비용: 청구서에서 가장 늦게 발견되는 항목

컴퓨팅과 스토리지는 눈에 보인다. 인스턴스 몇 대, 몇 GB. 데이터 전송은 다르다. **어떤 경로로 얼마나 흘렀는지는 설계도만 봐서는 안 보인다.**
...
| 인터넷 outbound | 정적 파일을 EC2/S3에서 사용자에게 직접 전송 | **CloudFront**를 앞에 둔다. 캐시 히트만큼 오리진 전송이 사라진다 |
| 퍼블릭 IP 경유 | 같은 리전 안인데도 퍼블릭 IP/도메인으로 통신 | 사설 IP나 VPC 내부 엔드포인트로 호출 |
| 리전 간 복제 | S3 교차 리전 복제, 크로스 리전 읽기 | 정말 필요한 prefix만 복제 대상으로 지정 |
| 퍼블릭 IPv4 주소 | 지금은 미할당 EIP뿐 아니라 **사용 중인 퍼블릭 IPv4 주소에도** 시간당 요금이 붙는다 | 안 쓰는 EIP는 해제하고, 퍼블릭 IP가 꼭 필요한 리소스인지부터 다시 본다 |

**결론: 데이터 전송비를 줄이는 방법은 "전송량을 줄이는 것"이 아니라 "경로를 바꾸는 것"이다.** 같은 데이터라도 NAT를 지나느냐 VPC Endpoint를 지나느냐, 오리진에서 직접 나가느냐 CDN을 거치느냐에 따라 비용이 크게 달라진다.
...
청구서에 "EC2 1,200달러"라고 적혀 있으면 할 수 있는 일이 없다. 누구 것인지, 어느 서비스 것인지, 운영인지 개발인지 모르기 때문이다. **비용을 줄이는 첫 단계는 비용에 주인을 붙이는 것**이다.

최소한 이 정도는 조직 표준으로 정한다.
...
태그를 만들었다면 반드시 두 가지를 해야 한다.

1. **비용 할당 태그로 활성화한다.** Billing 콘솔에서 활성화하지 않으면 Cost Explorer에서 그 태그로 나눠 볼 수 없다. 활성화 이후 데이터부터 적용되므로 **빨리 켤수록 좋다.**
2. **태그 없는 리소스를 만들지 못하게 한다.** Organizations의 태그 정책이나 IAM 정책의 `Condition`으로 필수 태그를 강제하고, IaC 모듈에서 기본 태그를 자동 부착한다. 사람의 성실함에 기대는 태깅은 반드시 무너진다.
...
- **Cost Anomaly Detection**: 평소 패턴에서 벗어난 지출을 자동 탐지. 실수로 켠 대형 인스턴스를 월말이 아니라 다음 날 잡는다.
...
**결론: Budgets와 Anomaly Detection은 오늘 당장 켠다.** 이 둘은 설정에 몇 분이면 되고, 사고를 한 달이 아니라 하루 만에 잡아준다.
...
Auto Scaling Group의 **목표 추적(Target Tracking)** 정책이 기본 선택지다. "평균 CPU 60% 유지" 같은 목표만 주면 AWS가 알아서 늘리고 줄인다. 임계치를 직접 계단식으로 짜는 것보다 단순하고 대개 더 잘 동작한다.

실무 주의점 세 가지다.

- **쿨다운을 너무 짧게 잡으면 출렁인다(thrashing).** 늘렸다 줄였다를 반복하면 오히려 불안정해진다.
- **스케일 아웃은 빠르게, 스케일 인은 느리게.** 부족해서 생기는 손해가 남아서 생기는 손해보다 크다.
- **인스턴스 기동 시간이 길면 스케일링이 늦는다.** AMI를 미리 굽거나(golden image) 웜 풀(Warm Pool)을 쓴다.

### 비운영 환경: 시간 기반 스케줄링

개발·스테이징 환경은 대개 **업무 시간에만** 필요하다. 예약 스케일링으로 평일 아침에 desired 수를 올리고 저녁에 0으로 내리면, 주말까지 포함해 가동 시간을 절반 이하로 줄일 수 있다. 인스턴스 타입을 낮추는 것보다 훨씬 큰 효과가 난다.

RDS도 마찬가지다. 개발용 DB 인스턴스는 중지할 수 있다. 다만 **RDS 중지는 일정 기간이 지나면 AWS가 자동으로 다시 시작**시키므로(현재 최대 7일), 계속 꺼두려면 재중지 자동화가 필요하다. Aurora Serverless 계열은 사용량에 따라 용량이 자동 조절되므로 이 관리 자체가 필요 없어진다.
...
**증상**: 월중 비용 알림이 예산 임계치를 넘겼다는 메일. 배포도 없었고 트래픽도 그대로다.
...
**대응 원칙**: 원인을 찾기 전에 리소스를 삭제하지 않는다. 운영 데이터일 수 있다. **먼저 태그와 CloudTrail로 누가 언제 만들었는지 확인**하고, 소유자에게 확인한 뒤 정리한다. 그리고 같은 일이 반복되지 않도록 Budgets 알림과 Anomaly Detection을 그 계정에 걸어둔다.
...
- 꼬리 질문: "약정을 얼마나 잡아야 합니까?" → Cost Explorer의 사용량 이력에서 1년 내내 유지된 최저 사용량을 기준으로 보수적으로 시작하고, 커버리지 리포트를 보며 늘린다. 과도한 약정은 되돌릴 수 없으므로 부족한 쪽이 안전하다.

**Q. Spot Instance를 안전하게 쓰려면 무엇이 필요합니까?**

A. Spot은 인프라 옵션이 아니라 아키텍처 요구사항이라고 봅니다. 애플리케이션이 **무상태여야 하고, 작업이 중단 후 재시작 가능해야 하며, 멱등해야** 합니다. 인프라 쪽에서는 인스턴스 타입과 AZ를 다양화하고, 여유 용량이 많은 풀을 우선하는 할당 전략을 쓰고, ASG에 On-Demand 기반선을 섞어 전멸을 막습니다. 그리고 인스턴스 안에서는 메타데이터나 EventBridge로 2분 중단 예고를 받아 대상 그룹에서 스스로 빠지고 체크포인트를 저장한 뒤 종료하는 처리가 있어야 합니다.

- 꼬리 질문: "어떤 워크로드에 씁니까?" → CI 빌드 러너, 배치 처리, 미디어 인코딩, 체크포인트가 있는 ML 학습, 개발 환경. 상태를 가진 DB나 중단 시 복구 불가능한 단일 장시간 작업에는 쓰지 않는다.

**Q. 데이터 전송 비용이 왜 문제가 됩니까?**

A. 컴퓨팅과 스토리지는 리소스 목록으로 보이지만, 데이터 전송은 **아키텍처 다이어그램만 봐서는 얼마가 나올지 알 수 없기 때문**입니다. 대표적으로 앱 서버와 DB가 다른 AZ에 있으면 매 쿼리가 AZ 간 전송을 발생시키고, Private 서브넷의 모든 아웃바운드가 NAT Gateway를 지나면서 처리량 요금이 붙습니다. 대응은 전송량을 줄이는 게 아니라 **경로를 바꾸는 것**입니다. S3는 Gateway Endpoint로 NAT를 우회하고, 사용자 대상 전송은 CloudFront를 앞에 두고, 서비스 간 통신은 같은 AZ 안에서 사설 IP로 하도록 배치합니다.

- 꼬리 질문: "AZ를 하나로 몰면 전송비가 없어지지 않나요?" → 없어지지만 가용성을 포기하게 된다. 비용과 가용성의 트레이드오프이며, 보통은 Multi-AZ를 유지하고 AZ 인식 라우팅으로 불필요한 교차만 줄인다.

**Q. 비용 최적화를 어디서부터 시작하겠습니까?**

A. 인스턴스 타입 조정보다 **가시성 확보가 먼저**입니다. 필수 태그 표준을 정하고 IaC와 IAM 조건으로 강제한 뒤, 비용 할당 태그를 활성화합니다. 그다음 Budgets 알림과 Cost Anomaly Detection을 켜서 이상 지출을 하루 단위로 잡습니다. 그 상태에서 유휴 리소스(미연결 EBS, 미할당 EIP, 트래픽 없는 LB)를 정리하고, 비운영 환경에 스케줄링을 적용하고, 마지막으로 안정된 기저 부하에 Savings Plans를 겁니다. **약정은 사용 패턴이 안정된 뒤에 하는 마지막 단계**입니다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|---|---|---|
| 인스턴스를 중지하면 비용이 0이 된다고 생각 | EBS 볼륨, 스냅샷, 미할당 EIP는 계속 과금 | 안 쓰면 삭제한다. 중지와 삭제는 다르다 |
...
| S3 버킷이 비어 보이는데 요금이 나옴 | 실패한 멀티파트 조각이나 이전 버전이 남아 있다 | 미완료 업로드 중단 규칙과 이전 버전 만료 규칙을 넣는다 |
...
AWS 비용 최적화는 싼 옵션을 고르는 기술이 아니라 **"비용에 주인을 붙이고(태그), 안 쓰는 것을 끄고(스케줄링·정리), 확실히 쓰는 만큼만 약정하고(SP/RI), 중단을 견딜 수 있는 곳에만 Spot을 쓰며, 데이터가 흐르는 경로를 다시 그리는" 운영 습관**이다.
