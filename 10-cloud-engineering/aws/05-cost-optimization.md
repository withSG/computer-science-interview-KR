...
> AWS의 과금 모델이 왜 On-Demand / Reserved / Savings Plans / Spot으로 나뉘는지, Spot 중단을 어떻게 설계로 흡수하는지, 데이터 전송비가 왜 청구서에서 가장 늦게 발견되는 비용인지, 그리고 비용을 "누구의 것"으로 만드는 태깅 체계를 설명할 수 있게 됩니다.
...
> **주의**: 이 문서는 구체적인 금액이나 할인율을 적지 않습니다. AWS 요금은 리전과 시점에 따라 계속 바뀌기 때문에, 숫자를 외우는 것은 의미가 없고 오히려 위험합니다. **과금이 어떤 축으로 계산되는지**를 이해하면 요금표를 볼 때 스스로 계산할 수 있습니다.

---

## 1. 왜 필요한가

클라우드 비용 문제의 본질은 "비싸다"가 아닙니다. **비용이 만들어지는 시점과 그것을 인지하는 시점이 멀리 떨어져 있다**는 것입니다.

온프레미스에서는 서버를 사려면 결재를 받습니다. 누군가 승인 문서를 읽고 대수를 따지고 금액을 확인하므로, 비용이 **의사결정 시점에** 눈앞에 있습니다. 클라우드에서는 개발자가 콘솔에서 클릭 몇 번으로 인스턴스를 만듭니다. 승인도 없고 금액도 안 보입니다. 그 결정의 결과는 **한 달 뒤 청구서에** 나타나고, 그때는 이미 누가 왜 만들었는지 아무도 기억하지 못합니다.

그래서 비용 최적화는 "싼 걸 고르는 기술"이 아니라 **가시성(누가 얼마를 쓰는지 보이게 하기)과 책임(그 비용의 주인이 있게 하기)의 문제입니다. 인스턴스 타입을 한 단계 낮추는 것보다, 아무도 안 쓰는 스테이징 환경 열 개를 찾아 끄는 것이 대개 훨씬 큰 절감을 만듭니다.

> **비유**: 클라우드 비용은 수도 요금입니다. 수도꼭지가 100개인 집에서 몇 개가 새고 있는지 모르는 상태.
> **비유의 한계**: 수도는 잠그면 즉시 멈추지만, 클라우드는 **인스턴스를 꺼도 붙어 있는 디스크와 스냅샷과 미할당 IP는 계속 흐른다.** "껐다"와 "지웠다"는 다릅니다.

---

## 2. 컴퓨팅 과금 모델: 약정과 유연성의 교환

AWS가 컴퓨팅 요금제를 여러 개로 나눈 이유는 명확합니다. **AWS 입장에서 가장 비싼 것은 예측 불가능한 수요입니다. 사용자가 "1년간 이만큼 쓰겠다"고 약속해주면 AWS는 용량 계획이 쉬워지고, 그 대가로 할인을 줍니다. 반대로 "언제든 뺏어가도 좋다"고 하면 AWS는 남는 용량을 팔 수 있으므로 더 큰 할인을 줍니다.
...
**On-Demand**는 기준 가격입니다. 약정도 위험도 없고, 초 단위로 계산된다(리눅스 기준 최소 과금 단위가 존재). 할인이 없다는 게 단점이지만, **예측이 안 되는 부분에는 이게 정답**입니다. 필요 없어지면 그 순간 과금이 멈춘다는 것 자체가 가치입니다.

**Reserved Instance(RI)** 는 특정 인스턴스 구성을 1년 또는 3년 쓰겠다고 약정합니다. 선결제 비중(전액/부분/없음)이 높을수록 할인이 커집니다. Standard RI는 변경이 거의 불가능하고, Convertible RI는 비슷한 가치의 다른 타입으로 교환할 수 있는 대신 할인이 덜합니다. 또 특정 AZ에 지정하는 방식(zonal)을 쓰면 **용량 예약** 효과까지 얻습니다. 즉 "그 AZ에 인스턴스가 없어서 못 띄우는" 상황을 막습니다.
...
**Spot Instance**는 AWS의 유휴 용량을 빌립니다. 할인 폭이 가장 크지만 **AWS가 용량이 필요해지면 2분의 알림을 주고 회수한다.** 이 2분을 어떻게 쓰느냐가 Spot 설계의 전부입니다.
...
Spot을 "싸니까 그냥 쓰자"로 접근하면 반드시 사고가 납니다. Spot은 **인프라 할인이 아니라 아키텍처 요구사항**입니다. 중단을 전제로 설계해야 쓸 수 있습니다.
...
이 흐름이 성립하려면 애플리케이션이 세 가지 성질을 가져야 합니다.
...
- **인스턴스 타입을 다양화한다.** 한 타입만 쓰면 그 타입 풀이 마르는 순간 전멸합니다. Auto Scaling Group의 혼합 인스턴스 정책(Mixed Instances Policy)으로 여러 타입과 여러 AZ를 섞습니다.
- **할당 전략을 용량 기준으로 잡는다.** 가장 싼 풀만 고르면 그 풀이 가장 먼저 회수됩니다. 여유 용량이 많은 풀을 우선하는 전략을 쓰면 중단 빈도가 눈에 띄게 줄어듭니다.
- **On-Demand 기반선을 섞는다.** ASG에서 "최소 N대는 On-Demand, 나머지는 Spot"으로 구성하면 Spot이 전부 회수돼도 서비스가 죽지 않습니다.
- **ASG 수명 주기 훅(lifecycle hook)** 으로 종료 전 정리 시간을 확보하고, ELB의 등록 해제 지연(deregistration delay)으로 진행 중 요청을 흘려보냅니다.
...
S3 요금은 세 축으로 계산됩니다. 이 구조를 알면 클래스 선택이 자동으로 결정됩니다.
...
클래스가 나뉘는 이유는 이 세 축의 가중치가 다르기 때문입니다. **저장이 싼 클래스는 검색이 비싸다.** 그래서 "얼마나 자주 읽는가"가 유일한 판단 기준이 됩니다.
...
주의할 함정: **최소 보관 기간이 있는 클래스에 짧게 머무는 데이터를 넣으면 손해다.** IA로 옮긴 지 3일 만에 삭제해도 30일치가 청구됩니다. 그리고 클래스 전환 자체에도 객체당 요청 요금이 붙어서, **아주 작은 파일 수백만 개를 Glacier로 옮기면 전환 비용이 절감액을 넘어설 수 있다.**

### 수명 주기 정책

수동으로 옮길 수는 없습니다. 규칙을 걸어 자동화합니다.
...
두 번째 규칙을 특히 눈여겨보자. **멀티파트 업로드가 실패하면 이미 올라간 조각들이 버킷에 남아 계속 과금된다.** 그런데 이 조각은 `aws s3 ls`로 보이지 않아서 "버킷에 아무것도 없는데 저장 요금이 나온다"는 미스터리가 됩니다. 모든 버킷에 이 규칙을 기본으로 넣어두는 것이 좋습니다.
...
컴퓨팅과 스토리지는 눈에 보입니다. 인스턴스 몇 대, 몇 GB. 데이터 전송은 다릅니다. **어떤 경로로 얼마나 흘렀는지는 설계도만 봐서는 안 보인다.**

기본 원칙은 세 가지입니다. **들어오는 것(inbound)은 대체로 무료**이고, **나가는 것(outbound)이 과금**되며 인터넷으로 나갈수록·멀리 갈수록 비쌉니다. 그리고 **AWS 안에서도 경계를 넘으면 과금된다.** AZ를 넘고, 리전을 넘고, 퍼블릭 IP를 거치면 돈이 붙습니다.
...
| 인터넷 outbound | 정적 파일을 EC2/S3에서 사용자에게 직접 전송 | **CloudFront**를 앞에 둡니다. 캐시 히트만큼 오리진 전송이 사라진다 |
...
**결론: 데이터 전송비를 줄이는 방법은 "전송량을 줄이는 것"이 아니라 "경로를 바꾸는 것"이다.** 같은 데이터라도 NAT를 지나느냐 VPC Endpoint를 지나느냐, 오리진에서 직접 나가느냐 CDN을 거치느냐에 따라 비용이 크게 달라집니다.
...
청구서에 "EC2 1,200달러"라고 적혀 있으면 할 수 있는 일이 없습니다. 누구 것인지, 어느 서비스 것인지, 운영인지 개발인지 모르기 때문입니다. **비용을 줄이는 첫 단계는 비용에 주인을 붙이는 것**입니다.
...
1. **비용 할당 태그로 활성화한다.** Billing 콘솔에서 활성화하지 않으면 Cost Explorer에서 그 태그로 나눠 볼 수 없습니다. 활성화 이후 데이터부터 적용되므로 **빨리 켤수록 좋다.**
2. **태그 없는 리소스를 만들지 못하게 한다.** Organizations의 태그 정책이나 IAM 정책의 `Condition`으로 필수 태그를 강제하고, IaC 모듈에서 기본 태그를 자동 부착합니다. 사람의 성실함에 기대는 태깅은 반드시 무너집니다.
...
- **Cost Anomaly Detection**: 평소 패턴에서 벗어난 지출을 자동 탐지. 실수로 켠 대형 인스턴스를 월말이 아니라 다음 날 잡습니다.
...
**결론: Budgets와 Anomaly Detection은 오늘 당장 켠다.** 이 둘은 설정에 몇 분이면 되고, 사고를 한 달이 아니라 하루 만에 잡아줍니다.

---

## 7. 오토스케일링과 스케줄링으로 줄이기

가장 확실한 절감은 **안 쓰는 시간에 안 켜져 있는 것**입니다.
...
- **쿨다운을 너무 짧게 잡으면 출렁인다(thrashing).** 늘렸다 줄였다를 반복하면 오히려 불안정해집니다.
- **스케일 아웃은 빠르게, 스케일 인은 느리게.** 부족해서 생기는 손해가 남아서 생기는 손해보다 큽니다.
- **인스턴스 기동 시간이 길면 스케일링이 늦는다.** AMI를 미리 굽거나(golden image) 웜 풀(Warm Pool)을 씁니다.
...
RDS도 마찬가지입니다. 개발용 DB 인스턴스는 중지할 수 있습니다. 다만 **RDS 중지는 일정 기간이 지나면 AWS가 자동으로 다시 시작**시키므로(현재 최대 7일), 계속 꺼두려면 재중지 자동화가 필요합니다. Aurora Serverless 계열은 사용량에 따라 용량이 자동 조절되므로 이 관리 자체가 필요 없어집니다.
...
**증상**: 월중 비용 알림이 예산 임계치를 넘겼다는 메일. 배포도 없었고 트래픽도 그대로입니다.
...
- 꼬리 질문: "어떤 워크로드에 씁니까?" → CI 빌드 러너, 배치 처리, 미디어 인코딩, 체크포인트가 있는 ML 학습, 개발 환경. 상태를 가진 DB나 중단 시 복구 불가능한 단일 장시간 작업에는 쓰지 않습니다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|---|---|---|
| 인스턴스를 중지하면 비용이 0이 된다고 생각 | EBS 볼륨, 스냅샷, 미할당 EIP는 계속 과금 | 안 쓰면 삭제합니다. 중지와 삭제는 다르다 |
...
AWS 비용 최적화는 싼 옵션을 고르는 기술이 아니라 **"비용에 주인을 붙이고(태그), 안 쓰는 것을 끄고(스케줄링·정리), 확실히 쓰는 만큼만 약정하고(SP/RI), 중단을 견딜 수 있는 곳에만 Spot을 쓰며, 데이터가 흐르는 경로를 다시 그리는" 운영 습관**입니다.
