...
그래서 비용 최적화는 "싼 걸 고르는 기술"이 아니라 **가시성(누가 얼마를 쓰는지 보이게 하기)과 책임(그 비용의 주인이 있게 하기)의 문제**입니다. 인스턴스 타입을 한 단계 낮추는 것보다, 아무도 안 쓰는 스테이징 환경 열 개를 찾아 끄는 것이 대개 훨씬 큰 절감을 만듭니다.

> **비유**: 클라우드 비용은 수도 요금입니다. 수도꼭지가 100개인 집에서 몇 개가 새고 있는지 모르는 상태.
> **비유의 한계**: 수도는 잠그면 즉시 멈추지만, 클라우드는 **인스턴스를 꺼도 붙어 있는 디스크와 스냅샷과 미할당 IP는 계속 흐릅니다.** "껐다"와 "지웠다"는 다릅니다.

---

## 2. 컴퓨팅 과금 모델: 약정과 유연성의 교환

AWS가 컴퓨팅 요금제를 여러 개로 나눈 이유는 명확합니다. **AWS 입장에서 가장 비싼 것은 예측 불가능한 수요**입니다. 사용자가 "1년간 이만큼 쓰겠다"고 약속해주면 AWS는 용량 계획이 쉬워지고, 그 대가로 할인을 줍니다. 반대로 "언제든 뺏어가도 좋다"고 하면 AWS는 남는 용량을 팔 수 있으므로 더 큰 할인을 줍니다.
...
**Spot Instance**는 AWS의 유휴 용량을 빌립니다. 할인 폭이 가장 크지만 **AWS가 용량이 필요해지면 2분의 알림을 주고 회수합니다.** 이 2분을 어떻게 쓰느냐가 Spot 설계의 전부입니다.
...
**결론: 하나를 고르는 문제가 아니라 층을 쌓는 문제입니다.**
...
- **할당 전략을 용량 기준으로 잡습니다.** 가장 싼 풀만 고르면 그 풀이 가장 먼저 회수됩니다. 여유 용량이 많은 풀을 우선하는 전략을 쓰면 중단 빈도가 눈에 띄게 줄어듭니다.
...
클래스가 나뉘는 이유는 이 세 축의 가중치가 다르기 때문입니다. **저장이 싼 클래스는 검색이 비쌉니다.** 그래서 "얼마나 자주 읽는가"가 유일한 판단 기준이 됩니다.
...
주의할 함정: **최소 보관 기간이 있는 클래스에 짧게 머무는 데이터를 넣으면 손해입니다.** IA로 옮긴 지 3일 만에 삭제해도 30일치가 청구됩니다. 그리고 클래스 전환 자체에도 객체당 요청 요금이 붙어서, **아주 작은 파일 수백만 개를 Glacier로 옮기면 전환 비용이 절감액을 넘어설 수 있습니다.**
...
기본 원칙은 세 가지입니다. **들어오는 것(inbound)은 대체로 무료**이고, **나가는 것(outbound)이 과금**되며 인터넷으로 나갈수록·멀리 갈수록 비쌉니다. 그리고 **AWS 안에서도 경계를 넘으면 과금됩니다.** AZ를 넘고, 리전을 넘고, 퍼블릭 IP를 거치면 돈이 붙습니다.
...
**결론: 데이터 전송비를 줄이는 방법은 "전송량을 줄이는 것"이 아니라 "경로를 바꾸는 것"입니다.** 같은 데이터라도 NAT를 지나느냐 VPC Endpoint를 지나느냐, 오리진에서 직접 나가느냐 CDN을 거치느냐에 따라 비용이 크게 달라집니다.
...
1. **비용 할당 태그로 활성화합니다.** Billing 콘솔에서 활성화하지 않으면 Cost Explorer에서 그 태그로 나눠 볼 수 없습니다. 활성화 이후 데이터부터 적용되므로 **빨리 켤수록 좋습니다.**
...
**결론: Budgets와 Anomaly Detection은 오늘 당장 켭니다.** 이 둘은 설정에 몇 분이면 되고, 사고를 한 달이 아니라 하루 만에 잡아줍니다.
