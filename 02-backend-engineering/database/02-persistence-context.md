...
문제가 세 가지입니다. **같은 데이터를 여러 번 읽으면 그때마다 DB에 갑니다.** **같은 회원인데 객체가 여러 개라 어느 게 최신인지 알 수 없습니다.** **어디서 값을 바꿨는지 추적하려면 `update()` 호출을 개발자가 빠짐없이 챙겨야 합니다.**
...
**중요한 전제 하나.** Spring 환경에서 영속성 컨텍스트의 생존 범위는 기본적으로 **트랜잭션과 같습니다.** `@Transactional`이 붙은 메서드가 시작할 때 만들어지고, 끝날 때 닫힙니다. 이 문서의 거의 모든 동작은 "트랜잭션 안에서"라는 조건이 붙습니다.
...
**1차 캐시는 트랜잭션 범위입니다.** 다른 트랜잭션, 다른 요청과 공유되지 않습니다. 여러 트랜잭션이 공유하는(정확히는 `EntityManagerFactory` 단위의) 캐시는 2차 캐시이고, 따로 켜야 하는 별개의 기능입니다. "1차 캐시 덕분에 DB 부하가 줄어든다"는 말은 **한 트랜잭션 안에서 같은 엔티티를 여러 번 조회할 때만** 유효합니다.
...
**단, `GenerationType.IDENTITY`에서는 쓰기 지연이 동작하지 않습니다.** 영속성 컨텍스트는 엔티티를 식별자로 관리하는데, IDENTITY는 DB가 INSERT를 실행해야 식별자를 알려줍니다. 그래서 `persist()` 호출 즉시 INSERT가 나갑니다. MySQL을 쓰면서 "쓰기 지연 덕분에 성능이 좋다"고 말하면 꼬리 질문에서 무너지기 쉬운 지점입니다.
...
**기본 UPDATE는 모든 컬럼을 포함합니다.** `name`만 바꿔도 SQL에는 전체 컬럼이 들어갑니다. SQL 모양이 항상 같아야 DB가 실행 계획을 재사용할 수 있고, JDBC 배치로 묶기도 쉽기 때문입니다. 컬럼이 아주 많고 변경 컬럼이 적은 테이블이라면 `@DynamicUpdate`로 바꾼 컬럼만 보낼 수 있지만, 매번 다른 SQL이 만들어지는 비용도 함께 생기므로 기본값을 벗어날 근거가 있을 때만 씁니다.

**"조회했을 뿐인데 UPDATE가 나가는" 사고가 여기서 나옵니다.** 조회 메서드 안에서 엔티티 값을 잠깐 만졌다면(로깅용 필드 갱신, 방어 로직에서의 기본값 세팅 등) 그대로 DB에 반영됩니다. 읽기 전용 트랜잭션에는 `@Transactional(readOnly = true)`를 붙이자. Spring이 Hibernate 세션의 flush 모드를 `MANUAL`로 바꿔 커밋 시점의 flush 자체를 생략하므로, 실수로 값을 건드려도 UPDATE가 나가지 않습니다.
...
프록시는 **엔티티를 상속해서 런타임에 만들어진 가짜 객체**입니다. 내부에 실제 객체 참조(`target`)를 들고 있는데, 처음에는 비어 있습니다. 식별자를 제외한 메서드를 호출하는 순간 영속성 컨텍스트에게 "초기화해 달라"고 요청하고, 그때 SELECT가 실행됩니다.
...
**`readOnly = true`를 조회 메서드의 기본값으로 삼습니다.** flush가 생략되어 의도치 않은 UPDATE를 원천 차단하고, DB 이중화 환경에서는 읽기 전용 트랜잭션을 리플리카로 라우팅하는 근거로도 쓰입니다.

**`save()`를 호출해야 저장된다고 착각하지 않습니다.** Spring Data JPA의 `save()`는 새 엔티티면 `persist`, 그렇지 않으면 `merge`를 호출합니다. 이미 영속 상태인 엔티티라면 `merge`가 1차 캐시에서 같은 인스턴스를 찾아 그대로 돌려주므로 결과적으로 아무 일도 일어나지 않습니다. 변경 감지가 이미 처리하고 있기 때문입니다. 조회한 엔티티를 수정한 뒤 `save()`를 호출하는 코드는 동작에 문제는 없지만, 변경 감지를 이해하지 못한 신호로 읽힙니다.
