...
> `em.update()` 같은 메서드는 존재하지 않는데 왜 값이 저장되는지, 트랜잭션이 끝난 엔티티를 건드리면 왜 예외가 터지는지를 하나의 원리로 설명할 수 있게 됩니다.
...
문제가 세 가지입니다. **같은 데이터를 여러 번 읽으면 그때마다 DB에 간다.** **같은 회원인데 객체가 여러 개라 어느 게 최신인지 알 수 없다.** **어디서 값을 바꿨는지 추적하려면 `update()` 호출을 개발자가 빠짐없이 챙겨야 한다.**
...
> **비유** — 도서관에서 책을 빌려와 앉는 **개인 열람석**입니다. 한 번 가져온 책은 자리에 두고 다시 서고까지 갈 필요가 없고(1차 캐시), 반납할 때 한꺼번에 정리해 돌려준다(쓰기 지연). 책에 붙인 포스트잇은 반납 시점에 사서가 확인한다(변경 감지).
>
> **비유의 한계** — 열람석은 내가 직접 정리하지만, 영속성 컨텍스트는 **커밋 시점에 자동으로** 정리합니다. "내가 저장 명령을 안 내렸으니 안 저장되겠지"라는 감각으로 코드를 짜면 의도치 않은 UPDATE가 나갑니다.

**중요한 전제 하나.** Spring 환경에서 영속성 컨텍스트의 생존 범위는 기본적으로 **트랜잭션과 같다.** `@Transactional`이 붙은 메서드가 시작할 때 만들어지고, 끝날 때 닫힙니다. 이 문서의 거의 모든 동작은 "트랜잭션 안에서"라는 조건이 붙습니다.
...
상태 전이도에서 한 가지를 오해하기 쉽습니다. `em.merge()`는 **준영속 엔티티를 그 자리에서 영속으로 되돌리는 메서드가 아니다.** 같은 식별자의 영속 엔티티를 찾아(없으면 DB에서 조회해) 값을 복사한 뒤 **그 영속 인스턴스를 반환**합니다. 인자로 넘긴 객체는 여전히 준영속이므로 이후 작업은 반환값으로 이어가야 합니다.

준영속은 일부러 만드는 상태가 아니라 **자연스럽게 발생하는 상태입니다. `@Transactional` 서비스 메서드가 엔티티를 반환하는 순간, (뒤에 나올 OSIV를 꺼 두었다면) 그 엔티티는 컨트롤러 입장에서 준영속입니다. 이게 `LazyInitializationException`의 무대입니다.
...
**1차 캐시는 트랜잭션 범위다.** 다른 트랜잭션, 다른 요청과 공유되지 않습니다. 여러 트랜잭션이 공유하는(정확히는 `EntityManagerFactory` 단위의) 캐시는 2차 캐시이고, 따로 켜야 하는 별개의 기능입니다. "1차 캐시 덕분에 DB 부하가 줄어든다"는 말은 **한 트랜잭션 안에서 같은 엔티티를 여러 번 조회할 때만** 유효합니다.

**JPQL은 1차 캐시를 거치지 않는다.** `em.find()`는 캐시를 먼저 보지만, `select m from Member m where ...` 같은 JPQL은 무조건 DB로 SQL을 날립니다. 조건에 맞는 행이 무엇인지는 DB만 알기 때문입니다. 다만 결과를 영속성 컨텍스트에 넣을 때, **이미 같은 식별자의 엔티티가 캐시에 있으면 DB에서 읽은 값을 버리고 캐시의 인스턴스를 반환한다.** 동일성을 깨뜨리지 않기 위한 규칙입니다.
...
SQL을 모아 뒀다가 한 번에 보내면 네트워크 왕복이 줄고, JDBC 배치(`hibernate.jdbc.batch_size`)를 켜면 여러 INSERT를 한 번의 통신으로 묶을 수도 있습니다.

**단, `GenerationType.IDENTITY`에서는 쓰기 지연이 동작하지 않는다.** 영속성 컨텍스트는 엔티티를 식별자로 관리하는데, IDENTITY는 DB가 INSERT를 실행해야 식별자를 알려줍니다. 그래서 `persist()` 호출 즉시 INSERT가 나갑니다. MySQL을 쓰면서 "쓰기 지연 덕분에 성능이 좋다"고 말하면 꼬리 질문에서 무너지기 쉬운 지점입니다.
...
**영속 상태에서만 동작한다.** 준영속 엔티티는 스냅샷이 없으므로 아무리 setter를 호출해도 UPDATE가 나가지 않습니다.

**기본 UPDATE는 모든 컬럼을 포함한다.** `name`만 바꿔도 SQL에는 전체 컬럼이 들어갑니다. SQL 모양이 항상 같아야 DB가 실행 계획을 재사용할 수 있고, JDBC 배치로 묶기도 쉽기 때문입니다. 컬럼이 아주 많고 변경 컬럼이 적은 테이블이라면 `@DynamicUpdate`로 바꾼 컬럼만 보낼 수 있지만, 매번 다른 SQL이 만들어지는 비용도 함께 생기므로 기본값을 벗어날 근거가 있을 때만 씁니다.

**"조회했을 뿐인데 UPDATE가 나가는" 사고가 여기서 나온다.** 조회 메서드 안에서 엔티티 값을 잠깐 만졌다면(로깅용 필드 갱신, 방어 로직에서의 기본값 세팅 등) 그대로 DB에 반영됩니다. 읽기 전용 트랜잭션에는 `@Transactional(readOnly = true)`를 붙이자. Spring이 Hibernate 세션의 flush 모드를 `MANUAL`로 바꿔 커밋 시점의 flush 자체를 생략하므로, 실수로 값을 건드려도 UPDATE가 나가지 않습니다.
...
JPQL은 DB에 SQL을 날려 결과를 받아옵니다. 메모리에만 있고 DB에 없는 데이터는 조회될 수 없습니다. 그래서 Hibernate는 쿼리를 실행하기 전에 먼저 flush해서 메모리와 DB를 맞춥니다. `FlushModeType.COMMIT`으로 바꾸면 이 자동 flush를 끌 수 있지만, 위 상황에서 조회 결과가 누락되므로 정확히 알고 써야 합니다.
...
프록시는 **엔티티를 상속해서 런타임에 만들어진 가짜 객체입니다. 내부에 실제 객체 참조(`target`)를 들고 있는데, 처음에는 비어 있습니다. 식별자를 제외한 메서드를 호출하는 순간 영속성 컨텍스트에게 "초기화해 달라"고 요청하고, 그때 SELECT가 실행됩니다.
...
가장 자주 만나는 JPA 예외입니다. 원인은 한 줄로 정리됩니다. **영속성 컨텍스트가 닫힌 뒤에 초기화되지 않은 프록시를 건드렸다.**
...
애초에 프록시를 만들지 않으므로 예외가 발생할 여지가 없습니다. 다만 무엇을 함께 가져올지 쿼리마다 정해야 하고, 컬렉션을 여러 개 fetch join할 때는 별도의 제약이 있습니다. [N+1 문제](./03-n-plus-one-problem.md) 문서에서 다룹니다.
...
**왜 문제인가.** 예외는 사라지지만 트랜잭션이 뷰 렌더링과 응답 직렬화까지 열려 있게 됩니다. DB 커넥션을 그만큼 오래 붙잡으므로 트래픽이 몰리면 커넥션 풀이 먼저 마릅니다. 게다가 프레젠테이션 계층이 트랜잭션 경계를 갖는 건 책임 분리가 깨지는 설계입니다. 위 2)번으로 해결하는 게 맞습니다.

**4) OSIV(Open Session In View)**

Spring Boot에는 `spring.jpa.open-in-view` 설정이 있고 기본값이 `true`다. 켜져 있으면 영속성 컨텍스트를 컨트롤러/뷰 구간까지 열어두기 때문에 지연 로딩이 동작합니다. 애플리케이션 기동 시 이에 대한 경고 로그가 출력되는 이유가 바로 3)번과 같은 커넥션 점유 문제 때문입니다. 트래픽이 많은 API 서버라면 `false`로 끄고, 지연 로딩은 서비스 계층 안에서 끝내는 구조로 가는 편이 안전합니다.
...
이 UPDATE는 DB에 바로 실행되고, 1차 캐시에 있는 엔티티들은 예전 값을 그대로 들고 있습니다. 이후 같은 트랜잭션에서 엔티티를 읽으면 **DB와 다른 값**을 보게 됩니다. `clearAutomatically = true`로 실행 후 컨텍스트를 비우거나, 벌크 연산을 트랜잭션의 마지막에 배치합니다.

**`readOnly = true`를 조회 메서드의 기본값으로 삼는다.** flush가 생략되어 의도치 않은 UPDATE를 원천 차단하고, DB 이중화 환경에서는 읽기 전용 트랜잭션을 리플리카로 라우팅하는 근거로도 쓰입니다.

**`save()`를 호출해야 저장된다고 착각하지 않는다.** Spring Data JPA의 `save()`는 새 엔티티면 `persist`, 그렇지 않으면 `merge`를 호출합니다. 이미 영속 상태인 엔티티라면 `merge`가 1차 캐시에서 같은 인스턴스를 찾아 그대로 돌려주므로 결과적으로 아무 일도 일어나지 않습니다. 변경 감지가 이미 처리하고 있기 때문입니다. 조회한 엔티티를 수정한 뒤 `save()`를 호출하는 코드는 동작에 문제는 없지만, 변경 감지를 이해하지 못한 신호로 읽힙니다.
...
- 꼬리 질문: "변경된 필드만 UPDATE하나요?" → 기본은 전체 컬럼입니다. SQL 모양을 고정해 실행 계획 재사용과 배치 처리에 유리하기 때문이며, 필요하면 `@DynamicUpdate`로 바꿀 수 있습니다.
...
- 꼬리 질문: "flush하면 1차 캐시가 비워지나요?" → 아닙니다. 비우는 건 `clear()`다.
...
영속성 컨텍스트는 트랜잭션 동안 엔티티를 붙잡아 두는 작업 공간이고, 1차 캐시·동일성 보장·쓰기 지연·변경 감지·지연 로딩은 모두 "엔티티를 메모리에서 관리한다"는 하나의 전제에서 파생된 결과입니다.
