...
> `em.update()` 같은 메서드는 존재하지 않는데 왜 값이 저장되는지, 트랜잭션이 끝난 엔티티를 건드리면 왜 예외가 터지는지를 하나의 원리로 설명할 수 있게 된다.
...
문제가 세 가지다. **같은 데이터를 여러 번 읽으면 그때마다 DB에 간다.** **같은 회원인데 객체가 여러 개라 어느 게 최신인지 알 수 없다.** **어디서 값을 바꿨는지 추적하려면 `update()` 호출을 개발자가 빠짐없이 챙겨야 한다.**
...
>
> **비유의 한계** — 열람석은 내가 직접 정리하지만, 영속성 컨텍스트는 **커밋 시점에 자동으로** 정리한다. "내가 저장 명령을 안 내렸으니 안 저장되겠지"라는 감각으로 코드를 짜면 의도치 않은 UPDATE가 나간다.

**중요한 전제 하나.** Spring 환경에서 영속성 컨텍스트의 생존 범위는 기본적으로 **트랜잭션과 같다.** `@Transactional`이 붙은 메서드가 시작할 때 만들어지고, 끝날 때 닫힌다. 이 문서의 거의 모든 동작은 "트랜잭션 안에서"라는 조건이 붙는다.
...
상태 전이도에서 한 가지를 오해하기 쉽다. `em.merge()`는 **준영속 엔티티를 그 자리에서 영속으로 되돌리는 메서드가 아니다.** 같은 식별자의 영속 엔티티를 찾아(없으면 DB에서 조회해) 값을 복사한 뒤 **그 영속 인스턴스를 반환**한다. 인자로 넘긴 객체는 여전히 준영속이므로 이후 작업은 반환값으로 이어가야 한다.

준영속은 일부러 만드는 상태가 아니라 **자연스럽게 발생하는 상태**다. `@Transactional` 서비스 메서드가 엔티티를 반환하는 순간, (뒤에 나올 OSIV를 꺼 두었다면) 그 엔티티는 컨트롤러 입장에서 준영속이다. 이게 `LazyInitializationException`의 무대다.
...
같은 트랜잭션 안에서 같은 식별자로 조회하면 **항상 같은 인스턴스**가 나온다. 자바 컬렉션에서 같은 키로 꺼내면 같은 객체가 나오는 것과 같은 감각으로 엔티티를 다룰 수 있다는 뜻이다.

주의할 점 두 가지다.

**1차 캐시는 트랜잭션 범위다.** 다른 트랜잭션, 다른 요청과 공유되지 않는다. 여러 트랜잭션이 공유하는(정확히는 `EntityManagerFactory` 단위의) 캐시는 2차 캐시이고, 따로 켜야 하는 별개의 기능이다. "1차 캐시 덕분에 DB 부하가 줄어든다"는 말은 **한 트랜잭션 안에서 같은 엔티티를 여러 번 조회할 때만** 유효하다.

**JPQL은 1차 캐시를 거치지 않는다.** `em.find()`는 캐시를 먼저 보지만, `select m from Member m where ...` 같은 JPQL은 무조건 DB로 SQL을 날린다. 조건에 맞는 행이 무엇인지는 DB만 알기 때문이다. 다만 결과를 영속성 컨텍스트에 넣을 때, **이미 같은 식별자의 엔티티가 캐시에 있으면 DB에서 읽은 값을 버리고 캐시의 인스턴스를 반환한다.** 동일성을 깨뜨리지 않기 위한 규칙이다.
...
SQL을 모아 뒀다가 한 번에 보내면 네트워크 왕복이 줄고, JDBC 배치(`hibernate.jdbc.batch_size`)를 켜면 여러 INSERT를 한 번의 통신으로 묶을 수도 있다.

**단, `GenerationType.IDENTITY`에서는 쓰기 지연이 동작하지 않는다.** 영속성 컨텍스트는 엔티티를 식별자로 관리하는데, IDENTITY는 DB가 INSERT를 실행해야 식별자를 알려준다. 그래서 `persist()` 호출 즉시 INSERT가 나간다. MySQL을 쓰면서 "쓰기 지연 덕분에 성능이 좋다"고 말하면 꼬리 질문에서 무너지기 쉬운 지점이다.

---

## 5. 변경 감지 (Dirty Checking)

JPA에는 `em.update()`가 없다. 값만 바꾸면 커밋 시점에 UPDATE가 나간다.
...
**영속 상태에서만 동작한다.** 준영속 엔티티는 스냅샷이 없으므로 아무리 setter를 호출해도 UPDATE가 나가지 않는다.

**기본 UPDATE는 모든 컬럼을 포함한다.** `name`만 바꿔도 SQL에는 전체 컬럼이 들어간다. SQL 모양이 항상 같아야 DB가 실행 계획을 재사용할 수 있고, JDBC 배치로 묶기도 쉽기 때문이다. 컬럼이 아주 많고 변경 컬럼이 적은 테이블이라면 `@DynamicUpdate`로 바꾼 컬럼만 보낼 수 있지만, 매번 다른 SQL이 만들어지는 비용도 함께 생기므로 기본값을 벗어날 근거가 있을 때만 쓴다.

**"조회했을 뿐인데 UPDATE가 나가는" 사고가 여기서 나온다.** 조회 메서드 안에서 엔티티 값을 잠깐 만졌다면(로깅용 필드 갱신, 방어 로직에서의 기본값 세팅 등) 그대로 DB에 반영된다. 읽기 전용 트랜잭션에는 `@Transactional(readOnly = true)`를 붙이자. Spring이 Hibernate 세션의 flush 모드를 `MANUAL`로 바꿔 커밋 시점의 flush 자체를 생략하므로, 실수로 값을 건드려도 UPDATE가 나가지 않는다.
...
**JPQL 실행 직전에 flush가 자동으로 일어나는 이유**가 중요하다.
...
JPQL은 DB에 SQL을 날려 결과를 받아온다. 메모리에만 있고 DB에 없는 데이터는 조회될 수 없다. 그래서 Hibernate는 쿼리를 실행하기 전에 먼저 flush해서 메모리와 DB를 맞춘다. `FlushModeType.COMMIT`으로 바꾸면 이 자동 flush를 끌 수 있지만, 위 상황에서 조회 결과가 누락되므로 정확히 알고 써야 한다.
...
프록시는 원본 클래스가 아니라 **자식 클래스**이므로 타입 비교에 주의한다.
...
가장 자주 만나는 JPA 예외다. 원인은 한 줄로 정리된다. **영속성 컨텍스트가 닫힌 뒤에 초기화되지 않은 프록시를 건드렸다.**
...
애초에 프록시를 만들지 않으므로 예외가 발생할 여지가 없다. 다만 무엇을 함께 가져올지 쿼리마다 정해야 하고, 컬렉션을 여러 개 fetch join할 때는 별도의 제약이 있다. [N+1 문제](./03-n-plus-one-problem.md) 문서에서 다룬다.
...
엔티티를 계층 밖으로 내보내지 않는다는 원칙만 지키면 이 예외는 구조적으로 발생하지 않는다.
...
**왜 문제인가.** 예외는 사라지지만 트랜잭션이 뷰 렌더링과 응답 직렬화까지 열려 있게 된다. DB 커넥션을 그만큼 오래 붙잡으므로 트래픽이 몰리면 커넥션 풀이 먼저 마른다. 게다가 프레젠테이션 계층이 트랜잭션 경계를 갖는 건 책임 분리가 깨지는 설계다. 위 2)번으로 해결하는 게 맞다.

**4) OSIV(Open Session In View)**

Spring Boot에는 `spring.jpa.open-in-view` 설정이 있고 기본값이 `true`다. 켜져 있으면 영속성 컨텍스트를 컨트롤러/뷰 구간까지 열어두기 때문에 지연 로딩이 동작한다. 애플리케이션 기동 시 이에 대한 경고 로그가 출력되는 이유가 바로 3)번과 같은 커넥션 점유 문제 때문이다. 트래픽이 많은 API 서버라면 `false`로 끄고, 지연 로딩은 서비스 계층 안에서 끝내는 구조로 가는 편이 안전하다.
...
이 UPDATE는 DB에 바로 실행되고, 1차 캐시에 있는 엔티티들은 예전 값을 그대로 들고 있다. 이후 같은 트랜잭션에서 엔티티를 읽으면 **DB와 다른 값**을 보게 된다. `clearAutomatically = true`로 실행 후 컨텍스트를 비우거나, 벌크 연산을 트랜잭션의 마지막에 배치한다.

**`readOnly = true`를 조회 메서드의 기본값으로 삼는다.** flush가 생략되어 의도치 않은 UPDATE를 원천 차단하고, DB 이중화 환경에서는 읽기 전용 트랜잭션을 리플리카로 라우팅하는 근거로도 쓰인다.

**`save()`를 호출해야 저장된다고 착각하지 않는다.** Spring Data JPA의 `save()`는 새 엔티티면 `persist`, 그렇지 않으면 `merge`를 호출한다. 이미 영속 상태인 엔티티라면 `merge`가 1차 캐시에서 같은 인스턴스를 찾아 그대로 돌려주므로 결과적으로 아무 일도 일어나지 않는다. 변경 감지가 이미 처리하고 있기 때문이다. 조회한 엔티티를 수정한 뒤 `save()`를 호출하는 코드는 동작에 문제는 없지만, 변경 감지를 이해하지 못한 신호로 읽힌다.
...
A. 엔티티를 관리하는 애플리케이션과 DB 사이의 중간 저장소입니다. 1차 캐시로 같은 트랜잭션 내 반복 조회를 줄이고, 같은 식별자에 대해 같은 인스턴스를 반환해 동일성을 보장합니다. SQL을 모아 커밋 시점에 보내는 쓰기 지연과, 스냅샷 비교로 UPDATE를 자동 생성하는 변경 감지가 여기서 나옵니다.
- 꼬리 질문: "생명주기가 어떻게 되나요?" → Spring에서는 기본적으로 트랜잭션과 동일하다. 트랜잭션 시작 시 생성되고 종료 시 닫힌다.
...
- 꼬리 질문: "변경된 필드만 UPDATE하나요?" → 기본은 전체 컬럼이다. SQL 모양을 고정해 실행 계획 재사용과 배치 처리에 유리하기 때문이며, 필요하면 `@DynamicUpdate`로 바꿀 수 있다.

**Q. flush와 commit의 차이는 무엇인가요?**

A. flush는 쌓아둔 SQL을 DB로 보내 상태를 동기화하는 것이고, commit은 트랜잭션을 확정하는 것입니다. flush 이후에도 롤백이 가능하고 1차 캐시는 유지됩니다. commit은 내부적으로 flush를 먼저 수행한 뒤 트랜잭션을 끝냅니다. JPQL 실행 직전에도 자동 flush가 일어나는데, 아직 DB에 없는 변경분이 조회 결과에서 빠지는 걸 막기 위해서입니다.
- 꼬리 질문: "flush하면 1차 캐시가 비워지나요?" → 아니다. 비우는 건 `clear()`다.

**Q. `LazyInitializationException`은 왜 발생하고 어떻게 해결하나요?**

A. 트랜잭션이 끝나 영속성 컨텍스트가 닫힌 뒤 초기화되지 않은 프록시에 접근했기 때문입니다. 해결은 크게 두 갈래인데, 필요한 연관을 fetch join이나 `@EntityGraph`로 미리 가져오거나, 트랜잭션 안에서 DTO로 변환해 반환하는 것입니다. 컨트롤러에 `@Transactional`을 붙이거나 OSIV에 의존하는 방식은 커넥션 점유 시간이 길어져 권장하지 않습니다.
- 꼬리 질문: "OSIV를 끄면 뭐가 달라지나요?" → 지연 로딩이 서비스 계층 밖에서 동작하지 않으므로, 조회에 필요한 데이터를 트랜잭션 안에서 모두 확정해야 한다.
...
영속성 컨텍스트는 트랜잭션 동안 엔티티를 붙잡아 두는 작업 공간이고, 1차 캐시·동일성 보장·쓰기 지연·변경 감지·지연 로딩은 모두 "엔티티를 메모리에서 관리한다"는 하나의 전제에서 파생된 결과다.
