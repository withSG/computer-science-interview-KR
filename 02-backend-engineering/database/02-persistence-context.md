...
Spring Boot에는 `spring.jpa.open-in-view` 설정이 있고 기본값이 `true`다. 켜져 있으면 영속성 컨텍스트를 컨트롤러/뷰 구간까지 열어두기 때문에 지연 로딩이 동작합니다. 애플리케이션 기동 시 이에 대한 경고 로그가 출력되는 이유가 바로 3)번과 같은 커넥션 점유 문제 때문입니다. 트래픽이 많은 API 서버라면 `false`로 끄고 지연 로딩은 서비스 계층 안에서 끝내는 구조가 안전합니다.
