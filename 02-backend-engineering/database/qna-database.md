...
### 핵심 답변
영속성 컨텍스트는 엔티티를 영구 저장하는 환경입니다. 애플리케이션과 데이터베이스 사이에서 **중재자** 역할을 합니다.
...
### 핵심 답변
인덱스는 데이터베이스 검색 속도를 높이는 자료구조입니다. 대부분의 RDBMS는 **B-Tree(B+Tree)** 구조를 씁니다.
...
> 카디널리티(Cardinality)는 **그 컬럼에 들어 있는 서로 다른 값의 개수**입니다. 이메일처럼 값이 다양하면 카디널리티가 높고, 성별처럼 값이 두세 종류뿐이면 낮습니다.

**인덱스가 효과적인 경우:**
- WHERE 조건에 자주 사용되는 컬럼
- 카디널리티가 높은 컬럼 (유니크한 값이 많아 조건 하나로 결과가 크게 줄어듦)
- JOIN, ORDER BY에 사용되는 컬럼

**인덱스가 비효율적인 경우:**
- 카디널리티가 낮은 컬럼 (성별 등 — 읽어야 할 행이 너무 많아 옵티마이저가 풀 스캔을 고릅니다)
...
3. **DTO 변환**: 영속성 컨텍스트 내에서 DTO로 변환 (실무 표준)
4. **트랜잭션 범위 확장 / OSIV**: 예외는 사라지지만 DB 커넥션을 뷰 렌더링 구간까지 붙잡으므로 권장하지 않음
