...
> Java에 Call by Reference가 존재하지 않는다는 사실을 코드로 증명하고, 참조값 복사가 만드는 착시가 어떤 버그로 이어지는지, 불변 객체와 방어적 복사가 왜 해답인지 설명할 수 있게 된다.
...
이 코드를 보고 대부분 "Java는 참조를 넘기는구나"라고 생각한다.
...
같은 참조 타입인데 하나는 원본이 바뀌고 하나는 안 바뀐다. 이 두 결과를 **하나의 규칙으로** 설명하지 못하면 이후에 나올 방어적 복사나 불변 객체 이야기가 전부 암기가 된다.
...
핵심은 **"객체가 바뀌느냐"가 아니라 "호출부의 *변수*가 다른 것을 가리키게 되느냐"** 다. 이 구분이 두 방식을 가르는 유일한 기준이다.

C++에는 진짜 Call by Reference가 있다.
...
Java에는 이런 문법이 없다. **Java에서 메서드에 넘길 수 있는 것은 오직 값(원시 값 또는 참조값)의 복사본뿐이다.**

> **비유**: 친구에게 집 주소가 적힌 쪽지를 복사해 준다. 친구가 그 주소로 찾아가 가구를 옮기면 내 집도 바뀐다(객체 상태 변경). 하지만 친구가 자기 쪽지를 지우고 다른 주소를 적어도 내 쪽지는 그대로다(참조 재할당).
> **비유의 한계**: 쪽지의 "주소"는 사람이 읽을 수 있지만 Java의 참조값은 개발자가 볼 수도 연산할 수도 없다. 그래서 C의 포인터 산술 같은 건 불가능하다.
...
Call by Reference인 언어에서는 두 변수를 교환하는 메서드를 만들 수 있다. Java에서는 **어떤 수를 써도 만들 수 없다.**
...
`a`와 `b`는 `swap` 프레임 안의 지역 변수일 뿐이다. 이들을 아무리 뒤바꿔도 `main`의 `x`, `y`에는 닿을 방법이 없다.

Java에서 교환 효과를 내려면 **교환할 대상을 담은 컨테이너를 넘겨서 그 컨테이너의 내용을 바꿔야** 한다.
...
**이것이 "Java는 항상 Call by Value"의 가장 명확한 증거다.** 면접에서 이 예제 하나면 설명이 끝난다.
...
둘 다 참조 타입인데 결과가 다르다. 규칙은 여전히 하나다.

- **String은 불변**이라 "내용을 수정하는 연산"이 존재하지 않는다. `+`는 새 객체를 만든다. 그래서 참조 재할당(B 케이스)밖에 일어날 수 없고, 원본은 절대 안 바뀐다.
- **배열은 가변**이라 `arr[0] = 999`가 객체 내부 수정(A 케이스)이다.

**불변이냐 가변이냐가 결과를 가른다.** Call by Value 여부가 아니다. 이 통찰이 다음 절로 이어진다.
...
**왜 문제인가**: `addLine`에 애써 넣은 상태 검증이 무력화된다.
...
`getLines()`가 돌려준 것은 리스트의 **복사본이 아니라 내부 리스트를 가리키는 참조값**이다. 외부에서 이 참조로 `add()`를 호출하면 `Order`의 검증을 건너뛰고 내부 상태가 바뀐다. 캡슐화가 완전히 뚫린 것이다.
...
**"경계를 넘나드는 가변 객체는 들어올 때와 나갈 때 모두 복사한다"** 가 원칙이다.
...
`unmodifiableList`는 **수정만 막는 창문**이지 복사가 아니다. 원본이 바뀌면 뷰도 바뀐다. 내부 리스트를 계속 변경하는 클래스에서 이걸 반환하면 여전히 외부에 내부 상태 변화가 노출된다. 확실히 끊으려면 `List.copyOf`(Java 10+) 또는 `new ArrayList<>(internal)`을 쓴다.
...
리스트를 복사해도 **원소 객체는 같은 것을 공유**한다(얕은 복사). 원소가 가변이면 여전히 내부 상태가 바뀐다. 근본 해법은 **원소를 불변으로 만드는 것**이다.

---

## 8. 불변 객체가 왜 안전한가

불변 객체는 만들어진 뒤 상태가 절대 바뀌지 않는다. 그래서 아래 문제들이 **애초에 발생하지 않는다.**

- **방어적 복사가 불필요**하다. 넘겨줘도 상대가 바꿀 수 없다.
- **스레드 안전**하다. 바뀌는 값이 없으니 경쟁 조건이 없고 락도 필요 없다.
- **HashMap 키로 안전**하다. 키를 넣은 뒤 필드를 바꾸면 해시가 달라져 그 엔트리를 영영 못 찾는데, 불변이면 그런 일이 없다.
- **디버깅이 쉽다.** 값이 잘못됐으면 만든 곳 하나만 보면 된다.
...
Java 16의 `record`는 필드가 `final`이고 접근자가 자동 생성되지만, **참조하는 객체까지 불변으로 만들어주지는 않는다.**
...
`record`가 보장하는 것은 **"필드 참조가 다른 객체를 가리키도록 바뀌지 않는다"** 까지다. 그 너머는 직접 처리해야 한다.

---

## 9. 실무에서는

- **JPA 엔티티의 컬렉션**: `getOrderLines()`가 Hibernate의 영속성 컬렉션을 그대로 반환하면 외부에서 `add()`한 것이 그대로 DB에 반영된다. 도메인 규칙을 강제하려면 컬렉션은 `Collections.unmodifiableList` 또는 복사본으로 내보내고, 변경은 `addOrderLine()` 같은 의미 있는 메서드로만 하게 한다.
- **DTO와 엔티티의 경계**: 컨트롤러가 엔티티를 그대로 반환하면 영속성 컨텍스트의 객체가 직렬화 계층까지 흘러간다. DTO로 변환하는 것은 화면 스펙 분리 목적도 있지만 **참조를 끊는 방어적 복사** 성격이 크다.
- **`java.time` vs 옛날 `Date`**: `java.util.Date`는 가변이라 필드로 보관하려면 매번 복사해야 했다. `LocalDate`, `LocalDateTime`, `Instant`는 불변이라 그럴 필요가 없다. 새 코드에서 `Date`를 쓰지 않는 실질적인 이유 중 하나다.
- **설정 객체**: 애플리케이션 기동 시 읽어 여러 스레드가 공유하는 설정은 불변으로 만들면 동기화가 통째로 사라진다.
...
**Q. 불변 객체의 장점을 설명해주세요.**
A. 상태가 안 바뀌므로 여러 스레드가 락 없이 공유할 수 있고, 방어적 복사가 필요 없으며, HashMap 키로 써도 해시가 변하지 않아 안전합니다. 값이 잘못됐을 때 생성 지점 한 곳만 보면 되므로 디버깅도 쉽습니다. 단점은 값이 바뀔 때마다 새 객체를 만들어 GC 부담이 늘 수 있다는 점입니다.
...
Java에서 넘어가는 것은 언제나 **값의 복사본**이며, 원본이 바뀐 것처럼 보이는 이유는 **복사된 참조가 같은 객체를 가리키기 때문**이다. 이 착시를 막는 도구가 방어적 복사와 불변 객체다.
