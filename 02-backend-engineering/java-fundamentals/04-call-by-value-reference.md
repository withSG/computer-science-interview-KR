...
> 참조 타입을 넘기면 원본이 바뀌니, Java도 Call by Reference라고 믿기 쉽습니다. 그 믿음은 틀렸습니다. 이 문서는 틀린 이유를 코드로 증명하고, 참조값 복사가 만드는 착시가 어떤 버그로 이어지는지, 불변 객체와 방어적 복사가 왜 해답인지를 가려냅니다.
...
>
> **비유의 한계**: 쪽지의 "주소"는 사람이 읽을 수 있습니다. 하지만 Java의 참조값은 개발자가 볼 수도 연산할 수도 없어서, C의 포인터 산술 같은 건 불가능합니다.
...
- **String은 불변**이라 "내용을 수정하는 연산"이 존재하지 않습니다. `+`는 새 객체를 만듭니다. 그래서 일어날 수 있는 것은 참조 재할당(B 케이스)뿐이고, 원본은 절대 안 바뀝니다.
...
1. 클래스를 `final`로 (또는 생성자를 `private`으로) 만들어 하위 클래스가 가변성을 끼워 넣지 못하게 합니다
2. 모든 필드를 `private final`로 선언합니다
3. 가변 객체를 생성자로 받으면 **복사해서** 보관합니다
4. 가변 객체를 반환할 때도 **복사해서** 내보냅니다 (또는 불변 컬렉션으로 보관)
5. 상태를 바꾸는 메서드 대신 **새 인스턴스를 반환하는 메서드**를 만듭니다
