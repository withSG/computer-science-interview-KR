...
> 캡슐화·상속·다형성·추상화가 각각 어떤 문제를 푸는 도구인지 설명하고, SOLID 다섯 원칙을 위반 코드와 개선 코드로 대조해 말할 수 있게 된다.
...
- 클래스·인터페이스·상속의 문법 수준 이해면 충분하다. 이 문서부터 시작해도 된다.

---

## 1. 왜 필요한가

객체지향은 "현실 세계를 코드로 옮기기 위해" 나온 게 아니다. **변경 비용을 낮추기 위해** 나왔다.
...
1. **분기 폭발**: 결제 수단 N개 × 기능 M개 = N×M 개의 분기가 코드 곳곳에 흩어진다. 환불·취소 메서드에도 같은 if-else가 복사된다.
2. **변경 영향 범위 예측 불가**: 카카오페이 로직 하나 고치려고 열었는데 카드 결제까지 같이 배포된다.
3. **데이터 무결성 보호 실패**: `amount`를 아무나 음수로 바꿔도 막을 곳이 없다.

OOP의 네 가지 특성은 이 문제들에 대한 각각의 처방이다. "4대 특성 암기"가 아니라 **처방전으로** 읽어야 한다.
...
"이 필드 값이 언제 어디서 이상해졌는지 모르겠다"는 문제를 푼다.
...
**왜 문제인가**: 필드가 public이면 **잔액을 바꾸는 코드가 프로젝트 전체에 몇 군데인지 알 수 없다.** 버그가 났을 때 용의자가 수백 곳이다. 검증 로직을 나중에 추가하려 해도 붙일 자리가 없다.
...
이제 잔액이 변하는 통로는 `withdraw` 하나뿐이다. 버그 용의자가 수백 곳에서 한 곳으로 줄었다.

> **비유**: 자동차 운전대와 페달. 운전자는 엔진 내부를 몰라도 되고, 함부로 만질 수도 없다.
> **비유의 한계**: 자동차 보닛은 물리적으로 잠기지만 Java의 `private`은 리플렉션으로 뚫린다. 캡슐화는 *물리적 차단*이 아니라 *컴파일러가 강제하는 약속*이다.

### 흔한 오해

**"필드를 private으로 하고 getter/setter를 만들면 캡슐화다"** — 아니다. 모든 필드에 setter를 열어두면 `public` 필드와 다를 게 없다. 이름만 `setBalance()`로 바뀌었을 뿐 검증이 없다면 캡슐화가 아니다. 핵심은 접근 제어자가 아니라 **"객체에게 데이터를 달라고 하지 말고, 일을 시켜라"** 는 원칙이다.
...
공통 코드 복붙을 없앤다. `Dog`, `Cat`이 각자 `name`, `age`, `eat()`을 들고 있을 필요가 없다.

문제는 상속이 OOP 4대 특성 중 **가장 오용된다**는 점이다. 상속은 코드 재사용 수단이 아니라 **"is-a 관계 선언 + 부모의 모든 public 계약 승계"** 이기 때문이다.
...
**왜 문제인가**: `Stack`은 "맨 위에서만 넣고 뺀다"는 불변식을 지켜야 하는데, `ArrayList`를 상속하는 순간 `add(int, E)`, `remove(int)`, `set(...)` 같은 구멍이 전부 공개된다. 자식이 부모의 계약을 좁힐 방법이 없다.
...
외부에 노출되는 API는 이제 내가 고른 것만이다. 내부 자료구조를 `LinkedList`로 바꿔도 밖은 영향이 없다.

**판단 기준**: "B는 A의 일종인가?"에 자신 있게 예라고 답할 수 없으면 상속하지 마라. `Stack`은 `ArrayList`의 일종이 아니라 `ArrayList`를 *쓰는* 것이다.

---

## 4. 다형성 (Polymorphism)

1절에서 본 if-else 분기 폭발을 없앤다.
...
토스페이를 추가할 때 `PaymentService`는 **한 글자도 바뀌지 않는다**. 새 클래스 파일 하나만 추가하면 된다.
...
**오버로딩(Overloading)은 컴파일 타임**에, **오버라이딩(Overriding)은 런타임**에 결정된다. 확장 가능성을 만들어내는 것은 후자다. 면접에서 "다형성"이라고 하면 사실상 오버라이딩 기반의 동적 디스패치를 가리킨다.

---

## 5. 추상화 (Abstraction)

"이 기능을 쓰려면 알아야 할 것"의 양을 줄인다. JDBC가 교과서적인 예다. 애플리케이션은 `Connection`, `PreparedStatement`, `ResultSet` 세 인터페이스만 알면 MySQL이든 PostgreSQL이든 똑같이 쓴다. 드라이버 JAR을 갈아끼우고 URL만 바꾸면 된다.

추상화는 **"구현을 감췄는가"** 가 아니라 **"구현이 바뀌어도 이 약속이 안 깨지는가"** 로 판단한다.
...
위쪽은 MongoDB로 갈아끼우는 순간 컴파일이 깨진다. 감추는 데 실패한 것이다. 이걸 **추상화 누수(Leaky Abstraction)** 라고 한다.

---

## 6. SOLID — 위반 코드에서 개선 코드로

SOLID는 "지켜야 할 규칙 5개"가 아니라 **코드 냄새를 진단하는 다섯 개의 렌즈**다.

### SRP (Single Responsibility Principle) — 단일 책임

**정의**: 클래스를 변경할 이유는 하나여야 한다.
...
**왜 문제인가**: 이 클래스는 세 가지 이유로 바뀐다. 주문 정책, 메일 템플릿, 정산 포맷. 마케팅팀의 메일 문구 수정 요청 하나가 주문 로직 배포로 이어진다. 테스트도 지옥이다. 주문 저장만 검증하고 싶은데 메일 서버와 파일 시스템을 흉내 내야 한다.
...
**주의**: SRP는 "메서드를 잘게 쪼개라"가 아니다. 클래스 10개짜리 기능을 100개로 만들면 오히려 추적이 어려워진다. 기준은 크기가 아니라 **"변경을 요청하는 사람이 같은가"** 다.
...
**왜 문제인가**: 등급을 추가하려면 *이미 테스트를 통과해 운영 중인 코드*를 수정해야 한다. 수정하면 기존 등급까지 회귀 테스트 대상이 된다. 더 나쁜 건 이런 분기가 할인, 배송비, 포인트 적립에 각각 복사되어 있다는 점이다.
...
정책이 복잡해 enum에 담기 힘들면 `DiscountPolicy` 인터페이스 + 구현체(전략 패턴)로 간다. 핵심은 **"새 케이스 = 새 파일 추가"** 이지 "기존 파일 수정"이 아니라는 것.

### LSP (Liskov Substitution Principle) — 리스코프 치환

**정의**: 자식 타입은 부모 타입이 있던 자리에 넣어도 프로그램이 여전히 옳게 동작해야 한다.
...
**왜 문제인가**: `Rectangle`의 암묵적 계약은 "너비와 높이는 독립적으로 변한다"인데 `Square`가 이걸 깼다. **상속 관계는 현실의 분류가 아니라 계약의 호환성으로 판단해야 한다.**

실무에서는 **부모에 없던 예외를 자식이 던지는** 형태로 더 자주 나타난다. `List.of()`가 반환하는 불변 리스트에 `add()`를 호출하면 `UnsupportedOperationException`이 나는 것이 그 예다. JDK가 실용성을 위해 감수한 절충이지만 내 코드에서는 피해야 한다. 두 경우 모두 해법은 같다. 상속 계층을 억지로 만들지 말고 별도 타입으로 분리하거나 불변 값 객체로 만드는 것이다.

### ISP (Interface Segregation Principle) — 인터페이스 분리

**정의**: 클라이언트는 자기가 쓰지 않는 메서드에 의존하면 안 된다.
...
**왜 문제인가**: 컴파일은 되지만 런타임에 터진다. 게다가 `fax()` 시그니처가 바뀌면 팩스를 쓰지도 않는 `SimplePrinter`까지 재컴파일·재배포 대상이 된다. 위에서 본 LSP 위반이기도 하다.
...
Java의 `Runnable`, `Comparable`, `AutoCloseable`이 모두 메서드 하나짜리인 것은 우연이 아니다.

### DIP (Dependency Inversion Principle) — 의존 역전

**정의**: 상위 모듈이 하위 모듈에 의존하면 안 된다. 둘 다 추상에 의존해야 한다.
...
**왜 문제인가**: `OrderService`는 "주문 규칙"이라는 상위 정책인데 MySQL과 SMTP라는 하위 기술을 직접 알고 있다. 단위 테스트에서 가짜 저장소로 바꿔치울 수도 없고, DB를 옮기면 정책 코드가 바뀐다.
...
"역전"은 **의존 화살표의 방향이 뒤집힌다**는 뜻이다. 그래서 `OrderRepository` 인터페이스를 구현 패키지가 아니라 **도메인 패키지에 두어야** 진짜 역전이 된다. Spring의 DI 컨테이너는 이 구조를 편하게 조립해주는 도구일 뿐, DI를 쓴다고 자동으로 DIP가 지켜지는 건 아니다.

---

## 7. 인터페이스 vs 추상 클래스

Java 8 이후 인터페이스가 `default` 메서드를 갖게 되면서 "이제 추상 클래스는 필요 없는 것 아니냐"는 질문이 나온다. 결정적 차이는 **상태(state)** 다.
...
**선택 기준**: 하위 타입들이 **공유할 상태와 생성 규칙**이 있으면 추상 클래스, **능력만 약속**하면 되면 인터페이스. 확신이 안 서면 인터페이스로 시작하라. 인터페이스를 추상 클래스로 바꾸는 것보다 그 반대가 훨씬 어렵다. `List` 인터페이스와 `AbstractList`처럼 **둘 다 제공**하고 구현자가 고르게 하는 패턴도 흔하다.

**`default` 메서드의 용도**를 오해하지 말 것. "인터페이스에도 로직을 넣자"가 아니라 **이미 배포된 인터페이스에 메서드를 추가하면서 기존 구현체를 깨뜨리지 않기 위한** 하위 호환 장치다. `Collection.stream()`이 Java 8에 추가될 수 있었던 이유다.

---

## 8. 실무에서는

- **Spring의 `@Transactional`, `@Async`** 는 프록시 객체를 만들어 원본 앞에 끼워 넣는다. 이게 가능한 이유가 다형성이다. 인터페이스 기반 JDK 동적 프록시와 상속 기반 CGLIB 프록시를 모두 지원한다.
- **JDBC / JPA**: `EntityManager`라는 추상에만 의존하고 Hibernate 같은 구현을 갈아끼운다. DIP의 실제 적용 사례다.
- **Java 17의 `sealed`**: 허용된 구현체를 명시하면 "확장에 열려 있다"를 의도적으로 닫을 수 있다. 결제 결과처럼 케이스가 확정된 도메인에서는 무제한 확장보다 컴파일러의 완전성 검사가 더 값지다. 아래 예시처럼 `default` 없이 완전성을 검사받는 패턴 매칭 `switch`는 **Java 21에서 정식 기능이 됐다**(그 이전에는 프리뷰).
...
OOP 4대 특성과 SOLID는 외울 규칙이 아니라 **"변경이 왔을 때 몇 개의 파일을 열게 되는가"** 를 줄이기 위한 진단 도구다.
