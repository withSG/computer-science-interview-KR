...
> GC가 "쓸모없는 객체"를 어떤 기준으로 판정하는지, 세대별 수집이 왜 효율적인지, 어떤 GC를 언제 고르는지, 그리고 메모리 누수를 어떻게 추적하는지 설명할 수 있게 됩니다.
...
C 시절 개발자는 `malloc`으로 얻은 메모리를 직접 `free` 해야 했고, 여기서 세 가지 버그가 끊이지 않았습니다.
...
더 큰 문제는 **누가 언제 해제할 책임을 지는지 정하는 일 자체가 설계 부담**이라는 점입니다. 객체 하나를 여러 모듈이 공유하면 마지막 사용자가 누구인지 코드로 추적해야 합니다.

Java는 이 책임을 개발자에게서 걷어가 JVM에 넘겼습니다. 그 대가는 **"내가 통제할 수 없는 타이밍에 애플리케이션이 잠깐 멈출 수 있다"** 는 것입니다. GC 학습의 본질은 이 대가를 얼마나 작게 만드느냐입니다.

---

## 2. 무엇이 쓰레기인가 — Reachability

GC는 "안 쓰는 객체"를 지운다고 하는데, JVM은 개발자의 의도를 모릅니다. JVM이 쓰는 기준은 딱 하나입니다.
...
이 방식의 중요한 성질은 **순환 참조를 자동으로 처리한다**는 것입니다. A와 B가 서로를 가리켜도 루트에서 못 닿으면 둘 다 쓰레기입니다. 참조 카운팅 방식(Python의 일부, 예전 COM)은 순환을 못 지우지만 JVM은 루트 탐색이라 문제가 없습니다.

> **비유**: 도서관 사서가 폐기할 책을 고릅니다. 열람실 입구(GC Root)에서 시작해 대출 기록을 따라가며 손이 닿는 책을 전부 표시하고, 표시 안 된 책을 버립니다.
> **비유의 한계**: 사서는 "이 책 요즘 아무도 안 읽던데"를 판단할 수 있지만 GC는 못 합니다. **논리적으로는 안 쓰는데 참조만 남아 있는 객체는 GC가 절대 못 지운다.** 이게 뒤에서 다룰 메모리 누수의 정체입니다.
...
수많은 프로그램을 측정해보니 두 가지 경향이 관찰됐습니다.
...
2. Eden이 차면 Minor GC 발동. Eden과 사용 중인 Survivor(S0)에서 **살아있는 객체만** 비어 있는 Survivor(S1)로 복사합니다.
3. 복사된 객체의 **age(살아남은 횟수)** 를 1 증가시킵니다.
4. Eden과 S0을 통째로 비웁니다. 죽은 객체는 아무 처리도 하지 않습니다. **비용이 살아있는 객체 수에만 비례**한다는 게 핵심입니다.
5. age가 임계값(`-XX:MaxTenuringThreshold`, 기본 15이며 JVM이 동적으로 낮추기도 한다)을 넘거나 Survivor가 꽉 차면 Old로 **승격(Promotion)** 됩니다.

Survivor가 두 개인 이유는 복사 방식이라 **"복사해 넣을 빈 공간"이 항상 하나 필요**하기 때문입니다. 복사 후 두 Survivor의 역할이 매번 뒤바뀝니다.

### Major GC와 Full GC

Old 영역이 차면 Old를 대상으로 수집합니다. 이것이 **Major GC**다. Old에는 오래 살아남은 객체가 모여 있어 죽는 비율이 낮고, 영역 자체도 큽니다. 그래서 **Minor GC보다 훨씬 오래 걸린다.**

**Full GC**는 여기서 한 단계 더 나아가 Young·Old·Metaspace를 **한 번에 전부** 수집하는 것을 가리킵니다. 일상 대화에서는 두 용어를 섞어 쓰는 경우가 많지만, 대상 범위가 다르다는 점은 구분해두는 편이 좋습니다. 어느 쪽이든 정지 시간이 길기 때문에 GC 튜닝에서 "Full GC 빈도를 줄인다"가 첫 번째 목표가 됩니다.

Old 영역은 복사가 아니라 보통 **Mark-Sweep-Compact** 방식을 씁니다.
...
GC가 객체 그래프를 훑는 동안 애플리케이션 스레드가 계속 참조를 바꾸면, 방금 살아있다고 표시한 객체가 다음 순간 죽거나 그 반대가 됩니다. 그래서 GC는 최소한 일부 구간에서 **모든 애플리케이션 스레드를 멈춘다.** 이것이 Stop-the-World(STW)다.
...
STW는 **없앨 수 없고 줄일 수만 있다.** 최신 GC들의 전략은 두 가지입니다.
...
STW가 문제되는 정도는 서비스 성격에 따라 다릅니다. 배치 작업이라면 1초 멈춰도 상관없고 오히려 처리량이 중요합니다. 반면 결제 API라면 200ms 정지도 타임아웃과 알람으로 이어집니다. **GC 선택은 "처리량(Throughput) vs 지연 시간(Latency)"의 트레이드오프입니다.
...
**주의**: `MaxGCPauseMillis`를 무작정 낮추면 안 됩니다. 목표가 빡빡하면 G1이 한 번에 적은 Region만 수집하고, 회수 속도가 할당 속도를 못 따라가 결국 Full GC가 터집니다. 목표는 실제 측정한 값 주변에서 조금씩 조정합니다.

### ZGC

ZGC는 참조 포인터의 사용하지 않는 비트에 GC 상태를 새기는 **컬러 포인터(Colored Pointer)** 와, 참조를 읽을 때마다 개입하는 **로드 배리어(Load Barrier)** 로 객체 재배치를 애플리케이션 실행 중에 처리합니다. 그 결과 정지 시간이 **힙 크기에 비례하지 않는다.** 수십 GB에서 TB급 힙에서도 짧은 정지를 유지하는 게 설계 목표입니다.

대가로 로드 배리어 때문에 애플리케이션 코드의 참조 읽기 비용이 조금 늘어납니다. 즉 **처리량을 지연 시간과 맞바꾼 GC**다. JDK 21에서는 세대별 ZGC가 `-XX:+ZGenerational` 옵션으로 추가되어, 어린 객체를 따로 처리해 효율이 개선됐습니다.
...
앞서 말했듯 GC는 **참조가 남아 있으면 절대 지우지 않는다.** "논리적으로 다 쓴 객체인데 참조가 안 끊긴" 상황이 Java의 메모리 누수입니다.
...
**왜 문제인가**: `static` 필드는 GC Root다. 여기 담긴 객체는 클래스가 언로드될 때까지 절대 회수되지 않습니다. 세션이 만료돼도 맵에는 그대로 남아 며칠이면 힙이 찹니다.
...
`WeakHashMap`은 키를 약한 참조로 들고 있어, 그 키를 가리키는 다른 강한 참조가 없어지면 엔트리가 자동 제거됩니다. 다만 **값이 키를 참조하면 제거되지 않으므로** 만능은 아닙니다. 실무에서는 만료 정책이 있는 캐시 라이브러리가 더 안전합니다.
...
`InputStream`, `Connection`, `HttpClient` 응답 등은 힙 객체 외에 네이티브 자원과 파일 디스크립터를 붙들고 있습니다. 참조가 끊겨도 **반환 시점은 GC가 언제 도느냐에 달린다.** JDK 일부 클래스는 `Cleaner`로 뒤늦게 정리해주기도 하지만 시점을 보장하지 않으므로, 파일 디스크립터나 커넥션이 먼저 고갈되는 편입니다.
...
`-gcutil`은 사용률을 퍼센트로 보여줍니다. 바이트 단위 실제 값이 필요하면 `-gc`를 씁니다.
Full GC 직후에도 O(Old 사용률)가 계속 우상향하면 누수를 의심합니다. GC를 해도 안 줄어든다는 뜻이기 때문입니다. 톱니 모양으로 오르내리기만 하면 정상입니다.
...
Eclipse MAT나 VisualVM으로 열고 **Dominator Tree**로 메모리를 가장 많이 붙들고 있는 객체를 찾은 뒤, **Path to GC Roots**로 "누가 이걸 놓아주지 않는지"를 봅니다. 이 두 기능이 누수 분석의 핵심입니다. 운영 서버에는 `-XX:+HeapDumpOnOutOfMemoryError`를 미리 켜두어 사고 순간의 덤프를 확보합니다.
...
GC 옵션을 만지기 전에 **애플리케이션 코드를 먼저 본다.** 대부분의 GC 문제는 GC 설정이 아니라 과도한 객체 생성이 원인입니다.
...
| `System.gc()`로 메모리를 확보하려 함 | 실행 보장이 없고 실행되면 Full GC | 호출하지 않습니다. 코드에서 객체 생성을 줄인다 |
