...
대가로 로드 배리어 때문에 애플리케이션 코드의 참조 읽기 비용이 조금 늘어납니다. 즉 **처리량을 지연 시간과 맞바꾼 GC**입니다. JDK 21에서는 세대별 ZGC가 `-XX:+ZGenerational` 옵션으로 추가되어, 어린 객체를 따로 처리하면서 효율이 개선됐습니다.
...
| `System.gc()`로 메모리를 확보하려 함 | 실행 보장이 없고 실행되면 Full GC | 호출하지 않습니다. 코드에서 객체 생성을 줄입니다 |
