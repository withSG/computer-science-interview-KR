...
실제 CPU를 쓰는 시간은 1ms도 안 되는데 **150ms 동안 OS 스레드를 붙들고 있습니다.** 스레드 풀이 200개라면 초당 처리 가능한 요청은 대략 `200 / 0.15초 ≈ 1,300건`에서 막힙니다. 트래픽이 늘어 풀이 고갈되면 요청은 큐에서 대기하다 타임아웃 납니다.
...
**가상 스레드의 목표는 이 대가 없이 처리량만 가져오는 것입니다.** 코드는 예전 그대로 블로킹 스타일로 쓰고, 대기 처리는 JVM이 알아서 합니다.
...
**왜 문제인가**: DB 커넥션 풀이 10개면 동시에 진행 가능한 쿼리는 10개입니다. 나머지 9,990개는 커넥션을 기다리며 대기하고, 대기 시간이 풀 타임아웃을 넘기면 **`SQLTransientConnectionException`이 대량 발생**합니다. 스레드를 없애도 **아래쪽 자원 한계는 그대로**입니다. 오히려 부하를 DB로 그대로 밀어 넣어 상황이 나빠질 수 있습니다.
