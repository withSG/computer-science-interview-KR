...
### 핵심 답변
힙 메모리에서 더 이상 참조되지 않는 객체를 JVM이 자동으로 제거하는 것이 GC입니다.
...
### 핵심 답변
Java는 **항상 Call By Value**입니다. 참조 타입일 때는 참조 값(주소)의 복사본이 넘어갑니다.
...
### 핵심 답변
Virtual Threads는 JVM 내부에서 관리되는 경량 스레드로, 다수의 가상 스레드가 소수의 캐리어 스레드(OS 스레드)를 나눠 쓰는 **M:N 매핑**입니다. 기존 동기식 코드 스타일을 유지하면서 비동기 I/O의 성능을 얻을 수 있습니다.
...
| 디버깅 | 어려움 | 쉬움 |
| 적합 환경 | 초장기 연결·대량 스트리밍 | 일반적인 I/O 대기 (DB·외부 API) |
...
## Q12. Thread-safe(스레드 세이프)란 무엇인가요? ⭐⭐
...
### 핵심 답변
JVM은 Java 프로그램을 실행하는 가상 머신입니다. 내부는 **클래스 로더, 런타임 데이터 영역, 실행 엔진**으로 나뉩니다.
...
### 핵심 답변
제네릭은 **클래스나 메서드에서 사용할 타입을 파라미터로 지정**하는 기능으로, 컴파일 타임에 타입 안전성을 보장합니다.
...
### 꼬리 질문 대비
- "타입 소거란 무엇인가요?"
  → 컴파일 후 제네릭 타입 정보가 사라지고 Object로 변환
...
### 핵심 답변
Java 8은 **람다 표현식, Stream API, Optional, 새로운 날짜/시간 API** 등 함수형 프로그래밍을 지원하는 기능을 새로 추가했습니다.
...
### 핵심 답변
직렬화는 **객체를 바이트 스트림으로 변환**하는 과정으로, 네트워크 전송이나 파일 저장, 세션 저장 등에 씁니다.
