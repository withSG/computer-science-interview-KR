...
> JVM이 코드를 실행하면서 데이터를 어디에 놓는지 그림으로 그릴 수 있게 되고, 원시 타입·참조 타입·String Pool·오토박싱에서 생기는 버그의 원인을 메모리 관점으로 설명할 수 있게 됩니다.
...
이 문서는 이 네 증상의 원인을 전부 한 장의 그림 위에서 설명합니다.
...
Java 7까지는 이 영역이 `PermGen`이라는 이름의 힙 일부로 구현되어 고정 크기였고 애플리케이션이 클래스를 동적으로 많이 만들면 `OutOfMemoryError: PermGen space`가 흔히 났습니다. **Java 8부터 Metaspace로 바뀌면서 힙 밖의 네이티브 메모리를 쓰고 기본적으로 자동 확장**됩니다. 대신 무한정 늘어나 OS 메모리를 다 먹는 사고를 막으려면 `-XX:MaxMetaspaceSize`를 지정합니다.
...
메서드를 호출할 때마다 **스택 프레임**이 하나 쌓이고 메서드가 끝나면 사라집니다. 프레임 안에는 지역 변수 배열, 피연산자 스택, 반환 주소가 들어 있습니다.
...
> **비유의 한계**: 창고 선반은 물건 위치가 고정이지만 힙 객체는 GC가 압축하면서 실제 주소가 바뀔 수 있습니다. 그래서 Java에는 C처럼 주소를 직접 가리키는 포인터 연산이 없습니다.
...
웹 애플리케이션 하나에서 `"application/json"`, `"UTF-8"` 같은 문자열은 수만 번 등장하는데 매번 별도 객체를 만들면 똑같은 내용의 객체가 힙을 가득 채웁니다. JVM은 이걸 막으려고 **String Pool**을 둡니다. 문자열 리터럴은 풀에 한 번만 만들고 이후에는 같은 객체를 재사용합니다.
...
String Pool은 Java 7부터 PermGen에서 **힙으로 이동**했습니다. 덕분에 풀에 들어간 문자열도 참조가 끊기면 GC 대상이 되고 풀 크기 제약이 사실상 사라졌습니다.
...
**왜 문제인가**: String은 불변이므로 `+=`는 기존 문자열을 수정하는 게 아니라 **새 문자열을 만들어 다시 대입**합니다. 10,000번 반복하면 버려지는 중간 String이 10,000개 생기고 길이가 늘어날수록 복사 비용도 커집니다.
...
참고로 **반복문 밖의 단순 연결(`"a" + b + "c"`)은 컴파일러가 알아서 최적화**해주므로 굳이 `StringBuilder`로 바꿀 필요가 없습니다. 문제가 되는 건 루프 안에서 누적할 때입니다.

멀티스레드에서 하나의 버퍼를 공유해야 하면 `StringBuffer`(모든 메서드 `synchronized`)를 쓰지만 실무에서 그런 상황은 드뭅니다. 대개 지역 변수로 쓰므로 `StringBuilder`가 정답입니다.
...
`Integer.valueOf()`는 **-128 ~ 127 범위의 값을 미리 만들어 캐싱**합니다. 그래서 127까지는 같은 객체가 재사용되어 `==`가 `true`, 128부터는 매번 새 객체라 `false`가 됩니다.
...
**왜 문제인가**: `sum`이 래퍼 타입이라 매 반복마다 `Long.valueOf()`가 호출되는데 캐시 범위를 벗어나므로 전부 새 객체입니다. 힙에 쓰레기가 쌓이고 GC가 계속 돌아 실행 시간이 크게 늘어납니다.
...
Java 메모리 모델은 **"값이냐 주소냐"** 와 **"공유냐 스레드 전용이냐"** 두 축으로 정리되며 실무 버그의 상당수는 이 두 축을 착각한 데서 나옵니다.
