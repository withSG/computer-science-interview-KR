...
> JVM에서 데이터가 놓이는 자리는 저마다 다릅니다. 그 자리를 그림으로 그려 보고, 원시 타입·참조 타입·String Pool·오토박싱에서 생기는 버그의 원인을 메모리 관점으로 설명합니다.
...
- [01-oop-solid.md](./01-oop-solid.md) - 객체와 참조라는 말에 익숙하면 읽기 편합니다

---

## 1. 왜 알아야 하는가

Java에는 `malloc`/`free`가 없어서 "메모리는 JVM이 알아서 해주는 것"으로 넘어가기 쉽습니다. 하지만 실무에서 만나는 아래 증상은 전부 메모리 구조를 모르면 원인을 추적할 수 없습니다.
...
이 문서는 이 네 증상의 원인을 전부 한 장의 그림 위에서 설명합니다. 그림부터 그려 보겠습니다.

---

## 2. Runtime Data Area 전체 구조

JVM이 프로그램을 실행할 때 쓰는 메모리는 **스레드끼리 공유하는가**를 기준으로 다섯 영역으로 나뉩니다.
...
필드 이름과 타입, 메서드 시그니처와 바이트코드, 런타임 상수 풀. 클래스가 로딩될 때 이런 **클래스 자체에 대한 정보**가 여기 올라갑니다.

Java 7까지는 이 영역이 `PermGen`이라는 이름의 힙 일부로 구현되어 고정 크기였습니다. 애플리케이션이 클래스를 동적으로 많이 만들면 `OutOfMemoryError: PermGen space`가 흔히 났습니다. **Java 8부터 Metaspace로 바뀌면서 힙 밖의 네이티브 메모리를 쓰고 기본적으로 자동 확장**됩니다. 대신 무한정 늘어나 OS 메모리를 다 먹는 사고를 막으려면 `-XX:MaxMetaspaceSize`를 지정합니다.

### Heap (힙)

`new`로 만들어진 **모든 객체와 배열**이 사는 곳입니다. GC가 다루는 핵심 영역이라 실무 튜닝의 대부분도 여기서 벌어집니다(Metaspace 역시 클래스가 언로드될 때 회수되지만 양이 훨씬 적습니다). 내부는 Young/Old 세대로 나뉘는데 자세한 내용은 [03-garbage-collection.md](./03-garbage-collection.md)에서 다룹니다.

### JVM Stack (스택)

메서드를 호출할 때마다 **스택 프레임**이 하나 쌓입니다. 메서드가 끝나면 사라집니다. 프레임 안에는 지역 변수 배열, 피연산자 스택, 반환 주소가 들어 있습니다.
...
스택 크기는 스레드 하나당 `-Xss`로 조절합니다. 재귀가 너무 깊어 프레임을 더 못 쌓으면 `StackOverflowError`가 납니다.

> **비유**: 스택은 식당 주방의 접시 더미입니다. 맨 위에 올리고 맨 위에서 뺍니다. 힙은 창고 선반이고, 스택의 메모지에는 "창고 3번 칸"이라는 위치만 적혀 있습니다.
>
> **비유의 한계**: 창고 선반은 물건 위치가 고정입니다. 하지만 힙 객체는 GC가 압축하면서 실제 주소가 바뀔 수 있습니다. 그래서 Java에는 C처럼 주소를 직접 가리키는 포인터 연산이 없습니다.

### PC Register

스레드마다 **지금 실행 중인 JVM 명령어의 위치**를 담습니다. 스레드가 CPU를 빼앗겼다가 돌아왔을 때 어디부터 이어서 할지 알기 위해 필요합니다. 네이티브 메서드를 실행 중일 때는 이 값이 정의되지 않습니다(undefined).

### Native Method Stack

JNI로 C/C++ 코드를 호출할 때는 별도의 스택을 씁니다. `Thread.start()`처럼 JDK가 내부적으로 네이티브를 호출하는 지점이 여기에 해당합니다.
...
**언제 무엇을 쓰나**: 값 자체가 의미의 전부이고 `null`이 필요 없으면 원시 타입이 항상 낫습니다. 컬렉션에 담아야 하거나 "값이 없음"을 표현해야 할 때만 래퍼 타입을 씁니다.

### 흔한 오해 1 — "원시 타입은 항상 스택에 저장된다"

**지역 변수일 때만** 그렇습니다. 객체의 필드로 선언된 원시 타입은 그 객체와 함께 **힙 안에** 삽니다.
...
면접에서 "static은 Method Area"라고 답해도 대개 넘어갑니다. 위 구분까지 말하면 확실히 깊이가 드러납니다.
...
웹 애플리케이션 하나에서 `"application/json"`, `"UTF-8"` 같은 문자열은 수만 번 등장합니다. 매번 별도 객체를 만들면 똑같은 내용의 객체가 힙을 가득 채웁니다. JVM은 이걸 막으려고 **String Pool**을 둡니다. 문자열 리터럴은 풀에 한 번만 만들고 이후에는 같은 객체를 재사용합니다.

이게 가능한 유일한 이유는 **String이 불변**이기 때문입니다. 만약 String을 바꿀 수 있다면, 한 곳에서 값을 바꾸는 순간 그 리터럴을 쓰는 전 세계 코드가 같이 바뀝니다. 공유가 성립하지 않습니다.
...
String Pool은 Java 7부터 PermGen에서 **힙으로 이동**했습니다. 덕분에 풀에 들어간 문자열도 참조가 끊기면 GC 대상이 됩니다. 풀 크기 제약도 사실상 사라졌습니다.
...
**왜 문제인가**: String은 불변이므로 `+=`는 기존 문자열을 수정하는 게 아니라 **새 문자열을 만들어 다시 대입**합니다. 10,000번 반복하면 버려지는 중간 String이 10,000개 생깁니다. 길이가 늘어날수록 복사 비용도 커집니다.
...
참고로 **반복문 밖의 단순 연결(`"a" + b + "c"`)은 컴파일러가 알아서 최적화**해 주므로 굳이 `StringBuilder`로 바꿀 필요가 없습니다. 문제가 되는 건 루프 안에서 누적할 때입니다.

멀티스레드에서 하나의 버퍼를 공유해야 하면 `StringBuffer`(모든 메서드 `synchronized`)를 씁니다. 다만 실무에서 그런 상황은 드뭅니다. 대개 지역 변수로 쓰므로 `StringBuilder`가 정답입니다.
...
원시 타입과 래퍼 클래스 사이의 변환을 컴파일러가 자동으로 넣어주는 기능입니다.
...
`Integer.valueOf()`는 **-128 ~ 127 범위의 값을 미리 만들어 캐싱**합니다. 그래서 127까지는 같은 객체가 재사용되어 `==`가 `true`입니다. 128부터는 매번 새 객체라 `false`가 됩니다.

**왜 문제인가**: 개발 중에는 작은 숫자로 테스트해 `==`가 통과하고, 운영에서 ID가 커지면 조용히 틀린 판단을 합니다. 실제 장애로 이어지는 전형적인 패턴입니다.
...
`Byte`, `Short`, `Long`도 -128~127을 캐싱하고 `Character`는 0~127, `Boolean`은 두 값 모두 캐싱합니다. `Float`와 `Double`은 캐시가 없습니다.
...
`getOrDefault` 외에 `Optional`로 감싸 받거나, 카운터 용도라면 `Map<String, Long>` 대신 `Map<String, AtomicLong>`처럼 값 자체가 `null`이 되지 않는 구조로 바꾸는 방법도 있습니다.
...
**왜 문제인가**: `sum`이 래퍼 타입이라 매 반복마다 `Long.valueOf()`가 호출됩니다. 캐시 범위를 벗어나므로 전부 새 객체입니다. 힙에 쓰레기가 쌓이고 GC가 계속 돌아 실행 시간이 크게 늘어납니다.
...
Stream API에서도 같은 문제가 생깁니다. `Stream<Integer>` 대신 `IntStream`/`LongStream`을 쓰면 박싱이 사라집니다.
...
`Error`는 `Exception`과 달리 **잡아서 복구하려 들면 안 됩니다**. `OutOfMemoryError`를 `catch`하고 계속 돌리면 그 뒤의 모든 동작을 신뢰할 수 없습니다. 프로세스를 재시작시키는 편이 낫습니다.
...
컨테이너 환경에서 `-Xmx`를 호스트 기준으로 잡으면, JVM은 아직 여유가 있다고 판단하는데 cgroup 한도를 넘어 **컨테이너가 OOM Killer에게 강제 종료**됩니다. 이때는 힙 덤프도 남지 않아 원인 추적이 어렵습니다. `-XX:MaxRAMPercentage`로 컨테이너 한도 기준 비율을 지정하는 편이 안전합니다.
...
A. 스레드가 공유하는 영역과 스레드별 독립 영역으로 나뉩니다. 공유 영역은 Method Area와 Heap인데, Method Area에는 클래스 메타데이터가 올라가고 Heap에는 객체가 저장됩니다. 독립 영역은 셋입니다. JVM Stack에는 메서드 호출 프레임이 쌓이고, PC Register는 실행 위치를 담으며, Native Method Stack은 JNI용입니다. GC가 관리하는 곳은 Heap이며 튜닝 대상도 대부분 여기입니다.
...
A. `Integer.valueOf()`가 -128부터 127까지의 값을 캐싱해 두고 같은 객체를 재사용하기 때문입니다. 128 이상이면 새 객체가 만들어져 `false`가 됩니다. 그래서 래퍼 타입은 반드시 `equals()`로 비교해야 하고, 가능하면 원시 타입을 씁니다.
...
A. 우선 String Pool 때문입니다. 리터럴을 공유하려면 값이 안 바뀌어야 합니다. 여기에 `hashCode()`를 한 번 계산해 캐싱할 수 있어 HashMap 키로 쓸 때 빠르다는 이점이 붙습니다. 여러 스레드가 동기화 없이 공유해도 안전하고, 파일 경로나 DB URL 같은 값이 검증 이후 변조될 위험도 없습니다.
...
Java 메모리 모델은 **"값이냐 주소냐"** 와 **"공유냐 스레드 전용이냐"** 두 축으로 정리됩니다. 실무 버그의 상당수는 이 두 축을 착각한 데서 나옵니다.
