...
순번(`1`, `2`, `3`)이나 사용자 ID를 그대로 쓰면 숫자만 바꿔 가며 남의 세션에 올라탈 수 있습니다. OWASP는 세션 ID의 실질 엔트로피를 최소 64비트로 요구하고, 직접 생성한다면 128비트 이상의 크기로 만들도록 권장합니다. 직접 만든다면 반드시 암호학적 난수 생성기(`SecureRandom` 등)를 써야 합니다. `java.util.Random`은 안 됩니다. 시드를 알면 다음 값이 예측되기 때문입니다.
...
| Redis 세션 도입으로 안심 | Redis 단일 노드는 그 자체가 SPOF | 복제로 Redis도 이중화 |
