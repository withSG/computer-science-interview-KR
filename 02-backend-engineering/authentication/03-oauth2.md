...
> OAuth 2.0은 "비밀번호를 넘기지 않고 권한만 빌려주는" 프로토콜이다.
> 이 문서를 읽고 나면 소셜 로그인 버튼을 눌렀을 때 브라우저와 두 서버 사이에서 무슨 일이 벌어지는지,
> 왜 코드와 토큰을 두 단계로 나눴는지, 그리고 왜 OAuth만으로 로그인을 만들면 안 되는지 설명할 수 있다.
...
OAuth 2.0은 이 다섯을 한 번에 푼다. **비밀번호는 원래 주인(구글)에게만 입력하고, 제3자 서비스에는 "주소록 읽기 권한"이라는 제한된 표(Access Token)만 준다.**

> **비유**: 호텔 발렛파킹 키와 같다. 마스터 키를 넘기면 트렁크도 글로브박스도 열리지만, 발렛 키는 시동을 걸고 주차만 할
> 수 있다. 게다가 언제든 발렛 키만 무효화할 수 있다.
>
> **비유의 한계**: 발렛 키는 물리적으로 기능이 제한되지만, Access Token의 권한 제한은 **리소스 서버가 스코프를 성실히
> 검사해야만** 성립한다. 토큰 자체에는 문을 못 여는 물리적 장치가 없다.
...
백엔드일 수도 SPA일 수도 있다. 이 구분이 뒤에서 PKCE 필요성으로 이어진다. 그리고 **Authorization Server와
Resource Server는 같은 회사일 수 있다.** 구글은 둘 다 운영하지만 프로토콜상으로는 분리된 역할이다.

Client는 사전에 Authorization Server에 **등록**돼 있어야 한다. 등록 과정에서 허용할 `redirect_uri` 목록을
직접 신고하고, 그 대가로 `client_id`(공개 식별자)와 경우에 따라 `client_secret`(비밀 키)을 발급받는다.
...
(1)~(5)는 브라우저를 거치고 **(6)(7)만 서버 대 서버 통신(Back-channel)**이다. 이 구분이 핵심이다.
...
(실제 요청은 한 줄이다. 위는 읽기 편하도록 파라미터마다 줄을 나눴다.)
...
사용자가 동의하면 브라우저가 `redirect_uri`로 돌아온다. 그다음부터는 **우리 백엔드가 직접** 요청한다.
...
교환**되므로 코드만 훔쳐서는 쓸모가 없다. 즉 **위험한 경로로는 쓸모없는 값만 보내고, 값진 것은 안전한 경로로만 오간다.**
...
**왜 문제인가** — 프론트엔드 번들은 사용자에게 전송되는 파일이다. 개발자 도구 Sources 탭만 열면 보인다. 난독화해도
소용없다. 실행 시점에는 반드시 평문이어야 하기 때문이다. secret이 노출되면 공격자가 **우리 앱인 척** 토큰을 발급받을 수
있다. 모바일 앱도 APK/IPA를 디컴파일하면 나온다.

**개선** — 백엔드가 있다면 코드만 백엔드로 넘기고 교환은 백엔드에서 한다(secret은 서버에만 둔다). 백엔드가 없다면 앱을
"Public Client"로 등록하고 `client_secret` 대신 PKCE로 소유권을 증명한다.
...
Implicit이 폐기된 이유는 명확하다. Access Token이 **URL 프래그먼트에 실려 브라우저를 통과**하기 때문이다. 3장의 "브라우저
경유는 위험하다"는 원칙을 정면으로 위반한다. 애초에 Implicit이 만들어진 이유는 SPA가 다른 도메인의 토큰
엔드포인트를 직접 호출할 수 없던 시절의 제약 때문이었는데, 인가 서버들이 토큰 엔드포인트에 CORS를
허용하고 PKCE가 자리 잡으면서 SPA도 Authorization Code를 쓸 수 있게 되자 존재 이유가 사라졌다. ROPC(Password Grant)는 **OAuth가 없애려던 문제 그 자체**다. 앱이 비밀번호를 직접 받는 순간 1장의 다섯 문제가
전부 돌아오고 2단계 인증과도 공존할 수 없다. OAuth 2.1 초안은 Implicit과 ROPC를 제거하고, 모든 Authorization Code
흐름에 PKCE를 요구하며, `redirect_uri`를 문자열 완전 일치로 비교하도록 못 박는 방향으로 진행 중이다.

---

## 5. PKCE — Public Client의 구멍 메우기

`client_secret`을 안전하게 보관할 수 있는 Client를 **Confidential Client**, 그럴 수 없는 쪽을 **Public Client**라 한다. SPA와 모바일 앱이 Public Client다.

Public Client는 secret이 없으니 코드를 토큰으로 바꿀 때 "내가 진짜 그 앱이다"를 증명할 수단이 없다. 공격자가 리다이렉트
과정에서 코드를 가로채면 그대로 토큰을 받아간다. 특히 모바일에서는 커스텀 URL 스킴(`myapp://callback`)을 **악성 앱이
똑같이 등록**해 코드를 가로챌 수 있다. 해결 아이디어는 이렇다. secret을 미리 심어둘 수 없다면 **요청할 때마다 그
자리에서 만들면 된다.**
...
`code_verifier`는 43~128자의 랜덤 문자열이고, `code_challenge`는 그 SHA-256 해시를 Base64URL로 인코딩한 값이다.
RFC 7636 문서의 예제로 직접 확인할 수 있다.
...
계산한다. `code_challenge_method`는 반드시 `S256`을 쓴다. 스펙에 `plain`(해시 없이 그대로 전송)도 있지만, 그러면 인가
요청을 엿본 공격자가 verifier를 그대로 알게 되어 방어가 무의미해진다.

**중요한 오해 정리**: PKCE는 `client_secret`을 대체하는 게 아니라 **코드 가로채기(Authorization Code Interception)를
막는 장치**다. 그래서 Confidential Client도 함께 쓰는 것이 권장된다. "백엔드가 있으니 PKCE는 필요 없다"는 말은 절반만 맞다.
...
없다. 이렇게 권한을 가진 쪽이 요청자가 누구인지 확인하지 않고 대신 행동해 버리는 구조를 **혼동된 대리인(Confused
Deputy)** 문제라 부른다.
...
`aud` 검증이 토큰 대체 공격을 정확히 막는 지점이다. 공격자가 다른 앱용 ID Token을 가져와도 `aud`가 우리 `client_id`가
아니므로 거부된다. OIDC는 상호운용성도 개선했다. `https://<발급자>/.well-known/openid-configuration`을 조회하면
인가·토큰·UserInfo 엔드포인트와 JWKS 주소가 JSON으로 나오고(**Discovery**), 발급자 공개키가 표준 형식(**JWKS**)으로
제공돼 키 교체도 `kid`로 자동 대응된다.
...
**왜 문제인가** — 서명을 검증하지 않았다. 공격자가 `{"email":"admin@ourservice.com"}`을 직접 Base64로 인코딩해 보내면
그대로 관리자 계정으로 로그인된다. `iss`와 `aud`도 확인하지 않아 다른 발급자·다른 앱의 토큰까지 받는다. `email`을 식별자로
쓴 것도 문제다. 이메일은 바뀌고 소유가 검증되지 않을 수 있으므로 사용자 식별은 **`iss` + `sub` 조합**을 써야 한다.
...
안전하다. 직접 짜면 `aud` 검증이나 알고리즘 고정을 빠뜨리기 쉽다.
...
동시에 심을 수 있으므로 반드시 **서버 측 세션**에 두어야 하며, 검증 후 **즉시 폐기**해야 한다.
...
`https://myapp.com/*` 처럼 와일드카드로 등록하거나 앞부분만 비교하는 경우다.

**왜 문제인가** — 앱 어딘가에 오픈 리다이렉트 취약점이 있다면(`https://myapp.com/go?to=https://evil.com` 같은
페이지), 공격자는 `redirect_uri=https://myapp.com/go?to=https://evil.com`으로 인가 요청을 만들 수 있다. 등록
규칙이 앞부분만 비교하면 통과하고 코드는 공격자 서버로 넘어간다.

**개선** — 등록할 때 **경로까지 포함한 완전한 URI**를 넣고 비교는 문자열 완전 일치로 한다. 여러 환경이 필요하면 와일드카드 대신 URI를 여러 개 등록하고, 앱 내부에 오픈 리다이렉트가 없는지도 별도로 점검한다.
...
캘린더 동기화처럼 사용자 대신 주기적으로 API를 호출해야 한다면 Refresh Token을 저장해야 한다. 저장 시 암호화하고, 사용자가
연동을 해제하면 공급자의 토큰 폐기(revocation) 엔드포인트를 호출해 실제로 끊어준다. DB에서 행만 지우면 토큰은 여전히 살아
있다. 스코프는 처음부터 전부 요구하지 말고 필요한 시점에 추가로 요청하는 점진적 동의가 동의율에 유리하다.
...
- 꼬리 질문: "OAuth로 로그인을 구현하면 되나요?" → 안 된다. 토큰에 "누구인지"에 대한 검증 가능한 정보가 없다. 로그인은 OIDC로.
...
- 꼬리 질문: "코드가 재사용되면?" → 일회용으로 처리하고, 재사용이 감지되면 그 코드로 발급된 토큰까지 폐기한다.
...
- 꼬리 질문: "백엔드가 있으면 PKCE는 안 써도 되나요?" → 권장되지 않는다. secret의 대체재가 아니라 코드 가로채기 방어책이다.
...
- 꼬리 질문: "`nonce`와는?" → `state`는 콜백 쿼리로 돌아와 CSRF를, `nonce`는 ID Token 안에 담겨 돌아와 재생 공격을 막는다.
...
OAuth 2.0은 비밀번호 대신 **범위와 기간이 제한된 토큰을 위임**하는 인가 프로토콜이고, 위험한 브라우저 경로에는 쓸모없는 코드만 흘리는 설계이며, "누구인가"까지 안전하게 알고 싶다면 그 위에 OIDC를 얹어야 한다.
