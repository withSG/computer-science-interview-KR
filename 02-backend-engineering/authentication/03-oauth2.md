...
저장하고**(그 서비스가 털리면 구글 계정도 털린다), **2단계 인증과 공존할 수 없습니다.**
...
헷갈리기 쉬운 두 가지. **"Client"는 프론트엔드가 아닙니다.** OAuth에서 Client는 *등록된 애플리케이션*을 뜻하며
백엔드일 수도 SPA일 수도 있습니다. 이 구분이 뒤에서 PKCE 필요성으로 이어집니다. 그리고 **Authorization Server와
Resource Server는 같은 회사일 수 있습니다.** 구글은 둘 다 운영하지만 프로토콜상으로는 분리된 역할입니다.
...
교환**되므로 코드만 훔쳐서는 쓸모가 없습니다. 즉 **위험한 경로로는 쓸모없는 값만 보내고, 값진 것은 안전한 경로로만 오갑니다.**
...
허용하고 PKCE가 자리 잡으면서 SPA도 Authorization Code를 쓸 수 있게 되자 존재 이유가 사라졌습니다. ROPC(Password Grant)는 **OAuth가 없애려던 문제 그 자체**입니다. 앱이 비밀번호를 직접 받는 순간 1장의 다섯 문제가
...
막는 장치**입니다. 그래서 Confidential Client도 함께 쓰는 것이 권장됩니다. "백엔드가 있으니 PKCE는 필요 없다"는 말은 절반만 맞습니다.
...
**5번이 중요합니다.** 구글의 Access Token을 우리 서비스의 인증 수단으로 계속 쓰지 않습니다. 구글 토큰은 "구글 API를 호출할
