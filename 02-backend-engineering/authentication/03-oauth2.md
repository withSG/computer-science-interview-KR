...
공급자에 따라 토큰 내부 정보를 되묻는 Introspection(RFC 7662) 엔드포인트를 제공하기도 하지만 선택 확장이라 어디서나
...
Referer, 액세스 로그에 값이 남습니다. 그래서 이 경로로는 Authorization Code만 보냅니다. 일회용이고 수명이 짧으며 `client_secret` 없이는 쓸모없는
값이라 실제 Access Token은 백엔드가 secret과 함께 직접 요청해서 받습니다.
