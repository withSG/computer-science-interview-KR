...
> 어노테이션 한 줄로 트랜잭션이 걸리는 원리와, 그 원리 때문에 "분명 붙였는데 롤백이 안 되는" 상황들이 왜 생기는지 설명하고 피할 수 있게 됩니다.
...
계좌 이체처럼 두 개의 쓰기가 전부 성공하거나 전부 실패해야 하는 로직을 트랜잭션 없이 짤 수는 없습니다. JDBC로 직접 쓰면 이렇습니다.
...
진짜 로직은 가운데 두 줄인데 나머지가 전부 배관 코드입니다. 게다가 이 코드에는 세 가지 위험이 숨어 있습니다.
...
같은 코드가 서비스 메서드마다 반복되고 한 곳만 실수해도 데이터가 깨집니다. [AOP 문서](./02-aop-proxy.md)에서 본 횡단 관심사의 전형입니다.
...
배관이 전부 사라졌습니다. 편해진 만큼 대가가 있습니다. **트랜잭션 경계가 코드에 보이지 않기 때문에, 경계가 어긋나도 눈에 띄지 않는다.** 이 문서의 나머지는 전부 그 이야기입니다.
...
프록시가 하는 일을 의사코드로 옮기면 이렇습니다. 조건절에 `RuntimeException`과 `Error`만 적혀 있다는 점을 기억하자.
...
실제 구현(`TransactionAspectSupport`)은 모든 `Throwable`을 잡은 뒤 롤백 규칙에 물어보는 형태이고, 그 기본 규칙이 "`RuntimeException`과 `Error`만 롤백"입니다. `rollbackFor`를 지정하면 이 규칙이 바뀝니다. 뒤에 나올 함정의 절반이 이 조건 한 줄에서 나옵니다.

### 커넥션은 ThreadLocal에 있다

`@Transactional` 메서드 안에서 여러 Repository를 호출해도 전부 **같은 커넥션**을 씁니다. `TransactionSynchronizationManager`가 커넥션을 현재 스레드의 `ThreadLocal`에 담아두기 때문입니다. 그래서 Repository 메서드에 `Connection`을 넘길 필요가 없습니다.

이 사실에서 중요한 따름정리가 나옵니다. **스레드가 바뀌면 트랜잭션도 끊긴다.** `@Async` 메서드나 별도 스레드에서 실행되는 코드는 바깥 트랜잭션에 참여하지 못합니다.
...
**왜 문제인가**: 트랜잭션은 프록시에 있는데, `placeOrder`가 실행되는 시점에는 이미 원본 객체 안입니다. `this.saveOrder()`는 프록시를 다시 거치지 않으므로 트랜잭션이 시작되지 않습니다. 예외도 경고도 없습니다.
...
**왜 문제인가**: CGLIB 프록시는 원본 클래스를 상속해 메서드를 오버라이드하는데, `private` 메서드는 오버라이드할 수 없습니다. 게다가 `private` 메서드는 정의상 항상 내부 호출이라 프록시를 거칠 방법이 없습니다. `final`이나 `static` 메서드도 같은 이유로 적용되지 않습니다. 프록시 기반 트랜잭션은 **public 메서드에만 붙인다**고 생각하는 것이 안전합니다.
...
**왜 문제인가**: 기본 롤백 규칙이 보는 것은 `RuntimeException`과 `Error`뿐입니다. `IOException` 같은 Checked Exception은 롤백 대상이 아니라서, 예외가 밖으로 나가는데도 트랜잭션은 커밋됩니다. 영수증 없는 주문이 DB에 남습니다.

이 규칙은 EJB 시절의 관례를 계승한 것으로, "Checked Exception은 호출자가 복구할 수 있는 상황"이라는 전제에 서 있습니다. 지금의 코드 스타일과는 잘 맞지 않아 사고를 부르는 편입니다.
...
**왜 문제인가**: 프록시는 메서드 밖으로 나온 예외만 봅니다. 안에서 잡아버리면 프록시 입장에서는 정상 종료이므로 커밋합니다. 포인트는 안 빠지고 주문만 생성된 데이터가 남습니다.

**같은 코드가 더 나쁘게 동작하는 변형**도 있습니다. `pointService.usePoint()`가 `@Transactional`이고 그 안에서 예외가 났다면, 안쪽 트랜잭션이 전체 트랜잭션에 `rollback-only` 표시를 남깁니다. 바깥에서 예외를 삼키고 커밋을 시도하면 이런 예외를 만납니다.
...
"나는 예외를 다 잡았는데 왜 죽지?"의 정체가 이것입니다. 기본 전파인 `REQUIRED`에서는 안쪽과 바깥쪽이 **같은 물리 트랜잭션**을 공유하므로, 안에서 한 번 롤백 표시가 붙으면 되돌릴 수 없습니다.
...
또는 애초에 부가 작업을 트랜잭션 밖으로 빼거나 커밋 후 이벤트로 처리합니다.

### 함정 5. readOnly = true의 오해

`@Transactional(readOnly = true)`가 하는 일은 **최적화 힌트입니다. JPA를 쓰면 영속성 컨텍스트의 플러시 모드가 바뀌어 변경 감지를 위한 스냅샷 비교와 플러시를 건너뜁니다. 조회가 많은 메서드에서 메모리와 CPU를 아낄 수 있습니다.

오해는 여기서 생깁니다. **이것은 쓰기를 차단하는 보안 장치가 아니다.** 예를 들어 식별자 생성 전략에 따라 `persist` 시점에 곧바로 INSERT가 나가는 경우처럼, 플러시와 무관하게 SQL이 실행되는 경로가 있습니다. "`readOnly`를 붙였으니 실수로 저장될 일은 없다"고 믿으면 안 됩니다.

더 자주 사고를 내는 쪽은 **클래스 레벨 `readOnly`가 쓰기 메서드에까지 상속되는 것**입니다.
...
"클래스에 `readOnly = true`, 쓰기 메서드에만 `@Transactional`"은 널리 쓰이는 관례지만, 붙이는 것을 잊으면 조용히 반영이 안 된다는 위험이 늘 따라다닙니다.
...
1. **커넥션 점유 시간이 API 응답 시간만큼 늘어난다.** 풀 크기가 10인데 동시 주문이 10건이면 11번째 요청부터 커넥션을 못 받고 대기합니다. 결제 API가 느려지는 순간 서비스 전체가 멈춥니다. 재시도 로직까지 트랜잭션 안에서 돌면 점유 시간은 배로 늘어납니다.
2. **DB 락도 그만큼 유지된다.** 재고 행을 잠근 채 10초를 기다리면 같은 상품을 사려는 다른 요청이 전부 밀립니다. 데드락 확률도 올라갑니다.
3. **롤백해도 외부 호출은 되돌아가지 않는다.** DB는 롤백됐는데 결제는 이미 승인된 상태가 만들어집니다.
...
원칙은 하나입니다. **트랜잭션 안에는 DB 작업만 둔다.** 메일 발송, 파일 업로드, 외부 결제, 슬랙 알림은 모두 밖으로 뺍니다.

---

## 4. 전파 속성 (Propagation)

전파 속성은 **"이미 진행 중인 트랜잭션이 있을 때 어떻게 할 것인가"** 를 정합니다.
...
> 결론: 실무에서 쓰는 것은 사실상 `REQUIRED`(기본)와 `REQUIRES_NEW` 둘입니다. `MANDATORY`는 "이 메서드는 반드시 상위 트랜잭션 안에서만 불려야 한다"는 계약을 코드로 강제하고 싶을 때 가끔 씁니다.
...
`REQUIRES_NEW`의 대표 용도는 **본 작업의 성패와 무관하게 남아야 하는 기록**입니다. 주문이 실패해도 시도 이력은 남겨야 하는 감사 로그가 전형적입니다. 다만 안쪽 트랜잭션이 도는 동안 **커넥션을 두 개 점유한다.** 풀 크기가 10인데 `REQUIRES_NEW`를 쓰는 요청이 10개 동시에 들어오면, 바깥 트랜잭션 10개가 커넥션을 다 쓴 상태에서 안쪽이 커넥션을 기다리는 교착이 생길 수 있습니다.

### NESTED

`NESTED`는 부모 트랜잭션 안에 Savepoint를 찍는 방식입니다. 자식이 실패하면 Savepoint까지만 되돌리고 부모는 계속 진행할 수 있지만, **부모가 롤백되면 자식도 함께 사라진다.** 물리적으로 하나의 트랜잭션이기 때문입니다. 주의할 점은 Savepoint를 지원하는 트랜잭션 매니저에서만 동작한다는 것입니다. JDBC 기반 매니저는 지원하지만 **JPA 트랜잭션 매니저는 지원하지 않아 예외가 발생하므로**, JPA 프로젝트에서는 사실상 선택지가 아닙니다.

---

## 5. 격리 수준 (Isolation)

격리 수준은 동시에 실행되는 트랜잭션들이 서로의 중간 상태를 얼마나 볼 수 있는지를 정합니다.
...
**Dirty Read**는 아직 커밋되지 않은 다른 트랜잭션의 변경을 읽는 것, **Non-Repeatable Read**는 같은 행을 두 번 읽었는데 값이 달라지는 것, **Phantom Read**는 같은 조건으로 두 번 조회했는데 없던 행이 나타나는 것입니다.

> 결론: 아래로 갈수록 안전하지만 동시성이 떨어집니다. `@Transactional(isolation = ...)`로 바꿀 수 있지만, **기본값인 `DEFAULT`(DB 설정을 따름)를 그대로 쓰는 것이 대부분 맞다.** 특정 구간만 격리를 올려야 한다면 격리 수준을 손대기보다 비관적 락이나 낙관적 락처럼 목적이 분명한 도구를 먼저 검토합니다.

DB마다 기본값이 다르다는 점은 알아둘 만합니다. MySQL(InnoDB)은 `REPEATABLE READ`, PostgreSQL과 Oracle은 `READ COMMITTED`가 기본이라, 같은 코드가 DB를 바꾸면 다르게 동작할 수 있습니다.
...
- **타임아웃을 걸어두면 사고가 커지는 것을 막는다.** `@Transactional(timeout = 5)`처럼 상한을 두면 예상치 못하게 오래 걸리는 쿼리가 커넥션을 무한정 붙잡는 상황을 끊을 수 있습니다.
- **`readOnly = true`는 읽기 전용 복제본 라우팅에도 쓰인다.** `AbstractRoutingDataSource`로 현재 트랜잭션이 읽기 전용인지 판단해 리더/라이터 DB를 나눠 보내는 구성이 흔합니다. 반대로 트랜잭션이 실제로 걸렸는지 확인하려면 `org.springframework.transaction.interceptor` 로거를 DEBUG로 켜서 생성/커밋 로그를 보면 됩니다.
- **테스트의 `@Transactional`은 성격이 다르다.** 테스트 메서드에 붙이면 끝난 뒤 자동 롤백되어 DB가 깨끗해지는데, 이 때문에 실제 커밋 시점에만 드러나는 문제(제약조건 위반, `AFTER_COMMIT` 리스너 동작)를 테스트가 놓칠 수 있습니다.
...
- 꼬리 질문: "그럼 `@Async` 안에서 바깥 트랜잭션에 참여할 수 있나요?" → 스레드가 바뀌면 `ThreadLocal`이 달라지므로 참여할 수 없습니다.
...
- 꼬리 질문: "Checked Exception을 롤백하려면요?" → `rollbackFor = Exception.class`를 명시하거나, 애초에 비즈니스 예외를 `RuntimeException` 계열로 설계합니다.
...
- 꼬리 질문: "`AFTER_COMMIT`에서 실패하면 어떻게 하나요?" → 이미 커밋된 상태라 롤백은 불가능합니다. 재시도 큐에 넣거나 보상 트랜잭션으로 처리하는 설계가 필요합니다.
...
| "`readOnly = true`면 쓰기가 막힌다" | 플러시를 생략하는 최적화 힌트일 뿐이다 | 쓰기 차단 장치가 아닙니다. 예외 경로가 존재한다 |
...
`@Transactional`은 프록시가 메서드를 감싸 커밋과 롤백을 대신해주는 장치이고, 이 문서의 함정은 전부 **"프록시를 거치지 않았거나(1·2번), 프록시가 예외를 보지 못했거나(3·4번), 트랜잭션 경계가 너무 넓거나(6번)"** 세 가지 중 하나로 환원됩니다.
