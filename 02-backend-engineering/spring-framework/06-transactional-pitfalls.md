...
> 어노테이션 한 줄로 트랜잭션이 걸리는 원리를 짚고, "분명 붙였는데 롤백이 안 되는" 상황이 그 원리 때문에 왜 생기는지 봅니다. 읽고 나면 그런 상황을 설명하고 피할 수 있게 됩니다.
...
- [02-aop-proxy.md](./02-aop-proxy.md) - 이 문서의 함정 대부분이 프록시 구조에서 나옵니다
...
진짜 로직은 가운데 두 줄인데 나머지가 전부 배관 코드입니다. 게다가 이 코드에는 위험이 여럿 숨어 있습니다.

- `conn.close()`를 빠뜨리면 커넥션이 새고, 얼마 지나 풀이 고갈되어 서비스 전체가 멈춥니다
- `rollback()`을 조건에 따라 빠뜨리면 절반만 반영된 데이터가 남습니다
- `Connection`을 DAO 파라미터로 계속 넘겨야 해서 메서드 시그니처가 오염됩니다
...
프록시를 의사코드로 옮기면 이렇습니다. 조건절에 `RuntimeException`과 `Error`만 적혀 있다는 점을 기억해 둘 만합니다.
...
**왜 문제인가**: 기본 롤백 규칙은 `RuntimeException`과 `Error`만 봅니다. `IOException` 같은 Checked Exception은 롤백 대상이 아니라서, 예외가 밖으로 나가는데도 트랜잭션은 커밋됩니다. 영수증 없는 주문이 DB에 남습니다.
...
"나는 예외를 다 잡았는데 왜 죽지?"라는 의문이 이렇게 생깁니다. 기본 전파인 `REQUIRED`에서는 안쪽과 바깥쪽이 **같은 물리 트랜잭션**을 공유하므로, 안에서 한 번 롤백 표시가 붙으면 되돌릴 수 없습니다.
...
`@Transactional(readOnly = true)`는 **최적화 힌트**일 뿐입니다. JPA를 쓰면 영속성 컨텍스트의 플러시 모드가 바뀌어 변경 감지를 위한 스냅샷 비교와 플러시를 건너뜁니다. 조회가 많은 메서드에서 메모리와 CPU를 아낄 수 있습니다.

여기서 오해가 생깁니다. **이것은 쓰기를 차단하는 보안 장치가 아닙니다.** 예를 들어 식별자 생성 전략에 따라 `persist` 시점에 곧바로 INSERT가 나가는 경우처럼, 플러시와 무관하게 SQL이 실행되는 경로가 있습니다. "`readOnly`를 붙였으니 실수로 저장될 일은 없다"고 믿으면 안 됩니다.

**클래스 레벨 `readOnly`가 쓰기 메서드에까지 상속되는 것**이 더 자주 사고를 냅니다.
...
`REQUIRES_NEW`는 주로 기록에 씁니다. **본 작업의 성패와 무관하게 남아야 하는 기록**입니다. 주문이 실패해도 시도 이력은 남겨야 하는 감사 로그가 전형적입니다. 다만 안쪽 트랜잭션이 도는 동안 **커넥션을 두 개 점유합니다.** 풀 크기가 10인데 `REQUIRES_NEW`를 쓰는 요청이 10개 동시에 들어오면, 바깥 트랜잭션 10개가 커넥션을 다 쓴 상태에서 안쪽이 또 커넥션을 기다리게 되어 교착이 생길 수 있습니다.

### NESTED

`NESTED`는 부모 트랜잭션 안에 Savepoint를 찍는 방식입니다. 자식이 실패하면 Savepoint까지만 되돌리고 부모는 계속 진행할 수 있지만, **부모가 롤백되면 자식도 함께 사라집니다.** 물리적으로 하나의 트랜잭션이기 때문입니다. 다만 주의할 게 있습니다. Savepoint를 지원하는 트랜잭션 매니저에서만 동작합니다. JDBC 기반 매니저는 지원하지만 **JPA 트랜잭션 매니저는 지원하지 않아 예외가 발생하므로**, JPA 프로젝트에서는 사실상 선택지가 아닙니다.

---

## 5. 격리 수준 (Isolation)

동시에 실행되는 트랜잭션은 서로의 중간 상태를 어디까지 볼 수 있을까요. 격리 수준이 그 범위를 정합니다.
