...
배관이 전부 사라졌습니다. 편해진 만큼 대가가 있습니다. **트랜잭션 경계가 코드에 보이지 않기 때문에, 경계가 어긋나도 눈에 띄지 않습니다.** 이 문서의 나머지는 전부 그 이야기입니다.
...
이 사실에서 중요한 따름정리가 나옵니다. **스레드가 바뀌면 트랜잭션도 끊깁니다.** `@Async` 메서드나 별도 스레드에서 실행되는 코드는 바깥 트랜잭션에 참여하지 못합니다.
...
`@Transactional(readOnly = true)`가 하는 일은 **최적화 힌트**입니다. JPA를 쓰면 영속성 컨텍스트의 플러시 모드가 바뀌어 변경 감지를 위한 스냅샷 비교와 플러시를 건너뜁니다. 조회가 많은 메서드에서 메모리와 CPU를 아낄 수 있습니다.

오해는 여기서 생깁니다. **이것은 쓰기를 차단하는 보안 장치가 아닙니다.** 예를 들어 식별자 생성 전략에 따라 `persist` 시점에 곧바로 INSERT가 나가는 경우처럼, 플러시와 무관하게 SQL이 실행되는 경로가 있습니다. "`readOnly`를 붙였으니 실수로 저장될 일은 없다"고 믿으면 안 됩니다.
...
1. **커넥션 점유 시간이 API 응답 시간만큼 늘어납니다.** 풀 크기가 10인데 동시 주문이 10건이면 11번째 요청부터 커넥션을 못 받고 대기합니다. 결제 API가 느려지는 순간 서비스 전체가 멈춥니다. 재시도 로직까지 트랜잭션 안에서 돌면 점유 시간은 배로 늘어납니다.
2. **DB 락도 그만큼 유지됩니다.** 재고 행을 잠근 채 10초를 기다리면 같은 상품을 사려는 다른 요청이 전부 밀립니다. 데드락 확률도 올라갑니다.
3. **롤백해도 외부 호출은 되돌아가지 않습니다.** DB는 롤백됐는데 결제는 이미 승인된 상태가 만들어집니다.
...
원칙은 하나입니다. **트랜잭션 안에는 DB 작업만 둡니다.** 메일 발송, 파일 업로드, 외부 결제, 슬랙 알림은 모두 밖으로 뺍니다.
...
`REQUIRES_NEW`의 대표 용도는 **본 작업의 성패와 무관하게 남아야 하는 기록**입니다. 주문이 실패해도 시도 이력은 남겨야 하는 감사 로그가 전형적입니다. 다만 안쪽 트랜잭션이 도는 동안 **커넥션을 두 개 점유합니다.** 풀 크기가 10인데 `REQUIRES_NEW`를 쓰는 요청이 10개 동시에 들어오면, 바깥 트랜잭션 10개가 커넥션을 다 쓴 상태에서 안쪽이 커넥션을 기다리는 교착이 생길 수 있습니다.

### NESTED

`NESTED`는 부모 트랜잭션 안에 Savepoint를 찍는 방식입니다. 자식이 실패하면 Savepoint까지만 되돌리고 부모는 계속 진행할 수 있지만, **부모가 롤백되면 자식도 함께 사라집니다.** 물리적으로 하나의 트랜잭션이기 때문입니다. 주의할 점은 Savepoint를 지원하는 트랜잭션 매니저에서만 동작한다는 것입니다. JDBC 기반 매니저는 지원하지만 **JPA 트랜잭션 매니저는 지원하지 않아 예외가 발생하므로**, JPA 프로젝트에서는 사실상 선택지가 아닙니다.
...
> 결론: 아래로 갈수록 안전하지만 동시성이 떨어집니다. `@Transactional(isolation = ...)`로 바꿀 수 있지만, **기본값인 `DEFAULT`(DB 설정을 따름)를 그대로 쓰는 것이 대부분 맞습니다.** 특정 구간만 격리를 올려야 한다면 격리 수준을 손대기보다 비관적 락이나 낙관적 락처럼 목적이 분명한 도구를 먼저 검토합니다.
...
- **타임아웃을 걸어두면 사고가 커지는 것을 막습니다.** `@Transactional(timeout = 5)`처럼 상한을 두면 예상치 못하게 오래 걸리는 쿼리가 커넥션을 무한정 붙잡는 상황을 끊을 수 있습니다.
...
- **테스트의 `@Transactional`은 성격이 다릅니다.** 테스트 메서드에 붙이면 끝난 뒤 자동 롤백되어 DB가 깨끗해지는데, 이 때문에 실제 커밋 시점에만 드러나는 문제(제약조건 위반, `AFTER_COMMIT` 리스너 동작)를 테스트가 놓칠 수 있습니다.
