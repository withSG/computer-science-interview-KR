...
> `@SpringBootApplication` 한 줄이 실제로 무슨 일을 하는지, 내가 등록한 적 없는 Bean이 어떻게 컨테이너에 들어와 있는지, 그리고 그 자동 설정을 어떻게 덮어쓰고 어떻게 추적하는지 설명할 수 있게 된다.
...
Spring Boot 이전에 "DB에 연결하고 JPA를 쓰고 웹 요청을 받는" 최소 구성을 만들려면 XML이 이 정도 필요했다.
...
여기에 뷰 리졸버, 정적 리소스 핸들러, 메시지 컨버터 설정이 더 붙는다. 그러고 나서야 첫 번째 컨트롤러를 쓸 수 있었다.

문제는 분량이 아니라 세 가지다.

1. **프로젝트마다 거의 똑같다.** 위 XML의 90%는 어느 회사 어느 프로젝트에서든 동일하다. 그런데 매번 손으로 복사한다.
2. **오타가 런타임에야 드러난다.** `transactionManager`를 `transacionManager`로 적어도 컴파일은 통과한다. 서버를 띄워봐야 안다.
3. **버전 조합을 사람이 맞춰야 한다.** Spring 5.3에 어떤 Hibernate 버전이 맞는지, Jackson은 몇 번대여야 하는지를 개발자가 직접 조사했다.

### Spring Boot의 답: 설정보다 관례

Spring Boot는 "어차피 90%가 똑같다면, 그 90%는 기본으로 깔아두고 나머지 10%만 개발자가 적게 하자"는 발상이다. 이것이 설정보다 관례(Convention over Configuration)다.
...
위의 XML 전부가 이 몇 줄로 대체된다. **중요한 것은 이게 마법이 아니라는 점이다.** Spring Boot가 하는 일은 그 XML에 해당하는 `@Configuration` 클래스들을 미리 다 작성해두고, 조건에 맞을 때만 켜지도록 만든 것뿐이다.

> 비유: 인테리어가 끝난 풀옵션 오피스텔. 냉장고, 세탁기, 에어컨이 이미 들어와 있어서 짐만 들고 오면 된다. 마음에 안 드는 가전은 내가 가져온 것으로 바꿔 넣을 수 있다.
>
> **비유의 한계**: 오피스텔 가전은 물리적으로 이미 놓여 있지만, 자동 설정은 **조건을 만족할 때만** 등록된다. 세탁기를 쓸 배관이 없으면 세탁기는 아예 들어오지 않는다.

---

## 2. `@SpringBootApplication` 분해

이 어노테이션은 세 개를 합친 것이다.
...
`@ComponentScan`이 **메인 클래스의 패키지를 기준으로 한다**는 점이 중요하다. 메인 클래스를 `com.shop`에 두면 `com.shop.order`, `com.shop.member`가 모두 스캔되지만, `com.external`에 만든 클래스는 잡히지 않는다. "분명히 `@Service`를 붙였는데 Bean이 없다"는 문제의 흔한 원인이다.
...
자동 설정 클래스 목록은 각 스타터 JAR 안에 텍스트 파일로 들어 있다. `spring-boot-autoconfigure` JAR을 열어보면 이런 내용이다.
...
이 파일의 위치는 Spring Boot 버전에 따라 다르다.
...
> 결론: 면접에서는 "예전에는 `spring.factories`, 지금은 `AutoConfiguration.imports`"로 답하면 된다. 직접 스타터를 만드는 경우가 아니면 이 파일을 건드릴 일은 없다.

### [3] 조건 평가 — `@Conditional` 계열

각 자동 설정 클래스는 "언제 켜져야 하는가"를 어노테이션으로 선언한다.
...
> 결론: 자동 설정의 두 축은 **`@ConditionalOnClass`(라이브러리를 넣었는가)** 와 **`@ConditionalOnMissingBean`(개발자가 직접 만들었는가)** 이다. 나머지는 세부 조정이다.
...
대부분의 조정은 이 선에서 끝난다. 자동 설정 클래스들이 `@ConfigurationProperties`로 값을 받도록 만들어져 있기 때문이다.
...
이 Bean이 등록되는 순간 `DataSourceAutoConfiguration`의 `@ConditionalOnMissingBean`이 실패하고, 자동 설정은 손을 뗀다.
...
DB 없이 배치 애플리케이션만 띄우고 싶은데 `spring-boot-starter-data-jpa`가 딸려 들어와 기동이 실패하는 경우 같은, 예외적인 상황에서 쓴다.
...
> 결론: **프로퍼티 → Bean 등록 → exclude 순으로 시도한다.** `exclude`는 그 클래스가 등록하던 다른 Bean들까지 같이 사라지므로 부작용을 확인해야 한다.
...
**왜 문제인가**: `configureMessageConverters`는 기본 컨버터 목록을 **통째로 대체**한다. 여기에 하나만 추가하면 문자열 컨버터, 리소스 컨버터 등 나머지가 전부 사라져 예상치 못한 곳이 깨진다. 게다가 `ObjectMapper` Bean을 등록한 것만으로 이미 자동 설정이 그 인스턴스를 쓰도록 되어 있어 두 번째 코드는 애초에 불필요하다.
...
Spring Boot는 자동 설정 곳곳에 `Customizer` 인터페이스를 열어둔다. 전체를 갈아엎는 대신 **필요한 부분만 손대는 훅이 있는지 먼저 찾아보는 것**이 원칙이다.

---

## 5. 디버깅 — 무엇이 켜졌고 무엇이 꺼졌나

자동 설정의 가장 큰 불만은 "왜 이게 동작하는지/안 하는지 모르겠다"이다. Spring Boot는 이 질문에 답하는 리포트를 내장하고 있다.
...
- **Starter는 의존성 묶음일 뿐**이다. `spring-boot-starter-web`은 그 자체로 코드가 거의 없고, Spring MVC·내장 톰캣·Jackson을 함께 끌어오는 역할을 한다. 실제 설정은 `spring-boot-autoconfigure`에 들어 있다.
- **버전 관리는 부모 BOM이 한다.** `spring-boot-dependencies`가 수백 개 라이브러리의 검증된 버전 조합을 고정하기 때문에, 개발자가 버전을 적지 않아도 서로 호환되는 조합이 들어온다. 이 부분이 자동 설정만큼이나 실무 시간을 아껴준다.
- **회사 공통 모듈을 스타터로 만드는 경우**가 있다. 사내 인증 클라이언트나 로깅 규격을 자동 설정 클래스로 만들고 `AutoConfiguration.imports`에 등록하면, 다른 팀은 의존성만 추가하고 프로퍼티 몇 줄만 적으면 된다.
- **기동이 느려졌다면 자동 설정 개수를 먼저 본다.** 쓰지 않는 스타터가 딸려 들어와 불필요한 자동 설정이 켜져 있는 경우가 흔하다. `--debug` 리포트의 Positive matches 길이가 좋은 단서다.
...
- 꼬리 질문: "제가 `DataSource` Bean을 만들면 어떻게 되나요?" → `@ConditionalOnMissingBean`이 실패해 자동 설정이 물러난다. 개발자 정의가 항상 우선한다.
...
자동 설정은 **"클래스패스에 무엇이 있는지"** 와 **"개발자가 직접 만든 Bean이 있는지"** 두 가지 조건으로 미리 작성된 설정 클래스를 켜고 끄는 장치이며, 언제나 개발자가 만든 것이 우선하고 그 판단 과정은 `--debug` 리포트로 전부 들여다볼 수 있다.
