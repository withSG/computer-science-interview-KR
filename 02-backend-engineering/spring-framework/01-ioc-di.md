...
IoC(Inversion of Control)는 **설계 원칙**입니다. 객체의 생성, 의존 관계 연결, 생명주기 관리, 호출 시점에 대한 제어권을 개발자 코드가 아닌 프레임워크가 쥡니다.

"역전"이라는 표현이 헷갈리기 쉬운데, 뒤집히는 쪽은 데이터 흐름이 아니라 **제어 흐름**입니다.
...
요리사가 직접 시장에 가서 재료를 사 오는 주방과, 구매팀이 손질된 재료를 조리대에 올려두는 주방을 생각해봅시다. 후자에서 요리사는 재료가 어느 농장에서 왔는지 모른 채 요리에만 집중합니다. 산지를 바꾸는 결정은 구매팀 몫이고, 요리사의 레시피는 한 글자도 바뀌지 않습니다.

> **비유의 한계**: 컨테이너는 재료를 "사 오는" 데서 그치지 않고 직접 "만들기"까지 합니다. 또 실제 주방과 달리 재료(싱글톤 Bean)는 대개 한 번 준비되면 애플리케이션이 끝날 때까지 교체되지 않습니다.
...
그 우회가 늘 안전하지도 않습니다. 미리 건네진 것이 완성 전의 원본이라 나중에 씌워질 AOP 프록시가 빠진 채 주입되거나, 생성 순서에 따라 결국 `BeanCurrentlyInCreationException`이 나기도 합니다. 조건이 맞아야 재현되니 원인을 찾기도 어렵습니다. 흔히 "필드 주입은 순환 참조가 런타임 `StackOverflowError`로 드러난다"고 설명하는데, 이건 정확하지 않습니다. `StackOverflowError`는 두 Bean이 실제로 서로를 무한히 호출할 때 나는 것이지 순환 **주입** 자체가 만드는 오류가 아닙니다. 차이는 기동 시점에 명시적으로 실패하느냐, 아무 말 없이 넘어가느냐입니다.
...
4. 여기까지 정해지지 않으면 파라미터(필드) 이름과 Bean 이름이 일치하는 것을 고릅니다 — 마지막 수단

이름 매칭이 가장 나중입니다. `@Primary`가 붙어 있으면 이름을 맞춰도 `@Primary` 쪽이 이깁니다.
...
싱글톤 Bean은 무상태로 설계합니다. Spring 애플리케이션 설계의 가장 기본적인 규칙입니다.
...
A. IoC는 객체의 생성과 호출 시점에 대한 제어권을 개발자 코드가 아니라 프레임워크가 갖는다는 **설계 원칙**입니다. DI는 그 원칙을 구현하는 구체적인 패턴 중 하나입니다. 의존 객체를 스스로 `new` 하지 않고 외부에서 주입받는 방식이죠. IoC를 구현하는 방법에는 DI 외에도 Service Locator, Template Method, 이벤트 콜백이 있고, `JdbcTemplate`이 Template Method 방식의 예입니다.
...
IoC는 "무엇을 만들고 언제 부를지"의 결정권을 프레임워크에 넘기는 원칙입니다. DI는 그 결정을 생성자 파라미터로 흘려 넣는 구현입니다. 생성자 주입을 쓰는 이유는 불완전한 객체와 순환 참조를 **운영 전에** 잡아내기 위해서입니다.
