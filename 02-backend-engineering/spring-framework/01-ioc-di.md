...
A. 네 가지입니다. `final`을 쓸 수 있어 불변이 보장되고, 의존성 없이는 객체가 만들어지지 않아 반쯤 초기화된 객체가 존재할 수 없습니다. 순환 참조는 애플리케이션 기동 시점에 `BeanCurrentlyInCreationException`으로 잡히고, 컨테이너 없이 `new`만으로 단위 테스트를 짤 수 있습니다. 특히 세 번째가 중요한데, 필드 주입에서는 컨테이너가 미완성 참조를 미리 건네 순환을 우회해 버려서 잘못된 구조가 아무 신호 없이 그대로 배포됩니다.
