...
> `new` 한 줄이 왜 문제가 되는지, 그 제어권을 컨테이너에 넘기면 무엇이 좋아지는지, 그리고 왜 하필 생성자 주입인지 설명할 수 있게 됩니다.
...
동작은 합니다. 문제는 이 한 줄이 네 가지 부담을 동시에 떠안긴다는 것입니다.
...
타입만 인터페이스로 바뀌었을 뿐, **어떤 구현체를 쓸지 고르는 결정권은 여전히 `OrderService` 안에** 있습니다. 상위 정책(주문)이 하위 세부사항(카카오페이)에 의존하는 구조 — 의존 역전 원칙(DIP, Dependency Inversion Principle) 위반이 그대로 남아 있습니다.
...
이제 `OrderService`는 "결제를 시킨다"만 알고 "누가 결제하는지"는 모릅니다. 대신 **누군가는 그 "누구"를 정해서 넣어줘야 한다.** 그 역할을 애플리케이션 전체 규모로 대신 해주는 존재가 IoC 컨테이너입니다.
...
IoC(Inversion of Control)는 **객체의 생성, 의존 관계 연결, 생명주기 관리, 그리고 호출 시점에 대한 제어권을 개발자 코드가 아닌 프레임워크가 갖는 설계 원칙**입니다.

"역전"이라는 표현이 헷갈리기 쉬운데, 뒤집히는 것은 데이터 흐름이 아니라 **제어 흐름**입니다.
...
이 원칙을 한 문장으로 압축한 표현이 헐리우드 원칙(Hollywood Principle)입니다. "Don't call us, we'll call you" — 네가 우리를 부르지 마라, 필요하면 우리가 부르겠습니다.

### 비유: 레스토랑 주방

요리사가 직접 시장에 가서 재료를 사 오는 주방과, 구매팀이 손질된 재료를 조리대에 올려두는 주방을 생각해보자. 후자에서 요리사는 재료가 어느 농장에서 왔는지 모른 채 요리에만 집중합니다. 산지를 바꾸는 결정은 구매팀 몫이고, 요리사의 레시피는 한 글자도 바뀌지 않습니다.

> **비유의 한계**: 컨테이너는 재료를 "사 오는" 데서 그치지 않고 직접 "만든다". 또 실제 주방과 달리 재료(싱글톤 Bean)는 대개 한 번 준비되면 애플리케이션이 끝날 때까지 교체되지 않습니다.
...
`JdbcTemplate`을 예로 들면, 커넥션을 열고 닫고 예외를 변환하는 흐름은 Spring이 쥐고 있고 개발자는 SQL과 결과 매핑만 채웁니다. `new`를 한 번도 안 썼는데도 IoC인 이유입니다.
...
여기서 **3번(순서 결정)이 DI의 진짜 가치입니다. 의존 관계가 수십 개로 얽힌 애플리케이션에서 "무엇을 먼저 만들어야 하는가"를 사람이 손으로 관리하는 것은 금방 한계에 부딪힙니다. 컨테이너는 그래프를 스스로 정렬해 순서를 잡습니다.
...
Lombok의 `@RequiredArgsConstructor`를 클래스에 붙이면 `final` 필드를 모은 생성자를 컴파일 시점에 만들어주므로, 위 생성자 코드는 통째로 생략할 수 있습니다.
...
`final` 필드는 생성자에서 딱 한 번만 값이 정해집니다. 애플리케이션이 도는 동안 `paymentService`가 다른 객체로 바뀔 가능성이 원천 차단됩니다. 싱글톤 Bean은 모든 요청 스레드가 공유하므로, 참조가 도중에 바뀌면 어떤 요청은 A로 어떤 요청은 B로 결제하는 재현 불가능한 버그가 생깁니다.
...
생성자 주입은 의존성 없이는 객체가 아예 만들어지지 않습니다. 즉 **불완전한 상태의 객체가 코드 어디에도 흘러 다닐 수 없다.**
...
그 우회가 늘 안전한 것도 아닙니다. 미리 건네진 것이 완성 전의 원본이라 나중에 씌워질 AOP 프록시가 빠진 채 주입되거나, 생성 순서에 따라 결국 `BeanCurrentlyInCreationException`이 나기도 합니다. 조건이 맞아야 재현되니 원인을 찾기도 어렵습니다. 흔히 "필드 주입은 순환 참조가 런타임 `StackOverflowError`로 드러난다"고 설명하는데, 이건 정확하지 않습니다. `StackOverflowError`는 두 Bean이 실제로 서로를 무한히 호출할 때 나는 것이지 순환 **주입** 자체가 만드는 오류가 아닙니다. 진짜 차이는 **기동 시점에 명시적으로 실패하느냐, 아무 말 없이 넘어가느냐**에 있습니다.
...
순환 참조의 진짜 해법은 주입 방식을 바꾸는 것이 아닙니다. **책임 분리가 잘못됐다는 신호**로 읽고, 공통 로직을 제3의 클래스로 빼거나 `ApplicationEventPublisher`로 간접 통신으로 바꾸는 것이 정석입니다. `@Lazy`는 증상만 가립니다.
...
**왜 문제인가**: 컴파일러가 검증해주지 못하는 문자열 기반 리플렉션에 테스트가 묶입니다. 게다가 의존성이 하나 추가돼도 테스트 코드는 조용히 통과하다가 NPE로 터집니다.
...
Spring 컨테이너를 띄우지 않으므로 테스트가 수십 배 빠르다는 것은 덤입니다.

덧붙여 생성자 주입은 **설계 냄새 탐지기** 역할도 합니다. 파라미터가 일곱 개, 여덟 개로 늘어나면 "이 클래스가 너무 많은 일을 한다"가 눈에 보입니다. 필드 주입은 `@Autowired` 한 줄만 더 붙이면 그만이라 이 신호가 묻힙니다.
...
컨테이너가 생성하고 관리하는 객체를 Bean이라고 부릅니다. `new`로 만든 객체는 아무리 `@Service`가 붙어 있어도 Bean이 아닙니다. **컨테이너를 거쳐 나온 것만 Bean이고, Bean이어야만 DI, AOP, 트랜잭션 같은 Spring 기능이 적용된다.**
...
이름 매칭이 가장 나중이라는 점이 중요합니다. `@Primary`가 붙어 있으면 이름을 맞춰도 `@Primary` 쪽이 이깁니다.
...
싱글톤 Bean은 무상태(stateless)로 설계합니다. 이것이 Spring 애플리케이션 설계의 가장 기본적인 규칙입니다.
...
- **테스트 전략이 갈린다.** 컨테이너 없이 `new`로 만드는 순수 단위 테스트(빠름)와 `@SpringBootTest`로 컨테이너를 띄우는 통합 테스트(느림)를 구분해서 쓰는데, 생성자 주입이어야 전자가 가능합니다. 반대로 테스트 클래스 자체는 직접 `new` 할 일이 없으므로 필드 주입을 써도 무방합니다.
...
- 꼬리 질문: "그럼 DI가 없으면 IoC가 아닌가요?" → 아닙니다. 제어권이 프레임워크로 넘어갔다면 방식과 무관하게 IoC다. 다만 Spring의 핵심은 DI 컨테이너입니다.
...
- 꼬리 질문: "요청별 상태가 꼭 필요하면요?" → `request` 스코프 Bean이나 `ThreadLocal`을 쓰되, 스레드 풀 재사용 때문에 `ThreadLocal`은 반드시 정리(remove)해야 한다고 답합니다.
...
- 꼬리 질문: "이름을 Bean 이름과 똑같이 맞췄는데 다른 Bean이 주입됐습니다" → 다른 후보에 `@Primary`가 붙어 있는지 확인합니다. 이름 매칭은 마지막 수단이라 `@Primary`에 밀립니다.
...
| 실수 | 왜 틀렸나 | 올바른 이해 |
|------|----------|-----------|
| "생성자 주입은 의존성이 없으면 컴파일 에러가 난다" | 컴파일러는 Spring Bean의 존재 여부를 모릅니다. 컨테이너 기동 시점에 `NoSuchBeanDefinitionException`으로 실패하는 것이다 | "런타임이지만 **기동 시점**에 실패한다"가 정확한 표현. 요청 처리 중이 아니라 배포 직후에 잡힌다는 점이 핵심 |
| "IoC = DI" | DI는 IoC의 여러 구현 중 하나 | IoC는 원칙, DI는 패턴. Template Method나 이벤트 콜백도 IoC다 |
| "`@Autowired`는 이름으로 주입한다" | 1순위는 타입 매칭입니다. 이름은 후보가 끝까지 안 좁혀졌을 때 쓰는 마지막 수단 | 타입으로 후보를 모은 뒤 `@Qualifier` → `@Primary` → `@Priority` → 이름 순으로 좁힌다 |
| "필드 주입은 순환 참조가 런타임 `StackOverflowError`로 드러난다" | 순환 주입 자체는 `StackOverflowError`를 내지 않습니다. 컨테이너가 미완성 참조를 미리 건네 우회한다 | 진짜 문제는 아무 신호 없이 기동된다는 것. `StackOverflowError`는 두 Bean이 실제로 서로를 무한 호출할 때 나는 별개의 현상이다 |
...
IoC는 "무엇을 만들고 언제 부를지"의 결정권을 프레임워크에 넘기는 원칙이고, DI는 그 결정을 생성자 파라미터로 흘려 넣는 구현이며, 생성자 주입을 쓰는 이유는 불완전한 객체와 순환 참조를 **운영 전에** 잡아내기 위해서입니다.
