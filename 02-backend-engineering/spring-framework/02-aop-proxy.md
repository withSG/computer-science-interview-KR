...
필요한 것은 **비즈니스 코드를 한 글자도 건드리지 않고 밖에서 부가 기능을 끼워 넣는 방법**입니다. 그것이 AOP입니다.
...
핵심은 이것입니다. **`@Autowired`로 주입받는 객체는 원본이 아니라 프록시입니다.** 개발자가 눈치채지 못하는 이유는 프록시가 원본과 같은 타입이라 구분이 안 되기 때문입니다.
...
> 결론: **Spring Boot 2.0부터 `spring.aop.proxy-target-class=true`가 기본값이라 인터페이스 유무와 무관하게 CGLIB를 씁니다.** 면접에서는 "예전에는 인터페이스가 있으면 JDK, 없으면 CGLIB였는데 지금은 일관성을 위해 CGLIB 고정"이라고 답하면 됩니다.

### 왜 CGLIB로 통일했나

JDK 동적 프록시로 만들어진 프록시는 **인터페이스 타입일 뿐 구현 클래스 타입이 아닙니다.** 그래서 이런 코드가 터졌습니다.
...
**왜 문제인가**: CGLIB는 상속으로 프록시를 만드는데 `final` 클래스는 상속할 수 없습니다. 메서드에 `final`이 붙은 경우는 더 고약합니다. 프록시 생성 자체는 성공하고 그 메서드만 오버라이드되지 않아 **예외 없이 부가 기능만 빠집니다.** Spring이 관련 로그를 남기기는 하지만 기본 로그 레벨에서는 보이지 않는 경우가 대부분이라, 트랜잭션이 걸린 줄 알았는데 안 걸린 채 운영에 나갑니다.
...
- **직접 만드는 Aspect는 생각보다 적습니다.** 실무에서 커스텀 AOP를 쓰는 대표적인 곳은 API 요청/응답 로깅, 실행 시간 측정, 메서드 파라미터 기반 권한 체크, 분산 락 획득 정도입니다.
