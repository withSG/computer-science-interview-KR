...
주문 처리 메서드를 하나 만들었다고 해 봅시다. 처음에는 세 줄이었습니다.
...
> 면접에서 헷갈리기 쉬운 짝은 **JoinPoint(가능한 모든 지점)와 Pointcut(그중 고른 지점)** 입니다. AspectJ는 필드 접근·객체 생성까지 JoinPoint로 잡지만 **Spring AOP는 메서드 실행 하나만** 지원한다는 점을 반드시 함께 말해야 합니다.
...
> **비유의 한계**: 실제 프록시는 부재중 여부와 무관하게 **모든 호출을 항상** 가로챕니다. 또 비서와 팀장은 별개의 사람이지만, 프록시는 원본과 같은 타입이어야만 합니다.
...
**왜 문제인가**: CGLIB는 상속으로 프록시를 만드는데 `final` 클래스는 상속할 수 없습니다. 메서드에 `final`이 붙은 경우는 더 고약합니다. 프록시 생성 자체는 성공하고 그 메서드만 오버라이드되지 않아 **예외 없이 부가 기능만 빠집니다.** Spring이 관련 로그를 남기기는 하지만 기본 로그 레벨에서는 보이지 않는 경우가 대부분입니다. 트랜잭션이 걸린 줄 알았는데 안 걸린 채 운영에 나갑니다.
