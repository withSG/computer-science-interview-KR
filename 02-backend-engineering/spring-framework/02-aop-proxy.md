...
> 로깅·트랜잭션 코드가 왜 비즈니스 로직에 섞이면 안 되는지, Spring이 그것을 프록시로 어떻게 떼어내는지, 그리고 그 프록시 때문에 어떤 함정이 생기는지 설명할 수 있게 된다.

## 학습 목표

- [ ] 횡단 관심사(Cross-Cutting Concern)가 무엇이고 왜 문제인지 예시로 설명할 수 있다
- [ ] 프록시가 Bean을 대신 등록되는 과정을 그림으로 그릴 수 있다
...
주문 처리 메서드를 하나 만들었다고 하자. 처음에는 세 줄이었다.
...
그런데 운영을 시작하면 요구사항이 붙는다. "느린 API를 찾아야 하니 실행 시간을 로그로 남겨주세요." "결제 중 예외가 나면 주문도 롤백돼야 합니다." "관리자만 호출할 수 있게 해주세요."
...
핵심 로직 2줄을 부가 코드 12줄이 감쌌다. 문제는 분량이 아니라 **이 12줄이 서비스 메서드 수백 개에 똑같이 복사된다**는 것이다.

- 로그 포맷을 바꾸려면 수백 곳을 고쳐야 한다
- 어느 한 곳에서 `rollback`을 빠뜨려도 컴파일은 통과한다
- 코드 리뷰에서 진짜 봐야 할 두 줄이 파묻힌다

이렇게 **여러 모듈에 가로질러 흩어지는 공통 관심사**를 횡단 관심사(Cross-Cutting Concern)라고 부른다. 로깅, 트랜잭션, 보안, 캐싱, 재시도, 성능 측정이 대표적이다.

### 상속이나 유틸 메서드로는 안 되는 이유

"공통 부모 클래스를 만들면 되지 않나?" 자바는 단일 상속이라 이미 다른 클래스를 상속 중이면 못 쓴다. 게다가 `AbstractLoggingService`를 상속하는 순간 "이 클래스는 로깅을 한다"가 클래스 정의에 새겨져, 로깅을 빼려면 상속 구조를 바꿔야 한다. "유틸 메서드로 감싸면?" 결국 각 메서드마다 `LogUtil.wrap(() -> { ... })`를 손으로 적어야 하니 호출을 빠뜨리는 실수는 그대로 남는다.

필요한 것은 **비즈니스 코드를 한 글자도 건드리지 않고 밖에서 부가 기능을 끼워 넣는 방법**이다. 그것이 AOP다.
...
AOP(Aspect-Oriented Programming, 관점 지향 프로그래밍)는 횡단 관심사를 별도 모듈로 분리하고, 그 모듈을 원하는 지점에 자동으로 적용하는 프로그래밍 기법이다.

용어가 많아 보이지만 문장 하나로 엮인다.

> **어떤 지점(JoinPoint)** 중에서 **조건에 맞는 곳(Pointcut)** 에 **어떤 부가 기능(Advice)** 을 붙일지 묶어놓은 것이 **Aspect**이고, 실제로 붙이는 작업이 **Weaving**이다.
...
> 표 요약: 면접에서 헷갈리기 쉬운 짝은 **JoinPoint(가능한 모든 지점)와 Pointcut(그중 고른 지점)** 이다. AspectJ는 필드 접근·객체 생성까지 JoinPoint로 잡지만, **Spring AOP는 메서드 실행 하나만** 지원한다는 점을 반드시 덧붙이자.
...
Spring AOP가 런타임 위빙을 택한 이유는 **별도 컴파일러나 JVM 에이전트 없이 순수 자바만으로 동작**하기 때문이다. 대신 프록시를 거치는 호출에만 적용된다는 제약이 따라온다. 이 제약이 이 문서 후반의 함정 대부분의 원인이다.
...
부재중인 팀장에게 온 전화를 비서가 먼저 받는다고 하자. 비서는 통화 시각을 기록하고, 스팸이면 차단하고, 정상이면 팀장에게 연결한다. 전화 건 사람은 자기가 팀장과 통화했다고 생각한다. 팀장은 자기 앞에 비서가 있다는 사실을 모른 채 원래 하던 일만 한다.

> **비유의 한계**: 실제 프록시는 "부재중일 때만" 대신 받는 게 아니라 **모든 호출을 항상** 가로챈다. 또 비서와 팀장은 별개의 사람이지만, 프록시는 원본과 같은 타입이어야만 한다.
...
이제 `OrderService.order()`는 원래의 두 줄로 돌아간다. 측정 코드는 어디에도 없지만 로그는 남는다.
...
> 결론: 전후를 모두 제어해야 하는 트랜잭션·재시도·캐싱은 `@Around`가 아니면 구현할 수 없다. 반대로 단순 로깅에 `@Around`를 쓰면 `proceed()` 호출을 빠뜨리는 실수 위험이 생기므로 `@Before`/`@AfterReturning`이 안전하다.

한 Aspect 안에 여러 Advice를 몰아넣고 실행 순서에 의존하는 코드는 쓰지 않는 것이 좋다. Spring 버전에 따라 세부 순서가 조정된 적이 있어 이식성이 떨어진다. 여러 Aspect 사이의 순서가 중요하면 `@Order`를 명시한다. 값이 작을수록 바깥쪽에서 감싼다.
...
패키지 경로로 거는 방식은 리팩터링으로 패키지를 옮기는 순간 조용히 적용이 풀린다. **커스텀 어노테이션을 만들어 `@annotation`으로 거는 방식**이 의도도 드러나고 안전하다.
...
> 결론: **Spring Boot 2.0부터 `spring.aop.proxy-target-class=true`가 기본값이라 인터페이스 유무와 무관하게 CGLIB를 쓴다.** 면접에서는 "예전에는 인터페이스가 있으면 JDK, 없으면 CGLIB였는데 지금은 일관성을 위해 CGLIB 고정"이라고 답하면 된다.

### 왜 CGLIB로 통일했나

JDK 동적 프록시로 만들어진 프록시는 **인터페이스 타입일 뿐 구현 클래스 타입이 아니다.** 그래서 이런 코드가 터졌다.
...
인터페이스를 하나 추가했을 뿐인데 멀쩡하던 주입이 깨지는 경험은 상당히 당황스럽다. CGLIB는 원본 클래스를 상속하므로 구체 타입으로도 받을 수 있다. Spring Boot가 CGLIB를 기본값으로 잡은 이유가 이 일관성이다.
...
**왜 문제인가**: CGLIB는 상속으로 프록시를 만드는데 `final` 클래스는 상속할 수 없다. 메서드에 `final`이 붙은 경우는 더 고약하다. 프록시 생성 자체는 성공하고 그 메서드만 오버라이드되지 않아 **예외 없이 부가 기능만 빠진다.** Spring이 관련 로그를 남기기는 하지만 기본 로그 레벨에서는 보이지 않는 경우가 대부분이라, 트랜잭션이 걸린 줄 알았는데 안 걸린 채 운영에 나간다.
...
Kotlin으로 Spring을 쓸 때 클래스가 기본 `final`이라 `kotlin-spring` 플러그인(`allopen`)이 사실상 필수인 것도 같은 이유다.
...
> 결론: self-invocation을 만나면 "이 두 메서드가 정말 같은 클래스에 있어야 하는가"를 먼저 의심하자. 대부분은 **분리하는 것이 정답**이고, 나머지 두 방법은 레거시를 당장 고쳐야 할 때의 임시책이다.
...
| `final` 메서드 (CGLIB) | **미적용** | 오버라이드 불가. 예외도 안 나고 기본 로그에도 잘 안 보인다 |
| `static` 메서드 | **미적용** | 인스턴스 메서드가 아님 |
| `new`로 만든 객체 | **미적용** | 컨테이너를 거치지 않아 프록시가 아님 |
| 생성자 안에서의 호출 | **미적용** | 프록시가 만들어지기 전 |

> 결론: "부가 기능이 안 먹는다" 싶으면 이 표를 먼저 확인하자. 원인의 대부분이 여기 있다.

---

## 7. 실무에서는

- **Spring의 굵직한 기능 상당수가 이 프록시 위에 서 있다.** `@Transactional`(트랜잭션), `@Cacheable`(캐시), `@Async`(비동기), `@Retryable`(재시도), `@PreAuthorize`(인가)가 모두 같은 메커니즘이다. 프록시 원리를 한 번 이해하면 이 어노테이션들의 함정도 한꺼번에 이해된다. `@Async` 메서드를 self-invocation으로 부르면 그냥 동기 실행되는 것도 같은 이유다.
- **직접 만드는 Aspect는 생각보다 적다.** 실무에서 커스텀 AOP를 쓰는 대표적인 곳은 API 요청/응답 로깅, 실행 시간 측정, 메서드 파라미터 기반 권한 체크, 분산 락 획득 정도다.
- **Spring Security는 서블릿 필터 체인**으로 동작하고 `@PreAuthorize` 같은 메서드 보안만 AOP로 처리한다. 두 층위를 구분해두면 인증이 어디서 걸렸는지 추적하기 쉽다. 참고로 프록시가 씌워졌는지는 디버거에서 객체 타입을 보면 된다. 이름에 `$$SpringCGLIB$$`나 `$Proxy`가 보이면 프록시다.
...
A. Spring AOP는 프록시 패턴 기반의 런타임 위빙입니다. 컨테이너가 Bean을 만들 때, Pointcut에 걸리는 대상이면 원본 대신 원본을 감싼 프록시 객체를 컨테이너에 등록합니다. 다른 Bean이 주입받는 것도 이 프록시이고, 메서드가 호출되면 프록시가 먼저 Advice를 실행한 뒤 원본 메서드를 호출합니다. AspectJ와 달리 별도 컴파일러나 JVM 에이전트가 필요 없다는 게 장점이고, 대신 메서드 실행 JoinPoint만 지원하고 프록시를 거치는 호출에만 적용된다는 제약이 있습니다.
- 꼬리 질문: "AspectJ와 뭐가 다른가요?" → AspectJ는 컴파일/로드 타임에 바이트코드를 직접 변경해 필드 접근·생성자까지 잡을 수 있고 self-invocation 제약도 없다. 대신 빌드 설정이 복잡하다.

**Q. JDK 동적 프록시와 CGLIB의 차이는 무엇인가요?**
A. JDK 동적 프록시는 대상이 구현한 인터페이스를 똑같이 구현한 프록시를 리플렉션으로 만들기 때문에 인터페이스가 반드시 필요하고, 인터페이스에 선언된 메서드만 부가 기능을 붙일 수 있습니다. CGLIB는 대상 클래스를 상속한 하위 클래스를 바이트코드로 생성하므로 인터페이스가 없어도 되지만, `final` 클래스는 상속이 안 되고 `final` 메서드는 오버라이드가 안 돼 적용에서 빠집니다. Spring Boot 2.0부터는 타입 캐스팅 문제 등 일관성 때문에 인터페이스 유무와 관계없이 CGLIB가 기본입니다.
- 꼬리 질문: "`final` 메서드에 `@Transactional`을 붙이면 어떻게 되나요?" → 예외 없이 트랜잭션만 적용되지 않는다. 조용히 실패하는 유형이라 더 위험하다.
...
- 꼬리 질문: "자기 자신을 주입하는 방법은 어떤가요?" → 동작은 하지만 `@Lazy`가 필요하고 의도가 코드에 드러나지 않아 차선책이라고 답한다.

**Q. Pointcut을 패키지 경로로 거는 것과 어노테이션으로 거는 것 중 뭘 선호하나요?**
A. 어노테이션 방식을 선호합니다. 패키지 표현식은 리팩터링으로 클래스를 옮기면 조용히 적용이 풀리는데, 이때 아무 에러도 나지 않아 발견이 늦습니다. 커스텀 어노테이션을 만들어 `@annotation`으로 걸면 적용 대상이 코드에 명시적으로 드러나고, 클래스를 옮겨도 유지됩니다.
...
Spring AOP는 원본 Bean 앞에 같은 타입의 프록시를 세워 부가 기능을 대신 실행시키는 구조이고, 이 문서의 함정 대부분은 **"프록시를 거치지 않는 호출에는 아무 일도 일어나지 않는다"** 는 한 문장에서 파생된다.

---

## 연관 개념

- [01-ioc-di.md](./01-ioc-di.md) - 프록시가 원본 대신 등록되는 무대인 컨테이너와 Bean
