...
> 로깅·트랜잭션 코드가 왜 비즈니스 로직에 섞이면 안 되는지, Spring이 그것을 프록시로 어떻게 떼어내는지, 그리고 그 프록시 때문에 어떤 함정이 생기는지 설명할 수 있게 됩니다.
...
주문 처리 메서드를 하나 만들었다고 하자. 처음에는 세 줄이었습니다.
...
핵심 로직 2줄을 부가 코드 12줄이 감쌌습니다. 문제는 분량이 아니라 **이 12줄이 서비스 메서드 수백 개에 똑같이 복사된다**는 것입니다.
...
필요한 것은 **비즈니스 코드를 한 글자도 건드리지 않고 밖에서 부가 기능을 끼워 넣는 방법**입니다. 그것이 AOP다.
...
AOP(Aspect-Oriented Programming, 관점 지향 프로그래밍)는 횡단 관심사를 별도 모듈로 분리하고, 그 모듈을 원하는 지점에 자동으로 적용하는 프로그래밍 기법입니다.
...
> 표 요약: 면접에서 헷갈리기 쉬운 짝은 **JoinPoint(가능한 모든 지점)와 Pointcut(그중 고른 지점)** 입니다. AspectJ는 필드 접근·객체 생성까지 JoinPoint로 잡지만, **Spring AOP는 메서드 실행 하나만** 지원한다는 점을 반드시 덧붙이자.
...
Spring AOP가 런타임 위빙을 택한 이유는 **별도 컴파일러나 JVM 에이전트 없이 순수 자바만으로 동작**하기 때문입니다. 대신 프록시를 거치는 호출에만 적용된다는 제약이 따라옵니다. 이 제약이 이 문서 후반의 함정 대부분의 원인입니다.
...
부재중인 팀장에게 온 전화를 비서가 먼저 받는다고 하자. 비서는 통화 시각을 기록하고, 스팸이면 차단하고, 정상이면 팀장에게 연결합니다. 전화 건 사람은 자기가 팀장과 통화했다고 생각합니다. 팀장은 자기 앞에 비서가 있다는 사실을 모른 채 원래 하던 일만 합니다.

> **비유의 한계**: 실제 프록시는 "부재중일 때만" 대신 받는 게 아니라 **모든 호출을 항상** 가로챕니다. 또 비서와 팀장은 별개의 사람이지만, 프록시는 원본과 같은 타입이어야만 합니다.
...
핵심은 이것입니다. **`@Autowired`로 주입받는 객체는 원본이 아니라 프록시다.** 개발자가 눈치채지 못하는 이유는 프록시가 원본과 같은 타입이라 구분이 안 되기 때문입니다.
...
> 결론: 전후를 모두 제어해야 하는 트랜잭션·재시도·캐싱은 `@Around`가 아니면 구현할 수 없습니다. 반대로 단순 로깅에 `@Around`를 쓰면 `proceed()` 호출을 빠뜨리는 실수 위험이 생기므로 `@Before`/`@AfterReturning`이 안전합니다.

한 Aspect 안에 여러 Advice를 몰아넣고 실행 순서에 의존하는 코드는 쓰지 않는 것이 좋습니다. Spring 버전에 따라 세부 순서가 조정된 적이 있어 이식성이 떨어집니다. 여러 Aspect 사이의 순서가 중요하면 `@Order`를 명시합니다. 값이 작을수록 바깥쪽에서 감쌉니다.
...
생성되는 클래스 이름은 Spring 5.3을 기준으로 바뀌었습니다. 그 이전에는 `KakaoPayService$$EnhancerBySpringCGLIB$$...` 형태였고, 지금은 `$$SpringCGLIB$$`가 들어갑니다. 스택 트레이스에서 둘 중 어느 이름이 보이든 CGLIB 프록시라는 뜻입니다.
...
> 결론: **Spring Boot 2.0부터 `spring.aop.proxy-target-class=true`가 기본값이라 인터페이스 유무와 무관하게 CGLIB를 쓴다.** 면접에서는 "예전에는 인터페이스가 있으면 JDK, 없으면 CGLIB였는데 지금은 일관성을 위해 CGLIB 고정"이라고 답하면 됩니다.

### 왜 CGLIB로 통일했나

JDK 동적 프록시로 만들어진 프록시는 **인터페이스 타입일 뿐 구현 클래스 타입이 아니다.** 그래서 이런 코드가 터졌습니다.
...
인터페이스를 하나 추가했을 뿐인데 멀쩡하던 주입이 깨지는 경험은 상당히 당황스럽습니다. CGLIB는 원본 클래스를 상속하므로 구체 타입으로도 받을 수 있습니다. Spring Boot가 CGLIB를 기본값으로 잡은 이유가 이 일관성입니다.
...
**왜 문제인가**: CGLIB는 상속으로 프록시를 만드는데 `final` 클래스는 상속할 수 없습니다. 메서드에 `final`이 붙은 경우는 더 고약합니다. 프록시 생성 자체는 성공하고 그 메서드만 오버라이드되지 않아 **예외 없이 부가 기능만 빠진다.** Spring이 관련 로그를 남기기는 하지만 기본 로그 레벨에서는 보이지 않는 경우가 대부분이라, 트랜잭션이 걸린 줄 알았는데 안 걸린 채 운영에 나갑니다.
...
**왜 문제인가**: 앞에서 봤듯 부가 기능은 프록시에 있고, 원본 객체에는 없습니다. `createOrder`가 실행되는 시점에 이미 프록시를 통과해 **원본 객체 안**에 들어와 있으므로, 거기서 부르는 `this.processPayment()`는 원본의 메서드를 직접 호출합니다. 프록시를 다시 거치지 않으니 `@Transactional`이 개입할 틈이 없습니다.
...
> 결론: self-invocation을 만나면 "이 두 메서드가 정말 같은 클래스에 있어야 하는가"를 먼저 의심하자. 대부분은 **분리하는 것이 정답**이고, 나머지 두 방법은 레거시를 당장 고쳐야 할 때의 임시책입니다.
...
> 결론: "부가 기능이 안 먹는다" 싶으면 이 표를 먼저 확인하자. 원인의 대부분이 여기 있습니다.

---

## 7. 실무에서는

- **Spring의 굵직한 기능 상당수가 이 프록시 위에 서 있다.** `@Transactional`(트랜잭션), `@Cacheable`(캐시), `@Async`(비동기), `@Retryable`(재시도), `@PreAuthorize`(인가)가 모두 같은 메커니즘입니다. 프록시 원리를 한 번 이해하면 이 어노테이션들의 함정도 한꺼번에 이해됩니다. `@Async` 메서드를 self-invocation으로 부르면 그냥 동기 실행되는 것도 같은 이유입니다.
- **직접 만드는 Aspect는 생각보다 적다.** 실무에서 커스텀 AOP를 쓰는 대표적인 곳은 API 요청/응답 로깅, 실행 시간 측정, 메서드 파라미터 기반 권한 체크, 분산 락 획득 정도입니다.
- **Spring Security는 서블릿 필터 체인**으로 동작하고 `@PreAuthorize` 같은 메서드 보안만 AOP로 처리합니다. 두 층위를 구분해두면 인증이 어디서 걸렸는지 추적하기 쉽습니다. 참고로 프록시가 씌워졌는지는 디버거에서 객체 타입을 보면 됩니다. 이름에 `$$SpringCGLIB$$`나 `$Proxy`가 보이면 프록시입니다.
...
- 꼬리 질문: "`final` 메서드에 `@Transactional`을 붙이면 어떻게 되나요?" → 예외 없이 트랜잭션만 적용되지 않습니다. 조용히 실패하는 유형이라 더 위험합니다.
...
| "`@Autowired`로 받은 건 내가 만든 그 클래스다" | AOP 대상이면 프록시다 | 타입은 같지만 다른 객체입니다. 그래서 내부 호출 시 동작이 달라진다 |
...
| "`@Around`에서 `proceed()`를 안 불러도 원본은 실행된다" | `proceed()`가 원본 호출 그 자체다 | 안 부르면 원본이 실행되지 않습니다. 반환값도 그대로 넘겨야 한다 |

---

## 한 줄 정리

Spring AOP는 원본 Bean 앞에 같은 타입의 프록시를 세워 부가 기능을 대신 실행시키는 구조이고, 이 문서의 함정 대부분은 **"프록시를 거치지 않는 호출에는 아무 일도 일어나지 않는다"** 는 한 문장에서 파생됩니다.
