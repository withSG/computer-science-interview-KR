...
> 컨테이너가 Bean을 만들고 초기화하고 없애기까지 정확히 어떤 순서로 무엇을 호출하는지, 그리고 스코프를 잘못 조합했을 때 왜 조용히 잘못 동작하는지 설명할 수 있게 된다.
...
메시지 큐 컨슈머를 Bean으로 만든다고 하자. 이 객체는 태어나자마자 큐에 연결해야 하고, 애플리케이션이 내려갈 때는 연결을 끊고 처리 중인 메시지를 마무리해야 한다.

정리하지 않으면 어떻게 되나. 커넥션 풀이 반환되지 않아 DB 쪽 세션이 쌓이고, 스레드 풀이 종료되지 않아 JVM이 안 죽고, 파일 핸들이 열린 채로 남는다. 배포할 때마다 조금씩 새는 자원은 며칠 뒤 장애로 돌아온다.

그래서 컨테이너는 Bean에게 **"준비됐다"** 와 **"이제 없앤다"** 두 시점을 알려주는 약속된 지점을 제공한다. 그것이 생명주기 콜백이다.

### "그럼 생성자에서 하면 되지 않나"

가장 자연스러운 발상이지만 세 가지 이유로 막힌다.
...
**왜 문제인가**
1. **의존성이 아직 완전하지 않을 수 있다.** 필드 주입이나 Setter 주입을 함께 쓰는 클래스라면 생성자 실행 시점에 일부 필드가 `null`이다.
2. **프록시가 아직 없다.** AOP 프록시는 초기화가 끝난 뒤에 씌워진다. 생성자 안에서 자기 메서드를 불러봐야 트랜잭션도 캐시도 걸리지 않는다.
3. **생성자는 객체를 만드는 일만 해야 한다.** DB 조회처럼 실패할 수 있고 오래 걸리는 작업을 생성자에 넣으면, 객체 생성이 실패하는 것인지 초기화가 실패하는 것인지 구분되지 않는다.
...
**객체 생성**과 **사용 준비**는 별개의 단계다. 이 분리가 생명주기 콜백이 존재하는 이유다.

> 비유: 신입사원이 입사(생성)한 날 바로 실무에 투입되지는 않는다. 사번을 받고 계정을 만들고 장비를 세팅하는 온보딩(초기화)을 거쳐야 일할 수 있고, 퇴사할 때는 계정을 회수하고 인수인계(소멸)를 한다.
>
> **비유의 한계**: 사람은 온보딩 중에도 스스로 판단하지만, Bean은 컨테이너가 정해둔 순서대로 수동적으로 호출될 뿐이다. 순서를 개발자가 임의로 바꿀 수 없다.
...
**AOP 프록시는 초기화가 다 끝난 뒤에 씌워진다.** 여기서 실무에서 종종 만나는 함정이 나온다.
...
**왜 문제인가**: `@PostConstruct`는 [5]번 단계에서 **원본 객체**에 대해 호출된다. 프록시는 [7]번에서야 만들어지므로, 초기화 콜백이 실행되는 시점에는 트랜잭션을 걸어줄 프록시가 아직 존재하지 않는다.
...
> 결론: **내가 작성한 클래스는 `@PostConstruct`, 남이 만든 클래스는 `@Bean`의 속성.** 인터페이스 방식은 Spring에 코드가 묶이고 메서드 이름도 바꿀 수 없어 선택할 이유가 거의 없다.
...
`@Bean`의 `destroyMethod`는 지정하지 않아도 `close`나 `shutdown`이라는 이름의 public 메서드가 있으면 Spring이 자동으로 찾아 호출한다. 자동 호출을 원하지 않으면 `destroyMethod = ""`로 명시적으로 꺼야 한다.
...
> 결론: **"이 Bean만 있으면 되는가, 다른 Bean도 필요한가"** 로 판단한다. 다른 Bean을 호출해야 하면 `@PostConstruct`가 아니라 Runner를 쓴다. `CommandLineRunner`와 `ApplicationRunner`의 차이는 인자 파싱 편의뿐이라 옵션 인자를 안 쓰면 아무거나 써도 된다.

---

## 5. Bean Scope 여섯 가지

스코프는 **"이 Bean의 인스턴스가 몇 개 만들어지고 언제까지 사나"** 를 정한다.
...
> 결론: 실무에서 쓰는 건 사실상 **singleton이 99%**, 요청별 컨텍스트가 필요할 때 `request`, 아주 가끔 `prototype`이다. `application`은 singleton과 실질적으로 구분되지 않아 쓸 일이 거의 없다.

### prototype만 소멸 콜백이 없는 이유

prototype Bean에 대해 컨테이너는 **생성하고 주입한 뒤 손을 뗀다.** 몇 개가 만들어졌는지, 누가 들고 있는지 추적하지 않으므로 언제 없애야 할지 알 수 없다. 그래서 `@PreDestroy`가 호출되지 않는다.
...
prototype Bean이 파일 핸들이나 커넥션 같은 자원을 들면, 받은 쪽이 직접 닫아야 한다. 이런 이유로 자원을 쥐는 객체는 prototype으로 만들지 않는 편이 낫다.
...
**왜 문제인가**: singleton Bean은 컨테이너 기동 시점에 만들어진다. 그런데 그때는 처리 중인 HTTP 요청이 없으므로 `request` 스코프 Bean을 만들 수 없다. 컨테이너는 "Scope 'request' is not active for the current thread"라는 메시지와 함께 기동에 실패한다. 예외 타입은 Spring 5.3 이상이면 `ScopeNotActiveException`, 그 이전 버전이면 `BeanCreationException`이다.
...
`proxyMode`를 주면 컨테이너는 진짜 객체 대신 **껍데기 프록시**를 주입한다. 이 프록시는 메서드가 호출되는 순간에야 "지금 이 스레드가 처리 중인 요청"에 해당하는 실제 객체를 찾아 위임한다. singleton은 프록시 하나만 계속 들고 있고, 그 뒤에서 요청마다 다른 실체가 갈아 끼워진다.

---

## 6. singleton에 prototype을 주입하면

이 조합은 컴파일도 되고 기동도 되지만 **의도한 대로 동작하지 않는다.** 조용히 틀리는 유형이라 면접 단골이다.
...
> 결론: **"새 인스턴스가 필요하다"는 사실을 코드에서 보이게 하고 싶으면 `ObjectProvider`**, 주입받는 쪽을 건드릴 수 없다면 스코프 프록시를 쓴다. 애초에 prototype이 정말 필요한지부터 의심해보는 것이 먼저다. 요청 단위 상태라면 `request` 스코프나 지역 변수가 더 적합한 경우가 많다.

---

## 7. @Component와 @Bean

둘 다 Bean을 등록하지만 쓰는 자리와 이유가 다르다.
...
평범한 자바라면 `b()`가 두 번 호출되어 서로 다른 인스턴스가 만들어져야 한다. 그러면 싱글톤이 깨진다. 그래서 Spring은 `@Configuration` 클래스를 CGLIB로 프록시해서, **이미 만든 Bean이 있으면 메서드를 실행하지 않고 컨테이너에 있는 것을 돌려준다.** `a()`와 `c()`가 받는 `B`는 같은 인스턴스다.

이 동작은 `@Configuration(proxyBeanMethods = false)`로 끌 수 있다. 프록시를 만들지 않으므로 기동이 조금 빨라지지만, 위처럼 `@Bean` 메서드끼리 호출하는 코드가 있으면 싱글톤이 깨진다. **`@Bean` 메서드가 서로를 호출하지 않는 설정 클래스에만** 쓸 수 있는 최적화다.

`@Component`에 `@Bean` 메서드를 넣어도 등록은 되지만 이 프록시가 적용되지 않는다. 설정 클래스에는 `@Configuration`을 쓰는 것이 안전하다.

---

## 8. 실무에서는

- **`@PostConstruct`는 무겁게 쓰지 않는다.** 여기서 예외가 나면 애플리케이션이 아예 기동하지 않는다. 외부 시스템 호출처럼 실패 가능성이 있는 작업은 Runner로 옮기거나 실패해도 기동은 되도록 예외를 잡아 로그만 남기는 편이 낫다.
- **소멸 콜백은 정상 종료에서만 보장된다.** `SIGKILL`이나 컨테이너 강제 종료에서는 실행되지 않는다. 그래서 쿠버네티스 환경에서는 `preStop` 훅과 Graceful Shutdown 설정을 함께 쓴다.
- **`@Scope("request")`는 로그 추적에 요긴하다.** 요청 ID, 호출자 정보 같은 요청 단위 컨텍스트를 담기 좋다. 다만 비동기 스레드로 넘어가면 스코프가 따라가지 않으므로, `@Async` 메서드 안에서 request 스코프 Bean에 접근하면 실패한다.
- **prototype을 실무에서 볼 일은 드물다.** 매번 새 객체가 필요하다면 대개는 그냥 `new`로 만드는 값 객체이지 Bean일 이유가 없다. Bean이어야 하는 이유(다른 Bean 주입이 필요하다 등)가 명확할 때만 쓴다.
...
- 꼬리 질문: "`@PostConstruct`에 `@Transactional`을 붙이면 어떻게 되나요?" → 초기화 콜백은 프록시가 씌워지기 전에 원본 객체에서 실행되므로 트랜잭션이 적용되지 않는다고 답한다.
...
- 꼬리 질문: "prototype 빈의 `@PreDestroy`는 언제 호출되나요?" → 호출되지 않는다. 컨테이너가 생성과 주입까지만 관여하고 이후 생명주기를 추적하지 않기 때문이다.
...
- 꼬리 질문: "`@Configuration` 대신 `@Component`에 `@Bean`을 써도 되나요?" → 등록은 되지만 CGLIB 프록시가 적용되지 않아 `@Bean` 메서드끼리 호출할 때 싱글톤이 깨진다.
...
Bean은 **생성 → 주입 → 초기화 콜백 → (프록시 생성) → 사용 → 소멸 콜백** 순서로 살아가며, 스코프는 그 Bean이 몇 개 만들어지고 얼마나 사는지를 정하는데 서로 다른 수명을 가진 Bean을 직접 주입하면 짧은 쪽이 긴 쪽에 갇혀버린다.
