...
> 학습 점수는 좋은데 실전에서 무너지는 이유를 학습 곡선으로 가려내고, 완화 기법을 왜 그 순서로 적용하는지 다룹니다. 검증 오차를 어떤 지표로 잴지는 이 문서의 범위 밖입니다.
...
- 없음 — 손실 함수와 가중치라는 단어의 뜻만 알면 이 문서부터 시작해도 됩니다
...
극단적인 예를 보겠습니다. 학습 데이터를 통째로 표에 저장해 두고, 입력이 들어오면 표에서 찾아 정답을 뱉는 모델이 있다고 해 보겠습니다.
...
이 모델의 학습 정확도는 100%입니다. 그런데 실전에서는 완전히 쓸모없습니다. 학습 오차를 최소화하라는 지시를 가장 충실히 따른 결과가 최악의 모델이라는 것 — 이것이 과적합의 본질입니다.
...
> 표에서는 **간격(generalization gap)** 열이 가장 중요합니다. 검증 오차가 높다는 것만으로는 A와 B가 갈리지 않습니다. 학습 오차까지 같이 봐야 "덜 배운 것"과 "잘못 배운 것"이 나뉩니다.

D는 무조건 버그는 아닙니다. 두 손실을 같은 조건에서 재지 않아서 생기는 경우가 흔합니다. Dropout과 증강은 학습 중에만 켜지므로 학습 손실이 실제보다 부풀려지고, 학습 손실은 에폭 내내 갱신되는 가중치의 평균인 반면 검증 손실은 에폭 끝의 가중치로 한 번에 잽니다.

이 두 가지만으로도 역전이 나올 수 있습니다. 그래도 격차가 크다면 그때는 분할이나 지표 계산 코드를 의심합니다.
...
곡선이 끝에서 아직 수렴하지 않고 좁혀지는 중이라면, 데이터 수집이 가장 효과적인 투자입니다. 이미 높은 값에서 붙어 버렸다면 데이터를 열 배 모아도 성능은 거의 그대로입니다. 순서는 곡선이 먼저입니다. 데이터 라벨링에 예산을 쓰는 것은 그다음입니다.
...
모델 복잡도를 키우면 편향은 줄고 분산은 늡니다. 둘의 합이 최소가 되는 지점을 찾는 것이 튜닝의 목표입니다.
...
위쪽 곡선이 총 오차, 아래쪽 두 곡선이 그것을 이루는 두 성분입니다. 두 성분은 각각 단조롭게 움직이는데, 합만 U자가 됩니다. 이 점이 핵심입니다. 참고로 최적점은 두 곡선이 **교차하는 지점이 아닙니다**. 편향²이 줄어드는 속도와 분산이 늘어나는 속도가 같아지는 지점이며, 교차점과 우연히 가까울 뿐입니다.

> 주의: 이 U자 곡선은 고전적인 통계 학습 이론의 그림입니다. 파라미터가 데이터 수보다 훨씬 많은 대형 신경망에서는 U자를 지나 오차가 **다시** 내려가는 현상(double descent)이 보고되어 있습니다. 이 곡선을 모든 모델에 그대로 적용하면 안 됩니다. 면접에서는 "고전적 관점에서는 U자이고, 과매개변수화된 딥러닝에서는 예외가 관찰된다" 정도로 답하면 충분합니다.
...
정규화의 아이디어는 단순합니다. "오차만 줄이지 말고, 가중치도 작게 유지해라"는 제약을 손실 함수에 직접 써넣습니다.
...
가중치가 크다는 것은 입력이 조금만 흔들려도 출력이 크게 튄다는 뜻입니다. 노이즈를 따라 그리려면 가중치가 커져야 합니다. 그러니 가중치 크기에 벌금을 매기면 노이즈를 따라 그리는 일 자체가 비싸집니다.
...
LogisticRegression(l1_ratio=1, solver="liblinear", C=0.1)   # 1.8+ 방식. 1.7 이하는 penalty="l1"
```

`Ridge`/`Lasso`의 `alpha`와 `LogisticRegression`/`SVC`의 `C`는 서로 역수 관계입니다. 같은 라이브러리 안에서도 방향이 반대입니다. 그래서 튜닝 범위를 반대로 잡는 실수가 자주 나옵니다. 그리드를 짜기 전에 문서에서 방향을 확인하는 습관이 필요합니다.
...
한 가지 주의점이 있습니다. SGD에서 가중치 감쇠(weight decay)는 손실에 L2 항을 더한 것과 수학적으로 같지만, Adam처럼 파라미터별로 학습률을 조정하는 옵티마이저에서는 그렇지 않습니다. 이 불일치를 바로잡아 가중치 감쇠를 갱신식에서 분리한 것이 `AdamW`입니다. 그래서 트랜스포머 계열 학습에서는 Adam 대신 AdamW를 기본으로 씁니다.
...
목적은 **학습을 빠르고 안정적으로 만드는 것**입니다. 원 논문이 내세운 설명은 층을 통과한 값의 분포가 학습 중에 계속 이동해 뒷 층이 매번 새 분포에 적응해야 한다는 것(internal covariate shift)이었습니다. 다만 이 설명이 실제 작동 기전인지를 두고 이후 반론이 나왔습니다. 손실 표면을 매끄럽게 만들어 큰 학습률을 견디게 해준다는 쪽의 설명도 제시되어 있습니다. 면접에서는 "원 논문의 동기는 분포 이동 완화였고 기전을 두고 논쟁이 있다"까지 말하면 충분합니다. 어느 쪽 설명을 택하든 **관측되는 효과는 같습니다.** 더 큰 학습률을 쓸 수 있고 학습이 빨리 안정됩니다.

과적합 완화 효과는 **부수적으로** 따라옵니다. 미니배치마다 평균·분산이 조금씩 달라 일종의 노이즈가 주입되기 때문입니다. 그래서 배치 크기가 아주 작으면 통계가 불안정해져 오히려 해로울 수도 있습니다. 이럴 때는 Layer Normalization 같은 배치에 의존하지 않는 대안을 씁니다.
...
데이터 누수(Data Leakage)는 **평가 시점에는 알 수 없는 정보가 학습 과정에 새어 들어가는 것**입니다. 과적합은 검증 오차가 나빠지므로 눈에 띕니다. 그런데 누수는 검증 오차까지 같이 좋아집니다. 지표가 전부 훌륭한데 배포하면 무너지는, 진단이 가장 어려운 실패입니다.
...
스케일링뿐 아니라 결측치 대치의 평균값, 인코딩 사전, 특성 선택 기준까지 전부 같은 원칙을 따릅니다. 손으로 관리하면 언젠가 반드시 틀립니다. 파이프라인으로 묶어 교차 검증에 통째로 넘기는 방식이 정석입니다.
...
이탈 예측 모델에 `해지_사유_코드` 같은 컬럼이 들어가 있는 경우입니다. 이 값은 이탈이 확정된 뒤에야 생깁니다. 따라서 예측 시점에는 존재하지 않습니다. 모델은 이 컬럼 하나로 거의 완벽한 성능을 내고, 실전에서는 항상 비어 있는 값을 받습니다.
...
한 환자에게서 나온 여러 장의 사진, 같은 사용자의 여러 세션, 중복 크롤링된 문서가 무작위 분할로 양쪽에 나뉘어 들어가는 경우입니다. 모델은 개체의 고유한 특징을 외워서 맞히지만, 처음 보는 개체에는 그 능력이 통하지 않습니다. 개체 단위로 분할해야 합니다. `GroupKFold` 같은 그룹 인지 분할기를 씁니다.
...
| "규제를 세게 걸수록 일반화가 잘 된다" | λ가 과하면 모델이 데이터를 표현조차 못 해 과소적합으로 넘어간다 | λ는 검증 성능이 최고가 되는 값을 탐색해서 정합니다. 방향이 아니라 지점의 문제입니다 |
| "BatchNorm은 정규화(Regularization) 기법이다" | 목적은 층 입력 분포 안정화이며 과적합 완화는 부산물 | L1/L2·Dropout과 목적이 다릅니다. BatchNorm을 넣었다고 규제를 뺄 근거는 안 됩니다 |
| "`LogisticRegression(C=100)`이 규제가 세다" | `C`는 λ의 역수라 클수록 규제가 약하다 | `Ridge`의 `alpha`와 방향이 반대입니다. 튜닝 범위를 잡기 전에 방향부터 확인합니다 |
...
과적합은 학습 오차를 줄이라는 지시와 일반화하라는 목표가 어긋나서 생기는 문제입니다. 정규화·조기 종료·증강은 그 간격을 좁히는 도구입니다. 그 모든 노력을 무의미하게 만드는 것이 검증 지표까지 속이는 데이터 누수입니다.
