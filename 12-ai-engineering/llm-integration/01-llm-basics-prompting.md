...
> LLM은 "다음 토큰을 예측하는 모델"입니다. 이 한 문장에서 토큰 비용, 샘플링 파라미터, 프롬프트 기법이 어떻게 전부 파생되는지 설명합니다.
...
- 없음 — LLM API를 한 번도 호출해본 적 없어도 이 문서부터 시작해도 됩니다
- 확률 분포와 softmax를 알고 있으면 4장이 편하지만, 몰라도 그림으로 따라올 수 있습니다
...
LLM이 하는 일은 놀랄 만큼 단순합니다. 지금까지의 텍스트를 보고 **다음에 올 토큰의 확률 분포를 계산**한 뒤, 거기서 하나를 뽑습니다. 그리고 뽑은 토큰을 입력 끝에 붙여 같은 일을 반복합니다. 이걸 자기회귀 생성이라고 부릅니다.
...
**1. 사실을 검증하는 단계가 없습니다.** 모델은 "서울이 맞는지" 확인하지 않습니다. 학습 데이터에서 그 자리에 자주 왔던 토큰을 고를 뿐입니다. 그래서 존재하지 않는 API 이름도, 그 자리에 오기 그럴듯한 형태라면 높은 확률을 받습니다. 환각은 버그가 아니라 이 구조에서 나오는 특성입니다. 자세한 내용은 [02-hallucination-integration.md](./02-hallucination-integration.md)에서 다룹니다.

**2. 같은 입력에 다른 출력이 나옵니다.** 확률 분포에서 "뽑는" 과정이 있기 때문입니다. 이 뽑는 방식을 조절하는 손잡이가 Temperature와 Top-p입니다.

**3. 출력이 길어질수록 어긋날 여지가 커집니다.** 앞에서 뽑은 토큰이 뒤의 조건이 되므로, 중간에 한 번 이상한 방향으로 틀면 그 뒤는 그 틀린 전제 위에서 계속 이어집니다. 그래서 답변이 길수록 뒷부분이 흔들립니다.

**4. 입력이 출력을 지배합니다.** 모델 가중치는 고정이므로 우리가 바꿀 수 있는 건 조건, 즉 프롬프트뿐입니다. **프롬프트 엔지니어링이 성립하는 근거가 바로 이것**입니다. 프롬프트를 바꾸는 일은 확률 분포의 조건을 바꾸는 일입니다. 모델에게 부탁을 하는 것과는 다릅니다.

### 비유: 극단적으로 똑똑한 자동완성

휴대폰 키보드의 자동완성을 생각해봅시다. "오늘 저녁에"까지 치면 "뭐"를 제안합니다. 내가 저녁에 뭘 하는지 알아서가 아니라, 그 자리에 그 단어가 자주 왔기 때문입니다. LLM은 이 자동완성을 인터넷 규모의 텍스트로 훈련시켜, 문서 전체 수준의 맥락을 조건으로 삼게 만든 것입니다.
...
토큰은 모델이 텍스트를 다루는 최소 단위입니다. 대부분 BPE(Byte-Pair Encoding) 계열 서브워드 토크나이저로 만들어지며, 자주 등장하는 문자열은 통째로 한 토큰이 되고 드문 문자열은 잘게 쪼개집니다.
...
영어는 대략 1 토큰이 4글자 안팎입니다. 한국어는 학습 데이터에서 영어보다 비중이 작아서 서브워드가 잘게 쪼개지는 경우가 많습니다. **같은 뜻의 문장이라도 영어보다 토큰을 더 씁니다.** 그래서 한국어 서비스 비용을 영어 기준 수치로 추정하면 어긋납니다.
...
**`finish_reason`은 반드시 확인해야 합니다.** 잘린 출력을 정상 출력으로 착각하고 파싱하면 원인을 찾기 어려운 버그가 됩니다.
...
각 기법은 **서로 다른 문제**를 풉니다. 유행을 따라 고르지 말고 무엇을 푸는지 알고 써야 합니다.
...
다만 역할이 **지식을 만들어주지는 않는다**는 사실을 잊으면 안 됩니다. "당신은 우리 회사 사내 규정 전문가입니다"라고 써도 모델은 그 규정을 모릅니다. 이건 RAG로 풀 문제입니다.
...
- **예시는 매 호출마다 입력 토큰으로 지불됩니다.** 개수를 늘릴수록 비용과 지연이 함께 늡니다.
...
재시도할 때는 **무엇이 틀렸는지 알려줘야 합니다.** 같은 프롬프트로 그냥 다시 부르면 같은 실패를 반복할 가능성이 높습니다.
...
- **프롬프트는 코드보다 설정에 가깝습니다.** 별도 파일이나 프롬프트 관리 도구에 두고 버전을 매긴 뒤, 바꿀 때마다 고정된 평가셋(입력 30~100건 + 기대 결과)을 돌려 회귀를 확인합니다. 이 과정이 없으면 "프롬프트를 고쳤더니 다른 케이스가 깨졌다"를 영원히 반복합니다.
...
A. LLM은 다음 토큰의 확률 분포를 계산해 하나를 뽑는 일을 반복하는 모델입니다. 이 과정 어디에도 생성한 내용이 사실인지 검증하는 단계가 없습니다. 학습 데이터에서 그 자리에 오기 그럴듯했던 토큰에 높은 확률을 줄 뿐입니다. 존재하지 않는 API 이름이라도 형태가 그럴듯하면 자신 있게 생성됩니다. 그래서 환각은 고쳐야 할 버그가 아니라 구조적 특성이고, 없앨 수는 없으니 근거를 주입하고 출력을 검증하는 식으로 영향을 줄이는 접근을 씁니다.
...
A. Temperature는 softmax 이전 로짓을 T로 나눠 확률 분포 자체의 뾰족함을 바꿉니다. 낮추면 높은 확률 토큰에 집중되고 올리면 평탄해집니다. Top-p는 그렇게 만들어진 분포에서 확률 높은 순으로 누적해 p에 도달할 때까지만 후보로 남기고 나머지 꼬리를 잘라냅니다. 분포를 다시 그리느냐 후보 집합을 줄이느냐의 차이라 역할이 겹칩니다. 그래서 실무에서는 보통 하나만 조절합니다.
...
A. 먼저 입력 토큰이 어디서 나오는지 분해해 봅니다. 대화형이면 매 호출마다 이력 전체를 다시 지불하고 있을 가능성이 높으니 이력 압축이나 요약 전략을 검토합니다. RAG면 Top-K와 청크 크기가 입력의 대부분이므로, K를 줄였을 때 품질이 실제로 떨어지는지 평가셋으로 측정합니다. 그다음은 모델 라우팅입니다. 분류나 라우팅 같은 단순 단계는 작은 모델로 내리고, 동일 입력에 대한 응답 캐싱과 프롬프트 캐싱도 함께 적용합니다.
...
A. 출력 형식이 까다롭거나, 분류 기준의 경계를 말로 설명하기 어려울 때 씁니다. 특히 헷갈리는 경계 사례를 예시로 보여주면 지시문 몇 줄보다 효과가 큽니다. 반대로 모델이 이미 잘하는 일반적인 작업이라면 Zero-shot으로 둡니다. 예시는 매 호출마다 입력 토큰으로 지불되니 비용만 늘어납니다. 예시를 넣을 때는 라벨 분포가 한쪽으로 쏠리지 않게 구성합니다.
