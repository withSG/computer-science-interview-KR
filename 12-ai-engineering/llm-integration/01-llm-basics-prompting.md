...
영어는 대략 1 토큰이 4글자 안팎입니다. 한국어는 학습 데이터에서 영어보다 비중이 작아 서브워드가 잘게 쪼개지는 경우가 많고 **같은 뜻의 문장이라도 영어보다 토큰을 더 씁니다.** 그래서 한국어 서비스 비용을 영어 기준 수치로 추정하면 어긋납니다.
...
A. 먼저 입력 토큰이 어디서 나오는지 분해합니다. 대화형이면 매 호출마다 이력 전체를 다시 지불하고 있을 가능성이 높아 이력 압축이나 요약 전략을 검토하고, RAG면 Top-K와 청크 크기가 입력의 대부분이므로 K를 줄였을 때 품질이 실제로 떨어지는지 평가셋으로 측정합니다. 그다음은 모델 라우팅입니다. 분류나 라우팅 같은 단순 단계를 작은 모델로 내리고 동일 입력에 대한 응답 캐싱과 프롬프트 캐싱을 함께 적용합니다.
