...
> 이 문서는 환각을 없애는 방법이 아니라, 왜 없앨 수 없는 것으로 다뤄야 하는지를 짚습니다. 그 전제 위에서 LLM을 실제 서비스에 붙일 때 무엇을 설계해야 하는지도 설명합니다.
...
- 데모에서는 맞던 답이 실제 사용자 질문에서는 존재하지 않는 정책을 지어냅니다
- API가 가끔 30초 넘게 응답하지 않아 요청 스레드가 쌓입니다
- 429(rate limit)를 받고 곧바로 재시도했더니 더 심하게 막힙니다
- 월말에 청구서를 보고 나서야 특정 사용자가 API를 반복 호출하고 있었음을 압니다
- 프롬프트를 한 줄 고쳤더니 다른 케이스가 조용히 깨졌는데, 배포 2주 뒤 CS 문의로 알게 됩니다
...
환각(Hallucination)은 모델이 **사실이 아닌 내용을 사실인 것처럼 자연스럽게 생성**하는 현상을 가리키는 말입니다. 무게는 자연스럽게라는 말에 실려 있습니다. 모델은 헛소리를 티 나게 내놓지 않습니다. 맞는 답과 구별이 안 되는 형태로 내놓으니 문제입니다.
...
업무를 물으면 무엇이든 막힘없이 답하는 신입이 있다고 해 봅시다. 아는 것은 정확히 답하고, 모르는 것도 **비슷한 사례에서 유추해 그럴듯하게** 답합니다. 두 경우의 말투와 확신도가 똑같아서 듣는 사람은 어느 쪽인지 구별하지 못합니다. 틀리지 마라고 지적해 봐야 소용이 없습니다. 이 신입에게 필요한 조치는 답하기 전에 자료를 찾아보게 하고 근거를 함께 말하게 하는 절차입니다.

> **비유의 한계**: 신입은 자기가 확신이 없다는 사실을 적어도 스스로는 알고, 물어보면 말해 줍니다. 모델은 다릅니다. 사실을 회상하는 것과 지어내는 것이 똑같이 "다음 토큰 확률 계산" 한 가지 연산으로 처리되므로 자기 확신도를 답변에 제대로 반영하지 못합니다. 그래서 "모르면 모른다고 하라"는 지시가 사람에게 하는 것만큼 잘 듣지 않습니다.
...
더 나아가 별도 LLM 호출로 답변의 각 문장이 컨텍스트로 뒷받침되는지 판정하는 방식(LLM-as-a-Judge)도 있습니다. 정확도는 올라갑니다. 다만 호출이 두 배가 되고, 판정자 자신도 환각합니다. 전체 트래픽 대신 샘플링해서 품질 모니터링 용도로 쓰는 편이 현실적입니다.
...
> 표 요약: 어느 하나로 해결되지 않습니다. 1층(근거)을 기본으로 깔고, 2층(제어)은 비용이 거의 없으니 항상 적용합니다. 3층(검증)은 실수 비용이 큰 경로에만 붙이는 것이 일반적인 배치입니다.
...
A. 일반 외부 API와 같은 원칙에 LLM 특유의 항목을 더합니다. 타임아웃을 반드시 걸고, 429와 5xx 같은 일시적 오류에만 지수 백오프에 지터를 섞어 재시도하고, 400처럼 재시도해도 성공하지 않을 오류는 즉시 실패시킵니다. 반복 실패 시에는 서킷 브레이커로 차단하고 캐시된 답변이나 축소 모델로 폴백합니다. LLM 특유의 항목은 둘입니다. 컨텍스트 초과 오류에 대비해 입력을 잘라내거나 요약하는 경로가 하나이고, `finish_reason`이 length면 응답이 중간에 잘린 것이니 정상 응답으로 착각하지 않게 검사하는 것이 다른 하나입니다.
...
환각은 다음 토큰 예측 구조에서 나오는 특성이라 제거 대상이 아니라 관리 대상입니다. 프로덕션 통합이란 그 불확실한 외부 의존성 앞뒤에 근거 주입·출력 검증·타임아웃·상한·평가를 배치하는 일입니다.
