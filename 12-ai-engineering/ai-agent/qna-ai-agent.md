...
### 핵심 답변
LangGraph는 **에이전트의 흐름을 그래프(Graph)로 표현**하는 프레임워크입니다. 노드(작업)와 엣지(연결)로 워크플로우를 정의하고, 상태(State)를 노드 간에 전달합니다. 순환(cycle)을 표현할 수 있습니다. 그래서 에이전트의 반복 루프에 잘 맞습니다.
...
### 핵심 답변
조건부 엣지는 **현재 상태를 보고 다음에 실행할 노드를 동적으로 결정**하는 라우팅입니다. `add_conditional_edges`에 라우팅 함수를 넘깁니다. 그 함수의 반환값에 따라 다음 노드가 선택됩니다.
...
### 핵심 답변
에이전트는 "model ↔ tools" 같은 순환 구조입니다. 그래서 종료 조건을 충족하지 못하면 무한히 반복할 수 있습니다. LangGraph는 `recursion_limit`으로 **최대 실행(노드 전이) 횟수를 제한**해서 이를 막습니다.
...
복잡한 작업은 한 번의 프롬프트로 해결하기 어렵습니다. 그래서 작업을 단계로 나누고 각 단계를 노드로 구성하는 플로우 엔지니어링이 중요해졌습니다.
...
### 핵심 답변
AI 하네스는 **LLM(모델)을 실제로 동작하는 에이전트로 만들어주는 주변 소프트웨어 골격**입니다. 모델 자체는 텍스트를 생성할 뿐입니다. 에이전트 루프·도구 실행·컨텍스트 관리·프롬프트 구성·안전장치는 하네스가 담당합니다. 흔히 **"모델은 지능을, 하네스는 행위 능력(agency)을 제공한다"고** 표현합니다.
...
### 왜 중요한가
모델 성능이 같아도 하네스가 **무엇을 컨텍스트에 넣고(검색·메모리), 어떤 도구를 주고 언제 멈추는지를** 잘 설계하면 결과 품질이 크게 향상됩니다. 최근에는 "모델만큼 하네스가 중요하다"는 인식이 자리잡았습니다.
