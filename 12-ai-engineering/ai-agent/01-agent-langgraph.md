...
> 단발 LLM 호출로는 왜 안 되는 작업이 있는지, 그 작업을 루프로 푸는 에이전트가 어떻게 동작하는지, 그리고 그 루프가 폭주하지 않게 만드는 장치들을 설명할 수 있게 된다.
...
- [../rag-pipeline/01-rag-pipeline.md](../rag-pipeline/01-rag-pipeline.md) — 에이전트가 쓰는 대표적인 도구가 검색이다

---

## 1. 왜 필요한가

"이번 달 매출이 지난달보다 떨어진 이유를 정리해줘"라는 요청을 생각해보자. 이 한 문장을 처리하려면 이런 일들이 필요하다.
...
문제는 **3번 이후가 2번의 결과에 따라 달라진다**는 것이다. 전체 매출이 그대로인데 특정 카테고리만 빠졌다면 볼 곳이 다르고, 전 카테고리가 고르게 빠졌다면 또 다르다. 미리 순서를 다 적어둘 수가 없다.

여기서 두 가지가 필요해진다. **모델이 외부 데이터를 가져올 수 있어야 하고(도구), 가져온 결과를 보고 다음 행동을 스스로 정할 수 있어야 한다(루프).** 이 둘을 갖춘 것이 에이전트다.
...
> 표 요약: **워크플로우와 에이전트의 경계는 "경로를 누가 정하는가"다.** 순서를 개발자가 코드로 적을 수 있다면 워크플로우가 낫다. 훨씬 싸고, 빠르고, 무엇보다 디버깅이 가능하다. 에이전트는 그게 불가능할 때만 쓴다.

도입 전에 확인할 네 가지를 덧붙인다.
...
특히 4번이 관문이다. 되돌릴 수 없는 행동(결제, 삭제, 메일 발송)을 자율적으로 맡기려면 승인 게이트가 반드시 함께 와야 한다.
...
도구는 이름, 설명, 파라미터 스키마로 정의한다. 이 설명 전체가 매 호출마다 모델 컨텍스트에 들어가므로, **도구 설명은 문서가 아니라 프롬프트다.**
...
**왜 문제인가**: 무엇을 검색하는 도구인지(사내 문서? 웹? 상품?), 언제 써야 하는지, `q`에 무엇을 넣어야 하는지가 하나도 없다. 모델은 추측할 수밖에 없고, 그래서 엉뚱한 타이밍에 부르거나 잘못된 인자를 넣는다. **에이전트가 도구를 잘못 쓰는 원인의 대부분은 모델이 아니라 도구 설명에 있다.**
...
- **도구 개수를 줄인다.** 비슷한 도구가 여러 개면 모델이 헷갈린다. 20개짜리 도구 목록보다 잘 정리된 6개가 낫다
- **결과를 짧게 돌려준다.** 도구 결과는 그대로 컨텍스트에 쌓인다. DB 조회 결과 500행을 그대로 넣으면 컨텍스트가 몇 턴 만에 터진다. 요약하거나 상한을 둔다
- **에러도 모델이 읽을 수 있게 돌려준다.** 예외를 던져 루프를 끊는 대신 `{"error": "날짜 형식이 잘못됨. YYYY-MM-DD로 다시 시도"}`를 반환하면 모델이 스스로 고쳐 재시도한다
...
에이전트 루프를 `while`문으로 직접 짜면 처음엔 간단하다. 그런데 분기가 생기고, 중간에 사람 승인이 들어가고, 실패 시 재시도가 붙으면 금세 읽을 수 없는 코드가 된다. 무엇보다 **중간에 멈췄다가 나중에 재개하는 것**이 거의 불가능해진다.

LangGraph는 이 흐름을 **상태(State)를 들고 노드 사이를 이동하는 그래프**로 표현한다. 일반적인 체인이 DAG(순환 없는 그래프)라면, LangGraph는 **순환을 허용**한다. 에이전트 루프가 곧 순환이기 때문이다.
...
> **비유의 한계**: 실제 담당자는 파일 전체를 마음대로 고칠 수 있지만, 노드는 **갱신할 키만 담은 부분 딕셔너리**를 반환하고 병합은 리듀서가 정한 규칙대로 이루어진다.

### 리듀서가 핵심이다

State를 그냥 TypedDict로 정의하면 노드가 반환한 값이 **기존 값을 덮어쓴다.** 메시지 이력처럼 쌓여야 하는 데이터에는 이게 치명적이다.
...
`Annotated[list, add_messages]`의 두 번째 인자가 리듀서다. 노드가 `{"messages": [새 메시지]}`를 반환하면 기존 리스트에 **덧붙는다.** 이걸 빼먹으면 매 노드마다 대화 이력이 통째로 날아가고, 에이전트가 방금 한 일을 기억하지 못해 같은 도구를 무한히 반복하는 증상이 나온다.
...
순환을 허용한 순간 폭주 가능성이 생긴다. 실제로 이런 식으로 터진다.

- 도구가 계속 에러를 반환하는데 모델이 같은 인자로 재시도만 반복한다
- 검색 결과가 만족스럽지 않다며 조금씩 다른 쿼리로 영원히 검색한다
- 라우팅 함수의 조건이 잘못돼 `END`로 가는 경로가 실제로는 닿지 않는다

문제는 **이게 조용히 돈다**는 것이다. 에러도 안 나고, 로그도 정상으로 보이고, 청구서만 올라간다. 그래서 한 겹이 아니라 여러 겹으로 막는다.
...
노드 전이 횟수의 절대 상한이다. **최후의 안전망이지 정상 종료 수단이 아니다.** 여기에 걸렸다는 것 자체가 설계에 문제가 있다는 신호이므로 알림을 걸어두는 편이 좋다. ReAct 루프는 한 사이클에 노드를 2개 지나므로, 도구 호출 7회를 허용하려면 대략 그 두 배 이상을 잡아야 한다.
...
`recursion_limit`과 다른 점은 **예외가 아니라 정상 경로로 빠진다**는 것이다. `finalize` 노드에서 "지금까지 알아낸 것만으로 답하라"고 시키면, 사용자는 최소한 부분적인 답이라도 받는다. 이때 `finalize`를 노드로 등록하고 `add_conditional_edges`의 경로 맵에도 `{"tools": "tools", "finalize": "finalize", END: END}`처럼 함께 넣어야 한다. 라우팅 함수가 경로 맵에 없는 이름을 반환하면 그 자리에서 에러가 난다.
...
무한 루프의 상당수가 **똑같은 도구를 똑같은 인자로 반복**하는 형태다. 호출 서명을 기록해두고 중복이면 결과 대신 안내를 돌려주면, 모델이 경로를 바꾼다.
...
가장 싸고 가장 자주 빠지는 층이다. **모델에게 "언제 멈춰야 하는지"를 아무도 안 알려준 채 멈추지 않는다고 탓하는 경우가 많다.**

---

## 6. Human-in-the-Loop

되돌리기 어려운 행동 앞에는 사람이 서야 한다. LangGraph는 **체크포인터**로 상태를 저장해두고 실행 중간에 멈췄다가 재개할 수 있다.
...
- `thread_id`가 어떤 실행을 재개할지 식별하는 키다. 이게 없으면 재개할 대상을 찾을 수 없다
- `MemorySaver`는 프로세스 메모리에 저장하므로 **재시작하면 사라진다.** 실제 서비스에서는 DB 기반 체크포인터를 쓴다
- 승인 게이트를 너무 많이 두면 사람이 습관적으로 전부 승인하게 된다. **정말 되돌릴 수 없는 지점에만** 둔다
...
- **결과 평가**: 최종 답변이 맞았는가. 사용자가 체감하는 품질이다
- **궤적 평가**: 필요한 도구를 불렀는가, 불필요한 호출이 없었는가, 몇 번 만에 끝냈는가

결과만 보면 "운 좋게 맞은 경우"와 "제대로 푼 경우"가 구별되지 않는다. 반대로 궤적만 보면 사람이 예상한 것과 다른 더 나은 경로를 벌점 처리하게 된다. **둘을 같이 봐야 한다.**

---

## 8. 실무에서는

- **에이전트로 시작하지 않는다.** 워크플로우로 만들어 돌려보고, 코드로 못 적는 분기가 계속 나올 때 그 부분만 에이전트로 바꾼다. 전체를 자율에 맡기는 것보다 "대부분은 고정 흐름, 특정 구간만 자율"이 훨씬 안정적이다.
- **도구 권한을 최소화한다.** 에이전트가 SQL을 실행할 수 있다면 읽기 전용 계정을 준다. 프롬프트 방어는 뚫릴 수 있고, 권한은 뚫리지 않는다.
- **비용 상한을 요청 단위로 건다.** 사용자당 일일 요청 수, 요청당 최대 LLM 호출 수, 요청당 최대 토큰을 모두 정한다. 에이전트는 호출 횟수가 미리 정해지지 않으므로 단발 호출보다 상한이 더 중요하다.
- **체크포인터를 영속 저장소로 둔다.** 메모리 기반은 데모용이다. 사람 승인이 몇 시간 뒤에 올 수도 있는데 그 사이 배포가 일어나면 작업이 통째로 사라진다.
...
A. 경로를 누가 정하느냐입니다. 워크플로우는 단계와 분기를 개발자가 코드로 정해두고 각 단계에서 LLM을 쓰는 것이고, 에이전트는 다음에 무엇을 할지를 모델이 런타임에 결정합니다. 그래서 순서를 코드로 적을 수 있다면 워크플로우가 낫습니다. 훨씬 싸고 빠르고, 무엇보다 어디서 잘못됐는지 추적이 가능합니다. 에이전트는 조사나 디버깅처럼 이전 단계 결과에 따라 다음에 볼 것이 달라져서 미리 적을 수 없을 때만 씁니다.
- 꼬리 질문: "그럼 에이전트를 도입할지 어떻게 판단하나요?" → 절차를 정말 미리 정의할 수 없는지, 비용과 지연이 몇 배 늘어도 남는 이득이 있는지, 모델이 이 도구로 실제 해낼 수 있는지, 그리고 틀렸을 때 검출하고 되돌릴 수 있는지 네 가지를 본다. 특히 마지막이 안 되면 도입하지 않는다.

**Q. ReAct 루프를 설명해주세요.**
A. 생각(Thought), 행동(Action), 관찰(Observation)을 목표 달성까지 반복하는 패턴입니다. 모델이 지금 무엇이 필요한지 판단하고 도구 호출을 요청하면, 하네스가 실제로 그 도구를 실행해 결과를 컨텍스트에 붙여 모델을 다시 호출합니다. 여기서 중요한 건 모델이 도구를 실행하는 게 아니라 구조화된 호출 요청을 출력할 뿐이고, 실행과 결과 주입은 우리 코드가 한다는 점입니다. 그래서 도구 설명이 곧 프롬프트이고, 에이전트가 도구를 잘못 쓰는 원인은 대개 모델이 아니라 설명이 부실한 데 있습니다.
- 꼬리 질문: "도구 실행이 실패하면 어떻게 하나요?" → 예외를 던져 루프를 끊기보다 에러 내용을 결과 형태로 돌려주는 편이 낫다. 모델이 인자를 고쳐 재시도하거나 다른 도구로 우회할 수 있다고 답한다.
...
- 꼬리 질문: "그럼 모든 필드를 누적으로 하면 되나요?" → 아니다. 현재 단계 수나 최신 판단 결과처럼 마지막 값만 의미 있는 필드는 덮어쓰기가 맞다. 누적하면 컨텍스트만 불어난다.

**Q. 에이전트가 무한 루프에 빠지는 것을 어떻게 막나요?**
A. 한 겹으로는 부족해서 여러 겹을 둡니다. 프롬프트에 언제 멈춰야 하는지 종료 조건을 명시하는 것이 가장 싸고 먼저 할 일이고, State에 호출 횟수 예산을 두고 초과하면 예외가 아니라 마무리 노드로 정상 종료시킵니다. 그래도 안 되면 같은 도구를 같은 인자로 반복하는지 서명을 기록해 감지하고, 마지막으로 recursion_limit을 하드 리밋으로 겁니다. recursion_limit에 걸렸다는 건 앞의 세 겹이 다 실패했다는 뜻이라 알림을 걸어둡니다.
- 꼬리 질문: "무한 루프가 왜 위험한가요?" → 에러가 나지 않고 조용히 돌기 때문이다. 로그는 정상으로 보이는데 LLM 호출과 비용만 계속 쌓여서, 청구서를 보고 나서야 알게 되는 경우가 많다.
...
에이전트는 도구와 루프를 얻는 대신 예측 가능성을 내주는 거래이고, 잘 만든다는 것은 **가능한 한 워크플로우로 밀어두고, 남은 자율 구간에는 종료 조건과 예산과 궤적 로그를 촘촘히 채워두는 일**이다.
