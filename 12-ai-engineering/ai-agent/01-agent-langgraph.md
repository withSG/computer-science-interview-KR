...
> 단발 LLM 호출로는 왜 안 되는 작업이 있는지, 그 작업을 루프로 푸는 에이전트가 어떻게 동작하는지, 그리고 그 루프가 폭주하지 않게 만드는 장치들을 설명할 수 있게 됩니다.
...
"이번 달 매출이 지난달보다 떨어진 이유를 정리해줘"라는 요청을 생각해보자. 이 한 문장을 처리하려면 이런 일들이 필요합니다.
...
문제는 **3번 이후가 2번의 결과에 따라 달라진다**는 것입니다. 전체 매출이 그대로인데 특정 카테고리만 빠졌다면 볼 곳이 다르고, 전 카테고리가 고르게 빠졌다면 또 다릅니다. 미리 순서를 다 적어둘 수가 없습니다.

여기서 두 가지가 필요해집니다. **모델이 외부 데이터를 가져올 수 있어야 하고(도구), 가져온 결과를 보고 다음 행동을 스스로 정할 수 있어야 한다(루프).** 이 둘을 갖춘 것이 에이전트입니다.
...
> 표 요약: **워크플로우와 에이전트의 경계는 "경로를 누가 정하는가"다.** 순서를 개발자가 코드로 적을 수 있다면 워크플로우가 낫습니다. 훨씬 싸고, 빠르고, 무엇보다 디버깅이 가능합니다. 에이전트는 그게 불가능할 때만 씁니다.
...
ReAct(Reasoning + Acting)는 에이전트의 기본 패턴입니다. **생각하고, 행동하고, 결과를 본다**를 목표 달성까지 반복합니다.
...
여기서 중요한 오해 하나. **모델이 도구를 실행하는 게 아니다.** 모델은 "이 도구를 이런 인자로 부르고 싶다"는 구조화된 출력을 낼 뿐이고, 실제 실행은 우리 코드가 합니다. 그리고 그 결과를 다시 컨텍스트에 넣어 모델을 또 호출합니다. 이 실행과 주입을 담당하는 코드 골격을 **하네스(harness)**라고 부릅니다.

### 도구 설명이 곧 프롬프트다

도구는 이름, 설명, 파라미터 스키마로 정의합니다. 이 설명 전체가 매 호출마다 모델 컨텍스트에 들어가므로, **도구 설명은 문서가 아니라 프롬프트다.**
...
**왜 문제인가**: 무엇을 검색하는 도구인지(사내 문서? 웹? 상품?), 언제 써야 하는지, `q`에 무엇을 넣어야 하는지가 하나도 없습니다. 모델은 추측할 수밖에 없고, 그래서 엉뚱한 타이밍에 부르거나 잘못된 인자를 넣습니다. **에이전트가 도구를 잘못 쓰는 원인의 대부분은 모델이 아니라 도구 설명에 있다.**
...
- **도구 개수를 줄인다.** 비슷한 도구가 여러 개면 모델이 헷갈립니다. 20개짜리 도구 목록보다 잘 정리된 6개가 낫다
- **결과를 짧게 돌려준다.** 도구 결과는 그대로 컨텍스트에 쌓입니다. DB 조회 결과 500행을 그대로 넣으면 컨텍스트가 몇 턴 만에 터집니다. 요약하거나 상한을 둔다
...
LangGraph는 이 흐름을 **상태(State)를 들고 노드 사이를 이동하는 그래프**로 표현합니다. 일반적인 체인이 DAG(순환 없는 그래프)라면, LangGraph는 **순환을 허용**합니다. 에이전트 루프가 곧 순환이기 때문입니다.
...
> **비유의 한계**: 실제 담당자는 파일 전체를 마음대로 고칠 수 있지만, 노드는 **갱신할 키만 담은 부분 딕셔너리**를 반환하고 병합은 리듀서가 정한 규칙대로 이루어집니다.

### 리듀서가 핵심이다

State를 그냥 TypedDict로 정의하면 노드가 반환한 값이 **기존 값을 덮어쓴다.** 메시지 이력처럼 쌓여야 하는 데이터에는 이게 치명적입니다.
...
`Annotated[list, add_messages]`의 두 번째 인자가 리듀서입니다. 노드가 `{"messages": [새 메시지]}`를 반환하면 기존 리스트에 **덧붙는다.** 이걸 빼먹으면 매 노드마다 대화 이력이 통째로 날아가고, 에이전트가 방금 한 일을 기억하지 못해 같은 도구를 무한히 반복하는 증상이 나옵니다.
...
문제는 **이게 조용히 돈다**는 것입니다. 에러도 안 나고, 로그도 정상으로 보이고, 청구서만 올라갑니다. 그래서 한 겹이 아니라 여러 겹으로 막습니다.
...
노드 전이 횟수의 절대 상한입니다. **최후의 안전망이지 정상 종료 수단이 아니다.** 여기에 걸렸다는 것 자체가 설계에 문제가 있다는 신호이므로 알림을 걸어두는 편이 좋습니다. ReAct 루프는 한 사이클에 노드를 2개 지나므로, 도구 호출 7회를 허용하려면 대략 그 두 배 이상을 잡아야 합니다.
...
`recursion_limit`과 다른 점은 **예외가 아니라 정상 경로로 빠진다**는 것입니다. `finalize` 노드에서 "지금까지 알아낸 것만으로 답하라"고 시키면, 사용자는 최소한 부분적인 답이라도 받습니다. 이때 `finalize`를 노드로 등록하고 `add_conditional_edges`의 경로 맵에도 `{"tools": "tools", "finalize": "finalize", END: END}`처럼 함께 넣어야 합니다. 라우팅 함수가 경로 맵에 없는 이름을 반환하면 그 자리에서 에러가 납니다.
...
가장 싸고 가장 자주 빠지는 층입니다. **모델에게 "언제 멈춰야 하는지"를 아무도 안 알려준 채 멈추지 않는다고 탓하는 경우가 많다.**
...
- `thread_id`가 어떤 실행을 재개할지 식별하는 키입니다. 이게 없으면 재개할 대상을 찾을 수 없다
- `MemorySaver`는 프로세스 메모리에 저장하므로 **재시작하면 사라진다.** 실제 서비스에서는 DB 기반 체크포인터를 쓴다
- 승인 게이트를 너무 많이 두면 사람이 습관적으로 전부 승인하게 됩니다. **정말 되돌릴 수 없는 지점에만** 둔다
...
궤적 로그가 특히 중요합니다. 에이전트가 이상한 답을 냈을 때, **어느 단계에서 어긋났는지는 궤적을 보지 않으면 알 수 없다.**
...
결과만 보면 "운 좋게 맞은 경우"와 "제대로 푼 경우"가 구별되지 않습니다. 반대로 궤적만 보면 사람이 예상한 것과 다른 더 나은 경로를 벌점 처리하게 됩니다. **둘을 같이 봐야 한다.**

---

## 8. 실무에서는

- **에이전트로 시작하지 않는다.** 워크플로우로 만들어 돌려보고, 코드로 못 적는 분기가 계속 나올 때 그 부분만 에이전트로 바꿉니다. 전체를 자율에 맡기는 것보다 "대부분은 고정 흐름, 특정 구간만 자율"이 훨씬 안정적입니다.
- **도구 권한을 최소화한다.** 에이전트가 SQL을 실행할 수 있다면 읽기 전용 계정을 줍니다. 프롬프트 방어는 뚫릴 수 있고, 권한은 뚫리지 않습니다.
- **비용 상한을 요청 단위로 건다.** 사용자당 일일 요청 수, 요청당 최대 LLM 호출 수, 요청당 최대 토큰을 모두 정합니다. 에이전트는 호출 횟수가 미리 정해지지 않으므로 단발 호출보다 상한이 더 중요합니다.
- **체크포인터를 영속 저장소로 둔다.** 메모리 기반은 데모용입니다. 사람 승인이 몇 시간 뒤에 올 수도 있는데 그 사이 배포가 일어나면 작업이 통째로 사라집니다.
...
에이전트는 도구와 루프를 얻는 대신 예측 가능성을 내주는 거래이고, 잘 만든다는 것은 **가능한 한 워크플로우로 밀어두고, 남은 자율 구간에는 종료 조건과 예산과 궤적 로그를 촘촘히 채워두는 일**입니다.
