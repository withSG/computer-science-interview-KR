...
**3번 이후는 2번의 결과에 따라 달라집니다.** 전체 매출이 그대로인데 특정 카테고리만 빠졌다면 볼 곳이 다르고 전 카테고리가 고르게 빠졌다면 또 다릅니다. 미리 순서를 다 적어둘 수가 없습니다.
...
ReAct(Reasoning + Acting)는 에이전트의 기본 패턴입니다. **생각하고 행동하고 결과를 본다.** 이 셋을 목표 달성까지 반복합니다.
...
**이게 조용히 돕니다.** 에러도 안 나고 로그도 정상으로 보이고 청구서만 올라갑니다. 그래서 여러 겹으로 막습니다.
