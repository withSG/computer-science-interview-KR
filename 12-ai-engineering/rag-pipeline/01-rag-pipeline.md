...
RAG를 3단계(Indexing / Retrieval / Generation)로 요약하는 설명이 많습니다. 실제로 튜닝할 때는 더 잘게 봐야 어디가 문제인지 찾을 수 있습니다. 그리고 **두 경로가 서로 다른 시점에 돈다**는 점이 중요합니다.
...
재귀 분할이 실무 기본값인 건 동작 방식 때문입니다. 큰 구분자부터 시도해서 자르고 결과가 목표 크기보다 크면 더 작은 구분자로 다시 쪼갭니다.
...
임베딩 검색은 질문과 문서를 **각각 따로** 벡터로 만든 뒤 거리를 잽니다(bi-encoder). 빠른 대신, 둘을 함께 읽고 판단하지 않기 때문에 미묘한 관련성을 놓칩니다. Cross-encoder는 질문과 문서를 **붙여서 한 번에** 모델에 넣고 관련성 점수를 냅니다. 훨씬 정확하지만 문서 하나마다 모델을 돌려야 해서 느립니다.
