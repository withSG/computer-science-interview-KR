...
> MVC가 무엇을 분리하려고 나왔는지, MVP와 MVVM은 MVC의 어떤 불만에서 갈라졌는지, 그리고 레이어드에서 헥사고날로 넘어갈 때 뒤집히는 것이 정확히 무엇인지 설명할 수 있게 된다.
...
- [행위 패턴](./03-behavioral-patterns.md) - 옵저버는 MVC의 Model-View 갱신에 그대로 쓰인다
...
JSP 초창기(흔히 Model 1이라 부르는 방식)의 화면 하나는 이런 모습이었다.
...
**왜 문제인가**: VIP 할인 규칙이 이 화면 파일 안에만 있어서 모바일 API에서도 필요하면 복사해야 하고, 규칙이 바뀌면 두 곳이 어긋난다. 할인 로직만 검증하고 싶어도 JSP를 렌더링하지 않고는 실행할 수 없다. 디자이너가 `<tr>` 하나를 옮기려다 SQL을 건드리고, DB 컬럼명이 바뀌면 화면 파일 수십 개를 뒤져야 한다.

문제의 본질은 하나다. **바뀌는 이유가 서로 다른 코드가 한 파일에 있다.** 화면 디자인은 마케팅 때문에 바뀌고, 할인 규칙은 정책 때문에 바뀌고, SQL은 DB 스키마 때문에 바뀐다. 이 셋을 갈라놓자는 것이 관심사 분리(Separation of Concerns)이고, 그 첫 번째 정형화가 MVC다.
...
MVC는 웹에서 나온 것이 아니다. 1970년대 말 제록스 PARC에서 Smalltalk 환경을 만들던 트리그베 린스카우그(Trygve Reenskaug)가 **"사용자의 머릿속 모델과 화면에 보이는 것을 어떻게 연결할 것인가"**를 고민하다 정리한 구조다. 그래서 원래 MVC에서 View는 Model을 직접 구독하고, Model이 바뀌면 View가 스스로 갱신됐다. 옵저버 패턴이 MVC의 핵심 부품인 이유가 여기 있다.
...
**"Model = Entity"가 아니다.** MVC의 Model은 데이터 구조가 아니라 **비즈니스 로직을 포함한 영역 전체**다. Entity, Service, Repository가 모두 Model 쪽에 속한다.

가장 자주 보는 안티패턴은 컨트롤러가 비대해지는 것이다.
...
**왜 문제인가**: 이 정책은 배치 잡에서도, 모바일 API에서도 똑같이 필요하다. 컨트롤러 안에 있으면 HTTP 요청 없이는 재사용도 테스트도 안 된다. 컨트롤러가 두꺼워지는 만큼 Model은 데이터만 담은 껍데기가 된다.
...
> 컨트롤러의 일은 **HTTP를 애플리케이션 언어로 통역하고 그 반대로 되돌리는 것**뿐이다. 컨트롤러에서 `if`가 비즈니스 조건을 검사하기 시작하면 경계를 넘은 것이다.
...
원래 MVC에서 View는 Model을 직접 참조한다. 서버 렌더링 웹에서는 이게 큰 문제가 아니었다. 요청 하나가 끝나면 화면은 통째로 다시 그려지고, View는 잠깐 살았다 사라지기 때문이다.

문제는 **데스크톱과 모바일처럼 화면이 오래 살아 있고 상태를 계속 들고 있는 환경**에서 터졌다. View가 Model을 직접 알면 화면 코드에 로직이 섞이고, 그 코드는 화면 프레임워크 없이는 테스트할 수 없다. MVP와 MVVM은 이 문제를 서로 다른 방식으로 풀었다.
...
**서버 렌더링 웹 → MVC.** 요청-응답이 짧게 끝나므로 View가 상태를 들고 있을 이유가 없다. Controller가 Model을 부르고 템플릿에 데이터를 넘겨 통째로 렌더링하면 그만이다. 중재자를 더 두는 것은 비용만 늘린다.

**초기 Android, WinForms → MVP.** 화면 컴포넌트(`Activity`, `Form`)가 프레임워크에 강하게 묶여 있어 단위 테스트가 사실상 불가능했다. 그래서 로직을 전부 Presenter로 빼고, View를 인터페이스로 추상화해 테스트에서는 가짜 View를 주입하는 전략이 필요했다.
...
다만 Presenter가 View의 모든 갱신을 일일이 명령해야 해서, 화면이 복잡해지면 Presenter가 거대해진다. 이것이 MVP의 한계다.

**WPF, 현대 Android, Vue → MVVM.** 프레임워크가 데이터 바인딩을 제공하면 Presenter의 명령 코드가 통째로 사라진다. ViewModel은 "화면에 필요한 상태"만 들고 있으면 되고, 그 상태가 바뀌면 프레임워크가 화면을 갱신한다. MVVM은 2005년 마이크로소프트의 존 고스먼(John Gossman)이 WPF의 바인딩 엔진을 전제로 정리한 패턴이고, Android는 Jetpack의 `ViewModel`과 관찰 가능한 상태가 들어오면서 이 구조로 옮겨갔다.

한 가지 짚어둘 점이 있다. **React는 MVVM으로 분류하지 않는 것이 일반적이다.** 상태를 바꾸면 화면이 갱신된다는 점은 닮았지만, React의 데이터 흐름은 부모에서 자식으로 내려가는 단방향이고 화면에서 상태로 되돌아오는 경로는 명시적인 이벤트 핸들러다. MVVM의 특징인 양방향 바인딩이 없다. Vue의 `v-model`은 양방향 바인딩이라 MVVM 계보에 훨씬 가깝다.
...
**왜 문제인가**: 조회가 단순하다는 이유로 계층을 건너뛰면, 나중에 권한 검사나 캐싱을 넣을 자리가 사라진다. 엔티티를 그대로 응답하면 DB 컬럼 변경이 곧바로 API 스펙 변경이 되고, 지연 로딩 필드를 직렬화하다 예외가 나기도 한다.

두 번째 위반은 반대 방향이다. `OrderRepositoryImpl`이 상위 계층인 `NotificationService`를 주입받아 저장 직후 알림까지 보내는 코드가 그렇다. 의존 방향이 순환하는 순간 계층 구조의 의미가 사라진다. 저장만 하고 싶은 배치에서도 알림이 나가고, 저장소를 단독으로 테스트할 수 없다.

### 레이어드의 진짜 한계

계층을 잘 지켜도 남는 문제가 있다. **비즈니스 계층이 영속성 계층을 향해 의존한다**는 사실 자체다.
...
주문 정책이라는 가장 중요한 규칙이 "지금 우리가 JPA를 쓴다"는 기술 선택에 묶여 있다. 그 결과 정책 하나를 검증하려 해도 DB를 띄워야 하고, 테이블 구조에 맞춰 엔티티를 짜다 보니 로직은 Service로 몰린다. 데이터만 든 엔티티와 비대한 서비스, 흔히 도메인 빈혈(anemic domain model)이라 부르는 상태다.
...
핵심 아이디어는 한 문장이다. **"비즈니스 로직이 기술을 의존하지 말고, 기술이 비즈니스 로직을 의존하게 하라."**

방법은 [DI에서 본 것](../02-backend-engineering/spring-framework/01-ioc-di.md)과 같다. 필요한 인터페이스를 **쓰는 쪽이 정의**하고, 구현은 바깥에서 제공한다.
...
로버트 마틴의 클린 아키텍처는 이 아이디어를 동심원으로 정리한 것이다. 안쪽부터 엔티티(기업 전반의 규칙) → 유스케이스(애플리케이션 규칙) → 인터페이스 어댑터(컨트롤러, 프레젠터, 게이트웨이) → 프레임워크와 드라이버(웹, DB) 순이고, 규칙은 딱 하나다.

> **의존성 규칙: 소스 코드의 의존은 반드시 안쪽을 향해야 한다.** 안쪽 원은 바깥 원의 이름을 알아서는 안 된다.

헥사고날(포트와 어댑터, 앨리스터 코오번, 2005)과 클린 아키텍처는 이름과 그림이 다를 뿐 같은 원칙을 말한다. 헥사고날은 "안과 밖"의 경계를 강조하고, 클린 아키텍처는 안쪽을 다시 여러 겹으로 나눈 버전이라고 이해하면 된다.
...
- **파일이 늘어난다.** 도메인 `Order`, JPA `OrderJpaEntity`, API 응답 `OrderResponse`가 따로 존재하고 그 사이의 변환 코드가 필요하다.
- **JPA의 편의를 일부 포기한다.** 도메인 객체를 영속성 엔티티와 분리하면 더티 체킹 같은 기능을 그대로 쓰기 어려워 매핑 코드를 직접 써야 한다.
- **팀 전체가 규칙을 알아야 한다.** 한 명이 도메인에서 JPA 어노테이션을 쓰기 시작하면 경계는 그날로 무너진다.

---

## 6. 언제 이런 구조가 과할까

아키텍처 결정은 "좋은 것"이 아니라 "지금 우리에게 맞는 것"을 고르는 일이다. 판단 기준을 정리하면 이렇다.
...
> 결론: **도메인 규칙의 복잡도가 기술 세부사항의 복잡도를 넘어설 때** 헥사고날의 가치가 생긴다. 게시판 CRUD에 포트와 어댑터를 넣으면 파일 여섯 개로 나뉜 `save()` 하나가 남을 뿐이다.

과한 구조의 신호는 대체로 이렇게 나타난다. 유스케이스가 포트를 그대로 한 번 부르고 끝나는 **패스스루 계층**이 생기고, DTO 변환 코드가 비즈니스 코드보다 길어지고, 기능 하나를 추가할 때 손대는 파일이 다섯 개를 넘는다. 이런 신호가 보이면 계층을 하나 걷어내는 것이 개선이다.

---

## 7. 실무에서는

- **Spring MVC는 이름과 달리 Front Controller 패턴이 먼저다.** 모든 요청이 `DispatcherServlet` 하나로 들어와 핸들러를 찾아 분배한다. 컨트롤러 각각이 독립적으로 요청을 받는 구조가 아니다. 자세한 흐름은 [Spring MVC 요청 처리 흐름](../02-backend-engineering/spring-framework/03-spring-mvc-flow.md)에서 다룬다.
- **`@RestController`를 쓰면 View가 사라진 것처럼 보이지만 아니다.** JSON 직렬화 자체가 View의 역할을 대신하고, View 자리는 클라이언트 애플리케이션으로 옮겨간 것뿐이다. 그래서 프론트엔드에도 별도의 아키텍처 패턴이 필요해진 것이다.
- **모든 프로젝트를 한 아키텍처로 통일할 필요는 없다.** 결제나 정산처럼 규칙이 복잡한 모듈만 포트와 어댑터로 감싸고, 관리자 CRUD는 레이어드로 두는 혼합 구성이 현실적이다.
- **경계를 지키는 가장 실용적인 도구는 패키지 구조와 테스트다.** 도메인 패키지가 `jakarta.persistence`나 `org.springframework`를 import 하면 실패하는 아키텍처 테스트를 CI에 걸어두면, 규칙이 문서가 아니라 빌드로 강제된다.
...
**Q. MVC 패턴을 왜 쓰나요?**
A. 바뀌는 이유가 다른 코드를 분리하기 위해서입니다. 화면은 디자인 때문에, 비즈니스 규칙은 정책 때문에, 데이터 접근은 스키마 때문에 각각 바뀌는데 한 파일에 있으면 하나를 고칠 때 나머지가 깨질 위험이 생깁니다. 분리하면 같은 Model을 웹 화면과 JSON API가 함께 쓸 수 있고, 비즈니스 로직을 화면 없이 테스트할 수 있습니다.
- 꼬리 질문: "Model은 Entity를 말하는 건가요?" → 아니다. MVC의 Model은 데이터 구조가 아니라 비즈니스 로직을 포함한 영역 전체이고, Entity·Service·Repository가 모두 여기 속한다고 답한다.
...
- 꼬리 질문: "왜 Android는 MVP에서 MVVM으로 옮겨갔나요?" → MVP는 Presenter가 화면 갱신을 일일이 명령해야 해서 화면이 복잡해질수록 비대해지는데, Jetpack에서 관찰 가능한 상태와 바인딩이 제공되면서 그 명령 코드를 걷어낼 수 있게 됐다고 답한다.
...
- 꼬리 질문: "그럼 항상 헥사고날이 낫나요?" → 아니다. CRUD 위주 서비스에서는 변환 코드만 늘어난다. 도메인 복잡도가 기술 복잡도를 넘어설 때 가치가 생긴다고 답한다.
...
- 꼬리 질문: "그걸 어떻게 강제하나요?" → 패키지를 분리하고, 도메인 패키지가 프레임워크를 import 하면 실패하는 아키텍처 테스트를 CI에 두는 방법을 쓴다고 답한다.
...
| "Controller는 요청을 받아 처리하는 곳이다" | 처리는 Model의 일이다. Controller는 통역만 한다 | Controller에서 비즈니스 조건을 검사하기 시작하면 경계를 넘은 것이다 |
| "MVVM은 MVC의 상위 호환이다" | 셋은 서로 다른 환경의 제약에서 나온 해법이다 | 서버 렌더링 웹에 MVVM을 얹으면 비용만 늘어난다 |
| "React는 MVVM이다" | React는 단방향 데이터 흐름이고 양방향 바인딩이 없다 | MVVM에서 영감을 받은 부분은 있지만 보통 별도로 분류한다. Vue의 `v-model`이 MVVM에 더 가깝다 |
| "레이어드에서 조회가 단순하면 Service를 건너뛰어도 된다" | 권한 검사나 캐싱을 넣을 자리가 사라진다 | 얇더라도 계층은 유지한다. 일관성이 예측 가능성을 만든다 |
| "헥사고날은 인터페이스를 많이 만드는 것이다" | 인터페이스 개수가 아니라 **누가 그것을 정의하느냐**가 핵심이다 | 쓰는 쪽(도메인)이 인터페이스를 정의해야 의존이 뒤집힌다. 구현체 옆에 만든 인터페이스는 아무것도 뒤집지 않는다 |
...
MVC는 바뀌는 이유가 다른 코드를 갈라놓은 첫 시도이고, MVP와 MVVM은 화면이 오래 살아 있는 환경에서 View를 테스트 가능하게 만들려는 두 갈래 답이며, 헥사고날과 클린 아키텍처는 그 분리를 애플리케이션 전체로 확장해 **도메인이 기술을 의존하지 않게 의존 방향을 뒤집는 것**이다.
...
- [02-structural-patterns.md](./02-structural-patterns.md) - 헥사고날의 어댑터가 곧 어댑터 패턴이다
