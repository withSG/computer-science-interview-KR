...
문제의 본질은 하나입니다. **바뀌는 이유가 서로 다른 코드가 한 파일에 있습니다.** 화면 디자인은 마케팅 때문에 바뀌고, 할인 규칙은 정책 때문에 바뀌고, SQL은 DB 스키마 때문에 바뀝니다. 이 셋을 갈라놓자는 것이 관심사 분리(Separation of Concerns)이고, 그 첫 번째 정형화가 MVC다.
...
한 가지 짚어둘 점이 있습니다. **React는 MVVM으로 분류하지 않는 것이 일반적입니다.** 상태를 바꾸면 화면이 갱신된다는 점은 닮았지만, React의 데이터 흐름은 부모에서 자식으로 내려가는 단방향이고 화면에서 상태로 되돌아오는 경로는 명시적인 이벤트 핸들러입니다. MVVM의 특징인 양방향 바인딩이 없습니다. Vue의 `v-model`은 양방향 바인딩이라 MVVM 계보에 훨씬 가깝습니다.
