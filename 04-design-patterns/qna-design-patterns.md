...
### 실무 선택 기준
실무에서 새로 짠다면 **enum이 기본값**입니다. enum은 역직렬화 시 생성자를 거치지 않고 새 인스턴스가 생기는 문제와, 리플렉션으로 `private` 생성자를 강제 호출해 두 번째 인스턴스를 만드는 공격을 언어 차원에서 막아줍니다. 생성 시점에 외부 값을 주입해야 하거나 상속이 필요하면 LazyHolder를 씁니다. 위 DCL 코드는 `volatile`이 가시성과 명령어 재배치 방지를 어떻게 보장하는지 설명하는 교육용 예시로 이해하면 됩니다.
...
### 주의: 정적 팩토리 메서드와의 구분
위 코드는 문자열로 분기해 구현체를 골라 반환하는 `static` 메서드로, 엄밀히는 GoF의 Factory Method 패턴이 아니라 **정적 팩토리 메서드(static factory method)**라는 별개의 관용구입니다. GoF Factory Method는 추상 Creator 클래스가 생성 메서드(`createXxx()`)를 서브클래스에 위임해 상속으로 "무엇을 만들지"를 정하는 구조를 가리키며, 위 예시처럼 상속이나 다형성 없이 조건 분기만으로 구현체를 고르는 것과는 다릅니다.
