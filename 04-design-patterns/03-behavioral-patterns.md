...
> 늘어나는 `if-else`와 서로를 직접 부르는 서비스들이 왜 문제인지, 그리고 Strategy·Template Method·Observer·Command·State·Chain of Responsibility·Iterator가 그 통신 구조를 어떻게 바꾸는지 설명할 수 있게 된다.

## 학습 목표

- [ ] `if-else` 분기를 전략 객체로 바꾸는 리팩토링을 단계별로 수행할 수 있다
...
생성 패턴이 "누가 만드나", 구조 패턴이 "어떻게 조립하나"를 다뤘다면 행위 패턴은 **"객체들이 어떻게 일을 나누고 서로 부르나"**를 다룬다. 실무에서 이 문제는 거의 항상 두 가지 냄새로 나타난다.
...
둘 다 증상은 같다. **새 요구사항이 기존 코드를 열게 만든다.** 첫 번째는 전략 패턴이, 두 번째는 옵저버 패턴이 푸는 문제다.
...
**왜 문제인가**: 배송 방식이 하나 늘 때마다 이 메서드를 수정하므로 OCP를 어기고, 잘 돌던 로직이 있는 파일을 계속 열게 되어 회귀 위험이 쌓인다. `SAME_DAY`만 검증하고 싶어도 `calculate()` 전체를 통과해야 하고, 당일배송의 거리 제한 같은 개별 규칙이 다른 방식들과 한 메서드에 뒤엉킨다.
...
**왜 문제인가**: 분기가 계산 로직에서 생성 로직으로 자리만 옮겼다. 새 방식이 생기면 여전히 이 메서드를 연다.

Spring을 쓴다면 이 마지막 분기까지 없앨 수 있다. **같은 인터페이스를 구현한 빈이 여러 개면 컨테이너가 `Map<String, T>`에 빈 이름을 키로 넣어 주입해준다.**
...
이제 새 배송 방식은 **클래스 파일 하나를 추가하는 것으로 끝난다.**

**비유**: 내비게이션의 경로 옵션이다. "최단 거리", "무료 도로", "고속도로 우선"은 계산 규칙이 다르지만 목적지를 넣고 경로를 받는 사용법은 똑같다.
> **비유의 한계**: 내비게이션은 사용자가 옵션을 고르지만, 코드에서는 전략 선택 책임을 누구에게 줄지가 설계 결정이다. 그 책임을 잘못 두면 위 안티패턴처럼 분기가 되살아난다.

| 상황 | 판단 |
|------|------|
| 분기가 2~3개이고 늘어날 일이 없다 | `if-else`가 더 읽기 쉽다. 클래스만 늘어난다 |
| 분기마다 로직이 한 줄이다 | `enum` 상수별 메서드로 충분하다 |
| 분기가 계속 늘고 각각이 복잡하다 | 전략 패턴 |

> 결론: 전략 패턴은 **분기의 개수보다 "얼마나 자주 추가되는가"**로 판단한다. 늘지 않는 분기를 패턴으로 감싸면 파일만 흩어진다.

---

## 3. 템플릿 메서드 (Template Method)

CSV와 엑셀을 읽어 DB에 적재하는 배치 두 개가 있다고 하자. 검증 → 파싱 → 저장 → 로깅 중 **파싱 한 줄만 다른데** 나머지가 통째로 복제된다. 로깅 형식을 바꾸면 두 클래스를 똑같이 고쳐야 하고, 한 곳만 고치고 잊으면 두 배치의 동작이 조용히 달라진다.

**변하지 않는 흐름을 상위 클래스가 `final` 메서드로 고정하고, 달라지는 단계만 추상 메서드로 남긴다.**
...
"가입 축하 쿠폰"이 추가돼도 `UserService`는 열지 않는다. 리스너 클래스를 하나 만들면 된다.

**비유**: 유튜브 구독이다. 채널은 영상을 올릴 뿐 누가 보는지 신경 쓰지 않고 구독자는 각자 알아서 반응한다.
> **비유의 한계**: 유튜브 알림은 늦게 봐도 되지만, 기본 `@EventListener`는 **발행 스레드에서 동기적으로 즉시 실행된다.** 리스너가 느리면 회원가입 응답도 그만큼 늦어진다. 이 오해가 실무 사고의 단골 원인이다.

### 옵저버가 치르는 대가

**1. 기본은 동기다.** 비동기로 만들려면 리스너에 `@Async`를 붙이고 애플리케이션에 `@EnableAsync`를 켜야 한다.

**2. 트랜잭션 타이밍.** 위 `register()`는 `@Transactional`이 붙어 있다. 이벤트 발행 뒤 커밋 전에 예외가 나면 DB는 롤백되지만 **이메일은 이미 나간 뒤다.**
...
**3. 흐름 추적이 어렵다.** 발행자 코드만 봐서는 무슨 일이 일어나는지 알 수 없고 이벤트 타입으로 검색해야 리스너를 찾는다. 결합도를 낮춘 대가로 **가독성과 디버깅 난이도를 지불하는 것**이므로 진짜 부수적인 후속 처리에만 쓰는 것이 좋다. 주문 생성에서 재고 차감처럼 실패하면 안 되는 로직은 직접 호출이 낫다.

직접 구현할 때는 구독 해제도 챙겨야 한다. `unsubscribe()`를 부르지 않으면 리스너가 리스트에 남아 GC 대상이 되지 않고 그대로 메모리 누수가 된다. 참고로 자바 표준의 `java.util.Observable`은 설계 결함 때문에 Java 9에서 폐기(deprecated)됐으므로 쓰지 않는다.

---

## 5. 커맨드 (Command)

메서드 호출은 일회성 사건이라 저장하거나 전달하거나 되돌릴 수 없다. `editor.insertText("안녕")`을 부른 뒤 "방금 한 작업을 되돌려줘"를 표현할 방법이 없다. 커맨드는 **요청 자체를 객체로 만든다.**
...
"무엇을 할지"가 객체가 되는 순간 **저장·전달·지연 실행·취소**가 전부 가능해진다. 자바의 `Runnable`이 가장 널리 쓰이는 커맨드다. `ExecutorService.submit(runnable)`은 실행 요청을 객체로 받아 큐에 넣었다가 워커 스레드가 꺼내 실행하는 구조다.
...
**왜 문제인가**: 상태가 하나 추가되면 **모든 메서드의 분기를 찾아 고쳐야 한다.** 하나라도 빠뜨리면 그 상태에서만 이상하게 동작한다. 그리고 "PAID 다음에 올 수 있는 상태가 무엇인가"라는 전이 규칙이 코드 전체에 흩어져 한눈에 보이지 않는다.
...
`Order`에서 분기가 사라졌고 "결제 완료 상태에서 무엇이 가능한가"가 `PaidState` 한 파일에 모였다.

Strategy와 코드 모양이 또 같다. 차이는 **누가 교체를 결정하는가**다.
...
> 한 줄 기준: **"다음에 무엇이 올지 자기가 아느냐."** 안다면 State, 모른다면 Strategy다.

---

## 7. 책임 연쇄 (Chain of Responsibility)

요청 처리 앞에 인코딩·CORS·인증·레이트리밋·로깅 검사가 계속 붙으면, 서로 무관한 관심사가 한 메서드에 쌓인다. 순서를 바꾸거나 특정 경로에서만 인증을 빼는 변경이 전부 그 메서드 수정으로 이어진다.

책임 연쇄는 처리기를 줄로 세워 각자 자기 일만 하고 다음으로 넘기게 한다. 서블릿 필터가 정확히 이 구조다.
...
컬렉션을 순회할 때 내부가 배열인지 연결 리스트인지 호출부가 알아야 한다면, 자료구조를 바꾸는 순간 순회 코드가 전부 깨진다. 게다가 내부 리스트를 그대로 반환하면 외부에서 원소를 마음대로 바꿀 수 있다.
...
자바의 향상된 for문은 컴파일러가 `Iterator`를 꺼내 `hasNext()` / `next()`를 부르는 코드로 바꿔주는 문법 설탕이다. 순회 중 컬렉션을 수정하면 `ConcurrentModificationException`이 나는 것도 이 구조 때문이다. 이터레이터가 생성 시점의 변경 횟수(`modCount`)를 기억하고 있다가 어긋나면 즉시 실패(fail-fast)한다. 다만 이 검사는 최선 노력이라 항상 터진다는 보장은 없다. `ArrayList`에서 끝에서 두 번째 원소를 지우면 `hasNext()`가 먼저 `false`가 되어 예외 없이 조용히 끝난다. 순회 중 삭제가 필요하면 `iterator.remove()`나 `removeIf()`를 쓴다.

---

## 9. 실무에서는

- **전략 패턴은 Spring DI와 만나면 완성된다.** `Map<String, 전략>` 주입이 가장 자주 쓰이는 조합이다. 다만 빈 이름을 문자열 키로 쓰므로 오타가 런타임 오류가 된다. `enum`을 키로 쓰는 `Map<PayType, PaymentPolicy>`를 직접 구성하면 컴파일 시점에 잡힌다.
- **흐름을 프레임워크가 쥐는 구조는 이미 깔려 있는 경우가 많다.** `JdbcTemplate`이나 JUnit의 테스트 실행 생명주기(`@BeforeEach` → 테스트 → `@AfterEach`)가 그렇다. 직접 상속 구조를 만들기보다 함수형 인터페이스를 파라미터로 받는 편이 유연할 때가 많다.
- **이벤트는 남발하면 흐름을 잃는다.** "이 API가 무엇을 하는가"를 코드로 추적할 수 없게 되는 순간이 임계점이다. 실패해도 되는 부수 작업에만 쓰고 핵심 트랜잭션 로직은 직접 호출로 남긴다. 상태 패턴도 마찬가지로 상태가 4개를 넘고 전이 규칙이 복잡할 때 가치가 생기며, 두세 개면 `enum`으로 충분하다.
...
- 꼬리 질문: "전략을 고르는 `if-else`는 어떻게 없애나요?" → Spring에서 같은 인터페이스 빈들을 `Map<String, T>`로 주입받으면 빈 이름이 키가 되므로 `map.get(type)` 한 줄로 선택 분기까지 사라진다고 답한다.
...
- 꼬리 질문: "요즘은 어느 쪽을 선호하나요?" → 상속 결합이 강한 Template Method보다 함수형 인터페이스를 파라미터로 받는 Strategy 형태를 선호하는 흐름이라고 답한다.
...
- 꼬리 질문: "`@EventListener`와 `@TransactionalEventListener`의 차이는?" → 전자는 발행 즉시 같은 스레드에서 실행되고 후자는 트랜잭션 커밋 이후에 실행된다. 롤백됐는데 이메일이 나가는 사고를 막으려면 후자를 쓴다고 답한다.
...
- 꼬리 질문: "주문 상태를 `enum`으로만 관리하면 안 되나요?" → 상태가 적으면 충분하지만 상태별 가능한 동작이 많아지면 메서드마다 같은 분기가 복제되므로 상태 객체로 분리한다고 답한다.
...
행위 패턴은 "새 요구사항이 기존 코드를 열게 만드는" 두 가지 냄새 — 자라는 조건 분기와 서로를 직접 부르는 객체들 — 를 각각 다형성(Strategy·State·Template Method)과 간접 통신(Observer·Command·Chain of Responsibility)으로 바꾸는 기법이다.
