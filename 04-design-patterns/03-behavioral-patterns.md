...
> **비유의 한계**: 유튜브 알림은 늦게 봐도 되지만, 기본 `@EventListener`는 **발행 스레드에서 동기적으로 즉시 실행됩니다.** 리스너가 느리면 회원가입 응답도 그만큼 늦어집니다. 이 오해가 실무 사고의 단골 원인입니다.
...
**3. 흐름 추적이 어렵습니다.** 발행자 코드만 봐서는 무슨 일이 일어나는지 알 수 없고 이벤트 타입으로 검색해야 리스너를 찾습니다. 결합도를 낮춘 대가로 **가독성과 디버깅 난이도를 지불하는 것**이므로 진짜 부수적인 후속 처리에만 쓰는 것이 좋습니다. 주문 생성에서 재고 차감처럼 실패하면 안 되는 로직은 직접 호출이 낫습니다.
