...
> 상속만으로 구조를 짜면 왜 클래스가 폭발하는지, 그리고 Adapter·Decorator·Facade·Proxy·Bridge·Composite가 "감싼다"는 같은 수법으로 서로 다른 문제를 어떻게 푸는지 설명할 수 있게 된다.
...
옵션이 3개면 조합은 2³ = 8가지, 곧 클래스 7개가 필요하다(원본 제외). 옵션이 하나 더 늘면 15개다. **옵션 n개에 클래스 2ⁿ-1개**, 이것이 조합 폭발이다. 더 근본적인 문제는 조합이 컴파일 시점에 고정된다는 것이다. "이 사용자에게만 암호화를 켜자" 같은 런타임 결정을 표현할 방법이 없다.

구조 패턴 여섯 개는 놀랍게도 거의 같은 코드 모양을 갖는다. **대상과 같은 인터페이스를 구현하면서 내부에 대상을 필드로 들고 있는 것**이다.
...
코드가 같은데 왜 이름이 여섯 개일까. **의도가 다르기 때문이다.** 구조 패턴은 "코드 모양"이 아니라 "무엇을 해결하려는가"로 정리해야 한다.
...
> 표 요약: 앞의 네 개는 "감싸는 이유"의 차이고, Bridge는 감싸는 게 아니라 **처음부터 두 계층으로 설계**하는 것, Composite는 감싼 것이 **여러 개**라는 점이 다르다.
...
카카오페이로 짠 결제 코드가 서비스 100군데에서 호출되고 있다. 여기에 토스페이를 추가해야 한다.
...
메서드 이름도, 파라미터도, 반환 타입도 다르다. 호출부마다 변환 코드를 넣으면 **같은 변환이 100군데로 복제되고**, SDK가 업그레이드될 때 그중 하나만 빠뜨려도 그 경로에서만 결제가 깨진다.

### 어댑터

변환 책임을 클래스 하나로 몰아넣는다.
...
호출부 100군데는 한 글자도 바뀌지 않는다. 자기가 부르는 것이 토스인지도 모른다.

**비유**: 해외여행용 전원 어댑터다. 노트북 플러그(기존 코드)도 유럽 콘센트(새 SDK)도 바꾸지 않고 가운데에 변환기를 끼운다.
> **비유의 한계**: 전원 어댑터는 전압만 바꾸지만 소프트웨어 어댑터는 파라미터 조합·예외 타입·단위까지 바꿔야 할 때가 많다. 변환 로직이 두꺼워지면 어댑터가 아니라 별도 계층으로 승격시켜야 한다는 신호다.

실무 사례로는 배열을 `List`로 쓰게 해주는 `Arrays.asList()`, 바이트 스트림을 문자 스트림으로 바꾸는 `InputStreamReader`, 시그니처가 제각각인 핸들러들을 `DispatcherServlet`이 한 방식으로 부르게 해주는 Spring MVC의 `HandlerAdapter`가 있다.
...
> 결론: 기능이 **서로 독립적이고 조합이 자유로워야 하면** 데코레이터, 조합이 두세 가지로 고정이면 상속으로도 충분하다.
...
순수 JDBC로 조회 한 번을 하려면 커넥션 획득, `PreparedStatement` 생성, 파라미터 바인딩, `ResultSet` 순회, `SQLException` 변환, 그리고 `finally`에서 셋을 역순으로 `close()` 하는 15줄이 필요하다.

**왜 문제인가**: 진짜 하고 싶은 일은 SQL 한 줄과 결과 매핑뿐인데 그 주위를 자원 관리 코드가 둘러싼다. 이 15줄이 조회 메서드마다 복제되고, `close()` 하나를 빠뜨리면 커넥션 누수가 된다.

### 퍼사드

`JdbcTemplate`은 그 절차 전체를 감춘 퍼사드다.
...
> 판단 기준: **"이 인터페이스는 원래 있던 것인가, 내가 새로 만든 것인가?"** 원래 있던 것에 맞추면 어댑터, 새로 만들었으면 퍼사드다.

---

## 5. 프록시 (Proxy)

프록시는 대상과 같은 인터페이스를 구현해 **대상 앞에 서서 접근을 통제한다.** 통제하는 이유에 따라 이름이 나뉜다.

**가상 프록시(Virtual Proxy)** 는 무거운 객체의 생성을 실제로 필요한 순간까지 미룬다. JPA의 지연 로딩이 이 구조다. `order.getMember()`는 프록시를 돌려주고, `member.getName()`을 부르는 순간 SELECT가 나간다. [N+1 문제](../02-backend-engineering/database/03-n-plus-one-problem.md)가 생기는 지점도 여기다.

**보호 프록시(Protection Proxy)** 는 호출 전에 권한을 검사한다.
...
**원격 프록시(Remote Proxy)** 는 네트워크를 감춘다. 호출하는 쪽은 로컬 메서드를 부르는 것처럼 보이지만 실제로는 요청을 직렬화해 원격으로 보낸다. gRPC 스텁이나 Spring Cloud OpenFeign의 인터페이스 기반 클라이언트가 그렇다.

### Spring AOP와의 연결

`@Transactional`이 붙은 서비스를 주입받으면 컨테이너가 준 것은 원본이 아니라 프록시다.
...
이 구조에서 **같은 클래스 안의 메서드를 `this`로 부르면 프록시를 거치지 않아 트랜잭션이 걸리지 않는다.** 프록시 패턴의 성질에서 곧바로 따라 나오는 결론이고 실무에서 가장 자주 밟는 지뢰다. 자세한 내용은 [AOP와 프록시](../02-backend-engineering/spring-framework/02-aop-proxy.md)에서 다룬다.

### 데코레이터와 프록시의 차이

코드 모양이 사실상 같아 가장 헷갈리는 한 쌍이다. 구분 기준은 **누가 조립하는가**와 **무엇을 위해 감싸는가**다.
...
도형과 렌더링 방식을 상속으로만 표현하면 `VectorCircle`, `RasterCircle`, `VectorSquare`... 도형 n종 × 렌더러 m종 = n×m개의 클래스가 필요하다. 브릿지는 두 축을 나누고 합성으로 잇는다.
...
전략 패턴과 코드가 거의 같다. 둘 다 인터페이스를 필드로 들고 위임한다. 차이는 의도에 있다.
...
> 판단 기준: **"양쪽이 다 늘어나느냐."** 한쪽만 늘어나면 전략, 양쪽이 각자 늘어나면 브릿지다.

한 가지 덧붙이면, SLF4J를 브릿지 사례로 드는 설명이 흔한데 논쟁의 여지가 있다. 구조는 닮았지만 SLF4J는 처음부터 두 축을 설계한 것이 아니라 **이미 따로 존재하던 로깅 라이브러리들을 나중에 하나의 인터페이스로 묶은 것**이라 의도상으로는 퍼사드에 가깝다. 면접에서 이 구분을 짚어주면 오히려 좋은 인상을 준다.

---

## 7. 컴포지트 (Composite)

파일과 폴더의 전체 크기를 구한다고 하자. 노드 타입마다 `instanceof`로 분기하면, 노드를 다루는 **모든 메서드에 같은 분기가 복제된다.** 종류가 하나 추가되면 그 전부를 찾아 고쳐야 한다.

컴포지트는 잎과 가지를 같은 타입으로 본다.
...
- **어댑터는 외부 연동 경계에 둔다.** 결제, 알림, 지도 API처럼 벤더가 바뀔 수 있는 지점에 인터페이스를 두고 벤더별 어댑터를 붙이면 교체가 클래스 하나 추가로 끝난다.
- **데코레이터는 Spring에서 AOP로 대체되는 경우가 많다.** 여러 클래스에 공통으로 얹는 부가 기능은 `@Around` 어드바이스나 `@Cacheable`을 쓴다. 다만 **특정 객체 하나만 감싸고 싶을 때**는 데코레이터가 더 명시적이고 안전하다.
- **프록시는 대부분 프레임워크가 만들어준다.** 직접 구현할 일보다 "지금 내가 들고 있는 이 객체가 프록시인가"를 판단할 일이 훨씬 많다. 지연 로딩 예외, self-invocation이 모두 여기서 나온다.
...
- 꼬리 질문: "그럼 로깅을 붙이는 건 어느 쪽인가요?" → 클라이언트가 직접 감싸 조립하면 데코레이터, AOP로 컨테이너가 몰래 감싸면 프록시라고 답한다. 같은 기능도 조립 주체에 따라 분류가 달라진다.
...
- 꼬리 질문: "`InputStreamReader`는 어느 쪽인가요?" → `InputStream`을 받아 `Reader`가 되므로 인터페이스가 바뀐다. 어댑터라고 답한다.
...
- 꼬리 질문: "실무 예를 들어주세요" → `new BufferedReader(new InputStreamReader(new FileInputStream(...)))`를 들고 각각이 어댑터인지 데코레이터인지 구분해 설명한다.
...
- 꼬리 질문: "그럼 SLF4J는 브릿지인가요?" → 구조는 닮았지만 이미 존재하던 구현들을 나중에 통합한 것이라 의도로는 퍼사드에 가깝다고 답한다.
...
| "퍼사드는 서브시스템을 못 쓰게 막는다" | 퍼사드는 단순한 진입로일 뿐 차단막이 아니다 | 특수한 경우엔 서브시스템을 직접 써도 된다. 막으면 확장성이 죽는다 |
| "`@Transactional`이 있으면 항상 트랜잭션이 걸린다" | 프록시를 거치지 않는 `this.method()` 호출에는 부가 기능이 적용되지 않는다 | 프록시 패턴의 성질이다. 자기 호출은 프록시를 우회한다 |
...
구조 패턴은 "대상과 같은 인터페이스를 구현하고 대상을 안에 들고 있다"는 하나의 수법을 공유하지만, 인터페이스를 맞추려는지(Adapter) 기능을 얹으려는지(Decorator) 복잡도를 감추려는지(Facade) 접근을 막으려는지(Proxy)에 따라 이름과 설계 판단이 갈린다.
