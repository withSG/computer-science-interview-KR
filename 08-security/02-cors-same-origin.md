...
그런데 브라우저의 자바스크립트가 같은 요청을 보내면 에러가 납니다. 왜 브라우저만 다를까요?
...
>
> **비유의 한계**: 그럼 이 비유는 어디서 무너질까요? SOP가 막는 것은 문서를 "들고 나오는 것"뿐입니다.
> B사 사무실에 편지를 밀어 넣는 것(요청 전송)은 막지 않고, 이 차이 때문에 CSRF가 여전히 성립합니다.
...
네 번째 줄 때문에 "프론트는 `www.myapp.com`, API는 `api.myapp.com`" 구성에서는
반드시 CORS 설정이 필요합니다. 같은 회사 도메인이라는 사실은 브라우저에게 아무 의미가 없습니다.
...
CSRF가 여전히 성립하는 이유가 바로 이것입니다. "CORS 설정했으니 CSRF는 안전하다"가 틀린 이유이기도 합니다.
...
실무에서 Preflight가 붙는 이유는 거의 항상 둘 중 하나입니다. 하나는 `Content-Type: application/json`(조건 ③ 위반)이고,
다른 하나는 `Authorization: Bearer ...`나 `X-CSRF-TOKEN` 같은 커스텀 헤더(조건 ② 위반)입니다.
결국 **요즘 API 호출은 대부분 Preflight를 탑니다.**
...
`Access-Control-Allow-Origin: https://evil.com`이 돌아가고, 브라우저는 그대로 통과시킵니다.
...
이게 없으면 `app.example.com`용 응답이 캐시에 남았다가 다른 출처의 요청에도 그대로 나갑니다.
반대로 허용되지 않은 출처의 응답이 캐시되면, 정상 사용자가 CORS 에러를 겪습니다.
Spring Security의 CORS 필터는 자동으로 붙여 주지만, 직접 헤더를 세팅하는 코드에서는 빠지기 쉽습니다.
...
API는 그냥 공개 API입니다. 반대로 인증이 제대로 되어 있으면 넓게 열어도 서버가 뚫리지는 않습니다.
...
- **가장 확실한 CORS 대책은 CORS를 안 만나는 것입니다.** 리버스 프록시로 `app.example.com/api/*`를
  백엔드로 넘기면 브라우저 입장에서는 동일 출처라 CORS 자체가 발생하지 않습니다.
...
A. 아닙니다. Simple Request라면 요청은 그대로 서버에 도달해 처리까지 끝난 상태입니다.
브라우저가 응답의 `Access-Control-Allow-Origin`을 확인한 뒤 JS에게 넘기지 않았을 뿐입니다.
...
사전 확인이 의미가 없습니다. 실무에서는 `Content-Type: application/json`과 `Authorization` 때문에
...
그래서 명세가 이 조합을 금지하고, credentials를 쓰려면 정확한 출처 문자열을 반환하도록 강제합니다.
- 꼬리 질문: "그럼 요청 Origin을 그대로 되돌려주면 되지 않나요?" → 검증 없이 되비추면
  사실상 `*` + credentials와 같아집니다. 화이트리스트로 검증한 뒤에만 되비춰야 합니다.
