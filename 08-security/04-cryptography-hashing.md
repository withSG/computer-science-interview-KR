...
생깁니다. 특히 **GCM 모드에서 같은 키로 nonce를 재사용하면 인증 키가 노출되어 위조가 가능해진다.**
...
**왜 문제인가**: 암호화는 되돌리기 위한 기술입니다. 즉 **누군가는 원본을 볼 수 있다.**
...
하는데 그 조합 수가 감당 불가능하다), **같은 비밀번호 노출 차단**, 그리고 **공격 병렬화 방지입니다.
...
이 비대칭이 핵심입니다. **정상 사용자는 한 번만 계산하지만, 공격자는 수십억 번 계산해야 한다.**
...
- **암호 알고리즘을 직접 구현하지 않는다.** 표준 알고리즘도 구현 실수 하나로 무력해집니다.
  검증된 라이브러리(JDK JCE, Bouncy Castle, libsodium)를 쓰고 모드와 파라미터만 올바르게 고릅니다.
- **키는 코드나 저장소에 두지 않는다.** 환경 변수, AWS KMS/Secrets Manager, HashiCorp Vault를 씁니다.
...
- **MD5와 SHA-1은 새 코드에 쓰지 않는다.** 충돌을 만드는 방법이 공개되어 있습니다. 용도가 서명·인증이면
