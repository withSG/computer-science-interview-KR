...
> "비밀번호를 암호화해서 저장했습니다"가 왜 틀린 문장인지, 그리고 SHA-256으로 해싱했는데도 왜 부족한지를 공격 비용의 관점에서 설명할 수 있게 됩니다.
...
- 없음 — 이 문서부터 시작해도 됩니다. [03-https-tls.md](./03-https-tls.md)를 함께 읽으면 적용 사례가 붙는다
...
> 입력의 힌트를 얻을 수 없게 설계됩니다. 대신 "입력을 하나씩 넣어보며 맞춰보는" 공격은 여전히
> 가능한데, 이 차이가 뒤에서 핵심이 됩니다.
...
됩니다. 데이터의 반복 패턴이 암호문에 그대로 남아서 비트맵을 ECB로 암호화하면 원본 윤곽이 보일
...
GCM은 AEAD(Authenticated Encryption with Associated Data) 모드로 **암호화와 무결성 검증을 함께** 합니다.
...
복원되고 키를 아는 내부자는 언제든 원본을 봅니다. 사용자들은 비밀번호를 여러 사이트에서
...
설계됐고 그 빠르기가 비밀번호 저장에서는 정확히 약점이 됩니다. 현대 GPU는 SHA-256을 **초당 수십억 회**
...
하는데 그 조합 수가 감당 불가능하다), **같은 비밀번호 노출 차단**, 그리고 **공격 병렬화 방지**입니다.
...
> "DB만 새는" 흔한 사고에서 큰 효과를 냅니다. 다만 교체하려면 모든 해시를 다시 만들어야 하고
...
> 정리하면: **신규 프로젝트는 Argon2id, 라이브러리 제약이 있으면 bcrypt.** 둘 중 무엇이든
> SHA-256 단독보다 압도적으로 낫습니다. 고르느라 시간 쓰는 것보다 하나를 제대로 적용하는 편이 낫습니다.
...
검증 실패), **부인 방지**(본인만 개인키를 가지므로 "내가 안 했다"고 주장하기 어렵다) 셋입니다.
...
**왜 문제인가**: `Random`은 고르게 퍼진 값을 빠르게 만드는 것이 목적이지 예측 불가능성이 목적이 아닙니다.
...
시간이 미세하게 길어지고 이 차이를 반복 측정해 값을 한 글자씩 알아내는 **타이밍 공격**이 가능합니다.
...
암호화는 원본을 되찾기 위한 기술이고 해싱은 되찾지 못하게 하는 기술이며 비밀번호는 후자에 속하되 **Salt로 사전 계산을 막고 work factor로 시도 한 번의 비용을 올리는** 두 축이 함께 있어야 방어가 성립합니다.
