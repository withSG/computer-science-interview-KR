...
> "HTTPS를 쓰면 안전하다"의 안전이 정확히 무엇을 뜻하는지, 그리고 그 안전이 어떤 조건에서 무너지는지 공격자 관점으로 설명할 수 있게 된다.
...
> [../01-computer-science-fundamentals/network/05-ssl-tls-handshake.md](../01-computer-science-fundamentals/network/05-ssl-tls-handshake.md)에 있다.
> 이 문서는 **공격이 어디로 들어오고 무엇이 그것을 막는가**에 초점을 둔다.

---

## 1. 왜 필요한가 — 평문 회선 위의 세 가지 위험

내 노트북에서 서버까지 패킷은 공유기, ISP 장비, 여러 라우터를 지난다.
이 경로 위 아무 지점에나 앉아 있을 수 있는 사람을 가정하면 위험은 셋으로 갈린다.
...
TLS(Transport Layer Security)는 이 셋에 각각 대응한다.
...
여기서 놓치면 안 되는 순서가 있다. **인증이 없으면 암호화는 의미가 없다.**
공격자와 완벽하게 암호화된 통신을 하는 것은 아무것도 지키지 못한다.
그래서 TLS 설계의 절반은 암호화가 아니라 "상대가 진짜 그 도메인의 주인인가"를 증명하는 데 쓰인다.

> SSL은 TLS의 옛 이름이다. SSL 3.0 이후 TLS로 개명됐고 지금 쓰이는 것은 전부 TLS다.
> "SSL 인증서"라는 표현은 관용적으로 남아 있을 뿐이다.
...
두 방식의 약점이 정확히 서로의 강점이다. 그래서 TLS는 **핸드셰이크에서만 비대칭 연산을 쓰고,
그 결과로 만든 대칭 세션 키로 나머지 통신을 전부 처리**한다. 이것을 하이브리드 암호화라 부른다.
수백 MB 응답을 RSA로 암호화하면 실용성이 없지만, 32바이트 키 하나를 합의하는 데만 쓰면 부담이 없다.

> **비유**: 아주 튼튼한 금고(대칭키)가 있는데 열쇠를 상대에게 전달할 방법이 없다.
> 그래서 상대가 보낸 "누구나 잠글 수 있지만 자기만 열 수 있는 상자"(공개키)에 열쇠를 담아 보낸다.
>
> **비유의 한계**: 현대 TLS는 열쇠를 실제로 "보내지" 않는다. ECDHE 방식은 양쪽이 공개값만 교환한 뒤
> 각자 계산해서 같은 값에 도달한다. 세션 키 자체는 회선을 한 번도 지나가지 않는다.

### 키를 보내지 않는 방식으로 바뀐 이유 — 전방 비밀성

예전 RSA 키 교환은 클라이언트가 만든 비밀값을 서버 공개키로 암호화해 보내는 방식이었다.
문제는 **서버 개인키가 언젠가 유출되면**, 몇 년 전에 저장해둔 트래픽까지 소급해서 복호화할 수 있다는 것이다.
공격자가 지금 당장 못 푸는 트래픽을 그냥 쌓아두는 전략이 유효해진다.

ECDHE(Elliptic Curve Diffie-Hellman Ephemeral)는 연결마다 **임시 키 쌍**을 만들고 끝나면 버린다.
서버의 장기 개인키는 "내가 그 서버다"를 서명하는 데만 쓰이고 키 합의에는 관여하지 않는다.
개인키가 유출돼도 과거 세션은 복호화되지 않는다. 이것이 **전방 비밀성(Forward Secrecy)**이고,
TLS 1.3은 키 교환을 (EC)DHE로 못 박아 이 성질을 기본값으로 만들었다.
...
TLS 1.3이 1-RTT가 된 원리는 **추측**이다. 클라이언트가 첫 메시지에
"서버가 아마 이 방식을 고를 것이다"라며 ECDHE 공개값을 미리 넣어 보낸다.
맞으면 왕복 한 번으로 끝나고, 틀리면 한 번 더 왕복한다. 줄어든 왕복만큼 첫 화면이 빨라진다.
다만 세션 재개의 0-RTT 데이터는 전방 비밀성이 없고 재전송도 가능해서 멱등한 요청에만 쓴다.

1.3의 더 중요한 변화는 **선택지를 없앤 것**이다. RSA 키 교환, CBC 모드, RC4, SHA-1, 압축, 재협상이
전부 제거됐다. "설정을 잘못해서 취약해지는" 경우 자체를 줄이는 설계 방향이다.

> 왕복 수 자체보다 "왜 줄었는가"를 설명하는 편이 면접에서 낫다.
> 메시지 단위 상세는 [../01-computer-science-fundamentals/network/05-ssl-tls-handshake.md](../01-computer-science-fundamentals/network/05-ssl-tls-handshake.md)를 참고한다.

---

## 4. 인증서 체인과 CA 신뢰 모델

브라우저가 세상 모든 서버의 공개키를 미리 알 수는 없다. 그래서 **신뢰할 수 있는 제3자(CA)**가
"이 공개키는 이 도메인 주인의 것이 맞다"고 서명해준다.
...
이 모델의 성질을 정확히 이해하는 것이 중요하다.

- **신뢰의 뿌리는 수학이 아니라 목록이다.** 최종적으로 "루트 CA 목록에 있으니 믿는다"에 도달한다.
  이 목록은 OS나 브라우저가 배포한다. 목록을 바꿀 수 있는 사람은 신뢰 체계 전체를 바꿀 수 있다.
- **아무 CA나 아무 도메인 인증서를 발급할 수 있다.** 신뢰 저장소의 CA는 수십 개이고, 그중 하나만
  침해되거나 오발급해도 그 도메인의 위조 인증서가 만들어진다. 이 위험을 줄이려고 도입된 것이
  **Certificate Transparency**다. 발급된 인증서를 공개 로그에 기록하게 해서 도메인 주인이
  "내가 신청하지 않은 인증서"를 발견할 수 있게 한다.
- **중간 CA를 두는 이유는 루트 개인키를 격리하기 위해서다.** 루트 개인키는 오프라인 금고에 두고
  거의 꺼내지 않는다. 유출되면 전 세계 브라우저를 갱신해야 하는 재앙이기 때문이다.
  중간 CA는 유출돼도 그것만 폐기하면 된다.

### 실무에서 자주 터지는 두 가지

**중간 인증서 누락** — 서버가 leaf 인증서만 보내면 클라이언트가 체인을 이어붙일 수 없다.
브라우저는 캐시된 중간 인증서로 어찌어찌 통과해서 "브라우저는 되는데 모바일 앱만 안 된다"는
형태로 나타난다. **인증서 만료**는 가장 흔한 HTTPS 장애다. Let's Encrypt 인증서는 유효기간이 짧아
자동 갱신이 전제이고, 갱신 실패를 감지할 모니터링이 없으면 어느 날 갑자기 전체 서비스가 멈춘다.
...
조건 1은 카페 Wi-Fi 한 번이면 성립할 수 있다. **실질적인 방어선은 조건 2**이고,
TLS의 인증서 검증이 정확히 그 지점을 지킨다. 그래서 "인증서 검증을 끄는" 행위는
암호화만 남기고 방어를 통째로 버리는 것과 같다.
...
**왜 문제인가**: 셋 다 "어떤 인증서가 와도 통과"를 의미한다. 채널은 암호화되지만
**누구와 암호화했는지 확인하지 않으므로** MITM 조건 2가 그대로 열린다.
보통은 개발 환경에서 자체 서명 인증서 때문에 임시로 넣었다가 그대로 배포되고,
운영에서 조용히 살아 있다가 사고가 나서야 발견된다.
...
CA를 지정한다. 개발 환경이라면 `mkcert` 같은 도구로 로컬 신뢰 CA를 만들어 쓰는 편이 낫다.
**"검증을 끈다"와 "신뢰 대상을 추가한다"는 완전히 다른 행위**라는 점이 요지다.

### 회사 방화벽은 왜 HTTPS를 들여다볼 수 있나

기업 보안 장비는 사원 PC에 회사 루트 CA를 설치한다. 그러면 장비가 그 자리에서
`google.com` 인증서를 발급해도 브라우저가 신뢰한다. **합법적인 MITM**이고,
조건 2의 ④에 해당한다. 신뢰 저장소를 통제하는 쪽이 신뢰 체계를 통제한다는 사실을 보여주는 사례다.

---

## 6. HSTS — 첫 요청의 빈틈을 막는다

TLS가 잘 걸려 있어도 남는 구멍이 있다. **사용자는 주소창에 `https://`를 치지 않는다.**
...
공격자가 가로채 버릴 수 있다. 첫 한 번의 평문 요청이 문제의 뿌리다.

**HSTS(HTTP Strict Transport Security)**는 브라우저에게
"이 도메인은 앞으로 무조건 HTTPS로만 접속하라"고 기억시킨다.
...
내부적으로 `https://`로 바꾼다. 평문 요청 자체가 발생하지 않으므로 SSL Stripping이 무력화된다.

### HSTS로도 남는 빈틈 — 그리고 preload

HSTS 헤더는 **HTTPS 응답을 한 번 받아야** 저장된다. 즉 **생애 첫 접속은 여전히 평문일 수 있다.**
이것을 신뢰 우선 사용(TOFU, Trust On First Use) 문제라고 부른다.

`preload`는 이 빈틈을 메운다. 도메인을 브라우저 배포판에 내장된 목록에 등재하면,
그 브라우저는 **한 번도 접속한 적 없는 도메인에 대해서도** 처음부터 HTTPS만 시도한다.

다만 preload는 되돌리기가 매우 어렵다. 브라우저 배포 주기를 타야 해서 해제에 수개월이 걸린다.
`includeSubDomains`까지 걸린 상태에서 서브도메인 하나가 HTTPS를 지원하지 않으면
그 서브도메인은 접속 불가가 된다. **모든 서브도메인의 HTTPS 준비를 끝낸 뒤에 신청해야 한다.**

### 함께 걸어야 하는 것들

쿠키에 `Secure` 속성이 없으면 평문 HTTP 요청에도 쿠키가 실린다
(`Set-Cookie: SESSION=...; Secure; HttpOnly; SameSite=Lax`). HTTPS로 잘 통신하다가 어떤 경로로든
http 요청이 한 번 발생하면 그 순간 세션 쿠키가 노출된다.

**혼합 콘텐츠(Mixed Content)**도 같은 맥락이다. HTTPS 페이지 안에서 `http://`로 스크립트를 불러오면
그 스크립트는 변조 가능하고, 변조된 스크립트는 페이지 전체를 장악한다.
그래서 브라우저는 능동적 혼합 콘텐츠(스크립트, iframe, XHR)를 아예 차단한다.
...
- **TLS 종료(Termination)** — 로드밸런서나 리버스 프록시에서 TLS를 풀고 내부는 평문 HTTP로 보내는
  구성이 일반적이다. 인증서 관리 지점이 한 곳으로 모이고 백엔드 CPU 부담이 준다.
  내부망도 신뢰하지 않는 모델(제로 트러스트)이라면 백엔드까지 TLS를 유지한다.
- **인증서 만료 모니터링은 선택이 아니다.** 자동 갱신을 걸어두었더라도 갱신 실패는 일어난다.
  만료 30일 전 알림을 별도 채널로 받도록 구성한다.
- **mTLS(상호 TLS)** — 클라이언트도 인증서를 제시해 서로를 검증한다. 서비스 메시나 사내 시스템 간
  통신, 일부 금융 API에서 쓰인다.
- **인증서 피닝(Pinning)** — 특정 공개키만 신뢰하도록 앱에 못 박는 기법이다. CA 침해나 사내 프록시
  MITM까지 막지만, 키 교체 시 앱을 함께 업데이트하지 않으면 전체 사용자가 접속 불가가 된다.
  모바일 앱에서 제한적으로 쓰고 웹에서는 쓰지 않는다(웹용 HPKP 헤더는 같은 이유로 폐기됐다).
- **인증서 등급(DV/OV/EV)의 암호학적 강도는 동일하다.** 발급 시 신원 확인 절차만 다르다.
  브라우저가 EV 특별 표시 UI를 없앤 뒤로는 DV가 사실상 표준이다.
...
- 꼬리 질문: "그럼 HTTPS면 XSS나 SQL Injection도 막히나요?" → 전혀 다른 층위다.
  TLS는 전송 구간을 지킬 뿐, 서버가 만들어내는 응답의 내용이나 DB 질의는 관여하지 않는다.
...
- 꼬리 질문: "전방 비밀성이 왜 중요한가요?" → 서버 개인키가 나중에 유출돼도
  과거에 저장해둔 트래픽을 소급 복호화할 수 없다. 공격자의 "일단 저장해두기" 전략을 무력화한다.
...
- 꼬리 질문: "개발할 때 `verify=False`나 `-k`를 쓰는데 괜찮나요?" → 그 순간 인증 속성이 사라져
  MITM에 그대로 노출된다. 검증을 끄는 대신 사내 CA를 신뢰 저장소에 추가하는 방식으로 풀어야 하고,
  그 코드가 운영에 섞여 들어가지 않도록 설정으로 분리해야 한다.
...
- 꼬리 질문: "그럼 HSTS면 완전한가요?" → 헤더는 HTTPS 응답을 한 번 받아야 저장되므로
  ...
  해제가 매우 어려우니 모든 서브도메인의 HTTPS 준비를 끝낸 뒤 신청해야 한다.
...
HTTPS의 핵심은 암호화가 아니라 **인증**이다. CA 서명으로 상대가 진짜임을 먼저 확인하고, 그 상대와만 세션 키를 합의하기 때문에 도청·변조·위장이 동시에 막히며, 검증을 끄거나 첫 요청이 평문으로 새는 순간 이 보장은 통째로 무너진다.
