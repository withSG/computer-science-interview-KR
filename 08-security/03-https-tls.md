...
이 경로 위 아무 지점에나 앉아 있을 수 있는 사람을 가정해 보겠습니다.
그러면 위험은 셋으로 갈립니다.
...
> SSL은 TLS의 옛 이름입니다. SSL 3.0 이후 TLS로 개명됐고, 지금 쓰이는 것은 전부 TLS입니다.
...
수백 MB 응답을 RSA로 암호화하면 실용성이 없습니다. 하지만 32바이트 키 하나를 합의하는 데만 쓰면 부담이 없습니다.
...
문제는 **서버 개인키가 언젠가 유출되면**, 몇 년 전에 저장해둔 트래픽까지 소급해서 복호화할 수 있다는 데 있습니다.
공격자가 지금 당장 못 푸는 트래픽을 그냥 쌓아두는 전략이 유효해집니다.

ECDHE(Elliptic Curve Diffie-Hellman Ephemeral)는 연결마다 **임시 키 쌍**을 만들고 끝나면 버립니다.
서버의 장기 개인키는 "내가 그 서버다"를 서명하는 데만 쓰이고 키 합의에는 관여하지 않는 구조입니다.
...
다만 세션 재개의 0-RTT 데이터는 전방 비밀성이 없고 재전송도 가능합니다. 그래서 멱등한 요청에만 씁니다.
...
- **신뢰의 뿌리를 캐보면 수학이 아니라 목록과 만납니다.** 최종적으로 "루트 CA 목록에 있으니 믿는다"에 도달합니다.
...
- **아무 CA나 아무 도메인 인증서를 발급할 수 있습니다.** 신뢰 저장소의 CA는 수십 개이고, 그중 하나만
  ...
  **Certificate Transparency**입니다. 발급된 인증서를 공개 로그에 기록하게 해서 도메인 주인이
...
브라우저는 캐시된 중간 인증서로 어찌어찌 통과합니다. 그래서 "브라우저는 되는데 모바일 앱만 안 된다"는
형태로 나타납니다.

**인증서 만료**는 가장 흔한 HTTPS 장애입니다. Let's Encrypt 인증서는 유효기간이 짧아
...
보통은 개발 환경에서 자체 서명 인증서 때문에 임시로 넣었다가 그대로 배포됩니다.
...
`google.com` 인증서를 발급해도 브라우저가 신뢰합니다. 이것이 **합법적인 MITM**입니다.
조건 2의 ④가 바로 이 경우입니다. 신뢰 저장소를 통제하는 쪽이 신뢰 체계를 통제한다는 사실이 이 사례에서 드러납니다.
...
이 빈틈을 겨냥한 것이 **HSTS(HTTP Strict Transport Security)**입니다.
브라우저에게 "이 도메인은 앞으로 무조건 HTTPS로만 접속하라"고 기억시킵니다.
...
그 브라우저는 **한 번도 접속한 적 없는 도메인이라도** 처음부터 HTTPS만 시도합니다.
...
쿠키에 `Secure` 속성이 없으면 평문 HTTP 요청에도 쿠키가 실립니다
...
- **인증서 피닝(Pinning)** — 특정 공개키만 신뢰하도록 앱에 못 박는 기법입니다. CA 침해나 사내 프록시
  ...
  모바일 앱에서 제한적으로 쓰고 웹에서는 쓰지 않습니다(웹용 HPKP 헤더는 같은 이유로 폐기됐습니다).
...
각자 같은 값을 계산해냅니다. 그래서 세션 키가 회선을 지나가지 않고, 덕분에 전방 비밀성도 확보됩니다.
...
- 꼬리 질문: "그럼 HSTS면 완전한가요?" → 헤더는 HTTPS 응답을 한 번 받아야 저장되므로
  생애 첫 접속은 여전히 평문일 수도 있습니다(TOFU 문제). 이를 메우는 것이 preload 목록 등재입니다.
  다만 해제가 매우 어려우니 모든 서브도메인의 HTTPS 준비를 끝낸 뒤 신청해야 합니다.
