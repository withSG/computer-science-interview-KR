...
**[2]에서 지원자 대부분이 동점이 됩니다.** 부트캠프나 학교 커리큘럼이 비슷하면 스택도 비슷하기 때문입니다. 점수는 [3]과 [4]에서 갈립니다.
...
**왜 문제인가**: 무엇을 어떻게 쟀는지가 없습니다. "50%"는 응답 시간인지 처리량인지, 어떤 조건에서 몇 번 측정한 평균인지 알 수 없습니다. 면접관이 "어떻게 측정하셨어요?"를 물으면 대부분 여기서 무너집니다. **검증 못 할 숫자는 없느니만 못합니다.**
...
핵심은 하나입니다. **정확히 기억나지 않으면 정성적으로 말합니다.** 기억나지 않는 숫자를 지어내는 것보다, "정확한 수치는 기록을 봐야 하는데 체감상/프로파일러상 뚜렷하게 줄었습니다"가 훨씬 안전합니다. 지어낸 숫자는 꼬리 질문 한 번에 드러납니다.
...
"협업 중 갈등을 어떻게 해결했나요"는 갈등 해결 능력만 보는 질문이 아닙니다. 진짜 확인 대상은 **이 사람이 팀에 들어왔을 때 갈등을 만들 사람인가**입니다. 그래서 답변 내용보다 **상대를 서술하는 태도**가 먼저 채점됩니다.
...
면접 전에 GitHub 링크를 열어보는 면접관이 많습니다. 이때 처음 보이는 것이 README와 커밋 목록입니다. **말할 기회를 얻기 전에 이미 인상이 형성됩니다.**
...
7번이 특히 효과가 큽니다. 겪은 문제와 해결 과정을 적어두면 면접관이 그걸 읽고 질문하는데, **그 질문은 내가 준비한 영역 안에서 나옵니다.** 자기소개에서 키워드로 시험 범위를 유도하는 것과 같은 원리입니다.
...
- **프로젝트는 3개를 나열하기보다 1개를 깊게 파는 편이 유리합니다.** 면접 시간은 한정돼 있고, 얕은 프로젝트 3개는 얕은 질문 3번으로 끝납니다. 대표작 1개에 대해 앞의 꼬리 질문 표를 다 답할 수 있으면 그게 더 강합니다.
- **배포 링크는 면접 전날 반드시 확인합니다.** 무료 호스팅은 일정 기간 미사용 시 잠들거나 만료되는 경우가 있고, 도메인·인증서 만료도 흔합니다. 면접관 앞에서 안 열리면 그 자체가 인상으로 남습니다.
