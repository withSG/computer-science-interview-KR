...
> 면접관이 찾는 것은 의사결정의 흔적입니다. 기술 목록만 적으면 무엇이 빠지는지, 기술 선택을 어떤 프레임으로 방어하는지, 성능 개선을 어떤 절차를 거쳐 숫자로 정리하는지, 그리고 README와 커밋 히스토리가 남기는 인상까지 이 문서에서 다룹니다.
...
기술 면접에서 CS 질문은 어느 지원자에게나 비슷하게 나갑니다. 변별은 **포트폴리오 질문**에서 일어납니다. 이건 지원자마다 다르고, 대답을 준비해오기도 어렵고, 거짓말이 가장 빨리 드러나는 영역이기 때문입니다.

[qna-frontend-companies.md](./qna-frontend-companies.md)의 후기 중에 "노션을 바탕으로 질문을 해주시는 것을 보고 포트폴리오를 아예 보지 않는 것은 아니라고 생각했다"는 기록이 있습니다. 면접관이 포트폴리오를 실제로 훑어보고 거기서 질문을 뽑아 오기도 한다는 뜻입니다. 그 짧은 시간에 시선이 움직이는 경로는 대체로 이렇습니다.
...
**[2]까지는 지원자 대부분이 서로 겹칩니다.** 부트캠프나 학교 커리큘럼이 비슷하면 스택도 비슷하기 때문입니다. 점수는 [3]과 [4]에서 갈립니다.

### 비유: 완공 사진과 설계 도면

건축가의 포트폴리오에 완공된 건물 사진만 잔뜩 있으면 "예쁘네"로 끝납니다. 심사위원은 도면과 회의록을 보고 싶어 합니다. 왜 기둥을 여기 세웠는지, 예산이 부족했을 때 무엇을 포기했는지가 거기에 있습니다.

> **비유의 한계**: 배포 링크가 죽어 있거나 로컬에서도 안 뜨는 프로젝트는 도면이 아무리 훌륭해도 그 앞에서 걸립니다. 건축과 달리 소프트웨어 포트폴리오는 **실제로 동작해야 한다**는 전제가 먼저입니다. 결정의 근거는 동작하는 결과물 위에 얹는 것입니다.
...
> 표 요약: 같은 프로젝트, 같은 스택인데 평가는 왜 갈릴까요. **"썼다"와 "택했다"의 차이** 하나입니다. 포트폴리오 문서를 다시 볼 때 "사용했습니다"로 끝나는 문장을 전부 찾아 이유를 붙이는 것만으로 체감 품질이 달라집니다.
...
[3]이 결정적입니다. 단점까지 말하는 사람은 그 기술을 직접 써본 사람입니다. 장점만 늘어놓으면 문서만 읽고 왔다는 게 드러납니다. 면접관은 이 차이를 구분하려고 "그 기술의 단점은 뭔가요?"를 묻습니다.
...
**왜 문제인가**: "왜 이 프로젝트에서"를 물었는데 "Redux란 무엇인가"를 답했습니다. Redux를 안 써본 사람도 문서만 읽으면 할 수 있는 답이고, 이 프로젝트에 전역 상태가 필요했는지조차 확인되지 않았습니다. 면접관은 곧바로 "Context API로는 안 됐나요?"를 던집니다. 준비 안 된 지원자는 여기서 말문이 막힙니다.
...
**무엇이 달라졌나**: 선택 기준이 **프로젝트 제약, 즉 팀 숙련도와 남은 기간에서** 나왔습니다. 기술 스펙 비교는 뒤로 밀렸습니다. 실무 의사결정이 실제로 그렇게 이루어지기 때문에, 이 서술에서는 "일해본 감각"이 묻어납니다. 마지막의 "지금 다시 한다면"은 회고 능력을 보여주는 문장이라 성장 가능성 축에도 점수가 붙습니다.
...
성능 개선은 포트폴리오에서 가장 강한 소재입니다. 그러면서 가장 자주 망가지기도 합니다. 순서를 지켜야 살아남고, 순서가 없으면 자랑으로 끝납니다.
...
**[1]을 건너뛴 개선은 존재를 증명할 수 없습니다.** "빨라진 것 같습니다"는 면접에서 아무 점수도 얻지 못합니다. 반대로 [1]과 [4]가 같은 조건에서 측정됐다면, 개선 폭이 작아도 신뢰받습니다.

> 아래 예시에 나오는 숫자는 서술 **형식**을 보여주기 위한 가상의 값입니다. 절대 그대로 옮겨 쓰지 말고, 본인이 직접 측정한 값을 넣어야 합니다. 측정하지 않은 숫자를 적는 것은 면접에서 가장 위험한 선택 중 하나입니다.

### 비유: 진료 기록

증상(측정) → 검사와 진단(원인) → 처방(조치) → 경과 관찰(결과). 의사가 검사 없이 처방하면 돌팔이고, 경과를 안 보면 치료가 끝났는지 알 수 없습니다. 성능 개선 서술도 골격이 같습니다.

> **비유의 한계**: 캐시를 붙이면 응답은 빨라지고 데이터 일관성은 나빠집니다. 질병에는 대체로 표준 치료가 있습니다. 성능 문제에는 그런 정답이 없습니다. **성능 개선은 언제나 무언가를 내주고 얻는 거래**라서, 무엇을 내줬는지까지 말해야 답이 완성됩니다.
...
**왜 문제인가**: 무엇을 어떻게 쟀는지가 없습니다. "50%"는 응답 시간인지 처리량인지, 어떤 조건에서 몇 번 측정한 평균인지 알 수 없습니다. 면접관이 "어떻게 측정하셨어요?"를 물으면 대부분 여기서 말이 막힙니다. **검증 못 할 숫자는 없느니만 못합니다.**
...
**왜 문제인가**: [1] 측정과 [2] 원인이 없으니 이 조치들이 필요했는지 알 수 없습니다. 특히 메모이제이션은 비교 비용이 있어서 무분별하게 붙이면 오히려 손해입니다. 이 답변은 "리렌더링 문제가 있었다고 들어서 일단 다 붙였다"로 들리기 쉽습니다. 그래서 "memo를 붙이면 항상 빨라지나요?" 같은 확인 질문이 뒤따르곤 합니다.
...
**정확히 기억나지 않으면 정성적으로 말합니다.** 기억나지 않는 숫자를 지어내는 것보다, "정확한 수치는 기록을 봐야 하는데 체감상/프로파일러상 뚜렷하게 줄었습니다"가 훨씬 안전합니다. 지어낸 숫자는 꼬리 질문 한 번에 드러납니다.
...
> 표 요약: 경험을 노트에 정리할 때는 STAR로 적어두고, 말할 때는 질문에 맞는 열을 꺼내 씁니다. 오른쪽 세 열은 **면접관이 실제로 채점하는 조각, 즉 선택 이유와 측정 근거가 빠지지 않도록** 칸 이름을 구체화했습니다.
...
"협업 중 갈등을 어떻게 해결했나요"는 갈등 해결 능력만 보는 질문이 아닙니다. 면접관은 **이 사람이 팀에 들어왔을 때 갈등을 만들 사람인가**를 확인합니다. 그래서 답변 내용보다 **상대를 서술하는 태도**가 먼저 채점됩니다.
...
**왜 문제인가**: 사실일 수 있지만 면접관에게는 "이 사람은 팀 문제를 개인 탓으로 돌린다"로 들립니다. 게다가 해결책이 "혼자 우회했다"라서 협업이 성립하지 않습니다. 회피에 가깝습니다. 스펙이 왜 계속 바뀌었는지(요구사항 변경? 초기 합의 부재?)를 분석한 흔적도 없습니다.
...
**무엇이 달라졌나**: 원인을 사람에게서 찾다가 **정보 전달 구조**로 옮겨 갔습니다. 회피로 끝나던 해결이 절차 개선이 됐고, 마지막에 재사용 가능한 학습이 남았습니다. 같은 사건인데 협업 리스크가 협업 자산으로 뒤집혔습니다.

**갈등 경험이 정말 없다면** 억지로 만들지 말고 "심각한 갈등은 없었지만 의견이 갈린 적은 있다"로 시작해 의사결정 과정을 서술합니다. 지어낸 갈등은 세부를 물으면 바로 흔들립니다.

### 성향을 직접 묻는 형태

경험 사례를 묻지 않고 성향을 바로 묻는 질문도 자주 나옵니다. "일하면서 싫은 사람은 어떤 유형인가요", "솔직함과 무례함의 차이가 뭐라고 생각하나요", "사수가 없어도 성장할 수 있는 편인가요" 같은 것들입니다. 겉모습은 달라도 면접관은 하나만 봅니다. **이 사람이 팀에서 마찰을 어떻게 처리하나.**

- **싫은 사람 유형** — 성격이 아니라 행동으로 답합니다. "말이 많은 사람" 같은 유형 규정을 피하고 "정한 걸 공유 없이 바꾸는 경우가 힘들었습니다"처럼 상황을 말합니다. 그리고 그때 본인이 어떻게 대응했는지를 반드시 붙입니다. 유형만 말하고 끝내면 불만 토로가 됩니다.
- **솔직함과 무례함** — 기준을 하나만 제시해도 답이 됩니다. 지적 대상이 사람인지 사안인지, 상대가 바꿀 수 있는 것인지, 전달 시점과 자리가 맞는지 정도를 들면 됩니다.
- **사수 유무** — 한쪽을 단정하기보다 실제로 일하는 방식으로 답합니다. "막히면 일정 시간은 혼자 파보되, 결정 근거는 공유해서 검증받는 편입니다"처럼 말하면 됩니다. 팀 상황을 모르는 채로 "혼자서도 잘합니다"나 "사수가 꼭 필요합니다" 중 하나를 단정하면 손해입니다.
- **타 직군 협업** — 백엔드·디자이너와의 협업 경험에서는 **정보를 어떻게 맞췄는지**를 말해야 합니다. 사용한 도구 이름은 그다음입니다. 스펙을 어디에 두고 변경을 어떻게 알렸는지, 디자인 시안과 구현이 어긋났을 때 무엇을 기준으로 정리했는지를 준비해둡니다.
...
면접 전에 GitHub 링크를 열어보는 면접관이 많습니다. 그러면 README와 커밋 목록이 가장 먼저 눈에 들어옵니다. **말할 기회를 얻기 전에 이미 인상이 형성됩니다.**
...
2. **스크린샷 또는 시연 GIF** — 텍스트 열 줄보다 강합니다
3. **배포 링크** — 반드시 살아 있어야 합니다. 죽은 링크는 없느니만 못합니다
4. **실행 방법** — 명령어를 그대로 복사해 붙일 수 있게
5. **기술 스택과 선택 이유** — 한 줄씩이면 충분합니다
6. **담당 역할** — 팀 프로젝트라면 필수
7. **트러블슈팅 기록** — 있으면 그 자체가 면접 대본이 됩니다

7번이 특히 효과가 큽니다. 겪은 문제와 해결 과정을 적어두면 면접관이 그걸 읽고 질문을 만듭니다. **그 질문은 내가 준비한 영역 안에서 나옵니다.** 자기소개에서 키워드로 시험 범위를 유도하는 것과 같은 원리입니다.
...
A가 곧바로 탈락 사유가 되지는 않습니다. 다만 면접관은 여기서 "코드 리뷰 문화가 있는 팀에 들어오면 적응 비용이 얼마나 들까"를 가늠합니다. 커밋 메시지 규칙(`feat`, `fix`, `refactor` 같은 접두사)을 쓰는 것 자체는 부차적입니다. **하나의 커밋이 하나의 변경을 담고 있는가**를 봅니다. 500줄짜리 커밋 하나는 접두사가 붙어 있어도 리뷰할 수 없습니다.
...
> 표 요약: 코드는 이미 GitHub에 있습니다. 그래서 이 질문들은 하나같이 **판단할 줄 아는가**를 겨눕니다. 면접에서 확인하는 건 그 코드를 만든 사고 과정입니다.

### 특히 준비해야 할 두 가지

**"다시 만든다면 뭘 바꾸시겠어요?"** — 이 질문에 "없습니다"는 최악입니다. 회고를 안 했거나 문제를 못 본다는 뜻이 됩니다. 하나를 고르되 "그때는 왜 그렇게 했고, 지금은 왜 다르게 볼 수 있는지"까지 붙입니다.

**"사용자가 늘면 어디가 먼저 터질까요?"** — 대부분의 포트폴리오는 트래픽이 없는 상태로 만들어집니다. 그래서 이 질문의 정답은 **추론의 타당성**입니다. 실측한 숫자를 요구하는 질문이 아닙니다. "목록 조회에 페이지네이션이 없어서 데이터가 쌓이면 응답이 먼저 느려질 것 같고, 확인하려면 쿼리 실행 계획부터 보겠습니다"처럼 후보와 검증 방법을 함께 말하면 됩니다.

---

## 8. 실무에서는

- **프로젝트는 1개를 깊게 파는 편이 유리합니다.** 면접 시간은 한정돼 있고 얕은 프로젝트 3개는 얕은 질문 3번으로 끝납니다. 대표작 1개에 대해 앞의 꼬리 질문 표를 다 답할 수 있으면 그게 더 강합니다.
- **배포 링크는 면접 전날 반드시 확인합니다.** 무료 호스팅은 일정 기간 쓰지 않으면 잠들거나 만료되는 경우가 있습니다. 도메인·인증서 만료도 흔합니다. 면접관 앞에서 안 열리면 그 자체가 인상으로 남습니다.
- **화면 공유로 포트폴리오를 함께 보는 형식이 흔합니다.** 어떤 화면부터 보여줄지, 코드는 어느 파일을 열지 미리 정해두면 초반이 매끄럽습니다. 자신 있는 트러블슈팅 기록 쪽으로 유도하는 것도 방법입니다.
- **개선 과정을 기록으로 남기는 습관이 결국 가장 큰 자산입니다.** 측정 스크린샷, 트러블슈팅 메모, 결정 사유를 적어둔 이슈. 면접 준비 기간에 이걸 되살리는 건 거의 불가능하므로, 프로젝트를 진행하는 중에 남겨야 합니다.
...
- 꼬리 질문: "본인이 안 맡은 부분은 어떻게 돌아가는지 아세요?" → 전체 흐름은 설명할 수 있어야 합니다. 내 파트만 아는 사람으로 보이면 협업 측면에서 감점됩니다.

**Q. 다시 만든다면 무엇을 바꾸시겠어요?**

A. 하나를 구체적으로 고릅니다. 그리고 **당시 제약 하에서는 합리적이었지만 지금은 다르게 볼 수 있는 이유**를 설명합니다. 당시 결정이 틀렸다고 말할 필요는 없습니다. "폴더를 기능별이 아니라 타입별로 나눴는데, 화면이 늘어나면서 관련 파일이 흩어져 수정할 때마다 여러 폴더를 오갔습니다. 지금이라면 기능 단위로 묶겠습니다"처럼 경험에서 나온 근거가 있으면 됩니다.
...
포트폴리오는 내린 결정의 기록입니다. 만든 것의 목록만으로는 그 기록이 되지 않습니다. 기술은 대안-기준-트레이드오프로 방어하고, 성능은 측정-원인-조치-결과로 증명하고, 갈등은 사람에게서 원인을 떼어내 구조의 문제로 재해석해 서술해야 합니다.
