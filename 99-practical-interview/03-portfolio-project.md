...
> 면접관이 포트폴리오에서 실제로 찾는 것은 의사결정의 흔적입니다. 기술 목록만 적었을 때 무엇이 빠지는지, 기술 선택은 어떤 프레임으로 방어하는지, 성능 개선은 어떤 절차로 숫자를 정리하는지 설명할 수 있게 됩니다. README와 커밋 히스토리가 남기는 인상까지 함께 다룹니다.
...
**왜 문제인가**: "왜 이 프로젝트에서"를 물었는데 "Redux란 무엇인가"를 답했습니다. 이 답변은 Redux를 안 써본 사람도 문서만 읽으면 할 수 있습니다. 게다가 이 프로젝트에 전역 상태가 필요했는지조차 확인되지 않았습니다. 면접관은 곧바로 "Context API로는 안 됐나요?"를 던지고 여기서 준비 안 된 지원자는 말문이 막힙니다.
...
> 아래 예시에 나오는 숫자는 서술 **형식**을 보여주기 위한 가상의 값입니다. 절대 그대로 옮겨 쓰지 말고, 본인이 직접 측정한 값을 넣으세요. 측정하지 않은 숫자를 적는 것은 면접에서 가장 위험한 선택 중 하나입니다.

### 비유: 진료 기록

증상(측정) → 검사와 진단(원인) → 처방(조치) → 경과 관찰(결과). 의사가 검사 없이 처방하면 돌팔이고, 경과를 안 보면 치료가 끝났는지 알 수 없습니다. 성능 개선 서술도 골격이 같습니다.

> **비유의 한계**: 캐시를 붙이면 응답은 빨라지고 데이터 일관성은 나빠집니다. 질병에는 대체로 표준 치료가 있지만 성능 문제에 그런 정답은 없습니다. **성능 개선은 언제나 무언가를 내주고 얻는 거래**라서, 무엇을 내줬는지까지 말해야 답이 완성됩니다.
...
**왜 문제인가**: [1] 측정과 [2] 원인이 없으니 이 조치들이 필요했는지 알 수 없습니다. 특히 메모이제이션은 비교 비용이 있어서 무분별하게 붙이면 오히려 손해입니다. 이 답변은 "리렌더링 문제가 있었다고 들어서 일단 다 붙였다"로 들리기 쉽습니다. 그래서 "memo를 붙이면 항상 빨라지나요?" 같은 확인 질문이 뒤따르기 쉽습니다.
...
> 표 요약: 경험을 노트에 정리할 때는 STAR로 적어두고 말할 때는 질문에 맞는 열을 꺼내 씁니다. 오른쪽 세 열은 **면접관이 실제로 채점하는 조각, 즉 선택 이유와 측정 근거가 빠지지 않도록** 칸 이름을 구체화한 것입니다.
...
**왜 문제인가**: 사실일 수 있지만 면접관에게는 "이 사람은 팀 문제를 개인 탓으로 돌린다"로 들립니다. 게다가 해결책이 "혼자 우회했다"라서 협업이 성립하지 않습니다. 회피에 가깝습니다. 스펙이 왜 계속 바뀌었는지(요구사항 변경? 초기 합의 부재?)에 대한 분석도 없습니다.
...
- **사수 유무** — 한쪽을 단정하기보다 실제로 일하는 방식으로 답합니다. "막히면 일정 시간은 혼자 파보되, 결정 근거는 공유해서 검증받는 편입니다"처럼요. 팀 상황을 모르는 채로 "혼자서도 잘합니다"나 "사수가 꼭 필요합니다" 중 하나를 단정하면 손해입니다.
