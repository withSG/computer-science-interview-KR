...
>
> 사전에 인지하지 못한 내용이라, 구술 코딩 테스트 이후 진행했던 기술 질문에서 일정 부분 꼬이는 부분이 있었습니다. 나름 머리에 세겼다고 생각했는데, 리듀서 내부의 불변성을 유지하기 위해 얕은 복사라고 대답하였다가 얕은 복사가 아닌 깊은 복사를 한다고 말해버렸습니다.
>
> flux 패턴에 대한 질문이 첫 대면 질문에 들었던 내용인데, 그때 당시 아예 단어조차 처음 들어봐서 나중에 질문 회고를 작성할 때 추가조차 하지 못했습니다. flux 패턴은 MVC 패턴과 같은 디자인 패턴의 일종이라고 볼 수 있지만, 리액트 라이브러리를 사용하면서 생기는 문제점을 보완하기 위해 적용한 패턴입니다.
